...
begin
  -- Parentless places remain valid for the reachable legacy shelf editor. New
  -- visual-plan places are still required to provide a unit by the save RPC.
...
begin
  ...
  if nullif(trim(p_name),'') is null or p_kind not in ('unit','place') or (p_kind='place' and p_parent_id is null) then raise exception 'inventory place settings are invalid'; end if;
...
begin
  ...
  if p_action='transfer' then
    insert into public.stock_movements(item_id,location_id,typ,menge,vorher,nachher,referenz_typ,referenz_id,erfasst_von,notiz) values
      (v_item.id,p_location_id,'transfer',-p_amount,v_before,v_after,'inventory_shelf',p_place_id,p_actor_id,'Umlagerung Ausgang nach '||v_target.name),
      (v_item.id,p_location_id,'transfer',p_amount,v_before,v_after,'inventory_shelf',v_target.id,p_actor_id,'Umlagerung Eingang von '||v_source.name);
  elsif p_action='count' then
    insert into public.inventory_sessions(location_id,area_id,assigned_to,gestartet_von,typ,notiz)
    values(p_location_id,v_item.area_id,p_actor_id,p_actor_id,'geplant','QR-Zählung am Lagerplatz '||v_source.name)
    ...
    values(v_item.id,p_location_id,case p_action when 'book' then 'eingang' else 'ausgang' end,v_after-v_before,v_before,v_after,'inventory_shelf',p_place_id,p_actor_id,'Lagerplatz-Scan');
  end if;

// screens-auth.jsx — Login, Online/Waiting home, Incoming order request
// Exports to window: LoginScreen, HomeScreen, IncomingSheet, BrandMark
      ...
      <div style={{ flex: 1, padding: '4px 22px 0', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ paddingBottom: SAFE_BOTTOM + 14 }}>
          ...
          <div style={{ textAlign: 'center', marginTop: 14, fontSize: 13, color: 'var(--ink-3)', fontWeight: 500 }}>
            Hub Prenzlauer Berg · Schicht 16:00–22:00
          </div>
        </div>
      </div>
    </Screen>
...
function HomeScreen({ searching, onGoOffline }) {
  return (
    <Screen bg="var(--bg)" noTop>
      <div style={{ position: 'relative', flex: 1, overflow: 'hidden' }}>
        ...
        {/* center waiting card */}
        <div style={{ position: 'absolute', left: '50%', top: '46%', transform: 'translate(-50%,-50%)', textAlign: 'center', width: 240 }}>
          <div style={{ position: 'relative', width: 92, height: 92, margin: '0 auto 18px' }}>
            ...
            <div style={{ position: 'absolute', inset: 0, borderRadius: '50%', background: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 24px -8px rgba(0,0,0,.3)' }}>
              <Icon name="bag" size={40} stroke={1.8} style={{ color: 'var(--accent)' }} />
            </div>
          </div>
          ...
          <div style={{ fontSize: 14, color: 'var(--ink-2)', marginTop: 5, fontWeight: 500, lineHeight: 1.4 }}>
            Bleib in der Nähe vom Hub. Neue Aufträge kommen automatisch rein.
          </div>
        </div>
      </div>
...
// Incoming batch request sheet with countdown
function IncomingSheet({ orders, onAccept, onDecline }) {
  const TOTAL = 15;
  ...
  const dist = orders.reduce((s, o) => s + o.distanceKm, 0);
    ...
    <Sheet dismissable={false}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 13, marginBottom: 16 }}>
        <div style={{ width: 46, height: 46, borderRadius: 14, background: 'var(--accent-tint)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Icon name="bag" size={24} stroke={2} style={{ color: 'var(--accent)' }} className="ring-anim" />
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 20, fontWeight: 800, letterSpacing: '-0.02em' }}>Neue Tour</div>
          <div style={{ fontSize: 14, color: 'var(--ink-2)', fontWeight: 600 }}>{orders.length} Bestellungen · {DRIVER.hub}</div>
        </div>
        {/* countdown ring */}
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
        {[['box', items, 'Artikel'], ['route', dist.toFixed(1) + ' km', 'Strecke'], ['clock', '~38 min', 'Dauer']].map(([ic, val, lab], i) => (
          <div key={i} style={{ flex: 1, background: 'var(--surface-2)', borderRadius: 14, padding: '12px 10px', textAlign: 'center' }}>
            ...
            <div style={{ fontWeight: 800, fontSize: 17, marginTop: 3 }}>{val}</div>
      ...
      {/* stop list preview */}
      <div className="scroll" style={{ background: 'var(--surface-2)', borderRadius: 16, padding: 4, marginBottom: 16, maxHeight: 168 }}>
        {orders.map((o, i) => (
          <div key={o.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 12px', borderBottom: i < orders.length - 1 ? '1px solid var(--line-2)' : 'none' }}>
            <div className="mono" style={{ width: 26, height: 26, borderRadius: 8, background: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13, boxShadow: 'inset 0 0 0 1px var(--line)' }}>{i + 1}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 700, fontSize: 14.5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{o.shortAddr}</div>
              <div style={{ fontSize: 12.5, color: 'var(--ink-3)', fontWeight: 600 }}>{o.items.length} Positionen · {o.distanceKm} km</div>
            </div>
            <span className="mono" style={{ fontSize: 12.5, color: 'var(--ink-3)', fontWeight: 700, whiteSpace: 'nowrap' }}>#{o.code}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <Btn onClick={onAccept} iconRight="arrow" style={{ flex: 1 }}>Annehmen</Btn>
      </div>
    </Sheet>
  );
}

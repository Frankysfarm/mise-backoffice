...
## Runtime shadow/oracle and parallel HTTP chaos — 2026-08-04
...
- The focused comparison covers 2/4 drivers, Bike/Car, staggered readiness, endangered orders, incumbent route/load, capacities, two stores, traffic-matrix versions and bundles 1–4. Assignment and exact stop-sequence drift fail closed. Oracle/property aggregate: 20/20 pass including 500 deterministic seeds.
...
- The real local HTTP lifecycle adds two parallel GoTrue-authenticated Next Driver acknowledgements with one action envelope. Both return 200, exactly one is fresh and one an idempotent replay, with one action-registry row and one compatibility event. A distinct stale out-of-order assignment version returns 409, creates no registry/event write and leaves the already terminal batch push count unchanged. Full guarded lifecycle: 18/18 pass; P0 TypeScript and `git diff --check` pass.

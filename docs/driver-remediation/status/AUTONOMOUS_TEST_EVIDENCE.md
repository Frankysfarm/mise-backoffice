...
- The same real Driver HTTP boundary rejects reuse of that action ID with changed occurrence data as `IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST`, and rejects a fresh action carrying stale assignment version zero as `EXPECTED_ASSIGNMENT_VERSION_CONFLICT`. Both return 409; the stale action creates no registry row and neither case adds a compatibility event. Full lifecycle/recovery/boundary result: exit 0, 16/16.
- PostgREST is explicitly stopped before a new authenticated Driver acknowledgement. This exposed and corrected an API classification defect: infrastructure failures previously surfaced as a non-retryable-looking business 409. The production handler now returns `503 DRIVER_V2_SERVICE_UNAVAILABLE`, with no registry write. After the same container restarts and becomes healthy, the identical request succeeds with exactly one registry row and one compatibility event. The later hard Next restart also revalidates Storefront replay plus the existing Driver and Admin sessions. Full lifecycle result: exit 0, 17/17.

## Aggregate local readiness rerun — 2026-08-04

- `scripts/tests/run-t10-local-release-readiness.sh`: exit 0. All 14 disposable-PostgreSQL suites pass, followed by adaptive optimizer, Driver UI, web-push/recovery, Storefront and Dispatcher boundary contracts.
- Added canonical `npm run test:lab:full:local`; its database comes only from `with-local-remediation-postgres.sh`, and all external providers remain sink/fixture. A naked `test:lab:full` still fails closed without explicit laboratory environment.
- The first guarded aggregate exposed that the separately orchestrated HTTP lifecycle file aborted at discovery when Next/GoTrue/PostgREST were intentionally absent. Those cases now explicitly skip outside their orchestrator. Aggregate rerun: exit 0, 121 discovered, 97 pass, 24 explicit opt-in skips, 0 failures. The skips are 17 HTTP lifecycle, 6 browser and 1 soak case and are not claimed as aggregate executions.
- The canonical HTTP orchestrator was then rerun separately: `npm run test:lab:lifecycle:http-db`, exit 0, 17/17, 0 skips. P0 TypeScript and `git diff --check` pass.

## Reproducible local Chromium actor gate — 2026-08-04

- Added canonical `npm run test:lab:ui:local`. It obtains a disposable PostgreSQL URL from the existing guarded wrapper, starts only a loopback Next server and keeps all provider boundaries in sink/fixture mode.
- Heavy Next development routes are compiled sequentially before Chromium starts. Browser test files run with concurrency one. The wrapper removes only generated webpack cache state at startup and waits for the local server during cleanup, preventing interrupted pack files from contaminating later gate results.
- The Driver actor now waits for the production native-offer listener readiness boundary before injecting the canonical offer. The production Accept flow preserves the confirmed assignment version and uses `router.refresh()` rather than racing a hard document reload against the native bridge.
- Result: exit 0, **12/12 pass, 0 fail, 0 skip**. Covered actors are the guarded dashboard, strict production Dispatcher, synthetic Driver lifecycle, production Driver Accept, production Kitchen and production BISS Storefront, plus six UI contract cases. Run ID `tl_20260804t073000z_18b18b18`, seed 18; traces/screenshots are under `artifacts/driver-system-lab/browser-actors/full-local/`.
...
- The focused comparison captures 2/4 drivers, Bike/Car, staggered readiness, endangered metadata, incumbent route/load, capacities, two stores, traffic-matrix metadata and bundles 1–4. Capacity and bundle size are decision inputs; readiness/incumbent stops are validated constraints, while endangered/matrix values are capture-only here. Independently declared assignment and exact stop-sequence drift fail closed. Oracle/property aggregate: 20/20 pass including 500 deterministic seeds.
- Scope remains honest: active Frank runtime does not invoke the global adaptive optimizer, so TL-G4 remains PARTIAL until a reviewed default-off Frank shadow path loads a real DB snapshot and reads actual persisted assignment/stop rows.
- The ordered real local HTTP lifecycle adds two concurrent GoTrue-authenticated Driver acknowledgements through one Next process with one action envelope. Both return 200, exactly one is fresh and one an idempotent replay, with one action-registry row and one compatibility event. A distinct stale assignment version returns 409 and preserves a canonical fingerprint across assignment, driver, batch, order, stops, action/event registries and target-batch outbox. This is not independent queue-worker or terminal-state push-worker chaos. Full guarded lifecycle: 18/18 pass; P0 TypeScript and `git diff --check` pass.

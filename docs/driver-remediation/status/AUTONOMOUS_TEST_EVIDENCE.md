...
- The endpoint validates a bounded unique UUID order set, authenticated tenant context and an explicit dispatcher/admin role. It fails closed unless the tenant elected `atomic_v2`, the manual writer identity is configured, a writer lease is obtained, driver/order versions and tenant/location/routing data are present, and the canonical atomic assignment RPC succeeds.

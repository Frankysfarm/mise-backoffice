...
- The full `DispatchBoard` itself emits pre-existing server/client time-text hydration mismatches and many optional-widget fetch failures when those hundreds of unrelated APIs are deliberately blocked. The focused test classifies and retains these rather than treating this bounded mutation result as a clean whole-dashboard approval. Overall Dispatcher TL-G2 remains PARTIAL pending hydration cleanup and HTTP→disposable-DB persistence evidence.
...
- Adversarial review found three other rendered Legacy assignment/reassignment controls, including mock-success and cross-tenant/non-atomic service-role paths. Those controls are removed from `DispatchBoard`; `/batch-assign`, `/auto-zuweisung` and `/batch-reassign` are now mutation-free HTTP-410 tombstones pending canonical replacements. The browser test no longer suppresses arbitrary exceptions: it records the known initial whole-board hydration defect, clears that baseline, and requires the bounded assignment interaction itself to add zero browser exceptions.
...
- Independent adversarial re-review of frozen `48446b82` APPROVES the bounded Dispatcher manual-assignment subgate with no remaining P0/P1. A clean production build completes 454/454 pages. Full Dispatcher TL-G2/TL-G5 remain PARTIAL for the explicit hydration defect, intentionally disabled legacy operator features and missing canonical HTTP→disposable-DB lifecycle.

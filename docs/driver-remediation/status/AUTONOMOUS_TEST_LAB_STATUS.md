...
| TL-G2 UI actors | PARTIAL — local canonical DB lifecycle GREEN | Real Chromium covers production Storefront, Kitchen, Driver and the pruned production Dispatcher core. Unmocked Next/PostgREST/PostgreSQL now links Storefront→Kitchen-ready→Atomic-v2 dispatch→Driver pickup/depart/drop-off→terminal delivery with exact-once assignment/push. GoTrue-backed route authentication, Realtime and canonical replacements for disabled legacy controls remain open. |

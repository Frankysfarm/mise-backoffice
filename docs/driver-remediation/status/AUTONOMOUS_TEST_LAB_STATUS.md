...
| TL-G2 UI actors | PARTIAL | Real Chromium covers production Storefront, Kitchen and Driver plus the bounded production Dispatcher manual-assignment control through failed retry and success. The strict full Dispatcher startup gate remains RED and no longer discards initial page/console errors; HTTP-to-DB lifecycle evidence and canonical replacements for disabled legacy controls remain open. |

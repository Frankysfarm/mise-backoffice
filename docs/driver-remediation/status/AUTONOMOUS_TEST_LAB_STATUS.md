...
| TL-G4 Dispatch oracle | PARTIAL — runtime-shadow transformation/readback GREEN | A default-off read-only shadow seam compares runtime-shaped inputs, independently declared persisted assignments and exact stop sequences against the adaptive optimizer/oracle across 2/4 drivers, Bike/Car, readiness, route load, capacity, stores, traffic matrices and bundles 1–4. The active Frank runtime still uses per-order deterministic dispatch; real DB snapshot/readback wiring remains open. |
| TL-G5 Functional catalog | PARTIAL | 100+ named categories/cases are catalogued; they are not all executable E2E scenarios yet. |
| TL-G6 Race/chaos/recovery | PARTIAL | Real disposable DB transaction abort, killed-client rollback, PostgREST/GoTrue/Next restart, committed-response loss and pre-commit Driver network outage/recovery pass. Two parallel authenticated Next Driver events deduplicate to one write/replay; a stale out-of-order event remains write-free and cannot add a terminal-batch push. Realtime, service-worker, independent queue workers and provider crash paths remain open. |

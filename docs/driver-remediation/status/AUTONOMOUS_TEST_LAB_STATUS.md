...
| TL-G4 Dispatch oracle | PARTIAL — runtime-shadow format/comparison GREEN | A default-off read-only shadow seam compares runtime-shaped captures and independently declared persisted assignments/exact stop sequences against the adaptive optimizer/oracle. Capacity and bundles 1–4 are decision inputs; readiness/route prefix are validated, while endangered/matrix values are capture-only. The active Frank runtime still uses per-order deterministic dispatch; real DB snapshot/readback wiring remains open. |

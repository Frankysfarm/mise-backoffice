...
| TL-G2 UI actors | PARTIAL — local canonical DB + Driver/Admin auth lifecycle GREEN | Real Chromium covers production Storefront, Kitchen, Driver and the pruned production Dispatcher core. Unmocked Next/GoTrue/PostgREST/PostgreSQL links Storefront→Kitchen-ready→Atomic-v2 dispatch→Driver pickup/depart/drop-off→terminal delivery with exact-once assignment/push. GoTrue-issued Driver bearer and Admin SSR-cookie sessions reach their production API boundaries; invalid Driver JWT fails closed and the Admin drivers response excludes a second tenant's driver. Realtime and canonical replacements for disabled legacy controls remain open. |

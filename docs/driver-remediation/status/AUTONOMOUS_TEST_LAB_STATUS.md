...
| TL-G2 UI actors | PARTIAL — local canonical DB + Driver auth lifecycle GREEN | Real Chromium covers production Storefront, Kitchen, Driver and the pruned production Dispatcher core. Unmocked Next/GoTrue/PostgREST/PostgreSQL now links Storefront→Kitchen-ready→Atomic-v2 dispatch→Driver pickup/depart/drop-off→terminal delivery with exact-once assignment/push, and a GoTrue-issued JWT reaches the canonical Driver-v2 snapshot while an invalid JWT fails closed. Admin-cookie actor authentication, Realtime and canonical replacements for disabled legacy controls remain open. |

...
| TL-G2 UI actors | PARTIAL — local canonical DB + authenticated recovery lifecycle GREEN | Real Chromium covers production Storefront, Kitchen, Driver and the pruned production Dispatcher core. Unmocked Next/GoTrue/PostgREST/PostgreSQL links the terminal lifecycle with exact-once assignment/push. Driver bearer and tenant-isolated Admin SSR-cookie sessions survive a GoTrue restart; a lost committed Driver-ACK response replays idempotently with one registry/event write. Realtime and canonical replacements for disabled legacy controls remain open. |

...
| TL-G2 UI actors | PARTIAL | Real Chromium covers production Storefront, Kitchen and Driver plus the bounded production Dispatcher manual-assignment control through failed retry and success. The full Dispatcher dashboard still has hydration/optional-widget failures, and HTTP-to-DB full-stack lifecycle evidence remains open. |

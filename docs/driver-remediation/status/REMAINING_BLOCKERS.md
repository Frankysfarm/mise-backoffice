...
- Aggregate readiness is locally green: T10 exits 0; `test:lab:full:local` exits 0 with 97 pass, 24 explicit opt-in skips and zero failures. The 17 lifecycle cases separately pass 17/17 and all six browser cases separately pass 12/12 (including six UI contracts) through their guarded local orchestrators. Only the long soak opt-in remains unexecuted from that aggregate skip set.

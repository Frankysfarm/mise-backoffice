...
- TL-G2: Chromium/trace are green for production Storefront, Kitchen and Driver plus the production Dispatcher core. The strict Dispatcher startup subgate is GREEN after removing 744 overlapping historical/demo widgets from the boot rendergraph and 662 now-unused static imports; three repeated headless runs and one headed run have zero page/console/hydration errors. Real Storefront HTTP→Next→PostgREST→disposable-DB creation and idempotent replay are also green twice. Overall TL-G2 remains PARTIAL because Kitchen-ready→routing→dispatch→Driver→terminal lifecycle linkage, real actor RLS identities and final invariant evidence are open.

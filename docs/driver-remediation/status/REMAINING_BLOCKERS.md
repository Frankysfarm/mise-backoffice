...
- TL-G2: Chromium/trace are green for production Storefront, Kitchen and Driver plus the production Dispatcher core. The strict Dispatcher startup subgate is GREEN after removing 744 overlapping historical/demo widgets from the boot rendergraph and 662 now-unused static imports. Real Storefront HTTP→Next→PostgREST→disposable-DB creation/replay and token-bound atomic Kitchen-ready transitions (including concurrent final items and cross-station rejection) are green. Overall TL-G2 remains PARTIAL because routing→dispatch→Driver→terminal lifecycle linkage, real actor RLS identities and final invariant evidence are open.

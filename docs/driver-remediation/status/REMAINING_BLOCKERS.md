...
- TL-G2: Chromium/trace are green for production Storefront, Kitchen and Driver plus the production Dispatcher core. Real Storefront→Kitchen-ready→Atomic-v2 dispatch→Driver→terminal linkage through Next/GoTrue/PostgREST/disposable PostgreSQL is green; Redispatch, invariants, parallel writers, committed and pre-commit Driver-HTTP response/network loss, changed-idempotency/stale-version denial, PostgREST/Next/GoTrue restart, browser-role write denial and both authenticated roles are covered. Overall TL-G2 remains PARTIAL because Realtime, real runtime routing-provider capture and broader worker chaos remain open.
- Aggregate readiness is locally green: T10 exits 0; `test:lab:full:local` exits 0 with 97 pass, 24 explicit opt-in skips and zero failures. The expanded lifecycle separately passes 18/18, all six browser cases separately pass within three deterministic 12/12 UI runs, and the bounded 2,000-iteration model soak passes separately. Thus every opt-in skip has a guarded separate execution; a genuinely multi-hour process/resource soak remains open.
...
- TL-G4: the default-off shadow covers diverse multi-driver/store inputs and exact declared assignment/stop readback, but it is not yet wired to an actual Frank database snapshot and persisted runtime result. That reviewed read-only wiring and retained complete optimality-gap traces remain open.

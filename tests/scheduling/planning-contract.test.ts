...
describe('Dienstplan Vollausbau Verträge', () => {
  ...
  const corrections = source('supabase/migrations/20260831160000_vollausbau_review_corrections.sql');
  ...
  it('berücksichtigt aktive, eingearbeitete und Probe-Mitarbeiter', () => { expect(corrections).toContain("e.status::text in ('aktiv','in_training','in_probe')"); });
  it('begrenzt Empfänger und klassifiziert echte Absagen', () => { expect(corrections).toContain('<p>Deine Schichten'); expect(corrections).toContain("new.status::text in ('abgesagt','storniert')"); });

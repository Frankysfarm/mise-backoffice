...
app_port="3210"
lock_dir=.next-testlab-aachen-shift.lock
lock_owned=false
...
MISE_TEST_LAB_GOTRUE_CONTAINER="$auth_container" \
MISE_TEST_LAB_NEXT_PID="$app_pid" \
NEXT_PUBLIC_SUPABASE_URL="http://127.0.0.1:${postgrest_port}" \
NEXT_PUBLIC_SUPABASE_ANON_KEY="$service_key" \
SUPABASE_SERVICE_ROLE_KEY="$service_key" \
node --import tsx --test tests/driver-system-lab/lifecycle/aachen-shift.test.ts

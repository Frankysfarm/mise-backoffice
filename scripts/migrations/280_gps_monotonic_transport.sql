...
BEGIN
  ...
  v_fingerprint := md5(concat_ws('|','upload_gps',p_action_id,p_tenant_id,p_driver_id,
    p_expected_driver_version,p_session_id,p_sequence,p_captured_at,p_latitude,
    p_longitude,p_accuracy_m,p_speed_mps,p_heading_deg,p_app_version,p_app_build,
    p_platform,p_app_state,p_permission_state,p_network_state,
    coalesce(p_capability_flags,'{}'::jsonb)::text));
  ...
  IF p_sequence < 0 OR p_latitude NOT BETWEEN -90 AND 90 OR p_longitude NOT BETWEEN -180 AND 180
    ...
    OR p_app_state NOT IN ('foreground','background','locked','unknown') OR length(p_app_version) NOT BETWEEN 1 AND 64
  ...
  INSERT INTO public.mise_driver_locations(driver_id,lat,lng,recorded_at,tracking_session_id,sequence,captured_at,received_at,accuracy_m,speed_mps,heading_deg,app_version,app_build,platform,app_state,permission_state,network_state,capability_flags,quality_flags,correlation_id,ingest_outcome,action_id,event_fingerprint)
    VALUES(p_driver_id,p_latitude,p_longitude,v_now,p_session_id,p_sequence,p_captured_at,v_now,p_accuracy_m,p_speed_mps,p_heading_deg,p_app_version,p_app_build,p_platform,p_app_state,p_permission_state,p_network_state,coalesce(p_capability_flags,'{}'),v_flags,p_correlation_id,v_outcome,p_action_id,v_fingerprint);
  IF v_outcome='monotonic_current_advance' THEN
    ...
    INSERT INTO public.mise_driver_position_current(driver_id,tenant_id,session_id,sequence,captured_at,received_at,latitude,longitude,accuracy_m,speed_mps,heading_deg,app_version,app_build,platform,app_state,permission_state,network_state,capability_flags,quality_flags,operational_state,position_version,correlation_id)
      VALUES(p_driver_id,p_tenant_id,p_session_id,p_sequence,p_captured_at,v_now,p_latitude,p_longitude,p_accuracy_m,p_speed_mps,p_heading_deg,p_app_version,p_app_build,p_platform,p_app_state,p_permission_state,p_network_state,coalesce(p_capability_flags,'{}'),v_flags,v_driver.state,1,p_correlation_id)
    ON CONFLICT(driver_id) DO UPDATE SET session_id=excluded.session_id,sequence=excluded.sequence,captured_at=excluded.captured_at,received_at=excluded.received_at,latitude=excluded.latitude,longitude=excluded.longitude,accuracy_m=excluded.accuracy_m,speed_mps=excluded.speed_mps,heading_deg=excluded.heading_deg,app_version=excluded.app_version,app_build=excluded.app_build,platform=excluded.platform,app_state=excluded.app_state,permission_state=excluded.permission_state,network_state=excluded.network_state,capability_flags=excluded.capability_flags,quality_flags=excluded.quality_flags,operational_state=excluded.operational_state,position_version=mise_driver_position_current.position_version+1,correlation_id=excluded.correlation_id;
  END IF;
...
REVOKE ALL ON FUNCTION public.fn_ingest_driver_gps_v2(uuid,uuid,uuid,uuid,bigint,timestamptz,double precision,double precision,double precision,double precision,double precision,text,text,text,text,text,text,jsonb,bigint,uuid) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.fn_ingest_driver_gps_v2(uuid,uuid,uuid,uuid,bigint,timestamptz,double precision,double precision,double precision,double precision,double precision,text,text,text,text,text,text,jsonb,bigint,uuid) TO service_role,postgres;

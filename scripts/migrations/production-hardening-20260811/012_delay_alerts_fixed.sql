-- Production-fixed Migration 023: Proactive Delay Alert System
...
WHERE
  co.status NOT IN ('geliefert', 'abgeschlossen', 'storniert', 'cancelled', 'abgeholt')
...
ALTER TABLE delivery_delay_alerts ENABLE ROW LEVEL SECURITY;
ALTER TABLE delay_compensation_vouchers ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'delivery_delay_alerts' AND policyname = 'service_role_all_delay_alerts'
  ) THEN
    CREATE POLICY service_role_all_delay_alerts ON delivery_delay_alerts
      FOR ALL TO service_role USING (true) WITH CHECK (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'delay_compensation_vouchers' AND policyname = 'service_role_all_delay_vouchers'
  ) THEN
    CREATE POLICY service_role_all_delay_vouchers ON delay_compensation_vouchers
      FOR ALL TO service_role USING (true) WITH CHECK (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'delivery_delay_alerts' AND policyname = 'auth_read_own_delay_alerts'
  ) THEN
    CREATE POLICY auth_read_own_delay_alerts ON delivery_delay_alerts
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'delay_compensation_vouchers' AND policyname = 'auth_read_own_delay_vouchers'
  ) THEN
    CREATE POLICY auth_read_own_delay_vouchers ON delay_compensation_vouchers
      FOR SELECT TO authenticated
      USING (location_id IN (
        SELECT location_id FROM employees WHERE auth_user_id = auth.uid()
      ));

-- Production-fixed Phase 25: Webhook System + External Integration Engine
...
ALTER TABLE delivery_webhook_deliveries ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'delivery_webhooks' AND policyname = 'service_role_all_webhooks'
  ) THEN
    CREATE POLICY service_role_all_webhooks ON delivery_webhooks
      FOR ALL TO service_role USING (true) WITH CHECK (true);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'delivery_webhooks' AND policyname = 'auth_manage_own_webhooks'
  ) THEN
    CREATE POLICY auth_manage_own_webhooks ON delivery_webhooks
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'delivery_webhook_deliveries' AND policyname = 'service_role_all_webhook_deliveries'
  ) THEN
    CREATE POLICY service_role_all_webhook_deliveries ON delivery_webhook_deliveries
      FOR ALL TO service_role USING (true) WITH CHECK (true);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'delivery_webhook_deliveries' AND policyname = 'auth_read_own_webhook_deliveries'
  ) THEN
    CREATE POLICY auth_read_own_webhook_deliveries ON delivery_webhook_deliveries

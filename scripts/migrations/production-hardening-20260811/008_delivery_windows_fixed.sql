-- Production-fixed Migration 033: Delivery Time Window Booking
...
CROSS JOIN (
  SELECT (current_date + day_offset)::date AS booking_date
...
-- nur passende Wochentage (0=Montag, ISO-Wochentag minus 1)
WHERE s.day_of_week = EXTRACT(ISODOW FROM d.booking_date)::int - 1
...
WHERE wb.status = 'pending'
  ...
  AND co.status NOT IN ('storniert', 'geliefert', 'abgeholt');

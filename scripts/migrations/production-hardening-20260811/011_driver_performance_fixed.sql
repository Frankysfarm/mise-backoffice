-- Production-fixed Migration 016: Driver Auto-Rating + SLA Tracking
--
-- Zweck:
...
ALTER TABLE mise_drivers
  ADD COLUMN IF NOT EXISTS avg_delivery_min integer DEFAULT 25;
...
BEGIN
  ...
  SELECT b.driver_id, COALESCE(b.picked_up_at, b.accepted_at, b.created_at)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'delivery_performance' AND policyname = 'service_role_all_delivery_performance'
  ) THEN
    CREATE POLICY service_role_all_delivery_performance ON delivery_performance
      FOR ALL TO service_role USING (true) WITH CHECK (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'delivery_performance' AND policyname = 'auth_read_own_delivery_performance'
  ) THEN
    CREATE POLICY auth_read_own_delivery_performance ON delivery_performance

-- Production-fixed Phase 38: Surge Pricing + Driver Incentive Engine
  ...
  SELECT
    ...
    COUNT(*) FILTER (
      WHERE mise_batch_id IS NULL
        AND typ = 'lieferung'
        AND status NOT IN ('storniert', 'abgeholt', 'geliefert', 'abgeschlossen', 'cancelled')
    ) AS queue_depth,
    COUNT(*) FILTER (
...
LEFT JOIN (
  SELECT
    ...
    l.id AS location_id,
    CASE WHEN COUNT(md.id) = 0 THEN 0
         ELSE ROUND(
           100.0 * COUNT(md.id) FILTER (WHERE md.state NOT IN ('idle', 'offline')) / COUNT(md.id)
         )
    END AS busy_pct
  FROM locations l
  ...
  LEFT JOIN mise_drivers md
    ON md.auth_user_id = e.auth_user_id
   AND md.active = true
  GROUP BY l.id
...
SELECT
  ...
  COALESCE(md.name, 'Unbekannt') AS driver_name,
  md.vehicle
...
GROUP BY dsb.driver_id, dsb.location_id, md.name, md.vehicle
;

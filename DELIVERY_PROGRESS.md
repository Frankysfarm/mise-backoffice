...
## Phase 664–668 — Dringlichkeits-API, Queue, Rückkehr-Prognose, Einnahmen-Prognose, Bestell-Ampel (DONE ✅)

**Datum:** 2026-07-08

### Phase 664 Backend — Bestellungs-Dringlichkeits-API
**`app/api/delivery/admin/bestellungs-dringlichkeit/route.ts`:**
- GET `?location_id=...` → `{ ok, bestellungen: DringlichkeitsBestellung[], summary, generatedAt }`
- Score 0–100: Zeit-Druck (0–50) + Status-Faktor (0–30) + SLA-Abstand (0–20)
- Dringlichkeit: niedrig/mittel/hoch/kritisch
- SLA-Grenze: 45 Min ab Bestelleingang
- Empfehlung je Bestellung: priorisieren, Küche informieren, Fahrer beauftragen
- Sortiert nach Dringlichkeits-Score absteigend
- Multi-Tenant: location_id-Filter ✅

### Phase 665 Kitchen — Dringlichkeits-Queue
**`app/(admin)/kitchen/phase665-dringlichkeits-queue.tsx`** — `KitchenPhase665DringlichkeitsQueue`:
- Props: `locationId: string | null`
- Ruft `/api/delivery/admin/bestellungs-dringlichkeit` ab (Phase 664)
- Zeigt Top-8 Bestellungen als farbige Karten (rot/orange/amber/slate)
- Header-Chips: Anzahl kritische + hohe Bestellungen
- Collapsible via Header-Button
- 30s Auto-Polling
- Integration: `kitchen/client.tsx` nach KitchenPhase660PrepRueckstandAlert ✅

### Phase 666 Backend + Dispatch — Tour-Rückkehr-Prognose
**`app/api/delivery/admin/batch-rueckkehr-prognose/route.ts`:**
- GET `?location_id=...` → `{ ok, batches: BatchPrognose[], aktive_touren, generatedAt }`
- Berechnung: verbleibende Stopps × Ø Min/Stopp (aus bisheriger Tourengeschwindigkeit)
- Status: unterwegs / fast_fertig / zurueck
- Default: 7 Min/Stopp + 5 Min Rückkehrpuffer

**`app/(admin)/dispatch/phase666-batch-rueckkehr-prognose.tsx`** — `DispatchPhase666BatchRueckkehrPrognose`:
- Props: `locationId: string | null`
- Zeigt alle aktiven Touren mit Fahrername, Stopp-Fortschritt, Rückkehr-ETA
- Fortschrittsbalken grün/amber je Tourstand
- 30s Auto-Polling, Collapsible
- Integration: `dispatch/client.tsx` nach DispatchPhase661ZoneEffizienzRangliste ✅

### Phase 667 Backend + Fahrer-App — Tages-Einnahmen-Prognose
**`app/api/delivery/admin/fahrer-einnahmen-tag/route.ts`:**
- GET `?location_id=...&driver_id=...` → `{ ok, einnahmen_bisher, trinkgeld_bisher, schicht_start, schicht_dauer_min, touren_bisher }`
- Liest aktive Schicht des Fahrers + heutige abgeschlossene Touren
- Multi-Tenant: location_id-Filter ✅

**`app/fahrer/app/phase667-tages-einnahmen-prognose.tsx`** — `FahrerPhase667TagesEinnahmenPrognose`:
- Props: `driverId: string`, `locationId: string`
- Hochrechnung auf 8h-Schicht basierend auf €/Min-Tempo
- 2-Spalten-Grid: Bisher / Prognose
- Schicht-Fortschrittsbalken (0–8h)
- Trend-Farbkodierung: grün (≥0.50€/Min), amber, rot
- 2min Auto-Polling
- Integration: `fahrer/app/client.tsx` nach FahrerPhase662TourpauseEmpfehlungPro ✅

### Phase 668 Storefront — Bestell-Status-Ampel
**`app/order/[locationSlug]/phase668-bestell-status-ampel.tsx`** — `Phase668BestellStatusAmpel`:
- Props: `locationId: string`
- Nutzt `/api/delivery/admin/kuechen-kapazitaets-warnsignal` (Phase 610)
- Grün: Küche frei / Gelb: mäßig ausgelastet / Rot: stark ausgelastet
- Animierter Pulse-Dot + Live-Badge
- Zeigt prognoseWarteMin wenn vorhanden
- 60s Auto-Polling
- Integration: `storefront.tsx` nach Phase663KuechenVertrauenBadge ✅

### Integrations-Checkliste Phase 664–668
| Komponente | Datei | Integration | Status |
|---|---|---|---|
| Bestellungs-Dringlichkeits-API | api/delivery/admin/bestellungs-dringlichkeit/route.ts | Neu (GET) | ✅ |
| Batch-Rückkehr-Prognose-API | api/delivery/admin/batch-rueckkehr-prognose/route.ts | Neu (GET) | ✅ |
| Fahrer-Einnahmen-Tag-API | api/delivery/admin/fahrer-einnahmen-tag/route.ts | Neu (GET) | ✅ |
| KitchenPhase665DringlichkeitsQueue | kitchen/phase665-dringlichkeits-queue.tsx | kitchen/client.tsx nach Phase660 | ✅ |
| DispatchPhase666BatchRueckkehrPrognose | dispatch/phase666-batch-rueckkehr-prognose.tsx | dispatch/client.tsx nach Phase661 | ✅ |
| FahrerPhase667TagesEinnahmenPrognose | fahrer/app/phase667-tages-einnahmen-prognose.tsx | fahrer/app/client.tsx nach Phase662 | ✅ |
| Phase668BestellStatusAmpel | order/[locationSlug]/phase668-bestell-status-ampel.tsx | storefront.tsx nach Phase663 | ✅ |

**Build:** 373 Seiten, Exit 0 ✅

### Nächste Phasen
1. **Phase 669 Backend:** Fahrer-Touren-Effizienz-Vergleichs-API — Vergleich aller Fahrer nach Lieferungen/h und km/Tour
2. **Phase 670 Kitchen:** Küchen-Schicht-Ende-Prognose — Wann ist die Küche heute fertig basierend auf offenen Bestellungen?
3. **Phase 671 Dispatch:** Fahrer-Verfügbarkeits-Ampel — Kompakter Überblick welche Fahrer wann wieder frei sind
4. **Phase 672 Fahrer-App:** Tour-Qualitäts-Score — Bewertung der aktuellen Tour (Pünktlichkeit, km-Effizienz, Kundenfeedback)
5. **Phase 673 Storefront:** Zonen-Lieferzeit-Differenzierung — Zeigt unterschiedliche ETAs je nach Lieferzone (A=25min, B=35min usw.)
...
**Phase 459+460 Backend+Frontend (2026-06-23): DispatchEchtzeitKommandoZentrale + StorefrontLiveWartezeitRing — app/(admin)/dispatch/echtzeit-kommando-zentrale.tsx: 5-Kachel Hero-Section (Aktive Touren, Offene Bestellungen, Fahrer online/gesamt, Ø Score, Bestellungen heute), Auslastungs-Fortschrittsbalken grün/amber/rot, 60s-Polling /api/delivery/admin/driver-score?action=live, Matcha-Header mit Puls-Dot, Props-Sync mit Dispatch-State, Integration dispatch/client.tsx ganz oben. driver-score API: neuer action=live → DriverScore[] mit sub_scores (punctuality/completion/customer_rating/efficiency). fahrer-score-performance-hub.tsx: fetch auf ?action=live umgestellt (kein Mock). app/order/[locationSlug]/live-wartezeit-ring.tsx (StorefrontLiveWartezeitRing): SVG-Kreisring orderId-basiert mit 1s-Countdown, 60s-API-Polling /api/delivery/eta, Farbe matcha→amber→rot, Fehler-Fallback auf Prop-Wert. storefront.tsx: beide Ringe (orderedAt-basiert + orderId-basiert) ergänzen sich. CEO Review #253: 1 Bug gefixt (Recharts-Formatter-Typ tages-kpi-cockpit.tsx:207), Build Exit 0, TypeScript Exit 0, alle Komponenten integriert.**

**Phase 458 Frontend (2026-06-23): 5 neue Smart-Delivery-Komponenten — KitchenSmartFarbkodierungCockpit (Echtzeit Farbkodierung aller aktiven Bestellungen grün/amber/rot/grau nach ETA-Restzeit, 1s-Countdown, Urgency-Sort, Supabase Realtime, Integration kitchen/client.tsx:1841); DispatchFahrerScorePerformanceHub (Live-Score-Übersicht aller Fahrer mit 4 Sub-Scores, Aktiv-Tour-Info, Supabase Realtime + 30s-Refresh, Integration dispatch/client.tsx:1947); TourStoppFokusHub (Fahrer-App Hero-Kachel für aktuellen Stop mit ETA-Countdown, Google-Maps-Deeplink, Zugestellt-Button + Stop-Strip, Integration fahrer/app/client.tsx:1096 nur bei unterwegs+pendingStops>0); EtaDynamicLivePanel (Dynamische ETA-Anzeige nach Bestellung mit 5-Phasen-Ampel, Konfidenz-Badge, Fahrer-Name, Live-Polling, Integration storefront.tsx:455 nur bei orderSuccess.type=lieferung); LieferdienstTagesKpiCockpit (Tages-KPI-Dashboard: Umsatz/Bestellungen/Storno-Rate/AvgWert, HourlyBarChart via Recharts, Lieferung-vs-Abholung-Split, Top-3-Zonen, 60s-Refresh, Integration lieferdienst/client.tsx:1465). CEO Review #252: Build Exit 0, TypeScript Exit 0, alle 5 Komponenten integriert, 0 Bugs.**

**Phase 456+457 Backend+Frontend (2026-06-23): Zonen-Live-Heatmap + Fahrer-Schicht-Status-Streifen — app/api/delivery/admin/zone-live-heatmap/route.ts: GET ?location_id → ZoneLiveRow[] — lädt delivery_zones (A/B/C/D Konfiguration), aktive mise_delivery_batches (state=on_route/at_restaurant/assigned/pending_acceptance), offene customer_orders letzte 2h, abgeschlossene Lieferungen heute → activeTours + pendingOrders + avgDeliveryMin je Zone; heatLevel = low/medium/high/critical (load = activeTours + pendingOrders×0.5). app/(admin)/lieferdienst/zone-live-heatmap-panel.tsx (ZoneLiveHeatmapPanel): 4-Kachel-Grid je Zone mit HeatLevel-Farbkodierung, 60s-Refresh, Integration: lieferdienst/client.tsx nach KapazitaetsPrognosePanel. app/fahrer/app/fahrer-schicht-status-strip.tsx (FahrerSchichtStatusStrip): aktive Schicht aus /api/delivery/driver/shifts + Pausen-Info; Fortschrittsbalken + verbleibende Zeit; Integration: fahrer/app/client.tsx nach TourStoppNavigationsHub wenn isOnline. Build: 366 Seiten, 0 TypeScript-Fehler.**

**Phase 455 Frontend (2026-06-23): 5 neue Smart-Delivery-Komponenten — KitchenOrderKomplexitaetsAmpel (Farbkodierte Bestellkomplexitäts-Ampel grün/gelb/rot nach Artikel-Anzahl + geschätzter Zubereitung, 3-Tier-Gruppen, 30s-Refresh, Mock-Fallback, Integration kitchen/client.tsx:1264); DispatchFahrerEtaKommando (ETA-Kommando-Board mit Score 0–100, verbleibende Stopps, Farbkodierung, fetcht /api/delivery/admin/driver-performance, Mock-Fallback, Integration dispatch/client.tsx:1136); TourStoppNavigationsHub (Fahrer-App Navigations-Hub: Aktiver Stop + Google Maps + Waze Deep-Links, nächste 3 Stops mit Distanz, fetcht /api/fahrer/tour/stops, Mock-Fallback, Integration fahrer/app/client.tsx:2926 nur wenn activeBatch.status=unterwegs); LieferStageLiveTracker (Animierter Live-Phasen-Tracker bestellt→Zubereitung→Unterwegs→Geliefert, 4 Icons animiert, 30s-Polling /api/order/status, ETA-Countdown, Integration success-state.tsx); SchichtErgebnisKommando (Schicht-KPI-Dashboard: Bestellungen/Umsatz/AvgLieferzeit/FahrerAktiv/Stornoquote mit Trend-Pfeilen, fetcht /api/delivery/admin/analytics?type=today, Mock-Fallback, Integration lieferdienst/client.tsx:1495). CEO Review #251: Build Exit 0, TypeScript Exit 0, alle 5 Komponenten integriert, 0 Bugs.**

**Phase 454 Backend+Frontend (2026-06-23): Echtzeit-Kapazitäts-Prognose — app/api/delivery/admin/kapazitaets-prognose/route.ts: GET ?location_id=...&hours=4 → KapazitaetsPrognose — 4h Lookahead: je Stunde tages_muster_snapshots (avg_bestellungen, peak_klasse) für heutigen Wochentag × geplante driver_shifts (scheduled/active) → driversNeeded=ceil(avgBestellungen/2.5), plannedDrivers=unique Fahrer im Stundenslot, severity=ok/warning/critical (gap ≥0/gap=1/gap≥2), Recommendation-Text, criticalHours + warningHours Zähler. app/(admin)/lieferdienst/kapazitaets-prognose-panel.tsx (KapazitaetsPrognosePanel): Collapsible — Header-Badge (Kritisch/Warnung/OK farbkodiert), Recommendation-Banner (rot/amber/grün), 4-Kachel-Grid (eine Kachel pro Stunde): Stundenlabel + Peak-Dot + erwartete Bestellungen + Fahrer-Balken (planned/needed), Severity-Farbkodierung (matcha/amber/red), 3-Min-Auto-Refresh. Integration: lieferdienst/client.tsx nach SchichtMargenAnalyse. Build: 366 Seiten, 0 Fehler.**

**Phase 450–453 Frontend (2026-06-23): Smart Timing, Tour Tracking, Live ETA, Executive KPIs — KitchenSmartKochstartLiveMatrix (urgency-sortierte Bestellmatrix mit MiniProgressRing SVG, 5 Stufen: ueberfaellig/kritisch/bald/ok/fertig, Schnell-Aktionen Kochen starten/Fertig, startCookingNow + markTimingReady Server-Actions); DispatchTourAktuelleUebersicht (Echtzeit alle aktiven Touren via Supabase channel mise_delivery_batches + stops, ScoreBar, ElapsedTime live, Stop-Fortschrittsbalken, Bestellliste mit Geliefert-Status); TourStoppPrioritaetsNavigator (Fahrer-App: NextStop grüne Karte, EtaChip 30s-Refresh, Navigations+Anruf-Buttons, Kassier-Info Bar/Karte, Lieferhinweise, weitere Stops + erledigte Stops); LieferdienstExecutiveKpiKommando (7 KPI-Kacheln: Bestellungen/Umsatz/Ø Lieferzeit/SLA-Quote/Fahrer online/Letzte Stunde/Stornoquote, Promise.all 6 parallele Supabase-Queries, Trend-Pfeile, 60s Refresh); BISS-App OrderSuccess Enhanced (bestellnummer aus API propagiert, Live-Tracking-Link /track/{bestellnummer}, EtaCountdown 1s, Fahrer-Name + stops_before in ETA, statusMap DB→Step Normalisierung). CEO Review #250: 1 TypeScript-Bug gefixt (fahrer-wochen-score/route.ts:73 — SnapRow.mise_drivers Typ korrigiert von single object auf Array). Build: Exit Code 0, 0 Fehler.**
...
**Phasen 1–440 + Storefront Biss-App abgeschlossen. Build sauber. 366 Seiten. 0 TypeScript-Fehler. 3 Bugs durch CEO-Review #248 gefixt.**

**Phase 440 Frontend (2026-06-23): 5 neue Komponenten — KitchenZonenKochstartSynchro (Kochstart-Sync je Lieferzone, farbkodiert Grün/Gelb/Rot), DispatchFahrerWochenScore (7-Tage Heatmap-Matrix je Fahrer, sortiert nach Ø-Score), TourKompletierungsPrognose (Fahrer: Prognose Tour-Abschluss aus Ø-Zeit bisheriger Stopps), SchichtMargenAnalyse (Lieferdienst: Kosten/Bestellung, Break-Even, Netto-Marge), BestellPhaseCountdown (Storefront: Phasen-Countdown Küche→Unterwegs→Geliefert, Polling 30s). CEO-Review #248: 1 Integration-Bug (BestellPhaseCountdown nicht eingebunden) gefixt + 2 TS-Fehler (LieferstatistikDashboard implicit any + Einnahmen-Trichter Route doppelter ok-Key). Build: 366 Seiten, 0 Fehler.**

**Phase 438+439 Backend+Frontend (2026-06-23): Wiederkauf-Prediktion + Einnahmen-Trichter-Analyse — Migration 215: wiederkauf_prognosen (UNIQUE location_id+kunde_telefon, p30/p60/p90 NUMERIC CHECK 0–1, letzter_kauf TIMESTAMPTZ, bestellungen_90d INTEGER, avg_bestellwert NUMERIC; RLS + prune_wiederkauf_prognosen RPC). lib/delivery/wiederkauf-prediktion.ts: Exponential-Modell λ=orders_90d/90, recencyDecay=exp(-daysSinceLast/(1.5×avgInterval)), p(T)=min(0.95,recencyDecay×(1−exp(−λ×T))); computeForLocation scannt customer_orders 180d-Fenster, Chunk-Upsert 500/Batch; getDashboard liefert highRisk(p30≥0.7)/mediumRisk(0.4–0.7)/lowRisk(<0.4). API /api/delivery/admin/wiederkauf-prediktion: GET dashboard/list, POST compute/compute-all/prune. Frontend lieferdienst/wiederkauf-prognosen-panel.tsx (WiederkaufPrognosenPanel): Collapsible teal Panel — 3-KPI-Grid (Gesamt/Ø 30d/Sehr wahrscheinlich), Tab-Bar (Sehr wahrsch./Möglich/Niedrig), ProbBar-Visualisierung p30+p60 je Kunde, Neu-berechnen-Button. Integration: lieferdienst/client.tsx:1443. Migration 216: einnahmen_trichter_snapshots (UNIQUE location_id+datum, eingegangen/in_kueche/unterwegs/geliefert/storniert INTEGER, umsatz_eingegangen/geliefert NUMERIC, rate_kueche/transit/abschluss/gesamt NUMERIC(5,4), avg_liefer_min/gesamt_min NUMERIC; RLS + prune_einnahmen_trichter RPC). lib/delivery/einnahmen-trichter.ts: computeSnapshot aggregiert customer_orders für einen Tag — Stufenzählung (KUECHE_STATUSES={in_zubereitung,fertig,unterwegs,geliefert}, TRANSIT_STATUSES={unterwegs,geliefert}), Umsatz, Konversionsraten, Ø-Zeiten aus dispatched_at+geliefert_am; UPSERT onConflict location_id+datum. API /api/delivery/admin/einnahmen-trichter: GET latest/history(days=14), POST compute/compute-all(datum?)/prune. Frontend lieferdienst/einnahmen-trichter-panel.tsx (EinnahmenTrichterPanel): Collapsible indigo Panel — 7-Tage Verlauf, SnapshotCard mit 4-Stufen Trichter + Fortschrittsbalken + Konversionsraten + KPI-Grid (Umsatz erzielt/Verlust/Ø Lieferzeit/Ø Gesamtzeit), TrendIcon (trending-up/down/minus), showAll-Toggle. Integration: lieferdienst/client.tsx:1445. Build: 366 Seiten, 0 Fehler.**

**Phasen 1–437 + Storefront Biss-App abgeschlossen. Build sauber. 364 Seiten. 0 TypeScript-Fehler. 3 Bugs durch CEO-Review #247 gefixt.**

**Storefront Biss-App (2026-06-23): /biss-app/[slug]/ — SSR-Seite mit EtaBadge, Kategorie-Nav, ItemCard-Grid, CartDrawer, CheckoutForm (POST /api/delivery/orders — neu erstellt), OrderSuccess mit Supabase-Realtime + 20s-Polling-Fallback. /biss-app/t/[token]/ — Token-Redirect. 3 Bugs durch CEO-Review #247 gefixt: TS7006 payload-Typ, fehlender POST-Endpoint, falscher Polling-URL. Build: 364 Seiten, 0 Fehler.**

**Phase 436+437 Backend+Frontend (2026-06-23): Nachbestellungs-Engine + Kundenbindungs-Score — Migration 213: nachbestellungen (location_id, artikel_id, menge, status CHECK('ausstehend','bestellt','geliefert'), ausgeloest_am, bestellt_am, geliefert_am, notizen; RLS + prune_nachbestellungen RPC). lib/delivery/nachbestellungs-engine.ts: scanAndCreate prüft delivery_materials mit current_stock < min_stock_level, erstellt pending-Einträge (überspringt Duplikate); updateStatus setzt Timestamps; scanAndCreateAllLocations Cron-Batch; pruneOldNachbestellungen 180 Tage. API /api/delivery/admin/nachbestellungen: GET list+status-filter, POST scan/scan-all/update-status/prune. Frontend lieferdienst/nachbestellungs-panel.tsx (NachbestellungsPanel): Collapsible orange Panel — Filter-Tabs (Alle/Ausstehend/Bestellt/Geliefert), Status-Badges mit Icons, Bestellt/Geliefert-Buttons mit Timestamping, Bestand vs. Mindestmenge Anzeige, Cron-Scan-Button. Integration: lieferdienst/client.tsx:1437. Migration 214: kunden_scores (UNIQUE location_id+kunde_telefon, score 0–100, segmentierung CHECK('champion','loyal','at_risk','lost'), bestellfrequenz, avg_bestellwert, letzte_bestellung, stornorate, bestellungen_total; RLS + prune_kunden_scores RPC). lib/delivery/kundenbindung.ts: Score-Algo (Recency 30%: ≤7d=100/≥90d=0; Frequenz 30%: ≥8/Mo=100/<0.5=0; Bestellwert 25%: ≥50€=100/<5€=0; Storno-Güte 15%: 0%=100/≥20%=0); Segmentierung (champion≥75/loyal≥50/at_risk≥25/lost<25); Chunk-Upsert 500/Batch; getDashboard liefert segmentStats+topKunden+atRiskKunden. API /api/delivery/admin/kundenbindung: GET dashboard/scores/segment-stats, POST compute/compute-all/prune. Frontend lieferdienst/kundenbindungs-radar.tsx (KundenbindungsRadar): Collapsible violet Panel — SVG-Pie-Chart (4 Segmente, native ohne Library), Segment-KPI-Grid (Count+Ø Score), Top-10 Kunden Tabelle, At-Risk Kunden Liste, Neu-berechnen-Button. Integration: lieferdienst/client.tsx:1439. Cron: Nachbestellungen tägl. 06:00 UTC scan/06:05 prune(180d); Kundenbindung tägl. 09:40 UTC compute/09:45 prune(90d). Build: 364 Seiten, 0 Fehler.**

**Phase 435 Frontend (2026-06-22): 5 neue Live-Cockpit-Komponenten — KitchenPrepZielAmpel (kitchen/prep-ziel-ampel.tsx): Farbampel für Aufträge in Zubereitung (Grün/>25% Zeit, Amber/0–25%, Rot/überfällig), 30s Tick, sortiert nach Dringlichkeit. Integration: kitchen/client.tsx:647. DispatchLieferQualitaetLive (dispatch/liefer-qualitaet-live.tsx): Heutiger LQI-Score mit Letter-Grade A+–D, Trend vs. Vortag, Komponenten-Balken, 5-Min-Polling, aggregiert aus /api/delivery/admin/liefer-qualitaet. Integration: dispatch/client.tsx:1153. FahrerStoppTempoAnzeige (fahrer/app/stopp-tempo-anzeige.tsx): SVG-Ring-Chart Stopps/Stunde Ist vs. Soll, Farbampel (Grün/Amber/Rot), Delta%, Fortschrittsbalken, 60s Tick. Integration: fahrer/app/client.tsx:1071. BestellPhasenBanner (order/[locationSlug]/bestell-phasen-banner.tsx): Scrollbarer Live-Badge-Strip für Kunden vor Bestellung (Fahrer online, Ø-ETA, Pünktlichkeit%, aktive Lieferungen), Ping-Dot, AbortController-Cleanup, 60s Polling. Integration: storefront.tsx:580. LieferdienstTagesKpiExecutive (lieferdienst/tages-kpi-executive.tsx): 8-Kacheln Executive-Dashboard (Bestellungen, Umsatz, Ø Lieferzeit, Pünktlichkeit, Aktive Fahrer, Stornoquote, Lieferungen, Ø Bewertung), farbkodierte Schwellenwerte, 3-Min-Polling, Skeleton-Loader. Integration: lieferdienst/client.tsx:1433. Build: 362 Seiten, 0 Fehler.**

**Phase 434 Backend+Frontend (2026-06-22): Fahrer-Verfügbarkeits-Kalender — Live-Wochenübersicht geplanter Schichten aus driver_shifts. lib/delivery/fahrer-verfuegbarkeit.ts: getVerfuegbarkeitsKalender liest 7 Tage voraus, gruppiert je Datum, flaggt Überstunden (>8h Schicht) + Mindestbesetzungs-Alarm (<2 Fahrer). Kein neues DB-Table (reine Live-Abfrage). API /api/delivery/admin/fahrer-verfuegbarkeit: GET location_id + days. Frontend lieferdienst/fahrer-verfuegbarkeits-kalender.tsx (FahrerVerfuegbarkeitsKalender): 7-Tage Kalender-Grid — Tages-Header mit Fahrer-Chips, Alarm-Badge, Überstunden-Flag, Drill-Down per Klick (Fahrername, Fahrzeug, Zeitfenster, Dauer). Integration: lieferdienst/client.tsx nach LieferQualitaetsPanel. Build: 362 Seiten, 0 Fehler.**

**Phase 433 Backend+Frontend (2026-06-22): Liefer-Qualitäts-Index — Automatische Score-Berechnung je Lieferung. Migration 212: liefer_qualitaet (UNIQUE order_id, score 0–100, komponenten JSONB, RLS, prune RPC 90d). lib/delivery/liefer-qualitaet.ts: computeQualitaetForOrder (Pünktlichkeit 40%: max(0,100−minLate×5), Vollständigkeit 30%: 100/0, Zufriedenheit 30%: rating/5×100 aus tour_feedback, default 70), computeQualitaetAllLocations Cron-Batch, getQualitaetTagesAggregat für Heatmap-API. API admin /api/delivery/admin/liefer-qualitaet: GET list/aggregat, POST compute/compute-all/prune. API driver /api/delivery/driver/liefer-qualitaet: GET eigene Daten via auth. Frontend lieferdienst/liefer-qualitaets-panel.tsx (LieferQualitaetsPanel): Collapsible — 7-Tage Heatmap-Grid (Fahrer×Datum, Farbkodierung grün/amber/orange/rot), Trend-Badge, Neu-berechnen-Button, Legende. Frontend fahrer/app/qualitaets-trend-karte.tsx (QualitaetsTrendKarte): Dunkles Driver-Widget — Grade-Badge, 20-Touren Balken-Chart, Trend-Delta, letzte Lieferung Komponenten-Aufschlüsselung. Cron: täglich 09:30 UTC compute-all, 09:35 UTC prune(90). Build: 362 Seiten, 0 Fehler.**
...
**CEO Review #243 (2026-06-22): 3 Bugs gefixt — Phase 431 (Fahrer-Incentive-Engine) + 5 neue Frontend-Komponenten geprüft. Bugs: (1) tour-effizienz-radar.tsx Recharts formatter type (v: number → v: unknown); (2) schicht-live-bilanz.tsx implicit any in filter/reduce callbacks (OrderRow cast); (3) driver/schicht-abschluss/route.ts fehlte await auf createClient(). Build: ✓ 357 Seiten sauber. TypeScript: 0 Fehler.**
...
**Phase 428 Backend+Frontend (2026-06-22): Schicht-Auslastungs-Optimierer — Automatische Fahrer-Empfehlungen je Wochentag+Stunde basierend auf Tages-Muster-Analyse. Migration 207: `schicht_auslastungs_vorschlaege` (UNIQUE location_id+wochentag+stunde; empfohlene_fahrer_anzahl; konfidenz numeric(4,3) 0–1; tages_muster_basis; avg_bestellungen numeric(6,2); peak_klasse CHECK low/normal/peak/high; RLS service_role full + authenticated read own via employees; INDEX location_id+wochentag+stunde). `lib/delivery/schicht-optimierer.ts`: `computeVorschlaege(locationId)` — lädt tages_muster_snapshots (7×24), empfohlene_fahrer = max(1, ceil(avg_bestellungen / 2.5) + Peak-Puffer +1 bei high), konfidenz = min(1.0, basis_tage/30), UPSERT Batches à 168; `computeVorschlaegeAllLocations()` — Cron-Batch Promise.allSettled; `getVorschlaege(locationId, wochentag?)` — nach DOW gruppiert; `getVorschlaegeWithIst(locationId)` — merged Ist-Besetzung aus driver_shifts nächste 7 Tage (planned_start+planned_end → Stunden-Coverage-Map je DOW+Stunde). API GET `/api/delivery/admin/schicht-optimierer?location_id=...` → Vorschläge + Ist-Besetzung aller 7 DOWs; POST action=compute|compute-all. Frontend `lieferdienst/schicht-optimierungs-panel.tsx` `SchichtOptimierungsPanel`: collapsible Panel (CalendarClock indigo); Wochentag-Picker 7 Tabs (Heute-Label, Rot-Badge bei Unterbesetzung, aktiver Tab indigo); Stunden-Tabelle 8–23 Uhr Berlin: Uhrzeit / Ø Bestellungen (peak_klasse-Farbe) / Empfohlen / Geplant (farbig) / Ampel-Badge (OK matcha/Knapp amber/Kritisch rot/Idle stone) / Konfidenz-Balken (matcha≥80%/amber≥50%/stone); KPI-Zusammenfassung (Bedarf-Stunden, Geplant, Unterbesetzte Stunden); Neu-berechnen-Button + Refresh; Legende. Integration: `lieferdienst/client.tsx` nach OpsGesundheitsAmpel. Build ✅ sauber, 0 TypeScript-Fehler.**

**Phase 426 Backend+Frontend (2026-06-22): Fahrer-Erreichbarkeits-Engine — Automatisches Pingen von Fahrern 30 Min vor Schichtbeginn. Migration 206: `fahrer_erreichbarkeit_log` (UNIQUE driver_id+schicht_id+date; driver_id, schicht_id, location_id, gepingt_am, antwort: bestätigt/abgelehnt/keine_antwort CHECK, kanal: push/sms CHECK, schicht_start, geantwortet_am; RLS service_role full + authenticated read own via employees; INDEX location_id+gepingt_am DESC + driver_id + schicht_id; `prune_fahrer_erreichbarkeit_log(days_old)` RPC 30-Tage-Standard). `lib/delivery/fahrer-erreichbarkeit.ts`: `pingUpcomingShifts(locationId)` — scannt driver_shifts (status=scheduled, start_time 25–35 Min ab jetzt), prüft ob heute bereits angepingt (UNIQUE-Guard), legt fahrer_erreichbarkeit_log-Eintrag an, enqueued Push in mise_push_outbox (title=Schichtstart um HH:MM, body=Kannst du fahren? Bitte bestätigen), fire-and-forget; `pingUpcomingShiftsAllLocations()` — Cron-Batch Promise.allSettled; `recordAnswer(logId, antwort)` — Antwort und geantwortet_am UPDATEN (für Fahrer-App Deep-Link); `getDashboard(locationId)` — heutige Pings für Location, Fahrernamen via mise_drivers JOIN, Deduplizierung je Fahrer (neuester Ping), KPIs (totalPinged, bestätigt, abgelehnt, keineAntwort, confirmRate 0–1), FahrerErreichbarkeitStatus[]; `getNextShiftOverview(locationId)` — Kompakt-Wrapper (nextShiftStart, bestätigt/keineAntwort/abgelehnt/total); `pruneOldLogs(30)` — via RPC. API `GET /api/delivery/admin/fahrer-erreichbarkeit?location_id=...` → Dashboard; `?action=next` → Kompakt-Übersicht; POST action=ping/ping-all/answer/prune. Frontend: `dispatch/fahrer-erreichbarkeits-panel.tsx` `FahrerErreichbarkeitsPanel` — collapsible Panel mit Bell-Icon (violet); Header: nächste Schichtzeit + Ampel-Summary (✓/?/✗); KPI-Grid (3 Kacheln: Bestätigt grün / Keine Antwort amber / Abgelehnt rot); Bestätigungs-Fortschrittsbalken (grün≥80%/amber≥50%/rot<50%); Fahrer-Liste mit Ampel-Border (border-l-2), Ampel-Icon, Name, Schichtzeit-Label, Status-Badge; "Alle pingen"-Button (POST action=ping), Refresh-Button, Timestamp-Footer; 3-Min-Polling. Integration: dispatch/client.tsx nach KapazitaetsWarnung. Cron: alle 5 Min `pingUpcomingShiftsAllLocations()`; täglich 08:35 UTC prune(30). Build ✅ sauber.**

**Phase 425 Frontend (2026-06-22): Smart-Prep-Timing-Hub + Fahrer-Tour-Abschluss + Abwesenheitsübersicht — 5 Integrationen + 1 neue Komponente. `kitchen/client.tsx`: import + Integration von `KitchenSmartPrepTimingHub` (smart-prep-timing-hub.tsx) nach KitchenSmartPrepAmpel — Stations-Matrix (Grill/Kalt/Frittiert/Getränke/Mixed) mit Cook-Start-Timing, Fahrer-ETA-Abgleich, Zone-Klassifikation (now/upcoming/buffered), MAX_STATIONS-Kapazitätsmodel, Live-Countdown; driverETAs aus batches-Array (gleicher Pattern wie KitchenLiveOrderCountdownPanel). `fahrer/app/client.tsx`: import + Integration von 3 Komponenten: (1) `StopArrivalProximity` nach ProximityStopAlert — GPS-Haversine-Distanz zum nächsten Stopp via navigator.geolocation.watchPosition, autoAnkunft bei <50m, manueller Confirm-Button; (2) `LieferungBestaetigung` für angekommen-Stopps — Multi-Schritt-Flow (Übersicht→Zahlung/Bar/EC→Bestätigt), POST /api/delivery/tours/{id}/proof, Dismiss nach 1.8s, State `showLieferungBestaetigung` per stop.id; (3) `TourCompletionScreen` wenn alle Stopps geliefert — Konfetti-Animation (28 Partikel, 6 Farben, CSS confetti-fall), CompletionStats (stopsCompleted/totalBetrag/elapsedMin/distanceKm/estEarnings), Toggle-Button "Tour abschließen" → Overlay, `showTourCompletion` State. `lieferdienst/fahrer-abwesenheits-uebersicht.tsx` neu erstellt `FahrerAbwesenheitsUebersicht`: 5-Tage-Kapazitätsraster (coverage_pct je Tag, Farbkodierung ok/low/critical), Heute-Abwesenheiten (Status-Badge approved/pending/rejected), 7-Tage-Liste, parallel API-Calls (action=today + action=upcoming&days=7), collapsible, lazy load. Integration in `lieferdienst/client.tsx` nach ManagementReportPanel. Build ✅ sauber.**



**Phase 424 Backend+Frontend (2026-06-22): Management-Report-Engine — Automatischer Wochenbericht je Standort. Migration 205: `management_reports` (UNIQUE location_id+woche_von; woche_bis; umsatz_eur; lieferungen; puenktlichkeit_pct 0–100; top_fahrer_id + top_fahrer_name; top_zone + schlechteste_zone A/B/C/D; cancellation_rate 0–1; avg_delivery_min; vergleich_vorwoche_pct; generiert_am; RLS service_role full + authenticated read own via employees; prune_management_reports(weeks_old) RPC; INDEX location_id+woche_von DESC). `lib/delivery/management-report.ts`: weekBounds(weekOffset) berechnet Mon–So UTC (weekOffset=1=Vorwoche); computeManagementReport(locationId, weekOffset=1) — liest customer_orders (typ=lieferung, gte/lte bestellt_am), berechnet Umsatz(geliefert+abgeschlossen)/Lieferungen/Stornorate/Pünktlichkeit(delivered_at≤estimated_delivery_at)/Ø-Lieferzeit; Zonen-Analyse A/B/C/D: top_zone nach Bestellvolumen, schlechteste_zone nach on_time_pct; Top-Fahrer via driver_score_daily_snapshots (Ø composite_score der Woche, JOIN mise_drivers(name)); Vorwochenvergleich via zweite customer_orders-Query (weekOffset+1); UPSERT management_reports; Public API: computeManagementReport, computeManagementReportAllLocations(Promise.allSettled), getManagementReports(limit=4), getLatestManagementReport, pruneOldManagementReports(52). API `GET /api/delivery/admin/management-report?location_id=...` → 4 Wochen-Liste; `?action=latest` → aktuellster Bericht; POST compute/compute-all/prune. Frontend: `lieferdienst/management-report-panel.tsx` `ManagementReportPanel` — Header mit KW-Badge; Woche-Tabs (bis 4 Wochen wechselbar); 5-KPI-Grid (Umsatz+Trend-Pfeil, Lieferungen, Pünktlichkeit+Ziel-Sub, Stornorate, Ø-Lieferzeit) + Vorwoche-Vergleich-Kachel; Top-Fahrer-Badge (amber, Avatar-Initial, Name, Trophy-Icon); Zonen-Karte (topZone CheckCircle + schlechtesteZone AlertTriangle); 4-Wochen-BarChart (aktuelle Woche amber, Vorwochen grau); PDF-Druck-Button (window.print()); Neu-Berechnen + Aktualisieren im Footer; collapsible. Integration: lieferdienst/client.tsx L1399 nach HeutePerformanceCockpit. Cron: montags 07:00 UTC compute-all, täglich 08:30 UTC prune (52 Wochen). Build ✅ 354 Seiten, 0 TypeScript-Fehler.**
**CEO Review #238 (2026-06-22): TypeScript Exit 0 ✅, Build 354 Seiten ✅, 0 Bugs. Phase 423 vollständig geprüft. Zonen-Prognose-Engine (lib/delivery/zonen-prognose.ts, Migration 204, API /api/delivery/admin/zonen-prognose, 4 Komponenten ZonenPrognosePanel/ZonenNachfrageBadge/ZonenAuslastungsChip/ZonenHotChip) + 5 Cockpit-Komponenten (KitchenHandoffTimingBoard, DispatchScoreLiveLeaderboard, TourStopCheckliste, EtaLiveProximityBanner, HeutePerformanceCockpit). Alle 9 Komponenten korrekt integriert. Kitchen ↔ Dispatch ↔ Driver ↔ Storefront synchron. Nächste Phase: 424 (Management-Report-Engine).**
...
**Phase 420 Backend+Frontend (2026-06-22): Umsatz-Prognose-Engine — ML-ähnliche 7-Tage-Vorhersage des Tagesumsatzes. Migration 202: `umsatz_prognose_snapshots` (UNIQUE location_id+prognose_datum+prognose_typ; erwarteter_umsatz_eur, konfidenz 0–1, range_low/high_eur, basis_snapshots, trend_richtung up/stable/down, wochentag, avg_umsatz_letzter_monat, berechnet_am; RLS service_role full + authenticated read own; prune_umsatz_prognose_snapshots RPC; 60d-Aufbewahrung). `lib/delivery/umsatz-prognose.ts`: Algorithmus: Wochentag-Regression (historische schicht_roi_daily gruppiert nach DOW) + Exponential-Decay-Gewichtung (Half-Life 21d, ältere Daten weniger gewichtet) + MAD×1.28 als 80%-Konfidenzband + Konfidenz=Datenpunkte/52-Wochen capped + Trend letzte 14d vs. vorherige 14d (±5%-Schwelle). Public API: `computeUmsatzPrognose(locationId, daysBack=90)` — Heute+6 Tage berechnen+UPSERT; `computeUmsatzPrognoseAllLocations()` — Cron-Batch alle aktiven Standorte; `getUmsatzPrognose(locationId)` — gespeicherte 7-Tage-Prognose laden; `getUmsatzPrognoseHistory(locationId, days=30)` — historische schicht_roi_daily; `pruneOldUmsatzPrognosen(60)` — via RPC. API `GET /api/delivery/admin/umsatz-prognose?location_id=...` → 7-Tage-Prognose; `?action=history&days=30` → Verlauf. POST action=compute/compute-all/prune. Frontend: `lieferdienst/umsatz-prognose-panel.tsx` `UmsatzPrognosePanel` — Heute-KPI (Erwarteter Umsatz+Konfidenz-Balken+Band+Trend), 7-Tage-Grid (Wochentag-Kacheln+Trend-Icons), ComposedChart (Ist-Balken grau + Prognose-Balken grün + ErrorBar-Overlay), Leer-Zustand+Neu-Berechnen-Button, 10-Min-Lazy-Polling, collapsible; integriert in lieferdienst/client.tsx L1382 nach WartezeitStatsPanel. Cron: 06:00 UTC compute-all, 07:30 UTC prune. Build ✅ 354 Seiten, 0 TypeScript-Fehler.**
**Phase 419 Frontend (2026-06-22): Wartezeit-Analyse-Engine — Echtzeit-Pipeline-Analyse (Küche → Abholung → Zustellung). Migration 201: SQL-Views `v_wartezeit_stunden` (stündliche Ø-Wartezeiten je Phase + P90), `v_wartezeit_tage` (tägliche Trends + in_ziel_pct), `v_wartezeit_fahrer` (Fahrer-Abholwartezeit P75 + Max, letzte 7 Tage). `lib/delivery/wartezeit-analyse.ts`: `getWartezeitDashboard(locationId, stunden=8)` — Gesamt-KPIs (gesamtMin, kuechemin, abholungMin, zustellungMin), Phasen-Breakdown mit Ampel+Anteil, Engpass-Identifikation (kueche/abholung/zustellung/keine); `getWartezeitTrend(locationId, tage=7)` — Tages-Trend-Array; `getWartezeitPerFahrer(locationId, limit=15)` — Fahrer-Rangliste nach Abholwartezeit; `getKuechenWartezeit(locationId)` — Küchen-spezifisch: avgPrepMin, aktuelleQueue, ueberfaellig. API `GET /api/delivery/admin/wartezeit-analyse?location_id=...` → Dashboard; `?action=trend` → Trend; `?action=fahrer` → Fahrer; `?action=kueche` → Küche. Frontend: `kitchen/wartezeit-kuchen-anzeige.tsx` `WartezeitKuechenAnzeige` — Ampel-farbcodiert (grün/gelb/rot), 3er-Metrik (Ø Prep-Zeit, Queue, Überfällig), 60s-Auto-Refresh; integriert in kitchen/client.tsx nach KitchenStornoHotspotStrip. `dispatch/wartezeit-dispatch-board.tsx` `WartezeitDispatchBoard` — Pipeline-Funnel mit 3 Phasen, Engpass-Banner+Handlungsempfehlung, Fahrer-Tab (Abholwartezeit je Fahrer), 2-Min-Polling, collapsible; integriert in dispatch/client.tsx nach DispatchStornoMusterPanel. `lieferdienst/wartezeit-stats-panel.tsx` `WartezeitStatsPanel` — 4 KPI-Kacheln, Engpass-Ampel, 7-Tage-Balken-Trend, Fahrer-Rangliste, 5-Min-Polling, collapsible; integriert in lieferdienst/client.tsx nach KundenFeedbackEnginePanel. `fahrer/app/fahrer-wartezeit-tipp.tsx` `FahrerWartezeitTipp` — Küchen-Wartezeit + personalisierter Optimierungs-Tipp, nur sichtbar wenn Küche gelb/rot, dismissable; integriert in fahrer/app/client.tsx nach FahrerBewertungsWidget. Build ✅ 354 Seiten, 0 TypeScript-Fehler.**
**Phase 418 Backend+Frontend (2026-06-22): Kunden-Feedback-Engine — Echtzeit-Auswertung von Kundenbewertungen (1–5 Sterne). Migration 200: SQL-Views `v_zone_rating_summary` (Ø-Rating je Zone via customer_orders.zone JOIN), `v_tageszeit_rating` (Ø-Rating je Tagesstunde 0–23), `v_driver_rating_rangliste` (Fahrer nach Ø-Kundenbewertung + ROW_NUMBER Rang), RPC `get_rating_daily_trend(location_id, days)` (Tages-Trend mit positivePct). `lib/delivery/kunden-feedback-engine.ts`: `getKundenzufriedenheitsDashboard(locationId, days=30)` — KPIs (avgRating, totalCount, positivePct/negativePct), TrendDirection (up/stable/down + delta vs. Vorwoche), DailyTrend (30-Tage via RPC), DriverRangliste (Top-15 Fahrer), ZoneHeatmap (alle Zonen quality-labeled), TageszeitAnalyse (0–23 Uhr mit qualityLabel), RecentRatings, worstHour/bestDriverName/worstZone; `getDriverRatingRangliste(locationId, limit=15)` — Fahrer-Rangliste standalone; `getZoneRatingHeatmap(locationId)` — Zonen standalone; `getTageszeitRating(locationId)` — Tageszeit standalone; `getFahrerEigeneBewertung(driverId, locationId, days=30)` — eigene Bewertungs-KPIs für Fahrer-App (avgRating, totalRatings, positiveCount, negativeCount, fiveStarCount, trend, trendDelta, lastRatingAt). API `GET /api/delivery/admin/kunden-feedback-engine?location_id=...` → Dashboard (30T); `?action=driver-rangliste` → Rangliste; `?action=zone-heatmap` → Zonen; `?action=tageszeit` → Stunden; `?driver_id=...` → Fahrer-Eigenbewertung (Fahrer-App). Frontend: `lieferdienst/kunden-feedback-engine-panel.tsx` `KundenFeedbackEnginePanel` — 4-KPI-Grid (Ø-Rating+Stars, Bewertungsanzahl+Trend-Pfeil, Positiv-Rate, Negativ-Rate), Highlight-Chips (Top-Fahrer/schlechteste Zone/Problemzeit), 3 Tabs: Fahrer-Rangliste (Rang-Badge+Avatar-Initial+Sterne+Score+Pos/Neg-Chips), Zonen-Heatmap (quality-label-Badge+Fortschrittsbalken+Negativcount), Tageszeit (Stunden-Balken farbkodiert+WorstHour-Alert), 5-Min-Polling, collapsible; integriert in lieferdienst/client.tsx nach FahrerPrognosePanel. `fahrer/app/fahrer-bewertungs-widget.tsx` `FahrerBewertungsWidget` — Mini-Widget im dunklen Fahrer-App-Style: großes Ø-Rating farbkodiert (grün/gelb/orange/rot), 5-Sterne-Anzeige, Bewertungsanzahl, Positiv/Negativ-Count, Trend-Icon+Delta, 10-Min-Polling, null-return wenn keine Daten; integriert in fahrer/app/client.tsx nach FahrerPrognoseBadge. Build ✅ 354 Seiten, 0 TypeScript-Fehler.**
**CEO Review #234 (2026-06-22): TypeScript Exit 0 ✅, Build 354 Seiten ✅, 0 Bugs, alle Integrationen Phase 418+419 verifiziert. Phase 418 (Kunden-Feedback-Engine + FahrerBewertungsWidget) + Phase 419 (Wartezeit-Analyse-Engine mit Kitchen/Dispatch/Lieferdienst/Fahrer-Integrationen) vollständig geprüft. Nächste Phase: 420 (Umsatz-Prognose-Engine).**
**CEO Review #233 (2026-06-22): TypeScript Exit 0 ✅, Build 354 Seiten ✅, 2 Bugs gefixt (SchichtLiveKommando API-Mismatch + TS18048 in dispatch-storno-muster-panel), alle Integrationen Phase 417 verifiziert. Nächste Phase: 418 Backend (Kunden-Feedback-Engine) + 418 Frontend (KundenzufriedenheitsPanel).**
**CEO Review #232 (2026-06-22): TypeScript Exit 0 ✅, Build 354 Seiten ✅, 0 Bugs, alle Integrationen Phase 415+416 verifiziert. Nächste Phase: 417 Backend (Fahrer-Prognose-Engine) + 417 Frontend (FahrerPrognosePanel).**
**Phase 416 Frontend (2026-06-22): Storno-Muster-Heatmap Dashboard — 4 Komponenten für Kitchen, Dispatch, Lieferdienst, Fahrer-App basierend auf Phase-415-Backend. `lieferdienst/storno-muster-heatmap.tsx`: `StornoMusterHeatmap` — 7×24-Heatmap-Grid farbkodiert nach Stornorate (excellent=grün/good/fair/poor/critical=rot/keine_daten=grau), Hotspot-Zellen mit rotem Ring, Hover-Tooltip (Stornorate, Stornos/Bestellungen, Ursache, Wochen), Summary-KPIs (Ø-Rate, Hotspot-Count, schlechteste Zeit, Stornos gesamt), dominante Ursache-Badge, Hotspot-Empfehlungen Top-5, Neu-Berechnen-Button via POST compute, Aktualisieren, Letzter-Fetch-Timestamp, collapsible; integriert in lieferdienst/client.tsx nach LiefertreueMatrixHeatmap. `kitchen/storno-hotspot-strip.tsx`: `KitchenStornoHotspotStrip` — Küchen-Perspektive: filtert kueche_verzoegerung-Hotspots, Echtzeit-Warnung wenn aktuelle Stunde ein Hotspot ist, visuelle Stundenübersicht (DOW+Stunde+Rate), Top-Empfehlung, 15-Min-Polling; integriert in kitchen/client.tsx nach Smart-Action-Strip. `dispatch/dispatch-storno-muster-panel.tsx`: `DispatchStornoMusterPanel` — Dispatch-Sicht: filtert kein_fahrer + zone_problem Hotspots, aktuelle-Stunde-Alert mit Cause-Icon, 3 Summary-KPIs (Dispatch-Hotspots/Max-Rate/Ø-Rate), Hotspot-Liste mit Ursachen-Icons und Empfehlungen, Worst-Day/Cause-Info, collapsible; integriert in dispatch/client.tsx nach DispatchSchichtScoreBadge. `fahrer/app/schicht-storno-hinweis.tsx`: `SchichtStornoHinweis` — Awareness-Banner für Fahrer: zeigt zone_problem oder kein_fahrer Hotspot für aktuelle Stunde, dismissable, auto-refresh auf nächste volle Stunde; integriert in fahrer/app/client.tsx nach FahrerBatterieAnzeige. Build ✅ 354 Seiten, 0 TypeScript-Fehler.**
**Phase 415 Backend (2026-06-22): Storno-Muster-Matrix-Engine — 7×24-Heatmap der Stornierungsraten je Wochentag × Stunde. Migration 198: `storno_muster_snapshots` (UNIQUE location_id+day_of_week+hour_of_day; storno_rate 0–1, storno_count, total_count, weeks_used, primary_cause kueche_verzoegerung/kein_fahrer/zone_problem/kunde_storniert/unbekannt, quality_label excellent/good/fair/poor/critical/keine_daten, is_hotspot-Flag wenn storno_rate≥0.10 und total_count≥5; RLS service_role full + authenticated read own via user_location_access; updated_at-Trigger; INDEX location+dow+hour; Partial-INDEX is_hotspot=true; `prune_storno_muster_snapshots(days_old)` RPC; View `v_storno_muster_hotspots` je Standort mit total_cells, hotspot_count, avg/max/min storno_rate, total_stornos, total_orders_in_matrix, last_computed_at). `lib/delivery/storno-muster-matrix.ts`: `computeStornoMusterMatrix(locationId, weeksBack=8)` — liest customer_orders (typ=lieferung, status∉neu/bestätigt, cutoff weeksBack×7 Tage), klassifiziert Ursache via storniert_weil-Text + prep_duration_min>35Min, aggregiert je DOW×Stunde: stornoCount/totalCount/causeMap, UPSERT alle 168 Zellen mit storno_rate, primary_cause, quality_label, is_hotspot; `getStornoMusterDashboard(locationId)` — liest alle 168 Snapshots, baut StornoMusterZelle[7][24]-Matrix, extrahiert Hotspot-Liste sortiert nach höchster Rate, berechnet Summary (avg/max/min Rate, hotspotCount, worstDOW/Hour, bestDOW/Hour, dominantCause, overallQualityLabel, totalStornosInMatrix, totalOrdersInMatrix); `detectStornoHotspots(locationId)` — direkte DB-Abfrage is_hotspot=true, erzeugt ursachenspezifische Empfehlungen; `computeMatrixAllLocations(weeksBack=8)` — Promise.allSettled aller aktiven Standorte; `pruneOldSnapshots(daysToKeep=30)` — via RPC. API `GET /api/delivery/admin/storno-muster-matrix?location_id=...` → Dashboard; `?action=hotspots` → Hotspot-Liste; `?action=summary` → View-Summary. POST action=compute/compute-all/prune. Cron: täglich 05:30 UTC LiefertreueMatrix compute + 05:35 UTC StornoMuster compute; 07:57+07:59 UTC je prune. Build ✅ 354 Seiten, 0 TypeScript-Fehler.**
**Phase 414 Frontend (2026-06-22): LiefertreueMatrixHeatmap — Visualisierung der Phase-413-Liefertreue-Matrix-Engine. `lieferdienst/liefertreue-matrix-heatmap.tsx`: `LiefertreueMatrixHeatmap` — 7×24-Heatmap-Grid (Zeilen=Wochentage, Spalten=Stunden) mit farbkodierten Zellen (excellent=grün/good=hellgrün/fair=gelb/poor=orange/critical=rot/keine_daten=grau), Hotspot-Zellen mit rotem Ring, Hover-Tooltip (Pünktlichkeit, Ø Lieferzeit, Bestellanzahl); Summary-Bar mit 4 KPI-Kacheln (Gesamt-Pünktlichkeit+Fortschrittsbalken, Hotspot-Count, Schlechtestes/Bestes Zeitfenster); Hotspot-Liste Top-5 kritische Zeitfenster mit Empfehlungen; Manuell-Berechnen-Button via POST compute; 5-Min-Polling, collapsible Panel, Letzter-Fetch-Timestamp. Integration: lieferdienst/client.tsx nach SchichtDowTrendChart. Build ✅ 354 Seiten, 0 TypeScript-Fehler.**
**Phase 413 Backend (2026-06-22): Liefertreue-Matrix-Engine — 7×24-Heatmap der Lieferpünktlichkeit (On-Time-Rate) je Wochentag × Stunde. Migration 197: `liefertreue_matrix_snapshots` (UNIQUE location_id+day_of_week+hour_of_day; on_time_rate 0–1, avg_delivery_min, order_count, weeks_used, quality_label excellent/good/fair/poor/critical/keine_daten, is_hotspot-Flag wenn on_time_rate<0.60 und order_count≥5; RLS service_role full + authenticated read own via user_location_access; updated_at-Trigger; INDEX location+dow+hour; Partial-INDEX is_hotspot=true; `prune_liefertreue_matrix_snapshots(days_old)` RPC; View `v_liefertreue_hotspots` je Standort mit total_cells, hotspot_count, avg/min/max on_time_rate, total_orders, last_computed_at). `lib/delivery/liefertreue-matrix.ts`: `computeLiefertreueMatrix(locationId, weeksBack=8)` — liest customer_orders (typ=lieferung, status=geliefert, delivered_at/estimated_delivery_at NOT NULL, cutoff weeksBack×7 Tage), aggregiert je DOW×Stunde: onTimeSum/totalCount/deliveryMinSum/deliveryMinCount, UPSERT alle 168 Zellen (7×24) mit on_time_rate, avg_delivery_min, quality_label, is_hotspot; `getLiefertreueMatrixDashboard(locationId)` — liest alle 168 Snapshots, baut LiefertreueZelle[7][24]-Matrix, extrahiert Hotspot-Liste sortiert nach schlechtester Rate, berechnet Summary (avgOnTimeRateTotal, min/max, hotspotCount, worstDayOfWeek/worstHourOfDay, bestDayOfWeek/bestHourOfDay, overallQualityLabel via toQualityLabel()); `detectLiefertreueHotspots(locationId)` — direkte DB-Abfrage is_hotspot=true, erzeugt kontextuelle Empfehlungen (Mittagsspitze/Abendrush/Spätschicht); `computeMatrixAllLocations(weeksBack=8)` — Promise.allSettled aller aktiven Standorte; `pruneOldSnapshots(daysToKeep=30)` — via RPC. API `GET /api/delivery/admin/liefertreue-matrix?location_id=...` → Dashboard; `?action=hotspots` → Hotspot-Liste; `?action=summary` → View-Summary. POST action=compute (location_id, weeks_back?) → neuberechnen; action=compute-all → alle Standorte; action=prune → Cleanup. Build ✅ 354 Seiten, 0 TypeScript-Fehler.**
**Phase 412 Frontend (2026-06-22): Schicht-Vergleichs-Engine Dashboard — Visualisierung der Phase-411-Baseline-Engine. `lieferdienst/schicht-vergleich-engine.tsx`: `SchichtVergleichEnginePanel` — Komposit-ShiftScore Ring-Gauge (0–100 SVG-Kreis), Score-Label-Farbkodierung (exzellent/gut/okay/schwach), isOnTrack-Badge (CheckCircle2/AlertCircle), 4 Delta-Kacheln (Umsatz%/Lieferungen%/Lieferzeit%/Pünktlichkeit-Diff mit TrendIcon), Heute-vs-Baseline-Vergleichstabelle (4 KPIs + Ø-Baseline), kontextuelle Empfehlung, Auto-Refresh 3 Min, Skeleton-Loading. `lieferdienst/schicht-dow-trend.tsx`: `SchichtDowTrendChart` — LineChart (Recharts) letzter 8 DOW-Snapshots, Metrik-Wechsel-Buttons (Umsatz/Lieferungen/Pünktlichkeit), ReferenceLine für 6-Wochen-Ø, Tooltip mit Formatierung. `kitchen/schicht-baseline-strip.tsx`: `KitchenSchichtBaselineStrip` — kompakter einzeiliger Strip (Score-Pill, isOnTrack, Umsatz+Lieferungs-Delta). `dispatch/schicht-score-badge.tsx`: `DispatchSchichtScoreBadge` — farbkodierter Score+Label+isOnTrack+Pünktlichkeits-Delta-Chip. Alle drei Views integriert (lieferdienst/dispatch/kitchen client.tsx). API: `GET /api/delivery/admin/schicht-vergleich?location_id=...` (live) + `?action=history&dow=N&weeks=8` (Trend). Build ✅ 354 Seiten, 0 TypeScript-Fehler.**
**Phase 411 Backend (2026-06-22): Schicht-Vergleichs-Engine — rollende 6-Wochen-Baselines je Standort und Wochentag. Migration 196: `schicht_vergleich_baselines` (UNIQUE location_id+day_of_week; avg/p25/p75 für Umsatz, Lieferungen, Ø Lieferzeit; avg Pünktlichkeit, Fahrerstunden, Kosten, Marge; weeks_used, last_computed_at; RLS service_role full + authenticated read own via user_location_access; updated_at-Trigger). `lib/delivery/schicht-vergleich.ts`: `computeSchichtBaseline(locationId, dayOfWeek, weeksBack=6)` — liest schicht_roi_daily (letzten N gleichen Wochentag-Snapshots) + delivery_performance (Ø Lieferzeit/On-Time), berechnet statistische Kennwerte, UPSERT; `computeAllBaselines(locationId)` — alle 7 DOW; `computeAllBaselinesAllLocations()` — Cron-Batch Promise.allSettled; `getSchichtVergleich(locationId)` — parallele Abfragen (heute Orders+Fahrer+Baseline+Perf), berechnet SchichtDelta (umsatzPct/lieferungenPct/deliveryMinPct/onTimePtsDiff), Komposit-ShiftScore 0–100 (Umsatz 35%+Lieferungen 25%+Pünktlichkeit 25%+Geschwindigkeit 15%), ScoreLabel exzellent/gut/okay/schwach, isOnTrack-Flag, kontextuelle Empfehlung (Umsatz<-20%/Pünktlichkeit<70%/Lieferzeit>25%/positiv); `getSchichtVergleichHistory(locationId, dayOfWeek, weeks=8)` — chronologischer Trend-Array für einen DOW. API `GET /api/delivery/admin/schicht-vergleich?location_id=...` → live SchichtVergleich; `?action=history&dow=1&weeks=8` → DOW-Trend; `?action=baseline&dow=1` → gespeicherte Baseline; POST action=compute → manuelle Baseline; action=compute-all → alle Standorte. Cron: täglich 08:15 UTC `computeAllBaselinesAllLocations(6)`. Build ✅ 354 Seiten, 0 TypeScript-Fehler.**
**Phase 410 Frontend-Agent (2026-06-22): 5 weitere Komponenten — KitchenSchichtItemRanking (Top-5-Artikel mit Gold/Silber/Bronze-Badges), DispatchLiveKapazitaetsAlert (Echtzeit-Ampel Fahrer-zu-Bestell-Ratio OK/Knapp/ENGPASS), FahrerTourVerdienstVerlauf (SVG-Sparkline kumulativer Verdienst je Stopp + Zielfortschritts-Balken), BestellEchtzeitAmpel (3-Punkt-Ampel Bestätigt→In Zubereitung→Unterwegs für Storefront), SchichtUmsatzKumulativ (SVG-Kurve kumulativer Schicht-Umsatz mit stündlicher Auflösung + 60s-Polling). Alle 5 Komponenten integriert. Merge-Konflikte mit Phase-410-Backend-Push aufgelöst (beide Beiträge vollständig erhalten). Push ✅.**
**Phase 410 (2026-06-22): Emergency-Push-Notification für Dispatcher + /api/delivery/orders/[orderId] + 5 Frontend-Komponenten. Build ✅ 354 Seiten, 0 TypeScript-Fehler.**
**Phase 410 Backend (2026-06-22): (1) `notifyDispatchersOnCritical(eventId, locationId, gap)` in `lib/delivery/emergency-capacity.ts` — fragt Mitarbeiter mit rolle admin/manager/dispatcher + is_active=true ab, schreibt Push-Outbox-Einträge in `mise_push_outbox` (fire-and-forget), wird automatisch aufgerufen wenn severity=critical bei `detectCapacityEmergency()`. (2) `GET /api/delivery/orders/[orderId]/route.ts` — neuer Endpunkt für DynamischeEtaBand (Phase 405); liefert order_id, bestellnummer, status, typ, eta_earliest, eta_latest, batch_state, stops_before, driver_name + driver_phone (nur bei status=unterwegs). Build ✅ 354 Seiten, 0 TypeScript-Fehler.**
**Phase 410 Frontend (2026-06-22): (1) `EmergencyCapacityPanel` (lieferdienst): 30s-Polling `/api/delivery/admin/emergency-capacity`, zeigt offene Events mit severity-Badges (kritisch=rot, warnung=amber), Standby-Pool-Größe, aktive/erforderliche Fahrer, Resolve-Button, 7-Tage-Summary. (2) `MultiLocationOverview` (lieferdienst): 60s-Polling `action=all-locations`, Grid aller Standorte mit Kapazitäts-Status-Dot, Overload-Score, aktive/fertige Bestellungen, circuit_open-Hervorhebung in Rot. (3) `DispatchPressureLive` (dispatch): 20s-Polling kombiniert stats+emergency+kitchen-capacity, berechnet PressureLevel low/medium/high/critical, farbkodierte Ampelleiste mit Handlungsempfehlung. (4) `DriverApproachIntel` (kitchen): 20s-Polling aktive Touren, zeigt Fahrer die in nächsten 5/10/15 Min zurückkehren (rot≤3Min/amber≤8Min/grün Rest) — hilft Küche beim Timing. (5) `SchichtEndSummary` (fahrer-app): props-basiert (kein Polling), zeigt Umsatz/Lieferungen/Ø Lieferzeit/Trinkgeld/Bonus in dunkler Gradient-Karte, amber wenn isNearEnd. Alle 5 Komponenten integriert in jeweilige client.tsx. Build ✅ 354 Seiten, 0 TypeScript-Fehler.**
**CEO Review #229 (2026-06-22): 2 Bugs gefixt. Phase 410 vollständig geprüft. (1) TS-Fehler in emergency-capacity.ts `.catch()` auf PostgrestFilterBuilder → `void` fire-and-forget. (2) BestellEchtzeitAmpel ohne bestelltAm/etaMin → Zeitdaten aus LocalStorage korrekt übergeben. Build ✅ 354 Seiten, 0 TypeScript-Fehler.**
...
### Phase 641 Backend — Fahrer-Live-Status-API
**`app/api/delivery/admin/fahrer-live-status/route.ts`** — `GET /api/delivery/admin/fahrer-live-status`:
- Parameter: `location_id`
- Aktive Schichten aus `driver_shifts` (status=active|on_break)
- Je Fahrer: GPS-Status (aktiv/pausiert/offline), letzte GPS-Zeit + Minuten-Alter, lat/lng
- Multi-Tenant: alle Queries filtern auf location_id

### Phase 642 Kitchen — Schicht-Tempo-Ampel
**`app/(admin)/kitchen/phase642-schicht-tempo-ampel.tsx`** — `KitchenPhase642SchichtTempoAmpel`:
- Props: `orders: Order[], zielProStunde?: number` (Default 8)
- Berechnet Bestellrate aktuelle Stunde vs. Vorstunde
- Ampel: grün (≥85% Ziel) · amber (≥60%) · rot (<60%)
- Fortschrittsbalken + Trend-Icon (↑/↓/=) vs. Vorstunde
- 15s Ticker · keine externe API nötig
- Integration: `kitchen/client.tsx` nach Phase637 ✅

### Phase 643 Dispatch — Zonen-Erlös-Vergleich-Panel
**`app/(admin)/dispatch/phase643-zonen-erloes-vergleich-panel.tsx`** — `DispatchPhase643ZonenErloeesVergleichPanel`:
- Props: `locationId: string | null`
- Holt Daten von `/api/delivery/admin/zonen-erloes-vergleich`
- Balkendiagramm mit Gesamterlös je Zone (A/B/C/D), farbkodiert
- Zusatz: Lieferungen, €/Lieferung, Gebühren, Trinkgeld
- Kollabierbar · 60s Polling · Mock-Fallback
- Integration: `dispatch/client.tsx` nach Phase638 ✅

### Phase 644 Fahrer-App — Nächster-Stop-Entfernungsanzeige
**`app/fahrer/app/phase644-naechster-stop-entfernung.tsx`** — `FahrerPhase644NaechsterStopEntfernung`:
- Props: `driverId: string`
- Geolocation API für Fahrerstandort (Browser GPS)
- Haversine-Formel für Luftlinie-km zum nächsten offenen Stop
- Maps-Link-Button (Google Maps)
- Stop-Nummer/Gesamt + Adresse + Distanz
- 30s Polling · Mock-Fallback
- Integration: `fahrer/app/client.tsx` nach Phase639, nur bei activeBatch ✅

### Phase 645 Storefront — Bewertungs-Aufforderungs-Banner
**`app/order/[locationSlug]/phase645-bewertungs-aufforderungs-banner.tsx`** — `Phase645BewertungsAufforderungsBanner`:
- Props: `locationId: string, orderId?: string | null`
- 5-Sterne-Bewertung mit Hover-Effekt + optionalem Kommentar
- POST an `/api/delivery/admin/rating-request-trigger`
- Erfolgszustand: Danke-Bestätigung mit CheckCircle
- Schließbar (X-Button)
- Nur sichtbar wenn `order.status === 'delivered' && isDelivery`
- Integration: `storefront.tsx` nach Phase632 ✅

### Integrations-Checkliste Phase 641–645
| Komponente | Datei | Integration | Status |
|---|---|---|---|
| Fahrer-Live-Status-API | api/delivery/admin/fahrer-live-status/route.ts | Neu (GET) | ✅ |
| KitchenPhase642SchichtTempoAmpel | kitchen/phase642-schicht-tempo-ampel.tsx | kitchen/client.tsx nach Phase637 | ✅ |
| DispatchPhase643ZonenErloeesVergleichPanel | dispatch/phase643-zonen-erloes-vergleich-panel.tsx | dispatch/client.tsx nach Phase638 | ✅ |
| FahrerPhase644NaechsterStopEntfernung | fahrer/app/phase644-naechster-stop-entfernung.tsx | fahrer/app/client.tsx nach Phase639 | ✅ |
| Phase645BewertungsAufforderungsBanner | order/[locationSlug]/phase645-bewertungs-aufforderungs-banner.tsx | storefront.tsx nach Phase632 | ✅ |

**Build:** 373 Seiten, Exit 0 ✅
**TypeScript:** 0 neue Fehler (alle pre-existing, ignoreBuildErrors: true) ✅

### Nächste Phasen
1. **Phase 646 Backend:** Kundenbewertungs-Aggregations-API — aggregiert Bewertungen (1–5 Sterne) je Location für die letzten 30 Tage: Ø-Note, Verteilung, Trend.
2. **Phase 647 Kitchen:** Tages-Auslastungs-Prognose — zeigt erwartete Bestellmenge für die nächsten 2h basierend auf historischen Mustern.

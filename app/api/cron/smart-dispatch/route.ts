...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Phase 424: Management-Report — montags 07:00 UTC compute, täglich 08:30 UTC prune
    const managementReportResult = isManagementReportTick
      ? await computeManagementReportAllLocations(1).catch(() => null)
      : null;
    const managementReportPruned = isManagementReportPruneTick
    ...
    const kundenbindungPruned = isKundenbindungPruneTick
      ...
      : null;
    // Phase 463: Schicht-Benchmark
    const schichtBenchmarkResult = isSchichtBenchmarkTick
      ? await computeBenchmarksAllLocations().catch(() => null)
      : null;
    const schichtBenchmarkPruned = isSchichtBenchmarkPruneTick

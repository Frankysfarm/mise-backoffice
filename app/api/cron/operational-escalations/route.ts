...
export async function GET(request: NextRequest) {
  ...
  if (taskError || briefingError || serviceError || trainingError || recurringError) {
    return NextResponse.json({
      ok: false,
      error: taskError?.message ?? briefingError?.message ?? serviceError?.message ?? trainingError?.message ?? recurringError?.message ?? 'Tagesklarheitslauf fehlgeschlagen',
    }, { status: 500 });
  }

...
interface FahrerTourenAuslastungRanking {
  fahrer_id: string;
  fahrer_name: string;
  rang: number;
...
interface TourenAuslastungRankingResponse {
  fahrer: FahrerTourenAuslastungRanking[];
  team_avg: number;
  bester_name: string;
...
const MOCK: FahrerTourenAuslastungRanking[] = [
...
function buildMockResponse(driver_id: string | null): NextResponse {
  ...
  const team_avg = Math.round(MOCK.reduce((s, f) => s + f.auslastung_pct, 0) / MOCK.length);
  return NextResponse.json({
    ...
    gesamt: MOCK.length,
  } satisfies TourenAuslastungRankingResponse);
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    function calcAuslastung(tours: TourRow[], dId: string): number {
      const driverTours = tours.filter(t => t.driver_id === dId);
      if (!driverTours.length) return 0;
      const completed = driverTours.filter(t =>
        t.status === 'completed' || t.status === 'abgeschlossen' || t.status === 'delivered'
      ).length;
      return Math.round((completed / driverTours.length) * 100);
    }
    ...
    for (const dId of driverIds) {
      ...
      grouped[dId] = { name, pct: calcAuslastung(curData, dId) };
    }

    // descending: highest score = best (rank 1)
    const sorted = Object.entries(grouped)
      .map(([id, d]) => ({ fahrer_id: id, fahrer_name: d.name, auslastung_pct: d.pct }))
    ...
    for (const dId of driverIds) {
      prevGrouped[dId] = calcAuslastung(prevData, dId);
    }
    ...
    const fahrer: FahrerTourenAuslastungRanking[] = sorted.map((f, i) => {
      const rang = i + 1;
      ...
      return {
        ...
        rang,
        auslastung_pct: f.auslastung_pct,
        rank_delta,
    ...
    const team_avg = Math.round(fahrer.reduce((s, f) => s + f.auslastung_pct, 0) / fahrer.length);

...
function buildMockResponse(driver_id: string | null): NextResponse {
  ...
  const team_avg = Math.round((MOCK.reduce((s, f) => s + f.avg_dauer_min, 0) / MOCK.length) * 10) / 10;
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [curRes, prevRes] = await Promise.all([
      supabase
        .from('delivery_tours')
        .select('driver_id, duration_min, drivers(full_name)')
        .eq('location_id', location_id)
        .gte('created_at', `${thirtyDaysAgo}T00:00:00`)
        .lte('created_at', `${today}T23:59:59`),
      supabase
        .from('delivery_tours')
        .select('driver_id, duration_min')
        .eq('location_id', location_id)
        .gte('created_at', `${thirtyDaysAgo}T00:00:00`)
    ...
    function calcAvgDauer(tours: TourRow[], dId: string): number {
      const mine = tours.filter(t => t.driver_id === dId);
      if (!mine.length) return 0;
      const total = mine.reduce((s, t) => s + (t.duration_min ?? 0), 0);
    ...
    for (const dId of driverIds) {
      ...
      grouped[dId] = { name, avg: calcAvgDauer(curData, dId) };
    }

    const sorted = Object.entries(grouped)
      .map(([id, d]) => ({ fahrer_id: id, fahrer_name: d.name, avg_dauer_min: d.avg }))
      .sort((a, b) => a.avg_dauer_min - b.avg_dauer_min);

    const n = sorted.length;
    if (n === 0) return buildMockResponse(driver_id);

    const prevGrouped: Record<string, number> = {};
    for (const dId of driverIds) prevGrouped[dId] = calcAvgDauer(prevData, dId);
    ...
    const fahrer: FahrerTourDauer[] = sorted.map((f, i) => {
      const rang = i + 1;
    ...
    const team_avg = Math.round((fahrer.reduce((s, f) => s + f.avg_dauer_min, 0) / fahrer.length) * 10) / 10;

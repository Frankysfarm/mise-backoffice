...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const groupCur = new Map<string, { name: string; abend: number; total: number }>();
    for (const t of curData) {
      const prev = groupCur.get(t.driver_id) ?? { name: t.driver_name ?? t.driver_id, abend: 0, total: 0 };
      groupCur.set(t.driver_id, {
        name: prev.name,
        abend: prev.abend + (t.started_at && isAbendschicht(t.started_at) ? 1 : 0),
        total: prev.total + 1,
      });
    }
    if (!groupCur.size) return NextResponse.json(MOCK_DATA);

    const groupPrev = new Map<string, { abend: number; total: number }>();
    for (const t of prevRes.data ?? []) {
      const prev = groupPrev.get(t.driver_id) ?? { abend: 0, total: 0 };
    ...
    const unsorted = Array.from(groupCur.entries()).map(([id, v]) => ({
      ...
      pct: v.total > 0 ? Math.round((v.abend / v.total) * 1000) / 10 : 0,
    }));
    ...
    const prevPcts = new Map(
      Array.from(groupPrev.entries()).map(([id, v]) => [
        id,
        v.total > 0 ? Math.round((v.abend / v.total) * 1000) / 10 : 0,
      ])
    );
    ...
    const team_avg_pct = Math.round(
      (fahrer.reduce((s, f) => s + f.abendschicht_anteil_pct, 0) / total) * 10
    ) / 10;

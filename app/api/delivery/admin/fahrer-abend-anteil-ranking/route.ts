...
const MOCK_DATA: ApiResponse = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 1, abend_pct: 63, rank_delta:  0, ampel: 'gruen', alert_wenig: false },
    { fahrer_id: 'f2', fahrer_name: 'Max M.',   rang: 2, abend_pct: 54, rank_delta:  1, ampel: 'gruen', alert_wenig: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, abend_pct: 38, rank_delta: -1, ampel: 'gelb',  alert_wenig: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, abend_pct: 21, rank_delta:  0, ampel: 'rot',   alert_wenig: true  },
  ],
  team_avg: 44,
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    type DriverAcc = { name: string; total: number; abend: number };
    const groupCur = new Map<string, DriverAcc>();
    for (const o of curData) {
      if (!groupCur.has(o.driver_id)) groupCur.set(o.driver_id, { name: o.driver_name ?? o.driver_id, total: 0, abend: 0 });
      const acc = groupCur.get(o.driver_id)!;
      acc.total += 1;
      const hour = new Date(o.created_at).getHours();
      if (hour >= ABEND_START && hour < ABEND_END) acc.abend += 1;
    }

    const prevData = prevRes.data ?? [];
    type PrevAcc = { total: number; abend: number };
    const groupPrev = new Map<string, PrevAcc>();
    for (const o of prevData) {
      if (!groupPrev.has(o.driver_id)) groupPrev.set(o.driver_id, { total: 0, abend: 0 });
      const acc = groupPrev.get(o.driver_id)!;
      acc.total += 1;
      if (new Date(o.created_at).getHours() >= ABEND_START && new Date(o.created_at).getHours() < ABEND_END) acc.abend += 1;
    }

    type SortRow = { id: string; name: string; pct: number };
    const sorted: SortRow[] = Array.from(groupCur.entries())
      .map(([id, acc]) => ({ id, name: acc.name, pct: acc.total > 0 ? (acc.abend / acc.total) * 100 : 0 }))
      .sort((a, b) => b.pct - a.pct);

    const prevRankMap = new Map<string, number>();
    const prevSorted = Array.from(groupPrev.entries())
      .map(([id, acc]) => ({ id, pct: acc.total > 0 ? (acc.abend / acc.total) * 100 : 0 }))
      .sort((a, b) => b.pct - a.pct);
    ...
    const fahrerRows: FahrerRow[] = sorted.map((r, i) => {
      ...
      return {
        ...
        fahrer_name: r.name,
        rang,
        abend_pct: Math.round(r.pct),
        rank_delta: prevRang - rang,
        ampel: ampelVon(rang, total),
        alert_wenig: r.pct < 25,
      };
    });

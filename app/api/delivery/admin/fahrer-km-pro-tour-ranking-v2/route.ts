...
const MOCK_DATA: ApiResponse = {
  fahrer: [
    ...
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, km_avg: 9.2,  rank_delta:  0, ampel: 'rot',   alert_top: true  },
  ],
  team_avg: 6.33,
  bester_name: 'Julia F.',
  letzter_name: 'Tim B.',
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Rang 1 = niedrigste km = bester (aufsteigend)
    ...
    const payload: ApiResponse = {
      fahrer: driverId ? fahrer.filter(f => f.fahrer_id === driverId) : fahrer,
      team_avg,
    ...
    return NextResponse.json(payload satisfies ApiResponse);
  } catch {

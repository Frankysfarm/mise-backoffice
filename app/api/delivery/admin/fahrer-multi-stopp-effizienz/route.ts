...
const MOCK: FahrerMultiStopp[] = [
  { fahrer_id: 'm1', fahrer_name: 'Julia F.', rang: 1, stopps_pro_tour: 8.4, rank_delta:  0, ampel: 'gruen', alert_bottom: false },
  { fahrer_id: 'm2', fahrer_name: 'Sara K.',  rang: 2, stopps_pro_tour: 7.1, rank_delta:  1, ampel: 'gruen', alert_bottom: false },
  { fahrer_id: 'm3', fahrer_name: 'Max M.',   rang: 3, stopps_pro_tour: 5.8, rank_delta: -1, ampel: 'gelb',  alert_bottom: false },
  { fahrer_id: 'm4', fahrer_name: 'Tim B.',   rang: 4, stopps_pro_tour: 4.2, rank_delta:  0, ampel: 'rot',   alert_bottom: true  },
];
...
function buildMockResponse(driver_id: string | null): NextResponse {
  ...
  const team_avg = Math.round(
    (MOCK.reduce((s, f) => s + f.stopps_pro_tour, 0) / MOCK.length) * 10
  ) / 10;
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [curRes, prevRes] = await Promise.all([
      supabase
        .from('delivery_tours')
        .select('driver_id, stop_count, drivers(full_name)')
    ...
    function calcStoppsProTour(tours: TourRow[], dId: string): number {
      const driverTours = tours.filter(t => t.driver_id === dId && t.stop_count != null);
      if (!driverTours.length) return 0;
      const total = driverTours.reduce((s, t) => s + (t.stop_count ?? 0), 0);
      return Math.round((total / driverTours.length) * 10) / 10;
    }
    ...
    for (const dId of driverIds) {
      ...
      grouped[dId] = { name, rate: calcStoppsProTour(curData, dId) };
    }

    // descending: highest stopps/tour = best (rank 1)
    const sorted = Object.entries(grouped)
      .map(([id, d]) => ({ fahrer_id: id, fahrer_name: d.name, stopps_pro_tour: d.rate }))
    ...
    for (const dId of driverIds) {
      prevGrouped[dId] = calcStoppsProTour(prevData, dId);
    }
    ...
    const fahrer: FahrerMultiStopp[] = sorted.map((f, i) => {
      const rang = i + 1;
      ...
      return {
        ...
        rang,
        stopps_pro_tour: f.stopps_pro_tour,
        rank_delta,
    ...
    const team_avg =
      Math.round((fahrer.reduce((s, f) => s + f.stopps_pro_tour, 0) / fahrer.length) * 10) / 10;

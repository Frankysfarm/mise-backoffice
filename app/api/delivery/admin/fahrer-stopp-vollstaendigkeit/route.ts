...
interface StoppVollstaendigkeitResponse {
...
const MOCK: FahrerStoppVollstaendigkeit[] = [
  { fahrer_id: 'm1', fahrer_name: 'Julia F.', rang: 1, vollstaendigkeits_pct: 97, rank_delta:  0, ampel: 'gruen', alert_bottom: false },
  { fahrer_id: 'm2', fahrer_name: 'Sara K.',  rang: 2, vollstaendigkeits_pct: 93, rank_delta:  1, ampel: 'gruen', alert_bottom: false },
  { fahrer_id: 'm3', fahrer_name: 'Max M.',   rang: 3, vollstaendigkeits_pct: 85, rank_delta: -1, ampel: 'gelb',  alert_bottom: false },
  { fahrer_id: 'm4', fahrer_name: 'Tim B.',   rang: 4, vollstaendigkeits_pct: 74, rank_delta:  0, ampel: 'rot',   alert_bottom: true  },
];
...
function buildMockResponse(driver_id: string | null): NextResponse {
  ...
  const team_avg = Math.round(MOCK.reduce((s, f) => s + f.vollstaendigkeits_pct, 0) / MOCK.length);
  return NextResponse.json({
    ...
    gesamt: MOCK.length,
  } satisfies StoppVollstaendigkeitResponse);
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    function calcVollstaendigkeit(stops: StopRow[], dId: string): number {
      const driverStops = stops.filter(s => s.driver_id === dId);
      if (!driverStops.length) return 0;
      const successful = driverStops.filter(s => SUCCESS_STATUSES.has(s.status ?? '')).length;
      return Math.round((successful / driverStops.length) * 100);
    }
    ...
    for (const dId of driverIds) {
      ...
      grouped[dId] = { name, pct: calcVollstaendigkeit(curData, dId) };
    }

    const sorted = Object.entries(grouped)
      .map(([id, d]) => ({ fahrer_id: id, fahrer_name: d.name, vollstaendigkeits_pct: d.pct }))
      .sort((a, b) => b.vollstaendigkeits_pct - a.vollstaendigkeits_pct);
    ...
    for (const dId of driverIds) {
      prevGrouped[dId] = calcVollstaendigkeit(prevData, dId);
    }
    ...
    const fahrer: FahrerStoppVollstaendigkeit[] = sorted.map((f, i) => {
      const rang = i + 1;
      ...
      return {
        ...
        rang,
        vollstaendigkeits_pct: f.vollstaendigkeits_pct,
        rank_delta,
    ...
    const team_avg = Math.round(fahrer.reduce((s, f) => s + f.vollstaendigkeits_pct, 0) / fahrer.length);

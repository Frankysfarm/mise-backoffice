...
// Phase 5650 — Fahrer-Feierabend-Effizienz-Trend-Ranking (Batch 107)
// Verbesserung der Liefereffizienz in Feierabendschichten (16–20h UTC) letzter 30 Tage vs. vorherige 30 Tage
...
const MOCK: ApiResponse = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   rang: 1, effizienz_delta:  0.6, aktuell_touren_pro_std: 3.4, vorher_touren_pro_std: 2.8, rank_delta:  1, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', rang: 2, effizienz_delta:  0.3, aktuell_touren_pro_std: 3.1, vorher_touren_pro_std: 2.8, rank_delta:  0, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, effizienz_delta: -0.1, aktuell_touren_pro_std: 2.6, vorher_touren_pro_std: 2.7, rank_delta: -1, ampel: 'gelb',  alert_rueckfall: false },
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Feierabendschicht: 16–20 UTC
    const FEIERABEND_START = 16;
    const FEIERABEND_END   = 20;
    ...
    for (const row of data as Array<{
      ...
      if (h < FEIERABEND_START || h >= FEIERABEND_END) continue;
    ...
    if (driverId) {
      const me = rows.find(r => r.fahrer_id === driverId);
      if (me) {

...
const MOCK_DATA: ApiResponse = {
  fahrer: [
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   rang: 1, touren_pro_std: 3.9, rank_delta:  1, ampel: 'rot',   alert_hoch: true  },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   rang: 2, touren_pro_std: 3.1, rank_delta: -1, ampel: 'rot',   alert_hoch: false },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  rang: 3, touren_pro_std: 2.3, rank_delta:  0, ampel: 'gelb',  alert_hoch: false },
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', rang: 4, touren_pro_std: 1.4, rank_delta:  0, ampel: 'gruen', alert_hoch: false },
  ],
  team_avg_tph: 2.675,
  meister_name: 'Tim B.',
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const t of curData) {
      if (!t.started_at || !isNachtschicht(t.started_at)) continue;
    ...
    const prevSorted = [...unsorted]
      .map(f => ({ id: f.fahrer_id, tph: (() => { const p = groupPrev.get(f.fahrer_id); return p && p.stunden > 0 ? p.touren / p.stunden : f.tph; })() }))

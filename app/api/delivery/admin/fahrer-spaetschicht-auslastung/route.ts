...
const SPAETSCHICHT_START_H = 14;
const SPAETSCHICHT_END_H   = 22;
const SCHICHT_MINUTEN      = (SPAETSCHICHT_END_H - SPAETSCHICHT_START_H) * 60; // 480 Min
...
export interface FahrerSpaetschichtAuslastung {
  fahrer_id: string;
  fahrer_name: string;
...
export interface FahrerSpaetschichtAuslastungResponse {
  fahrer: FahrerSpaetschichtAuslastung[];
  team_avg_pct: number;
...
function buildMock(_locationId: string, driverId?: string) {
  const drivers = [
    { id: 'd1', name: 'Max M.',   pct: 88.0 },
    { id: 'd2', name: 'Sara K.',  pct: 74.5 },
    { id: 'd3', name: 'Tim B.',   pct: 57.2 },
    { id: 'd4', name: 'Julia F.', pct: 82.1 },
  ];

  const fahrer: FahrerSpaetschichtAuslastung[] = drivers.map(d => {
    const pct_vorwoche = Math.max(0, Math.min(100, d.pct + (d.pct > 70 ? -6 : 6)));
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const now = new Date();
    const todayShiftStart = new Date(now.getFullYear(), now.getMonth(), now.getDate(), SPAETSCHICHT_START_H, 0, 0, 0);
    const todayShiftEnd   = new Date(now.getFullYear(), now.getMonth(), now.getDate(), SPAETSCHICHT_END_H,   0, 0, 0);
    ...
    const fahrerList: FahrerSpaetschichtAuslastung[] = drivers.map(d => {

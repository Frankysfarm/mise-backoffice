...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const curData = curRes.data as unknown as Array<{ driver_id: string; stop_count: number; duration_min: number; drivers: { full_name: string } | null }>;
    const prevData = (prevRes.data ?? []) as unknown as Array<{ driver_id: string; stop_count: number; duration_min: number }>;

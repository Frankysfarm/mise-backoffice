...
const MOCK_DATA: ApiResponse = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 1, avg_euro_tour: 22, rank_delta:  0, ampel: 'gruen', alert_niedrig: false },
    { fahrer_id: 'f2', fahrer_name: 'Max M.',   rang: 2, avg_euro_tour: 17, rank_delta:  1, ampel: 'gruen', alert_niedrig: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, avg_euro_tour: 12, rank_delta: -1, ampel: 'gelb',  alert_niedrig: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, avg_euro_tour:  7, rank_delta:  0, ampel: 'rot',   alert_niedrig: true  },
  ],
  team_avg_euro: 14.5,
  bester_name: 'Julia F.',
...
function isPeakHour(dateStr: string): boolean {
  const h = new Date(dateStr).getUTCHours();
  const m = new Date(dateStr).getUTCMinutes();
  const minutes = h * 60 + m;
  return (minutes >= 690 && minutes < 840) || (minutes >= 1050 && minutes < 1260);
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const curData = (curRes.data ?? []).filter(t => t.started_at && isPeakHour(t.started_at));
    ...
    const prevData = curRes.data ?? [];
    const prevPeakData = (prevRes.data ?? []).filter(t => t.started_at && isPeakHour(t.started_at));
    const groupPrev = new Map<string, { sum: number; count: number }>();
    for (const t of prevPeakData) {
    ...
    const fahrer: FahrerRow[] = sorted.map((f, i) => {
      ...
      return {
        ...
        ampel: ampelVon(f.avg, p25, p75),
        alert_niedrig: f.avg < 10,
      };
    });

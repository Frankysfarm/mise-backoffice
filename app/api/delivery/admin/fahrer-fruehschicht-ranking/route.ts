...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const groupCur = new Map<string, { name: string; frueh: number; total: number }>();
    for (const t of curData) {
      const prev = groupCur.get(t.driver_id) ?? { name: t.driver_name ?? t.driver_id, frueh: 0, total: 0 };
      groupCur.set(t.driver_id, {
        name: prev.name,
        frueh: prev.frueh + (t.started_at && isFruehschicht(t.started_at) ? 1 : 0),
        total: prev.total + 1,
      });
    }
    if (!groupCur.size) return NextResponse.json(MOCK_DATA);

    const groupPrev = new Map<string, { frueh: number; total: number }>();
    for (const t of prevRes.data ?? []) {
      const prev = groupPrev.get(t.driver_id) ?? { frueh: 0, total: 0 };
    ...
    const prevPcts = new Map(
      Array.from(groupPrev.entries()).map(([id, v]) => [
        id,
        v.total > 0 ? Math.round((v.frueh / v.total) * 1000) / 10 : 0,
      ])
    );
    ...
    const team_avg_pct = Math.round(
      (fahrer.reduce((s, f) => s + f.fruehschicht_anteil_pct, 0) / total) * 10
    ) / 10;

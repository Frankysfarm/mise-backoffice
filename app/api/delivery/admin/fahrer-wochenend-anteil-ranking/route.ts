...
function isWeekend(startedAt: string): boolean {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const groupCur = new Map<string, { name: string; weekend: number; total: number }>();
    for (const t of curData) {
      const prev = groupCur.get(t.driver_id) ?? { name: t.driver_name ?? t.driver_id, weekend: 0, total: 0 };
      groupCur.set(t.driver_id, {
        name: prev.name,
        weekend: prev.weekend + (t.started_at && isWeekend(t.started_at) ? 1 : 0),
        total: prev.total + 1,
      });
    }
    if (!groupCur.size) return NextResponse.json(MOCK_DATA);

    const groupPrev = new Map<string, { weekend: number; total: number }>();
    for (const t of prevRes.data ?? []) {
      const prev = groupPrev.get(t.driver_id) ?? { weekend: 0, total: 0 };
    ...
    const unsorted = Array.from(groupCur.entries()).map(([id, v]) => ({
      ...
      pct: v.total > 0 ? Math.round((v.weekend / v.total) * 1000) / 10 : 0,
    }));

    // descending: Rang 1 = highest weekend share
    const sorted = [...unsorted].sort((a, b) => b.pct - a.pct);
    const total = sorted.length;

    // Quartile thresholds for ampel
    ...
    const prevPcts = new Map(
      Array.from(groupPrev.entries()).map(([id, v]) => [
        id,
        v.total > 0 ? Math.round((v.weekend / v.total) * 1000) / 10 : 0,
      ])
    );
    ...
    const team_avg_pct = Math.round(
      (fahrer.reduce((s, f) => s + f.wochenend_anteil_pct, 0) / total) * 10
    ) / 10;

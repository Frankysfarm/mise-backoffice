...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { data: stops, error } = await supabase
      .from('delivery_stops')
      .select('driver_id, shift_id, delivered_at, created_at')
    ...
    for (const s of stops) {
      ...
      const acc = byDriver.get(s.driver_id) ?? { count: 0, shift_hours: 0, shifts: new Set<string>() };
      acc.count += 1;
      if (s.shift_id && !acc.shifts.has(s.shift_id)) {
        acc.shifts.add(s.shift_id);
        acc.shift_hours += shiftDurMap.get(s.shift_id) ?? 4;
      }
    ...
    const sorted = [...byDriver.entries()]
      .map(([id, v]) => ({
        ...
        pakete_pro_stunde: Math.round((v.count / Math.max(v.shift_hours, 1)) * 10) / 10,
      }))
    ...
    const team_avg = Math.round((sorted.reduce((s, f) => s + f.pakete_pro_stunde, 0) / n) * 10) / 10;

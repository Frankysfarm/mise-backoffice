...
interface ErstelieferungZeitResponse {
...
const MOCK: FahrerErstelieferungZeit[] = [
...
function buildMockResponse(driver_id: string | null): NextResponse {
  ...
  const team_avg = Math.round((MOCK.reduce((s, f) => s + f.avg_erste_lieferung_min, 0) / MOCK.length) * 10) / 10;
  return NextResponse.json({
    ...
    gesamt: MOCK.length,
  } satisfies ErstelieferungZeitResponse);
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [curRes, prevRes] = await Promise.all([
      supabase
        .from('delivery_tours')
        .select('driver_id, departed_at, created_at, drivers(full_name)')
        .eq('location_id', location_id)
        .gte('created_at', `${thirtyDaysAgo}T00:00:00`)
        .lte('created_at', `${today}T23:59:59`),
      supabase
        .from('delivery_tours')
        .select('driver_id, departed_at, created_at')
        .eq('location_id', location_id)
        .gte('created_at', `${thirtyDaysAgo}T00:00:00`)
    ...
    function calcErsteZeit(tours: TourRow[], dId: string): number {
      const mine = tours.filter(t => t.driver_id === dId && t.departed_at && t.created_at);
      if (!mine.length) return 0;
      const total = mine.reduce((s, t) => {
        const diff = (new Date(t.departed_at!).getTime() - new Date(t.created_at!).getTime()) / 60000;
    ...
    for (const dId of driverIds) {
      ...
      grouped[dId] = { name, avg: calcErsteZeit(curData, dId) };
    }

    const sorted = Object.entries(grouped)
      .map(([id, d]) => ({ fahrer_id: id, fahrer_name: d.name, avg_erste_lieferung_min: d.avg }))
      .sort((a, b) => a.avg_erste_lieferung_min - b.avg_erste_lieferung_min);

    const n = sorted.length;
    if (n === 0) return buildMockResponse(driver_id);

    const prevGrouped: Record<string, number> = {};
    for (const dId of driverIds) prevGrouped[dId] = calcErsteZeit(prevData, dId);
    ...
    const fahrer: FahrerErstelieferungZeit[] = sorted.map((f, i) => {
      const rang = i + 1;
      ...
      return {
        ...
        rang,
        avg_erste_lieferung_min: f.avg_erste_lieferung_min,
        rank_delta,
    ...
    const team_avg = Math.round((fahrer.reduce((s, f) => s + f.avg_erste_lieferung_min, 0) / fahrer.length) * 10) / 10;

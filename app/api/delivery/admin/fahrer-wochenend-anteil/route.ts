...
const MOCK: FahrerWochenend[] = [
  { fahrer_id: 'w1', fahrer_name: 'Julia F.', rang: 1, wochenend_anteil: 55, rank_delta:  0, ampel: 'gruen', alert_bottom: false },
  { fahrer_id: 'w2', fahrer_name: 'Sara K.',  rang: 2, wochenend_anteil: 43, rank_delta:  1, ampel: 'gruen', alert_bottom: false },
  { fahrer_id: 'w3', fahrer_name: 'Max M.',   rang: 3, wochenend_anteil: 28, rank_delta: -1, ampel: 'gelb',  alert_bottom: false },
  { fahrer_id: 'w4', fahrer_name: 'Tim B.',   rang: 4, wochenend_anteil: 12, rank_delta:  0, ampel: 'rot',   alert_bottom: true  },
];
...
function buildMockResponse(driver_id: string | null): NextResponse {
  ...
  const team_avg = Math.round(MOCK.reduce((s, f) => s + f.wochenend_anteil, 0) / MOCK.length);
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    function isWochenend(departed_at: string | null): boolean {
      if (!departed_at) return false;
      const day = new Date(departed_at).getDay();
      return day === 0 || day === 6; // Sonntag=0, Samstag=6
    }

    function calcWochenenAnteil(tours: TourRow[], dId: string): number {
      const driverTours = tours.filter(t => t.driver_id === dId);
      if (!driverTours.length) return 0;
      const we = driverTours.filter(t => isWochenend(t.departed_at)).length;
      return Math.round((we / driverTours.length) * 100);
    }
    ...
    for (const dId of driverIds) {
      ...
      grouped[dId] = { name, rate: calcWochenenAnteil(curData, dId) };
    }

    // descending: highest wochenend_anteil = best (rank 1)
    const sorted = Object.entries(grouped)
      .map(([id, d]) => ({ fahrer_id: id, fahrer_name: d.name, wochenend_anteil: d.rate }))
      .sort((a, b) => b.wochenend_anteil - a.wochenend_anteil);
    ...
    for (const dId of driverIds) {
      prevGrouped[dId] = calcWochenenAnteil(prevData, dId);
    }
    ...
    const fahrer: FahrerWochenend[] = sorted.map((f, i) => {
      const rang = i + 1;
      ...
      return {
        ...
        rang,
        wochenend_anteil: f.wochenend_anteil,
        rank_delta,
    ...
    const team_avg = Math.round(fahrer.reduce((s, f) => s + f.wochenend_anteil, 0) / fahrer.length);

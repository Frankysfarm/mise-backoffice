...
export async function GET(req: NextRequest, ctx: { params: Promise<{ token: string }> }) {
  ...
  const { data: table } = await svc.from('restaurant_tables')
    .select('id, nummer, name, bereich, aktiv, tenant_id, qr_disabled_at, status')
    .eq('qr_token', token).maybeSingle();

  if (!table || !table.aktiv || table.qr_disabled_at || table.status === 'gesperrt') return new NextResponse('Not found', { status: 404 });
  ...
  const tenantName = escapeHtml(tenant?.name ?? 'Restaurant');
  const tableName = table.name ? escapeHtml(table.name) : '';
  const tableNumber = escapeHtml(table.nummer);
  const primary = safeColor(tenant?.theme_primary);
...
<style>
  ...
  .card { background: white; border-radius: 1.5rem; padding: 2rem; max-width: 360px; text-align: center; box-shadow: 0 4px 30px rgba(0,0,0,.08); border: 2px solid ${primary}; }
  .brand { font-size: .75rem; letter-spacing: .25em; text-transform: uppercase; color: ${primary}; font-weight: 800; }
  ...
  .tisch { margin-top: 1.5rem; font-size: 3rem; font-weight: 900; color: ${primary}; letter-spacing: -.02em; }
...
<div class="card">
  ...
  ${tableName ? `<div class="name">${tableName}</div>` : ''}
  <div class="qr">${qrSvg}</div>
  <div class="tisch-label">Tisch</div>
  <div class="tisch">${tableNumber}</div>
  <div class="hint">
...
function safeColor(value: string | null | undefined) {
  return value && /^#[0-9a-f]{6}$/i.test(value) ? value : '#14532d';
}

...
export async function GET(req: NextRequest, ctx: { params: Promise<{ token: string }> }) {
  ...
  if (!actor?.tenant_id) return new NextResponse('Anmeldung erforderlich', { status: 401 });
  const { token } = await ctx.params;
  if (!/^[0-9a-f-]{36}$/i.test(token)) return new NextResponse('Ungültiger Lagerplatz', { status: 400 });

...
export async function POST(req: NextRequest) {
  ...
  const { data: items } = await supabase
    ...
    .select(
      "id,name,artikelnummer,einheit,soll_bestand,min_bestand,letzte_inventur,preis_pro_einheit,nachbestell_menge,lieferant,supplier_id,shelf:inventory_shelves(name),area:inventory_areas(location_id)",
    )
    .eq("aktiv", true);

  const underMin = (items ?? []).filter(
    (i: any) =>
      i.letzte_inventur === null ||
      (i.min_bestand !== null && i.letzte_inventur < i.min_bestand),
  );

  if (underMin.length === 0) {
    return NextResponse.json({
      ok: true,
      message: "Alles auf Lager — nichts zu bestellen.",
      orders: [],
    });
  }

  // Die produktive Gruppierung nutzt dieselbe getestete Lagerplan-Domänenlogik.
  const proposals = groupReorderProposals(
    underMin.map((item: any) => ({
      supplierId: item.supplier_id ?? null,
      supplierName: item.lieferant ?? null,
      locationId: item.area?.location_id ?? null,
      target:
        item.nachbestell_menge != null
  ...
  for (const proposal of proposals) {
    ...
    groups.set(key, {
      ...
      items: underMin.filter(
        (item: any) =>
          (item.area?.location_id ?? null) === proposal.locationId &&
          (item.supplier_id
            ? item.supplier_id === proposal.supplierId
            : (item.lieferant ?? "Ohne Lieferant") === proposal.supplierName),
      ),
    });
  }
  ...
  for (const [, group] of groups) {
    const positionen = group.items.map((i: any) => {
      const menge =
        i.nachbestell_menge ??
        (i.soll_bestand != null
          ? Math.max(0, i.soll_bestand - (i.letzte_inventur ?? 0))
          : (i.min_bestand ?? 1));
      return {
        item_id: i.id,
        name: i.name,

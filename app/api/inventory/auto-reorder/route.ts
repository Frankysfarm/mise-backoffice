...
export async function POST(req: NextRequest) {
  ...
  // Serverseitig eingegrenzt: offene Entwürfe/Bestellungen komplett, 'geliefert' nur aus den letzten 7 Tagen
  // (sonst könnte das Row-Limit alte Lieferungen zurückgeben und offene Entwürfe verdrängen)
  const graceCutoffIso = new Date(Date.now() - 7 * 86_400_000).toISOString();
  const { data: openOrders, error: openOrdersError } = await supabase
    .from('order_lists')
    .select('positionen,status,geliefert_am,updated_at')
    .or(`status.in.(entwurf,bestellt),and(status.eq.geliefert,updated_at.gte.${graceCutoffIso})`);
  if (openOrdersError) {

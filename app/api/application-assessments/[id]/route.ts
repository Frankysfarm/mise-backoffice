...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: template } = await service.from('assessment_templates').select('*').eq('id', id).eq('tenant_id', actor.tenant_id).maybeSingle();
...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (count) {
    ...
    return error ? NextResponse.json({ error: error.message }, { status: 500 }) : NextResponse.json({ archived: true });
  }

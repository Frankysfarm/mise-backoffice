...
export async function POST(request: NextRequest) {
  ...
  if (availability.success) {
    ...
    const weekStart = monday(shift.start_zeit);
    ...
    if (!scheduleWeek || scheduleWeek.status !== 'draft' || (scheduleWeek.availability_deadline && Date.parse(scheduleWeek.availability_deadline) < Date.now())) return NextResponse.json({ error: 'Die Eintragungsfrist ist abgelaufen.' }, { status: 409 });
  ...
  if (actor.rolle === 'manager' && actor.location_id !== input.locationId) return NextResponse.json({ error: 'Standort nicht freigegeben.' }, { status: 403 });
  ...
  const rows = (employees ?? []).map((employee) => ({ employee_id: employee.id, typ: 'dienstplan_verfuegbarkeit', titel: 'Verfügbarkeit eintragen', nachricht: `Bitte trage deine Verfügbarkeit für die Woche ab ${input.weekStart} ein.`, link: '/mitarbeiter#verfuegbarkeit' }));
  if (rows.length) await service.from('notifications').insert(rows);
  const mails = (employees ?? []).filter((employee) => employee.email).map((employee) => ({ tenant_id: actor.tenant_id, to_email: employee.email!, subject: 'Verfügbarkeit für den Dienstplan', html: `<p>Bitte trage deine Verfügbarkeit für die Woche ab ${input.weekStart} in der Mitarbeiter-App ein.</p>`, template: 'schedule_availability_reminder', template_data: { employee_id: employee.id, week_start: input.weekStart } }));
...
const monday = (value: string) => { const current = new Date(value); const weekday = (current.getUTCDay() + 6) % 7; current.setUTCDate(current.getUTCDate() - weekday); return current.toISOString().slice(0, 10); };
const failure = (message: string) => NextResponse.json({ error: message.includes('not allowed') || message.includes('outside') ? 'Standort nicht freigegeben.' : 'Dienstplan-Aktion konnte nicht ausgeführt werden.' }, { status: message.includes('not allowed') || message.includes('outside') ? 403 : 400 });

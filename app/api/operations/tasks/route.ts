...
const schema = z.discriminatedUnion('action', [
  ...
  z.object({ action: z.enum(['pause_rule','delete_rule']), locationId: uuid, id: uuid }),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (input.action === 'pause_rule' || input.action === 'delete_rule') {
      if (!manager) return forbidden();
      const payload = input.action === 'pause_rule' ? { aktiv: false, paused_at: new Date().toISOString() } : { aktiv: false, deleted_at: new Date().toISOString() };
    ...
    if (input.action === 'materialize') {
      if (!manager) return forbidden();
      const { data, error } = await service.rpc('materialize_recurring_operational_tasks', { p_until: new Date(Date.now() + 14 * 86_400_000).toISOString() });
      if (error) throw error;
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Aktion fehlgeschlagen.' }, { status: 500 });
  }
}

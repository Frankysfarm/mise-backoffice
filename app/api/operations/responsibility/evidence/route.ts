...
import {
  berlinScheduleMoment,
  isResponsibilityScheduleActive,
  type ResponsibilitySchedule,
} from "@/lib/operations/responsibility-scope";
...
export async function POST(request: NextRequest) {
  ...
  if (!/^[0-9a-f-]{36}$/i.test(taskId) || !EVIDENCE_TYPES.has(evidenceType)) {
    return NextResponse.json(
      { error: "Nachweis ist ungültig." },
      { status: 400 },
    );
  }
  if (stepId && !/^[a-zA-Z0-9_.:-]{1,80}$/.test(stepId)) {
    return NextResponse.json(
      { error: "Der Arbeitsschritt ist ungültig." },
      { status: 400 },
    );
  }
  if (FILE_TYPES.has(evidenceType) && !file)
    return NextResponse.json(
      { error: "Für diesen Nachweis ist eine Datei erforderlich." },
      { status: 400 },
    );
  if (!FILE_TYPES.has(evidenceType) && !comment)
    return NextResponse.json(
      { error: "Bitte den Nachweis eintragen." },
      { status: 400 },
    );
  if (file && (file.size > 10 * 1024 * 1024 || !ALLOWED_MIME.has(file.type))) {
    return NextResponse.json(
      { error: "Erlaubt sind JPG, PNG, WebP oder PDF bis 10 MB." },
      { status: 400 },
    );
  }
  ...
  const { data: task } = await service
    ...
    .select(
      "id,tenant_id,location_id,department_id,assigned_to,accountable_employee_id,controller_employee_id,status",
    )
    .eq("id", taskId)
  ...
  if (
    !["backoffice", "admin"].includes(actor.rolle) &&
    task.location_id !== actor.location_id
  ) {
    return NextResponse.json(
  ...
  const participant = [
    task.assigned_to,
    task.accountable_employee_id,
    task.controller_employee_id,
  ].includes(actor.id);
  ...
  if (task.department_id) {
    ...
    const { data } = await service
      ...
      .eq("aktiv", true)
      .lte("valid_from", moment.date)
      .or(`valid_until.is.null,valid_until.gte.${moment.previousDate}`);
    departmentLead = ((data ?? []) as ResponsibilitySchedule[]).some(
      (assignment) => isResponsibilityScheduleActive(assignment, moment),
    );
  }
  if (
    !participant &&
  ...
  if (["erledigt", "nicht_bestanden", "storniert"].includes(task.status)) {
    return NextResponse.json(
      { error: "Die Aufgabe ist bereits abgeschlossen." },
      { status: 409 },
    );
  }
  ...
  const { data: evidence, error } = await service
    ...
    .select(
      "id,task_id,evidence_type,storage_path,content,verification_status,submitted_at",
    )
    .single();
  if (error || !evidence) {
    if (storagePath)
    ...
    return NextResponse.json(
      { error: "Nachweis konnte nicht dokumentiert werden." },
      { status: 500 },
    );
  }

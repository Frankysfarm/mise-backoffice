...
export default async function AssignInventoryPage() {
  ...
  const [{ data: areas }, { data: employees }, { data: locs }] = await Promise.all([
    ...
    supabase.from('employees').select('id,vorname,nachname').in('status', ['aktiv', 'in_probe', 'in_training']).order('nachname'),
    supabase.from('locations').select('id,name').order('name'),
  ]);
  return (
    <div>
      <PageHeader backHref={basePath} title="Inventur zuweisen" description="Wähle Bereich, Mitarbeiter und optional Schicht." />
      <AssignForm areas={(areas ?? []) as any[]} employees={employees ?? []} locations={(locs ?? []) as any[]} actorId={employee.id} successPath={`${basePath}/sessions`} />
    </div>
  );
}

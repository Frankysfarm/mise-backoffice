...
export function DispatchPhase5243BewertungsBoard({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-yellow-700 bg-yellow-900/30 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        <Star className="w-4 h-4 text-yellow-400 shrink-0" />
        <span className="text-sm font-semibold text-gray-200">Bewertungs-Ranking (letzte 30 Tage)</span>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-lg bg-yellow-900/50 px-3 py-2 text-center">
            ...
            <div className="flex-1 h-2 rounded-full bg-gray-800 overflow-hidden">
              <div
                className={`h-full rounded-full ${
                  f.ampel === 'gruen' ? 'bg-yellow-500' :
                  f.ampel === 'gelb'  ? 'bg-orange-500' : 'bg-red-500'
                }`}
      ...
      {data.bester_name && (
        <div className="mt-3 text-[10px] text-yellow-400 border-t border-yellow-800/40 pt-2">
          Beste Bewertung: {data.bester_name} — {data.fahrer[0]?.avg_rating.toFixed(1)} ★ Durchschnitt
        </div>
      )}
    </div>
  );

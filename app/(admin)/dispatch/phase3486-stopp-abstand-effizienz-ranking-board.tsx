...
import { MapPin, ChevronDown, ChevronUp, AlertTriangle, TrendingDown } from 'lucide-react';
...
export function DispatchPhase3486StoppAbstandEffizienzRankingBoard({
  locationId,
}: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-stopp-abstand-effizienz?location_id=${locationId}`, { cache: 'no-store' });
  ...
  const maxKm = Math.max(...data.fahrer.map(f => f.avg_abstand_km), 1);
    ...
    <div className="border rounded-lg bg-white dark:bg-gray-900 shadow-sm mb-3">
      ...
      >
        <span className="flex items-center gap-2 font-semibold text-sm">
          ...
          Stopp-Abstand-Effizienz-Ranking
          {data.alert_count > 0 && (
        ...
        <div className="px-3 pb-3 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-1.5 text-center">
            <div className="rounded bg-gray-50 dark:bg-gray-800 p-2">
              <div className="text-[9px] text-gray-400 uppercase tracking-wide">Effizientester</div>
              <div className="text-xs font-bold truncate">{data.effizientester_name}</div>
            </div>
            <div className="rounded bg-gray-50 dark:bg-gray-800 p-2">
              <div className="text-[9px] text-gray-400 uppercase tracking-wide">Team-Ø</div>
              <div className="text-xs font-bold">{data.team_avg} km</div>
            </div>
            <div className="rounded bg-gray-50 dark:bg-gray-800 p-2">
          ...
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1.5 p-1.5 rounded bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 text-xs">
              ...
              {data.alert_count} Fahrer mit hohen Stopp-Abständen — Route optimieren!
            </div>
          )}
              ...
              <div key={f.fahrer_id} className="flex items-center gap-2">
                ...
                <div className="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.round((f.avg_abstand_km / maxKm) * 100)}%` }}
                  />
                </div>
          ...
          <div className="flex items-center gap-1 text-[9px] text-gray-400 pt-0.5">
            ...
            Rang 1 = niedrigster Ø Abstand = effizienteste Bündelung · Ziel ≤2 km
          </div>
        </div>
      )}
    </div>

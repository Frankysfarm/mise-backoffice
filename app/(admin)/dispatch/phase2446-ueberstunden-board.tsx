...
function StundenBar({ h }: { h: number }) {
  const max = 12;
  const width = Math.min(100, (h / max) * 100);
  const color = h < 8 ? 'bg-green-500' : h <= 10 ? 'bg-amber-400' : 'bg-red-500';
  return (
    <div className="relative h-1.5 rounded-full bg-gray-200 w-24">
      ...
      <div className="absolute top-0 h-full border-l border-dashed border-gray-400" style={{ left: `${(8 / max) * 100}%` }} title="8h" />
      <div className="absolute top-0 h-full border-l border-dashed border-gray-600" style={{ left: `${(10 / max) * 100}%` }} title="10h" />
    </div>
  );
}
...
export function DispatchPhase2446UeberstundenBoard({ locationId }: { locationId: string | null }) {
  ...
  const sorted = data ? [...data.fahrer].sort((a, b) => b.schicht_stunden - a.schicht_stunden) : [];
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="rounded-lg bg-white border border-orange-100 p-2 text-center">
                  <div className="text-lg font-bold text-orange-700">{data.team_durchschnitt.toFixed(1)}h</div>
                  <div className="text-xs text-gray-500">Team-Ø heute</div>
                </div>
                <div className="rounded-lg bg-white border border-orange-100 p-2 text-center">
                  <div className="text-lg font-bold text-gray-700">
                    {data.fahrer.length > 0
                      ? (data.fahrer.reduce((s, f) => s + f.vw_stunden, 0) / data.fahrer.length).toFixed(1)
                      : '—'}h
                  </div>
                ...
                <div className="rounded-lg bg-white border border-green-100 p-2 text-center">
                  <div className="text-lg font-bold text-green-700">&lt;8h</div>
              ...
              {hasAlert && (
                <div className="flex items-start gap-2 bg-red-100 border border-red-200 rounded-lg p-2">
                  ...
                  <p className="text-xs text-red-800">
                    Überstunden &gt;10h: {alerts.map(f => f.fahrer_name).join(', ')} — Entlastung organisieren, Pausen prüfen!
                  </p>
                </div>
              )}

              {/* Podium Top-3 (längste Schicht) */}
              {top3.length > 0 && (
                <div className="space-y-1">
                  <div className="text-xs text-gray-500 font-medium">Längste Schicht heute</div>
                  <div className="flex gap-2">
                    {top3.map((f, i) => (
                      <div key={f.fahrer_id} className="flex-1 rounded-lg bg-white border border-gray-100 p-2 text-center">
                        ...
                        <div className={`text-sm font-bold ${f.schicht_stunden > 10 ? 'text-red-600' : f.schicht_stunden > 8 ? 'text-amber-600' : 'text-green-700'}`}>
                          {f.schicht_stunden.toFixed(1)}h
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="space-y-1.5">
                {sorted.map(f => (
                  <div key={f.fahrer_id} className={`flex items-center gap-2 rounded-lg border px-3 py-2 ${ampelColor(f.ampel)}`}>
                    ...
                    <StundenBar h={f.schicht_stunden} />
                    <span className="text-xs font-bold w-10 text-right">{f.schicht_stunden.toFixed(1)}h</span>
              ...
              <div className="flex gap-3 text-xs text-gray-500 pt-1">
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 inline-block" /> &lt;8h</span>
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" /> 8–10h</span>
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" /> &gt;10h</span>
              </div>
            </>
          )}
        </div>

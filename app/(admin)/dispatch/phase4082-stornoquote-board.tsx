...
export function DispatchPhase4082StornoquoteBoard({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-orange-50 rounded-lg p-2">
          ...
          <p className="text-sm font-bold text-orange-700">{bester?.stornoquote_pct?.toFixed(1)}%</p>
        ...
        <div className="bg-red-50 rounded-lg p-2">
          <p className="text-[10px] text-gray-500 mb-0.5">Höchste</p>
          <p className="text-sm font-bold text-red-600">{letzter?.stornoquote_pct?.toFixed(1)}%</p>
          <p className="text-[10px] text-gray-400 truncate">{data.hoechste_name}</p>
        </div>
      </div>
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.stornoquote_pct?.toFixed(1)}%</span>
              </div>
              <div className="ml-6 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${barColor}`} style={{ width: `${(f.stornoquote_pct / maxPct) * 100}%` }} />
              </div>
            </div>
          );
        })}

...
export function DispatchPhase3467TourenAuslastungRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-touren-auslastung-ranking?location_id=${locationId}`, { cache: 'no-store' });
        ...
        <div className="px-3 pb-3 space-y-2">
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1.5 p-2 rounded bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 text-xs border border-red-200">
              ...
              Niedrige Auslastung! {data.alert_count} Fahrer unter Zielwert
            </div>
          )}
          ...
          <div className="flex justify-between text-[10px] text-gray-400 pt-1">
            <span>Rang 1 = höchste Auslastung · letzte 30 Tage</span>
            <span>Ziel: ≥85%</span>
          </div>
        </div>
      )}

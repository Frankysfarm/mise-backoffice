...
function barColor(a: string) {
  if (a === 'gruen') return 'bg-slate-400';
  if (a === 'gelb') return 'bg-yellow-500';
  return 'bg-red-500';
}

export function DispatchPhase5133PausenComplianceBoard({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-600 bg-slate-950/40 overflow-hidden mb-4">
      <div className="px-4 py-3 flex items-center justify-between border-b border-slate-600/50 bg-slate-800/20">
        <div className="flex items-center gap-2">
          <Coffee className="w-4 h-4 text-slate-300" />
          <span className="text-sm font-semibold text-slate-200">Pausen-Compliance je Fahrer (letzte 30 Tage)</span>
        </div>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 divide-x divide-slate-700/40 border-b border-slate-600/30">
        <div className="px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500 mb-0.5">Höchste</div>
          <div className="text-sm font-bold text-slate-300">{bester?.compliance_pct ?? '–'} %</div>
        ...
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-slate-200">{data.team_avg_pct} %</div>
          <div className="text-[10px] text-gray-500">Compliance</div>
        </div>
        <div className="px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500 mb-0.5">Niedrigste</div>
          <div className="text-sm font-bold text-gray-400">{letzter?.compliance_pct ?? '–'} %</div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="w-20 h-1.5 rounded-full bg-gray-800 overflow-hidden shrink-0">
              <div
                ...
                style={{ width: `${Math.min(100, f.compliance_pct)}%` }}
              />
            </div>
      ...
      <div className="px-4 py-2 border-t border-slate-700/30 bg-slate-800/10">
        <div className="text-[10px] text-gray-500">
          Beste: <span className="text-slate-300 font-semibold">{data.bester_name}</span> ·

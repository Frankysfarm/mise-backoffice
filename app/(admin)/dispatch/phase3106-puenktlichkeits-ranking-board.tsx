...
export function DispatchPhase3106PuenktlichkeitsRankingBoard({ locationId }: { locationId: string | null }) {
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            ...
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="text-xs text-gray-500">{letzter ? `${letzter.rate_pct}%` : '—'}</div>
            </div>
          </div>
              ...
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
                    <div className={`h-1.5 rounded-full transition-all ${cls.bar}`} style={{ width: `${Math.max(f.rate_pct, 2)}%` }} />
                  </div>
                  {f.alert_bottom && (
                    <div className="mt-1 text-xs text-red-600 font-semibold flex items-center gap-1">
                      <AlertTriangle size={10} /> Niedrigste Pünktlichkeitsrate!
                    </div>
                  )}
                </div>
              );

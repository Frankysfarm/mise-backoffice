...
export function DispatchPhase3749ReaktionszeitRankingBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-emerald-50 rounded-lg p-2">
          <div className="text-base font-black text-emerald-600">{data.fahrer[0]?.avg_min}min</div>
          <div className="text-[10px] text-gray-500">Schnellster</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2">
          <div className="text-base font-black text-gray-800">{data.team_avg_min}min</div>
        ...
        <div className={`rounded-lg p-2 ${data.alert_count > 0 ? 'bg-red-50' : 'bg-gray-50'}`}>
          <div className={`text-base font-black ${data.alert_count > 0 ? 'text-red-600' : 'text-gray-800'}`}>
            {data.fahrer[data.fahrer.length - 1]?.avg_min}min
          </div>
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 bg-red-50 border border-red-200 rounded-lg text-xs text-red-700">
          ...
          <span>{data.alert_count} Fahrer mit hoher Reaktionszeit — Coaching empfohlen!</span>
        </div>
      )}
          ...
          return (
            <div key={f.fahrer_id} className={`rounded-lg border ${c.border} ${c.bg} p-2.5 space-y-1.5`}>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm font-black ${c.text}`}>{f.avg_min}min</span>
                {f.rank_delta !== 0 && (
                  f.rank_delta < 0
              ...
              {f.alert_hoch && (
                <div className="text-[10px] text-red-600 font-medium">Hohe Reaktionszeit!</div>
              )}
            </div>
          );
        })}
      ...
      <div className="text-[10px] text-gray-400 text-center">
        Ziel ≤{data.ziel_min}min · Rang 1=schnellste Reaktion=bester · 30-Min-Polling
      </div>
    </div>
  );
}

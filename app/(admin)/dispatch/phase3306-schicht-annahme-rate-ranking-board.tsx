...
export function DispatchPhase3306SchichtAnnahmeRateRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxRate = Math.max(...list.map(f => f.rate_pct), 1);
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-yellow-50 dark:bg-yellow-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <CheckSquare size={10} className="text-blue-500" /> Bester
              </div>
              <div className="font-bold text-sm text-yellow-700 dark:text-yellow-400">{bester?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{bester ? `${bester.rate_pct}%` : '—'}</div>
            </div>
            <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5">Team-Ø</div>
              <div className="font-bold text-base text-blue-600 dark:text-blue-400">{d.team_avg_pct}%</div>
            ...
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="text-xs text-gray-500">{niedrigster ? `${niedrigster.rate_pct}%` : '—'}</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {list.map(f => {
              ...
              const barWidth = Math.max((f.rate_pct / maxRate) * 100, 2);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className={`font-bold text-sm ${cls.text}`}>{f.rate_pct}%</span>
                      <span className="text-xs text-gray-400">({f.accepted}/{f.offered})</span>
                      {f.rank_delta > 0
                  ...
                  {f.alert_bottom && (
                    <div className="mt-1 text-xs text-red-600 dark:text-red-400 font-semibold flex items-center gap-1">
                      <AlertTriangle size={10} /> Niedrige Annahme-Rate!
                    </div>
                  )}
                </div>
              );
          ...
          <div className="flex flex-wrap gap-3 text-xs text-gray-500 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500" /> Sehr gut (≥90%)</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400" /> Mittel (70–89%)</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500"   /> Niedrig (&lt;70%)</span>
          </div>
        </div>
      )}
    </div>

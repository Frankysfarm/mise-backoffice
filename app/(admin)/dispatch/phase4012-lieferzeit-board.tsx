...
export function DispatchPhase4012LieferzeitBoard({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-emerald-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-emerald-600 font-medium">Schnellste Lieferzeit</div>
          <div className="text-xs font-bold text-emerald-700 truncate">{data.bester_name ?? bester?.fahrer_name ?? '–'}</div>
          <div className="text-[10px] text-emerald-500">{bester?.avg_min ?? 0} min</div>
        </div>
        ...
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-red-500 font-medium">Langsamste Lieferzeit</div>
          <div className="text-xs font-bold text-red-700 truncate">{data.letzter_name ?? letzter?.fahrer_name ?? '–'}</div>
          <div className="text-[10px] text-red-400">{letzter?.avg_min ?? 0} min</div>
        </div>
      </div>
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              ...
              <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full ${barColor} rounded-full transition-all`} style={{ width: `${(f.avg_min / maxMin) * 100}%` }} />
              </div>
              <span className={`text-xs font-bold w-12 text-right ${tColor}`}>{f.avg_min} min</span>
              {DeltaIcon}
            </div>
          );
        })}

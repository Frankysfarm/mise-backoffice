...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; trinkgeld_quote: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_bottom: boolean; }
...
const MOCK: ApiData = {
  fahrer: [
    ...
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  rang: 2, trinkgeld_quote:  9, rank_delta:  0, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f3', fahrer_name: 'Max M.',   rang: 3, trinkgeld_quote:  6, rank_delta: -1, ampel: 'gelb',  alert_bottom: false },
...
export function DispatchPhase4192TrinkgeldQuoteBoard({ locationId }: Props) {
  ...
  const maxQuote = Math.max(...data.fahrer.map((f) => f.trinkgeld_quote), 1);
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-amber-50 rounded-lg p-2">
          <p className="text-[10px] text-amber-600 font-medium">Höchste</p>
          <p className="text-sm font-bold text-amber-700">{bester?.trinkgeld_quote?.toFixed(1)}%</p>
        ...
        <div className="bg-gray-50 rounded-lg p-2">
          ...
          <p className="text-sm font-bold text-gray-700">{data.team_avg?.toFixed(1)}%</p>
          <p className="text-[10px] text-gray-400">Trinkgeld-Quote</p>
        </div>
        <div className="bg-red-50 rounded-lg p-2">
          <p className="text-[10px] text-red-500 font-medium">Niedrigste</p>
          <p className="text-sm font-bold text-red-600">{letzter?.trinkgeld_quote?.toFixed(1)}%</p>
      ...
      <div className="space-y-1.5">
        {data.fahrer.map((f) => {
          const barW = maxQuote > 0 ? (f.trinkgeld_quote / maxQuote) * 100 : 0;
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.trinkgeld_quote?.toFixed(1)}%</span>
              </div>

...
export function DispatchPhase4412GesamtstundenBoard({ locationId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/delivery/admin/fahrer-gesamtstunden-ranking?location_id=${locationId}`);
  ...
  const maxH    = Math.max(...data.fahrer.map(f => f.gesamt_stunden), 1);
    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-gray-900">Gesamtstunden-Ranking (30 Tage)</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          {!loading && data.alert_count > 0 && (
            <span className="flex items-center gap-0.5 text-xs text-red-600 font-semibold bg-red-50 px-2 py-0.5 rounded-full">
              <AlertTriangle className="w-3 h-3" /> {data.alert_count} Niedrige Stunden!
            </span>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        <div className="bg-blue-50 rounded-xl p-2">
          <div className="font-bold text-blue-700">{bester?.gesamt_stunden.toFixed(1)}h</div>
          <div className="text-gray-500 truncate">{data.fleissigster_name}</div>
          <div className="text-[10px] text-gray-400">Meiste Stunden</div>
        </div>
        <div className="bg-gray-50 rounded-xl p-2">
          <div className="font-bold text-gray-700">{data.team_avg_h.toFixed(1)}h</div>
          <div className="text-gray-500">{data.gesamt} Fahrer</div>
          <div className="text-[10px] text-gray-400">Team-Ø</div>
        </div>
        <div className="bg-red-50 rounded-xl p-2">
          <div className="font-bold text-red-700">{letzter?.gesamt_stunden.toFixed(1)}h</div>
          <div className="text-gray-500 truncate">{data.wenigster_name}</div>
          <div className="text-[10px] text-gray-400">Wenigste Stunden</div>
        </div>
      </div>
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-900 w-16 text-right">{f.gesamt_stunden.toFixed(1)}h</span>
              </div>
              <div className="ml-7 h-1 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${barColor}`} style={{ width: `${(f.gesamt_stunden / maxH) * 100}%` }} />
              </div>
            </div>
          );
        })}
      ...
      <div className="flex items-center justify-between text-[10px] text-gray-400 pt-1 border-t border-gray-100">
        <span>Team-Ø: {data.team_avg_h.toFixed(1)}h</span>
        <span>#1 = meiste Arbeitsstunden</span>
      </div>
    </div>
  );
}

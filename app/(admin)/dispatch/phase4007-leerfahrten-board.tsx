...
export function DispatchPhase4007LeerfahrtenBoard({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Leerfahrten-Ranking</span>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-emerald-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-emerald-600 font-medium">Wenigste Leerfahrten</div>
          <div className="text-xs font-bold text-emerald-700 truncate">{data.bester_name ?? bester?.fahrer_name ?? '–'}</div>
          <div className="text-[10px] text-emerald-500">{bester?.leerfahrten_pct ?? 0}%</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-gray-500 font-medium">Team-Ø</div>
          <div className="text-sm font-bold text-gray-700">{data.team_avg_leerfahrten_pct?.toFixed(1)}%</div>
          <div className="text-[10px] text-gray-400">Leerfahrten</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-red-500 font-medium">Meiste Leerfahrten</div>
          <div className="text-xs font-bold text-red-700 truncate">{data.letzter_name ?? letzter?.fahrer_name ?? '–'}</div>
          <div className="text-[10px] text-red-400">{letzter?.leerfahrten_pct ?? 0}%</div>
        </div>
      </div>

...
import { TrendingUp, TrendingDown, Minus, AlertTriangle, Sunrise } from 'lucide-react';
...
export function DispatchPhase4932FruehprodBoard({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-2xl border border-sky-700 bg-sky-950/40 overflow-hidden mb-4">
      {/* Header */}
      <div className="px-4 py-3 flex items-center justify-between border-b border-sky-700/50 bg-sky-900/20">
        <div className="flex items-center gap-2">
          <Sunrise className="w-4 h-4 text-sky-300" />
          <span className="text-sm font-semibold text-sky-200">Frühschicht-Produktivität (06–11 Uhr)</span>
        </div>
        {data.alert_count > 0 && (
      ...
      {/* KPI Grid */}
      <div className="grid grid-cols-3 divide-x divide-sky-800/50 border-b border-sky-800/50">
        {[
          ...
          <div key={k.label} className="flex flex-col items-center py-3 gap-0.5">
            <span className="text-sm font-bold text-sky-100">{k.val}</span>
            <span className="text-[10px] text-sky-400">{k.label}</span>
          ...
          <div key={f.fahrer_id} className="px-4 py-2.5 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {f.alert_hoch && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-sky-900/70 text-sky-300">
                    &gt;3,0 T/h
                  </span>
                )}
              </div>
              <div className="mt-1 h-1.5 bg-sky-900/50 rounded-full overflow-hidden">
                <div
      ...
      <div className="px-4 py-2 border-t border-sky-800/30 flex items-center justify-between">

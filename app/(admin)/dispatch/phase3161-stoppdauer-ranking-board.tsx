...
export function DispatchPhase3161StoppdauerRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const minSec  = list.length > 0 ? Math.min(...list.map(f => f.avg_sec)) : 0;
  const maxSec  = list.length > 0 ? Math.max(...list.map(f => f.avg_sec)) : 1;
  const rangeSec = Math.max(maxSec - minSec, 1);
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-yellow-50 dark:bg-yellow-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <Timer size={10} className="text-purple-500" /> Bester
              </div>
              <div className="font-bold text-sm text-yellow-700 dark:text-yellow-400">{bester?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{bester ? fmtSec(bester.avg_sec) : '—'}</div>
            </div>
            <div className="rounded-lg bg-purple-50 dark:bg-purple-900/20 p-2">
              ...
              <div className="font-bold text-base text-purple-600 dark:text-purple-400">{fmtSec(teamAvg)}</div>
            </div>
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="text-xs text-gray-500">{letzter ? fmtSec(letzter.avg_sec) : '—'}</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {list.map(f => {
              ...
              // Inverted bar: shorter duration = longer bar
              const barW = Math.round(((maxSec - f.avg_sec) / rangeSec) * 100);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`font-bold text-sm ${cls.text}`}>{fmtSec(f.avg_sec)}</span>

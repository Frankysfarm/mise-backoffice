...
export function DispatchPhase5227TourenProSchichtBoard({ locationId }: { locationId: string | null }) {
  const [data, setData] = useState<ApiResponse | null>(null);

  async function load() {
    const url = locationId
      ? `/api/delivery/admin/fahrer-touren-pro-schicht-ranking?location_id=${locationId}`
      : '/api/delivery/admin/fahrer-touren-pro-schicht-ranking';
  ...
  return (
    <div className="rounded-xl border border-indigo-700 bg-indigo-900/30 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-gray-200">Ø Touren/Schicht-Ranking</span>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-lg bg-indigo-900/50 px-3 py-2 text-center">
          ...
          <div className="text-xs font-bold text-indigo-300 truncate">{data.bester_name}</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500">Team-Ø</div>
          <div className="text-xs font-bold text-gray-200">{data.team_avg.toFixed(1)}/Schicht</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
        ...
        {data.fahrer.slice(0, 6).map(f => (
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-400 w-16 text-right tabular-nums">
              {f.touren_pro_schicht.toFixed(1)}/Sch.
            </span>
      ...
      <div className="mt-3 text-[10px] text-indigo-400 border-t border-indigo-800/40 pt-2">
        Ziel: {data.ziel.toFixed(1)} Touren/Schicht · Meiste: {data.bester_name} — {data.fahrer[0]?.touren_pro_schicht.toFixed(1)}/Schicht
      </div>
    </div>
  );
}

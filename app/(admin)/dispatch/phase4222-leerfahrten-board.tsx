...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; leerfahrten_pct: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_bottom: boolean; }
interface ApiData { fahrer: FahrerRow[]; team_avg_leerfahrten_pct: number; bester_name: string; letzter_name: string; alert_count: number; gesamt: number; }
...
export function DispatchPhase4222LeerfahrtenBoard({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Leerfahrten-Quote</span>
        ...
        {data.alert_count > 0 && (
          <span className="flex items-center gap-1 text-xs text-red-600 font-semibold bg-red-50 px-2 py-0.5 rounded-full">
            <AlertTriangle className="w-3.5 h-3.5" /> {data.alert_count} Hohe Leerfahrten!
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-orange-50 rounded-lg p-2">
          <p className="text-[10px] text-orange-600 font-medium">Niedrigste</p>
          <p className="text-sm font-bold text-orange-700">{bester?.leerfahrten_pct}%</p>
        ...
        <div className="bg-gray-50 rounded-lg p-2">
          ...
          <p className="text-sm font-bold text-gray-700">{data.team_avg_leerfahrten_pct}%</p>
          <p className="text-[10px] text-gray-400">Leerfahrten-km</p>
        </div>
        <div className="bg-red-50 rounded-lg p-2">
          <p className="text-[10px] text-red-500 font-medium">Höchste</p>
          <p className="text-sm font-bold text-red-600">{letzter?.leerfahrten_pct}%</p>
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.leerfahrten_pct}%</span>
              </div>

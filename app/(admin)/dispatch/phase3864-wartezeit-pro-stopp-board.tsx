...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 1, avg_wartezeit_min: 3.2, trend_delta: -0.3, ampel: 'gruen', alert: null },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  rang: 2, avg_wartezeit_min: 5.1, trend_delta:  0.2, ampel: 'gelb',  alert: null },
    { fahrer_id: 'f3', fahrer_name: 'Max M.',   rang: 3, avg_wartezeit_min: 7.8, trend_delta:  0.5, ampel: 'rot',   alert: 'Lange Wartezeit!' },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, avg_wartezeit_min: 12.4, trend_delta: 1.3, ampel: 'rot',   alert: 'Lange Wartezeit!' },
  ],
  team_avg_wartezeit_min: 7.1,
  alert_count: 2,
};

export function DispatchPhase3864WartezeitProStoppBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/delivery/admin/fahrer-wartezeit-stopp?location_id=${locationId}`);
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-gray-900">Wartezeit/Stopp</h3>
          {loading && <span className="w-3 h-3 border-2 border-red-400 border-t-transparent rounded-full animate-spin" />}
        </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-emerald-50 rounded-lg p-2 text-center">
          ...
          <div className="text-lg font-black text-emerald-700">{best?.avg_wartezeit_min ?? '—'}</div>
        ...
        <div className="bg-blue-50 rounded-lg p-2 text-center">
          ...
          <div className="text-lg font-black text-blue-700">{data.team_avg_wartezeit_min}</div>
          <div className="text-[10px] text-gray-600">min</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-gray-500 mb-0.5">Längste</div>
          <div className="text-lg font-black text-red-600">{worst?.avg_wartezeit_min ?? '—'}</div>
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-red-50 border border-red-200 rounded-lg text-[11px] text-red-700">
          ...
          <span>{data.alert_count} Fahrer mit langen Wartezeiten!</span>
        </div>
      )}
      ...
      <div className="text-[10px] text-gray-400 border-t border-gray-100 pt-1.5 text-right">
        Ø Wartezeit Ankunft → Abfahrt je Stopp
      </div>
    </div>
  );
}

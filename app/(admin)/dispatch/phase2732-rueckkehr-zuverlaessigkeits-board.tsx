...
function RateBalken({ rate, barClass }: { rate: number; barClass: string }) {
  const fill    = Math.min(100, (rate / MAX) * 100);
...
const MOCK: ApiData = {
  fahrer: [
    ...
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  rate: 80, puenktlich: 16, gesamt: 20, trend: 'stabil',   alert: null,                      rang: 2 },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   rate: 75, puenktlich: 15, gesamt: 20, trend: 'fallend',  alert: null,                      rang: 3 },
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', rate: 60, puenktlich: 12, gesamt: 20, trend: 'fallend',  alert: 'Rückkehr unzuverlässig!', rang: 4 },
  ],
  team_avg_rate: 77.5,
};

export function DispatchPhase2732RueckkehrZuverlaessigkeitsBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-4`}>
      ...
      >
        <div className="flex items-center gap-2">
          <RotateCcw size={16} className={hasAlert ? 'text-red-500' : 'text-blue-600'} />
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø', value: `${data.team_avg_rate.toFixed(1)}%`, cls: teamCls.text },
              { label: 'Bester', value: bester ? `${bester.rate}%` : '–',    cls: 'text-green-600' },
          ...
          <div className="flex items-center gap-3 text-xs text-gray-500">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" /> {WARN}–{ZIEL}%</span>
                ...
                <div key={f.fahrer_id} className={`rounded-lg border ${cls.bg} px-3 py-2`}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`text-xs font-bold ${cls.text}`}>{f.rate}%</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <RateBalken rate={f.rate} barClass={cls.bar} />
                    <span className="text-[10px] text-gray-400 w-28 text-right">
                      {f.puenktlich}/{f.gesamt} Schichten
                    </span>
                  </div>
                </div>
              );

...
export function DispatchPhase3679WartezeitProStoppRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxWait = Math.max(...sorted.map(f => f.wartezeit_min), 1);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900">Wartezeit/Stopp — Ranking</h3>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        <div className="bg-emerald-50 rounded-lg p-2">
          <div className="font-bold text-emerald-700">{schnellster?.wartezeit_min.toFixed(1)} min</div>
          <div className="text-gray-500">Schnellster</div>
          <div className="text-gray-700 truncate">{schnellster?.fahrer_name ?? '–'}</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2">
          <div className="font-bold text-gray-700">{data.team_avg.toFixed(1)} min</div>
          <div className="text-gray-500">Team-Ø</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2">
          <div className="font-bold text-red-700">{langsamster?.wartezeit_min.toFixed(1)} min</div>
          ...
          <div key={f.fahrer_id} className={`flex items-center gap-2 p-2 rounded-lg border ${AMPEL_BG[f.ampel]}`}>
            ...
            <div className="w-20 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${Math.max((f.wartezeit_min / maxWait) * 100, 4)}%` }}
              />
            </div>
            <span className={`text-xs font-bold w-14 text-right ${AMPEL_COLOR[f.ampel]}`}>{f.wartezeit_min.toFixed(1)} min</span>
            {f.rank_delta < 0 ? (
      ...
      <div className="text-xs text-gray-400 text-center">Ziel ≤3 min/Stopp · Letzte 30 Tage</div>
    </div>
  );
}

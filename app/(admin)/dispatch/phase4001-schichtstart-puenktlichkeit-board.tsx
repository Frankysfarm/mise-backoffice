...
export function DispatchPhase4001SchichtstartPuenktlichkeitBoard({ locationId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/delivery/admin/fahrer-schichtstart-ranking?location_id=${locationId}`);
      if (res.ok) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Schichtstart-Pünktlichkeit</span>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-emerald-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-emerald-600 font-medium">Pünktlichster</div>
          <div className="text-xs font-bold text-emerald-700 truncate">{data.bester_name ?? bester?.fahrer_name ?? '–'}</div>
          <div className="text-[10px] text-emerald-500">{bester?.puenktlichkeit_pct ?? 0}%</div>
        </div>
        ...
        <div className="bg-red-50 rounded-lg p-2 text-center">
          ...
          <div className="text-xs font-bold text-red-700 truncate">{data.niedrigster_name ?? niedrigster?.fahrer_name ?? '–'}</div>
          <div className="text-[10px] text-red-400">{niedrigster?.puenktlichkeit_pct ?? 0}%</div>
        </div>
      </div>
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              ...
              <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full ${barColor} rounded-full transition-all`} style={{ width: `${f.puenktlichkeit_pct}%` }} />
              </div>
              <span className={`text-xs font-bold w-10 text-right ${tColor}`}>{f.puenktlichkeit_pct}%</span>
              {DeltaIcon}
            </div>
          );
        })}

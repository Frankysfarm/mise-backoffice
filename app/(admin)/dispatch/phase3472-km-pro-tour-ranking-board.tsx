...
import { ChevronDown, ChevronUp, TrendingUp, TrendingDown, Minus, AlertTriangle, Route } from 'lucide-react';
...
export function DispatchPhase3472KmProTourRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-km-pro-tour?location_id=${locationId}`, { cache: 'no-store' });
        ...
        <div className="px-3 pb-3 space-y-2">
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1.5 p-2 rounded bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 text-xs border border-red-200">
              ...
              Hohe km/Tour! {data.alert_count} Fahrer über Zielwert
            </div>
          )}

          <div className="grid grid-cols-3 gap-2 py-1">
            <div className="text-center">
              <div className="text-[10px] text-gray-500 uppercase">Effizientester</div>
              <div className="text-xs font-semibold text-emerald-600 truncate">{data.effizientester_name}</div>
            </div>
            <div className="text-center">
              <div className="text-[10px] text-gray-500 uppercase">Team-Ø</div>
              <div className="text-xs font-semibold">{data.team_avg} km</div>
            </div>
            <div className="text-center">
          ...
          <div className="flex justify-between text-[10px] text-gray-400 pt-1">
            <span>Rang 1 = niedrigste km/Tour (effizientester) · Ø letzte 30 Tage</span>

...
import { XCircle, AlertTriangle, ChevronDown, ChevronUp, Minus, TrendingDown, TrendingUp } from 'lucide-react';
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', quote_pct: 2.1, rang: 1, trend_delta: -0.5, ampel: 'gruen' },
    { fahrer_id: 'f2', fahrer_name: 'Max M.',   quote_pct: 6.3, rang: 2, trend_delta:  0.8, ampel: 'gelb'  },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  quote_pct: 8.9, rang: 3, trend_delta:  1.2, ampel: 'gelb'  },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   quote_pct: 14.2, rang: 4, trend_delta: 3.1, ampel: 'rot'   },
  ],
...
export function DispatchPhase3286StornoQuoteRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxQuote = Math.max(...list.map(f => f.quote_pct), 1);
  const bester      = list.find(f => f.rang === 1);
  const hoechster   = list[list.length - 1];
        ...
        <div className="p-4 space-y-3">
          {alerts.length > 0 && (
            <div className="flex items-center gap-2 rounded-lg bg-red-50 border border-red-200 text-red-700 dark:text-red-300 dark:bg-red-950 dark:border-red-800 text-xs font-semibold px-3 py-2">
              ...
              {alerts.map(f => f.fahrer_name).join(', ')} — Hohe Storno-Quote!
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-yellow-50 dark:bg-yellow-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <XCircle size={10} className="text-red-500" /> Bester
              </div>
            ...
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5">Höchste Quote</div>
              <div className="font-bold text-sm text-gray-600 dark:text-gray-300">{hoechster?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{hoechster ? `${hoechster.quote_pct.toFixed(1)}%` : '—'}</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {list.map(f => {
              ...
              const barWidth = maxQuote > 0 ? Math.max((f.quote_pct / maxQuote) * 100, 2) : 2;
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  ...
                  {f.ampel === 'rot' && (
                    <div className="mt-1 text-xs text-red-600 dark:text-red-400 font-semibold flex items-center gap-1">
                      <AlertTriangle size={10} /> Hohe Storno-Quote!
                    </div>
                  )}
                </div>
              );
          ...
          <div className="flex flex-wrap gap-3 text-xs text-gray-500 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500" /> Niedrig (&lt;5%)</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400" /> Mittel (5–10%)</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500"   /> Hoch (&gt;10%)</span>
          </div>
        </div>
      )}
    </div>

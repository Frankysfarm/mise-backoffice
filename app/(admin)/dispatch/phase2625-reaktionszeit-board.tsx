...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f3', fahrer_name: 'Tim W.',   avg_min: 8.2, touren_heute:  7, trend: 'steigend', trend_delta:  1.7, rang: 1 },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  avg_min: 4.5, touren_heute:  9, trend: 'fallend',  trend_delta: -0.5, rang: 2 },
    { fahrer_id: 'f4', fahrer_name: 'Anna B.',  avg_min: 3.1, touren_heute: 11, trend: 'stabil',   trend_delta:  0.3, rang: 3 },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   avg_min: 2.1, touren_heute: 12, trend: 'fallend',  trend_delta: -0.7, rang: 4 },
  ],
  team_durchschnitt: 4.5,
  alert_count: 1,
};

export function DispatchPhase2625ReactionsZeitBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-4`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800">Reaktionszeit Board</span>
          {hasAlert && (

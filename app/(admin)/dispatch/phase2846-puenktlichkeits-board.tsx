...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Anna K.',  quote_pct: 96, puenktlich: 24, gesamt_stopps: 25, ampel: 'gruen', trend: 'steigend', trend_delta: 3,  rang: 1 },
    { fahrer_id: 'd2', fahrer_name: 'Max M.',   quote_pct: 91, puenktlich: 20, gesamt_stopps: 22, ampel: 'gruen', trend: 'stabil',   trend_delta: 0,  rang: 2 },
    { fahrer_id: 'd3', fahrer_name: 'Sara K.',  quote_pct: 78, puenktlich: 14, gesamt_stopps: 18, ampel: 'gelb',  trend: 'fallend',  trend_delta: -4, rang: 3 },
    { fahrer_id: 'd4', fahrer_name: 'Tim W.',   quote_pct: 62, puenktlich:  8, gesamt_stopps: 13, ampel: 'rot',   trend: 'fallend',  trend_delta: -8, rang: 4 },
  ],
  team_durchschnitt: 81.8,
};

export function DispatchPhase2846PuenktlichkeitsBoard({ locationId }: { locationId: string | null }) {
  ...
  const best     = sorted[0]?.quote_pct ?? 0;
    ...
    <div className={`rounded-xl border p-4 mb-4 ${hasAlert ? 'border-red-300 bg-red-50' : 'border-gray-200 bg-white'}`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800">Pünktlichkeit Fahrer</span>
      ...
      {open && (
        <div className="mt-3 space-y-3">
            ...
            <div className="bg-red-100 border border-red-300 rounded-lg p-2 space-y-1">
              {alerts.map(f => (
                <div key={f.fahrer_id} className="flex items-center gap-2 text-xs text-red-700">
                  ...
                  <span>— Niedrige Pünktlichkeit! ({f.quote_pct.toFixed(1)}% / Ziel ≥{ZIEL_PCT}%)</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø', val: `${data.team_durchschnitt.toFixed(1)}%` },
              { label: 'Bester', val: `${best.toFixed(1)}%` },
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${a.bg}`}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-500">
                      <span>{f.puenktlich}/{f.gesamt_stopps}</span>
                      <span className={`font-bold ${a.text}`}>{f.quote_pct.toFixed(1)}%</span>
                    </div>
                  </div>
          ...
          <div className="flex gap-3 text-[10px] text-gray-500">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400" /> {WARN_PCT}–{ZIEL_PCT - 1}%</span>

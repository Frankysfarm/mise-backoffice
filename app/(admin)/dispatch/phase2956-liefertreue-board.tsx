...
interface ApiData {
  ...
  team_avg_liefertreue_gestern: number;
  alert_count: number;
}
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Max M.',   liefertreue_pct: 95, liefertreue_gestern: 92, puenktlich_heute: 19, gesamt_heute: 20, trend: 'steigend', trend_delta:  3, ampel: 'gruen', alert: false },
    { fahrer_id: 'd3', fahrer_name: 'Tim B.',   liefertreue_pct: 85, liefertreue_gestern: 80, puenktlich_heute: 17, gesamt_heute: 20, trend: 'steigend', trend_delta:  5, ampel: 'gelb',  alert: false },
    { fahrer_id: 'd4', fahrer_name: 'Julia F.', liefertreue_pct: 72, liefertreue_gestern: 68, puenktlich_heute: 18, gesamt_heute: 25, trend: 'steigend', trend_delta:  4, ampel: 'gelb',  alert: false },
    { fahrer_id: 'd2', fahrer_name: 'Sara K.',  liefertreue_pct: 60, liefertreue_gestern: 75, puenktlich_heute: 12, gesamt_heute: 20, trend: 'fallend',  trend_delta: -15, ampel: 'rot',  alert: true  },
  ],
  team_avg_liefertreue: 78,
  team_avg_liefertreue_gestern: 79,
  alert_count: 1,
};

export function DispatchPhase2956LiefertreueBoard({ locationId }: { locationId: string | null }) {
  ...
  const alerts    = enriched.filter(f => f.liefertreue_pct < ALERT_SCHWELLE);
  const hasAlert  = alerts.length > 0;
  const best      = sorted[0]?.liefertreue_pct ?? 0;
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Liefertreue-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø',  value: `${data.team_avg_liefertreue.toFixed(1)}%`, cls: teamText },
              { label: 'Bester',  value: `${best}%`,                                cls: 'text-green-600' },
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.liefertreue_pct}%</span>
                    </div>
                  </div>
                  ...
                  <div className="text-xs text-gray-400 mt-1">{f.puenktlich_heute}/{f.gesamt_heute} pünktlich heute</div>
                </div>
              );
            })}
          </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />70–89%</span>

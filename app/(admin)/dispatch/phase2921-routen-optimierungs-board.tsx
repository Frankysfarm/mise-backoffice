...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd4', fahrer_name: 'Julia F.', score: 96, score_vw: 94, trend: 'steigend', trend_delta:  2, ampel: 'gruen', alert_ineffizient: false },
    { fahrer_id: 'd1', fahrer_name: 'Max M.',   score: 92, score_vw: 93, trend: 'fallend',  trend_delta: -1, ampel: 'gruen', alert_ineffizient: false },
    { fahrer_id: 'd2', fahrer_name: 'Sara K.',  score: 82, score_vw: 80, trend: 'steigend', trend_delta:  2, ampel: 'gelb',  alert_ineffizient: false },
    { fahrer_id: 'd3', fahrer_name: 'Tim B.',   score: 61, score_vw: 66, trend: 'fallend',  trend_delta: -5, ampel: 'rot',   alert_ineffizient: true  },
  ],
  team_avg_score: 83,
  team_avg_score_vw: 83,
  alert_count: 1,
};

export function DispatchPhase2921RoutenOptimierungsBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Routen-Optimierungs-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 text-sm">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: Ineffiziente Route! Score {f.score} Pkt
              </span>
            </div>
          ))}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø',  value: `${data.team_avg_score} Pkt`, cls: teamText },
              { label: 'Bester',  value: `${best} Pkt`,                cls: 'text-green-600' },
              { label: 'Ziel',    value: `≥${ZIEL} Pkt`,              cls: 'text-gray-600 dark:text-gray-400' },
            ].map(k => (
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.score} Pkt</span>
                    </div>
                  </div>

...
export function DispatchPhase3689TrinkgeldQuoteRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxQuote = Math.max(...sorted.map(f => f.trinkgeld_quote), 1);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900">Trinkgeld-Quote — Ranking</h3>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        <div className="bg-emerald-50 rounded-lg p-2">
          <div className="font-bold text-emerald-700">{bester?.trinkgeld_quote.toFixed(1)}%</div>
        ...
        <div className="bg-red-50 rounded-lg p-2">
          <div className="font-bold text-red-700">{schlechtester?.trinkgeld_quote.toFixed(1)}%</div>
          ...
          <div key={f.fahrer_id} className={`flex items-center gap-2 p-2 rounded-lg border ${AMPEL_BG[f.ampel]}`}>
            ...
            <div className="w-20 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${Math.max((f.trinkgeld_quote / maxQuote) * 100, 4)}%` }}
              />
            </div>
            <span className={`text-xs font-bold w-12 text-right ${AMPEL_COLOR[f.ampel]}`}>{f.trinkgeld_quote.toFixed(1)}%</span>
            {f.rank_delta > 0 ? (
      ...
      <div className="text-xs text-gray-400 text-center">Ziel ≥8% · Letzte 30 Tage</div>
    </div>
  );
}

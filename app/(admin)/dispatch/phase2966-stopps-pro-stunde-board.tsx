...
interface ApiData {
  ...
  team_avg_stopps_pro_stunde_gestern: number;
  alert_count: number;
}
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Max M.',   stopps_pro_stunde: 6.2, stopps_pro_stunde_gestern: 5.8, stopps_heute: 37, trend: 'steigend', trend_delta:  0.4, ampel: 'gruen', alert: false },
    { fahrer_id: 'd4', fahrer_name: 'Julia F.', stopps_pro_stunde: 5.7, stopps_pro_stunde_gestern: 5.2, stopps_heute: 34, trend: 'steigend', trend_delta:  0.5, ampel: 'gruen', alert: false },
    { fahrer_id: 'd3', fahrer_name: 'Tim B.',   stopps_pro_stunde: 4.1, stopps_pro_stunde_gestern: 3.9, stopps_heute: 25, trend: 'steigend', trend_delta:  0.2, ampel: 'gelb',  alert: false },
    { fahrer_id: 'd2', fahrer_name: 'Sara K.',  stopps_pro_stunde: 2.4, stopps_pro_stunde_gestern: 3.1, stopps_heute: 14, trend: 'fallend',  trend_delta: -0.7, ampel: 'rot',   alert: true  },
  ],
  team_avg_stopps_pro_stunde: 4.6,
  team_avg_stopps_pro_stunde_gestern: 4.5,
  alert_count: 1,
};

export function DispatchPhase2966StoppsProStundeBoard({ locationId }: { locationId: string | null }) {
  ...
  const alerts   = enriched.filter(f => f.stopps_pro_stunde < ALERT_SPH);
  const hasAlert = alerts.length > 0;
  const best     = sorted[0]?.stopps_pro_stunde ?? 0;
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Stopps/h-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø', value: `${data.team_avg_stopps_pro_stunde.toFixed(1)}/h`, cls: teamText },
              { label: 'Bester', value: `${best.toFixed(1)}/h`,                             cls: 'text-green-600' },
              { label: 'Ziel',   value: `≥${ZIEL_SPH}/h`,                                  cls: 'text-gray-600 dark:text-gray-400' },
            ].map(k => (
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.stopps_pro_stunde.toFixed(1)} /h</span>
                    </div>
                  </div>
                  ...
                  <div className="text-xs text-gray-400 mt-1">{f.stopps_heute} Stopps heute</div>
                </div>
              );
            })}
          </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 inline-block" />≥5/h</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />3–4/h</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />&lt;3/h</span>
            <span className="text-gray-400">| Ziel ≥{ZIEL_SPH}/h</span>
          </div>
        </div>
      )}
    </div>

...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   avg_min: 22.5, trend: 'fallend',  trend_delta: -2.1, ampel: 'gruen', alert_zu_lang: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', avg_min: 24.1, trend: 'stabil',   trend_delta:  0.2, ampel: 'gruen', alert_zu_lang: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  avg_min: 27.8, trend: 'steigend', trend_delta:  1.5, ampel: 'gelb',  alert_zu_lang: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   avg_min: 38.2, trend: 'steigend', trend_delta:  3.4, ampel: 'rot',   alert_zu_lang: true  },
  ],
  team_avg_min: 28.2,
  alert_count: 1,
};

export function DispatchPhase3026DurchschnittsLieferzeitBoard({ locationId }: { locationId: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-durchschnitts-lieferzeit?location_id=${locationId}`)
        .then(r => r.json())
  ...
  const alerts   = data.fahrer.filter(f => f.alert_zu_lang);
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Ø Lieferzeit-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 text-sm">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: Zu lange Lieferzeit! ({f.avg_min.toFixed(1)} Min)
              </span>
            </div>
          ))}
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 inline-block" />≤25 Min</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />25–35 Min</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />&gt;35 Min</span>

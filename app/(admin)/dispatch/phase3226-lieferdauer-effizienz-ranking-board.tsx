...
export function DispatchPhase3226LieferdauerEffizienzRankingBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 shadow-sm space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Lieferdauer-Effizienz-Ranking</span>
        </div>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        ...
        <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-2">
          ...
          <div className="font-semibold text-blue-700 dark:text-blue-300">{data.team_avg_pct}%</div>
        </div>
      ...
      <div className="space-y-2">
        {data.fahrer.map(f => (
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <span className={`text-xs font-semibold w-10 text-right ${f.ampel === 'gruen' ? 'text-green-600' : f.ampel === 'gelb' ? 'text-yellow-500' : 'text-red-500'}`}>
              {f.effizienz_pct}%
            </span>

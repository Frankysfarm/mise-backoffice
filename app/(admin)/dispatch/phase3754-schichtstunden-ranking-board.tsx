...
export function DispatchPhase3754SchichtstundenRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxH = Math.max(...data.fahrer.map(f => f.avg_stunden), 1);
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-900 text-sm">Schichtstunden-Ranking</span>
          {loading && <span className="w-3 h-3 border-2 border-blue-400 border-t-transparent rounded-full animate-spin" />}
        </div>
        <div className="text-xs text-gray-500">
          Fleißigster: <span className="font-bold text-gray-800">{data.fleissigster_name}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-emerald-50 rounded-lg p-2">
          <div className="text-base font-black text-emerald-600">{data.fahrer[0]?.avg_stunden}h</div>
          <div className="text-[10px] text-gray-500">Fleißigster</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2">
          <div className="text-base font-black text-gray-800">{data.team_avg_stunden}h</div>
        ...
        <div className={`rounded-lg p-2 ${data.alert_count > 0 ? 'bg-red-50' : 'bg-gray-50'}`}>
          <div className={`text-base font-black ${data.alert_count > 0 ? 'text-red-600' : 'text-gray-800'}`}>
            {data.fahrer[data.fahrer.length - 1]?.avg_stunden}h
          </div>
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 bg-red-50 border border-red-200 rounded-lg text-xs text-red-700">
          ...
          <span>{data.alert_count} Fahrer mit wenigen Schichtstunden — bitte prüfen!</span>
        </div>
      )}
      ...
      <div className="space-y-2">
        {data.fahrer.map(f => {
          ...
          const barPct = (f.avg_stunden / maxH) * 100;
          return (
            <div key={f.fahrer_id} className={`rounded-lg border ${c.border} ${c.bg} p-2.5 space-y-1.5`}>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm font-black ${c.text}`}>{f.avg_stunden}h</span>
                {f.rank_delta !== 0 && (
                  f.rank_delta < 0
              ...
              {f.alert_wenig && (
                <div className="text-[10px] text-red-600 font-medium">Wenige Schichtstunden!</div>
              )}
            </div>
          );
        })}
      ...
      <div className="text-[10px] text-gray-400 text-center">
        Ziel ≥{data.ziel_stunden}h · Rang 1=meiste Stunden=bester · 30-Min-Polling
      </div>
    </div>
  );
}

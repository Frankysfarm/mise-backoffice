...
export function DispatchPhase5362ReaktionszeitBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxVal = ranking[ranking.length - 1]?.avg_reaktionszeit_min ?? 1;
  ...
  return (
    <div className="rounded-xl border border-violet-700 bg-violet-900/20 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-gray-200">Reaktionszeit-Ranking (30 Tage)</span>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-lg bg-violet-900/40 px-3 py-2 text-center">
          ...
          <div className="text-xs font-bold text-violet-300 truncate">{data.schnellster_name}</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500">Team-Ø</div>
          <div className="text-xs font-bold text-violet-300">{data.team_avg_min.toFixed(1)} min</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 h-2 rounded-full bg-gray-800 overflow-hidden">
              <div
                ...
                style={{ width: `${Math.round((f.avg_reaktionszeit_min / maxVal) * 100)}%` }}
              />
            </div>
            <span className="text-xs font-bold tabular-nums text-violet-300 w-14 text-right">{f.avg_reaktionszeit_min.toFixed(1)} min</span>
      ...
      <div className="mt-2 text-[9px] text-gray-600 text-right">{data.gesamt} Fahrer · Reaktionszeit aufsteigend · 30-Min-Polling</div>
    </div>
  );
}

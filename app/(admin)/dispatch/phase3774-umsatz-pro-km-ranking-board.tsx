...
export function DispatchPhase3774UmsatzProKmRankingBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-900 text-sm">Umsatz/km-Ranking</span>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-emerald-50 rounded-lg p-2">
          <div className="text-base font-black text-emerald-600">€{data.fahrer[0]?.umsatz_pro_km.toFixed(2)}</div>
          <div className="text-[10px] text-gray-500">Bester</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2">
          <div className="text-base font-black text-gray-800">€{data.team_avg_umsatz_pro_km.toFixed(2)}</div>
        ...
        <div className={`rounded-lg p-2 ${data.alert_count > 0 ? 'bg-red-50' : 'bg-gray-50'}`}>
          <div className={`text-base font-black ${data.alert_count > 0 ? 'text-red-600' : 'text-gray-800'}`}>
            €{data.fahrer[data.fahrer.length - 1]?.umsatz_pro_km.toFixed(2)}
          </div>
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 bg-red-50 border border-red-200 rounded-lg text-xs text-red-700">
          ...
          <span>{data.alert_count} Fahrer mit niedrigem Umsatz/km — bitte prüfen!</span>
        </div>
      )}
          ...
          return (
            <div key={f.fahrer_id} className={`rounded-lg border ${c.border} ${c.bg} p-2.5 space-y-1.5`}>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm font-black ${c.text}`}>€{f.umsatz_pro_km.toFixed(2)}/km</span>
                {f.rank_delta !== 0 && (
                  f.rank_delta > 0
              ...
              {f.alert_niedrig && (
                <div className="text-[10px] text-red-600 font-medium">Niedriger Umsatz/km!</div>
              )}
            </div>
          );
        })}
      ...
      <div className="text-[10px] text-gray-400 text-center">
        Ziel ≥€{data.ziel_umsatz_pro_km.toFixed(2)}/km · Rang 1=höchster Wert=bester · 30-Min-Polling
      </div>
    </div>
  );
}

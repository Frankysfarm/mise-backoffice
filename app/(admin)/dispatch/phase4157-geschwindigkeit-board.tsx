...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; avg_kmh: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_langsam: boolean; }
interface ApiData { fahrer: FahrerRow[]; team_avg_kmh: number; schnellster_name: string; langsamster_name: string; alert_count: number; gesamt: number; ziel_kmh: number; }
...
export function DispatchPhase4157GeschwindigkeitBoard({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Ø Geschwindigkeit</span>
        ...
        {data.alert_count > 0 && (
          <span className="flex items-center gap-1 text-xs text-red-600 font-semibold bg-red-50 px-2 py-0.5 rounded-full">
            <AlertTriangle className="w-3.5 h-3.5" /> {data.alert_count} Niedrige Geschw.!
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-sky-50 rounded-lg p-2">
          <p className="text-[10px] text-sky-600 font-medium">Schnellste</p>
          <p className="text-sm font-bold text-sky-700">{schnellster?.avg_kmh?.toFixed(0)} km/h</p>
          <p className="text-[10px] text-sky-500 truncate">{data.schnellster_name}</p>
        </div>
        <div className="bg-gray-50 rounded-lg p-2">
          <p className="text-[10px] text-gray-500 font-medium">Team-Ø</p>
          <p className="text-sm font-bold text-gray-700">{data.team_avg_kmh?.toFixed(0)} km/h</p>
          <p className="text-[10px] text-gray-400">Ziel ≥{data.ziel_kmh?.toFixed(0)}</p>
        </div>
        <div className="bg-red-50 rounded-lg p-2">
          <p className="text-[10px] text-red-500 font-medium">Langsamste</p>
          <p className="text-sm font-bold text-red-600">{langsamster?.avg_kmh?.toFixed(0)} km/h</p>
      ...
      <div className="space-y-1.5">
        {data.fahrer.map((f) => {
          const barW = maxKmh > 0 ? (f.avg_kmh / maxKmh) * 100 : 0;
          const barColor = f.ampel === 'gruen' ? 'bg-sky-400' : f.ampel === 'gelb' ? 'bg-yellow-400' : 'bg-red-400';
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.avg_kmh?.toFixed(0)} km/h</span>
              </div>
      ...
      <div className="flex justify-between text-[10px] text-gray-400 pt-1 border-t border-gray-100">
        <span>Team-Ø {data.team_avg_kmh?.toFixed(0)} km/h · Ziel ≥{data.ziel_kmh?.toFixed(0)}</span>

...
import { ChevronDown, ChevronUp, BarChart2, TrendingUp, TrendingDown, Minus, AlertTriangle } from 'lucide-react';
...
export function DispatchPhase3442MultiStoppEffizienzRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-multi-stopp-effizienz?location_id=${locationId}`, { cache: 'no-store' });
  ...
  const maxStopps = Math.max(...data.fahrer.map(f => f.stopps_pro_tour), 1);
        ...
        <div className="px-3 pb-3 space-y-2">
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1.5 p-2 rounded bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 text-xs border border-red-200">
              ...
              Wenig Stopps/Tour! {data.alert_count} Fahrer unter Zielwert
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2 py-1">
            ...
            <div className="text-center">
              ...
              <div className="text-xs font-semibold">{data.team_avg} Stopps/Tour</div>
            </div>
            <div className="text-center">

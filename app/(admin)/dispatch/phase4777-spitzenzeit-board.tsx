...
export function DispatchPhase4777SpitzenzeitBoard({ locationId }: { locationId: string | null }) {
  const [data, setData] = useState<ApiResponse | null>(null);

  async function load() {
    const url = locationId
      ? `/api/delivery/admin/fahrer-spitzenzeit-ranking?location_id=${locationId}`
      : '/api/delivery/admin/fahrer-spitzenzeit-ranking';
  ...
  const maxEuro = Math.max(...data.fahrer.map(f => f.avg_euro_tour), 1);

  return (
    <div className="rounded-xl border border-orange-800 bg-orange-950/40 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        <Zap className="w-4 h-4 text-orange-400" />
        <span className="text-sm font-semibold text-orange-300">Spitzenzeit-Effizienz-Ranking</span>
        <span className="ml-auto text-xs text-gray-500">Rang 1 = höchster €/Tour (11:30–14:00 & 17:30–21:00)</span>
      </div>

      {data.alert_count > 0 && (
        <div className="flex items-center gap-2 text-xs text-red-300 bg-red-900/30 rounded px-3 py-1.5 mb-3">
          <AlertTriangle className="w-3 h-3" />
          {data.alert_count} Fahrer mit niedrigem Spitzenzeit-Wert (&lt;10€/Tour)
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-2 mb-3 text-center">
        <div className="bg-black/20 rounded p-2">
          ...
          <div className="text-sm font-bold text-orange-400">{data.fahrer[0]?.avg_euro_tour.toFixed(2)}€</div>
        </div>
        <div className="bg-black/20 rounded p-2">
          <div className="text-xs text-gray-400">Team-Ø</div>
          <div className="text-sm font-bold text-orange-300">{data.team_avg_euro.toFixed(2)}€</div>
        </div>
        <div className="bg-black/20 rounded p-2">
      ...
      <div className="mt-3 text-xs text-gray-500 text-center">
        Bester Spitzenzeit-Wert: {data.bester_name}
      </div>
    </div>
  );
}

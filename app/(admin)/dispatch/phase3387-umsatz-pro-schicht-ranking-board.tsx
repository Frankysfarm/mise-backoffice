...
import { ChevronDown, ChevronUp, Euro, TrendingUp, TrendingDown, Minus, AlertTriangle } from 'lucide-react';
...
export function DispatchPhase3387UmsatzProSchichtRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-umsatz-pro-schicht?location_id=${locationId}`, { cache: 'no-store' });
        ...
        <div className="px-3 pb-3 space-y-2">
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1.5 p-2 rounded bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 text-xs border border-red-200">
              ...
              Niedriger Umsatz/Schicht! {data.alert_count} Fahrer unter Zielwert
            </div>
          )}

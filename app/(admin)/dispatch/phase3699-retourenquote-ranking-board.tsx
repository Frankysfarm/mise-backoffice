...
export function DispatchPhase3699RetourenquoteRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/delivery/admin/fahrer-retourenquote?location_id=${locationId}`);
  ...
  const sorted = [...data.fahrer].sort((a, b) => a.retourenquote - b.retourenquote);
  const maxVal = Math.max(...sorted.map(f => f.retourenquote), 1);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900">Retourenquote — Ranking</h3>
      </div>

      {data.alert_count > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">
          <AlertTriangle className="w-4 h-4" />
          <span>{data.alert_count} Fahrer mit hoher Retourenquote!</span>
        </div>
      )}

      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        <div className="bg-emerald-50 rounded-lg p-2">
          <div className="font-bold text-emerald-700">{bester?.retourenquote.toFixed(1)}%</div>
        ...
        <div className="bg-red-50 rounded-lg p-2">
          <div className="font-bold text-red-700">{hoechste?.retourenquote.toFixed(1)}%</div>
          ...
          <div key={f.fahrer_id} className={`flex items-center gap-2 p-2 rounded-lg border ${AMPEL_BG[f.ampel]}`}>
            ...
            <div className="w-20 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${Math.max((f.retourenquote / maxVal) * 100, 4)}%` }}
              />
            </div>
            <span className={`text-xs font-bold w-12 text-right ${AMPEL_COLOR[f.ampel]}`}>{f.retourenquote.toFixed(1)}%</span>
            {f.rank_delta > 0 ? (
      ...
      <div className="text-xs text-gray-400 text-center">Ziel ≤2% Retourenquote · Letzte 30 Tage</div>
    </div>
  );
}

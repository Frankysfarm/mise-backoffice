...
function StarRow({ avg }: { avg: number }) {
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <Star key={i} size={10} className={i <= Math.round(avg) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'} />
      ))}
    </div>
  );
}
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Julia F.', avg_sterne: 4.8, avg_sterne_vw: 4.6, anzahl_bewertungen: 11, trend: 'steigend', trend_delta:  0.2, ampel: 'gruen', alert_niedrig: false },
    { fahrer_id: 'd2', fahrer_name: 'Max M.',   avg_sterne: 4.2, avg_sterne_vw: 4.4, anzahl_bewertungen:  9, trend: 'fallend',  trend_delta: -0.2, ampel: 'gelb',  alert_niedrig: false },
    { fahrer_id: 'd3', fahrer_name: 'Sara K.',  avg_sterne: 3.8, avg_sterne_vw: 3.6, anzahl_bewertungen:  7, trend: 'steigend', trend_delta:  0.2, ampel: 'rot',   alert_niedrig: true  },
    { fahrer_id: 'd4', fahrer_name: 'Tim B.',   avg_sterne: 3.1, avg_sterne_vw: 3.6, anzahl_bewertungen:  5, trend: 'fallend',  trend_delta: -0.5, ampel: 'rot',   alert_niedrig: true  },
  ],
  team_avg_sterne: 4.0,
  team_avg_sterne_vw: 4.1,
  alert_count: 2,
};

export function DispatchPhase2951BewertungsBoard({ locationId }: { locationId: string | null }) {
  ...
  const best      = sorted[0]?.avg_sterne ?? 0;
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Bewertungs-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 text-sm">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: Bewertung zu niedrig! ({f.avg_sterne.toFixed(1)} ★)
              </span>
            </div>
          ))}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø',  value: `${data.team_avg_sterne.toFixed(1)} ★`, cls: teamText },
          ...
          <div className="space-y-2">
            {sorted.map(f => {
              ...
              const pct = Math.min(100, (f.avg_sterne / MAX_BAR) * 100);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <StarRow avg={f.avg_sterne} />
                    </div>
                    <div className="flex items-center gap-1">
                      <TrendIcon trend={f.trend} />
                      <span className={`text-xs font-bold ${text}`}>{f.avg_sterne.toFixed(1)} ★</span>
                    </div>
                  </div>
                  ...
                  <div className="text-xs text-gray-400 mt-1">{f.anzahl_bewertungen} Bewertungen heute</div>
                </div>
              );
            })}
          </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />4.0–4.4 ★</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />&lt;4.0 ★</span>

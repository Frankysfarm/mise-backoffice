...
export function DispatchPhase3181TrinkgeldQuoteRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxVal = data.fahrer[0]?.trinkgeld_quote ?? 1;
    ...
    <div style={{ background: '#1a1a2e', border: '1px solid #2d2d4e', borderRadius: 12, padding: 16, marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ color: '#fff', fontWeight: 700, fontSize: 15 }}>Trinkgeld-Quote Ranking</span>
        </div>
      ...
      {hasAlert && (
        <div style={{ background: '#7f1d1d', borderRadius: 8, padding: '6px 12px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{ color: '#fca5a5', fontSize: 12, fontWeight: 600 }}>Niedrige Trinkgeld-Quote!</span>
        </div>
      )}

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 12 }}>
        {[
          { label: 'Bester', value: best ? `${best.trinkgeld_quote.toFixed(1)}%` : '—', name: best?.driver_name ?? '—', color: '#22c55e' },
          { label: 'Team-Ø', value: `${data.team_avg.toFixed(1)}%`, name: '', color: '#60a5fa' },
          { label: 'Niedrigster', value: worst ? `${worst.trinkgeld_quote.toFixed(1)}%` : '—', name: worst?.driver_name ?? '—', color: '#ef4444' },
        ].map(kpi => (
          ...
          {data.fahrer.map(f => (
            <div key={f.driver_id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ flex: 1, background: '#2d2d4e', borderRadius: 4, height: 8 }}>
                <div style={{ width: `${(f.trinkgeld_quote / maxVal) * 100}%`, background: AMPEL_COLOR[f.ampel], height: 8, borderRadius: 4 }} />
              </div>
              <span style={{ color: '#ddd', fontSize: 12, width: 48, textAlign: 'right' }}>{f.trinkgeld_quote.toFixed(1)}%</span>

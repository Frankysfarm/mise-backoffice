...
export function DispatchPhase5143FruehAnteilBoard({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-2xl border border-amber-800/40 bg-amber-950/20 overflow-hidden mb-4">
      <div className="px-4 py-3 flex items-center justify-between border-b border-amber-800/30 bg-amber-900/10">
        <div className="flex items-center gap-2">
          <Sun className="w-4 h-4 text-amber-400" />
          <span className="text-sm font-semibold text-amber-200">Früh-Anteil je Fahrer — 06–10 Uhr UTC (letzte 30 Tage)</span>
        </div>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 divide-x divide-amber-800/30 border-b border-amber-800/20">
        <div className="px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500 mb-0.5">Höchster</div>
          <div className="text-sm font-bold text-amber-300">{bester?.frueh_anteil_pct ?? '–'} %</div>
        ...
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-amber-200">{data.team_avg_pct} %</div>
          <div className="text-[10px] text-gray-500">Früh-Anteil</div>
        </div>
        <div className="px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500 mb-0.5">Niedrigster</div>
          <div className="text-sm font-bold text-gray-400">{letzter?.frueh_anteil_pct ?? '–'} %</div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="w-20 h-1.5 rounded-full bg-gray-800 overflow-hidden shrink-0">
              <div
                ...
                style={{ width: `${Math.min(100, f.frueh_anteil_pct)}%` }}
              />
            </div>
      ...
      <div className="px-4 py-2 border-t border-amber-800/20 bg-amber-900/5">
        <div className="text-[10px] text-gray-500">
          Meister: <span className="text-amber-300 font-semibold">{data.meister_name}</span> ·
          Wenigster: <span className="text-gray-400">{data.wenigster_name}</span>
        </div>
      </div>
    </div>
  );

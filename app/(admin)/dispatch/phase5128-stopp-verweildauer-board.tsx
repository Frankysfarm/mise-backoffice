...
export function DispatchPhase5128StoppVerweildauerBoard({ locationId }: { locationId: string | null }) {
  const [data, setData] = useState<ApiResponse | null>(null);

  async function load() {
    const url = locationId
      ? `/api/delivery/admin/fahrer-stopp-verweildauer-ranking?location_id=${locationId}`
      : '/api/delivery/admin/fahrer-stopp-verweildauer-ranking';
    ...
    <div className="rounded-2xl border border-orange-700 bg-orange-950/40 overflow-hidden mb-4">
      <div className="px-4 py-3 flex items-center justify-between border-b border-orange-700/50 bg-orange-900/20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-orange-200">Stopp-Verweildauer je Fahrer (letzte 30 Tage)</span>
        </div>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 divide-x divide-orange-800/40 border-b border-orange-700/30">
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-orange-300">{bester?.avg_min ?? '–'} min</div>
        ...
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-orange-200">{data.team_avg_min} min</div>
          <div className="text-[10px] text-gray-500">Verweildauer</div>
        </div>
        <div className="px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500 mb-0.5">Längste</div>
          <div className="text-sm font-bold text-gray-400">{letzter?.avg_min ?? '–'} min</div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="w-20 h-1.5 rounded-full bg-gray-800 overflow-hidden shrink-0">
              <div
                ...
                style={{ width: `${Math.min(100, Math.round((f.avg_min / maxMin) * 100))}%` }}
              />
            </div>
      ...
      <div className="px-4 py-2 border-t border-orange-800/30 bg-orange-900/10">
        <div className="text-[10px] text-gray-500">
          Effizienteste: <span className="text-orange-300 font-semibold">{data.effizienteste_name}</span> ·

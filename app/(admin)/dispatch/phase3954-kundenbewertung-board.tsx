    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-emerald-50 rounded-lg p-2 text-center">
          ...
          <div className="text-lg font-black text-emerald-700">{best?.avg_bewertung.toFixed(1) ?? '–'}</div>
        ...
        <div className="bg-gray-50 rounded-lg p-2 text-center">
          ...
          <div className="text-lg font-black text-gray-700">{data.team_avg_bewertung.toFixed(1)}</div>
          <div className="text-[10px] text-gray-400">Ziel ≥{data.ziel_bewertung}</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-red-500 font-medium">Niedrigster</div>
          <div className="text-lg font-black text-red-600">{worst?.avg_bewertung.toFixed(1) ?? '–'}</div>
        ...
        {sorted.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg ${bgColor}`}>
              ...
              <span className={`text-sm font-bold ${tColor}`}>{f.avg_bewertung.toFixed(1)}</span>
              {DeltaIcon}
              {f.alert_niedrig && <AlertTriangle className="w-3 h-3 text-red-500 shrink-0" />}
            </div>
          );
        })}
      </div>

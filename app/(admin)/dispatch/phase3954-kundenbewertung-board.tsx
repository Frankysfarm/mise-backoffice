...
export function DispatchPhase3954KundenbewertungBoard({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-gray-400">{sorted.length} Fahrer</span>
      </div>
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-red-50 border border-red-200 rounded-lg text-xs text-red-700">
          ...
          <span>Schlechte Bewertungen! ({data.alert_count} Fahrer)</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-gray-50 rounded-lg p-2 text-center">
          ...
          <div className="text-sm font-bold text-gray-800">{best?.avg_bewertung.toFixed(1)} ★</div>
          <div className="text-[10px] text-gray-500 truncate">{best?.fahrer_name ?? '–'}</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-gray-400 mb-0.5">Team-Ø</div>
          <div className="text-sm font-bold text-gray-700">{data.team_avg_bewertung.toFixed(1)} ★</div>
          <div className="text-[10px] text-gray-400">Ø</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-gray-400 mb-0.5">Niedrigster</div>
          <div className="text-sm font-bold text-red-500">{lowest?.avg_bewertung.toFixed(1)} ★</div>
          <div className="text-[10px] text-gray-500 truncate">{lowest?.fahrer_name ?? '–'}</div>
        </div>
      </div>

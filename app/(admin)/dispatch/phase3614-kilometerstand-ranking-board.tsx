...
export function DispatchPhase3614KilometerstandRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const wenigsten = sorted[sorted.length - 1];
  const maxKm = Math.max(...sorted.map(f => f.gesamt_km), 1);
  const teamAvg = sorted.length > 0
    ? Math.round((data.team_gesamt_km / sorted.length) * 10) / 10
    : 0;
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900">Kilometerstand — Ranking</h3>
      </div>

      {data.alert_count > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">
          <AlertTriangle className="w-4 h-4" />
          <span>{data.alert_count} Fahrer mit hoher Kilometerleistung!</span>
        </div>
      )}

      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        <div className="bg-blue-50 rounded-lg p-2">
          <div className="font-bold text-blue-700">{meister?.gesamt_km.toFixed(1) ?? '–'} km</div>
          <div className="text-gray-500">Meistgefahren</div>
          <div className="text-gray-700 truncate">{meister?.fahrer_name ?? '–'}</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2">
          <div className="font-bold text-gray-700">{teamAvg.toFixed(1)} km</div>
          <div className="text-gray-500">Team-Ø</div>
        </div>
        <div className="bg-emerald-50 rounded-lg p-2">
          <div className="font-bold text-emerald-700">{wenigsten?.gesamt_km.toFixed(1) ?? '–'} km</div>
          <div className="text-gray-500">Wenigsten</div>
          <div className="text-gray-700 truncate">{wenigsten?.fahrer_name ?? '–'}</div>
        </div>
      </div>
          ...
          <div key={f.fahrer_id} className={`flex items-center gap-2 p-2 rounded-lg border ${AMPEL_BG[f.ampel]}`}>
            ...
            <div className="w-24 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${Math.max((f.gesamt_km / maxKm) * 100, 2)}%` }}
              />
            </div>
            <span className={`text-xs font-bold w-16 text-right ${AMPEL_COLOR[f.ampel]}`}>{f.gesamt_km.toFixed(1)} km</span>
            {f.trend_delta < 0 ? (

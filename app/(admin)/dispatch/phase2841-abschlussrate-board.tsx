...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Anna K.',  rate_pct: 100, abgeschlossen: 10, gesamt: 10, ampel: 'gruen', trend: 'stabil',   trend_delta: 0,  alert: false, rang: 1 },
    { fahrer_id: 'd2', fahrer_name: 'Max M.',   rate_pct:  97, abgeschlossen: 11, gesamt: 11, ampel: 'gruen', trend: 'steigend', trend_delta: 2,  alert: false, rang: 2 },
    { fahrer_id: 'd3', fahrer_name: 'Sara K.',  rate_pct:  88, abgeschlossen:  7, gesamt:  8, ampel: 'gelb',  trend: 'fallend',  trend_delta: -3, alert: false, rang: 3 },
    { fahrer_id: 'd4', fahrer_name: 'Tim W.',   rate_pct:  75, abgeschlossen:  6, gesamt:  8, ampel: 'rot',   trend: 'fallend',  trend_delta: -5, alert: true,  rang: 4 },
  ],
  team_avg_pct: 90.0,
  alert_count: 1,
};

export function DispatchPhase2841AbschlussrateBoard({ locationId }: { locationId: string | null }) {
  ...
  const alerts   = enriched.filter(f => f.rate_pct < WARN_PCT);
    ...
    <div className={`rounded-xl border p-4 mb-4 ${hasAlert ? 'border-red-300 bg-red-50' : 'border-gray-200 bg-white'}`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800">Abschlussrate</span>
      ...
      {open && (
        <div className="mt-3 space-y-3">
            ...
            <div className="bg-red-100 border border-red-300 rounded-lg p-2 space-y-1">
              {alerts.map(f => (
                <div key={f.fahrer_id} className="flex items-center gap-2 text-xs text-red-700">
                  ...
                  <span>— Niedrige Abschlussrate! ({f.rate_pct.toFixed(1)}% / Ziel ≥{ZIEL_PCT}%)</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø', val: `${data.team_avg_pct.toFixed(1)}%` },
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${a.bg}`}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-500">
                      ...
                      <span className={`font-bold ${a.text}`}>{f.rate_pct.toFixed(1)}%</span>
                    </div>
                  </div>

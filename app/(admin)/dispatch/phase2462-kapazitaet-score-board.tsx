...
import { ChevronDown, ChevronUp, AlertTriangle, TrendingUp, TrendingDown, Minus, BarChart2 } from 'lucide-react';

interface FahrerKapazitaet {
  fahrer_id: string;
  fahrer_name: string;
...
export function DispatchPhase2462KapazitaetScoreBoard({ locationId }: { locationId: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-kapazitaet-score?location_id=${locationId}`)
        .then(r => r.json())
        .then(setData)
        .catch(console.error);
    load();
        ...
        <div className="bg-white px-4 pb-3 pt-2 space-y-3">
          ...
          {hasAlerts && (
            <div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-xs text-red-700">
              ...
              <span>
                <strong>Unterauslastung:</strong>{' '}
                {data.fahrer.filter(f => f.alert_unterauslastung).map(f => f.fahrer_name).join(', ')} — Kapazität unter 60%!
              </span>
            </div>
          )}
          ...
          <div className="space-y-1.5">
            {data.fahrer.map(f => (
              <div key={f.fahrer_id} className={`flex items-center gap-2 text-xs rounded-lg px-2.5 py-1.5 border ${ampelColor(f.ampel)}`}>
                ...
                <span className="text-gray-400 text-[10px]">{f.touren_heute}/{f.max_kapazitaet} Touren</span>
                <ScoreBar score={f.kapazitaet_score} />
                <span className="w-10 text-right font-semibold">{f.kapazitaet_score.toFixed(0)}%</span>

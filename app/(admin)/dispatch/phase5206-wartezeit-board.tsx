...
export function DispatchPhase5206WartezeitBoard({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-purple-700 bg-purple-900/30 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        <Hourglass className="w-4 h-4 text-purple-400 shrink-0" />
        <span className="text-sm font-semibold text-gray-200">Ø Wartezeit-Ranking (Restaurant)</span>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-lg bg-purple-900/50 px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500 truncate">Beste</div>
          <div className="text-xs font-bold text-purple-300 truncate">{data.beste_name}</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500">Team-Ø</div>
          <div className="text-xs font-bold text-gray-200">{data.team_avg_wartezeit.toFixed(0)} min</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500 truncate">Längste</div>
          <div className="text-xs font-bold text-red-400 truncate">{data.laengste_name}</div>
        </div>
      </div>
        ...
        {data.fahrer.slice(0, 6).map(f => (
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-400 w-10 text-right tabular-nums">
              {f.avg_wartezeit_min.toFixed(0)}min
            </span>
      ...
      {data.beste_name && (
        <div className="mt-3 text-[10px] text-purple-400 border-t border-purple-800/40 pt-2">
          Schnellste: {data.beste_name} — {data.fahrer[0]?.avg_wartezeit_min.toFixed(0)} min Ø-Wartezeit
        </div>
      )}
    </div>
  );

...
export function DispatchPhase3266RetourenquoteRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxPct = Math.max(...list.map(f => f.retouren_pct), 1);
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-green-50 dark:bg-green-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <XCircle size={10} className="text-green-500" /> Bester
              </div>
              <div className="font-bold text-sm text-green-700 dark:text-green-400">{bester?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{bester ? fmtPct(bester.retouren_pct) : '—'}</div>
            </div>
            <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-2">
              ...
              <div className="text-xs text-gray-500">Ø Retouren</div>
            </div>
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5">Höchster</div>
              <div className="font-bold text-sm text-gray-600 dark:text-gray-300">{hoechster?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{hoechster ? fmtPct(hoechster.retouren_pct) : '—'}</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {list.map(f => {
              ...
              const barWidth = maxPct > 0 ? Math.max((f.retouren_pct / maxPct) * 100, f.retouren_pct > 0 ? 2 : 0) : 0;
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`font-bold text-sm ${cls.text}`}>{fmtPct(f.retouren_pct)}</span>
                      <span className="text-xs text-gray-400">({f.returned}/{f.total})</span>
                  ...
                  {f.alert_bottom && (
                    <div className="mt-1 text-xs text-red-600 dark:text-red-400 font-semibold flex items-center gap-1">
                      <AlertTriangle size={10} /> Hohe Retourenquote!
                    </div>
                  )}
                </div>
              );
          ...
          <div className="flex flex-wrap gap-3 text-xs text-gray-500 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500" /> Top 25% (niedrigste Retouren)</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400" /> Mitte 50%</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500"   /> Bottom 25% (höchste Retouren)</span>
          </div>
        </div>
      )}
    </div>

...
export function DispatchPhase5011UmsatzProStundeBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxVal = Math.max(...data.fahrer.map(f => f.umsatz_pro_stunde), 1);
    ...
    <div className="rounded-2xl border border-cyan-700 bg-cyan-950/40 overflow-hidden mb-4">
      ...
      <div className="px-4 py-3 flex items-center justify-between border-b border-cyan-700/50 bg-cyan-900/20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-cyan-200">Umsatz/h-Ranking (letzte 30 Tage)</span>
        </div>
        {data.alert_count > 0 && (
      ...
      {/* KPI Grid */}
      <div className="grid grid-cols-3 divide-x divide-cyan-700/30 border-b border-cyan-700/30">
        <div className="px-3 py-2 text-center">
          <div className="text-xs text-gray-500 mb-0.5">Bester Umsatz/h</div>
        ...
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-gray-300">{data.team_avg_umsatz} €/h</div>
        </div>
        <div className="px-3 py-2 text-center">
      ...
      {/* Driver List */}
      <div className="divide-y divide-cyan-700/20">
        {data.fahrer.map(f => (
      ...
      {/* Champion Footer */}
      <div className="px-4 py-2 border-t border-cyan-700/30 bg-cyan-900/10">
        <p className="text-xs text-gray-500">
          Rang 1 = höchster Umsatz/h = bester ·{' '}
          <span className="text-green-400">{bester?.fahrer_name}</span> führt mit {bester?.umsatz_pro_stunde} €/h
        </p>
      </div>
    </div>
  );

...
export function DispatchPhase5031SchichtBilanzBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="rounded-2xl border border-amber-700 bg-amber-950/40 overflow-hidden mb-4">
      ...
      <div className="px-4 py-3 flex items-center justify-between border-b border-amber-700/50 bg-amber-900/20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-amber-200">Ø Netto-Verdienst je Schicht (letzte 30 Tage)</span>
        </div>
        {data.alert_count > 0 && (
      ...
      {/* KPI Grid */}
      <div className="grid grid-cols-3 divide-x divide-amber-700/30 border-b border-amber-700/30">
        ...
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-gray-300">{data.team_avg.toFixed(2)} €</div>
        </div>
        <div className="px-3 py-2 text-center">
      ...
      {/* Champion Footer */}
      <div className="px-4 py-2 border-t border-amber-700/30 bg-amber-900/10">
        <p className="text-xs text-gray-500">
          Rang 1 = höchster Ø-Netto-Verdienst ·{' '}
          <span className="text-green-400">{bester?.fahrer_name}</span> führt mit {bester?.avg_netto.toFixed(2)} €
        </p>
      </div>
    </div>
  );

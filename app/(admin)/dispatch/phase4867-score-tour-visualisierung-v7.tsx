...
import { Trophy, TrendingUp, TrendingDown, AlertTriangle, MapPin, Clock, Navigation2, CheckCircle2, XCircle, Gauge, Target, Zap, Route } from 'lucide-react';
...
const MOCK: ApiResponse = {
  ...
  eta_accuracy_pct: 81,
  route_effizienz_gesamt_pct: 78,
  alerts: [],
  fahrer: [
    {
      fahrer_id: 'f1', fahrer_name: 'M. Schulz', score: 97, score_ring: 'platin',
      trend: 2, stopps_offen: 2, stopps_gesamt: 9, stopps_fertig: 7, eta_naechster_min: 3, km_heute: 37.5,
      avg_lieferzeit_min: 23, puenktlichkeit_pct: 96, eta_accuracy_pct: 90, route_effizienz_pct: 88, co2_kg_heute: 2.1,
      tour_stops: [
        { stopp_nr: 1, adresse: 'Hauptstr. 12', status: 'geliefert', eta_min: null, km: 1.2, verspaetet_min: null },
        { stopp_nr: 2, adresse: 'Marktplatz 5', status: 'aktiv', eta_min: 3, km: 2.0, verspaetet_min: null },
        { stopp_nr: 3, adresse: 'Bergweg 8', status: 'ausstehend', eta_min: 13, km: 3.4, verspaetet_min: null },
      ],
    },
    {
      fahrer_id: 'f2', fahrer_name: 'A. Klein', score: 80, score_ring: 'gut',
      trend: -1, stopps_offen: 3, stopps_gesamt: 8, stopps_fertig: 5, eta_naechster_min: 8, km_heute: 28.9,
      avg_lieferzeit_min: 28, puenktlichkeit_pct: 82, eta_accuracy_pct: 75, route_effizienz_pct: 72, co2_kg_heute: 1.8,
      tour_stops: [
        { stopp_nr: 1, adresse: 'Seestr. 3', status: 'geliefert', eta_min: null, km: 0.9, verspaetet_min: null },
        { stopp_nr: 2, adresse: 'Am Ring 17', status: 'verspaetet', eta_min: 8, km: 1.9, verspaetet_min: 4 },
        { stopp_nr: 3, adresse: 'Parkweg 22', status: 'ausstehend', eta_min: 20, km: 4.1, verspaetet_min: null },
      ],
    },
    {
      fahrer_id: 'f3', fahrer_name: 'T. Bauer', score: 63, score_ring: 'schwach',
      trend: -4, stopps_offen: 4, stopps_gesamt: 7, stopps_fertig: 3, eta_naechster_min: 12, km_heute: 21.4,
      avg_lieferzeit_min: 34, puenktlichkeit_pct: 65, eta_accuracy_pct: 62, route_effizienz_pct: 61, co2_kg_heute: 1.4,
      tour_stops: [
        { stopp_nr: 1, adresse: 'Lindenweg 4', status: 'aktiv', eta_min: 12, km: 2.2, verspaetet_min: null },
        { stopp_nr: 2, adresse: 'Schulstr. 9', status: 'batch', eta_min: 25, km: 3.0, verspaetet_min: null },
        { stopp_nr: 3, adresse: 'Feldweg 2', status: 'ausstehend', eta_min: 39, km: 5.3, verspaetet_min: null },
      ],
    },
  ],
};
...
export function DispatchPhase4867ScoreTourVisualisierungV7({ locationId }: { locationId: string | null }) {
      ...
      <div className="px-4 py-3 border-b border-slate-700 bg-indigo-950/20">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-indigo-300">Score + Tour-Visualisierung V7</span>
            <span className="text-[10px] px-1.5 py-0.5 bg-emerald-900/50 text-emerald-300 rounded-full">Route-Effizienz</span>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="mt-3 flex items-center gap-4">
          <div className="relative w-16 h-16 shrink-0">
            <svg className="w-16 h-16 -rotate-90" viewBox="0 0 64 64">
              ...
              <circle cx="32" cy="32" r="26" fill="none"
                stroke={data.team_score >= 85 ? '#22c55e' : data.team_score >= 70 ? '#eab308' : '#ef4444'}
                strokeWidth="8" strokeDasharray={`${arcPct * 1.634} 163.4`} strokeLinecap="round" />
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              {!data.alert_unter_ziel && <span className="text-xs text-green-400 bg-green-950/40 px-1.5 py-0.5 rounded-full">✓ Ziel</span>}
            </div>
            <div className="mt-2 grid grid-cols-4 gap-1.5">
              {[
                { label: 'Pünktl.', val: `${data.puenktlichkeit_pct}%`, color: data.puenktlichkeit_pct >= 80 ? 'text-green-400' : 'text-yellow-400' },
                { label: 'Ø Zeit', val: `${data.avg_lieferzeit_min}m`, color: data.avg_lieferzeit_min <= 30 ? 'text-green-400' : 'text-orange-400' },
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {f.trend > 0 && <TrendingUp className="w-3 h-3 text-green-400" />}
                    {f.trend < 0 && <TrendingDown className="w-3 h-3 text-red-400" />}
                  </div>
                  <div className="mt-1 flex items-center gap-3 text-[10px] text-slate-500 flex-wrap">
                    ...
                    <span className="flex items-center gap-1 text-emerald-400"><Route className="w-2.5 h-2.5" />Eff {f.route_effizienz_pct}%</span>
                    <span className="text-slate-600">CO₂ {f.co2_kg_heute.toFixed(1)} kg</span>
                  </div>
                    ...
                    <div key={st.stopp_nr} className={`flex items-start gap-2 px-2 py-1.5 rounded-lg border ${stoppBg(st.status)}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          {st.verspaetet_min && <span className="text-[10px] text-red-400">+{st.verspaetet_min} Min verspätet</span>}
                          <span className="text-[10px] text-slate-500">{st.km.toFixed(1)} km</span>
                        </div>
                      </div>
                      {st.eta_min !== null && <span className="text-xs text-slate-400 shrink-0">{st.eta_min} Min</span>}
                    </div>
                  ))}
                </div>
              )}

...
export function DispatchPhase3532LieferzeitProStoppRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxMin = Math.max(...data.fahrer.map(f => f.lieferzeit_pro_stopp), 1);
    ...
    <div className="border rounded-lg bg-white dark:bg-gray-900 shadow-sm mb-3">
      ...
      >
        <span className="flex items-center gap-2 font-semibold text-sm">
          ...
          Lieferzeit/Stopp — #1 {data.schnellster_name}
          {data.alert_count > 0 && (
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="rounded-md p-2 text-center bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800">
              <div className="text-lg font-bold text-emerald-700 dark:text-emerald-400">
                {data.fahrer[0]?.lieferzeit_pro_stopp.toFixed(1)}min
              </div>
            ...
            <div className="rounded-md p-2 text-center bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
              <div className="text-lg font-bold text-gray-700 dark:text-gray-300">
                {data.team_avg.toFixed(1)}min
              </div>
              <div className="text-[10px] text-gray-500">Team-Ø /Stopp</div>
            </div>
            <div className="rounded-md p-2 text-center bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
              <div className="text-lg font-bold text-red-700 dark:text-red-400">
                {data.fahrer[data.fahrer.length - 1]?.lieferzeit_pro_stopp.toFixed(1)}min
              </div>
              <div className="text-[10px] text-red-600 dark:text-red-500 truncate">
                {data.langsamster_name}
              </div>
            </div>
          </div>
              ...
              >
                ...
                <div className="w-16 h-1.5 rounded-full bg-gray-200 dark:bg-gray-700 overflow-hidden flex-shrink-0">
                  <div
                    ...
                    style={{ width: `${(f.lieferzeit_pro_stopp / maxMin) * 100}%` }}
                  />
                </div>

...
import { ClipboardCheck, TrendingUp, TrendingDown, Minus, AlertTriangle } from 'lucide-react';

interface FahrerVollstaendigkeit {
  fahrer_id: string;
  fahrer_name: string;
  rang: number;
...
interface VollstaendigkeitData {
  fahrer: FahrerVollstaendigkeit[];
  team_avg_pct: number;
  alert_count: number;
  beste_name: string;
...
export function DispatchPhase4533VollstaendigkeitBoard({ locationId }: Props) {
  const [data, setData] = useState<VollstaendigkeitData | null>(null);
  const [loading, setLoading] = useState(true);

  const fetchData = useCallback(async () => {
    const params = locationId ? `?location_id=${locationId}` : '';
    const res = await fetch(`/api/delivery/admin/fahrer-vollstaendigkeit-ranking${params}`, { cache: 'no-store' });
  ...
  const maxVal = best?.vollstaendigkeit_pct ?? 1;

  const ampelColor = (a: FahrerVollstaendigkeit['ampel']) =>
    a === 'gruen' ? 'text-green-600 dark:text-green-400' :
    a === 'rot'   ? 'text-red-500 dark:text-red-400' : 'text-yellow-500 dark:text-yellow-400';

  const ampelBg = (a: FahrerVollstaendigkeit['ampel']) =>
    a === 'gruen' ? 'bg-green-600' : a === 'rot' ? 'bg-red-500' : 'bg-yellow-400';
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900 dark:text-gray-100 text-sm">Vollständigkeits-Ranking (30 Tage)</h3>
        <span className="ml-auto text-xs text-gray-400">% erfolgreich</span>
      </div>

      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="rounded-lg bg-green-50 dark:bg-green-900/20 p-2">
          <div className="text-lg font-bold text-green-600 dark:text-green-400">{best?.vollstaendigkeit_pct?.toFixed(1)}%</div>
        ...
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          <div className="text-lg font-bold text-gray-500 dark:text-gray-400">{worst?.vollstaendigkeit_pct?.toFixed(1)}%</div>
      ...
      {alerts.length > 0 && (
        <div className="flex items-center gap-2 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 px-3 py-2">
          ...
          <span className="text-xs text-red-700 dark:text-red-300">
            Niedrige Vollständigkeit: {alerts.map(a => a.fahrer_name).join(', ')}
          </span>
        </div>
      )}
        ...
        {data.fahrer.map((f) => (
          <div key={f.fahrer_id} className="space-y-1">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className={`font-semibold text-xs ${ampelColor(f.ampel)}`}>{f.vollstaendigkeit_pct?.toFixed(1)}%</span>
            </div>
            <div className="h-1.5 rounded-full bg-gray-100 dark:bg-gray-800">
              <div
                className={`h-full rounded-full ${ampelBg(f.ampel)}`}
                style={{ width: `${maxVal > 0 ? Math.round((f.vollstaendigkeit_pct / maxVal) * 100) : 0}%` }}
              />
            </div>
          </div>
        ))}
      ...
      <div className="text-xs text-gray-400">Ziel: ≥92% · {data.gesamt} Fahrer</div>
    </div>
  );
}

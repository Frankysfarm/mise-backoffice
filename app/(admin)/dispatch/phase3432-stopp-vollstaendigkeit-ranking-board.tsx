...
import { ChevronDown, ChevronUp, CheckCircle, TrendingUp, TrendingDown, Minus, AlertTriangle } from 'lucide-react';

interface FahrerStoppVollstaendigkeit {
  fahrer_id: string;
  fahrer_name: string;
  rang: number;
  vollstaendigkeits_pct: number;
  rank_delta: number;
...
interface ApiResponse {
  fahrer: FahrerStoppVollstaendigkeit[];
  team_avg: number;
  bester_name: string;
...
export function DispatchPhase3432StoppVollstaendigkeitRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-stopp-vollstaendigkeit?location_id=${locationId}`, { cache: 'no-store' });
        ...
        <div className="px-3 pb-3 space-y-2">
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1.5 p-2 rounded bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 text-xs border border-red-200">
              ...
              Niedrige Vollständigkeit! {data.alert_count} Fahrer unter Zielwert
            </div>
          )}

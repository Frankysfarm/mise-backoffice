...
export function DispatchPhase5186UmsatzProLieferungBoard({ locationId }: { locationId: string | null }) {
  const [data, setData] = useState<ApiResponse | null>(null);

  async function load() {
    const url = locationId
      ? `/api/delivery/admin/fahrer-umsatz-pro-lieferung-ranking?location_id=${locationId}`
      : '/api/delivery/admin/fahrer-umsatz-pro-lieferung-ranking';
  ...
  return (
    <div className="rounded-xl border border-emerald-700 bg-emerald-900/30 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-gray-200">Ø Umsatz/Lieferung-Ranking</span>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-lg bg-emerald-900/50 px-3 py-2 text-center">
          ...
          <div className="text-xs font-bold text-emerald-300 truncate">{data.bester_name}</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500">Team-Ø</div>
          <div className="text-xs font-bold text-gray-200">{data.team_avg.toFixed(2)}€</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500 truncate">Niedrigster</div>
          <div className="text-xs font-bold text-red-400 truncate">{data.schlechtester_name}</div>
        </div>
      </div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 h-2 rounded-full bg-gray-800 overflow-hidden">
              <div
                ...
                style={{ width: `${Math.round((f.avg_euro / maxEuro) * 100)}%` }}
              />
            </div>
            <span className="text-xs text-gray-400 w-12 text-right tabular-nums">
              {f.avg_euro.toFixed(2)}€
            </span>
      ...
      {data.bester_name && (
        <div className="mt-3 text-[10px] text-emerald-400 border-t border-emerald-800/40 pt-2">
          Champion: {data.bester_name} — {data.fahrer[0]?.avg_euro.toFixed(2)}€ Ø/Lieferung
        </div>
      )}
    </div>
  );

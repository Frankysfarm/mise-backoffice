...
import { Sunset, TrendingUp, TrendingDown, Minus, AlertTriangle } from 'lucide-react';
...
export function DispatchPhase4402AbendschichtBoard({ locationId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/delivery/admin/fahrer-abendschicht-ranking?location_id=${locationId}`);
    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-gray-900">Abendschicht-Anteil-Ranking</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          {!loading && data.alert_count > 0 && (
            <span className="flex items-center gap-0.5 text-xs text-red-600 font-semibold bg-red-50 px-2 py-0.5 rounded-full">
              <AlertTriangle className="w-3 h-3" /> {data.alert_count} Hoher Abendschicht-Anteil!
            </span>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        <div className="bg-rose-50 rounded-xl p-2">
          <div className="font-bold text-rose-700">{bester?.abendschicht_anteil_pct.toFixed(1)}%</div>
          <div className="text-gray-500 truncate">{data.meister_name}</div>
          <div className="text-[10px] text-gray-400">Meiste Abend</div>
        </div>
        ...
        <div className="bg-green-50 rounded-xl p-2">
          <div className="font-bold text-green-700">{letzter?.abendschicht_anteil_pct.toFixed(1)}%</div>
          <div className="text-gray-500 truncate">{data.wenigster_name}</div>
          <div className="text-[10px] text-gray-400">Wenigste Abend</div>
        </div>
      </div>
      ...
      <div className="space-y-1.5">
        {data.fahrer.map((f) => {
          ...
          const barColor = f.ampel === 'rot' ? 'bg-red-400' : f.ampel === 'gelb' ? 'bg-rose-300' : 'bg-rose-500';
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-900 w-14 text-right">{f.abendschicht_anteil_pct.toFixed(1)}%</span>
              </div>
              <div className="ml-7 h-1 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${barColor}`} style={{ width: `${(f.abendschicht_anteil_pct / maxPct) * 100}%` }} />
              </div>
            </div>
          );
        })}
      ...
      <div className="flex items-center justify-between text-[10px] text-gray-400 pt-1 border-t border-gray-100">
        ...
        <span>#1 = höchster Abendschicht-Anteil</span>
      </div>
    </div>
  );
}

...
export function DispatchPhase3949StoppsProStundeBoard({ locationId }: { locationId: string | null }) {
  ...
  // descending: most stopps_pro_stunde = Rang 1 = best
  const sorted = [...data.fahrer].sort((a, b) => a.rang - b.rang);
  const best = sorted[0];
  const worst = sorted[sorted.length - 1];
  const maxVal = Math.max(...sorted.map(f => f.stopps_pro_stunde), 1);
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-red-50 border border-red-200 rounded-lg text-[11px] text-red-700">
          ...
          <span>Niedrige Produktivität!</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-blue-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-blue-600 font-medium">Bester</div>
          <div className="text-lg font-black text-blue-700">{best?.stopps_pro_stunde?.toFixed(1) ?? '–'}</div>
          <div className="text-[10px] text-blue-500 truncate">{best?.fahrer_name ?? '–'}</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-gray-500 font-medium">Team-Ø</div>
          <div className="text-lg font-black text-gray-700">{data.team_avg?.toFixed(1) ?? '–'}</div>
          <div className="text-[10px] text-gray-400">Ziel ≥4 /h</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-red-500 font-medium">Niedrigster</div>
          <div className="text-lg font-black text-red-600">{worst?.stopps_pro_stunde?.toFixed(1) ?? '–'}</div>
      ...
      <div className="space-y-1.5">
        {sorted.map((f) => {
          ...
          const bgColor = f.ampel === 'gruen' ? 'bg-blue-50' : f.ampel === 'gelb' ? 'bg-yellow-50' : 'bg-red-50';
          ...
          return (
            <div key={f.fahrer_id} className={`space-y-0.5 px-2.5 py-1.5 rounded-lg ${bgColor}`}>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm font-bold ${tColor}`}>{f.stopps_pro_stunde?.toFixed(1)}/h</span>
                {DeltaIcon}
              ...
              <div className="h-1 bg-white bg-opacity-70 rounded-full overflow-hidden ml-7">
                <div className={`h-full rounded-full ${barColor}`} style={{ width: `${(f.stopps_pro_stunde / maxVal) * 100}%` }} />
              </div>
            </div>
          );
        })}

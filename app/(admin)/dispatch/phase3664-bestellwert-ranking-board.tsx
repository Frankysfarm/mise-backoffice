...
export function DispatchPhase3664BestellwertRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxVal = bester?.avg_bestellwert ?? 1;

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        <TrendingUp className="w-5 h-5 text-emerald-500" />
        <h3 className="font-semibold text-gray-900">Ø Bestellwert — Ranking</h3>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        <div className="bg-emerald-50 rounded-lg p-2">
          <div className="font-semibold text-emerald-700 truncate">{bester?.fahrer_name ?? '—'}</div>
          <div className="text-emerald-600">{bester ? `${bester.avg_bestellwert.toFixed(2)} €` : '—'}</div>
          <div className="text-gray-400">Bester</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2">
          <div className="font-semibold text-gray-700">{data.team_avg.toFixed(2)} €</div>
          <div className="text-gray-400">Team-Ø</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2">
          <div className="font-semibold text-red-700 truncate">{schlechtester?.fahrer_name ?? '—'}</div>
          <div className="text-red-600">{schlechtester ? `${schlechtester.avg_bestellwert.toFixed(2)} €` : '—'}</div>
          <div className="text-gray-400">Niedrigster</div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {sorted.map((f) => {
          ...
          const pct = maxVal > 0 ? (f.avg_bestellwert / maxVal) * 100 : 0;
          ...
          return (
            <div key={f.fahrer_id} className={`rounded-lg border p-2.5 ${AMPEL_BG[f.ampel]}`}>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`text-sm font-bold ${AMPEL_COLOR[f.ampel]}`}>{f.avg_bestellwert.toFixed(2)} €</span>
              ...
              {f.alert_low && (
                <div className="text-xs text-red-600 mt-1">Niedriger Bestellwert!</div>
              )}
            </div>
          );
        })}

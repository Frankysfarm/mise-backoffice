...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f4', fahrer_name: 'Tom B.',   erstkontakt_min: 25, trend: 'steigend', trend_delta:  5, ampel: 'rot',   rang: 1 },
    { fahrer_id: 'f5', fahrer_name: 'Jana F.',  erstkontakt_min: 18, trend: 'stabil',   trend_delta:  0, ampel: 'gelb',  rang: 2 },
    { fahrer_id: 'f2', fahrer_name: 'Sarah K.', erstkontakt_min: 12, trend: 'stabil',   trend_delta:  1, ampel: 'gelb',  rang: 3 },
    { fahrer_id: 'f3', fahrer_name: 'Lena S.',  erstkontakt_min:  9, trend: 'fallend',  trend_delta: -3, ampel: 'gruen', rang: 4 },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   erstkontakt_min:  8, trend: 'fallend',  trend_delta: -2, ampel: 'gruen', rang: 5 },
  ],
  team_durchschnitt: 14.4,
  alert_count: 1,
};

export function DispatchPhase2620ErstkontaktZeitBoard({ locationId }: { locationId: string | null }) {
  ...
  const teamAmpel  = data.team_durchschnitt <= 10 ? 'gruen' : data.team_durchschnitt <= 20 ? 'gelb' : 'rot';
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-4`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800">Erstkontakt-Zeit Board</span>
          {hasAlert && (
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3">
            ...
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className="text-lg font-bold text-gray-700">
                {data.fahrer.length > 0
                  ? `${Math.min(...data.fahrer.map(f => f.erstkontakt_min))} Min`
                  : '—'}
              </div>
              <div className="text-xs text-gray-500">Bester heute</div>
            </div>
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className="text-lg font-bold text-green-600">≤10 Min</div>
          ...
          {hasAlert && (
            <div className="flex items-center gap-2 bg-red-100 border border-red-200 rounded-lg px-3 py-2">
              ...
              <p className="text-xs text-red-700 font-medium">
                Anlaufzeit zu lang: {alertNames.join(', ')}
              </p>
            </div>
          )}
            ...
            {sorted.map(f => {
              ...
              return (
                <div key={f.fahrer_id} className={`flex items-center gap-2 rounded-lg border px-2.5 py-1.5 ${cls.bg}`}>
                  ...
                  <ZeitBalken min={f.erstkontakt_min} barClass={cls.bar} />
                  <span className={`text-xs font-bold w-14 text-right ${cls.text}`}>{f.erstkontakt_min} Min</span>
          ...
          <div className="flex gap-3 text-xs text-gray-400">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 inline-block" />≤10 Min</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />11–20 Min</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />&gt;20 Min</span>
          </div>
        </div>
      )}
    </div>

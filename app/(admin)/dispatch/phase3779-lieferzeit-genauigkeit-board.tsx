...
function ampelColor(ampel: FahrerRow['ampel']) {
  if (ampel === 'gruen') return { text: 'text-violet-700', bg: 'bg-violet-50', bar: 'bg-violet-500', border: 'border-violet-200' };
...
export function DispatchPhase3779LieferzeitGenauigkeitBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-900 text-sm">Lieferzeit-Genauigkeit-Ranking</span>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-violet-50 rounded-lg p-2">
          <div className="text-base font-black text-violet-700">{data.fahrer[0]?.genauigkeit_pct}%</div>
        ...
        <div className={`rounded-lg p-2 ${data.alert_count > 0 ? 'bg-red-50' : 'bg-gray-50'}`}>
          <div className={`text-base font-black ${data.alert_count > 0 ? 'text-red-600' : 'text-gray-800'}`}>
            {data.fahrer[data.fahrer.length - 1]?.genauigkeit_pct}%
          </div>
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 bg-red-50 border border-red-200 rounded-lg text-xs text-red-700">
          ...
          <span>{data.alert_count} Fahrer mit späten Lieferungen — bitte prüfen!</span>
        </div>
      )}
          ...
          return (
            <div key={f.fahrer_id} className={`rounded-lg border ${c.border} ${c.bg} p-2.5 space-y-1.5`}>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm font-black ${c.text}`}>{f.genauigkeit_pct}%</span>
                {f.rank_delta !== 0 && (
                  f.rank_delta > 0
              ...
              {f.alert_spaet && (
                <div className="text-[10px] text-red-600 font-medium">Späte Lieferungen!</div>
              )}
            </div>
          );
        })}

...
export function DispatchPhase4678StornoBoard({ locationId }: { locationId: string | null }) {
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Storno-Ranking nicht verfügbar</span>
      </div>
    );
  }
  ...
  const maxPct = Math.max(...data.fahrer.map(f => f.storno_pct), 20);
  ...
  return (
    <div className="rounded-2xl border border-orange-200 dark:border-orange-900 bg-orange-50 dark:bg-orange-950 p-3 space-y-2">
      <div className="flex items-center justify-between">
        ...
        {data.alert_count > 0 && (
          <div className="flex items-center gap-1 bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 rounded-full px-2 py-0.5">
            ...
            <span className="text-[10px] font-semibold">{data.alert_count} Hohe Storno-Quote</span>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {[
          { label: 'Niedrigste', value: `${data.fahrer[0]?.storno_pct.toFixed(1)}%` },
          { label: 'Team-Ø', value: `${data.team_avg_pct.toFixed(1)}%` },
          { label: 'Höchste', value: `${data.fahrer[data.fahrer.length - 1]?.storno_pct.toFixed(1)}%` },
        ].map(kpi => (
          <div key={kpi.label} className="bg-white dark:bg-orange-900/30 rounded-xl p-1.5 text-center">
            <div className="text-[11px] font-bold text-orange-900 dark:text-orange-200">{kpi.value}</div>
            <div className="text-[9px] text-orange-700 dark:text-orange-400 mt-0.5">{kpi.label}</div>
          </div>
        ))}
      </div>
          ...
          <div key={f.fahrer_id} className="space-y-0.5">
            <div className="flex items-center gap-1.5">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-[10px] font-bold text-orange-900 dark:text-orange-200">{f.storno_pct.toFixed(1)}%</span>
              </div>
            </div>
            <div className="flex items-center gap-1">
              <div className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-full h-1.5">
                <div
                  ...
                  style={{ width: `${(f.storno_pct / maxPct) * 100}%` }}
                />
              </div>
            </div>
          </div>
      ...
      <div className="flex items-center justify-between text-[10px] text-gray-500 dark:text-gray-400 border-t border-orange-100 dark:border-orange-900 pt-1.5">
        <span>Ziel: <span className="font-medium text-orange-900 dark:text-orange-200">≤ 5% Storno</span></span>

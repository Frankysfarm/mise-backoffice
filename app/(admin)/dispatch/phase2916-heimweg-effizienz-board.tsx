...
import { ChevronDown, ChevronUp, AlertTriangle, TrendingUp, TrendingDown, Minus, Home } from 'lucide-react';
...
// Trend ist invertiert: fallend = gut (weniger Min), steigend = schlecht
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Julia F.', heimweg_min: 6.3,  heimweg_min_vw: 7.5,  trend: 'fallend',  trend_delta: -1.2, ampel: 'gruen', alert_zu_lang: false },
    { fahrer_id: 'd2', fahrer_name: 'Max M.',   heimweg_min: 8.5,  heimweg_min_vw: 7.3,  trend: 'steigend', trend_delta:  1.2, ampel: 'gruen', alert_zu_lang: false },
    { fahrer_id: 'd3', fahrer_name: 'Sara K.',  heimweg_min: 14.2, heimweg_min_vw: 15.5, trend: 'fallend',  trend_delta: -1.3, ampel: 'gelb',  alert_zu_lang: false },
    { fahrer_id: 'd4', fahrer_name: 'Tim B.',   heimweg_min: 23.1, heimweg_min_vw: 21.0, trend: 'steigend', trend_delta:  2.1, ampel: 'rot',   alert_zu_lang: true  },
  ],
  team_avg_heimweg_min: 13.0,
  team_avg_heimweg_min_vw: 12.8,
  alert_count: 1,
};

export function DispatchPhase2916HeimwegEffizienzBoard({ locationId }: { locationId: string | null }) {
  ...
  const best      = sorted[0]?.heimweg_min ?? 0;
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Heimweg-Effizienz Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 text-sm">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: Heimweg zu lang! {f.heimweg_min.toFixed(1)} Min
              </span>
            </div>
          ))}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø',  value: `${data.team_avg_heimweg_min.toFixed(1)} Min`, cls: teamText },
              { label: 'Bester',  value: `${best.toFixed(1)} Min`,                      cls: 'text-green-600' },
              { label: 'Ziel',    value: `≤${ZIEL} Min`,                               cls: 'text-gray-600 dark:text-gray-400' },
            ].map(k => (
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.heimweg_min.toFixed(1)} Min</span>
                    </div>
                  </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 inline-block" />≤{ZIEL} Min</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />{ZIEL + 1}–{WARN} Min</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />&gt;{WARN} Min</span>
          </div>
        </div>
      )}
    </div>

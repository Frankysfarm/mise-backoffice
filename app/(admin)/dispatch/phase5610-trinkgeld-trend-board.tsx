...
// Phase 5610 — Trinkgeld-Trend-Board (Dispatch)
// Coins green-400; trinkgeld_delta ABSTEIGEND Rang 1=größte positive Verbesserung=bester;
...
const MOCK: ApiResponse = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 1, trinkgeld_delta:  0.60, aktuell_avg: 2.80, rank_delta:  1, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f2', fahrer_name: 'Max M.',   rang: 2, trinkgeld_delta:  0.30, aktuell_avg: 2.10, rank_delta:  0, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, trinkgeld_delta: -0.20, aktuell_avg: 1.50, rank_delta: -1, ampel: 'gelb',  alert_rueckfall: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, trinkgeld_delta: -0.80, aktuell_avg: 0.40, rank_delta:  0, ampel: 'rot',   alert_rueckfall: true  },
  ],
  team_avg_delta: -0.03,
  bester_name: 'Julia F.',
...
export function DispatchPhase5610TrinkgeldTrendBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxAbs   = Math.max(...data.fahrer.map(f => Math.abs(f.trinkgeld_delta)), 0.01);
    ...
    <div className={`rounded-lg bg-gray-900 border ${data.alert_count > 0 ? 'border-red-700/60' : 'border-gray-700/50'} p-3 space-y-2.5`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-white">Trinkgeld-Trend</span>
      ...
      <div className="grid grid-cols-3 gap-1.5 text-center">
        <div className="rounded bg-gray-800 px-2 py-1.5">
          ...
          <div className="text-xs font-bold text-green-400">{bester ? fmtDelta(bester.trinkgeld_delta) : '—'}</div>
        ...
        <div className="rounded bg-gray-800 px-2 py-1.5">
          ...
          <div className="text-xs font-bold text-red-400">{schwaech ? fmtDelta(schwaech.trinkgeld_delta) : '—'}</div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 h-1.5 rounded-full bg-gray-800">
              <div
                ...
                style={{ width: `${(Math.abs(f.trinkgeld_delta) / maxAbs) * 100}%`, backgroundColor: barColor(f.trinkgeld_delta) }}
              />
            </div>
            <span className="text-[10px] font-mono text-green-300 w-12 text-right">{f.aktuell_avg.toFixed(2)}€</span>
            <span className="text-[10px] font-mono text-gray-400 w-10 text-right">{fmtDelta(f.trinkgeld_delta)}</span>

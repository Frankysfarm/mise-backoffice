...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Tim B.',   co2_kg: 15.8, trend: 'fallend',  trend_delta: -0.5 },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  co2_kg: 17.0, trend: 'fallend',  trend_delta: -0.3 },
    { fahrer_id: 'f3', fahrer_name: 'Max M.',   co2_kg: 19.8, trend: 'steigend', trend_delta:  1.4 },
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', co2_kg: 28.2, trend: 'steigend', trend_delta:  1.9 },
  ],
  team_avg_co2: 20.2,
  alert_count: 1,
};

export function DispatchPhase2996Co2Board({ locationId }: { locationId: string | null }) {
  ...
  const enriched = data.fahrer.map(f => ({ ...f, ampel: calcAmpel(f.co2_kg) }));
  // aufsteigend — niedrigste CO2 = umweltfreundlichste oben
  const sorted   = [...enriched].sort((a, b) => a.co2_kg - b.co2_kg);
  const alerts   = enriched.filter(f => f.co2_kg > ALERT_CO2);
  const hasAlert = alerts.length > 0;
  const best     = sorted[0]?.co2_kg ?? 0;
  const teamAmpel = calcAmpel(data.team_avg_co2);
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">CO2-Ausstoss-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø', value: `${data.team_avg_co2.toFixed(1)} kg`, cls: teamText },
              { label: 'Bester', value: `${best.toFixed(1)} kg`,              cls: 'text-green-600' },
              { label: 'Ziel',   value: `≤${ZIEL_CO2} kg`,                   cls: 'text-gray-600 dark:text-gray-400' },
            ].map(k => (
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.co2_kg.toFixed(1)} kg CO2</span>
                    </div>
                  </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 inline-block" />≤15 kg</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />15–25 kg</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />&gt;25 kg</span>
            <span className="text-gray-400">| Ziel ≤{ZIEL_CO2} kg</span>
          </div>
        </div>
      )}
    </div>

    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-900 text-sm">Lieferzeit-Ranking</span>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-emerald-50 rounded-lg p-2">
          <div className="text-base font-black text-emerald-700">{data.fahrer[0]?.avg_min} min</div>
        ...
        <div className={`rounded-lg p-2 ${data.alert_count > 0 ? 'bg-red-50' : 'bg-gray-50'}`}>
          ...
          <div className="text-[10px] text-gray-500">Längste</div>
        </div>
      </div>
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 bg-orange-50 border border-orange-200 rounded-lg text-xs text-orange-800">
          ...
          <span>{data.alert_count} Fahrer mit langen Lieferzeiten!</span>
        </div>
      )}

      {/* Ranking-Liste (aufsteigend: kürzeste Lieferzeit = Rang 1 = bester) */}
        ...
        {data.fahrer.map(f => {
          ...
          return (
            <div key={f.fahrer_id} className={`rounded-lg border ${c.border} ${c.bg} p-2.5 space-y-1.5`}>
              ...
              {f.alert_langsam && (
                <div className="text-[10px] text-orange-700 font-medium">Lange Lieferzeiten!</div>
              )}
            </div>
          );
        })}
      ...
      <div className="text-[10px] text-gray-400 text-center">
        Ziel ≤{data.ziel_min} min · Rang 1=kürzeste Lieferzeit=bester · 30-Min-Polling
      </div>
    </div>
  );
}

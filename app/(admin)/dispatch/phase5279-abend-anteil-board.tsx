...
export function DispatchPhase5279AbendAnteilBoard({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-amber-700 bg-amber-900/30 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-gray-200">Abendanteil-Ranking (letzte 30 Tage)</span>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-lg bg-amber-900/50 px-3 py-2 text-center">
          ...
          <div className="text-xs font-bold text-amber-300 truncate">{data.hoechste_name}</div>
        </div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 h-2 rounded-full bg-gray-800 overflow-hidden">
              <div
                ...
                style={{ width: `${f.abend_pct}%` }}
              />
            </div>
            <span className="text-xs text-gray-400 w-10 text-right tabular-nums">
              {f.abend_pct.toFixed(0)}%
            </span>
      ...
      {data.hoechste_name && (
        <div className="mt-3 text-[10px] text-amber-400 border-t border-amber-800/40 pt-2">
          Höchste: {data.hoechste_name} — {data.fahrer[0]?.abend_pct.toFixed(0)}% Abend-Touren (18–22 Uhr)
        </div>
      )}
    </div>
  );

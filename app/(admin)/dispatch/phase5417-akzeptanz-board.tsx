...
// ThumbsUp green-400; akzeptanz_rate ABSTEIGEND Rang 1=höchste Rate=bester;
...
export function DispatchPhase5417AkzeptanzBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxRate = Math.max(...data.fahrer.map(f => f.akzeptanz_rate), 0.01);
    ...
    <div className="rounded-xl bg-gray-900 border border-gray-700/50 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-white">Akzeptanzrate-Ranking</span>
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="bg-gray-800 rounded-lg p-2 text-center">
          ...
          <div className="text-xs font-bold text-gray-200">{data.team_avg_akzeptanz.toFixed(1)} %</div>
        </div>
        <div className="bg-gray-800 rounded-lg p-2 text-center">
      ...
      <div className="space-y-1.5">
        {data.fahrer.map(f => (
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono text-gray-300 w-12 text-right">{f.akzeptanz_rate.toFixed(0)} %</span>
            <div className="w-20 bg-gray-800 rounded-full h-1.5 shrink-0">
              <div
                className="h-1.5 rounded-full transition-all"
                style={{ width: `${(f.akzeptanz_rate / maxRate) * 100}%`, backgroundColor: ampelColor(f.ampel) }}
              />
            </div>
      ...
      <div className="text-[10px] text-gray-600 text-right">Auftrags-Akzeptanzrate · 30-Min-Update · {data.gesamt} Fahrer</div>
    </div>
  );
}

...
interface ApiData { fahrer: FahrerRow[]; team_avg_stopps_h: number; bester_name: string; letzter_name: string; alert_count: number; gesamt: number; }

const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'p1', fahrer_name: 'Julia F.', rang: 1, stopps_pro_stunde: 4.8, rank_delta:  1, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'p2', fahrer_name: 'Max M.',   rang: 2, stopps_pro_stunde: 4.2, rank_delta:  0, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'p3', fahrer_name: 'Sara K.',  rang: 3, stopps_pro_stunde: 3.1, rank_delta: -1, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'p4', fahrer_name: 'Tim B.',   rang: 4, stopps_pro_stunde: 2.0, rank_delta:  0, ampel: 'rot',   alert_bottom: true  },
  ],
  team_avg_stopps_h: 3.525, bester_name: 'Julia F.', letzter_name: 'Tim B.', alert_count: 1, gesamt: 4,
};

interface Props { locationId: string | null; }

export function DispatchPhase4107StoppEffizienzBoard({ locationId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/delivery/admin/fahrer-stopp-effizienz-ranking?location_id=${locationId}`);
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Stopp-Effizienz Ranking</span>
        </div>
        <div className="flex items-center gap-2">
          {loading && <span className="w-3 h-3 border-2 border-gray-300 border-t-transparent rounded-full animate-spin" />}
          {!loading && data.alert_count > 0 && (
            <span className="flex items-center gap-1 text-xs text-red-600 font-semibold">
              <AlertTriangle className="w-3.5 h-3.5" /> {data.alert_count} Niedrige Effizienz!
            </span>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-rose-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-gray-500">Effizienteste</div>
          <div className="text-sm font-bold text-rose-600">{bester?.stopps_pro_stunde?.toFixed(1)} Sph</div>
        ...
        <div className="bg-gray-50 rounded-lg p-2 text-center">
          ...
          <div className="text-sm font-bold text-gray-700">{data.team_avg_stopps_h?.toFixed(1)} Sph</div>
          <div className="text-[10px] text-gray-400">{data.gesamt} Fahrer</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-gray-500">Wenigste</div>
          <div className="text-sm font-bold text-red-500">{letzter?.stopps_pro_stunde?.toFixed(1)} Sph</div>
      ...
      <div className="space-y-1.5">
        {data.fahrer.map((f) => {
          const barW = `${(f.stopps_pro_stunde / maxSph) * 100}%`;
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.stopps_pro_stunde?.toFixed(1)} Sph</span>
      ...
      <div className="flex justify-between text-[10px] text-gray-400 px-1 pt-1 border-t border-gray-100">
        <span>Rang 1 = meiste Stopps/h (höchste Effizienz)</span>
        <span>Avg {data.team_avg_stopps_h?.toFixed(1)} Sph</span>
      </div>
    </div>
  );
}

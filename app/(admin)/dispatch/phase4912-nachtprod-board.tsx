...
export function DispatchPhase4912NachtprodBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="rounded-2xl border border-indigo-700 bg-indigo-950/40 overflow-hidden mb-4">
      ...
      <div className="px-4 py-3 flex items-center justify-between border-b border-indigo-700/50 bg-indigo-900/20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-indigo-200">Nachtschicht-Produktivität (21–02 Uhr)</span>
        </div>
        {data.alert_count > 0 && (
      ...
      {/* KPI Grid */}
      <div className="grid grid-cols-3 divide-x divide-indigo-800/50 border-b border-indigo-800/50">
        {[
          ...
          <div key={k.label} className="flex flex-col items-center py-3 gap-0.5">
            <span className="text-sm font-bold text-indigo-100">{k.val}</span>
            <span className="text-[10px] text-indigo-400">{k.label}</span>
          ...
          <div key={f.fahrer_id} className="px-4 py-2.5 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {f.alert_hoch && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-indigo-900/70 text-indigo-300">
                    &gt;3,0 T/h
                  </span>
                )}
              </div>
              <div className="mt-1 h-1.5 bg-indigo-900/50 rounded-full overflow-hidden">
                <div
      ...
      <div className="px-4 py-2 border-t border-indigo-800/30 flex items-center justify-between">

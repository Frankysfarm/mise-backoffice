...
import { Route, Clock, Zap, MapPin, CheckCircle2, AlertTriangle, TrendingUp, Navigation, BarChart2, Target } from 'lucide-react';
  ...
  touren: [
    {
      ...
      laufend: true, eta_rest_min: 8, km_bisher: 4.2, zone: 'Nord',
      stops: [
    ...
    },
    {
      ...
      laufend: true, eta_rest_min: 14, km_bisher: 3.1, zone: 'Mitte',
      stops: [
    ...
    },
    {
      ...
      laufend: true, eta_rest_min: 22, km_bisher: 1.8, zone: 'Süd',
      stops: [
...
function scoreBg(s: number) {
  return s >= 85 ? 'bg-green-50' : s >= 70 ? 'bg-yellow-50' : s >= 50 ? 'bg-orange-50' : 'bg-red-50';
}
...
export function DispatchPhase4395TourScoreVisualisierungV5({ locationId }: Props) {
  ...
  const fleetTc = scoreColor(data.fleet_score);
  const delayedTours = data.touren.filter((t) => t.stops.some((s) => s.versp_min > 0));

  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-3 space-y-2.5">

      {/* Delay-Alert */}
      {delayedTours.length > 0 && (
        <div className="flex items-center gap-1.5 px-2 py-1 bg-amber-50 border border-amber-200 rounded-lg">
          <AlertTriangle className="w-3 h-3 text-amber-500 flex-shrink-0" />
          <span className="text-[10px] font-medium text-amber-700">
            {delayedTours.length} Tour{delayedTours.length > 1 ? 'en' : ''} mit Verspätung: {delayedTours.map((t) => t.fahrer_name).join(', ')}
          </span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-gray-900">Tour-Score V5</span>
          {loading && <span className="w-2 h-2 border-2 border-blue-300 border-t-transparent rounded-full animate-spin" />}
        </div>
        <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded-full ${scoreBg(data.fleet_score)}`}>
          <span className="text-[9px] font-bold text-gray-400">Fleet</span>
          <span className={`text-xs font-bold ${fleetTc}`}>{data.fleet_score}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-5 gap-1 text-center">
        <div className="bg-blue-50 rounded-lg p-1.5">
          <p className="text-[7px] text-blue-400 font-bold uppercase tracking-wide">Score</p>
          <p className={`text-sm font-bold ${fleetTc}`}>{data.fleet_score}</p>
        </div>
        <div className="bg-green-50 rounded-lg p-1.5">
          <p className="text-[7px] text-green-500 font-bold uppercase tracking-wide">Pünktl.</p>
          <p className="text-sm font-bold text-green-700">{data.on_time_pct}%</p>
        </div>
        <div className="bg-purple-50 rounded-lg p-1.5">
          <p className="text-[7px] text-purple-400 font-bold uppercase tracking-wide">Touren</p>
          <p className="text-sm font-bold text-purple-700">{data.aktive_touren}</p>
        </div>
        <div className="bg-amber-50 rounded-lg p-1.5">
          <p className="text-[7px] text-amber-500 font-bold uppercase tracking-wide">Ø Liefer</p>
          <p className="text-sm font-bold text-amber-700">{data.avg_lieferzeit_min}m</p>
        </div>
        <div className="bg-teal-50 rounded-lg p-1.5">
          <p className="text-[7px] text-teal-500 font-bold uppercase tracking-wide">ETA-Güte</p>
          <p className="text-sm font-bold text-teal-700">{data.eta_genauigkeit_pct}%</p>
        </div>
      </div>
      ...
      <div className="flex gap-1 flex-wrap">
        {data.touren.map((t) => (
          <span key={t.id} className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-medium ${scoreBg(t.score)} border border-gray-100`}>
            <span className={`w-1.5 h-1.5 rounded-full ${t.score >= 85 ? 'bg-green-400' : t.score >= 70 ? 'bg-yellow-400' : 'bg-red-400'}`} />
            {t.fahrer_name} · {t.zone}
          </span>
        ))}
      </div>
      ...
      <div className="space-y-1.5">
        {data.touren.map((tour) => {
          const tc = scoreColor(tour.score);
          ...
          const hasDelay = tour.stops.some((s) => s.versp_min > 0);

          return (
            <div key={tour.id} className={`border rounded-lg overflow-hidden ${hasDelay ? 'border-amber-200' : 'border-gray-100'}`}>
              <button
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-[8px] text-gray-400 flex-shrink-0">{tour.zone}</span>
                    {tour.score_delta !== 0 && (
                    ...
                    {hasDelay && <AlertTriangle className="w-3 h-3 text-amber-500 flex-shrink-0" />}
                  </div>
              ...
              <div className="flex items-center gap-3 px-2.5 py-1 text-[9px] text-gray-500 border-t border-gray-100 bg-white">
                <span className="flex items-center gap-0.5"><CheckCircle2 className="w-2.5 h-2.5 text-green-400" />{tour.abgeschlossene_stopps}/{totalStops}</span>
                ...
                <span className="flex items-center gap-0.5"><BarChart2 className="w-2.5 h-2.5 text-gray-400" />{tour.km_bisher}km</span>
              ...
              {isExpanded && (
                <div className="px-2.5 py-1.5 space-y-1.5 border-t border-gray-100 bg-white">
                  {tour.stops.map((stop, idx) => {
                    const isDone = stop.status === 'abgeschlossen';
                    const isActive = stop.status === 'aktiv';
                    const hasStopDelay = stop.versp_min > 0;
                    const isLast = idx === tour.stops.length - 1;

                    return (
                      <div key={stop.id} className="flex items-start gap-1.5">
                        <div className="flex flex-col items-center">
                          <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${isDone ? 'bg-green-100' : isActive ? 'bg-blue-100' : 'bg-gray-100'}`}>
                            {isDone
                        ...
                        <div className="flex-1 min-w-0 pb-1">
                          <div className="flex items-center gap-1">
                            <span className="text-[9px] text-gray-500 font-medium">{stop.bestellnummer}</span>
                            ...
                            {stop.eta_min != null && !isDone && (
                              <span className={`text-[9px] font-bold flex-shrink-0 flex items-center gap-0.5 ${hasStopDelay ? 'text-red-500' : 'text-gray-400'}`}>
                                {hasStopDelay && <AlertTriangle className="w-2 h-2" />}
                                {stop.eta_min}m
                                {hasStopDelay && <span className="text-[8px]">(+{stop.versp_min}m)</span>}
                              </span>
                            )}
                          </div>
                        </div>
      ...
      <div className="flex items-center justify-between text-[8px] text-gray-400 pt-1 border-t border-gray-100">
        <span className="flex items-center gap-0.5"><Target className="w-2 h-2" />Score: grün ≥85 · gelb ≥70 · orange ≥50 · rot &lt;50</span>

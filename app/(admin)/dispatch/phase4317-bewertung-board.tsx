...
export function DispatchPhase4317BewertungBoard({ locationId }: Props) {
  ...
  const maxScore = Math.max(...data.fahrer.map(f => f.score), 0.01);
    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-gray-900">Fahrer-Bewertungen</span>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        <div className="bg-yellow-50 rounded-xl p-2">
          <div className="font-bold text-yellow-700">{bester?.score.toFixed(1)} ★</div>
          <div className="text-gray-500 truncate">{data.bester_name}</div>
          <div className="text-[10px] text-gray-400">Bester</div>
        </div>
        <div className="bg-gray-50 rounded-xl p-2">
          <div className="font-bold text-gray-700">{data.team_avg_score.toFixed(1)} ★</div>
          <div className="text-gray-500">{data.gesamt} Fahrer</div>
          <div className="text-[10px] text-gray-400">Team-Ø</div>
        </div>
        <div className="bg-red-50 rounded-xl p-2">
          <div className="font-bold text-red-600">{niedrigster?.score.toFixed(1)} ★</div>
          <div className="text-gray-500 truncate">{data.niedrigster_name}</div>
      ...
      <div className="space-y-1.5">
        {data.fahrer.map((f) => {
          ...
          const barColor = f.ampel === 'gruen' ? 'bg-yellow-400' : f.ampel === 'gelb' ? 'bg-yellow-300' : 'bg-red-400';
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-900 w-10 text-right">{f.score.toFixed(1)} ★</span>
              </div>

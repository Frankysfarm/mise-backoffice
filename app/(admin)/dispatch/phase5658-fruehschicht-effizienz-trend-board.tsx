...
// Phase 5658 — Frühschicht-Effizienz-Trend-Board (Dispatch) — Batch 109
// Sunrise amber-400; effizienz_delta ABSTEIGEND Rang 1=größte Verbesserung=bester;
...
const MOCK: ApiResponse = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   rang: 1, effizienz_delta:  0.6, aktuell_touren_pro_std: 2.8, rank_delta:  1, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', rang: 2, effizienz_delta:  0.3, aktuell_touren_pro_std: 2.5, rank_delta:  0, ampel: 'gruen', alert_rueckfall: false },
...
function barColor(delta: number): string {
  if (delta > 0)   return '#fbbf24'; // amber-400
...
export function DispatchPhase5658FruehschichtEffizienzTrendBoard({
  locationId,
}: {
  ...
  return (
    <div className={`rounded-lg bg-gray-900 border ${data.alert_count > 0 ? 'border-red-700/60' : 'border-amber-700/40'} p-3 space-y-2.5`}>
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          <Sunrise className="h-3.5 w-3.5 text-amber-400" />
          <span className="text-xs font-semibold text-white">Frühschicht-Effizienz-Trend</span>
      ...
      <div className="grid grid-cols-3 gap-1.5 text-center">
        <div className="bg-gray-800 rounded p-1.5">
          ...
          <div className="text-xs font-semibold text-amber-400 truncate">{data.bester_name}</div>
        ...
        <div className="bg-gray-800 rounded p-1.5">
          ...
          <div className={`text-xs font-mono font-semibold ${data.team_avg_delta > 0 ? 'text-amber-400' : data.team_avg_delta < 0 ? 'text-red-400' : 'text-gray-400'}`}>
      ...
      <div className="space-y-1">
        {data.fahrer.map(f => (
          <div key={f.fahrer_id} className={`flex items-center gap-2 rounded px-2 py-1 ${f.alert_rueckfall ? 'bg-red-900/20' : 'bg-gray-800/50'}`}>
            ...
            <span className={`text-[10px] font-mono w-16 text-right ${f.effizienz_delta > 0 ? 'text-amber-400' : f.effizienz_delta < 0 ? 'text-red-400' : 'text-gray-400'}`}>
      ...
      <div className="text-[9px] text-gray-600 text-right">6–11h · 30-Tage-Trend · Touren/Std</div>
    </div>
  );
}

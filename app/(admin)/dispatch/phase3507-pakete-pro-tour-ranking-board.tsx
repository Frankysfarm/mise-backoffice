...
export function DispatchPhase3507PaketeProTourRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-pakete-pro-tour?location_id=${locationId}`, { cache: 'no-store' });
  ...
  const maxPakete = Math.max(...data.fahrer.map(f => f.pakete_pro_tour), 1);
        ...
        <div className="px-3 pb-3 space-y-2">
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1.5 p-2 rounded bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 text-xs border border-red-200">
              ...
              Niedrige Pakete/Tour! {data.alert_count} Fahrer unter Zielwert
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2 py-1">
            ...
            <div className="text-center">
              ...
              <div className="text-xs font-semibold">{data.team_avg}/Tour</div>
            </div>
            <div className="text-center">
              ...
              <div key={f.fahrer_id} className="flex items-center gap-2">
                ...
                <div className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-full h-2 relative">
                  <div
                    ...
                    style={{ width: `${(f.pakete_pro_tour / maxPakete) * 100}%` }}
                  />
                </div>

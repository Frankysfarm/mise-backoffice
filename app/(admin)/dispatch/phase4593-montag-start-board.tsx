...
import { Sunrise, TrendingUp, TrendingDown, Minus, WifiOff } from 'lucide-react';
...
export function DispatchPhase4593MontagStartBoard({ locationId }: { locationId: string | null }) {
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-sm">Montag-Start-Board nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-orange-200 dark:border-orange-800 bg-white dark:bg-gray-900 p-4 space-y-4">
      {/* Header */}
      <div className="flex items-center gap-2">
        <Sunrise className="w-5 h-5 text-orange-500" />
        <h3 className="font-semibold text-gray-900 dark:text-gray-100">Montag-Start-Ranking</h3>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-orange-50 dark:bg-orange-950 rounded-xl p-2">
          <p className="text-[10px] text-gray-500 dark:text-gray-400">Höchste</p>
          <p className="text-sm font-bold text-orange-600 dark:text-orange-400 truncate">{data.hoechste_name}</p>
        </div>
        <div className="bg-orange-50 dark:bg-orange-950 rounded-xl p-2">
          <p className="text-[10px] text-gray-500 dark:text-gray-400">Team-Ø</p>
          <p className="text-sm font-bold text-orange-600 dark:text-orange-400">{data.team_avg}%</p>
        </div>
        <div className="bg-orange-50 dark:bg-orange-950 rounded-xl p-2">
          <p className="text-[10px] text-gray-500 dark:text-gray-400">Niedrigste</p>
          <p className="text-sm font-bold text-orange-600 dark:text-orange-400 truncate">{data.niedrigste_name}</p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {data.fahrer.map((f) => (
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            {f.alert_niedrig && (
              <span className="text-[10px] text-red-500 shrink-0">Wenig Montag-Starts!</span>
            )}
          </div>
        ))}
      </div>

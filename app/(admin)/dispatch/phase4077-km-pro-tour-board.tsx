...
export function DispatchPhase4077KmProTourBoard({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-violet-50 rounded-lg p-2">
          <p className="text-[10px] text-gray-500 mb-0.5">Effizienteste</p>
          <p className="text-sm font-bold text-violet-700">{bester?.km_avg?.toFixed(1)} km</p>
        ...
        <div className="bg-gray-50 rounded-lg p-2">
          ...
          <p className="text-[10px] text-gray-400">Rang 1 = kürzeste</p>
        </div>
        <div className="bg-red-50 rounded-lg p-2">
          <p className="text-[10px] text-gray-500 mb-0.5">Längste km</p>
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              ...
              <div className="ml-6 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${barColor}`} style={{ width: `${(f.km_avg / maxKm) * 100}%` }} />
              </div>
            </div>
          );
        })}

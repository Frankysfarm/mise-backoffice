...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f3', fahrer_name: 'Tim W.',   avg_min: 13.1, touren_heute:  7, trend: 'steigend', trend_delta:  2.1, rang: 1 },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  avg_min:  7.8, touren_heute:  9, trend: 'fallend',  trend_delta: -1.3, rang: 2 },
    { fahrer_id: 'f4', fahrer_name: 'Anna B.',  avg_min:  5.2, touren_heute: 11, trend: 'stabil',   trend_delta:  0.4, rang: 3 },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   avg_min:  3.5, touren_heute: 12, trend: 'fallend',  trend_delta: -0.7, rang: 4 },
  ],
  team_durchschnitt: 7.4,
  alert_count: 1,
};

export function DispatchPhase2630WarteZeitBestellungBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-4`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800">Wartezeit Bestellung</span>
          {hasAlert && (
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3">
          {hasAlert && (
            <div className="flex items-start gap-2 bg-red-100 border border-red-200 rounded-lg px-3 py-2">
              ...
              <p className="text-xs text-red-700 font-medium">
                Wartezeit &gt;10 Min — Depot-Übergabe beschleunigen: {alerts.map((f: { fahrer_name: string }) => f.fahrer_name).join(', ')}
              </p>
            </div>
          )}
          ...
          <div className="text-xs text-gray-400 text-right">Ziel ≤5 Min — alle 30 Min aktualisiert</div>
        </div>
      )}
    </div>
  );

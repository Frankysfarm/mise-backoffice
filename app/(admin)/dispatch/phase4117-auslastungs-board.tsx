...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; auslastung_pct: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_bottom: boolean; }
...
const MOCK: ApiData = {
  ...
  team_avg_pct: 59.5, bester_name: 'Max M.', letzter_name: 'Tim B.', alert_count: 1, gesamt: 4,
};

interface Props { locationId: string | null; }

export function DispatchPhase4117AuslastungsBoard({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Auslastungs-Ranking</span>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-orange-50 rounded-lg p-2">
          <div className="text-[10px] text-orange-600 font-medium mb-0.5">Ausgelastetste</div>
          <div className="text-sm font-bold text-orange-700 truncate">{data.bester_name}</div>
          <div className="text-xs text-orange-600">{besterFahrer?.auslastung_pct?.toFixed(1)} %</div>
        </div>
        ...
        <div className="bg-red-50 rounded-lg p-2">
          ...
          <div className="text-xs text-red-600">{letzterFahrer?.auslastung_pct?.toFixed(1)} %</div>
        </div>
      </div>
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.auslastung_pct?.toFixed(1)} %</span>
              </div>
              <div className="w-full bg-gray-100 rounded-full h-1.5">
                <div className={`h-1.5 rounded-full ${barColor}`} style={{ width: `${barW}%` }} />
              </div>
              {f.alert_bottom && (
                <p className="text-[10px] text-red-500 font-medium">Niedrige Auslastung!</p>
              )}
            </div>
          );
        })}

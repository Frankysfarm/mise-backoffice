...
export function DispatchPhase5173AvgLieferzeitBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="rounded-2xl border border-teal-700 bg-teal-950/40 overflow-hidden mb-4">
      <div className="px-4 py-3 flex items-center justify-between border-b border-teal-700/50 bg-teal-900/20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-teal-200">Ø Lieferzeit je Fahrer (letzte 30 Tage)</span>
        </div>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 divide-x divide-teal-800/40 border-b border-teal-700/30">
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-green-300">{schnellste?.avg_lieferzeit_min?.toFixed(1) ?? '–'} min</div>
        ...
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-teal-200">{data.team_avg_min.toFixed(1)} min</div>
          <div className="text-[10px] text-gray-500">Ø Lieferzeit</div>
        </div>
        <div className="px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500 mb-0.5">Längste</div>
          <div className="text-sm font-bold text-gray-400">{langsamste?.avg_lieferzeit_min?.toFixed(1) ?? '–'} min</div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="w-20 h-1.5 rounded-full bg-gray-800 overflow-hidden shrink-0">
              <div
                ...
                style={{ width: `${Math.round((f.avg_lieferzeit_min / maxMin) * 100)}%` }}
              />
            </div>
            <span className={`text-xs font-semibold w-14 text-right shrink-0 ${ampelColor(f.ampel)}`}>
              {f.avg_lieferzeit_min.toFixed(1)} min
            </span>
      ...
      <div className="px-4 py-2 border-t border-teal-800/30 bg-teal-900/10">
        <div className="text-[10px] text-gray-500">
          ...
          Langsamste: <span className="text-gray-400">{data.langsamster_name}</span>
        </div>
      </div>
    </div>
  );

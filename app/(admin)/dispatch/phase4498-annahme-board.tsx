...
interface AnnahmeData {
  fahrer: FahrerAnnahme[];
  team_avg_pct: number;
  alert_count: number;
  bester_name: string;
  letzter_name: string;
...
export function DispatchPhase4498AnnahmeBoard({ locationId }: Props) {
  const [data, setData] = useState<AnnahmeData | null>(null);
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900 dark:text-gray-100 text-sm">Annahme-Quote-Ranking (30 Tage)</h3>
        <span className="ml-auto text-xs text-gray-400">% Angenommen</span>
      </div>

      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="rounded-lg bg-emerald-50 dark:bg-emerald-900/20 p-2">
          <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400">{best?.annahme_pct.toFixed(1)}%</div>
        ...
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          <div className="text-lg font-bold text-gray-500 dark:text-gray-400">{worst?.annahme_pct.toFixed(1)}%</div>
      ...
      {alerts.length > 0 && (
        <div className="flex items-center gap-2 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 px-3 py-2">
          ...
          <span className="text-xs text-red-700 dark:text-red-300">
            Niedrige Annahme-Quote: {alerts.map(a => a.fahrer_name).join(', ')}
          </span>
        </div>
      )}
        ...
        {data.fahrer.map((f) => (
          <div key={f.fahrer_id} className="space-y-1">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className={`font-semibold text-xs ${ampelColor(f.ampel)}`}>{f.annahme_pct.toFixed(1)}%</span>
            </div>
            <div className="h-1.5 rounded-full bg-gray-100 dark:bg-gray-800">
              <div
                className={`h-full rounded-full ${ampelBg(f.ampel)}`}
                style={{ width: `${maxPct > 0 ? Math.round((f.annahme_pct / maxPct) * 100) : 0}%` }}
              />
            </div>
          </div>
        ))}
      ...
      <div className="text-xs text-gray-400">Ziel: ≥90% · {data.gesamt} Fahrer</div>
    </div>
  );
}

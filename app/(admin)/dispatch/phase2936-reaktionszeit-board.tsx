...
function TrendIcon({ trend, invertColors }: { trend: string; invertColors: boolean }) {
  if (trend === 'steigend') return <TrendingUp   size={12} className={invertColors ? 'text-red-500'   : 'text-green-600'} />;
  if (trend === 'fallend')  return <TrendingDown size={12} className={invertColors ? 'text-green-600' : 'text-red-500'  } />;
  return                           <Minus        size={12} className="text-gray-400"  />;
}

const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Max M.',   avg_min: 1.8, trend: 'fallend',  trend_delta: -0.3 },
    { fahrer_id: 'd4', fahrer_name: 'Anna B.',  avg_min: 2.9, trend: 'stabil',   trend_delta:  0.0 },
    { fahrer_id: 'd2', fahrer_name: 'Sara K.',  avg_min: 4.5, trend: 'steigend', trend_delta:  0.8 },
    { fahrer_id: 'd3', fahrer_name: 'Tim W.',   avg_min: 7.2, trend: 'steigend', trend_delta:  1.2 },
  ],
  team_durchschnitt: 4.1,
};

export function DispatchPhase2936ReaktionszeitBoard({ locationId }: { locationId: string | null }) {
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø',  value: `${data.team_durchschnitt.toFixed(1)} Min`, cls: teamText },
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      <TrendIcon trend={f.trend} invertColors />

...
function SternBar({ avg, max = 5 }: { avg: number; max?: number }) {
  const pct = Math.min(100, (avg / max) * 100);
  const color = avg >= 4.5 ? 'bg-green-500' : avg >= 3.5 ? 'bg-amber-400' : 'bg-red-500';
  return (
    <div className="relative h-1.5 rounded-full bg-gray-200 w-24">
      ...
      <div className="absolute top-0 h-full border-l border-dashed border-gray-400" style={{ left: `${(3.5 / max) * 100}%` }} />
      <div className="absolute top-0 h-full border-l border-dashed border-gray-600" style={{ left: `${(4.5 / max) * 100}%` }} />
    </div>
  );
}

function Sterne({ avg }: { avg: number }) {
  return (
...
export function DispatchPhase2426BewertungsBoard({ locationId }: { locationId: string | null }) {
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={`font-semibold text-sm ${hasAlert ? 'text-red-800' : 'text-yellow-800'}`}>
            Bewertungs-Board
            {data ? ` — Ø ${data.team_avg_sterne.toFixed(1)} ★` : ''}
          </span>
          {hasAlert && (
          ...
          {!data ? (
            ...
            <>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { label: 'Ø heute', value: `${data.team_avg_sterne.toFixed(1)} ★` },
                  { label: 'Ø VW', value: `${data.team_avg_sterne_vw.toFixed(1)} ★` },
                  { label: 'Ziel', value: '≥4,5 ★' },
                ].map(k => (
                  <div key={k.label} className="bg-white rounded-lg p-2 border border-yellow-100 text-center">
                    <p className="text-xs text-gray-500">{k.label}</p>
                    <p className="text-sm font-bold text-yellow-800">{k.value}</p>
                  </div>
                ))}
              </div>
              ...
              {hasAlert && (
                <div className="flex items-start gap-2 bg-red-100 border border-red-200 rounded-lg p-2">
                  ...
                  <p className="text-xs text-red-800">
                    Unter 3,5 ★: {data.fahrer.filter(f => f.alert_niedrig).map(f => f.fahrer_name).join(', ')} — Service sofort prüfen!
                  </p>
                </div>
              )}
              ...
              <div className="space-y-2">
                    ...
                    {(['🥇', '🥈', '🥉'] as const).map((medal, i) => {
                      ...
                      return (
                        <div key={f.fahrer_id} className={`flex-1 rounded-lg p-2 text-center border ${ampelColor(f.ampel)}`}>
                          ...
                          <p className="text-sm font-bold">{f.avg_sterne.toFixed(1)} ★</p>
                        </div>
                      );
                    })}
                  </div>
                ...
                {data.fahrer.map(f => (
                  <div key={f.fahrer_id} className="bg-white rounded-lg p-2 border border-yellow-100">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs font-bold text-gray-800">{f.avg_sterne.toFixed(1)} ★</span>
                        {f.trend_delta !== 0 && (
                    ...
                    <div className="flex items-center justify-between">
                      <SternBar avg={f.avg_sterne} />
                      <Sterne avg={f.avg_sterne} />
                    </div>
                    <div className="flex justify-between text-xs text-gray-400 mt-0.5">
                      <span>{f.anzahl_bewertungen} Bewertungen</span>
                      <span>VW {f.avg_sterne_vw.toFixed(1)} ★</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="flex gap-3 text-xs text-gray-500 pt-1">
                ...
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" /> 3,5–4,5 ★</span>

...
import { AlertTriangle, ChevronDown, ChevronUp, Minus, TrendingDown, TrendingUp, Zap } from 'lucide-react';
...
function DeltaIcon({ delta }: { delta: number }) {
  // For reaction time: negative delta = faster today = improved
...
export function DispatchPhase3141TourstartReaktionszeitBoard({ locationId }: { locationId: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-tourstart-reaktionszeit-ranking?location_id=${locationId}`)
        .then(r => r.json())
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">
            Tourstart-Reaktionszeit-Ranking heute
          </span>
        ...
        <div className="p-4 space-y-3">
          {alerts.length > 0 && (
            <div className="flex items-center gap-2 rounded-lg bg-red-50 border border-red-200 text-red-700 dark:text-red-300 dark:bg-red-950 dark:border-red-800 text-xs font-semibold px-3 py-2">
              ...
              {alerts.map(f => f.fahrer_name).join(', ')} — Langsamste Tourstart-Reaktion!
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-yellow-50 dark:bg-yellow-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <Zap size={10} className="text-yellow-500" /> Schnellster
              </div>
            ...
            <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-2">
              ...
              <div className="text-xs text-gray-500">Reaktionszeit</div>
            </div>
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  ...
                  {f.alert_bottom && (
                    <div className="mt-1 text-xs text-red-600 dark:text-red-400 font-semibold flex items-center gap-1">
                      <AlertTriangle size={10} /> Langsamste Tourstart-Reaktion!
                    </div>
                  )}
                </div>
              );
          ...
          <div className="flex flex-wrap gap-3 text-xs text-gray-500 pt-1">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500"   /> Bottom 25% (langsamste)</span>
          </div>
        </div>
      )}
    </div>

...
import { ChevronDown, ChevronUp, ShoppingCart, TrendingUp, TrendingDown, Minus, AlertTriangle } from 'lucide-react';
...
export function DispatchPhase3392BestellwertProTourRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-bestellwert-pro-tour?location_id=${locationId}`, { cache: 'no-store' });
  ...
  const maxWert = Math.max(...data.fahrer.map(f => f.bestellwert_pro_tour), 1);
        ...
        <div className="px-3 pb-3 space-y-2">
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1.5 p-2 rounded bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 text-xs border border-red-200">
              ...
              Niedriger Bestellwert/Tour! {data.alert_count} Fahrer unter Zielwert
            </div>
          )}

...
function ampelCls(a: string) {
  ...
  return                   { bg: 'bg-purple-50 border-purple-200 dark:bg-purple-950 dark:border-purple-800', dot: 'bg-purple-500', text: 'text-purple-700 dark:text-purple-300', bar: 'bg-purple-500' };
}
...
export function DispatchPhase3330LieferzeitPraezisionRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxMin = Math.max(...list.map(f => f.avg_abweichung_min), 1);
        ...
        <div className="p-4 space-y-3">
          {alerts.length > 0 && (
            <div className="flex items-center gap-2 rounded-lg bg-red-50 border border-red-200 text-red-700 dark:text-red-300 dark:bg-red-950 dark:border-red-800 text-xs font-semibold px-3 py-2">
              ...
              {alerts.map(f => f.fahrer_name).join(', ')} — Hohe ETA-Abweichung!
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-yellow-50 dark:bg-yellow-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <Target size={10} className="text-purple-500" /> Präzisester
              </div>
              <div className="font-bold text-sm text-yellow-700 dark:text-yellow-400">{bester?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{bester ? `±${bester.avg_abweichung_min} min` : '—'}</div>
            </div>
            ...
            <div className="rounded-lg bg-red-50 dark:bg-red-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5">Ungenauester</div>
              <div className="font-bold text-sm text-red-600 dark:text-red-400">{letzter?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{letzter ? `±${letzter.avg_abweichung_min} min` : '—'}</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {list.map(f => {
              ...
              const barW = maxMin > 0 ? Math.round((f.avg_abweichung_min / maxMin) * 100) : 0;
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className={`text-sm font-bold ${cls.text}`}>±{f.avg_abweichung_min} min</span>
                      {f.rank_delta < 0
          ...
          <div className="pt-1 border-t border-gray-100 dark:border-gray-700 flex justify-between text-xs text-gray-400">
            <span>Team-Ø: ±{d.team_avg_min} min · Ziel &lt; 5 min</span>
            <span>heute</span>
          </div>
        </div>
      )}

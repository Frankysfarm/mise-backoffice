...
export function DispatchPhase3624UmsatzProStundeRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxUmsatz = Math.max(...sorted.map(f => f.umsatz_pro_stunde), 1);

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        <TrendingUp className="w-5 h-5 text-emerald-500" />
        <h3 className="font-semibold text-gray-900">Umsatz/h — Ranking</h3>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        <div className="bg-emerald-50 rounded-lg p-2">
          <div className="font-bold text-emerald-700">{bester?.umsatz_pro_stunde.toFixed(1) ?? '–'} €/h</div>
          <div className="text-gray-500">Bester</div>
          <div className="text-gray-700 truncate">{bester?.fahrer_name ?? '–'}</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2">
          <div className="font-bold text-gray-700">{data.team_avg_umsatz.toFixed(1)} €/h</div>
          <div className="text-gray-500">Team-Ø</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2">
          <div className="font-bold text-red-700">{schlechtester?.umsatz_pro_stunde.toFixed(1) ?? '–'} €/h</div>
          ...
          <div key={f.fahrer_id} className={`flex items-center gap-2 p-2 rounded-lg border ${AMPEL_BG[f.ampel]}`}>
            ...
            <div className="w-20 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${Math.max((f.umsatz_pro_stunde / maxUmsatz) * 100, 4)}%` }}
              />
            </div>
            <span className={`text-xs font-bold w-16 text-right ${AMPEL_COLOR[f.ampel]}`}>{f.umsatz_pro_stunde.toFixed(1)} €/h</span>
            {f.rank_delta > 0 ? (

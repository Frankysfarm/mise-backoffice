...
export function DispatchPhase4147LeerfahrtenBoard({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-violet-50 rounded-lg p-2">
          <p className="text-[10px] text-violet-600 font-medium">Niedrigste</p>
          <p className="text-sm font-bold text-violet-700">{bester?.leerfahrten_pct?.toFixed(0)} %</p>
        ...
        <div className="bg-gray-50 rounded-lg p-2">
          ...
          <p className="text-sm font-bold text-gray-700">{data.team_avg_leerfahrten_pct?.toFixed(1)} %</p>
          <p className="text-[10px] text-gray-400">{data.gesamt} Fahrer</p>
        </div>
        <div className="bg-red-50 rounded-lg p-2">
          <p className="text-[10px] text-red-500 font-medium">Höchste</p>
          <p className="text-sm font-bold text-red-600">{letzter?.leerfahrten_pct?.toFixed(0)} %</p>
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.leerfahrten_pct?.toFixed(0)} %</span>
              </div>
      ...
      <div className="flex justify-between text-[10px] text-gray-400 pt-1 border-t border-gray-100">
        <span>Team-Ø {data.team_avg_leerfahrten_pct?.toFixed(1)} % Leerfahrten</span>

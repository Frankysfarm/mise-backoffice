...
// Phase 5662 — Spätabend-Pünktlichkeits-Trend-Board (Dispatch) — Batch 110
...
const MOCK: ApiResponse = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   rang: 1, puenktlichkeit_delta:  8.5, aktuell_pct: 91.0, rank_delta:  2, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', rang: 2, puenktlichkeit_delta:  4.2, aktuell_pct: 88.0, rank_delta:  0, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, puenktlichkeit_delta: -2.1, aktuell_pct: 79.0, rank_delta: -1, ampel: 'gelb',  alert_rueckfall: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, puenktlichkeit_delta: -7.3, aktuell_pct: 72.0, rank_delta: -2, ampel: 'rot',   alert_rueckfall: true  },
  ],
  team_avg_delta: 0.83,
  bester_name: 'Max M.',
...
function barColor(delta: number): string {
  if (delta > 0)   return '#a78bfa'; // violet-400
  if (delta === 0) return '#6b7280'; // gray-500
  return '#f87171'; // red-400
}

function DeltaIcon({ d }: { d: number }) {
  if (d > 0) return <TrendingUp   className="h-3 w-3 text-violet-400" />;
...
export function DispatchPhase5662SpaetabendPuenktlichkeitsTrendBoard({
  locationId,
}: {
  ...
  return (
    <div className={`rounded-lg bg-gray-900 border ${data.alert_count > 0 ? 'border-red-700/60' : 'border-violet-700/40'} p-3 space-y-2.5`}>
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          <Moon className="h-3.5 w-3.5 text-violet-400" />
          <span className="text-xs font-semibold text-white">Spätabend-Pünktlichkeits-Trend</span>
      ...
      <div className="grid grid-cols-3 gap-1.5 text-center">
        <div className="bg-gray-800 rounded p-1.5">
          ...
          <div className="text-xs font-semibold text-violet-400 truncate">{data.bester_name}</div>
          <div className="text-[10px] font-mono text-violet-300">
            {best ? fmtDelta(best.puenktlichkeit_delta) : '—'}
          </div>
        </div>
        <div className="bg-gray-800 rounded p-1.5">
          <div className="text-[10px] text-gray-400">Team-Trend</div>
          <div className={`text-xs font-mono font-semibold ${data.team_avg_delta > 0 ? 'text-violet-400' : data.team_avg_delta < 0 ? 'text-red-400' : 'text-gray-400'}`}>
        ...
        <div className="bg-gray-800 rounded p-1.5">
          ...
          <div className="text-[10px] font-mono text-gray-400">
            {worst ? fmtDelta(worst.puenktlichkeit_delta) : '—'}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-1">
        {data.fahrer.map(f => (
          <div key={f.fahrer_id} className={`flex items-center gap-2 rounded px-2 py-1 ${f.alert_rueckfall ? 'bg-red-900/20' : 'bg-gray-800/50'}`}>
            ...
            <DeltaIcon d={f.puenktlichkeit_delta} />
            <span className={`text-[10px] font-mono w-16 text-right ${f.puenktlichkeit_delta > 0 ? 'text-violet-400' : f.puenktlichkeit_delta < 0 ? 'text-red-400' : 'text-gray-400'}`}>
              {fmtDelta(f.puenktlichkeit_delta)}
            </span>
            <div className="w-16 h-1.5 bg-gray-700 rounded overflow-hidden shrink-0">
              <div
                className="h-full rounded"
                style={{
                  width: `${Math.min(100, (Math.abs(f.puenktlichkeit_delta) / maxAbs) * 100)}%`,
                  backgroundColor: barColor(f.puenktlichkeit_delta),
                }}
              />
            </div>
          </div>

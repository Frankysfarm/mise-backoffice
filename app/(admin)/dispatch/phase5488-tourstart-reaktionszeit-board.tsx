...
// Phase 5488 — Tourstart-Reaktionszeit-Board (Dispatch)
// Timer violet-400; avg_min AUFSTEIGEND Rang 1=schnellste Reaktion=bester;
...
export function DispatchPhase5488TourstartReaktionszeitBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxVal  = Math.max(...data.fahrer.map(f => f.avg_min), 1);

  return (
    <div className={`rounded-lg bg-gray-900 border ${data.alert_count > 0 ? 'border-violet-700/60' : 'border-gray-700/50'} p-3 space-y-2.5`}>
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          <Timer className="h-3.5 w-3.5 text-violet-400" />
          <span className="text-xs font-semibold text-white">Tourstart-Reaktionszeit</span>
      ...
      <div className="grid grid-cols-3 gap-1.5 text-center">
        <div className="rounded bg-gray-800 px-2 py-1.5">
          ...
          <div className="text-xs font-bold text-violet-400">{bester ? fmtMin(bester.avg_min) : '—'}</div>
        ...
        <div className="rounded bg-gray-800 px-2 py-1.5">
          ...
          <div className="text-[10px] text-gray-500">Reaktion</div>
        </div>
        <div className="rounded bg-gray-800 px-2 py-1.5">
          <div className="text-[10px] text-gray-400">Langsamste/r</div>
          <div className="text-xs font-bold text-red-400">{letzter ? fmtMin(letzter.avg_min) : '—'}</div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 h-1.5 rounded-full bg-gray-800">
              <div
                ...
                style={{ width: `${(f.avg_min / maxVal) * 100}%`, backgroundColor: barColor(f.ampel) }}
              />
            </div>
            <span className="text-[10px] font-mono text-gray-300 w-12 text-right">{fmtMin(f.avg_min)}</span>
            <DeltaIcon d={f.rank_delta} />
            {f.alert_bottom && <AlertTriangle className="h-3 w-3 text-yellow-400" />}
          </div>
        ))}
      </div>
    </div>

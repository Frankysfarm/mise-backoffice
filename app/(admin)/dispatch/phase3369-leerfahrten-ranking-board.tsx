...
export function DispatchPhase3369LeerfahrtenRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxLF   = Math.max(...list.map(f => f.leerfahrten), 1);
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-orange-50 dark:bg-orange-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <Route size={10} className="text-orange-600" /> Bester
              </div>
              <div className="font-bold text-sm text-orange-700 dark:text-orange-400">{bester?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{bester ? `${bester.leerfahrten} Leerfahrten` : '—'}</div>
            </div>
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5">Team-Ø</div>
              <div className="font-bold text-sm text-gray-700 dark:text-gray-200">{d.team_avg_leerfahrten}</div>
              <div className="text-xs text-gray-400">Leerfahrten</div>
            </div>
            <div className="rounded-lg bg-red-50 dark:bg-red-900/20 p-2">
              ...
              <div className="text-xs text-gray-500">{letzter ? `${letzter.leerfahrten} Leerfahrten` : '—'}</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {list.map(f => {
              ...
              const barW = Math.round((f.leerfahrten / maxLF) * 100);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className={`text-sm font-bold ${cls.text}`}>{f.leerfahrten} LF</span>
                      {f.rank_delta < 0
          ...
          <div className="pt-1 border-t border-gray-100 dark:border-gray-700 flex justify-between text-xs text-gray-400">
            <span>Team-Ø: {d.team_avg_leerfahrten} Leerfahrten · heute</span>
            <span>{d.gesamt} Fahrer · Ziel: 0</span>
          </div>
        </div>
      )}
    </div>

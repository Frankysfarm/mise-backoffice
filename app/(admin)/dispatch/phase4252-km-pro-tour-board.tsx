...
export function DispatchPhase4252KmProTourBoard({ locationId }: Props) {
  ...
  const maxKm = Math.max(...data.fahrer.map((f) => f.km_avg), 0.1);
    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-violet-50 rounded-lg p-2 text-center">
          <div className="text-xs text-violet-600 font-medium truncate">{data.bester_name}</div>
          <div className="text-lg font-bold text-violet-700">{bester?.km_avg}</div>
          <div className="text-[10px] text-violet-500">km (Effizient.)</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2 text-center">
          ...
          <div className="text-[10px] text-gray-400">km / Tour</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-xs text-red-600 font-medium truncate">{data.letzter_name}</div>
          <div className="text-lg font-bold text-red-700">{letzter?.km_avg}</div>
          <div className="text-[10px] text-red-500">km (Höchste)</div>
        </div>
      </div>
      ...
      {hasAlert && (
        <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5">
          ...
          <span className="text-xs text-amber-700 font-medium">Hohe km/Tour – Routen optimieren!</span>
        </div>
      )}
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-900 w-14 text-right">{f.km_avg} km</span>
              </div>
      ...
      <div className="text-[10px] text-gray-400 text-right">#1 = niedrigste km/Tour • 30-Min-Aktualisierung</div>
    </div>
  );
}

...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', auslastung_pct: 71.8, trend: 'steigend', trend_delta:  4.0, ampel: 'gruen', alert_gering: false },
    { fahrer_id: 'f2', fahrer_name: 'Max M.',   auslastung_pct: 64.2, trend: 'stabil',   trend_delta:  0.3, ampel: 'gruen', alert_gering: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  auslastung_pct: 52.7, trend: 'fallend',  trend_delta: -3.1, ampel: 'gelb',  alert_gering: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   auslastung_pct: 37.5, trend: 'fallend',  trend_delta: -4.0, ampel: 'rot',   alert_gering: true  },
  ],
  team_avg_pct: 56.6,
  alert_count: 1,
  jahr: 2026,
...
export function DispatchPhase3066JahresauslastungBoard({ locationId }: { locationId: string | null }) {
        ...
        <div className="p-4 space-y-3">
          ...
          {alerts.length > 0 && (
            <div className="flex items-center gap-2 rounded-lg bg-red-50 border border-red-200 text-red-700 text-xs font-semibold px-3 py-2">
              ...
              {alerts.map(f => f.fahrer_name).join(', ')} — Geringe Jahresauslastung!
            </div>
          )}
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  ...
                  {f.alert_gering && (
                    <div className="mt-1.5 text-xs text-red-600 font-semibold flex items-center gap-1">
                      <AlertTriangle size={10} /> Geringe Jahresauslastung!
                    </div>
                  )}
                </div>
              );

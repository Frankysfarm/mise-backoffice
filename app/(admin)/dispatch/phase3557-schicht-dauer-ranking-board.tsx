...
export function DispatchPhase3557SchichtDauerRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const minMin = Math.min(...data.fahrer.map(f => f.dauer_min), maxMin);

  return (
    <div className="rounded-xl border bg-white shadow-sm p-4 space-y-3">
      <div className="flex items-center gap-2">
        <Timer className="w-5 h-5 text-purple-500" />
        <h3 className="font-semibold text-gray-800">Schicht-Dauer-Ranking</h3>
        {data.alert_count > 0 && (
          <span className="ml-auto flex items-center gap-1 text-xs text-red-600 bg-red-50 px-2 py-0.5 rounded-full">
            <AlertTriangle className="w-3 h-3" /> {data.alert_count} Lange Schicht-Dauer!
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-xs">
        <div className="bg-green-50 rounded-lg p-2 text-center">
          ...
          <div className="font-bold text-green-700 truncate">{data.kuerzester_name}</div>
        </div>
        <div className="bg-purple-50 rounded-lg p-2 text-center">
          <div className="text-gray-500">Team-Ø</div>
          <div className="font-bold text-purple-700">{data.team_avg} min</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-gray-500">Längster</div>
          <div className="font-bold text-red-700 truncate">{data.laengster_name}</div>
        </div>
      </div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
              <div
                ...
                style={{ width: `${maxMin > minMin ? ((f.dauer_min - minMin) / (maxMin - minMin)) * 100 : 50}%` }}
              />
            </div>

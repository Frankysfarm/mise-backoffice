...
// neg=grün: weniger Zeit = besser
function Delta({ v }: { v: number }) {
  if (v < 0) return <span className="flex items-center gap-0.5 text-green-600 text-xs font-medium"><TrendingDown size={11} />{v}s</span>;
  if (v > 0) return <span className="flex items-center gap-0.5 text-red-500 text-xs font-medium"><TrendingUp size={11} />+{v}s</span>;
  return <span className="flex items-center gap-0.5 text-gray-400 text-xs"><Minus size={11} />0</span>;
}

export function DispatchPhase3241ReaktionszeitRankingBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 shadow-sm space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Reaktionszeit-Ranking</span>
        </div>
        {data.alert_count > 0 && (
      ...
      <div className="space-y-2">
        {data.fahrer.map(f => (
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <span className={`text-xs font-semibold w-14 text-right ${f.ampel === 'gruen' ? 'text-green-600' : f.ampel === 'gelb' ? 'text-yellow-500' : 'text-red-500'}`}>
              {fmtSek(f.avg_sek)}
            </span>

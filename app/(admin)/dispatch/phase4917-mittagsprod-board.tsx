...
import { TrendingUp, TrendingDown, Minus, AlertTriangle, Utensils } from 'lucide-react';
...
export function DispatchPhase4917MittagsprodBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="rounded-2xl border border-orange-700 bg-orange-950/40 overflow-hidden mb-4">
      ...
      <div className="px-4 py-3 flex items-center justify-between border-b border-orange-700/50 bg-orange-900/20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-orange-200">Mittagsschicht-Produktivität (11–14 Uhr)</span>
        </div>
        {data.alert_count > 0 && (
          <div className="flex items-center gap-1 text-xs text-orange-300">
            <AlertTriangle className="w-3.5 h-3.5" />
            {data.alert_count} &gt;4,0 T/h
          </div>
        )}
      </div>

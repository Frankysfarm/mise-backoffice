...
export function DispatchPhase5198TrinkgeldProLieferungBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxTip = Math.max(...data.fahrer.map(f => f.avg_trinkgeld), 1);
  ...
  return (
    <div className="rounded-xl border border-rose-700 bg-rose-900/30 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-gray-200">Ø Trinkgeld/Lieferung-Ranking</span>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-lg bg-rose-900/50 px-3 py-2 text-center">
          ...
          <div className="text-xs font-bold text-rose-300 truncate">{data.beste_name}</div>
        </div>
            ...
            <div className="flex-1 h-2 rounded-full bg-gray-800 overflow-hidden">
              <div
                className={`h-full rounded-full ${
                  f.ampel === 'gruen' ? 'bg-rose-500' :
                  f.ampel === 'gelb'  ? 'bg-yellow-500' : 'bg-red-500'
                }`}
                style={{ width: `${Math.round((f.avg_trinkgeld / maxTip) * 100)}%` }}
              />
            </div>
      ...
      {data.beste_name && (
        <div className="mt-3 text-[10px] text-rose-400 border-t border-rose-800/40 pt-2">
          Champion: {data.beste_name} — {data.fahrer[0]?.avg_trinkgeld.toFixed(2)}€ Ø-Trinkgeld/Lieferung
        </div>
      )}
    </div>
  );

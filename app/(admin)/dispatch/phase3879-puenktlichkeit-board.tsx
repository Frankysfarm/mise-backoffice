...
export function DispatchPhase3879PuenktlichkeitBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-gray-900">Pünktlichkeit</h3>
          {loading && <span className="w-3 h-3 border-2 border-green-400 border-t-transparent rounded-full animate-spin" />}
        </div>
        <span className="text-xs text-gray-400">Ziel ≥85%</span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-emerald-50 rounded-lg p-2 text-center">
          ...
          <div className="text-lg font-black text-emerald-700">{best?.puenktlichkeit_pct ?? '—'}%</div>
        ...
        <div className="bg-blue-50 rounded-lg p-2 text-center">
          ...
          <div className="text-lg font-black text-blue-700">{data.team_avg_pct}%</div>
          <div className="text-[10px] text-gray-600">On-Time</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-gray-500 mb-0.5">Niedrigste</div>
          <div className="text-lg font-black text-red-600">{worst?.puenktlichkeit_pct ?? '—'}%</div>
      ...
      <div className="text-[10px] text-gray-400 border-t border-gray-100 pt-1.5 text-right">
        Pünktliche Lieferungen in % (letzte 30 Tage)
      </div>
    </div>
  );
}

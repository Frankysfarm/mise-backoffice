...
// Phase 5646 — Mittagsschicht-Pünktlichkeits-Trend-Board (Dispatch) — Batch 106
...
const MOCK: ApiResponse = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   rang: 1, puenktlichkeit_delta:  8.5, aktuell_pct: 94.5, rank_delta:  2, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', rang: 2, puenktlichkeit_delta:  4.2, aktuell_pct: 91.2, rank_delta:  0, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, puenktlichkeit_delta: -2.1, aktuell_pct: 83.4, rank_delta: -1, ampel: 'gelb',  alert_rueckfall: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, puenktlichkeit_delta: -7.3, aktuell_pct: 75.2, rank_delta: -1, ampel: 'rot',   alert_rueckfall: true  },
  ],
  team_avg_delta: 0.8,
  bester_name: 'Max M.',
...
export function DispatchPhase5646MittagsschichtPuenktlichkeitsTrendBoard({
  locationId,
}: {
    ...
    <div className={`rounded-lg bg-gray-900 border ${data.alert_count > 0 ? 'border-red-700/60' : 'border-amber-700/40'} p-3 space-y-2.5`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-white">Mittagsschicht-Pünktlichkeit</span>
      ...
      <div className="space-y-1">
        {data.fahrer.map(f => (
          <div key={f.fahrer_id} className={`flex items-center gap-2 rounded px-2 py-1 ${f.alert_rueckfall ? 'bg-red-900/20' : 'bg-gray-800/50'}`}>
            ...
            <span className={`text-[10px] font-mono w-16 text-right ${f.puenktlichkeit_delta > 0 ? 'text-amber-400' : f.puenktlichkeit_delta < 0 ? 'text-red-400' : 'text-gray-400'}`}>

...
import { Euro, AlertTriangle, ChevronDown, ChevronUp, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
export function DispatchPhase3340UmsatzProTourRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const alerts = list.filter(f => f.alert_low);
  const bester = list.find(f => f.rang === 1);
  const letzter = list[list.length - 1];
  const maxUmsatz = Math.max(...list.map(f => f.umsatz_avg), 1);
        ...
        <div className="p-4 space-y-3">
          {alerts.length > 0 && (
            <div className="flex items-center gap-2 rounded-lg bg-red-50 border border-red-200 text-red-700 dark:text-red-300 dark:bg-red-950 dark:border-red-800 text-xs font-semibold px-3 py-2">
              ...
              {alerts.map(f => f.fahrer_name).join(', ')} — Niedriger Umsatz pro Tour!
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-green-50 dark:bg-green-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <Euro size={10} className="text-green-600" /> Bester
              </div>
              <div className="font-bold text-sm text-green-700 dark:text-green-400">{bester?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{bester ? `${bester.umsatz_avg.toFixed(2)} €` : '—'}</div>
            </div>
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="text-xs text-gray-400">Ø pro Tour</div>
            </div>
            <div className="rounded-lg bg-red-50 dark:bg-red-900/20 p-2">
              ...
              <div className="text-xs text-gray-500">{letzter ? `${letzter.umsatz_avg.toFixed(2)} €` : '—'}</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {list.map(f => {
              ...
              const barW = Math.round((f.umsatz_avg / maxUmsatz) * 100);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className={`text-sm font-bold ${cls.text}`}>{f.umsatz_avg.toFixed(2)} €</span>
                      {f.rank_delta > 0
          ...
          <div className="pt-1 border-t border-gray-100 dark:border-gray-700 flex justify-between text-xs text-gray-400">
            <span>Team-Ø: {d.team_avg.toFixed(2)} € · letzte 30 Tage</span>

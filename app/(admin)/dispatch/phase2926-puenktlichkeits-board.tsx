...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Julia F.', quote_pct: 95.0, trend: 'steigend', trend_delta:  2.0, ampel: 'gruen' },
    { fahrer_id: 'd2', fahrer_name: 'Max M.',   quote_pct: 88.0, trend: 'steigend', trend_delta:  1.5, ampel: 'gelb'  },
    { fahrer_id: 'd4', fahrer_name: 'Tim B.',   quote_pct: 78.0, trend: 'fallend',  trend_delta: -2.0, ampel: 'gelb'  },
    { fahrer_id: 'd3', fahrer_name: 'Sara K.',  quote_pct: 65.0, trend: 'fallend',  trend_delta: -3.0, ampel: 'rot'   },
  ],
  team_durchschnitt: 81.5,
};

export function DispatchPhase2926PuenktlichkeitsBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Pünktlichkeits-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 text-sm">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: Pünktlichkeit zu niedrig! {f.quote_pct.toFixed(0)}%
              </span>
            </div>
          ))}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø',  value: `${data.team_durchschnitt.toFixed(0)}%`, cls: teamText },

...
function ampelColor(ampel: FahrerRow['ampel']) {
  if (ampel === 'gruen') return { text: 'text-red-700', bg: 'bg-red-50', bar: 'bg-red-500', border: 'border-red-200' };
...
export function DispatchPhase3789StornoquoteRankingBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-900 text-sm">Stornoquote-Ranking</span>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-red-50 rounded-lg p-2">
          <div className="text-base font-black text-red-700">{data.fahrer[0]?.stornoquote_pct}%</div>
        ...
        <div className={`rounded-lg p-2 ${data.alert_count > 0 ? 'bg-red-50' : 'bg-gray-50'}`}>
          <div className={`text-base font-black ${data.alert_count > 0 ? 'text-red-600' : 'text-gray-800'}`}>
            {data.fahrer[data.fahrer.length - 1]?.stornoquote_pct}%
          </div>
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 bg-red-50 border border-red-200 rounded-lg text-xs text-red-700">
          ...
          <span>{data.alert_count} Fahrer mit hoher Stornoquote — bitte prüfen!</span>
        </div>
      )}
          ...
          return (
            <div key={f.fahrer_id} className={`rounded-lg border ${c.border} ${c.bg} p-2.5 space-y-1.5`}>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm font-black ${c.text}`}>{f.stornoquote_pct}%</span>
                {f.rank_delta !== 0 && (
                  f.rank_delta < 0
              ...
              {f.alert_hoch && (
                <div className="text-[10px] text-red-600 font-medium">Hohe Stornoquote!</div>
              )}
            </div>
          );
        })}

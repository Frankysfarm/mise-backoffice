...
function barColor(a: string) {
  if (a === 'gruen') return 'bg-yellow-400';
  if (a === 'gelb') return 'bg-yellow-500';
  return 'bg-red-500';
}

export function DispatchPhase5037PraemienQuoteBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="rounded-2xl border border-yellow-700 bg-yellow-950/40 overflow-hidden mb-4">
      ...
      <div className="px-4 py-3 flex items-center justify-between border-b border-yellow-700/50 bg-yellow-900/20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-yellow-200">Prämien-Erreichungsquote (letzte 30 Tage)</span>
        </div>
        {data.alert_count > 0 && (
          <div className="flex items-center gap-1 text-xs text-red-400">
            <AlertTriangle className="w-3.5 h-3.5" />
            {data.alert_count} unter 50%
          </div>
        )}
      </div>

      {/* KPI Grid */}
      <div className="grid grid-cols-3 divide-x divide-yellow-700/30 border-b border-yellow-700/30">
        <div className="px-3 py-2 text-center">
          <div className="text-xs text-gray-500 mb-0.5">Höchste Quote</div>
        ...
        <div className="px-3 py-2 text-center">
          <div className="text-xs text-gray-500 mb-0.5">Niedrigste</div>
      ...
      {/* Champion Footer */}
      <div className="px-4 py-2 border-t border-yellow-700/30 bg-yellow-900/10">
        <p className="text-xs text-gray-500">
          Rang 1 = höchste Prämien-Quote ·{' '}
          <span className="text-green-400">{bester?.fahrer_name}</span> führt mit {bester?.praemien_quote}%
        </p>
      </div>
    </div>
  );

...
const AMPEL_BG: Record<string, string> = {
  gruen: 'bg-blue-50 border-blue-200',
...
export function DispatchPhase3649PuenktlichkeitRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const puenktlichster = sorted[0];
  const unpuenktlichster = sorted[sorted.length - 1];
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        <div className="bg-blue-50 rounded-lg p-2">
          <div className="font-bold text-blue-700">{puenktlichster?.puenktlichkeit_pct ?? '–'}%</div>
          <div className="text-gray-500">Pünktlichster</div>
          <div className="text-gray-700 truncate">{puenktlichster?.fahrer_name ?? '–'}</div>
        </div>
        ...
        <div className="bg-red-50 rounded-lg p-2">
          <div className="font-bold text-red-700">{unpuenktlichster?.puenktlichkeit_pct ?? '–'}%</div>
          <div className="text-gray-500">Unpünktlichster</div>
          <div className="text-gray-700 truncate">{unpuenktlichster?.fahrer_name ?? '–'}</div>
        </div>
      </div>
        ...
        {sorted.map((f, i) => (
          <div key={f.fahrer_id} className={`flex items-center gap-2 p-2 rounded-lg border ${AMPEL_BG[f.ampel]}`}>
            ...
            <div className="w-20 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                className={`h-2 rounded-full ${f.ampel === 'gruen' ? 'bg-blue-400' : f.ampel === 'gelb' ? 'bg-orange-400' : 'bg-red-500'}`}
                style={{ width: `${Math.max(f.puenktlichkeit_pct, 4)}%` }}
              />
            </div>
            <span className={`text-xs font-bold w-10 text-right ${AMPEL_COLOR[f.ampel]}`}>{f.puenktlichkeit_pct}%</span>
            {f.rank_delta > 0 ? (

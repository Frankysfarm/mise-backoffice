...
export function DispatchPhase3964PaketeProStundeBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Pakete/Stunde</span>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-purple-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-purple-600 font-medium">Bester</div>
          <div className="text-lg font-black text-purple-700">{best?.pakete_pro_stunde?.toFixed(1)}/h</div>
          <div className="text-[10px] text-purple-500 truncate">{best?.fahrer_name ?? '–'}</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-gray-500 font-medium">Team-Ø</div>
          <div className="text-lg font-black text-gray-700">{data.team_avg?.toFixed(1)}/h</div>
          <div className="text-[10px] text-gray-400">Ziel ≥4.0/h</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-red-500 font-medium">Niedrigster</div>
          <div className="text-lg font-black text-red-600">{worst?.pakete_pro_stunde?.toFixed(1)}/h</div>
      ...
      <div className="space-y-1.5">
        {sorted.map((f) => {
          ...
          const bgColor = f.ampel === 'gruen' ? 'bg-purple-50' : f.ampel === 'gelb' ? 'bg-yellow-50' : 'bg-red-50';
          ...
          return (
            <div key={f.fahrer_id} className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg ${bgColor}`}>
              ...
              <span className={`text-sm font-bold ${tColor}`}>{f.pakete_pro_stunde?.toFixed(1)}/h</span>
              {DeltaIcon}

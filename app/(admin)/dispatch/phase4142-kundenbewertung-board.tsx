...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; avg_bewertung: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_niedrig: boolean; }
interface ApiData { fahrer: FahrerRow[]; team_avg_bewertung: number; bester_name: string; niedrigster_name: string; alert_count: number; gesamt: number; ziel_bewertung: number; }
...
export function DispatchPhase4142KundenbewertungBoard({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-amber-50 rounded-lg p-2">
          <p className="text-[10px] text-amber-600 font-medium">Beste</p>
          <p className="text-sm font-bold text-amber-700">★ {bester?.avg_bewertung?.toFixed(1)}</p>
          <p className="text-[10px] text-amber-500 truncate">{data.bester_name}</p>
        </div>
        <div className="bg-gray-50 rounded-lg p-2">
          <p className="text-[10px] text-gray-500 font-medium">Team-Ø</p>
          <p className="text-sm font-bold text-gray-700">★ {data.team_avg_bewertung?.toFixed(1)}</p>
          <p className="text-[10px] text-gray-400">Ziel ≥{data.ziel_bewertung?.toFixed(1)}</p>
        </div>
        <div className="bg-red-50 rounded-lg p-2">
          <p className="text-[10px] text-red-500 font-medium">Niedrigste</p>
          <p className="text-sm font-bold text-red-600">★ {letzter?.avg_bewertung?.toFixed(1)}</p>
      ...
      <div className="space-y-1.5">
        {data.fahrer.map((f) => {
          const barW = (f.avg_bewertung / 5.0) * 100;
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${tColor}`}>★ {f.avg_bewertung?.toFixed(1)}</span>
              </div>
      ...
      <div className="flex justify-between text-[10px] text-gray-400 pt-1 border-t border-gray-100">
        ...
        <span>#1 = höchste Kundenbewertung</span>
      </div>
    </div>
  );
}

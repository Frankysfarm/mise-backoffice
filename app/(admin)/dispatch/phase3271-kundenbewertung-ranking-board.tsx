...
import { Star, AlertTriangle, ChevronDown, ChevronUp, Minus, TrendingDown, TrendingUp } from 'lucide-react';
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max Müller',  bewertung_avg: 4.8, bewertungen_heute: 12, trend: 'steigend', trend_delta:  0.3, ampel: 'gruen', rang: 1 },
    { fahrer_id: 'f4', fahrer_name: 'Lisa Fuchs',  bewertung_avg: 4.6, bewertungen_heute: 11, trend: 'steigend', trend_delta:  0.2, ampel: 'gruen', rang: 2 },
    { fahrer_id: 'f2', fahrer_name: 'Sara Koch',   bewertung_avg: 4.2, bewertungen_heute:  9, trend: 'stabil',   trend_delta:  0.0, ampel: 'gelb',  rang: 3 },
    { fahrer_id: 'f3', fahrer_name: 'Tim Becker',  bewertung_avg: 3.8, bewertungen_heute:  7, trend: 'fallend',  trend_delta: -0.4, ampel: 'rot',   rang: 4 },
  ],
...
export function DispatchPhase3271KundenbewertungRankingBoard({ locationId }: { locationId: string | null }) {
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-yellow-50 dark:bg-yellow-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <Star size={10} className="text-yellow-500" /> Bester
              </div>
              <div className="font-bold text-sm text-yellow-700 dark:text-yellow-400">{bester?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{bester ? fmtStar(bester.bewertung_avg) : '—'}</div>
            </div>
            <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5">Team-Ø</div>
              <div className="font-bold text-base text-blue-600 dark:text-blue-400">{fmtStar(d.team_durchschnitt)}</div>
            ...
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="text-xs text-gray-500">{niedrigster ? fmtStar(niedrigster.bewertung_avg) : '—'}</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {list.map(f => {
              ...
              const barWidth = Math.max((f.bewertung_avg / 5) * 100, f.bewertung_avg > 0 ? 2 : 0);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`font-bold text-sm ${cls.text}`}>{fmtStar(f.bewertung_avg)}</span>
                      <span className="text-xs text-gray-400">({f.bewertungen_heute} Bew.)</span>
                  ...
                  {f.ampel === 'rot' && (
                    <div className="mt-1 text-xs text-red-600 dark:text-red-400 font-semibold flex items-center gap-1">
                      <AlertTriangle size={10} /> Niedrige Kundenbewertung!
                    </div>
                  )}
                </div>
              );
          ...
          <div className="flex flex-wrap gap-3 text-xs text-gray-500 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500" /> Sehr gut (≥4.5 ★)</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400" /> Gut (≥4.0 ★)</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500"   /> Niedrig (&lt;4.0 ★)</span>
          </div>
        </div>
      )}
    </div>

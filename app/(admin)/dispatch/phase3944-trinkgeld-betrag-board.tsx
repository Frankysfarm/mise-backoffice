...
export function DispatchPhase3944TrinkgeldBetragBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-emerald-50 rounded-lg p-2 text-center">
          ...
          <div className="text-lg font-black text-emerald-700">{best?.avg_tip_eur?.toFixed(2) ?? '–'}€</div>
        ...
        <div className="bg-gray-50 rounded-lg p-2 text-center">
          ...
          <div className="text-lg font-black text-gray-700">{data.team_avg_eur?.toFixed(2) ?? '–'}€</div>
          <div className="text-[10px] text-gray-400">Ziel ≥2€</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-red-500 font-medium">Niedrigster</div>
          <div className="text-lg font-black text-red-600">{worst?.avg_tip_eur?.toFixed(2) ?? '–'}€</div>
        ...
        {sorted.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg ${bgColor}`}>
              ...
              <span className={`text-sm font-bold ${tColor}`}>{f.avg_tip_eur?.toFixed(2)}€</span>
              {DeltaIcon}

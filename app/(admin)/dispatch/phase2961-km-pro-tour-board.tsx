...
interface ApiData {
  ...
  team_avg_km_pro_tour_gestern: number;
  alert_count: number;
}
...
// Trend-Pfeile invertiert: fallend = gut (grün), steigend = schlecht (rot)
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Max M.',   km_pro_tour: 6.2,  km_pro_tour_gestern: 7.0,  touren_heute: 8, trend: 'fallend',  trend_delta: -0.8, ampel: 'gruen', alert: false },
    { fahrer_id: 'd4', fahrer_name: 'Julia F.', km_pro_tour: 7.4,  km_pro_tour_gestern: 8.1,  touren_heute: 9, trend: 'fallend',  trend_delta: -0.7, ampel: 'gruen', alert: false },
    { fahrer_id: 'd3', fahrer_name: 'Tim B.',   km_pro_tour: 9.1,  km_pro_tour_gestern: 9.8,  touren_heute: 7, trend: 'fallend',  trend_delta: -0.7, ampel: 'gelb',  alert: false },
    { fahrer_id: 'd2', fahrer_name: 'Sara K.',  km_pro_tour: 13.5, km_pro_tour_gestern: 11.0, touren_heute: 6, trend: 'steigend', trend_delta:  2.5, ampel: 'rot',   alert: true  },
  ],
  team_avg_km_pro_tour: 9.1,
  team_avg_km_pro_tour_gestern: 9.0,
  alert_count: 1,
};

export function DispatchPhase2961KmProTourBoard({ locationId }: { locationId: string | null }) {
  ...
  const alerts   = enriched.filter(f => f.km_pro_tour > ALERT_KM);
  const hasAlert = alerts.length > 0;
  const best     = sorted[0]?.km_pro_tour ?? 0;
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">km/Tour-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 text-sm">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: Zu hohe km pro Tour! ({f.km_pro_tour} km)
              </span>
            </div>
          ))}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø', value: `${data.team_avg_km_pro_tour.toFixed(1)} km`, cls: teamText },
              { label: 'Bester', value: `${best.toFixed(1)} km`,                      cls: 'text-green-600' },
              { label: 'Ziel',   value: `≤${ZIEL_KM} km`,                            cls: 'text-gray-600 dark:text-gray-400' },
            ].map(k => (
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.km_pro_tour.toFixed(1)} km</span>
                    </div>
                  </div>
                  ...
                  <div className="text-xs text-gray-400 mt-1">{f.touren_heute} Touren heute</div>
                </div>
              );
            })}
          </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 inline-block" />≤8 km</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />8–12 km</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />&gt;12 km</span>
            <span className="text-gray-400">| Ziel ≤{ZIEL_KM} km</span>
          </div>
        </div>
      )}
    </div>

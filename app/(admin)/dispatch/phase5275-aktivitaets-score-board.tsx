...
export function DispatchPhase5275AktivitaetsScoreBoard({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-violet-700 bg-violet-900/30 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-gray-200">Aktivitäts-Score-Ranking (letzte 30 Tage)</span>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-lg bg-violet-900/50 px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500 truncate">Aktivste</div>
          <div className="text-xs font-bold text-violet-300 truncate">{data.bester_name}</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500">Team-Ø</div>
          <div className="text-xs font-bold text-gray-200">{data.team_avg.toFixed(0)}</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 h-2 rounded-full bg-gray-800 overflow-hidden">
              <div
                ...
                style={{ width: `${f.aktivitaets_score}%` }}
              />
            </div>
      ...
      {data.bester_name && (
        <div className="mt-3 text-[10px] text-violet-400 border-t border-violet-800/40 pt-2">
          Aktivste: {data.bester_name} — Score {data.fahrer[0]?.aktivitaets_score} von 100
        </div>
      )}
    </div>
  );

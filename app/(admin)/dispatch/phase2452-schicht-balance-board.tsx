...
import { ChevronDown, ChevronUp, AlertTriangle, TrendingUp, TrendingDown, Minus, Scale } from 'lucide-react';
...
export function DispatchPhase2452SchichtBalanceBoard({ locationId }: { locationId: string | null }) {
      ...
      {open && (
        <div className="bg-white px-4 pb-3 pt-2 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            {[
              { label: 'Ø heute', val: `${data.team_avg_balance_pct.toFixed(1)}%`, color: data.team_avg_balance_pct >= 80 ? 'text-green-600' : data.team_avg_balance_pct >= 60 ? 'text-amber-600' : 'text-red-600' },
              { label: 'Ø Vorwoche', val: `${(data.team_avg_balance_pct_vw ?? 0).toFixed(1)}%`, color: 'text-gray-600' },
          ...
          {hasAlerts && (
            <div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-xs text-red-700">
              ...
              <span>
                <strong>Schicht-Ungleichgewicht:</strong>{' '}
                {data.fahrer.filter(f => f.alert_imbalance).map(f => f.fahrer_name).join(', ')} — Regulär-Anteil unter 60%!
              </span>
            </div>
          )}
          ...
          <div className="space-y-1.5">
            {data.fahrer.map(f => (
              <div key={f.fahrer_id} className={`flex items-center gap-2 text-xs rounded-lg px-2.5 py-1.5 border ${ampelColor(f.ampel)}`}>
                ...
                <span className="text-gray-400 text-[10px]">{f.regulaer_schichten}R/{f.sonder_schichten}S</span>
                <BalanceBar pct={f.balance_score_pct} />
                <span className="w-10 text-right font-semibold">{f.balance_score_pct.toFixed(1)}%</span>

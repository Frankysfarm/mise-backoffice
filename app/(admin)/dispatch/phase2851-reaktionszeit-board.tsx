...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Max M.',   avg_min: 2.1, touren_heute: 12, ampel: 'gruen', trend: 'fallend',  trend_delta: -0.7, rang: 1 },
    { fahrer_id: 'd2', fahrer_name: 'Anna K.',  avg_min: 3.1, touren_heute: 11, ampel: 'gelb',  trend: 'stabil',   trend_delta:  0.0, rang: 2 },
    { fahrer_id: 'd3', fahrer_name: 'Sara K.',  avg_min: 4.5, touren_heute:  9, ampel: 'gelb',  trend: 'steigend', trend_delta:  0.5, rang: 3 },
    { fahrer_id: 'd4', fahrer_name: 'Tim W.',   avg_min: 8.2, touren_heute:  7, ampel: 'rot',   trend: 'steigend', trend_delta:  1.7, rang: 4 },
  ],
...
export function DispatchPhase2851ReaktionszeitBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className={`rounded-xl border p-4 mb-4 ${hasAlert ? 'border-red-300 bg-red-50' : 'border-gray-200 bg-white'}`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800">Reaktionszeit Fahrer</span>
      ...
      {open && (
        <div className="mt-3 space-y-3">
            ...
            <div className="bg-red-100 border border-red-300 rounded-lg p-2 space-y-1">
              {alerts.map(f => (
                <div key={f.fahrer_id} className="flex items-center gap-2 text-xs text-red-700">
                  ...
                  <span>— Langsame Reaktion! ({f.avg_min.toFixed(1)} Min / Ziel &lt;{ZIEL_MIN} Min)</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="flex gap-3 text-[10px] text-gray-500">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400" /> {ZIEL_MIN}–{WARN_MIN} Min</span>

...
export function DispatchPhase3759ErstabholungPuenktlichkeitBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-900 text-sm">Erstabholung-Pünktlichkeit</span>
          {loading && <span className="w-3 h-3 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin" />}
        </div>
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 bg-red-50 border border-red-200 rounded-lg text-xs text-red-700">
          ...
          <span>{data.alert_count} Fahrer mit später Erstabholung — bitte prüfen!</span>
        </div>
      )}
          ...
          return (
            <div key={f.fahrer_id} className={`rounded-lg border ${c.border} ${c.bg} p-2.5 space-y-1.5`}>
              <div className="flex items-center gap-2">
                ...
                {f.alert_spaet && <AlertTriangle className="w-3.5 h-3.5 text-red-500 shrink-0" />}
              </div>
              <div className="h-1.5 bg-white/60 rounded-full overflow-hidden">
                <div className={`h-full ${c.bar} rounded-full transition-all duration-500`} style={{ width: `${barPct}%` }} />
              </div>
              {f.alert_spaet && (
                <div className="text-[10px] text-red-600 font-medium">Späte Erstabholung!</div>
              )}
            </div>
          );
        })}

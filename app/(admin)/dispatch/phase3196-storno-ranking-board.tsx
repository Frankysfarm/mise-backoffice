...
export function DispatchPhase3196StornoRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxRate = Math.max(...list.map(f => f.storno_rate), 1);
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-yellow-50 dark:bg-yellow-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <XCircle size={10} className="text-red-400" /> Bester
              </div>
              <div className="font-bold text-sm text-yellow-700 dark:text-yellow-400">{bester?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{bester ? `${bester.storno_rate}%` : '—'}</div>
            </div>
            <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-2">
              ...
              <div className="text-xs text-gray-500">Storno-Rate</div>
            </div>
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="text-xs text-gray-500">{letzter ? `${letzter.storno_rate}%` : '—'}</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {list.map(f => {
              ...
              const barW = maxRate > 0 ? Math.round((f.storno_rate / maxRate) * 100) : 0;
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`font-bold text-sm ${cls.text}`}>{f.storno_rate}%</span>
                      <span className="text-xs text-gray-400">({f.storno_count}/{f.gesamt_touren})</span>
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
                    <div className={`h-1.5 rounded-full transition-all ${cls.bar}`} style={{ width: `${Math.max(barW, f.storno_rate > 0 ? 4 : 0)}%` }} />
                  </div>
                  {f.alert_bottom && (
                    <div className="mt-1 text-xs text-red-600 dark:text-red-400 font-semibold flex items-center gap-1">
                      <AlertTriangle size={10} /> Hohe Storno-Rate!
                    </div>
                  )}
                </div>
              );
          ...
          <div className="flex flex-wrap gap-3 text-xs text-gray-500 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500" /> Top 25% (niedrigste Rate)</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400" /> Mitte 50%</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500"   /> Bottom 25% (höchste Rate)</span>
          </div>
        </div>
      )}
    </div>

...
export function DispatchPhase5021BestellwertBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="rounded-2xl border border-emerald-700 bg-emerald-950/40 overflow-hidden mb-4">
      ...
      <div className="px-4 py-3 flex items-center justify-between border-b border-emerald-700/50 bg-emerald-900/20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-emerald-200">Ø Bestellwert je Tour (letzte 30 Tage)</span>
        </div>
        {data.alert_count > 0 && (
      ...
      {/* KPI Grid */}
      <div className="grid grid-cols-3 divide-x divide-emerald-700/30 border-b border-emerald-700/30">
        ...
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-gray-300">{data.team_avg_bestellwert.toFixed(2)} €</div>
        </div>
        <div className="px-3 py-2 text-center">
      ...
      {/* Champion Footer */}
      <div className="px-4 py-2 border-t border-emerald-700/30 bg-emerald-900/10">
        <p className="text-xs text-gray-500">
          Rang 1 = höchster Ø-Bestellwert · {' '}
          <span className="text-green-400">{bester?.fahrer_name}</span> führt mit {bester?.avg_bestellwert.toFixed(2)} €
        </p>
      </div>
    </div>
  );

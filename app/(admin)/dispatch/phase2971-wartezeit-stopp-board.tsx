...
// Inverted: fallend (less wait) = green = gut; steigend (more wait) = red = schlecht
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   avg_wartezeit_min: 2.1, stopps: 10, trend: 'fallend',  trend_delta: -0.3, ampel: 'gruen', alert: null },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  avg_wartezeit_min: 4.3, stopps:  8, trend: 'steigend', trend_delta:  0.3, ampel: 'gelb',  alert: null },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   avg_wartezeit_min: 5.8, stopps:  6, trend: 'steigend', trend_delta:  0.3, ampel: 'gelb',  alert: null },
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', avg_wartezeit_min: 7.5, stopps:  5, trend: 'steigend', trend_delta:  1.3, ampel: 'rot',   alert: 'Zu lange Wartezeit!' },
  ],
  team_avg_wartezeit_min: 4.9,
  alert_count: 1,
};

export function DispatchPhase2971WartezeitStoppBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Wartezeit-Stopp-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 text-sm">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: Zu lange Wartezeit! ({f.avg_wartezeit_min.toFixed(1)} Min)
              </span>
            </div>
          ))}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø', value: `${data.team_avg_wartezeit_min.toFixed(1)} Min`, cls: teamText },
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.avg_wartezeit_min.toFixed(1)} Min</span>
                    </div>
                  </div>
                  ...
                  <div className="text-xs text-gray-400 mt-1">{f.stopps} Stopps heute</div>
                </div>
              );
            })}
          </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />3–6 Min</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />&gt;6 Min</span>

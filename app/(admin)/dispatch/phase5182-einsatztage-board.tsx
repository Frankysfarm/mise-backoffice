...
export function DispatchPhase5182EinsatztageBoard({ locationId }: { locationId: string | null }) {
  const [data, setData] = useState<ApiResponse | null>(null);

  async function load() {
    const url = locationId
      ? `/api/delivery/admin/fahrer-einsatztage-ranking?location_id=${locationId}`
      : '/api/delivery/admin/fahrer-einsatztage-ranking';
  ...
  const maxTage = Math.max(...data.fahrer.map(f => f.einsatztage), 1);

  return (
    <div className="rounded-xl border border-blue-700 bg-blue-900/30 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        <Calendar className="w-4 h-4 text-blue-400 shrink-0" />
        <span className="text-sm font-semibold text-gray-200">Einsatztage-Ranking (30 Tage)</span>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        ...
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          ...
          <div className="text-xs font-bold text-gray-200">{data.team_avg_tage.toFixed(1)} T</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 h-2 rounded-full bg-gray-800 overflow-hidden">
              <div
                ...
                style={{ width: `${Math.round((f.einsatztage / maxTage) * 100)}%` }}
              />
            </div>
      ...
      {data.fleissigster_name && (
        <div className="mt-3 text-[10px] text-blue-400 border-t border-blue-800/40 pt-2">
          Champion: {data.fleissigster_name} — {data.fahrer[0]?.einsatztage} Einsatztage
        </div>
      )}
    </div>
  );

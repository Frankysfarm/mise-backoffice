...
import { AlarmClock, TrendingUp, TrendingDown, Minus, AlertTriangle } from 'lucide-react';

interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; wartezeit_min: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_top: boolean; }
...
const MOCK: ApiData = {
  ...
  team_avg: 4.725, bester_name: 'Julia F.', letzter_name: 'Tim B.', alert_count: 1, gesamt: 4,
};

interface Props { locationId: string | null; }

export function DispatchPhase4097WartezeitStoppBoard({ locationId }: Props) {
  ...
  const maxMin = Math.max(...data.fahrer.map(f => f.wartezeit_min), 1);
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Wartezeit am Stopp</span>
        </div>
        <div className="flex items-center gap-2">
          {loading && <span className="w-3 h-3 border-2 border-gray-300 border-t-transparent rounded-full animate-spin" />}
          {!loading && data.alert_count > 0 && (
            <span className="flex items-center gap-1 text-xs text-red-600 font-semibold">
              <AlertTriangle className="w-3.5 h-3.5" /> {data.alert_count} Hohe Wartezeit!
            </span>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-amber-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-gray-500">Kürzeste</div>
          <div className="text-sm font-bold text-amber-600">{bester?.wartezeit_min?.toFixed(1)} min</div>
        ...
        <div className="bg-red-50 rounded-lg p-2 text-center">
          ...
          <div className="text-sm font-bold text-red-500">{letzter?.wartezeit_min?.toFixed(1)} min</div>
      ...
      <div className="space-y-1.5">
        {data.fahrer.map((f) => {
          const barW = `${(f.wartezeit_min / maxMin) * 100}%`;
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.wartezeit_min?.toFixed(1)} min</span>
                {f.alert_top && <AlertTriangle className="w-3 h-3 text-red-500 flex-shrink-0" />}
              </div>
      ...
      <div className="flex justify-between text-[10px] text-gray-400 px-1 pt-1 border-t border-gray-100">
        <span>Rang 1 = kürzeste Wartezeit am Stopp</span>
        <span>Team-Avg {data.team_avg?.toFixed(1)} min</span>
      </div>
    </div>
  );
}

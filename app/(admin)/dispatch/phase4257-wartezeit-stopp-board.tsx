...
export function DispatchPhase4257WartezeitStoppBoard({ locationId }: Props) {
  ...
  const maxMin = Math.max(...data.fahrer.map((f) => f.wartezeit_min), 0.1);
    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-orange-50 rounded-lg p-2 text-center">
          <div className="text-xs text-orange-600 font-medium truncate">{data.bester_name}</div>
          <div className="text-lg font-bold text-orange-700">{bester?.wartezeit_min}</div>
          <div className="text-[10px] text-orange-500">min (Schnellste)</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2 text-center">
          ...
          <div className="text-[10px] text-gray-400">min / Stopp</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-xs text-red-600 font-medium truncate">{data.letzter_name}</div>
          <div className="text-lg font-bold text-red-700">{letzter?.wartezeit_min}</div>
          <div className="text-[10px] text-red-500">min (Langsamste)</div>
        </div>
      </div>
      ...
      {hasAlert && (
        <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5">
          ...
          <span className="text-xs text-amber-700 font-medium">Lange Wartezeit am Stopp!</span>
        </div>
      )}

      <div className="space-y-2">
        {data.fahrer.map((f) => {
          const barW = maxMin > 0 ? (f.wartezeit_min / maxMin) * 100 : 0;
          const barColor = f.ampel === 'gruen' ? 'bg-orange-400' : f.ampel === 'gelb' ? 'bg-yellow-400' : 'bg-red-400';
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-900 w-16 text-right">{f.wartezeit_min} min</span>
              </div>
      ...
      <div className="text-[10px] text-gray-400 text-right">#1 = niedrigste Wartezeit • 30-Min-Aktualisierung</div>
    </div>
  );
}

...
export function DispatchPhase5107ZuverlaessigkeitBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="rounded-2xl border border-violet-700 bg-violet-950/40 overflow-hidden mb-4">
      <div className="px-4 py-3 flex items-center justify-between border-b border-violet-700/50 bg-violet-900/20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-violet-200">Zuverlässigkeits-Score je Fahrer (letzte 30 Tage)</span>
        </div>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 divide-x divide-violet-800/40 border-b border-violet-700/30">
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-violet-300">{bester?.score ?? '–'}</div>
        ...
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-violet-200">{data.team_avg_score}</div>
          <div className="text-[10px] text-gray-500">Zuverlässigkeits-Score</div>
        </div>
        <div className="px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500 mb-0.5">Niedrigster</div>
          <div className="text-sm font-bold text-gray-400">{letzter?.score ?? '–'}</div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="w-20 h-1.5 rounded-full bg-gray-800 overflow-hidden shrink-0">
              <div
                ...
                style={{ width: `${maxVal > 0 ? Math.round((f.score / maxVal) * 100) : 0}%` }}
              />
            </div>
      ...
      <div className="px-4 py-2 border-t border-violet-800/30 bg-violet-900/10">
        <div className="text-[10px] text-gray-500">
          Zuverlässigste: <span className="text-violet-300 font-semibold">{data.beste_name}</span> ·

...
export function DispatchPhase4377WochenendBoard({ locationId }: Props) {
    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-gray-900">Wochenend-Anteil-Ranking</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          {!loading && data.alert_count > 0 && (
            <span className="flex items-center gap-0.5 text-xs text-red-600 font-semibold bg-red-50 px-2 py-0.5 rounded-full">
              <AlertTriangle className="w-3 h-3" /> {data.alert_count} Hoher Wochenend-Anteil!
            </span>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        <div className="bg-orange-50 rounded-xl p-2">
          <div className="font-bold text-orange-700">{bester?.wochenend_anteil_pct.toFixed(1)}%</div>
          <div className="text-gray-500 truncate">{data.meister_name}</div>
          <div className="text-[10px] text-gray-400">Meiste WE</div>
        </div>
        ...
        <div className="bg-green-50 rounded-xl p-2">
          <div className="font-bold text-green-700">{letzter?.wochenend_anteil_pct.toFixed(1)}%</div>
          <div className="text-gray-500 truncate">{data.wenigster_name}</div>
          <div className="text-[10px] text-gray-400">Wenigste WE</div>
        </div>
      </div>
      ...
      <div className="space-y-1.5">
        {data.fahrer.map((f) => {
          ...
          const barColor = f.ampel === 'rot' ? 'bg-red-400' : f.ampel === 'gelb' ? 'bg-orange-300' : 'bg-orange-500';
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-900 w-14 text-right">{f.wochenend_anteil_pct.toFixed(1)}%</span>
              </div>
              <div className="ml-7 h-1 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${barColor}`} style={{ width: `${(f.wochenend_anteil_pct / maxPct) * 100}%` }} />
              </div>
            </div>
          );
        })}

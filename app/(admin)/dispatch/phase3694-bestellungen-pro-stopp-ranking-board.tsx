...
export function DispatchPhase3694BestellungenProStoppRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxVal = Math.max(...sorted.map(f => f.bestellungen_pro_stopp), 1);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900">Bestellungen/Stopp — Ranking</h3>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        <div className="bg-emerald-50 rounded-lg p-2">
          <div className="font-bold text-emerald-700">{bester?.bestellungen_pro_stopp.toFixed(1)}</div>
        ...
        <div className="bg-red-50 rounded-lg p-2">
          <div className="font-bold text-red-700">{wenigste?.bestellungen_pro_stopp.toFixed(1)}</div>
          ...
          <div key={f.fahrer_id} className={`flex items-center gap-2 p-2 rounded-lg border ${AMPEL_BG[f.ampel]}`}>
            ...
            <div className="w-20 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${Math.max((f.bestellungen_pro_stopp / maxVal) * 100, 4)}%` }}
              />
            </div>
            <span className={`text-xs font-bold w-10 text-right ${AMPEL_COLOR[f.ampel]}`}>{f.bestellungen_pro_stopp.toFixed(1)}</span>
            {f.rank_delta > 0 ? (

...
export function DispatchPhase5194KundenzufriedenheitBoard({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-amber-700 bg-amber-900/30 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-gray-200">Kundenzufriedenheit-Ranking</span>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        ...
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          ...
          <div className="text-xs font-bold text-gray-200">{data.team_avg_rating.toFixed(1)} ★</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
            ...
            <div className="flex-1 h-2 rounded-full bg-gray-800 overflow-hidden">
              <div
                className={`h-full rounded-full ${
                  f.ampel === 'gruen' ? 'bg-amber-400' :
                  f.ampel === 'gelb'  ? 'bg-yellow-500' : 'bg-red-500'
                }`}
                style={{ width: `${Math.round((f.avg_rating / 5) * 100)}%` }}
              />
            </div>
      ...
      {data.bester_name && (
        <div className="mt-3 text-[10px] text-amber-400 border-t border-amber-800/40 pt-2">
          Champion: {data.bester_name} — {data.fahrer[0]?.avg_rating.toFixed(1)} ★ Kundenbewertung
        </div>
      )}
    </div>
  );

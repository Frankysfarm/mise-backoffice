...
export function DispatchPhase3522WartezeitStoppRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-wartezeit-stopp-ranking?location_id=${locationId}`, { cache: 'no-store' });
        ...
        <div className="px-3 pb-3 space-y-2">
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1.5 p-2 rounded bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 text-xs border border-red-200">
              ...
              Hohe Wartezeit! {data.alert_count} Fahrer über Zielwert
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2 py-1">
            ...
            <div className="text-center">
              ...
              <div className="text-xs font-semibold text-red-600 truncate">{data.letzter_name}</div>
            </div>
          </div>
              ...
              <div key={f.fahrer_id} className="flex items-center gap-2">
                ...
                <div className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-full h-2 relative">
                  <div
                    ...
                    style={{ width: `${(f.wartezeit_min / maxMin) * 100}%` }}
                  />
                </div>

...
import { ChevronDown, ChevronUp, AlertTriangle, TrendingUp, TrendingDown, Minus, CalendarRange } from 'lucide-react';
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', auslastung_pct: 85.7, auslastung_pct_vormonat: 83.2, wochen_pct: [88, 84, 87, 84], trend: 'steigend', trend_delta: 2.5, ampel: 'gruen', alert_gering: false },
    { fahrer_id: 'f2', fahrer_name: 'Max M.',   auslastung_pct: 74.5, auslastung_pct_vormonat: 76.1, wochen_pct: [78, 72, 76, 72], trend: 'fallend',  trend_delta: -1.6, ampel: 'gruen', alert_gering: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  auslastung_pct: 62.1, auslastung_pct_vormonat: 60.5, wochen_pct: [65, 60, 63, 60], trend: 'steigend', trend_delta:  1.6, ampel: 'gelb',  alert_gering: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   auslastung_pct: 41.3, auslastung_pct_vormonat: 44.2, wochen_pct: [45, 40, 42, 38], trend: 'fallend',  trend_delta: -2.9, ampel: 'rot',   alert_gering: true  },
  ],
  team_avg_pct: 65.9,
  team_avg_pct_vormonat: 66.0,
  alert_count: 1,
};

export function DispatchPhase3056MonatsauslastungBoard({ locationId }: { locationId: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-monatsauslastung?location_id=${locationId}`)
        .then(r => r.json())
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Monatsauslastung Fahrer</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 text-sm">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: Geringe Monatsauslastung! ({f.auslastung_pct.toFixed(1)} %)
              </span>
            </div>
          ))}
            ...
            {sorted.map(f => {
              ...
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <button className="w-full text-left" onClick={() => setExpanded(isExpanded ? null : f.fahrer_id)}>
                  ...
                  {isExpanded && (
                    <div className="mt-2 grid grid-cols-4 gap-1">
                      {KW.map((kw, i) => {
                        const v = f.wochen_pct[i] ?? 0;
                        const color = v >= ZIEL_PCT ? 'bg-green-500' : v >= ALERT_PCT ? 'bg-amber-400' : 'bg-red-500';
                        return (
                          <div key={kw} className="text-center">
                            <div className="text-[9px] text-gray-500">{kw}</div>
                            <div className={`mx-auto mt-0.5 w-5 rounded-sm ${color}`} style={{ height: `${Math.max(2, v / 100 * 24)}px` }} />
                            <div className="text-[9px] font-semibold text-gray-600 dark:text-gray-400 mt-0.5">{v.toFixed(0)} %</div>
                          </div>
                        );
                      })}
                    </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            ...
            <span className="text-gray-400">| Ziel ≥{ZIEL_PCT} % · 30-Min-Polling</span>
          </div>
        </div>
      )}
    </div>

...
// Phase 5650 — Feierabend-Effizienz-Trend-Board (Dispatch) — Batch 107
...
const MOCK: ApiResponse = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   rang: 1, effizienz_delta:  0.6, aktuell_touren_pro_std: 3.4, rank_delta:  1, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', rang: 2, effizienz_delta:  0.3, aktuell_touren_pro_std: 3.1, rank_delta:  0, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, effizienz_delta: -0.1, aktuell_touren_pro_std: 2.6, rank_delta: -1, ampel: 'gelb',  alert_rueckfall: false },
...
function barColor(delta: number): string {
  if (delta > 0)   return '#f97316'; // orange-500
  if (delta === 0) return '#6b7280'; // gray-500
  return '#f87171'; // red-400
}

function DeltaIcon({ d }: { d: number }) {
  if (d > 0) return <TrendingUp   className="h-3 w-3 text-orange-500" />;
...
export function DispatchPhase5650FeierabendEffizienzTrendBoard({
  locationId,
}: {
    ...
    <div className={`rounded-lg bg-gray-900 border ${data.alert_count > 0 ? 'border-red-700/60' : 'border-orange-700/40'} p-3 space-y-2.5`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-white">Feierabend-Effizienz-Trend</span>
      ...
      <div className="grid grid-cols-3 gap-1.5 text-center">
        <div className="bg-gray-800 rounded p-1.5">
          ...
          <div className="text-xs font-semibold text-orange-500 truncate">{data.bester_name}</div>
        ...
        <div className="bg-gray-800 rounded p-1.5">
          ...
          <div className={`text-xs font-mono font-semibold ${data.team_avg_delta > 0 ? 'text-orange-500' : data.team_avg_delta < 0 ? 'text-red-400' : 'text-gray-400'}`}>
      ...
      <div className="space-y-1">
        {data.fahrer.map(f => (
          <div key={f.fahrer_id} className={`flex items-center gap-2 rounded px-2 py-1 ${f.alert_rueckfall ? 'bg-red-900/20' : 'bg-gray-800/50'}`}>
            ...
            <span className={`text-[10px] font-mono w-16 text-right ${f.effizienz_delta > 0 ? 'text-orange-500' : f.effizienz_delta < 0 ? 'text-red-400' : 'text-gray-400'}`}>

...
function FeiertagBar({ h, max = 12 }: { h: number; max?: number }) {
  const pct = Math.min(100, (h / max) * 100);
  const color = h === 0 ? 'bg-green-500' : h <= 8 ? 'bg-amber-400' : 'bg-red-500';
  return (
    <div className="relative h-1.5 rounded-full bg-gray-200 w-24">
      ...
      <div className="absolute top-0 h-full border-l border-dashed border-red-400" style={{ left: `${(8 / max) * 100}%` }} title=">8h Alert" />
    </div>
  );
}
...
export function DispatchPhase2447FeiertagsschichtBoard({ locationId }: { locationId: string | null }) {
      ...
      {open && (
        <div className="bg-white px-4 pb-3 pt-2 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            {[
              { label: 'Ø heute', val: fmtH(data.team_avg_feiertag_h), color: data.team_avg_feiertag_h > 8 ? 'text-red-600' : data.team_avg_feiertag_h > 0 ? 'text-amber-600' : 'text-teal-600' },
              { label: 'Ø Vorwoche', val: fmtH(data.team_avg_feiertag_h_vw ?? 0), color: 'text-gray-600' },
          ...
          {hasAlerts && (
            <div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-xs text-red-700">
              ...
              <span>
                ...
                {data.fahrer.filter(f => f.alert_ueberlastung).map(f => f.fahrer_name).join(', ')} — mehr als 8h Feiertagsschicht!
              </span>
            </div>
          )}
          ...
          <div className="space-y-1.5">
            {data.fahrer.map(f => (
              <div key={f.fahrer_id} className={`flex items-center gap-2 text-xs rounded-lg px-2.5 py-1.5 border ${ampelColor(f.ampel)}`}>
                ...
                <FeiertagBar h={f.feiertag_h} />
                <span className="w-9 text-right font-semibold">{fmtH(f.feiertag_h)}</span>
          ...
          <div className="flex gap-3 text-xs text-gray-400">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500" />0h</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400" />1–8h</span>

...
export function DispatchPhase3542SchichtStartzeitRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxMin = Math.max(...data.fahrer.map(f => f.startzeit_min), 1);
  const minMin = Math.min(...data.fahrer.map(f => f.startzeit_min), maxMin);

  return (
    <div className="rounded-xl border bg-white shadow-sm p-4 space-y-3">
      <div className="flex items-center gap-2">
        <Clock className="w-5 h-5 text-green-500" />
        <h3 className="font-semibold text-gray-800">Schicht-Startzeit-Ranking</h3>
        {data.alert_count > 0 && (
          <span className="ml-auto flex items-center gap-1 text-xs text-red-600 bg-red-50 px-2 py-0.5 rounded-full">
            <AlertTriangle className="w-3 h-3" /> {data.alert_count} Späte Schicht-Startzeit!
          </span>
        )}
      </div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
              <div
                ...
                style={{ width: `${maxMin > minMin ? ((f.startzeit_min - minMin) / (maxMin - minMin)) * 100 : 50}%` }}
              />
            </div>

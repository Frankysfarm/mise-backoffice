...
import { ChevronDown, ChevronUp, TimerReset, TrendingUp, TrendingDown, Minus, AlertTriangle } from 'lucide-react';
...
export function DispatchPhase3402ErsteStoppZeitRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-erste-stopp-zeit?location_id=${locationId}`, { cache: 'no-store' });
  ...
  const maxMin = Math.max(...data.fahrer.map(f => f.erste_stopp_min), 1);
        ...
        <div className="px-3 pb-3 space-y-2">
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1.5 p-2 rounded bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 text-xs border border-red-200">
              ...
              Langer Anlauf! {data.alert_count} Fahrer über Zielwert
            </div>
          )}

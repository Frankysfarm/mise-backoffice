...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   bewertung_avg: 4.8, bewertungen_heute: 12, trend: 'steigend', trend_delta: 0.3,  ampel: 'gruen', rang: 1 },
    { fahrer_id: 'f2', fahrer_name: 'Lisa F.',  bewertung_avg: 4.6, bewertungen_heute: 11, trend: 'steigend', trend_delta: 0.2,  ampel: 'gruen', rang: 2 },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  bewertung_avg: 4.2, bewertungen_heute:  9, trend: 'stabil',  trend_delta: 0.0,  ampel: 'gelb',  rang: 3 },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   bewertung_avg: 3.8, bewertungen_heute:  7, trend: 'fallend', trend_delta: -0.4, ampel: 'rot',   rang: 4 },
  ],
  team_durchschnitt: 4.35,
  alert_count: 1,
};

export function DispatchPhase3011BewertungsBoard({ locationId }: { locationId: string | null }) {
  ...
  const teamAmpel = data.team_durchschnitt >= ZIEL_AVG ? 'gruen' : data.team_durchschnitt >= ALERT_AVG ? 'gelb' : 'rot';
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Kundenbewertungs-Board</span>
        ...
        <div className="px-4 py-3 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              { label: 'Ziel',   value: `≥${ZIEL_AVG} ★`,                         cls: 'text-gray-600 dark:text-gray-400' },
            ].map(k => (
          ...
          <div className="space-y-2">
            {sorted.map(f => {
              ...
              const pct = Math.min(100, (f.bewertung_avg / MAX_BAR) * 100);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.bewertung_avg.toFixed(1)} ★</span>
                    </div>
                  </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            ...
            <span className="text-gray-400">| Ziel ≥{ZIEL_AVG} ★</span>
          </div>
        </div>
      )}
    </div>

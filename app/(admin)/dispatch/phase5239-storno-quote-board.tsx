...
export function DispatchPhase5239StornoQuoteBoard({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-rose-700 bg-rose-900/30 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-gray-200">Storno-Quote-Ranking (letzte 30 Tage)</span>
        {data.alert_count > 0 && (
      ...
      {data.bester_name && (
        <div className="mt-3 text-[10px] text-green-400 border-t border-rose-800/40 pt-2">
          Niedrigste Quote: {data.bester_name} — {data.fahrer[0]?.stornoquote_pct.toFixed(0)}% Stornierungen
        </div>
      )}
    </div>
  );

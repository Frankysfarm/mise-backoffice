...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f2', fahrer_name: 'Sarah K.',  avg_trinkgeld_heute: 1.40, avg_trinkgeld_gestern: 4.10, lieferungen_heute: 8,  trend: 'schlechter', trend_delta: -2.70, ampel: 'rot',   alert: true  },
    { fahrer_id: 'f5', fahrer_name: 'Jana F.',   avg_trinkgeld_heute: 0.80, avg_trinkgeld_gestern: 4.50, lieferungen_heute: 6,  trend: 'schlechter', trend_delta: -3.70, ampel: 'rot',   alert: true  },
    { fahrer_id: 'f3', fahrer_name: 'Lena S.',   avg_trinkgeld_heute: 3.50, avg_trinkgeld_gestern: 3.80, lieferungen_heute: 10, trend: 'stabil',     trend_delta: -0.30, ampel: 'gelb',  alert: false },
    { fahrer_id: 'f4', fahrer_name: 'Tom B.',    avg_trinkgeld_heute: 5.90, avg_trinkgeld_gestern: 6.20, lieferungen_heute: 11, trend: 'stabil',     trend_delta: -0.30, ampel: 'gruen', alert: false },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',    avg_trinkgeld_heute: 6.80, avg_trinkgeld_gestern: 5.20, lieferungen_heute: 12, trend: 'besser',     trend_delta: 1.60,  ampel: 'gruen', alert: false },
  ],
  team_avg_heute: 3.68, team_avg_gestern: 4.76, ziel: 5, alert_count: 2,
};

export function DispatchPhase2610TrinkgeldAnalyseBoard({ locationId }: { locationId: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-trinkgeld-analyse?location_id=${locationId}`)
  ...
  const sorted     = [...data.fahrer].sort((a, b) => a.avg_trinkgeld_heute - b.avg_trinkgeld_heute);
  const hasAlert   = data.alert_count > 0;
  const teamAmpel  = data.team_avg_heute >= data.ziel ? 'gruen' : data.team_avg_heute >= 2 ? 'gelb' : 'rot';
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-4`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800">Trinkgeld-Analyse-Board</span>
          {hasAlert && (
        ...
        <div className="flex items-center gap-3">
          <span className={`text-sm font-bold ${teamCls.text}`}>Ø {data.team_avg_heute.toFixed(2)} €</span>
        ...
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className={`text-lg font-bold ${teamCls.text}`}>{data.team_avg_heute.toFixed(2)} €</div>
              <div className="text-xs text-gray-500">Team Ø heute</div>
            </div>
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className="text-lg font-bold text-gray-700">
                {data.team_avg_gestern !== null ? `${data.team_avg_gestern.toFixed(2)} €` : '—'}
              </div>
            ...
            {sorted.map(f => {
              ...
              return (
                <div key={f.fahrer_id} className={`flex items-center gap-2 rounded-lg border px-2.5 py-1.5 ${cls.bg}`}>
                  ...
                  <TrinkgeldBalken avg={f.avg_trinkgeld_heute} ziel={data.ziel} barClass={cls.bar} />
                  <span className={`text-xs font-bold w-14 text-right ${cls.text}`}>{f.avg_trinkgeld_heute.toFixed(2)} €</span>
                  <TrendIcon trend={f.trend} />
                  <span className="text-xs text-gray-400 w-12 text-right">
                    {f.trend_delta > 0 ? '+' : ''}{f.trend_delta.toFixed(2)} €
                  </span>
                </div>
              );
            })}
          ...
          <div className="flex gap-3 text-xs text-gray-400">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />2–{data.ziel - 0.01} €</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />&lt;2 €</span>
          </div>
        </div>
      )}
    </div>

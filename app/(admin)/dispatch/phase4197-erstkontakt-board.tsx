...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; avg_sek: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_bottom: boolean; }
interface ApiData { fahrer: FahrerRow[]; team_avg_sek: number; bester_name: string; letzter_name: string; alert_count: number; gesamt: number; }
...
export function DispatchPhase4197ErstkontaktBoard({ locationId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/delivery/admin/fahrer-erstkontakt-ranking?location_id=${locationId}`);
  ...
  const maxSek = Math.max(...data.fahrer.map((f) => f.avg_sek), 1);
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Erstkontakt-Zeit</span>
        ...
        {data.alert_count > 0 && (
          <span className="flex items-center gap-1 text-xs text-red-600 font-semibold bg-red-50 px-2 py-0.5 rounded-full">
            <AlertTriangle className="w-3.5 h-3.5" /> {data.alert_count} Langer Erstkontakt!
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-indigo-50 rounded-lg p-2">
          <p className="text-[10px] text-indigo-600 font-medium">Schnellster</p>
          <p className="text-sm font-bold text-indigo-700">{bester?.avg_sek} s</p>
          <p className="text-[10px] text-indigo-500 truncate">{data.bester_name}</p>
        </div>
        <div className="bg-gray-50 rounded-lg p-2">
          <p className="text-[10px] text-gray-500 font-medium">Team-Ø</p>
          <p className="text-sm font-bold text-gray-700">{data.team_avg_sek} s</p>
          <p className="text-[10px] text-gray-400">Erstkontakt</p>
        </div>
        <div className="bg-red-50 rounded-lg p-2">
          <p className="text-[10px] text-red-500 font-medium">Langsamster</p>
          <p className="text-sm font-bold text-red-600">{letzter?.avg_sek} s</p>
      ...
      <div className="space-y-1.5">
        {data.fahrer.map((f) => {
          const barW = maxSek > 0 ? (f.avg_sek / maxSek) * 100 : 0;
          const barColor = f.ampel === 'gruen' ? 'bg-indigo-400' : f.ampel === 'gelb' ? 'bg-yellow-400' : 'bg-red-400';
          const tColor = f.ampel === 'gruen' ? 'text-indigo-600' : f.ampel === 'gelb' ? 'text-yellow-500' : 'text-red-500';
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.avg_sek} s</span>
              </div>
      ...
      <div className="flex justify-between text-[10px] text-gray-400 pt-1 border-t border-gray-100">
        ...
        <span>#1 = schnellster Kontakt</span>
      </div>
    </div>
  );
}

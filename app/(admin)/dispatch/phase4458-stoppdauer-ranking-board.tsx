...
export function DispatchPhase4458StoppdauerRankingBoard({ locationId }: Props) {
  const [data, setData] = useState<StoppdauerData | null>(null);
  ...
  const alerts = data.fahrer.filter(d => d.alert_bottom);
  const maxSec = worst?.avg_sec ?? 1;

  const ampelColor = (a: FahrerStoppdauer['ampel']) =>
    a === 'gruen' ? 'text-emerald-600 dark:text-emerald-400' :
    a === 'rot' ? 'text-red-500 dark:text-red-400' : 'text-yellow-500 dark:text-yellow-400';

  const ampelBg = (a: FahrerStoppdauer['ampel']) =>
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900 dark:text-gray-100 text-sm">Stoppdauer-Ranking (30 Tage)</h3>
        <span className="ml-auto text-xs text-gray-400">∅ Stoppdauer</span>
      </div>

      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="rounded-lg bg-emerald-50 dark:bg-emerald-900/20 p-2">
          <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400">{secToMin(best?.avg_sec ?? 0)} min</div>
          <div className="text-xs text-gray-500 dark:text-gray-400">Schnellste ({best?.fahrer_name})</div>
        </div>
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          <div className="text-lg font-bold text-gray-700 dark:text-gray-300">{secToMin(data.team_avg_sec)} min</div>
          <div className="text-xs text-gray-500 dark:text-gray-400">Team-Avg</div>
        </div>
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          <div className="text-lg font-bold text-gray-500 dark:text-gray-400">{secToMin(worst?.avg_sec ?? 0)} min</div>
          <div className="text-xs text-gray-500 dark:text-gray-400">Langsamste ({worst?.fahrer_name})</div>
        </div>
      </div>

      {alerts.length > 0 && (
        <div className="flex items-center gap-2 rounded-lg bg-violet-50 dark:bg-violet-900/20 border border-violet-200 dark:border-violet-800 px-3 py-2">
          <AlertTriangle className="w-4 h-4 text-violet-500 flex-shrink-0" />
          <span className="text-xs text-violet-700 dark:text-violet-300">
            Hohe Stoppdauer: {alerts.map(a => a.fahrer_name).join(', ')}
          </span>
        </div>
      )}
      ...
      <div className="space-y-2">
        {data.fahrer.map((f) => {
          const balkenPct = Math.round((f.avg_sec / maxSec) * 100);
          return (
            <div key={f.fahrer_id} className="space-y-1">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className={`font-semibold text-xs ${ampelColor(f.ampel)}`}>{secToMin(f.avg_sec)} min</span>
              </div>
              <div className="h-1.5 rounded-full bg-gray-100 dark:bg-gray-800">
                <div className={`h-full rounded-full ${ampelBg(f.ampel)}`} style={{ width: `${balkenPct}%` }} />
              </div>
            </div>
          );
        })}
      ...
      <div className="text-xs text-gray-400">Ziel: ≤5 min/Stopp · {data.gesamt} Fahrer</div>
    </div>
  );
}

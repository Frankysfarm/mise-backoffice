...
export function DispatchPhase5178SchichtDauerBoard({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-indigo-700 bg-indigo-900/30 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-gray-200">Ø Schichtdauer-Ranking</span>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-lg bg-indigo-900/50 px-3 py-2 text-center">
          ...
          <div className="text-xs font-bold text-indigo-300 truncate">{data.fleissigster_name}</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500">Team-Ø</div>
          <div className="text-xs font-bold text-gray-200">{data.team_avg_stunden.toFixed(1)}h</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 h-2 rounded-full bg-gray-800 overflow-hidden">
              <div
                ...
                style={{ width: `${Math.round((f.avg_stunden / maxStunden) * 100)}%` }}
              />
            </div>
            <span className="text-xs text-gray-400 w-10 text-right tabular-nums">
              {f.avg_stunden.toFixed(1)}h
            </span>
      ...
      {data.fleissigster_name && (
        <div className="mt-3 text-[10px] text-indigo-400 border-t border-indigo-800/40 pt-2">
          Champion: {data.fleissigster_name} — {data.fahrer[0]?.avg_stunden.toFixed(1)}h Ø-Schichtdauer
        </div>
      )}
    </div>
  );

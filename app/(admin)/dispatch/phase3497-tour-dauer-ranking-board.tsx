...
import { Timer, ChevronDown, ChevronUp, AlertTriangle, TrendingDown } from 'lucide-react';

interface FahrerTourDauer {
  fahrer_id: string;
  fahrer_name: string;
  rang: number;
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'u1', fahrer_name: 'Julia F.', rang: 1, avg_dauer_min: 42, rank_delta:  0, ampel: 'gruen', alert_top: false },
    { fahrer_id: 'u2', fahrer_name: 'Sara K.',  rang: 2, avg_dauer_min: 55, rank_delta:  1, ampel: 'gelb',  alert_top: false },
    { fahrer_id: 'u3', fahrer_name: 'Max M.',   rang: 3, avg_dauer_min: 71, rank_delta: -1, ampel: 'gelb',  alert_top: false },
    { fahrer_id: 'u4', fahrer_name: 'Tim B.',   rang: 4, avg_dauer_min: 89, rank_delta:  0, ampel: 'rot',   alert_top: true  },
  ],
  team_avg: 64.3,
...
export function DispatchPhase3497TourDauerRankingBoard({
  locationId,
}: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-tour-dauer?location_id=${locationId}`, { cache: 'no-store' });
  ...
  const maxMin = Math.max(...data.fahrer.map(f => f.avg_dauer_min), 1);
        ...
        <div className="px-3 pb-3 space-y-2">
          ...
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1.5 p-1.5 rounded bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 text-xs">
              ...
              {data.alert_count} Fahrer mit langer Tour-Dauer — Routen prüfen!
            </div>
          )}
              ...
              <div key={f.fahrer_id} className="flex items-center gap-2">
                ...
                <div className="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.round((f.avg_dauer_min / maxMin) * 100)}%` }}
                  />
                </div>
          ...
          <div className="flex items-center gap-1 text-[9px] text-gray-400 pt-0.5">
            ...
            Rang 1 = kürzeste Tour-Dauer = effizientester Fahrer · Ziel ≤50min
          </div>
        </div>
      )}
    </div>

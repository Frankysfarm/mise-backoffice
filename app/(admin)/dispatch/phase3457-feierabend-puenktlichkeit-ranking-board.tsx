...
import { ChevronDown, ChevronUp, LogOut, TrendingUp, TrendingDown, Minus, AlertTriangle } from 'lucide-react';
...
export function DispatchPhase3457FeierabendPuenktlichkeitRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-feierabend-puenktlichkeit?location_id=${locationId}`, { cache: 'no-store' });
    ...
    <div className="border rounded-lg bg-white dark:bg-gray-900 shadow-sm mb-3">
      ...
      >
        <span className="flex items-center gap-2 font-semibold text-sm">
          ...
          Feierabend-Pünktlichkeit Ranking
          {data.alert_count > 0 && (
        ...
        <div className="px-3 pb-3 space-y-2">
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1.5 p-2 rounded bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 text-xs border border-red-200">
              ...
              Überstunden-Risiko! {data.alert_count} Fahrer unter Zielwert
            </div>
          )}

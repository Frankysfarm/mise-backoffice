...
// Phase 5602 — Pünktlichkeits-Trend-Board (Dispatch)
// TrendingUp cyan-400; trend_delta_pct ABSTEIGEND Rang 1=größte positive Verbesserung=bester;
...
const MOCK: ApiResponse = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   rang: 1, trend_delta_pct: 12, rank_delta:  2, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', rang: 2, trend_delta_pct:  5, rank_delta: -1, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, trend_delta_pct: -2, rank_delta:  0, ampel: 'gelb',  alert_rueckfall: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, trend_delta_pct:-11, rank_delta: -1, ampel: 'rot',   alert_rueckfall: true  },
  ],
  team_avg_trend: 1.0,
  bester_name: 'Max M.',
...
export function DispatchPhase5602PuenktlichkeitsTrendBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-puenktlichkeits-trend-ranking?location_id=${locationId}`);
  ...
  const maxAbs   = Math.max(...data.fahrer.map(f => Math.abs(f.trend_delta_pct)), 1);
    ...
    <div className={`rounded-lg bg-gray-900 border ${data.alert_count > 0 ? 'border-red-700/60' : 'border-gray-700/50'} p-3 space-y-2.5`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-white">Pünktlichkeits-Trend</span>
      ...
      <div className="grid grid-cols-3 gap-1.5 text-center">
        <div className="rounded bg-gray-800 px-2 py-1.5">
          ...
          <div className="text-xs font-bold text-green-400">{bester ? fmtDelta(bester.trend_delta_pct) : '—'}</div>
          <div className="text-[10px] text-gray-500 truncate">{data.bester_name}</div>
        </div>
        <div className="rounded bg-gray-800 px-2 py-1.5">
          <div className="text-[10px] text-gray-400">Team-Trend</div>
          <div className="text-xs font-bold text-white">{fmtDelta(data.team_avg_trend)}</div>
          <div className="text-[10px] text-gray-500">Ø 30 Tage</div>
        </div>
        <div className="rounded bg-gray-800 px-2 py-1.5">
          <div className="text-[10px] text-gray-400">Schwächste/r</div>
          <div className="text-xs font-bold text-red-400">{schwaech ? fmtDelta(schwaech.trend_delta_pct) : '—'}</div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 h-1.5 rounded-full bg-gray-800">
              <div
                ...
                style={{ width: `${(Math.abs(f.trend_delta_pct) / maxAbs) * 100}%`, backgroundColor: barColor(f.trend_delta_pct) }}
              />
            </div>
            <span className="text-[10px] font-mono text-gray-300 w-10 text-right">{fmtDelta(f.trend_delta_pct)}</span>

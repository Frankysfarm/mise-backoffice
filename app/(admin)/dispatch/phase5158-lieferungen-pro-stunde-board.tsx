...
export function DispatchPhase5158LieferungenProStundeBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="rounded-2xl border border-violet-700 bg-violet-950/40 overflow-hidden mb-4">
      <div className="px-4 py-3 flex items-center justify-between border-b border-violet-700/50 bg-violet-900/20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-violet-200">Lieferungen / Stunde (letzte 30 Tage)</span>
        </div>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 divide-x divide-violet-800/40 border-b border-violet-700/30">
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-green-300">{beste?.deliveries_pro_h?.toFixed(1) ?? '–'} /h</div>
        ...
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-violet-200">{data.team_avg_pro_h.toFixed(1)} /h</div>
          <div className="text-[10px] text-gray-500">Lieferungen/h</div>
        </div>
        <div className="px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500 mb-0.5">Niedrigste</div>
          <div className="text-sm font-bold text-gray-400">{niedrigste?.deliveries_pro_h?.toFixed(1) ?? '–'} /h</div>
          <div className="text-[10px] text-gray-500 truncate">{niedrigste?.fahrer_name ?? '–'}</div>
        </div>
      </div>
      ...
      <div className="px-4 py-2 border-t border-violet-800/30 bg-violet-900/10">
        <div className="text-[10px] text-gray-500">
          Produktivste: <span className="text-green-300 font-semibold">{data.produktivste_name}</span> ·
          Wenigste: <span className="text-gray-400">{data.wenigste_name}</span>
        </div>
      </div>
    </div>
  );

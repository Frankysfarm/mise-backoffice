...
export function DispatchPhase2892BewertungsTrendBoard({ locationId }: { locationId: string | null }) {
  ...
  const best     = sorted[0]?.avg_bewertung ?? 0;
    ...
    <div className={`rounded-xl border p-4 mb-4 ${hasAlert ? 'border-red-300 bg-red-50' : 'border-gray-200 bg-white'}`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800">Bewertungs-Trend Fahrer</span>
        ...
        <div className="mt-3 space-y-3">
          {hasAlert && (
            <div className="bg-red-100 border border-red-300 rounded-lg p-2 space-y-1">
              {alerts.map(f => (
                <div key={f.driver_id} className="flex items-center gap-2 text-xs text-red-700">
                  <AlertTriangle size={12} />
                  <span className="font-medium">{f.name}</span>
                  <span>— Niedrige Bewertung! ({f.avg_bewertung.toFixed(1)} ★ / Ziel ≥{ZIEL} ★)</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø', val: `${data.team_avg.toFixed(1)} ★` },
          ...
          <div className="space-y-2">
            {sorted.map(f => {
              ...
              const barPct  = Math.min((f.avg_bewertung / MAX_BAR) * 100, 100);
              const zielPct = (ZIEL / MAX_BAR) * 100;
              return (
                <div key={f.driver_id} className={`rounded-lg border p-2 ${a.bg}`}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-500">
                      ...
                      <span className={`font-bold ${a.text}`}>{f.avg_bewertung.toFixed(1)} ★</span>
                    </div>
                  </div>
          ...
          <div className="flex gap-3 text-[10px] text-gray-500">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400" /> {WARN}–{ZIEL - 0.1} ★</span>

...
function LieferzeitBar({ avg }: { avg: number }) {
  const MAX = 45;
  ...
  return (
    <div className="relative h-1.5 rounded-full bg-gray-200 w-24">
      ...
      <div className="absolute top-0 h-full border-l border-dashed border-green-500" style={{ left: `${(20 / MAX) * 100}%` }} title="20 min Ziel" />
      <div className="absolute top-0 h-full border-l border-dashed border-amber-400" style={{ left: `${(30 / MAX) * 100}%` }} title="30 min Alert" />
    </div>
  );
}

export function DispatchPhase2473LieferzeitEffizienzBoard({ locationId }: { locationId: string | null }) {
      ...
      {open && (
        <div className="bg-white px-4 pb-3 pt-2 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            {[
              { label: 'Ø heute', val: `${data.team_avg_lieferzeit_min.toFixed(1)} min`, color: data.team_avg_lieferzeit_min <= 20 ? 'text-green-600' : data.team_avg_lieferzeit_min <= 30 ? 'text-amber-600' : 'text-red-600' },
              { label: 'Ø Vorwoche', val: `${(data.team_avg_lieferzeit_min_vw ?? 0).toFixed(1)} min`, color: 'text-gray-600' },
          ...
          {hasAlerts && (
            <div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-xs text-red-700">
              ...
              <span>
                <strong>Zu lange Lieferzeit:</strong>{' '}
          ...
          <div className="space-y-1.5">
            {data.fahrer.map(f => (
              <div key={f.fahrer_id} className={`flex items-center gap-2 text-xs rounded-lg px-2.5 py-1.5 border ${ampelColor(f.ampel)}`}>
                ...
                <LieferzeitBar avg={f.avg_lieferzeit_min} />
                <span className="w-14 text-right font-semibold">{f.avg_lieferzeit_min.toFixed(1)} min</span>
          ...
          <div className="flex gap-3 text-xs text-gray-400">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500" />≤20 min</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400" />20–30 min</span>

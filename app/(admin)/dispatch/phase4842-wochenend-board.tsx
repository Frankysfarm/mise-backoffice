...
export function DispatchPhase4842WochenendBoard({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-purple-800 bg-purple-950/40 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-purple-300">Wochenend-Anteil-Ranking</span>
        <span className="ml-auto text-xs text-gray-500">Rang 1 = höchster Wochenend-Anteil</span>
      </div>

      {data.alert_count > 0 && (
        <div className="flex items-center gap-2 text-xs text-purple-300 bg-purple-900/30 rounded px-3 py-1.5 mb-3">
          <AlertTriangle className="w-3 h-3" />
          {data.alert_count} Fahrer mit hohem Wochenend-Anteil (&gt;60%)
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-2 mb-3 text-center">
        <div className="bg-black/20 rounded p-2">
          ...
          <div className="text-sm font-bold text-purple-300">{data.fahrer[0]?.wochenend_anteil_pct.toFixed(1)}%</div>
        </div>
        <div className="bg-black/20 rounded p-2">
          <div className="text-xs text-gray-400">Team-Ø</div>
          <div className="text-sm font-bold text-purple-200">{data.team_avg_pct.toFixed(1)}%</div>
        </div>
        <div className="bg-black/20 rounded p-2">

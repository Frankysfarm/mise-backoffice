...
export function DispatchPhase3979TrinkgeldBoard({ locationId }: { locationId: string | null }) {
  ...
  // descending: highest avg_tip_eur = Rang 1 = best
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Trinkgeld heute</span>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-emerald-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-emerald-600 font-medium">Höchstes Tip</div>
          <div className="text-lg font-black text-emerald-700">{fmt(best?.avg_tip_eur ?? 0)}</div>
        ...
        <div className="bg-gray-50 rounded-lg p-2 text-center">
          ...
          <div className="text-lg font-black text-gray-700">{fmt(data.team_avg_eur)}</div>
          <div className="text-[10px] text-gray-400">pro Lieferung</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-red-500 font-medium">Niedrigstes Tip</div>
          <div className="text-lg font-black text-red-600">{fmt(worst?.avg_tip_eur ?? 0)}</div>
        ...
        {sorted.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg ${bgColor}`}>
              ...
              <span className={`text-sm font-bold ${tColor}`}>{fmt(f.avg_tip_eur)}</span>
              {DeltaIcon}

...
function calcAmpel(t: number): string {
  if (t >= ZIEL_TOUREN) return 'gruen';
  if (t === ALERT_TOUREN) return 'gelb';
  return 'rot';
}

// Normal: steigend (mehr Touren) = grün = gut
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   touren_heute: 4, touren_gestern: 3, trend: 'steigend', trend_delta:  1, ampel: 'gruen', alert: null },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  touren_heute: 3, touren_gestern: 3, trend: 'stabil',   trend_delta:  0, ampel: 'gruen', alert: null },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   touren_heute: 2, touren_gestern: 3, trend: 'fallend',  trend_delta: -1, ampel: 'gelb',  alert: null },
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', touren_heute: 1, touren_gestern: 2, trend: 'fallend',  trend_delta: -1, ampel: 'rot',   alert: 'Zu wenige Touren!' },
  ],
  team_avg_touren: 2.5,
  alert_count: 1,
};

export function DispatchPhase2976TourenProTagBoard({ locationId }: { locationId: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-touren-pro-tag?location_id=${locationId}`)
        .then(r => r.json())
  ...
  const enriched  = data.fahrer.map(f => ({ ...f, ampel: calcAmpel(f.touren_heute) }));
  const sorted    = [...enriched].sort((a, b) => b.touren_heute - a.touren_heute);
  const alerts    = enriched.filter(f => f.alert !== null);
  const hasAlert  = alerts.length > 0;
  const best      = sorted[0]?.touren_heute ?? 0;
  const teamAmpel = calcAmpel(Math.round(data.team_avg_touren));
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Touren/Tag-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 text-sm">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: Zu wenige Touren! ({f.touren_heute} heute)
              </span>
            </div>
          ))}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø', value: `${data.team_avg_touren.toFixed(1)} Touren`, cls: teamText },
              { label: 'Bester', value: `${best} Touren`,                             cls: 'text-green-600' },
              { label: 'Ziel',   value: `≥${ZIEL_TOUREN} Touren`,                    cls: 'text-gray-600 dark:text-gray-400' },
            ].map(k => (
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.touren_heute} Touren</span>
                    </div>
                  </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 inline-block" />≥3 Touren</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />2 Touren</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />&lt;2 Touren</span>
            <span className="text-gray-400">| Ziel ≥{ZIEL_TOUREN}</span>
          </div>
        </div>
      )}
    </div>

...
import { ChevronDown, ChevronUp, AlertTriangle, Trophy, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   rang: 1, auslastung_pct: 72.4, rank_delta:  0, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', rang: 2, auslastung_pct: 65.8, rank_delta: -1, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, auslastung_pct: 58.2, rank_delta:  1, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, auslastung_pct: 41.5, rank_delta: -1, ampel: 'rot',   alert_bottom: true  },
  ],
  team_avg_pct: 59.5,
  bester_name: 'Max M.',
  letzter_name: 'Tim B.',
...
export function DispatchPhase3076AuslastungsRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-auslastungs-ranking?location_id=${locationId}`)
        .then(r => r.json())
        ...
        <div className="p-4 space-y-3">
          ...
          {alerts.length > 0 && (
            <div className="flex items-center gap-2 rounded-lg bg-red-50 border border-red-200 text-red-700 text-xs font-semibold px-3 py-2">
              ...
              {alerts.map(f => f.fahrer_name).join(', ')} — Niedrigstes Auslastungs-Ranking!
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-yellow-50 dark:bg-yellow-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <Trophy size={10} className="text-yellow-500" /> Bester
              </div>
              <div className="font-bold text-sm text-yellow-700 dark:text-yellow-400">{bester?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{bester?.auslastung_pct.toFixed(1) ?? '—'} %</div>
            </div>
            <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5">Team-Ø</div>
              <div className={`font-bold text-base ${teamAvg >= 60 ? 'text-green-600' : teamAvg >= 40 ? 'text-amber-600' : 'text-red-600'}`}>
                {teamAvg.toFixed(1)} %
              </div>
            </div>
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="text-xs text-gray-500">{letzter?.auslastung_pct.toFixed(1) ?? '—'} %</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {list.map(f => {
              ...
              const barW = Math.round(f.auslastung_pct);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  ...
                  {f.alert_bottom && (
                    <div className="mt-1.5 text-xs text-red-600 font-semibold flex items-center gap-1">
                      <AlertTriangle size={10} /> Niedrigstes Auslastungs-Ranking!
                    </div>
                  )}
                </div>
              );

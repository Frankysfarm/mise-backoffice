...
export function DispatchPhase4588WochenendAnteilBoard({ locationId }: Props) {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2 text-center">
          ...
          <div className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 truncate">{data.meister_name}</div>
        </div>
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2 text-center">
          <div className="text-xs text-gray-500 dark:text-gray-400">Team-Avg</div>
          <div className="text-sm font-semibold text-gray-700 dark:text-gray-300">{data.team_avg_pct}%</div>
        </div>
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2 text-center">
          <div className="text-xs text-gray-500 dark:text-gray-400">Niedrigste</div>
          <div className="text-sm font-semibold text-red-500 dark:text-red-400 truncate">{data.wenigster_name}</div>
        </div>
      </div>
          ...
          <div key={f.fahrer_id} className="rounded-lg bg-gray-50 dark:bg-gray-800 px-3 py-2">
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-2">
                ...
                {f.alert_hoch && <AlertTriangle className="w-3 h-3 text-red-400" />}
              </div>
            ...
            <div className="h-1.5 rounded-full bg-gray-200 dark:bg-gray-700 overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${barClass(f.ampel)}`}

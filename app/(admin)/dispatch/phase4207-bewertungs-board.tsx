...
import { ThumbsUp, TrendingUp, TrendingDown, Minus, AlertTriangle } from 'lucide-react';

interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; score: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_bottom: boolean; }
interface ApiData { fahrer: FahrerRow[]; team_avg_score: number; bester_name: string; niedrigster_name: string; alert_count: number; gesamt: number; }
...
export function DispatchPhase4207BewertungsBoard({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-green-50 rounded-lg p-2">
          ...
          <p className="text-sm font-bold text-green-700">{bester?.score?.toFixed(1)} ★</p>
        ...
        <div className="bg-gray-50 rounded-lg p-2">
          ...
          <p className="text-sm font-bold text-gray-700">{data.team_avg_score?.toFixed(1)} ★</p>
          <p className="text-[10px] text-gray-400">Bewertung</p>
        </div>
        <div className="bg-red-50 rounded-lg p-2">
          <p className="text-[10px] text-red-500 font-medium">Niedrigste</p>
          <p className="text-sm font-bold text-red-600">{letzter?.score?.toFixed(1)} ★</p>
      ...
      <div className="space-y-1.5">
        {data.fahrer.map((f) => {
          const barW = maxScore > 0 ? (f.score / maxScore) * 100 : 0;
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.score?.toFixed(1)} ★</span>
              </div>
      ...
      <div className="flex justify-between text-[10px] text-gray-400 pt-1 border-t border-gray-100">
        <span>Team-Ø {data.team_avg_score?.toFixed(1)} ★ Bewertung</span>

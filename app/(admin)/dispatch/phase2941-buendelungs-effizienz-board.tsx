...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Julia F.', avg_stopps: 4.2, touren_heute: 11, trend: 'steigend', trend_delta:  0.4, ampel: 'gruen' },
    { fahrer_id: 'd2', fahrer_name: 'Max M.',   avg_stopps: 3.6, touren_heute: 9,  trend: 'fallend',  trend_delta: -0.3, ampel: 'gruen' },
    { fahrer_id: 'd3', fahrer_name: 'Sara K.',  avg_stopps: 2.1, touren_heute: 8,  trend: 'stabil',   trend_delta:  0.0, ampel: 'gelb'  },
    { fahrer_id: 'd4', fahrer_name: 'Tim B.',   avg_stopps: 1.3, touren_heute: 6,  trend: 'steigend', trend_delta:  0.3, ampel: 'rot'   },
  ],
  team_avg_stopps: 2.8,
};

export function DispatchPhase2941BuendelungsEffizienzBoard({ locationId }: { locationId: string | null }) {
  ...
  const best      = sorted[0]?.avg_stopps ?? 0;
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Bündelungs-Effizienz-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 text-sm">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: Zu wenige Stopps je Tour — schlechte Bündelung! ({f.avg_stopps.toFixed(1)} Ø)
              </span>
            </div>
          ))}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø',  value: `${data.team_avg_stopps.toFixed(1)} Stopps`, cls: teamText },
              { label: 'Bester',  value: `${best.toFixed(1)} Stopps`,                 cls: 'text-green-600' },
              { label: 'Ziel',    value: `≥${ZIEL} Stopps`,                           cls: 'text-gray-600 dark:text-gray-400' },
            ].map(k => (
          ...
          <div className="space-y-2">
            {sorted.map(f => {
              ...
              const pct = Math.min(100, (f.avg_stopps / MAX_BAR) * 100);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.avg_stopps.toFixed(1)} Stopps/Tour</span>
                    </div>
                  </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 inline-block" />≥{ZIEL} Stopps</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />{WARN}–{ZIEL - 1}</span>

...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 1, avg_min: 17, rank_delta: -2, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f2', fahrer_name: 'Max M.',   rang: 2, avg_min: 22, rank_delta:  0, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, avg_min: 28, rank_delta:  1, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, avg_min: 34, rank_delta:  0, ampel: 'rot',   alert_bottom: true  },
  ],
  team_avg_min: 25,
  bester_name: 'Julia F.',
  letzter_name: 'Tim B.',
...
function ampelCls(a: string) {
  ...
  return                   { bg: 'bg-blue-50 border-blue-200 dark:bg-blue-950 dark:border-blue-800',     dot: 'bg-blue-500',  text: 'text-blue-700 dark:text-blue-300',   bar: 'bg-blue-500'  };
}
...
export function DispatchPhase3321AvgLieferzeitRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxMin = Math.max(...list.map(f => f.avg_min), 1);
        ...
        <div className="p-4 space-y-3">
          {alerts.length > 0 && (
            <div className="flex items-center gap-2 rounded-lg bg-red-50 border border-red-200 text-red-700 dark:text-red-300 dark:bg-red-950 dark:border-red-800 text-xs font-semibold px-3 py-2">
              ...
              {alerts.map(f => f.fahrer_name).join(', ')} — Hohe Lieferzeit!
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-yellow-50 dark:bg-yellow-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <Clock size={10} className="text-blue-500" /> Schnellster
              </div>
              <div className="font-bold text-sm text-yellow-700 dark:text-yellow-400">{schnellster?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{schnellster ? `${schnellster.avg_min} min` : '—'}</div>
            </div>
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="text-xs text-gray-400">Ziel ≤ 20 min</div>
            </div>
            <div className="rounded-lg bg-red-50 dark:bg-red-900/20 p-2">
              ...
              <div className="text-xs text-gray-500">{langsamster ? `${langsamster.avg_min} min` : '—'}</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {list.map(f => {
              ...
              const barW = maxMin > 0 ? Math.round((f.avg_min / maxMin) * 100) : 0;
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className={`text-sm font-bold ${cls.text}`}>{f.avg_min} min</span>
                      {f.rank_delta < 0
          ...
          <div className="pt-1 border-t border-gray-100 dark:border-gray-700 flex justify-between text-xs text-gray-400">
            <span>Team-Ø: {d.team_avg_min} min · Ziel ≤ 20 min</span>
            <span>heute</span>
          </div>
        </div>
      )}

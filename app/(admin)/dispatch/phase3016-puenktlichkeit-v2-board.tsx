...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',  puenktlichkeit_pct: 96, lieferungen_heute: 20, trend: 'steigend', trend_delta:  3, ampel: 'gruen', alert_unpuenktlich: false },
    { fahrer_id: 'f2', fahrer_name: 'Lisa F.', puenktlichkeit_pct: 91, lieferungen_heute: 18, trend: 'steigend', trend_delta:  2, ampel: 'gruen', alert_unpuenktlich: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.', puenktlichkeit_pct: 78, lieferungen_heute: 15, trend: 'fallend',  trend_delta: -4, ampel: 'gelb',  alert_unpuenktlich: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',  puenktlichkeit_pct: 62, lieferungen_heute: 12, trend: 'fallend',  trend_delta: -6, ampel: 'rot',   alert_unpuenktlich: true  },
  ],
  team_durchschnitt: 81.8,
  alert_count: 1,
};

export function DispatchPhase3016PuenktlichkeitV2Board({ locationId }: { locationId: string | null }) {
  ...
  const best     = sorted[0]?.puenktlichkeit_pct ?? 0;
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Pünktlichkeits-Board v2</span>
        ...
        <div className="px-4 py-3 space-y-3">
          ...
          <div className="space-y-2">
            {sorted.map(f => {
              ...
              const pct = Math.min(100, f.puenktlichkeit_pct);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.puenktlichkeit_pct.toFixed(1)}%</span>
                    </div>
                  </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            ...
            <span className="text-gray-400">| Ziel ≥{ZIEL_PCT}% · ±5 Min ETA-Fenster</span>
          </div>
        </div>
      )}
    </div>

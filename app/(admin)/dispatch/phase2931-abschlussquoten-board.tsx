...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Julia F.', quote_pct: 97.6, trend: 'steigend', trend_delta:  1.0, ampel: 'gruen' },
    { fahrer_id: 'd2', fahrer_name: 'Max M.',   quote_pct: 97.4, trend: 'steigend', trend_delta:  0.5, ampel: 'gruen' },
    { fahrer_id: 'd3', fahrer_name: 'Sara K.',  quote_pct: 88.6, trend: 'stabil',   trend_delta:  0.0, ampel: 'gelb'  },
    { fahrer_id: 'd4', fahrer_name: 'Tim B.',   quote_pct: 82.1, trend: 'fallend',  trend_delta: -2.0, ampel: 'rot'   },
  ],
  team_avg_quote: 91.4,
};

export function DispatchPhase2931AbschlussquotenBoard({ locationId }: { locationId: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-abschlussquote?location_id=${locationId}`)
        .then(r => r.json())
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Abschlussquoten-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 text-sm">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: Abschlussquote zu niedrig! {f.quote_pct.toFixed(0)}%
              </span>
            </div>
          ))}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø',  value: `${data.team_avg_quote.toFixed(0)}%`, cls: teamText },
              { label: 'Bester',  value: `${best.toFixed(0)}%`,                cls: 'text-green-600' },
          ...
          <div className="space-y-2">
            {sorted.map(f => {
              ...
              const pct = Math.min(100, f.quote_pct);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.quote_pct.toFixed(0)}%</span>
                    </div>
                  </div>

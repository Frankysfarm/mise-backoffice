...
export function DispatchPhase5247TrinkgeldProLieferungBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxVal = data.fahrer[0].avg_trinkgeld || 1;
  ...
  return (
    <div className="rounded-xl border border-green-700 bg-green-900/30 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-gray-200">Trinkgeld/Lieferung-Ranking (letzte 30 Tage)</span>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-lg bg-green-900/50 px-3 py-2 text-center">
          ...
          <div className="text-xs font-bold text-green-300 truncate">{data.beste_name}</div>
        </div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 h-2 rounded-full bg-gray-800 overflow-hidden">
              <div
                ...
                style={{ width: `${Math.min(100, Math.round((f.avg_trinkgeld / maxVal) * 100))}%` }}
              />
            </div>
      ...
      {data.beste_name && (
        <div className="mt-3 text-[10px] text-green-400 border-t border-green-800/40 pt-2">
          Meiste: {data.beste_name} — {data.fahrer[0]?.avg_trinkgeld.toFixed(2)} € Trinkgeld/Lieferung
        </div>
      )}
    </div>
  );

...
export function DispatchPhase4473PuenktlichkeitBoard({ locationId }: Props) {
  const [data, setData] = useState<PuenktlichkeitData | null>(null);
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900 dark:text-gray-100 text-sm">Pünktlichkeits-Ranking (30 Tage)</h3>
        <span className="ml-auto text-xs text-gray-400">% pünktlich</span>
      </div>

      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="rounded-lg bg-emerald-50 dark:bg-emerald-900/20 p-2">
          <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400">{best?.puenktlichkeit_pct}%</div>
          <div className="text-xs text-gray-500 dark:text-gray-400">Pünktlichste ({best?.name})</div>
        </div>
        ...
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          <div className="text-lg font-bold text-gray-500 dark:text-gray-400">{worst?.puenktlichkeit_pct}%</div>
          <div className="text-xs text-gray-500 dark:text-gray-400">Unpünktlichste ({worst?.name})</div>
        </div>
      </div>

      {alerts.length > 0 && (
        <div className="flex items-center gap-2 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 px-3 py-2">
          <AlertTriangle className="w-4 h-4 text-indigo-500 flex-shrink-0" />
          <span className="text-xs text-indigo-700 dark:text-indigo-300">
            Pünktlichkeitsproblem: {alerts.map(a => a.name).join(', ')}
          </span>
        </div>
      )}
        ...
        {data.ranking.map((f) => (
          <div key={f.driver_id} className="space-y-1">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className={`font-semibold text-xs ${ampelColor(f.ampel)}`}>{f.puenktlichkeit_pct}%</span>
            </div>
      ...
      <div className="text-xs text-gray-400">Ziel: ≥90% pünktlich · {data.gesamt} Fahrer</div>
    </div>
  );
}

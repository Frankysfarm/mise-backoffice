...
import { Clock4, TrendingUp, TrendingDown, Minus, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="ml-auto text-xs text-gray-400">Pünktlichkeitsrate</span>
      </div>

      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="rounded-lg bg-emerald-50 dark:bg-emerald-900/20 p-2">
          <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400">{best?.puenktlichkeit_pct.toFixed(1)}%</div>
          <div className="text-xs text-gray-500 dark:text-gray-400">Beste ({best?.name})</div>
        </div>
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          <div className="text-lg font-bold text-gray-700 dark:text-gray-300">{data.team_avg.toFixed(1)}%</div>
          <div className="text-xs text-gray-500 dark:text-gray-400">Team-Avg</div>
        </div>
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          <div className="text-lg font-bold text-gray-500 dark:text-gray-400">{worst?.puenktlichkeit_pct.toFixed(1)}%</div>
      ...
      {alerts.length > 0 && (
        <div className="flex items-center gap-2 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 px-3 py-2">
          ...
          <span className="text-xs text-blue-700 dark:text-blue-300">
            Hohe Verspätungsrate: {alerts.map(a => a.name).join(', ')}
          </span>
        </div>
      )}
        ...
        {data.ranking.map((f) => (
          <div key={f.driver_id} className="space-y-1">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className={`font-semibold text-xs ${ampelColor(f.ampel)}`}>{f.puenktlichkeit_pct.toFixed(1)}%</span>
            </div>
      ...
      <div className="text-xs text-gray-400">Ziel: ≥85% pünktlich · {data.gesamt} Fahrer</div>
    </div>
  );
}

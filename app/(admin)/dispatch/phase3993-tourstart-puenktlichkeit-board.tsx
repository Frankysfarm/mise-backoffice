...
export function DispatchPhase3993TourstartPuenktlichkeitBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxVal = Math.max(...sorted.map(f => f.avg_verzoegerung_min), 1);
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Tourstart-Pünktlichkeit</span>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-emerald-50 rounded-lg p-2 text-center">
          ...
          <div className="text-lg font-black text-emerald-700">+{best?.avg_verzoegerung_min ?? 0}min</div>
        ...
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-red-500 font-medium">Verspätetster</div>
          <div className="text-lg font-black text-red-600">+{worst?.avg_verzoegerung_min ?? 0}min</div>
      ...
      <div className="space-y-1.5">
        {sorted.map((f) => {
          ...
          const barPct = maxVal > 0 ? Math.round((f.avg_verzoegerung_min / maxVal) * 100) : 0;
          // rank_delta < 0 = verbessert (niedrigerer Rang = besser bei aufsteigend)
          ...
          return (
            <div key={f.fahrer_id} className={`px-2.5 py-1.5 rounded-lg ${bgColor}`}>
              <div className="flex items-center gap-2">
                <span className="w-5 text-center text-[11px] font-mono text-gray-400">{badge ?? `#${f.rang}`}</span>
                <span className="flex-1 text-xs font-medium text-gray-800 truncate">{f.fahrer_name}</span>
                <span className={`text-sm font-bold ${tColor}`}>+{f.avg_verzoegerung_min}min</span>
                {DeltaIcon}
                {f.alert_verspaetet && <AlertTriangle className="w-3 h-3 text-red-500 shrink-0" />}
              </div>
              <div className="mt-1 h-1 bg-gray-200 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${f.ampel === 'gruen' ? 'bg-emerald-400' : f.ampel === 'gelb' ? 'bg-yellow-400' : 'bg-red-400'}`}
                  style={{ width: `${barPct}%` }} />
              </div>
            </div>
          );

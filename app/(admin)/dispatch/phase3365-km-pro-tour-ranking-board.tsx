...
export function DispatchPhase3365KmProTourRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxKm = Math.max(...list.map(f => f.km_avg), 1);
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <Route size={10} className="text-blue-600" /> Effizientester
              </div>
              <div className="font-bold text-sm text-blue-700 dark:text-blue-400">{bester?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{bester ? `${bester.km_avg.toFixed(1)} km/Tour` : '—'}</div>
            </div>
            ...
            <div className="rounded-lg bg-red-50 dark:bg-red-900/20 p-2">
              ...
              <div className="text-xs text-gray-500">{letzter ? `${letzter.km_avg.toFixed(1)} km/Tour` : '—'}</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {list.map(f => {
              ...
              const barW = Math.round((f.km_avg / maxKm) * 100);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className={`text-sm font-bold ${cls.text}`}>{f.km_avg.toFixed(1)} km</span>
                      {f.rank_delta < 0

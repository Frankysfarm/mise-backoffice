...
export function DispatchPhase4162ErstkontaktBoard({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        {data.alert_count > 0 && (
          <span className="flex items-center gap-1 text-xs text-red-600 font-semibold bg-red-50 px-2 py-0.5 rounded-full">
            <AlertTriangle className="w-3.5 h-3.5" /> {data.alert_count} Langsamer Kontakt!
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-rose-50 rounded-lg p-2">
          <p className="text-[10px] text-rose-600 font-medium">Schnellste</p>
          <p className="text-sm font-bold text-rose-700">{bester?.avg_sek?.toFixed(0)} sek</p>
          <p className="text-[10px] text-rose-500 truncate">{data.bester_name}</p>
        </div>
        <div className="bg-gray-50 rounded-lg p-2">
          <p className="text-[10px] text-gray-500 font-medium">Team-Ø</p>
          <p className="text-sm font-bold text-gray-700">{data.team_avg_sek?.toFixed(0)} sek</p>
        ...
        <div className="bg-red-50 rounded-lg p-2">
          ...
          <p className="text-sm font-bold text-red-600">{letzter?.avg_sek?.toFixed(0)} sek</p>
      ...
      <div className="space-y-1.5">
        {data.fahrer.map((f) => {
          ...
          const tColor = f.ampel === 'gruen' ? 'text-rose-600' : f.ampel === 'gelb' ? 'text-yellow-500' : 'text-red-500';
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.avg_sek?.toFixed(0)} sek</span>
              </div>
      ...
      <div className="flex justify-between text-[10px] text-gray-400 pt-1 border-t border-gray-100">
        <span>Team-Ø {data.team_avg_sek?.toFixed(0)} sek Erstkontakt</span>
        <span>#1 = schnellster Erstkontakt</span>
      </div>
    </div>
  );
}

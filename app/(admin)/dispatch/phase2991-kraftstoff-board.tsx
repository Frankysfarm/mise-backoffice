...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   km_pro_liter: 16.8, trend: 'steigend', trend_delta:  0.9 },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', km_pro_liter: 14.9, trend: 'steigend', trend_delta:  1.1 },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  km_pro_liter: 13.5, trend: 'fallend',  trend_delta: -0.7 },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   km_pro_liter:  9.4, trend: 'fallend',  trend_delta: -0.7 },
  ],
  team_avg_kml: 13.7,
  alert_count: 1,
};

export function DispatchPhase2991KraftstoffBoard({ locationId }: { locationId: string | null }) {
  ...
  const enriched = data.fahrer.map(f => ({ ...f, ampel: calcAmpel(f.km_pro_liter) }));
  const sorted   = [...enriched].sort((a, b) => b.km_pro_liter - a.km_pro_liter);
  const alerts   = enriched.filter(f => f.km_pro_liter < ALERT_KML);
  const hasAlert = alerts.length > 0;
  const best     = sorted[0]?.km_pro_liter ?? 0;
  const teamAmpel = calcAmpel(data.team_avg_kml);
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Kraftstoff-Effizienz-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø', value: `${data.team_avg_kml.toFixed(1)} km/l`, cls: teamText },
              { label: 'Bester', value: `${best.toFixed(1)} km/l`,              cls: 'text-green-600' },
              { label: 'Ziel',   value: `≥${ZIEL_KML} km/l`,                   cls: 'text-gray-600 dark:text-gray-400' },
            ].map(k => (
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.km_pro_liter.toFixed(1)} km/l</span>
                    </div>
                  </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 inline-block" />≥15 km/l</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />10–14 km/l</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />&lt;10 km/l</span>
            <span className="text-gray-400">| Ziel ≥{ZIEL_KML} km/l</span>
          </div>
        </div>
      )}
    </div>

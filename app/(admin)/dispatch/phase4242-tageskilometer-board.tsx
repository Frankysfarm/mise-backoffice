...
export function DispatchPhase4242TageskilometerBoard({ locationId }: Props) {
  ...
  const maxKm = Math.max(...data.fahrer.map((f) => f.km), 0.1);
    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Tages-Kilometer</span>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-blue-50 rounded-lg p-2 text-center">
          <div className="text-xs text-blue-600 font-medium truncate">{data.bester_name}</div>
          <div className="text-lg font-bold text-blue-700">{bester?.km}</div>
          <div className="text-[10px] text-blue-500">km (Meiste)</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2 text-center">
          <div className="text-xs text-gray-500 font-medium">Team-Ø</div>
          <div className="text-lg font-bold text-gray-700">{data.team_avg_km}</div>
          <div className="text-[10px] text-gray-400">km / Fahrer</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-xs text-red-600 font-medium truncate">{data.letzter_name}</div>
          <div className="text-lg font-bold text-red-700">{letzter?.km}</div>
          <div className="text-[10px] text-red-500">km (Wenigste)</div>
        </div>
      </div>
      ...
      {hasAlert && (
        <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5">
          ...
          <span className="text-xs text-amber-700 font-medium">Wenige km heute – Auslastung prüfen!</span>
        </div>
      )}

      <div className="space-y-2">
        {data.fahrer.map((f) => {
          const barW = maxKm > 0 ? (f.km / maxKm) * 100 : 0;
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-900 w-14 text-right">{f.km} km</span>
              </div>
      ...
      <div className="text-[10px] text-gray-400 text-right">#1 = meiste km heute • 30-Min-Aktualisierung</div>
    </div>
  );
}

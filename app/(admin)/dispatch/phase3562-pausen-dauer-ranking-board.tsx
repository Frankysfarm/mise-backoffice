...
export function DispatchPhase3562PausenDauerRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxMin = Math.max(...data.fahrer.map(f => f.pausen_dauer_min), 1);
        ...
        <div className="px-3 pb-3 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2 py-1">
            <div className="text-center">
              <div className="text-[10px] text-gray-500 uppercase">Kürzester</div>
              <div className="text-xs font-semibold text-emerald-600 truncate">{data.kuerzester_name}</div>
            </div>
            <div className="text-center">
            ...
            <div className="text-center">
              <div className="text-[10px] text-gray-500 uppercase">Längster</div>
              <div className="text-xs font-semibold text-red-600 truncate">{data.laengster_name}</div>
            </div>
          </div>
              ...
              <div key={f.fahrer_id} className="flex items-center gap-2">
                ...
                <div className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-full h-2 relative">
                  <div
                    ...
                    style={{ width: `${(f.pausen_dauer_min / maxMin) * 100}%` }}
                  />
                </div>

...
export function DispatchPhase4508KmProLieferungBoard({ locationId }: Props) {
  const [data, setData] = useState<KmData | null>(null);
  const [loading, setLoading] = useState(true);

  const fetchData = useCallback(async () => {
    const params = locationId ? `?location_id=${locationId}` : '';
    const res = await fetch(`/api/delivery/admin/fahrer-km-pro-lieferung-ranking${params}`, { cache: 'no-store' });
  ...
  // INVERTED: wenigste km = bester → Balken relativ zu maxKm
  const maxKm = worst?.avg_km_pro_lieferung ?? 1;

  const ampelColor = (a: FahrerKm['ampel']) =>
    a === 'gruen' ? 'text-green-600 dark:text-green-400' :
    a === 'rot'   ? 'text-red-500 dark:text-red-400' : 'text-yellow-500 dark:text-yellow-400';

  const ampelBg = (a: FahrerKm['ampel']) =>
    a === 'gruen' ? 'bg-green-500' : a === 'rot' ? 'bg-red-500' : 'bg-yellow-400';
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900 dark:text-gray-100 text-sm">km/Lieferung-Ranking (30 Tage)</h3>
        <span className="ml-auto text-xs text-gray-400">Ø km pro Lieferung</span>
      </div>

      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="rounded-lg bg-green-50 dark:bg-green-900/20 p-2">
          <div className="text-lg font-bold text-green-600 dark:text-green-400">{best?.avg_km_pro_lieferung.toFixed(1)} km</div>
          <div className="text-xs text-gray-500 dark:text-gray-400">Effizienteste ({best?.fahrer_name})</div>
        </div>
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          <div className="text-lg font-bold text-gray-700 dark:text-gray-300">{data.team_avg_km.toFixed(1)} km</div>
          <div className="text-xs text-gray-500 dark:text-gray-400">Team-Avg</div>
        </div>
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          <div className="text-lg font-bold text-gray-500 dark:text-gray-400">{worst?.avg_km_pro_lieferung.toFixed(1)} km</div>
          <div className="text-xs text-gray-500 dark:text-gray-400">Ineffizienteste ({worst?.fahrer_name})</div>
        </div>
      </div>
      ...
      {alerts.length > 0 && (
        <div className="flex items-center gap-2 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 px-3 py-2">
          ...
          <span className="text-xs text-red-700 dark:text-red-300">
            Hoher km-Verbrauch (&gt;8 km): {alerts.map(a => a.fahrer_name).join(', ')}
          </span>
        </div>
      )}
        ...
        {data.fahrer.map((f) => (
          <div key={f.fahrer_id} className="space-y-1">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className={`font-semibold text-xs ${ampelColor(f.ampel)}`}>{f.avg_km_pro_lieferung.toFixed(1)} km</span>
            </div>
            <div className="h-1.5 rounded-full bg-gray-100 dark:bg-gray-800">
              <div
                className={`h-full rounded-full ${ampelBg(f.ampel)}`}
                style={{ width: `${maxKm > 0 ? Math.round((f.avg_km_pro_lieferung / maxKm) * 100) : 0}%` }}
              />
            </div>
          </div>
        ))}
      ...
      <div className="text-xs text-gray-400">Ziel: ≤4 km/Lieferung · {data.gesamt} Fahrer</div>
    </div>
  );
}

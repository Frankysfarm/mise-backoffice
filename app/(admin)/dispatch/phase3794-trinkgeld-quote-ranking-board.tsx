...
export function DispatchPhase3794TrinkgeldQuoteRankingBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-emerald-50 rounded-lg p-2">
          <div className="text-base font-black text-emerald-700">€{data.fahrer[0]?.avg_tip_eur.toFixed(2)}</div>
          <div className="text-[10px] text-gray-500">Bester</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2">
          <div className="text-base font-black text-gray-800">€{data.team_avg_eur.toFixed(2)}</div>
        ...
        <div className={`rounded-lg p-2 ${data.alert_count > 0 ? 'bg-red-50' : 'bg-gray-50'}`}>
          <div className={`text-base font-black ${data.alert_count > 0 ? 'text-red-600' : 'text-gray-800'}`}>
            €{data.fahrer[data.fahrer.length - 1]?.avg_tip_eur.toFixed(2)}
          </div>
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 bg-yellow-50 border border-yellow-200 rounded-lg text-xs text-yellow-800">
          ...
          <span>{data.alert_count} Fahrer mit niedrigem Trinkgeld — Niedriges Trinkgeld!</span>
        </div>
      )}

      {/* Ranking-Liste (absteigend: höchstes Trinkgeld = Rang 1 = bester) */}
          ...
          return (
            <div key={f.fahrer_id} className={`rounded-lg border ${c.border} ${c.bg} p-2.5 space-y-1.5`}>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm font-black ${c.text}`}>€{f.avg_tip_eur.toFixed(2)}</span>
                {f.rank_delta !== 0 && (
                  f.rank_delta > 0
                    ? <TrendingUp className="w-3 h-3 text-emerald-500 shrink-0" />
                    : <TrendingDown className="w-3 h-3 text-red-500 shrink-0" />
                )}
                {f.alert_niedrig && <AlertTriangle className="w-3.5 h-3.5 text-yellow-500 shrink-0" />}
              </div>
              <div className="h-1.5 bg-white/60 rounded-full overflow-hidden">
                <div className={`h-full ${c.bar} rounded-full transition-all duration-500`} style={{ width: `${barPct}%` }} />
              </div>
              {f.alert_niedrig && (
                <div className="text-[10px] text-yellow-700 font-medium">Niedriges Trinkgeld!</div>
              )}
            </div>
          );
        })}
      ...
      <div className="text-[10px] text-gray-400 text-center">
        Ziel ≥€{data.ziel_eur.toFixed(2)} · Rang 1=höchstes Trinkgeld=bester · 30-Min-Polling
      </div>
    </div>
  );
}

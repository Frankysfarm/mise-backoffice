...
export function DispatchPhase5283BonusQuoteBoard({ locationId }: { locationId: string | null }) {
  const [data, setData] = useState<ApiResponse | null>(null);

  async function load() {
    const url = locationId
      ? `/api/delivery/admin/fahrer-bonus-quote-ranking?location_id=${locationId}`
      : '/api/delivery/admin/fahrer-bonus-quote-ranking';
  ...
  return (
    <div className="rounded-xl border border-green-700 bg-green-900/30 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-gray-200">Bonus-Quote-Ranking (letzte 30 Tage)</span>
        {data.alert_count > 0 && (
          <span className="ml-auto flex items-center gap-1 text-xs text-green-400">
            <AlertTriangle className="w-3 h-3" />
            {data.alert_count} ≥50%-Alert
          </span>
        )}
      </div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 h-2 rounded-full bg-gray-800 overflow-hidden">
              <div
                ...
                style={{ width: `${f.bonus_quote_pct}%` }}
              />
            </div>
            <span className="text-xs text-gray-400 w-10 text-right tabular-nums">
              {f.bonus_quote_pct.toFixed(0)}%
            </span>
            <DeltaIcon delta={f.rank_delta} />
            {f.alert_hoch && <Gift className="w-3 h-3 text-green-400 shrink-0" />}
          </div>
        ))}
      </div>
      ...
      {data.meister_name && (
        <div className="mt-3 text-[10px] text-green-400 border-t border-green-800/40 pt-2">
          Meister: {data.meister_name} — {data.fahrer[0]?.bonus_quote_pct.toFixed(0)}% Touren mit Bonus
        </div>
      )}
    </div>
  );

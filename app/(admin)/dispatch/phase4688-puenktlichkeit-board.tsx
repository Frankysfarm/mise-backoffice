...
export function DispatchPhase4688PuenktlichkeitBoard({ locationId }: { locationId: string | null }) {
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Pünktlichkeit-Ranking nicht verfügbar</span>
      </div>
    );
  }
  ...
  const maxPct = Math.max(...data.fahrer.map(f => f.puenktlichkeit_pct), 1);
  ...
  return (
    <div className="rounded-2xl border border-teal-200 dark:border-teal-900 bg-teal-50 dark:bg-teal-950 p-3 space-y-2">
      <div className="flex items-center justify-between">
        ...
        {data.alert_count > 0 && (
          <div className="flex items-center gap-1 bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 rounded-full px-2 py-0.5">
            ...
            <span className="text-[10px] font-semibold">Niedrige Pünktlichkeit!</span>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {[
          { label: 'Höchste', value: `${data.fahrer[0]?.puenktlichkeit_pct ?? 0} %` },
          { label: 'Team-Ø', value: `${data.team_avg_pct} %` },
          { label: 'Niedrigste', value: `${data.fahrer[data.fahrer.length - 1]?.puenktlichkeit_pct ?? 0} %` },
        ].map(kpi => (
          ...
          <div key={f.fahrer_id} className="space-y-0.5">
            <div className="flex items-center gap-1.5">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-[10px] font-bold text-teal-900 dark:text-teal-200">{f.puenktlichkeit_pct} %</span>
              </div>
            </div>
            <div className="flex items-center gap-1">
              <div className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-full h-1.5">
                <div
                  ...
                  style={{ width: `${(f.puenktlichkeit_pct / maxPct) * 100}%` }}
                />
              </div>
            </div>
          </div>
      ...
      <div className="flex items-center justify-between text-[10px] text-gray-500 dark:text-gray-400 border-t border-teal-100 dark:border-teal-900 pt-1.5">
        <span>Ziel: <span className="font-medium text-teal-900 dark:text-teal-200">≥ 90 %</span></span>

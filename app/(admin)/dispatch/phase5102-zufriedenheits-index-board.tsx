...
import { Smile, TrendingUp, TrendingDown, Minus, AlertTriangle } from 'lucide-react';
...
export function DispatchPhase5102ZufriedenheitsIndexBoard({ locationId }: { locationId: string | null }) {
  const [data, setData] = useState<ApiResponse | null>(null);

  async function load() {
    const url = locationId
      ? `/api/delivery/admin/fahrer-zufriedenheits-index-ranking?location_id=${locationId}`
      : '/api/delivery/admin/fahrer-zufriedenheits-index-ranking';
  ...
  const maxVal  = bester?.zufriedenheits_index ?? 100;
    ...
    <div className="rounded-2xl border border-emerald-700 bg-emerald-950/40 overflow-hidden mb-4">
      <div className="px-4 py-3 flex items-center justify-between border-b border-emerald-700/50 bg-emerald-900/20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-emerald-200">Zufriedenheits-Index je Fahrer (letzte 30 Tage)</span>
        </div>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 divide-x divide-emerald-800/40 border-b border-emerald-700/30">
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-emerald-300">{bester?.zufriedenheits_index ?? '–'}</div>
        ...
        <div className="px-3 py-2 text-center">
          ...
          <div className="text-sm font-bold text-emerald-200">{data.team_avg_index}</div>
          <div className="text-[10px] text-gray-500">Zufriedenheits-Score</div>
        </div>
        <div className="px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500 mb-0.5">Niedrigste</div>
          <div className="text-sm font-bold text-gray-400">{letzter?.zufriedenheits_index ?? '–'}</div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="w-20 h-1.5 rounded-full bg-gray-800 overflow-hidden shrink-0">
              <div
                ...
                style={{ width: `${maxVal > 0 ? Math.round((f.zufriedenheits_index / maxVal) * 100) : 0}%` }}
              />
            </div>
            <span className={`text-xs font-semibold w-8 text-right shrink-0 ${ampelColor(f.ampel)}`}>
              {f.zufriedenheits_index}
            </span>
      ...
      <div className="px-4 py-2 border-t border-emerald-800/30 bg-emerald-900/10">
        <div className="text-[10px] text-gray-500">
          Höchster Index: <span className="text-emerald-300 font-semibold">{data.beste_name}</span> ·

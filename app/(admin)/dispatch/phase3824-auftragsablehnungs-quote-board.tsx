...
export function DispatchPhase3824AuftragsablehnungsQuoteBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/delivery/admin/fahrer-auftragsablehnungs-quote?location_id=${locationId}`);
  ...
  const maxPct = Math.max(...data.fahrer.map(f => f.ablehnungsquote_pct), 1);
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-900 text-sm">Ablehnungsquote-Ranking</span>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-emerald-50 rounded-lg p-2">
          <div className="text-base font-black text-emerald-700">{data.fahrer[0]?.ablehnungsquote_pct}%</div>
        ...
        <div className={`rounded-lg p-2 ${data.alert_count > 0 ? 'bg-red-50' : 'bg-gray-50'}`}>
          <div className={`text-base font-black ${data.alert_count > 0 ? 'text-red-600' : 'text-gray-800'}`}>
            {data.fahrer[data.fahrer.length - 1]?.ablehnungsquote_pct}%
          </div>
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 bg-orange-50 border border-orange-200 rounded-lg text-xs text-orange-800">
          ...
          <span>{data.alert_count} Fahrer mit hoher Ablehnungsquote — Hohe Ablehnungsquote!</span>
        </div>
      )}
      ...
      <div className="space-y-2">
        {data.fahrer.map(f => {
          ...
          const barPct = (f.ablehnungsquote_pct / maxPct) * 100;
          return (
            <div key={f.fahrer_id} className={`rounded-lg border ${c.border} ${c.bg} p-2.5 space-y-1.5`}>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm font-black ${c.text}`}>{f.ablehnungsquote_pct}%</span>
                {f.rank_delta !== 0 && (
                  f.rank_delta < 0
              ...
              {f.alert_hoch && (
                <div className="text-[10px] text-orange-700 font-medium">Hohe Ablehnungsquote!</div>
              )}
            </div>
          );
        })}

...
export function DispatchPhase4628MittwochNachtBoard({ locationId }: { locationId: string | null }) {
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-sm">Mittwochnacht-Board nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-900 p-4 space-y-4">
      {/* Header */}
      <div className="flex items-center gap-2">
        <Moon className="w-5 h-5 text-slate-700 dark:text-slate-300" />
        <h3 className="font-semibold text-gray-900 dark:text-gray-100">Mittwochnacht-Ranking</h3>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-slate-50 dark:bg-slate-950 rounded-xl p-2">
          ...
          <p className="text-sm font-bold text-slate-700 dark:text-slate-300 truncate">{data.hoechste_name}</p>
        </div>
        <div className="bg-slate-50 dark:bg-slate-950 rounded-xl p-2">
          <p className="text-[10px] text-gray-500 dark:text-gray-400">Team-Ø</p>
          <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{data.team_avg}%</p>
        </div>
        <div className="bg-slate-50 dark:bg-slate-950 rounded-xl p-2">
          <p className="text-[10px] text-gray-500 dark:text-gray-400">Niedrigste</p>
          <p className="text-sm font-bold text-slate-700 dark:text-slate-300 truncate">{data.niedrigste_name}</p>
        </div>
      </div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-full h-2 overflow-hidden">
              <div
                ...
                style={{ width: `${Math.min(f.mittwoch_pct, 100)}%` }}
              />
            </div>
            ...
            {f.alert_niedrig && (
              <span className="text-[10px] text-red-500 shrink-0">Wenig Mittwochnacht!</span>
            )}
          </div>
        ))}
      </div>

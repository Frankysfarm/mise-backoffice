...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Julia F.', avg_stoppzeit_min: 3.8, avg_stoppzeit_min_vw: 4.2, trend: 'fallend',  trend_delta: -0.4, ampel: 'gruen', alert_zu_lang: false },
    { fahrer_id: 'd2', fahrer_name: 'Max M.',   avg_stoppzeit_min: 4.2, avg_stoppzeit_min_vw: 4.0, trend: 'steigend', trend_delta:  0.2, ampel: 'gruen', alert_zu_lang: false },
    { fahrer_id: 'd3', fahrer_name: 'Sara K.',  avg_stoppzeit_min: 7.1, avg_stoppzeit_min_vw: 6.5, trend: 'steigend', trend_delta:  0.6, ampel: 'gelb',  alert_zu_lang: false },
    { fahrer_id: 'd4', fahrer_name: 'Tim B.',   avg_stoppzeit_min: 12.5, avg_stoppzeit_min_vw: 11.0, trend: 'steigend', trend_delta: 1.5, ampel: 'rot',  alert_zu_lang: true  },
  ],
  team_avg_stoppzeit_min: 6.9,
  team_avg_stoppzeit_min_vw: 6.4,
  alert_count: 1,
};

export function DispatchPhase2910StoppzeitBoard({ locationId }: { locationId: string | null }) {
  ...
  const best      = sorted[0]?.avg_stoppzeit_min ?? 0;
  const teamAmpel = calcAmpel(data.team_avg_stoppzeit_min);
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Stoppzeit-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          ...
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 text-sm">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: {f.avg_stoppzeit_min.toFixed(1)} Min Stoppzeit zu lang!
              </span>
            </div>
          ))}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø', value: `${data.team_avg_stoppzeit_min.toFixed(1)} Min`, cls: teamText },
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.avg_stoppzeit_min.toFixed(1)} Min</span>
                    </div>
                  </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />5–10 Min</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />&gt;10 Min</span>
            <span className="flex items-center gap-1 ml-auto"><span className="w-0.5 h-3 bg-gray-600 dark:bg-gray-300 opacity-60 inline-block" />Ziel 5 Min</span>
          </div>
        </div>
      )}
    </div>

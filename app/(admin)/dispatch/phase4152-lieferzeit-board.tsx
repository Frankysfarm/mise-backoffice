...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; avg_min: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_top: boolean; }
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 1, avg_min: 18, rank_delta: -1, ampel: 'gruen', alert_top: false },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  rang: 2, avg_min: 22, rank_delta:  0, ampel: 'gruen', alert_top: false },
    { fahrer_id: 'f3', fahrer_name: 'Max M.',   rang: 3, avg_min: 28, rank_delta:  1, ampel: 'gelb',  alert_top: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, avg_min: 36, rank_delta:  0, ampel: 'rot',   alert_top: true  },
  ],
  team_avg: 26,
...
export function DispatchPhase4152LieferzeitBoard({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        {data.alert_count > 0 && (
          <span className="flex items-center gap-1 text-xs text-red-600 font-semibold bg-red-50 px-2 py-0.5 rounded-full">
            <AlertTriangle className="w-3.5 h-3.5" /> {data.alert_count} Hohe Lieferzeit!
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-teal-50 rounded-lg p-2">
          <p className="text-[10px] text-teal-600 font-medium">Schnellste</p>
          <p className="text-sm font-bold text-teal-700">{schnellster?.avg_min?.toFixed(0)} min</p>
          <p className="text-[10px] text-teal-500 truncate">{data.schnellster_name}</p>
        </div>
        ...
        <div className="bg-red-50 rounded-lg p-2">
          ...
          <p className="text-sm font-bold text-red-600">{langsamster?.avg_min?.toFixed(0)} min</p>
      ...
      <div className="space-y-1.5">
        {data.fahrer.map((f) => {
          ...
          // rank_delta < 0 = rang verbessert (kleiner = schneller) = grün
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.avg_min?.toFixed(0)} min</span>
              </div>
      ...
      <div className="flex justify-between text-[10px] text-gray-400 pt-1 border-t border-gray-100">
        <span>Team-Ø {data.team_avg?.toFixed(0)} min/Lieferung</span>

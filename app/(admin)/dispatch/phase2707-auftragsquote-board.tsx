...
function QuoteBalken({ q, barClass }: { q: number; barClass: string }) {
  const fill    = Math.min(100, (q / MAX) * 100);
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   quote: 4.2, touren_heute: 13, schicht_stunden: 3.1, trend: 'steigend', alert: null },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  quote: 3.1, touren_heute:  9, schicht_stunden: 2.9, trend: 'fallend',  alert: null },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   quote: 2.0, touren_heute:  6, schicht_stunden: 3.0, trend: 'stabil',   alert: null },
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', quote: 1.2, touren_heute:  4, schicht_stunden: 3.3, trend: 'fallend',  alert: 'Auftragsquote zu niedrig!' },
  ],
  team_avg_quote: 2.6,
};

export function DispatchPhase2707AuftragsquoteBoard({ locationId }: { locationId: string | null }) {
  ...
  const enriched  = data.fahrer.map((f: FahrerEntry) => ({ ...f, ampel: calcAmpel(f.quote) }));
  const sorted    = [...enriched].sort((a, b) => b.quote - a.quote);
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-4`}>
      ...
      >
        <div className="flex items-center gap-2">
          <Zap size={16} className={hasAlert ? 'text-red-500' : 'text-green-600'} />
          <span className="font-semibold text-sm text-gray-800">Auftragsquote</span>
          {hasAlert && (
          ...
          <span className={`ml-2 text-xs font-bold ${teamCls.text}`}>
            Ø {data.team_avg_quote.toFixed(1)}/h
          </span>
        </div>
        ...
        <div className="px-4 pb-4 space-y-3">
          {alerts.map((f: FahrerEntry & { ampel: string }) => (
            <div key={f.fahrer_id} className="flex items-center gap-2 rounded-lg bg-red-100 border border-red-200 px-3 py-2 text-xs text-red-700 font-medium">
              ...
              <span className="font-bold">{f.fahrer_name}</span>: {f.alert} ({f.quote.toFixed(1)}/h)
            </div>
          ))}

          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø', value: `${data.team_avg_quote.toFixed(1)}/h`, cls: teamCls.text },
              { label: 'Bester', value: bester ? `${bester.quote.toFixed(1)}/h` : '–',            cls: 'text-green-600' },
          ...
          <div className="flex items-center gap-3 text-xs text-gray-500">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 inline-block" /> ≥3/h</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" /> 1.5–2.9/h</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500  inline-block" /> &lt;1.5/h</span>
          </div>
                ...
                <div key={f.fahrer_id} className={`rounded-lg border ${cls.bg} px-3 py-2`}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`text-xs font-bold ${cls.text}`}>{f.quote.toFixed(1)}/h</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <QuoteBalken q={f.quote} barClass={cls.bar} />

...
// Star yellow-400; avg_rating ABSTEIGEND Rang 1=höchste Bewertung=bester;
// 3-KPI-Grid Beste/r/Team-Ø/Niedrigste/r; Balken farbkodiert grün/gelb/rot;
// DeltaIcons TrendingUp/Down/Minus; Niedrig-Alert alert_niedrig; 30-Min-Polling; Mock-Fallback
...
const MOCK: ApiResponse = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 1, avg_rating: 4.9, rank_delta:  1, ampel: 'gruen', alert_niedrig: false },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  rang: 2, avg_rating: 4.6, rank_delta:  0, ampel: 'gruen', alert_niedrig: false },
    { fahrer_id: 'f3', fahrer_name: 'Max M.',   rang: 3, avg_rating: 4.1, rank_delta: -1, ampel: 'gelb',  alert_niedrig: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, avg_rating: 3.5, rank_delta:  0, ampel: 'rot',   alert_niedrig: true  },
  ],
...
export function DispatchPhase5413BewertungsBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxRating = Math.max(...data.fahrer.map(f => f.avg_rating), 0.01);
    ...
    <div className="rounded-xl bg-gray-900 border border-gray-700/50 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-white">Bewertungs-Ranking</span>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-gray-800 rounded-lg p-2 text-center">
          ...
          <div className="text-xs font-bold text-emerald-400 truncate">{data.bester_name}</div>
        </div>
        <div className="bg-gray-800 rounded-lg p-2 text-center">
          <div className="text-[10px] text-gray-400">Team-Ø</div>
          <div className="text-xs font-bold text-gray-200">★ {data.team_avg_rating.toFixed(2)}</div>
        </div>
        <div className="bg-gray-800 rounded-lg p-2 text-center">
      ...
      <div className="space-y-1.5">
        {data.fahrer.map(f => (
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono text-gray-300 w-12 text-right">★ {f.avg_rating.toFixed(2)}</span>
            <div className="w-20 bg-gray-800 rounded-full h-1.5 shrink-0">
              <div
                className="h-1.5 rounded-full transition-all"
                style={{ width: `${(f.avg_rating / maxRating) * 100}%`, backgroundColor: ampelColor(f.ampel) }}
              />
            </div>
      ...
      <div className="text-[10px] text-gray-600 text-right">Ø Kundenbewertung · 30-Min-Update · {data.gesamt} Fahrer</div>
    </div>
  );
}

...
// Phase 5622 — Gesamtleistungs-Score-Trend-Board (Dispatch) — Batch 100 Milestone
// Trophy violet; score_delta ABSTEIGEND Rang 1=größte Verbesserung=bester;
...
const MOCK: ApiResponse = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 1, score_delta:  8.4, aktuell_score: 87.2, rank_delta:  2, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f2', fahrer_name: 'Max M.',   rang: 2, score_delta:  3.1, aktuell_score: 82.5, rank_delta:  0, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, score_delta:  0.7, aktuell_score: 76.3, rank_delta: -1, ampel: 'gelb',  alert_rueckfall: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, score_delta: -4.2, aktuell_score: 68.1, rank_delta: -1, ampel: 'rot',   alert_rueckfall: true  },
  ],
  team_avg_delta: 2.0,
  bester_name: 'Julia F.',
...
function barColor(delta: number): string {
  if (delta > 0)  return '#a78bfa'; // violet — improved
...
export function DispatchPhase5622GesamtleistungsTrendBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxAbs   = Math.max(...data.fahrer.map(f => Math.abs(f.score_delta)), 1);
    ...
    <div className={`rounded-lg bg-gray-900 border ${data.alert_count > 0 ? 'border-red-700/60' : 'border-violet-700/40'} p-3 space-y-2.5`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-white">Gesamtleistungs-Trend</span>
      ...
      <div className="grid grid-cols-3 gap-1.5 text-center">
        <div className="rounded bg-gray-800 px-2 py-1.5">
          ...
          <div className="text-xs font-bold text-violet-400">{bester ? fmtDelta(bester.score_delta) : '—'}</div>
        ...
        <div className="rounded bg-gray-800 px-2 py-1.5">
          ...
          <div className="text-xs font-bold text-red-400">{schwaech ? fmtDelta(schwaech.score_delta) : '—'}</div>
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 h-1.5 rounded-full bg-gray-800">
              <div
                ...
                style={{ width: `${(Math.abs(f.score_delta) / maxAbs) * 100}%`, backgroundColor: barColor(f.score_delta) }}
              />
            </div>
            <span className="text-[10px] font-mono text-violet-300 w-10 text-right">{f.aktuell_score.toFixed(1)}</span>
            <span className="text-[10px] font-mono text-gray-400 w-10 text-right">{fmtDelta(f.score_delta)}</span>

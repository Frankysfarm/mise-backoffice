...
export function DispatchPhase3360KmProStoppRankingBoard({ locationId }: { locationId: string | null }) {
  ...
  const maxKm   = Math.max(...list.map(f => f.km_pro_stopp), 0.1);
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <Route size={10} className="text-blue-600" /> Bester
              </div>
              <div className="font-bold text-sm text-blue-700 dark:text-blue-400">{bester?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{bester ? `${bester.km_pro_stopp.toFixed(2)} km/Stopp` : '—'}</div>
            </div>
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5">Team-Ø</div>
              <div className="font-bold text-sm text-gray-700 dark:text-gray-200">{d.team_avg.toFixed(2)}</div>
              <div className="text-xs text-gray-400">km/Stopp</div>
            </div>
            <div className="rounded-lg bg-red-50 dark:bg-red-900/20 p-2">
              ...
              <div className="font-bold text-sm text-red-600 dark:text-red-400">{letzter?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{letzter ? `${letzter.km_pro_stopp.toFixed(2)} km/Stopp` : '—'}</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {list.map(f => {
              ...
              const barW = Math.round((f.km_pro_stopp / maxKm) * 100);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className={`text-sm font-bold ${cls.text}`}>{f.km_pro_stopp.toFixed(2)} km/S</span>
                      {f.rank_delta < 0
          ...
          <div className="pt-1 border-t border-gray-100 dark:border-gray-700 flex justify-between text-xs text-gray-400">
            <span>Team-Ø: {d.team_avg.toFixed(2)} km/Stopp · heute</span>
            <span>Ziel: &lt;2.0 km/Stopp</span>
          </div>
        </div>
      )}
    </div>

...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; touren_pro_stunde: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_bottom: boolean; }
...
export function DispatchPhase4087TourenProStundeBoard({ locationId }: Props) {
  ...
  const maxRate = Math.max(...data.fahrer.map(f => f.touren_pro_stunde), 1);
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Touren/h-Ranking</span>
        </div>
        <div className="flex items-center gap-2">
          {loading && <span className="w-3 h-3 border-2 border-gray-300 border-t-transparent rounded-full animate-spin" />}
          {!loading && data.alert_count > 0 && (
            <span className="flex items-center gap-1 text-xs text-red-600 font-semibold">
              <AlertTriangle className="w-3.5 h-3.5" /> {data.alert_count} Niedrige Touren/h!
            </span>
          )}
        </div>
      </div>
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-green-50 rounded-lg p-2">
          ...
          <p className="text-sm font-bold text-green-700">{bester?.touren_pro_stunde?.toFixed(1)} /h</p>
        ...
        <div className="bg-red-50 rounded-lg p-2">
          ...
          <p className="text-sm font-bold text-red-600">{letzter?.touren_pro_stunde?.toFixed(1)} /h</p>
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.touren_pro_stunde?.toFixed(1)} /h</span>
              </div>
              <div className="ml-6 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${barColor}`} style={{ width: `${(f.touren_pro_stunde / maxRate) * 100}%` }} />
              </div>
            </div>
          );
        })}

...
export function DispatchPhase5259AkzeptanzrateBoard({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-sky-700 bg-sky-900/30 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-gray-200">Akzeptanzrate-Ranking (letzte 30 Tage)</span>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-lg bg-sky-900/50 px-3 py-2 text-center">
          ...
          <div className="text-xs font-bold text-sky-300 truncate">{data.beste_name}</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          <div className="text-[10px] text-gray-500">Team-Ø</div>
          <div className="text-xs font-bold text-gray-200">{data.team_avg_akzeptanz.toFixed(0)}%</div>
        </div>
        <div className="rounded-lg bg-gray-800/50 px-3 py-2 text-center">
          ...
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <div className="flex-1 h-2 rounded-full bg-gray-800 overflow-hidden">
              <div
                ...
                style={{ width: `${f.akzeptanz_rate}%` }}
              />
            </div>
            <span className="text-xs text-gray-400 w-10 text-right tabular-nums">
              {f.akzeptanz_rate.toFixed(0)}%
            </span>
      ...
      {data.beste_name && (
        <div className="mt-3 text-[10px] text-sky-400 border-t border-sky-800/40 pt-2">
          Beste: {data.beste_name} — {data.fahrer[0]?.akzeptanz_rate.toFixed(0)}% Akzeptanzrate
        </div>
      )}
    </div>
  );

...
// Normal: steigend (mehr Umsatz/h) = grün = gut
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   umsatz_pro_stunde: 32.5, trend: 'steigend', trend_delta:  2.5 },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  umsatz_pro_stunde: 27.0, trend: 'stabil',   trend_delta:  0.0 },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   umsatz_pro_stunde: 18.0, trend: 'fallend',  trend_delta: -3.0 },
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', umsatz_pro_stunde: 11.5, trend: 'fallend',  trend_delta: -2.0 },
  ],
  team_avg_uph: 22.3,
  alert_count: 1,
};

export function DispatchPhase2981UmsatzProStundeBoard({ locationId }: { locationId: string | null }) {
  ...
  const enriched = data.fahrer.map(f => ({ ...f, ampel: calcAmpel(f.umsatz_pro_stunde) }));
  const sorted   = [...enriched].sort((a, b) => b.umsatz_pro_stunde - a.umsatz_pro_stunde);
  const alerts   = enriched.filter(f => f.umsatz_pro_stunde < ALERT_UPH);
  const hasAlert = alerts.length > 0;
  const best     = sorted[0]?.umsatz_pro_stunde ?? 0;
  const teamAmpel = calcAmpel(data.team_avg_uph);
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Umsatz/h-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø', value: `${data.team_avg_uph.toFixed(1)} €/h`, cls: teamText },
              { label: 'Bester', value: `${best.toFixed(1)} €/h`,              cls: 'text-green-600' },
              { label: 'Ziel',   value: `≥${ZIEL_UPH} €/h`,                   cls: 'text-gray-600 dark:text-gray-400' },
            ].map(k => (
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.umsatz_pro_stunde.toFixed(1)} €/h</span>
                    </div>
                  </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 inline-block" />≥25 €/h</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />15–24 €/h</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />&lt;15 €/h</span>
            <span className="text-gray-400">| Ziel ≥{ZIEL_UPH} €/h</span>
          </div>
        </div>
      )}
    </div>

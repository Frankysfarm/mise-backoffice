...
export function DispatchPhase4608SonntagAbendBoard({ locationId }: { locationId: string | null }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const res = await fetch(`/api/delivery/admin/fahrer-sonntag-abend-ranking${params}`);
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-sm">Sonntagabend-Board nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-purple-200 dark:border-purple-800 bg-white dark:bg-gray-900 p-4 space-y-4">
      {/* Header */}
      <div className="flex items-center gap-2">
        <Star className="w-5 h-5 text-purple-500" />
        <h3 className="font-semibold text-gray-900 dark:text-gray-100">Sonntagabend-Ranking</h3>
        {data.alert_count > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-purple-50 dark:bg-purple-950 rounded-xl p-2">
          ...
          <p className="text-sm font-bold text-purple-600 dark:text-purple-400 truncate">{data.hoechste_name}</p>
        </div>
        <div className="bg-purple-50 dark:bg-purple-950 rounded-xl p-2">
          <p className="text-[10px] text-gray-500 dark:text-gray-400">Team-Ø</p>
          <p className="text-sm font-bold text-purple-600 dark:text-purple-400">{data.team_avg}%</p>
        </div>
        <div className="bg-purple-50 dark:bg-purple-950 rounded-xl p-2">
          <p className="text-[10px] text-gray-500 dark:text-gray-400">Niedrigste</p>
          <p className="text-sm font-bold text-purple-600 dark:text-purple-400 truncate">{data.niedrigste_name}</p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {data.fahrer.map((f) => (
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            {f.alert_niedrig && (
              <span className="text-[10px] text-red-500 shrink-0">Wenig Sonntagabend!</span>
            )}
          </div>
        ))}
      </div>

...
import { AlertTriangle, CheckSquare, ChevronDown, ChevronUp, Minus, TrendingDown, TrendingUp } from 'lucide-react';
...
export function DispatchPhase3136LieferzeitPuenktlichkeitBoard({ locationId }: { locationId: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-lieferzeit-puenktlichkeit-ranking?location_id=${locationId}`)
        .then(r => r.json())
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">
            Lieferzeit-Pünktlichkeits-Ranking heute
          </span>
        ...
        <div className="p-4 space-y-3">
          {alerts.length > 0 && (
            <div className="flex items-center gap-2 rounded-lg bg-red-50 border border-red-200 text-red-700 dark:text-red-300 dark:bg-red-950 dark:border-red-800 text-xs font-semibold px-3 py-2">
              ...
              {alerts.map(f => f.fahrer_name).join(', ')} — Niedrigste Lieferzeit-Pünktlichkeit!
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-yellow-50 dark:bg-yellow-900/20 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5 flex items-center justify-center gap-1">
                <CheckSquare size={10} className="text-green-500" /> Bester
              </div>
              <div className="font-bold text-sm text-yellow-700 dark:text-yellow-400">{bester?.fahrer_name ?? '—'}</div>
              <div className="text-xs text-gray-500">{bester ? `${bester.pct}%` : '—'}</div>
            </div>
            ...
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="text-xs text-gray-500">{letzter ? `${letzter.pct}%` : '—'}</div>
            </div>
          </div>
                ...
                <div key={f.fahrer_id} className={`rounded-lg border p-3 ${cls.bg}`}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className={`font-bold text-sm ${cls.text}`}>{f.pct}%</span>
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
                    <div className={`h-1.5 rounded-full transition-all ${cls.bar}`} style={{ width: `${Math.max(f.pct, 5)}%` }} />
                  </div>
                  {f.alert_bottom && (
                    <div className="mt-1 text-xs text-red-600 dark:text-red-400 font-semibold flex items-center gap-1">
                      <AlertTriangle size={10} /> Niedrigste Lieferzeit-Pünktlichkeit!
                    </div>
                  )}
                </div>
              );
          ...
          <div className="flex flex-wrap gap-3 text-xs text-gray-500 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500" /> Top 25% (pünktlichste)</span>

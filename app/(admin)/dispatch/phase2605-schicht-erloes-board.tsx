...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f2', fahrer_name: 'Sarah K.',  erloes_heute: 78.30,  erloes_gestern: 165.40, trend: 'schlechter', trend_delta: -87.10, ampel: 'rot',   alert: true  },
    { fahrer_id: 'f5', fahrer_name: 'Jana F.',   erloes_heute: 62.40,  erloes_gestern: 183.70, trend: 'schlechter', trend_delta: -121.30,ampel: 'rot',   alert: true  },
    { fahrer_id: 'f3', fahrer_name: 'Lena S.',   erloes_heute: 142.60, erloes_gestern: 138.90, trend: 'stabil',     trend_delta: 3.70,   ampel: 'gelb',  alert: false },
    { fahrer_id: 'f4', fahrer_name: 'Tom B.',    erloes_heute: 215.80, erloes_gestern: 229.10, trend: 'stabil',     trend_delta: -13.30, ampel: 'gruen', alert: false },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',    erloes_heute: 248.50, erloes_gestern: 211.20, trend: 'besser',     trend_delta: 37.30,  ampel: 'gruen', alert: false },
  ],
  team_avg_heute: 149.52, team_avg_gestern: 185.66, ziel: 200, alert_count: 2,
};

export function DispatchPhase2605SchichtErloesBoard({ locationId }: { locationId: string | null }) {
  ...
  const teamAmpel  = data.team_avg_heute >= data.ziel ? 'gruen' : data.team_avg_heute >= 100 ? 'gelb' : 'rot';
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-4`}>
      ...
      >
        <div className="flex items-center gap-2">
          <Euro size={16} className={hasAlert ? 'text-red-500' : 'text-green-600'} />
          <span className="font-semibold text-sm text-gray-800">Schicht-Erlös-Board</span>
          {hasAlert && (
        ...
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className={`text-lg font-bold ${teamCls.text}`}>{data.team_avg_heute.toFixed(0)} €</div>
              <div className="text-xs text-gray-500">Team Ø heute</div>
            </div>
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className="text-lg font-bold text-gray-700">
                {data.team_avg_gestern !== null ? `${data.team_avg_gestern.toFixed(0)} €` : '—'}
              </div>
            ...
            {sorted.map(f => {
              ...
              return (
                <div key={f.fahrer_id} className={`flex items-center gap-2 rounded-lg border px-2.5 py-1.5 ${cls.bg}`}>
                  ...
                  <ErloesBalken erloes={f.erloes_heute} ziel={data.ziel} barClass={cls.bar} />
                  <span className={`text-xs font-bold w-16 text-right ${cls.text}`}>{f.erloes_heute.toFixed(0)} €</span>
                  <TrendIcon trend={f.trend} />
                  <span className="text-xs text-gray-400 w-12 text-right">
                    {f.trend_delta > 0 ? '+' : ''}{f.trend_delta.toFixed(0)} €
                  </span>
                </div>
              );
            })}
          ...
          <div className="flex gap-3 text-xs text-gray-400">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />100–{data.ziel - 1} €</span>

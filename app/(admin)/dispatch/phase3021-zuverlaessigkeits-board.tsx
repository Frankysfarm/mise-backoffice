...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',  abschluss_pct: 98, lieferungen_heute: 20, trend: 'steigend', trend_delta:  2, ampel: 'gruen', alert_niedrig: false },
    { fahrer_id: 'f2', fahrer_name: 'Lisa F.', abschluss_pct: 95, lieferungen_heute: 18, trend: 'steigend', trend_delta:  1, ampel: 'gruen', alert_niedrig: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.', abschluss_pct: 88, lieferungen_heute: 15, trend: 'fallend',  trend_delta: -3, ampel: 'gelb',  alert_niedrig: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',  abschluss_pct: 80, lieferungen_heute: 12, trend: 'fallend',  trend_delta: -5, ampel: 'rot',   alert_niedrig: true  },
  ],
  team_durchschnitt: 90.3,
  alert_count: 1,
};

export function DispatchPhase3021ZuverlaessigkeitsBoard({ locationId }: { locationId: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-lieferzuverlaessigkeit?location_id=${locationId}`)
        .then(r => r.json())
  ...
  const sorted   = [...data.fahrer].sort((a, b) => b.abschluss_pct - a.abschluss_pct);
  const alerts   = data.fahrer.filter(f => f.alert_niedrig);
  const hasAlert = alerts.length > 0;
  const best     = sorted[0]?.abschluss_pct ?? 0;
  const teamAmpel = data.team_durchschnitt >= ZIEL_PCT ? 'gruen' : data.team_durchschnitt >= ALERT_PCT ? 'gelb' : 'rot';
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Zuverlässigkeits-Board</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 text-sm">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: Geringe Zuverlässigkeit! ({f.abschluss_pct.toFixed(1)}%)
              </span>
            </div>
          ))}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Team-Ø', value: `${data.team_durchschnitt.toFixed(1)}%`, cls: teamText },
          ...
          <div className="space-y-2">
            {sorted.map(f => {
              ...
              const pct = Math.min(100, f.abschluss_pct);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs text-gray-400">({f.lieferungen_heute} Lief.)</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <TrendIcon trend={f.trend} />
                      <span className={`text-xs font-bold ${text}`}>{f.abschluss_pct.toFixed(1)}%</span>
                    </div>
                  </div>
          ...
          <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 pt-1">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 inline-block" />≥95%</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />85–94%</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />&lt;85%</span>

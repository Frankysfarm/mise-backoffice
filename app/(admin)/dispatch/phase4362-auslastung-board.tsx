...
export function DispatchPhase4362AuslastungBoard({ locationId }: Props) {
    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-gray-800">Fahrer-Auslastung</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          {!loading && data.alert_count > 0 && (
            <span className="flex items-center gap-0.5 text-[10px] text-red-600 font-semibold bg-red-50 px-2 py-0.5 rounded-full">
              <AlertTriangle className="w-3 h-3" /> {data.alert_count} Niedrige Auslastung
            </span>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-sky-50 rounded-lg p-2">
          <div className="text-[10px] text-sky-600 font-medium">Aktivste</div>
          <div className="text-sm font-bold text-sky-700">{bester?.auslastung_pct.toFixed(1)} %</div>
        ...
        <div className="bg-red-50 rounded-lg p-2">
          <div className="text-[10px] text-red-500 font-medium">Inaktivste</div>
          <div className="text-sm font-bold text-red-600">{letzter?.auslastung_pct.toFixed(1)} %</div>
      ...
      <div className="space-y-1.5">
        {data.fahrer.map((f) => {
          // STANDARD rank_delta: rang - prevRang; <0 = verbessert = TrendingUp emerald
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-2">
                ...
                {delta}
                <span className="text-[11px] font-bold text-gray-900 w-14 text-right">{f.auslastung_pct.toFixed(1)} %</span>
              ...
              <div className="ml-7 h-1 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${barColor}`} style={{ width: `${(f.auslastung_pct / maxPct) * 100}%` }} />
              </div>
            </div>
          );
        })}
      ...
      <div className="text-[10px] text-gray-400 text-right">#1 = höchste Auslastung (Anteil aktiver Schichtzeit)</div>
    </div>
  );
}

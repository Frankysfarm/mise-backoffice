...
export function DispatchPhase4503TourzeitBoard({ locationId }: Props) {
  const [data, setData] = useState<TourzeitData | null>(null);
  const [loading, setLoading] = useState(true);

  const fetchData = useCallback(async () => {
    const params = locationId ? `?location_id=${locationId}` : '';
    const res = await fetch(`/api/delivery/admin/fahrer-tourzeit-ranking${params}`, { cache: 'no-store' });
  ...
  // INVERTED: kürzeste Zeit = bester → Balken relativ zu maxMin
  const maxMin = worst?.avg_tourzeit_min ?? 1;

  const ampelColor = (a: FahrerTourzeit['ampel']) =>
    a === 'gruen' ? 'text-teal-600 dark:text-teal-400' :
    a === 'rot'   ? 'text-red-500 dark:text-red-400' : 'text-yellow-500 dark:text-yellow-400';

  const ampelBg = (a: FahrerTourzeit['ampel']) =>
    a === 'gruen' ? 'bg-teal-500' : a === 'rot' ? 'bg-red-500' : 'bg-yellow-400';

  // INVERTED: rank_delta>0 bedeutet besser geworden (kleinere Zahl) → TrendingUp emerald
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900 dark:text-gray-100 text-sm">Tourzeit-Ranking (30 Tage)</h3>
        <span className="ml-auto text-xs text-gray-400">Ø Minuten/Tour</span>
      </div>

      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="rounded-lg bg-teal-50 dark:bg-teal-900/20 p-2">
          <div className="text-lg font-bold text-teal-600 dark:text-teal-400">{best?.avg_tourzeit_min.toFixed(0)} min</div>
          <div className="text-xs text-gray-500 dark:text-gray-400">Schnellste ({best?.fahrer_name})</div>
        </div>
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          <div className="text-lg font-bold text-gray-700 dark:text-gray-300">{data.team_avg_min.toFixed(0)} min</div>
          <div className="text-xs text-gray-500 dark:text-gray-400">Team-Avg</div>
        </div>
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          <div className="text-lg font-bold text-gray-500 dark:text-gray-400">{worst?.avg_tourzeit_min.toFixed(0)} min</div>
      ...
      {alerts.length > 0 && (
        <div className="flex items-center gap-2 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 px-3 py-2">
          ...
          <span className="text-xs text-red-700 dark:text-red-300">
            Hohe Tourzeit (&gt;90 min): {alerts.map(a => a.fahrer_name).join(', ')}
          </span>
        </div>
      )}
        ...
        {data.fahrer.map((f) => (
          <div key={f.fahrer_id} className="space-y-1">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className={`font-semibold text-xs ${ampelColor(f.ampel)}`}>{f.avg_tourzeit_min.toFixed(0)} min</span>
            </div>
            <div className="h-1.5 rounded-full bg-gray-100 dark:bg-gray-800">
              <div
                className={`h-full rounded-full ${ampelBg(f.ampel)}`}
                style={{ width: `${maxMin > 0 ? Math.round((f.avg_tourzeit_min / maxMin) * 100) : 0}%` }}
              />
            </div>
          </div>
        ))}
      ...
      <div className="text-xs text-gray-400">Ziel: ≤60 min · {data.gesamt} Fahrer</div>
    </div>
  );
}

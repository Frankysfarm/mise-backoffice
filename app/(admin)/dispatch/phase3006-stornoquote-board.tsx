...
function TrendIcon({ trend }: { trend: string }) {
  // INVERTIERT: fallend=grün (Quote sinkt = besser), steigend=rot (Quote steigt = schlechter)
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   quote_pct:  0.0, stornierungen: 0, gesamt_auftraege: 14, trend: 'fallend',  trend_delta: -3.0, ampel: 'gruen' },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  quote_pct:  0.0, stornierungen: 0, gesamt_auftraege: 11, trend: 'fallend',  trend_delta: -5.0, ampel: 'gruen' },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   quote_pct: 11.1, stornierungen: 1, gesamt_auftraege:  9, trend: 'steigend', trend_delta:  3.1, ampel: 'gelb'  },
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', quote_pct: 16.7, stornierungen: 2, gesamt_auftraege: 12, trend: 'steigend', trend_delta:  6.7, ampel: 'rot'   },
  ],
  team_durchschnitt: 7.0,
  alert_count: 1,
};

export function DispatchPhase3006StornoquoteBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Stornoquote-Board</span>
        ...
        <div className="px-4 py-3 space-y-3">
          ...
          <div className="space-y-2">
            {sorted.map(f => {
              ...
              const pct = Math.min(100, (f.quote_pct / MAX_BAR) * 100);
              return (
                <div key={f.fahrer_id} className={`rounded-lg border p-2 ${bg} dark:bg-transparent dark:border-gray-700`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs font-bold ${text}`}>{f.quote_pct.toFixed(1)}%</span>
                    </div>
                  </div>

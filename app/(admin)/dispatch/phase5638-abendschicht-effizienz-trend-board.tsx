...
// Phase 5638 — Abendschicht-Effizienz-Trend-Board (Dispatch) — Batch 104
...
const MOCK: ApiResponse = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   rang: 1, effizienz_delta:  0.6, aktuell_touren_pro_std: 3.2, rank_delta:  1, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', rang: 2, effizienz_delta:  0.3, aktuell_touren_pro_std: 2.9, rank_delta:  0, ampel: 'gruen', alert_rueckfall: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, effizienz_delta: -0.1, aktuell_touren_pro_std: 2.4, rank_delta: -1, ampel: 'gelb',  alert_rueckfall: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, effizienz_delta: -0.5, aktuell_touren_pro_std: 2.0, rank_delta: -1, ampel: 'rot',   alert_rueckfall: true  },
  ],
...
function barColor(delta: number): string {
  if (delta > 0)   return '#60a5fa'; // blue-400
  if (delta === 0) return '#fbbf24'; // yellow
  return '#f87171'; // red
}

function DeltaIcon({ d }: { d: number }) {
  if (d > 0) return <TrendingUp  className="h-3 w-3 text-blue-400" />;
...
export function DispatchPhase5638AbendschichtEffizienzTrendBoard({ locationId }: { locationId: string | null }) {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-abendschicht-effizienz-trend-ranking?location_id=${locationId}`);
  ...
  return (
    <div className={`rounded-lg bg-gray-900 border ${data.alert_count > 0 ? 'border-red-700/60' : 'border-blue-700/40'} p-3 space-y-2.5`}>
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          <Moon className="h-3.5 w-3.5 text-blue-400" />
          <span className="text-xs font-semibold text-white">Abendschicht-Effizienz</span>
      ...
      <div className="grid grid-cols-3 gap-1.5 text-center">
        <div className="bg-gray-800 rounded p-1.5">
          ...
          <div className="text-xs font-semibold text-blue-400 truncate">{data.bester_name}</div>
        ...
        <div className="bg-gray-800 rounded p-1.5">
          ...
          <div className={`text-xs font-mono font-semibold ${data.team_avg_delta > 0 ? 'text-blue-400' : data.team_avg_delta < 0 ? 'text-red-400' : 'text-gray-400'}`}>
      ...
      <div className="space-y-1">
        {data.fahrer.map(f => (
          <div key={f.fahrer_id} className={`flex items-center gap-2 rounded px-2 py-1 ${f.alert_rueckfall ? 'bg-red-900/20' : 'bg-gray-800/50'}`}>
            ...
            <span className={`text-[10px] font-mono w-14 text-right ${f.effizienz_delta > 0 ? 'text-blue-400' : f.effizienz_delta < 0 ? 'text-red-400' : 'text-gray-400'}`}>
      ...
      <div className="text-[9px] text-gray-600 text-right">17–22h · 30-Tage-Trend · Touren/Std</div>
    </div>
  );
}

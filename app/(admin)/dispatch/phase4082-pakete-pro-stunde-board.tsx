...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; pakete_pro_stunde: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_bottom: boolean; }
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 1, pakete_pro_stunde: 8.2, rank_delta:  1, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 2, pakete_pro_stunde: 7.1, rank_delta:  0, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f2', fahrer_name: 'Max M.',   rang: 3, pakete_pro_stunde: 5.8, rank_delta: -1, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, pakete_pro_stunde: 4.2, rank_delta:  0, ampel: 'rot',   alert_bottom: true  },
  ],
  team_avg: 6.3,
  bester_name: 'Julia F.',
  letzter_name: 'Tim B.',
...
export function DispatchPhase4082PaketeProStundeBoard({ locationId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/delivery/admin/fahrer-pakete-pro-stunde-ranking?location_id=${locationId}`);
  ...
  const maxRate = Math.max(...data.fahrer.map(f => f.pakete_pro_stunde), 1);
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Pakete/h-Ranking</span>
        </div>
        <div className="flex items-center gap-2">
          {loading && <span className="w-3 h-3 border-2 border-gray-300 border-t-transparent rounded-full animate-spin" />}
          {!loading && data.alert_count > 0 && (
            <span className="flex items-center gap-1 text-xs text-red-600 font-semibold">
              <AlertTriangle className="w-3.5 h-3.5" /> {data.alert_count} Niedrige Pakete/h!
            </span>
          )}
        </div>
      </div>
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-orange-50 rounded-lg p-2">
          ...
          <p className="text-sm font-bold text-orange-700">{bester?.pakete_pro_stunde?.toFixed(1)} /h</p>
        ...
        <div className="bg-gray-50 rounded-lg p-2">
          ...
          <p className="text-[10px] text-gray-400">Rang 1 = höchste</p>
        </div>
        <div className="bg-red-50 rounded-lg p-2">
          <p className="text-[10px] text-gray-500 mb-0.5">Niedrigste Rate</p>
          <p className="text-sm font-bold text-red-600">{letzter?.pakete_pro_stunde?.toFixed(1)} /h</p>
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.pakete_pro_stunde?.toFixed(1)} /h</span>
              </div>
              <div className="ml-6 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${barColor}`} style={{ width: `${(f.pakete_pro_stunde / maxRate) * 100}%` }} />
              </div>
            </div>
          );
        })}

...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; km_pro_stopp: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_top: boolean; }
...
export function DispatchPhase4137KmProStoppBoard({ locationId }: Props) {
  ...
  const maxKm = Math.max(...data.fahrer.map(f => f.km_pro_stopp), 0.01);
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">km pro Stopp</span>
        ...
        {data.alert_count > 0 && (
          <span className="flex items-center gap-1 text-xs text-red-600 font-semibold bg-red-50 px-2 py-0.5 rounded-full">
            <AlertTriangle className="w-3.5 h-3.5" /> {data.alert_count} Hohe km/Stopp!
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-blue-50 rounded-lg p-2">
          <p className="text-[10px] text-blue-600 font-medium">Effizienteste</p>
          <p className="text-sm font-bold text-blue-700">{bester?.km_pro_stopp?.toFixed(1)} km</p>
          <p className="text-[10px] text-blue-500 truncate">{data.bester_name}</p>
        </div>
        ...
        <div className="bg-red-50 rounded-lg p-2">
          ...
          <p className="text-sm font-bold text-red-600">{letzter?.km_pro_stopp?.toFixed(1)} km</p>
      ...
      <div className="space-y-1.5">
        {data.fahrer.map((f) => {
          const barW = maxKm > 0 ? (f.km_pro_stopp / maxKm) * 100 : 0;
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${tColor}`}>{f.km_pro_stopp?.toFixed(1)} km</span>
              </div>
      ...
      <div className="flex justify-between text-[10px] text-gray-400 pt-1 border-t border-gray-100">
        <span>Team-Ø {data.team_avg?.toFixed(1)} km/Stopp</span>
        <span>#1 = niedrigste km/Stopp</span>
      </div>
    </div>
  );
}

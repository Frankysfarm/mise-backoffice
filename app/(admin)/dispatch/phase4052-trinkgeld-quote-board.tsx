...
export function DispatchPhase4052TrinkgeldQuoteBoard({ locationId }: Props) {
  ...
  const maxScore = Math.max(...data.fahrer.map(f => f.trinkgeld_quote), 1);
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Trinkgeld-Ranking</span>
        </div>
        <div className="flex items-center gap-2">
          {loading && <span className="w-3 h-3 border-2 border-gray-300 border-t-transparent rounded-full animate-spin" />}
          {!loading && data.alert_count > 0 && (
            <span className="flex items-center gap-1 text-xs text-red-600 font-semibold">
              <AlertTriangle className="w-3.5 h-3.5" /> {data.alert_count} Niedrige Trinkgeld-Quote!
            </span>
          )}
        </div>
      </div>
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-yellow-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-yellow-600 font-medium">Höchste Quote</div>
          <div className="text-xs font-bold text-yellow-700 truncate">{data.bester_name ?? bester?.fahrer_name ?? '–'}</div>
          <div className="text-[10px] text-yellow-500">{bester?.trinkgeld_quote?.toFixed(1) ?? '0.0'} %</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-2 text-center">
          ...
          <div className="text-[10px] text-gray-400">Trinkgeld-Quote</div>
        </div>
        <div className="bg-red-50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-red-500 font-medium">Niedrigste Quote</div>
          <div className="text-xs font-bold text-red-700 truncate">{data.letzter_name ?? letzter?.fahrer_name ?? '–'}</div>
          <div className="text-[10px] text-red-400">{letzter?.trinkgeld_quote?.toFixed(1) ?? '0.0'} %</div>
        </div>
      </div>
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              ...
              <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full ${barColor} rounded-full transition-all`} style={{ width: `${(f.trinkgeld_quote / maxScore) * 100}%` }} />
              </div>
              <span className={`text-xs font-bold w-12 text-right ${tColor}`}>{f.trinkgeld_quote?.toFixed(1)} %</span>
              {DeltaIcon}
            </div>
          );
        })}

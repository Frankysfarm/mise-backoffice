...
// Phase 5426 — km-pro-Tour-Board (Dispatch)
// Route green-400; km_avg AUFSTEIGEND Rang 1=niedrigste km=kürzeste Route=effizientester=bester;
// 3-KPI-Grid Kürzeste/r/Team-Ø/Längste/r; Balken farbkodiert grün/gelb/rot;
// DeltaIcons TrendingUp/Down/Minus; Hoch-Alert alert_top; 30-Min-Polling; Mock-Fallback
...
export function DispatchPhase5426KmProTourBoard({ locationId }: { locationId: string | null }) {
    ...
    <div className="rounded-xl bg-gray-900 border border-gray-700/50 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-white">km/Tour-Ranking</span>
          {loading && <span className="text-xs text-gray-500">…</span>}
        </div>
        {data.alert_count > 0 && (
          <div className="flex items-center gap-1 text-xs text-red-400">
            <AlertTriangle className="h-3 w-3" />
            {data.alert_count} Lange-Route-Alert
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-gray-800 rounded-lg p-2 text-center">
          <div className="text-[10px] text-gray-400">Kürzeste/r</div>
        ...
        <div className="bg-gray-800 rounded-lg p-2 text-center">
          ...
          <div className="text-xs font-bold text-gray-200">{data.team_avg.toFixed(1)} km</div>
        </div>
        <div className="bg-gray-800 rounded-lg p-2 text-center">
      ...
      <div className="space-y-1.5">
        {data.fahrer.map(f => (
          <div key={f.fahrer_id} className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono text-gray-300 w-14 text-right">{f.km_avg.toFixed(1)} km</span>
            <div className="w-20 bg-gray-800 rounded-full h-1.5 shrink-0">
              <div
                className="h-1.5 rounded-full transition-all"
                style={{ width: `${(f.km_avg / maxKm) * 100}%`, backgroundColor: ampelColor(f.ampel) }}
              />
            </div>
            {f.alert_top && <AlertTriangle className="h-3 w-3 text-red-400 shrink-0" />}
          </div>
        ))}
      </div>

      <div className="text-[10px] text-gray-600 text-right">Ø km/Tour · AUFSTEIGEND · 30-Min-Update · {data.gesamt} Fahrer</div>
    </div>
  );
}

...
// Inverted: fallend (less wait) = green; steigend (more wait) = red
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   avg_wartezeit_min: 2.1, trend: 'fallend',  ampel: 'gruen', alert: null },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  avg_wartezeit_min: 4.3, trend: 'steigend', ampel: 'gelb',  alert: null },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   avg_wartezeit_min: 5.8, trend: 'steigend', ampel: 'gelb',  alert: null },
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', avg_wartezeit_min: 7.5, trend: 'steigend', ampel: 'rot',   alert: 'Zu lange Wartezeit!' },
  ],
  team_avg_wartezeit_min: 4.9,
  alert_count: 1,
};

export function KitchenPhase2974WartezeitStoppTicker({ locationId }: { locationId?: string | null }) {
  ...
  const teamAmpel = data.team_avg_wartezeit_min <= ZIEL_MIN ? 'gruen' : data.team_avg_wartezeit_min <= ALERT_MIN ? 'gelb' : 'rot';
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">
            Wartezeit/Stopp-Ticker
            <span className={`ml-2 font-black ${teamText}`}>{data.team_avg_wartezeit_min.toFixed(1)} Min</span>

...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   ruhe_h: 6.5,  trend: 'fallend'  },
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', ruhe_h: 7.0,  trend: 'fallend'  },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  ruhe_h: 9.5,  trend: 'steigend' },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   ruhe_h: 13.0, trend: 'steigend' },
  ],
  team_avg_ruhe_h: 9.0,
};

export function KitchenPhase2680RuhezeitenTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-4`}>
      ...
      >
        <div className="flex items-center gap-2">
          <BedDouble size={15} className={hasAlert ? 'text-red-500' : 'text-gray-500'} />
          <span className="font-semibold text-sm text-gray-800">Ruhezeiten</span>
          <span className={`text-xs font-bold ${textCls(teamAmpel)}`}>
            Ø {data.team_avg_ruhe_h} h
          </span>
          {hasAlert && (
        ...
        <div className="px-4 pb-3 space-y-2">
          ...
          {hasAlert && (
            <div className="flex items-start gap-2 bg-red-100 border border-red-200 rounded-lg px-3 py-1.5">
              ...
              <p className="text-xs text-red-700 font-medium">
                Ruhezeit zu kurz! {alerts.map(a => a.fahrer_name).join(', ')}
              </p>
            </div>
          )}
          ...
          {sorted.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              ...
              <span className={`text-xs font-bold ${textCls(f.ampel)}`}>{f.ruhe_h} h</span>
            </div>
          ))}

          {/* Ziel */}
          <p className="text-xs text-gray-400 pt-1">Ziel ≥11 h Ruhezeit/Fahrer</p>
        </div>
      )}
    </div>
  );

...
export function KitchenPhase4911WochenendeProdTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-cyan-700 bg-cyan-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Wochenende-Produktivität — Champion</div>
        <div className="text-sm font-bold text-cyan-100 truncate">
          #{top?.rang} {top?.fahrer_name} — {top?.wochenende_tph.toFixed(1)} Touren/h (Sa–So)
        </div>
        <div className="text-xs text-gray-500">Team-Ø: {data.team_avg_wochenende_tph.toFixed(1)} T/h (Sa–So)</div>
      </div>
      {data.alert_count > 0 && (
        <div className="flex items-center gap-1 text-xs text-cyan-300 shrink-0">

...
export function KitchenPhase5029TrinkgeldTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-rose-700 bg-rose-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Trinkgeld-Ranking — Champion</div>
        <div className="text-sm font-bold text-rose-100 truncate">
          #{top?.rang} {top?.fahrer_name} — {top?.avg_trinkgeld.toFixed(2)} €
        </div>
        <div className="text-xs text-gray-500">Team-Ø: {data.team_avg_trinkgeld.toFixed(2)} € · Ziel ≥3 €</div>
      </div>
      {data.alert_count > 0 && (

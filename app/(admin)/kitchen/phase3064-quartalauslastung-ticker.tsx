...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', auslastung_pct: 79.3, trend: 'steigend', ampel: 'gruen', alert_gering: false },
    { fahrer_id: 'f2', fahrer_name: 'Max M.',   auslastung_pct: 68.4, trend: 'stabil',   ampel: 'gruen', alert_gering: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  auslastung_pct: 57.9, trend: 'fallend',  ampel: 'gelb',  alert_gering: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   auslastung_pct: 41.2, trend: 'fallend',  ampel: 'rot',   alert_gering: true  },
  ],
  team_avg_pct: 61.7,
  alert_count: 1,
  quartal: 3,
...
export function KitchenPhase3064QuartalauslastungTicker({ locationId }: { locationId?: string | null }) {
        ...
        <div className="p-3 space-y-1.5">
          ...
          {list.some(f => f.alert_gering) && (
            <div className="flex items-center gap-1.5 rounded-lg bg-red-50 border border-red-200 text-red-700 text-xs font-semibold px-2.5 py-1.5">
              ...
              {list.filter(f => f.alert_gering).map(f => f.fahrer_name).join(', ')} — Geringe Quartalauslastung!
            </div>
          )}

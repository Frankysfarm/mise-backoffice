...
export function KitchenPhase3837KundenbewertungsTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      ...
      <div className="flex items-center justify-between">
        ...
        {best && (
          <div className="flex items-center gap-1 text-xs">
            ...
            <span className="font-black text-emerald-700">★ {best.bewertungs_avg}</span>
          </div>
        )}
      </div>
      ...
      {/* Kompakt-Liste (absteigend: höchste Bewertung = Rang 1 = bester) */}
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5 text-xs">
                ...
                <span className={`font-bold ${tColor}`}>★ {f.bewertungs_avg}</span>
                {f.rank_delta !== 0 && (
                  f.rank_delta > 0
                    ? <TrendingUp className="w-3 h-3 text-emerald-500" />
                    : <TrendingDown className="w-3 h-3 text-red-400" />
                )}
                {f.alert_niedrig && <AlertTriangle className="w-3 h-3 text-orange-500" />}
              </div>

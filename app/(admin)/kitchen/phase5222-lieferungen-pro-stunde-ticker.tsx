...
export function KitchenPhase5222LieferungenProStundeTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-teal-700 bg-teal-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Ø Lieferungen/h — Produktivste</div>
        <div className="text-sm font-bold text-teal-100 truncate">
          #{top.rang} {top.fahrer_name} — {top.deliveries_pro_h.toFixed(1)}/h
        </div>
        <div className="text-[10px] text-gray-500 mt-0.5">
          Team-Ø: {data.team_avg_pro_h.toFixed(1)}/h
          {data.alert_count > 0 && (

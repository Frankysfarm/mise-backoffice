...
export function KitchenPhase4601FreitagAbendTicker({ locationId }: { locationId: string | null }) {
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Freitagabend-Ticker nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-indigo-200 dark:border-indigo-800 bg-white dark:bg-gray-900 p-3 space-y-2">
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">Freitagabend-Ticker</span>
      </div>
      ...
      {top && (
        <div className="flex items-center gap-2">
          ...
          <span className="ml-auto text-sm font-bold text-indigo-500">{top.freitag_pct}%</span>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-1">
        {data.fahrer.map(f => (
          <div key={f.fahrer_id} className="flex items-center gap-1 text-[10px] text-gray-600 dark:text-gray-400">
            ...
            <span className="font-medium">{f.freitag_pct}%</span>
          </div>
        ))}
      </div>

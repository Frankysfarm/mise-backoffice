...
export function KitchenPhase4606SamstagMittagTicker({ locationId }: { locationId: string | null }) {
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Samstagmittag-Ticker nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-yellow-200 dark:border-yellow-800 bg-white dark:bg-gray-900 p-3 space-y-2">
      <div className="flex items-center gap-1.5">
        <Sun className="w-4 h-4 text-yellow-400" />
        <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">Samstagmittag-Ticker</span>
      </div>

      {top && (
        <div className="flex items-center gap-2">
          <span className="text-lg font-extrabold text-yellow-600 dark:text-yellow-400">#1</span>
          <span className="text-sm font-semibold text-gray-800 dark:text-gray-200 truncate">{top.fahrer_name}</span>
          <span className="ml-auto text-sm font-bold text-yellow-500">{top.samstag_pct}%</span>
        </div>
      )}

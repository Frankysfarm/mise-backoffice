...
export function KitchenPhase4576AbendAnteilTicker({ locationId }: Props) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">Abend-Anteil</span>
        {data.alert_count > 0 && (
      ...
      {top && (
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-bold text-rose-600 dark:text-rose-400">{top.fahrer_name}</span>
          <span className="text-sm font-bold text-rose-600 dark:text-rose-400">{top.abend_pct}%</span>
        </div>
      )}
      ...
      <div className="mt-2 text-xs text-gray-400 dark:text-gray-500">
        Team-Avg {data.team_avg}% · Ziel ≥40%
      </div>
    </div>
  );
}

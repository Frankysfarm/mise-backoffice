...
import { ChevronDown, ChevronUp, ListChecks, TrendingUp, TrendingDown, Minus, AlertTriangle } from 'lucide-react';
...
export function KitchenPhase3515StoppsProSchichtTicker({ locationId }: { locationId?: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-stopps-pro-schicht?location_id=${locationId}`, { cache: 'no-store' });
    ...
    <div className="border rounded-lg bg-white dark:bg-gray-900 shadow-sm mb-2">
      ...
      >
        <span className="flex items-center gap-2 font-semibold text-xs">
          ...
          Stopps/Schicht — Bester: {data.bester_name}
          {data.alert_count > 0 && (
        ...
        <div className="px-2.5 pb-2.5 space-y-1.5">
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1 p-1.5 rounded bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 text-[10px] border border-red-200">
              ...
              Niedrige Stopps/Schicht! {data.alert_count} Fahrer unter Zielwert
            </div>
          )}

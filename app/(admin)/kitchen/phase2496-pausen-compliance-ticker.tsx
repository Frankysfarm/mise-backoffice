'use client';
import { useEffect, useState } from 'react';
import { ChevronDown, ChevronUp, AlertTriangle, Coffee } from 'lucide-react';

interface FahrerCompliance {
  fahrer_id: string;
  fahrer_name: string;
  compliance_heute: number;
  ampel: 'gruen' | 'gelb' | 'rot';
  alert: boolean;
}

interface ApiData {
  fahrer: FahrerCompliance[];
  team_avg_compliance: number;
  alert_count: number;
}

function dot(pct: number) {
  if (pct >= 100) return 'bg-green-500';
  if (pct >= 80)  return 'bg-amber-400';
  return 'bg-red-500';
}

function textColor(pct: number) {
  if (pct >= 100) return 'text-green-600';
  if (pct >= 80)  return 'text-amber-600';
  return 'text-red-600';
}

const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd2', fahrer_name: 'Sara K.',  compliance_heute: 75,  ampel: 'rot',   alert: true  },
    { fahrer_id: 'd3', fahrer_name: 'Tim B.',   compliance_heute: 85,  ampel: 'gelb',  alert: false },
    { fahrer_id: 'd1', fahrer_name: 'Max M.',   compliance_heute: 100, ampel: 'gruen', alert: false },
  ],
  team_avg_compliance: 86.7,
  alert_count: 1,
};

export function KitchenPhase2496PausenComplianceTicker({ locationId }: { locationId?: string | null }) {
  const [open, setOpen] = useState(false);
  const [data, setData] = useState<ApiData | null>(null);

  useEffect(() => {
    if (!locationId) { setData(MOCK); return; }
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-pausen-compliance?location_id=${locationId}`)
        .then(r => r.json()).then(setData).catch(() => setData(MOCK));
    load();
    const t = setInterval(load, 30 * 60 * 1000);
    return () => clearInterval(t);
  }, [locationId]);

  if (!data) return null;

  const hasAlert = data.alert_count > 0;
  const sorted = [...data.fahrer].sort((a, b) => a.compliance_heute - b.compliance_heute);
  const alertFahrer = data.fahrer.filter(f => f.alert).map(f => f.fahrer_name);

  return (
    <div className={`rounded-xl border shadow-sm ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-100 bg-white'}`}>
      <button
        onClick={() => setOpen(o => !o)}
        className="flex w-full items-center justify-between px-3 py-2 text-left"
      >
        <div className="flex items-center gap-2">
          <Coffee size={14} className={hasAlert ? 'text-red-600' : 'text-blue-500'} />
          <span className="text-xs font-bold text-gray-700">Pausen-Compliance</span>
          <span className={`text-xs font-black tabular-nums ${hasAlert ? 'text-red-700' : 'text-gray-700'}`}>
            Ø {data.team_avg_compliance.toFixed(0)}%
          </span>
          {hasAlert && (
            <span className="flex items-center gap-1 rounded-full bg-red-100 px-1.5 py-0.5 text-[9px] font-black text-red-700 animate-pulse">
              <AlertTriangle size={9} /> {data.alert_count}
            </span>
          )}
        </div>
        {open ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
      </button>

      {open && (
        <div className="border-t px-3 pb-3 pt-2 space-y-1.5">
          {alertFahrer.length > 0 && (
            <div className="flex items-center gap-1.5 rounded-lg bg-red-100 px-2 py-1.5 text-[10px] font-semibold text-red-800">
              <AlertTriangle size={11} className="shrink-0" />
              Pausen-Verstoß (&lt;80%): {alertFahrer.join(', ')} — sofort Pause!
            </div>
          )}
          {sorted.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              <span className={`h-2 w-2 rounded-full shrink-0 ${dot(f.compliance_heute)}`} />
              <span className="text-[10px] text-gray-700 flex-1 truncate">{f.fahrer_name}</span>
              <span className={`text-[10px] font-black tabular-nums shrink-0 ${textColor(f.compliance_heute)}`}>
                {f.compliance_heute}%
              </span>
            </div>
          ))}
          <div className="pt-0.5 text-[8px] text-gray-400">30-Min-Polling · Ziel: 100% Compliance</div>
        </div>
      )}
    </div>
  );
}

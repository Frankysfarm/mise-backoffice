...
export function KitchenPhase4270KundenzufriedenheitTicker({ locationId }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-bold text-yellow-600">
            Bester: {data.bester_name} — {bester?.avg_sterne} ★
          </span>
        </div>
        <div className="flex items-center gap-1.5">
      ...
      <div className="space-y-1">
        {data.fahrer.map((f) => {
          const dotColor = f.ampel === 'gruen' ? 'bg-yellow-400' : f.ampel === 'gelb' ? 'bg-orange-400' : 'bg-red-400';
          return (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold text-gray-900">{f.avg_sterne} ★</span>
            </div>
          );
        })}
      </div>

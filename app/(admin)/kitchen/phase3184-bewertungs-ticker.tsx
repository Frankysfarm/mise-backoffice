...
export function KitchenPhase3184BewertungsTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div style={{ background: '#1a1a2e', border: '1px solid #2d2d4e', borderRadius: 10, padding: 12, marginBottom: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
        ...
        {best && (
          <span style={{ color: '#eab308', fontWeight: 700, fontSize: 13 }}>
            #1 {best.fahrer_name} — ★{best.score.toFixed(1)}
          </span>
        )}
        {hasAlert && (
          <span style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 4 }}>
            <AlertTriangle size={12} color="#fca5a5" />
            <span style={{ color: '#fca5a5', fontSize: 11 }}>Niedriger Bewertungs-Score!</span>
          </span>
        )}
      </div>
      ...
      <div style={{ marginTop: 8, color: '#888', fontSize: 11, textAlign: 'right' }}>Team-Ø: ★{data.team_avg_score.toFixed(1)}</div>
    </div>
  );
}

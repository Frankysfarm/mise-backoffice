...
export function KitchenPhase3580SchichtAuslastungTicker({ locationId }: { locationId?: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-schicht-auslastungs-ranking?location_id=${locationId}`, { cache: 'no-store' });
    ...
    <div className="border rounded-lg bg-white dark:bg-gray-900 shadow-sm mb-2">
      ...
      >
        <span className="flex items-center gap-2 font-semibold text-xs">
          ...
          Schicht-Auslastung — Bester: {data.bester_name}
          {data.alert_count > 0 && (

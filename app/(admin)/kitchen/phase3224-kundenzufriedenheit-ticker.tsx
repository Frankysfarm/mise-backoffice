...
export function KitchenPhase3224KundenzufriedenheitTicker({ locationId }: { locationId: string | null }) {
    ...
    <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 shadow-sm space-y-2">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-gray-700 dark:text-gray-200">Kundenzufriedenheit</span>
          {best && (
            <span className="text-xs text-gray-500 dark:text-gray-400">
              #1 <span className="font-medium text-gray-700 dark:text-gray-200">{best.fahrer_name}</span> {best.score.toFixed(1)} ★
            </span>
          )}
        </div>
        {data.alert_count > 0 && (

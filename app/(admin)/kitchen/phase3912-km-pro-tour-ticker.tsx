...
export function KitchenPhase3912KmProTourTicker({ locationId }: { locationId: string | null }) {
  ...
  // ascending: lowest km_pro_tour = Rang 1 = best
  const sorted = [...data.fahrer].sort((a, b) => a.rang - b.rang);
  const best = sorted[0];
  const maxVal = Math.max(...sorted.map(f => f.km_pro_tour), 1);
      ...
      <div className="flex items-center justify-between">
        ...
        {best && (
          <div className="flex items-center gap-1 text-xs">
            ...
            <span className="font-black text-gray-700">{best.km_pro_tour} km</span>
          </div>
        )}
      </div>
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              ...
              <div className="h-1 bg-gray-100 rounded-full overflow-hidden ml-5">
                <div className={`h-full rounded-full ${barColor}`} style={{ width: `${(f.km_pro_tour / maxVal) * 100}%` }} />
              </div>
            </div>
          );
        })}

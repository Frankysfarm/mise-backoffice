...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', avg_wartezeit_min: 7.5, trend: 'steigend', alert: 'Zu lange Wartezeit!' },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   avg_wartezeit_min: 5.8, trend: 'steigend', alert: null },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  avg_wartezeit_min: 4.3, trend: 'steigend', alert: null },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   avg_wartezeit_min: 2.1, trend: 'fallend',  alert: null },
  ],
  team_avg_wartezeit_min: 4.9,
};

export function KitchenPhase2745WartezeitTicker({ locationId }: { locationId?: string | null }) {
  ...
  const enriched  = data.fahrer.map((f: FahrerEntry) => ({ ...f, ampel: calcAmpel(f.avg_wartezeit_min) }));
  // absteigend: höchste Wartezeit (schlechteste) oben
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-3`}>
      ...
      >
        <div className="flex items-center gap-2">
          <Clock size={15} className={hasAlert ? 'text-red-500' : 'text-indigo-600'} />
          ...
          <span className={`ml-1 text-xs font-bold ${textCls(teamAmpel)}`}>
            Ø {data.team_avg_wartezeit_min} Min
          </span>
          <span className="text-[10px] text-gray-400 ml-1">Ziel ≤{ZIEL} Min</span>
        </div>
          ...
          {sorted.map((f: FahrerEntry & { ampel: string }) => (
            <div key={f.fahrer_id} className="flex items-center justify-between py-1 border-b border-gray-100 last:border-0">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${textCls(f.ampel)}`}>{f.avg_wartezeit_min} Min</span>
              </div>
            </div>
          ))}
        </div>

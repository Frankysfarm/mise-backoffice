...
function Delta({ v }: { v: number }) {
  if (v > 0) return <span className="flex items-center gap-0.5 text-green-500 text-xs"><TrendingUp size={10} />+{v}</span>;
  if (v < 0) return <span className="flex items-center gap-0.5 text-red-400 text-xs"><TrendingDown size={10} />{v}</span>;
  return <span className="text-gray-400 text-xs flex items-center gap-0.5"><Minus size={10} />0</span>;
}

export function KitchenPhase3229LieferdauerEffizienzTicker({ locationId }: { locationId: string | null }) {
  ...
  const load = async () => {
    try {
      const url = locationId
        ? `/api/delivery/admin/fahrer-lieferdauer-effizienz?location_id=${locationId}`
        : `/api/delivery/admin/fahrer-lieferdauer-effizienz`;
    ...
    <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 shadow-sm space-y-2">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-gray-700 dark:text-gray-200">Lieferdauer-Effizienz</span>
          {best && (
            <span className="text-xs text-gray-500 dark:text-gray-400">
              #1 <span className="font-medium text-gray-700 dark:text-gray-200">{best.fahrer_name}</span> {best.effizienz_pct}%
            </span>
          )}
        </div>
        {data.alert_count > 0 && (
          <span className="text-xs px-1.5 py-0.5 rounded bg-red-100 text-red-700 font-medium">
            {data.alert_count}× Niedrige Lieferdauer-Effizienz!
          </span>
        )}
      </div>
      ...
      <div className="space-y-1">
        {data.fahrer.map(f => (
          <div key={f.fahrer_id} className="flex items-center gap-1.5 text-xs">
            ...
            <span className={`font-semibold w-8 text-right ${f.ampel === 'gruen' ? 'text-green-600' : f.ampel === 'gelb' ? 'text-yellow-500' : 'text-red-500'}`}>
              {f.effizienz_pct}%
            </span>
      ...
      <div className="text-xs text-gray-400 border-t border-gray-100 dark:border-gray-700 pt-1">
        Team-Ø: <span className="font-medium text-blue-600 dark:text-blue-400">{data.team_avg_pct}%</span>
      </div>
    </div>
  );
}

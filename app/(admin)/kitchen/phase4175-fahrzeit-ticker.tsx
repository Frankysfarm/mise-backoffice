...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; fahrzeit_min: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_langsam: boolean; }
interface ApiData { fahrer: FahrerRow[]; team_avg: number; schnellster_name: string; alert_count: number; }

const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Sara K.',  rang: 1, fahrzeit_min: 14, ampel: 'gruen', alert_langsam: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', rang: 2, fahrzeit_min: 18, ampel: 'gruen', alert_langsam: false },
    { fahrer_id: 'f3', fahrer_name: 'Max M.',   rang: 3, fahrzeit_min: 24, ampel: 'gelb',  alert_langsam: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, fahrzeit_min: 32, ampel: 'rot',   alert_langsam: true  },
  ],
  team_avg: 22,
...
export function KitchenPhase4175FahrzeitTicker({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-gray-900">Ø Fahrzeit</span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-cyan-600 font-medium">
            #{bester?.rang} {data.schnellster_name ?? bester?.fahrer_name} {bester?.fahrzeit_min?.toFixed(0)}min
          </span>
        </div>
      </div>
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-1.5">
              ...
              <span className={`text-[10px] font-bold ${tColor}`}>{f.fahrzeit_min?.toFixed(0)} min</span>
            </div>
          );
        })}
      </div>

...
export function KitchenPhase3363TrinkgeldQuoteTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          <BadgeDollarSign size={15} className="text-emerald-600 flex-shrink-0" />
          ...
          {bester && (
            <span className="text-xs text-emerald-700 dark:text-emerald-400 font-medium truncate">
              #{1} {bester.fahrer_name} {bester.trinkgeld_quote.toFixed(2)} €/Stopp
            </span>
          )}
          {alerts.length > 0 && (
        ...
        <div className="p-3 space-y-2">
          ...
          {list.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 text-xs">
              ...
              <span className={`font-bold tabular-nums ${textCls(f.ampel)}`}>{f.trinkgeld_quote.toFixed(2)} €/Stopp</span>
              {f.rank_delta > 0
          ...
          <div className="pt-1 border-t border-gray-100 dark:border-gray-700 flex justify-between text-xs text-gray-400">
            <span>Team-Ø: {d.team_avg.toFixed(2)} €/Stopp</span>
            <span>Ziel: &gt;1.00 €/Stopp</span>
          </div>
        </div>
      )}
    </div>

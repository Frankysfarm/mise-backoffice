...
export function KitchenPhase3134LieferzeitTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">
            Ø Lieferzeit · #1 {bester?.fahrer_name ?? data?.bester_name ?? '—'} ({bester ? `${bester.avg_min} Min` : '—'})
          </span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-2">
          ...
          <div className="text-xs text-gray-400 pt-1">Team-Ø {teamAvg} Min · Ziel Top 25% (kürzeste Zeit)</div>
        </div>
      )}
    </div>
  );

...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', punkte_heute: 41, trend: 'fallend',  alert: 'Schicht-Score zu niedrig!' },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   punkte_heute: 53, trend: 'fallend',  alert: null },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  punkte_heute: 68, trend: 'stabil',   alert: null },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   punkte_heute: 87, trend: 'steigend', alert: null },
  ],
  team_avg_punkte: 62,
};

export function KitchenPhase2750SchichtPunkteTicker({ locationId }: { locationId?: string | null }) {
  ...
  const enriched  = data.fahrer.map((f: FahrerEntry) => ({ ...f, ampel: calcAmpel(f.punkte_heute) }));
  // absteigend: niedrigste Punkte (schlechteste) oben für schnelles Eingreifen
  const sorted    = [...enriched].sort((a, b) => a.punkte_heute - b.punkte_heute);
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-3`}>
      ...
      >
        <div className="flex items-center gap-2">
          <Star size={15} className={hasAlert ? 'text-red-500' : 'text-indigo-600'} />
          ...
          <span className={`ml-1 text-xs font-bold ${textCls(teamAmpel)}`}>
            Ø {data.team_avg_punkte} Pkt
          </span>
          <span className="text-[10px] text-gray-400 ml-1">Ziel ≥{ZIEL} Pkt</span>
        </div>
          ...
          {sorted.map((f: FahrerEntry & { ampel: string }) => (
            <div key={f.fahrer_id} className="flex items-center justify-between py-1 border-b border-gray-100 last:border-0">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${textCls(f.ampel)}`}>{f.punkte_heute} Pkt</span>
              </div>
            </div>
          ))}
        </div>

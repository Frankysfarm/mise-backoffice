...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; km: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_bottom: boolean; }
interface ApiData { fahrer: FahrerRow[]; team_avg_km: number; bester_name: string; alert_count: number; }

const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   rang: 1, km: 48.2, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', rang: 2, km: 41.5, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, km: 28.0, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, km: 12.3, ampel: 'rot',   alert_bottom: true  },
  ],
  team_avg_km: 32.5,
  bester_name: 'Max M.',
  alert_count: 1,
...
export function KitchenPhase4095TageskilometerTicker({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-gray-900">Tages-km</span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-teal-600 font-medium">
            #{bester?.rang} {data.bester_name ?? bester?.fahrer_name} {bester?.km?.toFixed(1)}km
          </span>
        </div>
      </div>
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-1.5">
              ...
              <span className={`text-[10px] font-bold ${tColor}`}>{f.km?.toFixed(1)} km</span>
            </div>
          );
        })}
      </div>

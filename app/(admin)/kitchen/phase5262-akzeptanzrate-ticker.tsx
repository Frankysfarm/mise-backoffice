...
export function KitchenPhase5262AkzeptanzrateTicker({ locationId }: { locationId: string | null }) {
  const [data, setData] = useState<ApiResponse | null>(null);

  async function load() {
    const url = locationId
      ? `/api/delivery/admin/fahrer-akzeptanz-rate-ranking?location_id=${locationId}`
      : '/api/delivery/admin/fahrer-akzeptanz-rate-ranking';
  ...
  return (
    <div className="rounded-xl border border-sky-700 bg-sky-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Akzeptanzrate — Beste</div>
        <div className="text-sm font-bold text-sky-100 truncate">
          #{top.rang} {top.fahrer_name} — {top.akzeptanz_rate.toFixed(0)}%
        </div>
        <div className="text-[10px] text-gray-500 mt-0.5">
          Team-Ø: {data.team_avg_akzeptanz.toFixed(0)}% · {data.gesamt} Fahrer erfasst
          {data.alert_count > 0 && (

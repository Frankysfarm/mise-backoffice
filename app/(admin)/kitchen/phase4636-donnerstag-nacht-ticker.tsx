...
export function KitchenPhase4636DonnerstagNachtTicker({ locationId }: { locationId: string | null }) {
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Donnerstagnacht-Ticker nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-blue-200 dark:border-blue-900 bg-white dark:bg-gray-900 p-3 space-y-2">
      <div className="flex items-center gap-1.5">
        <Moon className="w-4 h-4 text-blue-700 dark:text-blue-400" />
        <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">Donnerstagnacht-Ticker</span>
      </div>

      {top && (
        <div className="flex items-center gap-2">
          <span className="text-lg font-extrabold text-blue-700 dark:text-blue-400">#1</span>
          <span className="text-sm font-semibold text-gray-800 dark:text-gray-200 truncate">{top.fahrer_name}</span>
          <span className="ml-auto text-sm font-bold text-blue-700 dark:text-blue-400">{top.donnerstag_pct}%</span>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-1">
        {data.fahrer.map(f => (
          <div key={f.fahrer_id} className="flex items-center gap-1 text-[10px] text-gray-600 dark:text-gray-400">
            ...
            <span className="font-medium">{f.donnerstag_pct}%</span>
          </div>
        ))}
      </div>

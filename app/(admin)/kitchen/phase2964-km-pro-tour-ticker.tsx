...
// Invertiert: fallend = gut (grün), steigend = schlecht (rot)
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Max M.',   km_pro_tour: 6.2,  trend: 'fallend',  ampel: 'gruen', alert: false },
    { fahrer_id: 'd4', fahrer_name: 'Julia F.', km_pro_tour: 7.4,  trend: 'fallend',  ampel: 'gruen', alert: false },
    { fahrer_id: 'd3', fahrer_name: 'Tim B.',   km_pro_tour: 9.1,  trend: 'fallend',  ampel: 'gelb',  alert: false },
    { fahrer_id: 'd2', fahrer_name: 'Sara K.',  km_pro_tour: 13.5, trend: 'steigend', ampel: 'rot',   alert: true  },
  ],
  team_avg_km_pro_tour: 9.1,
  alert_count: 1,
};

export function KitchenPhase2964KmProTourTicker({ locationId }: { locationId?: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-km-pro-tour?location_id=${locationId}`)
        .then(r => r.json())
  ...
  const enriched = data.fahrer.map(f => ({ ...f, ampel: calcAmpel(f.km_pro_tour) }));
  const sorted   = [...enriched].sort((a, b) => a.km_pro_tour - b.km_pro_tour);
  const alerts   = enriched.filter(f => f.alert);
  const hasAlert = alerts.length > 0;

  const teamAmpel = calcAmpel(data.team_avg_km_pro_tour);
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">
            km/Tour-Ticker
            <span className={`ml-2 font-black ${teamText}`}>{data.team_avg_km_pro_tour.toFixed(1)} km</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-2">
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-1.5 text-xs">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: Zu hohe km pro Tour! ({f.km_pro_tour.toFixed(1)} km)
              </span>
            </div>
          ))}
          ...
          <div className="text-xs text-gray-400 pt-1">Ziel ≤{ZIEL_KM} km | 30-Min-Polling</div>
        </div>
      )}
    </div>
  );

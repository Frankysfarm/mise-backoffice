...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; retour_quote_pct: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_top: boolean; }
interface ApiData { fahrer: FahrerRow[]; team_avg: number; bester_name: string; alert_count: number; }

const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'p1', fahrer_name: 'Julia F.', rang: 1, retour_quote_pct:  2, ampel: 'gruen', alert_top: false },
    { fahrer_id: 'p2', fahrer_name: 'Sara K.',  rang: 2, retour_quote_pct:  5, ampel: 'gelb',  alert_top: false },
    { fahrer_id: 'p3', fahrer_name: 'Max M.',   rang: 3, retour_quote_pct:  9, ampel: 'gelb',  alert_top: false },
    { fahrer_id: 'p4', fahrer_name: 'Tim B.',   rang: 4, retour_quote_pct: 15, ampel: 'rot',   alert_top: true  },
  ],
  team_avg: 7.75,
  bester_name: 'Julia F.',
  alert_count: 1,
...
export function KitchenPhase4115RetourQuoteTicker({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-gray-900">Retour-Quote</span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-purple-600 font-medium">
            #{bester?.rang} {data.bester_name ?? bester?.fahrer_name} {bester?.retour_quote_pct?.toFixed(1)}%
          </span>
        </div>
      </div>
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-1.5">
              ...
              <span className={`text-[10px] font-bold ${tColor}`}>{f.retour_quote_pct?.toFixed(1)} %</span>
            </div>
          );
        })}
      </div>

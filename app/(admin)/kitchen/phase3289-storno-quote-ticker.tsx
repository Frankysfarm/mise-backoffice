...
export function KitchenPhase3289StornoQuoteTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-3 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-xs text-gray-800 dark:text-gray-100">Storno-Quote-Ticker</span>
          {bester && (
            <span className="text-xs text-green-700 dark:text-green-400 font-semibold ml-1">
              ✓ {bester.fahrer_name} {bester.quote_pct.toFixed(1)}%
            </span>
          )}
          {(d.alert_count ?? 0) > 0 && (
          ...
          {list.map(f => (
            <div key={f.fahrer_id} className="flex items-center justify-between text-xs py-1 border-b border-gray-100 dark:border-gray-800 last:border-0">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-bold text-gray-800 dark:text-gray-100">{f.quote_pct.toFixed(1)}%</span>
                {f.trend_delta < 0  && <TrendingDown size={10} className="text-green-500" />}
                {f.trend_delta > 0  && <TrendingUp   size={10} className="text-red-400"   />}

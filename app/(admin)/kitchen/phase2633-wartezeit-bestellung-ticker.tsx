...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f3', fahrer_name: 'Tim W.',  avg_min: 13.1, trend: 'steigend' },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.', avg_min:  7.8, trend: 'stabil'   },
    { fahrer_id: 'f4', fahrer_name: 'Anna B.', avg_min:  5.2, trend: 'stabil'   },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',  avg_min:  3.5, trend: 'fallend'  },
  ],
  team_durchschnitt: 7.4,
  alert_count: 1,
};

export function KitchenPhase2633WarteZeitBestellungTicker({ locationId }: { locationId?: string | null }) {
  ...
  const alerts    = enriched.filter(f => f.avg_min > 10);
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-3`}>
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-semibold text-xs text-gray-800">Wartezeit Bestellung</span>
          {hasAlert && (
      ...
      {open && (
        <div className="px-3 pb-3 space-y-2">
          {hasAlert && (
            <div className="flex items-start gap-1.5 bg-red-100 border border-red-200 rounded-lg px-2 py-1.5">
              ...
              <p className="text-xs text-red-700 font-medium">
                Wartezeit zu lang: {alerts.map((f: { fahrer_name: string }) => f.fahrer_name).join(', ')}
              </p>
            </div>
          )}
          ...
          <div className="text-xs text-gray-400 text-right">Ziel: ≤5 Min</div>
        </div>
      )}
    </div>
  );

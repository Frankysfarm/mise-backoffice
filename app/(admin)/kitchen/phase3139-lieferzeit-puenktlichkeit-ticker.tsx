...
export function KitchenPhase3139LieferzeitPuenktlichkeitTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">
            Pünktlichkeit · #1 {bester?.fahrer_name ?? data?.bester_name ?? '—'} ({bester ? `${bester.pct}%` : '—'})
          </span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-2">
          {alerts.length > 0 && (
            <div className="flex items-center gap-2 rounded-lg bg-red-50 border border-red-200 dark:bg-red-950 dark:border-red-800 text-red-700 dark:text-red-300 text-xs font-semibold px-3 py-2">
            ...
            {list.map(f => (
              <div key={f.fahrer_id} className="flex items-center justify-between text-xs py-1 border-b border-gray-100 dark:border-gray-800 last:border-0">
                ...
                <div className="flex items-center gap-1">
                  <span className="font-semibold text-gray-700 dark:text-gray-200">{f.pct}%</span>
          ...
          <div className="text-xs text-gray-400 pt-1">Team-Ø {teamAvg}% · Ziel Top 25% (pünktlichste)</div>
        </div>
      )}
    </div>
  );

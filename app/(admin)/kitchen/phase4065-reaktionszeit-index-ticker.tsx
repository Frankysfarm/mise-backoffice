...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; reaktionszeit_min: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_langsam: boolean; }
interface ApiData { fahrer: FahrerRow[]; team_avg: number; schnellster_name: string; alert_count: number; }

const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'm1', fahrer_name: 'Sara K.',  rang: 1, reaktionszeit_min: 2.1, ampel: 'gruen', alert_langsam: false },
    { fahrer_id: 'm2', fahrer_name: 'Julia F.', rang: 2, reaktionszeit_min: 3.4, ampel: 'gruen', alert_langsam: false },
    { fahrer_id: 'm3', fahrer_name: 'Max M.',   rang: 3, reaktionszeit_min: 5.8, ampel: 'gelb',  alert_langsam: false },
    { fahrer_id: 'm4', fahrer_name: 'Tim B.',   rang: 4, reaktionszeit_min: 8.2, ampel: 'rot',   alert_langsam: true  },
  ],
  team_avg: 4.9,
...
export function KitchenPhase4065ReaktionszeitIndexTicker({ locationId }: Props) {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-violet-600 font-medium">
            #{bester?.rang} {data.schnellster_name ?? bester?.fahrer_name} {bester?.reaktionszeit_min?.toFixed(1)} min
          </span>
        </div>
      </div>
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-1.5">
              ...
              <span className={`text-[10px] font-bold ${tColor}`}>{f.reaktionszeit_min?.toFixed(1)} min</span>
            </div>
          );
        })}
      </div>
      <div className="flex justify-between text-[10px] text-gray-400 px-0.5 pt-0.5 border-t border-gray-100">
        <span>Ø {data.team_avg?.toFixed(1)} min</span>

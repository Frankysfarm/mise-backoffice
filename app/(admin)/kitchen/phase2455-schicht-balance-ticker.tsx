...
export function KitchenPhase2455SchichtBalanceTicker({ locationId }: { locationId?: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-schicht-balance?location_id=${locationId}`)
        .then(r => r.json())
        .then(setData)
        .catch(console.error);
    load();
    ...
    <div className={`border rounded-xl mb-2 overflow-hidden ${headerColor}`}>
      ...
      >
        <span className="flex items-center gap-1.5">
          <Scale size={12} />
          Schicht-Balance
          <span className="font-bold">{data.team_avg_balance_pct.toFixed(1)}%</span>
          {hasAlerts && (
        ...
        <div className="bg-white px-3 pb-2 pt-1 space-y-2">
          {hasAlerts && (
            <div className="flex items-start gap-1.5 bg-red-50 border border-red-200 rounded px-2 py-1 text-xs text-red-700">
              ...
              <span>
                <strong>Ungleichgewicht:</strong>{' '}
                {data.fahrer.filter(f => f.alert_imbalance).map(f => f.fahrer_name).join(', ')} — Regulär-Anteil &lt;60%
              </span>
            </div>
          )}
          <div className="space-y-1">
            {data.fahrer.map(f => (
              <div key={f.fahrer_id} className="flex items-center gap-1.5 text-xs">
                ...
                <span className="font-semibold text-gray-600">{f.balance_score_pct.toFixed(0)}%</span>
              </div>
            ))}
          </div>
        </div>

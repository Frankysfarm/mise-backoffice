...
export function KitchenPhase3294PuenktlichkeitTicker({ locationId }: { locationId?: string | null }) {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (res.ok && active) {
          ...
          const total = (json.fahrer ?? []).length;
          const cutoff = Math.ceil(total * 0.25);
          const alerts = (json.fahrer ?? []).filter((f: FahrerRow) => f.rang > total - cutoff).length;
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-3 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-xs text-gray-800 dark:text-gray-100">Pünktlichkeits-Ticker</span>
          {bester && (

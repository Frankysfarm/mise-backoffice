...
export function KitchenPhase5084AuftragsdichteTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-purple-700 bg-purple-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Auftrags-Dichte-Ranking — Aktivste</div>
        <div className="text-sm font-bold text-purple-100 truncate">
          #{top?.rang} {top?.fahrer_name} — {top?.dichte}/h
        </div>
        <div className="text-xs text-gray-500">Team-Ø: {data.team_avg}/h · Ziel {data.ziel}/h</div>
      </div>
      {data.alert_count > 0 && (

...
export function KitchenPhase4365AuslastungTicker({ locationId }: Props) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-bold text-sky-700">
            Aktivste: {data.bester_name} — {bester?.auslastung_pct.toFixed(1)} %
          </span>
        </div>
        <div className="flex items-center gap-1.5">
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold text-gray-900">{f.auslastung_pct.toFixed(1)} %</span>
            </div>
          );
        })}
      </div>

...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', auslastung_pct: 85.7, trend: 'steigend', ampel: 'gruen', alert_gering: false },
    { fahrer_id: 'f2', fahrer_name: 'Max M.',   auslastung_pct: 74.5, trend: 'stabil',   ampel: 'gruen', alert_gering: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  auslastung_pct: 62.1, trend: 'steigend', ampel: 'gelb',  alert_gering: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   auslastung_pct: 41.3, trend: 'fallend',  ampel: 'rot',   alert_gering: true  },
  ],
  team_avg_pct: 65.9,
  alert_count: 1,
};

export function KitchenPhase3059MonatsauslastungTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Monatsauslastung</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-2">
          ...
          <div className="text-xs text-gray-400 dark:text-gray-500 pt-1">
            Ziel ≥{ZIEL_PCT} % · laufender Monat (KW1–KW4) · 30-Min-Polling
          </div>
        </div>
      )}
    </div>

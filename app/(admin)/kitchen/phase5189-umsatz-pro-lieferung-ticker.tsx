...
export function KitchenPhase5189UmsatzProLieferungTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-emerald-700 bg-emerald-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Ø Umsatz/Lieferung — Bester</div>
        <div className="text-sm font-bold text-emerald-100 truncate">
          #{top.rang} {top.fahrer_name} — {top.avg_euro.toFixed(2)} €
        </div>
        <div className="text-[10px] text-gray-500 mt-0.5">
          Team-Ø: {data.team_avg.toFixed(2)} €
          {data.alert_count > 0 && (

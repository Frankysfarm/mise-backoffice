...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Julia F.', quote_pct: 97.6, trend: 'steigend', ampel: 'gruen' },
    { fahrer_id: 'd2', fahrer_name: 'Max M.',   quote_pct: 97.4, trend: 'steigend', ampel: 'gruen' },
    { fahrer_id: 'd3', fahrer_name: 'Sara K.',  quote_pct: 88.6, trend: 'stabil',   ampel: 'gelb'  },
    { fahrer_id: 'd4', fahrer_name: 'Tim B.',   quote_pct: 82.1, trend: 'fallend',  ampel: 'rot'   },
  ],
  team_avg_quote: 91.4,
};

export function KitchenPhase2934AbschlussquotenTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-xs text-gray-700 dark:text-gray-200">Abschlussquote</span>
          <span className={`text-xs font-bold ${teamColor}`}>{data.team_avg_quote.toFixed(0)}% Team-Ø</span>
        ...
        <div className="px-3 py-2 space-y-1.5">
          ...
          {alerts.map(f => (
            <div key={`alert-${f.fahrer_id}`} className="text-xs text-red-600 dark:text-red-400 font-medium pl-4">
              ⚠ {f.fahrer_name}: Abschlussquote zu niedrig!
            </div>
          ))}
          <div className="pt-1 text-xs text-gray-400 dark:text-gray-500">Ziel ≥{ZIEL}% | absteigend nach Quote</div>
        </div>
      )}
    </div>
  );

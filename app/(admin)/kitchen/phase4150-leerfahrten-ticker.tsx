...
export function KitchenPhase4150LeerfahrtenTicker({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-gray-900">Leerfahrten</span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-violet-600 font-medium">
            #{bester?.rang} {data.bester_name ?? bester?.fahrer_name} {bester?.leerfahrten_pct?.toFixed(0)}%
          </span>
        </div>
      </div>
      <div className="space-y-1">
        {data.fahrer.map((f) => {
          const dotColor = f.ampel === 'gruen' ? 'bg-violet-400' : f.ampel === 'gelb' ? 'bg-yellow-400' : 'bg-red-400';
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-1.5">
              ...
              <span className={`text-[10px] font-bold ${tColor}`}>{f.leerfahrten_pct?.toFixed(0)} %</span>
            </div>
          );
        })}
      </div>

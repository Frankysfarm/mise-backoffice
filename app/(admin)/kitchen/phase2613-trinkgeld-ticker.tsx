...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f2', fahrer_name: 'Sarah K.',  avg_trinkgeld_heute: 1.40, avg_trinkgeld_gestern: 4.10, trend: 'schlechter', ampel: 'rot',   alert: true  },
    { fahrer_id: 'f5', fahrer_name: 'Jana F.',   avg_trinkgeld_heute: 0.80, avg_trinkgeld_gestern: 4.50, trend: 'schlechter', ampel: 'rot',   alert: true  },
    { fahrer_id: 'f3', fahrer_name: 'Lena S.',   avg_trinkgeld_heute: 3.50, avg_trinkgeld_gestern: 3.80, trend: 'stabil',     ampel: 'gelb',  alert: false },
    { fahrer_id: 'f4', fahrer_name: 'Tom B.',    avg_trinkgeld_heute: 5.90, avg_trinkgeld_gestern: 6.20, trend: 'stabil',     ampel: 'gruen', alert: false },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',    avg_trinkgeld_heute: 6.80, avg_trinkgeld_gestern: 5.20, trend: 'besser',     ampel: 'gruen', alert: false },
  ],
  team_avg_heute: 3.68, team_avg_gestern: 4.76, ziel: 5, alert_count: 2,
};

export function KitchenPhase2613TrinkgeldTicker({ locationId }: { locationId?: string | null }) {
  ...
  const teamColor  = data.team_avg_heute >= data.ziel ? 'text-green-600' : data.team_avg_heute >= 2 ? 'text-amber-600' : 'text-red-600';
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-3`}>
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-semibold text-xs text-gray-800">Trinkgeld</span>
          {hasAlert && (
        ...
        <div className="flex items-center gap-1.5">
          <span className={`text-xs font-bold ${teamColor}`}>Ø {data.team_avg_heute.toFixed(2)} €</span>
          ...
          {hasAlert && (
            <div className="flex items-start gap-1.5 bg-red-100 border border-red-200 rounded-lg px-2 py-1.5">
              ...
              <p className="text-xs text-red-700 font-medium">
                Trinkgeld sehr niedrig: {alertNames.join(', ')}
              </p>
            </div>
          )}
            ...
            {sorted.map(f => (
              <div key={f.fahrer_id} className="flex items-center gap-1.5 py-0.5">
                ...
                <span className={`text-xs font-semibold ${textColor(f.ampel)}`}>
                  {f.avg_trinkgeld_heute.toFixed(2)} €
                </span>
              </div>
            ))}
          </div>

...
export function KitchenPhase2253PuenktlichkeitsTicker({ locationId }: { locationId?: string | null }) {
  ...
  const hinweis = useMemo(() => {
    if (!data) return '';
    if (data.team_durchschnitt < 75) return 'Pünktlichkeit kritisch — Dispatcher sofort informieren!';
    if (data.team_durchschnitt < 85) return 'Pünktlichkeit unter Ziel — Dispatcher auf Problemfälle hinweisen.';
    return 'Pünktlichkeit gut — kein Handlungsbedarf.';
  }, [data]);

  if (!locationId) return null;

  const level = data
    ? data.team_durchschnitt >= 95 ? 'gut' : data.team_durchschnitt >= 85 ? 'mittel' : 'schlecht'
    : 'mittel';
    ...
    <div className={`rounded-xl border p-4 mb-3 ${borderFarbe}`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={`font-semibold text-sm ${titelFarbe}`}>Pünktlichkeits-Ticker</span>
          {data && (
        ...
        <div className="mt-3 space-y-2">
          {alertFahrer.length > 0 && (
            <div className="flex items-center gap-2 rounded-lg bg-red-100 dark:bg-red-900/30 border border-red-300 dark:border-red-700 px-3 py-2 text-xs text-red-700 dark:text-red-300">
              ...
              <span>
                <strong>{alertFahrer.length} Fahrer</strong> unter 85%: {alertFahrer.map((f) => f.fahrer_name).join(', ')}
              </span>
            </div>
          )}

          {data && (
            <p className={`text-xs rounded px-2 py-1 ${level === 'schlecht' ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300' : level === 'mittel' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300' : 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300'}`}>
              {hinweis}
            </p>
          )}
        </div>

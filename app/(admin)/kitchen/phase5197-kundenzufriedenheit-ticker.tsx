...
export function KitchenPhase5197KundenzufriedenheitTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-amber-700 bg-amber-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Kundenbewertung — Bester</div>
        <div className="text-sm font-bold text-amber-100 truncate">
          #{top.rang} {top.fahrer_name} — {top.avg_rating.toFixed(1)} ★
        </div>
        <div className="text-[10px] text-gray-500 mt-0.5">
          Team-Ø: {data.team_avg_rating.toFixed(1)} ★
          {data.alert_count > 0 && (

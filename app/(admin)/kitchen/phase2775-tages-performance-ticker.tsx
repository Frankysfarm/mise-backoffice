...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', score: 45, trend: 'fallend',  alert: 'Tagesleistung zu niedrig!' },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   score: 61, trend: 'fallend',  alert: null },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  score: 74, trend: 'steigend', alert: null },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   score: 88, trend: 'steigend', alert: null },
  ],
  team_avg_score: 67,
};

export function KitchenPhase2775TagesPerformanceTicker({ locationId }: { locationId?: string | null }) {
  ...
  // aufsteigend: niedrigste Score (schlechteste) oben
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={`ml-1 text-xs font-bold ${textCls(teamAmpel)}`}>
            Team-Ø {data.team_avg_score} Pkt
          </span>
          ...
          {sorted.map(f => (
            <div key={f.fahrer_id} className="flex items-center justify-between py-1 border-b border-gray-100 last:border-0">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${textCls(f.ampel)}`}>{f.score} Pkt</span>
              </div>
            </div>
          ))}
        </div>

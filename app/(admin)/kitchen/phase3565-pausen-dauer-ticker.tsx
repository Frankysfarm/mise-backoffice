...
import { ChevronDown, ChevronUp, Coffee, TrendingUp, TrendingDown, Minus, AlertTriangle } from 'lucide-react';
...
export function KitchenPhase3565PausenDauerTicker({ locationId }: { locationId?: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-pausen-dauer?location_id=${locationId}`, { cache: 'no-store' });
    ...
    <div className="border rounded-lg bg-white dark:bg-gray-900 shadow-sm mb-2">
      ...
      >
        <span className="flex items-center gap-2 font-semibold text-xs">
          ...
          Pausen-Dauer — Kürzester: {data.kuerzester_name}
          {data.alert_count > 0 && (
        ...
        <div className="px-2.5 pb-2.5 space-y-1">
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1 text-[10px] text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 border border-red-200 rounded px-2 py-1">
              ...
              Lange Pause! {data.alert_count} Fahrer über Zielwert
            </div>
          )}

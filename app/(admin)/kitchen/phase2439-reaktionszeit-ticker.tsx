...
export function KitchenPhase2439ReaktionszeitTicker({ locationId }: { locationId?: string | null }) {
              ...
              {hasAlert && (
                <div className="flex items-start gap-2 bg-red-100 border border-red-200 rounded-lg p-2">
                  ...
                  <p className="text-xs text-red-800">
                    Über 7 Min: {alerts.map(f => f.fahrer_name).join(', ')} — Fahrer beschleunigen, Essen wird kalt!
                  </p>
                </div>
              )}
                ...
                {data.fahrer.map(f => (
                  <div key={f.fahrer_id} className="flex items-center justify-between text-xs">
                    ...
                    <div className="flex items-center gap-2 text-gray-700">
                      <span>{f.touren_heute} Tour{f.touren_heute !== 1 ? 'en' : ''}</span>
                      <span className="font-semibold">{f.avg_min.toFixed(1)} Min</span>
                    </div>
                  </div>
                ))}
              </div>

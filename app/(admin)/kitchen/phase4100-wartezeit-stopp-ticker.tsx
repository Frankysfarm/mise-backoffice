...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; wartezeit_min: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_top: boolean; }
interface ApiData { fahrer: FahrerRow[]; team_avg: number; bester_name: string; alert_count: number; }

const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'p1', fahrer_name: 'Julia F.', rang: 1, wartezeit_min: 2.1, ampel: 'gruen', alert_top: false },
    { fahrer_id: 'p2', fahrer_name: 'Sara K.',  rang: 2, wartezeit_min: 3.5, ampel: 'gruen', alert_top: false },
    { fahrer_id: 'p3', fahrer_name: 'Max M.',   rang: 3, wartezeit_min: 5.2, ampel: 'gelb',  alert_top: false },
    { fahrer_id: 'p4', fahrer_name: 'Tim B.',   rang: 4, wartezeit_min: 8.1, ampel: 'rot',   alert_top: true  },
  ],
  team_avg: 4.725,
  bester_name: 'Julia F.',
  alert_count: 1,
...
export function KitchenPhase4100WartezeitStoppTicker({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-gray-900">Wartezeit Stopp</span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-amber-600 font-medium">
            #{bester?.rang} {data.bester_name ?? bester?.fahrer_name} {bester?.wartezeit_min?.toFixed(1)}min
          </span>
        </div>
      </div>
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-1.5">
              ...
              <span className={`text-[10px] font-bold ${tColor}`}>{f.wartezeit_min?.toFixed(1)} min</span>
            </div>
          );
        })}
      </div>
      <div className="flex justify-between text-[10px] text-gray-400 px-0.5 pt-0.5 border-t border-gray-100">
        ...
        <span>#1 = kürzeste Wartezeit</span>
      </div>
    </div>
  );
}

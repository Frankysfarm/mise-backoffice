...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f2', fahrer_name: 'Jana F.',   lieferungen_heute: 8,  lieferungen_gestern: 10, trend: 'fallend',  ampel: 'rot',   alert: true  },
    { fahrer_id: 'f3', fahrer_name: 'Sarah K.',  lieferungen_heute: 7,  lieferungen_gestern: 9,  trend: 'fallend',  ampel: 'rot',   alert: true  },
    { fahrer_id: 'f4', fahrer_name: 'Lena S.',   lieferungen_heute: 12, lieferungen_gestern: 13, trend: 'stabil',   ampel: 'gelb',  alert: false },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',    lieferungen_heute: 18, lieferungen_gestern: 16, trend: 'steigend', ampel: 'gruen', alert: false },
    { fahrer_id: 'f5', fahrer_name: 'Tom B.',    lieferungen_heute: 20, lieferungen_gestern: 18, trend: 'steigend', ampel: 'gruen', alert: false },
  ],
  team_avg_heute: 13.0, team_avg_gestern: 13.2, ziel: 15, alert_count: 2,
};

export function KitchenPhase2588LiefervolumenTrendTicker({ locationId }: { locationId?: string | null }) {
  ...
  const teamAmpel   = data.team_avg_heute >= data.ziel ? 'text-green-600' : data.team_avg_heute >= 10 ? 'text-amber-600' : 'text-red-600';
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-3`}>
      ...
      >
        <div className="flex items-center gap-1.5">
          <Package size={14} className={hasAlert ? 'text-red-500' : 'text-green-600'} />
          <span className="font-semibold text-xs text-gray-800">Liefervolumen-Trend</span>
          {hasAlert && (

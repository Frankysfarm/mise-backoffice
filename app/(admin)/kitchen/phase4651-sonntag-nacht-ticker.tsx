...
export function KitchenPhase4651SonntagNachtTicker({ locationId }: { locationId: string | null }) {
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Sonntagnacht-Ticker nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-purple-200 dark:border-purple-900 bg-white dark:bg-gray-900 p-3 space-y-2">
      <div className="flex items-center gap-1.5">
        <Moon className="w-4 h-4 text-purple-900 dark:text-purple-300" />
        <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">Sonntagnacht-Ticker</span>
      </div>

      {top && (
        <div className="flex items-center gap-2">
          <span className="text-lg font-extrabold text-purple-900 dark:text-purple-300">#1</span>
          <span className="text-sm font-semibold text-gray-800 dark:text-gray-200 truncate">{top.fahrer_name}</span>
          <span className="ml-auto text-sm font-bold text-purple-900 dark:text-purple-300">{top.sonntag_pct}%</span>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-1">
        {data.fahrer.map(f => (
          <div key={f.fahrer_id} className="flex items-center gap-1 text-[10px] text-gray-600 dark:text-gray-400">
            ...
            <span className="font-medium">{f.sonntag_pct}%</span>
          </div>
        ))}
      </div>

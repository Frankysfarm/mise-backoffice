...
export function KitchenPhase2382ReaktionszeitTicker({ locationId }: { locationId?: string | null }) {
              ...
              {hasAlert && (
                <div className="flex items-start gap-2 bg-orange-100 border border-orange-200 rounded-lg p-2">
                  ...
                  <p className="text-xs text-orange-800">
                    {data.fahrer.filter(f => f.alert).map(f => f.name).join(', ')} — Reaktionszeit &gt;120s. Dispatcher informieren.
                  </p>
                </div>
              )}
              <div className="space-y-1">
                {data.fahrer.map(f => (
                  <div key={f.id} className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-semibold text-gray-800">{fmtSek(f.avg_sek)}</span>
                  </div>
                ))}
              </div>
            </>

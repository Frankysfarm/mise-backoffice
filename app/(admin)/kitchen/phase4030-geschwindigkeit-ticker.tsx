...
export function KitchenPhase4030GeschwindigkeitTicker({ locationId }: Props) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-emerald-600 font-medium">
            #{schnellster?.rang} {data.schnellster_name ?? schnellster?.fahrer_name} {schnellster?.avg_kmh} km/h
          </span>
        </div>
      </div>
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-1.5">
              ...
              <span className={`text-[10px] font-bold ${tColor}`}>{f.avg_kmh} km/h</span>
            </div>
          );
        })}
      </div>
      ...
      <div className="flex justify-between text-[10px] text-gray-400 px-0.5 pt-0.5 border-t border-gray-100">
        <span>Ø {data.team_avg_kmh} km/h</span>

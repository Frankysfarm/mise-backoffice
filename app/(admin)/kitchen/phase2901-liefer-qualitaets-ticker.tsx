...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd4', fahrer_name: 'Julia F.',  qualitaets_index: 93, sub_bewertung: 90, sub_puenktlichkeit: 92, sub_abschlussrate: 95, trend: 'steigend', alert_niedrig: false },
    { fahrer_id: 'd1', fahrer_name: 'Max M.',    qualitaets_index: 91, sub_bewertung: 95, sub_puenktlichkeit: 88, sub_abschlussrate: 97, trend: 'stabil',   alert_niedrig: false },
    { fahrer_id: 'd2', fahrer_name: 'Sara K.',   qualitaets_index: 75, sub_bewertung: 75, sub_puenktlichkeit: 70, sub_abschlussrate: 82, trend: 'fallend',  alert_niedrig: false },
    { fahrer_id: 'd3', fahrer_name: 'Tim B.',    qualitaets_index: 55, sub_bewertung: 50, sub_puenktlichkeit: 55, sub_abschlussrate: 60, trend: 'fallend',  alert_niedrig: true  },
  ],
  team_avg: 78,
  alert_count: 1,
};

export function KitchenPhase2901LieferQualitaetsTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className={`rounded-xl border p-3 mb-3 ${hasAlert ? 'border-red-300 bg-red-50' : 'border-gray-200 bg-white'}`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-xs text-gray-800">Liefer-Qualität</span>
            ...
            <div className="bg-red-100 border border-red-300 rounded-lg p-1.5 space-y-0.5">
              {alerts.map(f => (
                <div key={f.fahrer_id} className="flex items-center gap-1 text-[10px] text-red-700">
                  ...
                  <span>— Niedrige Lieferqualität! ({f.qualitaets_index} Pkt)</span>
                </div>
              ))}
            </div>
          )}
            ...
            {sorted.map(f => (
              <div key={f.fahrer_id} className="flex items-center justify-between text-[11px] text-gray-700">
                ...
                <div className="flex items-center gap-2 text-gray-500">
                  <span className="text-[10px]">★{f.sub_bewertung} ⏱{f.sub_puenktlichkeit} ✓{f.sub_abschlussrate}</span>
                  <span className="font-medium">{f.qualitaets_index} Pkt</span>
                </div>
              </div>
            ))}
          </div>

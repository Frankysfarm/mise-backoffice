...
import { Star, TrendingDown, TrendingUp, Minus, AlertTriangle, ChevronDown, ChevronUp } from 'lucide-react';
...
export function KitchenPhase3338KundenbewertungTicker({ locationId }: { locationId?: string | null }) {
        ...
        <div className="flex items-center gap-2">
          ...
          {bester && (
            <span className="text-xs text-gray-500 dark:text-gray-400">
              #1 {bester.fahrer_name} {bester.bewertung_avg} ★
            </span>
          )}
          {alerts.length > 0 && (
      ...
      {open && (
        <div className="p-3 space-y-2">
          ...
          {list.map(f => (
            <div key={f.fahrer_id} className="flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <span className={`text-sm font-bold ${ampelText(f.ampel)}`}>{f.bewertung_avg} ★</span>
                {f.rank_delta > 0
          ...
          <div className="pt-1 border-t border-gray-100 dark:border-gray-700 flex justify-between text-xs text-gray-400">
            <span>Team-Ø: {d.team_avg} ★ · Ziel ≥ 4.5 ★</span>

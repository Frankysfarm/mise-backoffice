...
export function KitchenPhase4390TourenProStundeTicker({ locationId }: Props) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-bold text-violet-700">
            Effizienz-König: {data.bester_name} — {bester?.touren_pro_stunde.toFixed(2)} t/h
          </span>
        </div>
        <div className="flex items-center gap-1.5">
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold text-gray-900">{f.touren_pro_stunde.toFixed(2)} t/h</span>
            </div>
          );
        })}
      </div>

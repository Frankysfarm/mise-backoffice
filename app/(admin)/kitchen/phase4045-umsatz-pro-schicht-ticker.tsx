...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 1, umsatz_pro_schicht: 285, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  rang: 2, umsatz_pro_schicht: 241, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f3', fahrer_name: 'Max M.',   rang: 3, umsatz_pro_schicht: 198, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, umsatz_pro_schicht: 143, ampel: 'rot',   alert_bottom: true  },
  ],
  team_avg: 217,
  bester_name: 'Julia F.',
  alert_count: 1,
...
export function KitchenPhase4045UmsatzProSchichtTicker({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-gray-900">Umsatz/Schicht</span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-emerald-600 font-medium">
            #{bester?.rang} {data.bester_name ?? bester?.fahrer_name} {bester?.umsatz_pro_schicht} €
          </span>
        </div>
      </div>
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-1.5">
              ...
              <span className={`text-[10px] font-bold ${tColor}`}>{f.umsatz_pro_schicht} €</span>
            </div>
          );
        })}
      </div>
      ...
      <div className="flex justify-between text-[10px] text-gray-400 px-0.5 pt-0.5 border-t border-gray-100">
        <span>Ø {data.team_avg} €/Schicht</span>
        <span>Ziel ≥200 €</span>
      </div>
    </div>
  );

...
export function KitchenPhase4938KmEffizienzTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-emerald-700 bg-emerald-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">km-Effizienz — Effizientester</div>
        <div className="text-sm font-bold text-emerald-100 truncate">
          #{top?.rang} {top?.fahrer_name} — {top?.km_pro_tour.toFixed(1)} km/Tour
        </div>
        <div className="text-xs text-gray-500">Team-Ø: {data.team_avg_km.toFixed(1)} km · Ziel &lt;5,5 km</div>
      </div>
      {data.alert_count > 0 && (

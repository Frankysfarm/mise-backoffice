...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; stopps_pro_km: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_bottom: boolean; }
interface ApiData { fahrer: FahrerRow[]; team_avg: number; bester_name: string; alert_count: number; }

const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 1, stopps_pro_km: 0.85, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f2', fahrer_name: 'Max M.',   rang: 2, stopps_pro_km: 0.71, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, stopps_pro_km: 0.54, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, stopps_pro_km: 0.32, ampel: 'rot',   alert_bottom: true  },
  ],
  team_avg: 0.61,
  bester_name: 'Julia F.',
  alert_count: 1,
...
export function KitchenPhase4135LieferdichteTicker({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-gray-900">Lieferdichte</span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-emerald-600 font-medium">
            #{bester?.rang} {data.bester_name ?? bester?.fahrer_name} {bester?.stopps_pro_km?.toFixed(2)}
          </span>
        </div>
      </div>
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-1.5">
              ...
              <span className={`text-[10px] font-bold ${tColor}`}>{f.stopps_pro_km?.toFixed(2)} Stkm</span>
            </div>
          );
        })}
      </div>

...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f2', fahrer_name: 'Jana F.',   storno_quote_heute: 21.4, storno_quote_gestern: 19.0, trend: 'steigend', ampel: 'rot',   alert: true  },
    { fahrer_id: 'f3', fahrer_name: 'Sarah K.',  storno_quote_heute: 18.5, storno_quote_gestern: 15.2, trend: 'steigend', ampel: 'rot',   alert: true  },
    { fahrer_id: 'f4', fahrer_name: 'Lena S.',   storno_quote_heute:  8.7, storno_quote_gestern:  9.5, trend: 'fallend',  ampel: 'gelb',  alert: false },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',    storno_quote_heute:  3.2, storno_quote_gestern:  4.1, trend: 'fallend',  ampel: 'gruen', alert: false },
    { fahrer_id: 'f5', fahrer_name: 'Tom B.',    storno_quote_heute:  2.1, storno_quote_gestern:  2.5, trend: 'fallend',  ampel: 'gruen', alert: false },
  ],
  team_avg_heute: 10.8, team_avg_gestern: 10.1, alert_count: 2,
};

export function KitchenPhase2583StornoQuoteTrendTicker({ locationId }: { locationId?: string | null }) {
  ...
  const teamColor   = data.team_avg_heute > 15 ? 'text-red-600' : data.team_avg_heute > 5 ? 'text-amber-600' : 'text-green-600';

  const teamTrend   = data.team_avg_gestern !== null
    ? data.team_avg_heute - data.team_avg_gestern
    : 0;
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-3`}>
      ...
      >
        <div className="flex items-center gap-1.5">
          <XCircle size={14} className={hasAlert ? 'text-red-500' : 'text-green-600'} />
          <span className="font-semibold text-xs text-gray-800">Storno-Quote-Trend</span>
          {hasAlert && (
        ...
        <div className="flex items-center gap-1.5">
          {teamTrend > 0 ? <TrendingUp size={12} className="text-red-500" /> : teamTrend < 0 ? <TrendingDown size={12} className="text-green-600" /> : <Minus size={12} className="text-gray-400" />}
          <span className={`text-xs font-bold ${teamColor}`}>Ø {data.team_avg_heute.toFixed(1)}%</span>
            ...
            {sorted.map(f => (
              <div key={f.fahrer_id} className="flex items-center gap-1.5 py-0.5">
                ...
                <span className={`text-xs font-semibold ${textColor(f.ampel)}`}>
                  {f.storno_quote_heute.toFixed(1)}%
                </span>
              </div>
            ))}
          </div>

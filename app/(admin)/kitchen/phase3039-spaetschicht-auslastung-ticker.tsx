...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   auslastung_pct: 88.0, trend: 'steigend', ampel: 'gruen', alert_gering: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', auslastung_pct: 82.1, trend: 'stabil',   ampel: 'gruen', alert_gering: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  auslastung_pct: 74.5, trend: 'fallend',  ampel: 'gelb',  alert_gering: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   auslastung_pct: 57.2, trend: 'fallend',  ampel: 'rot',   alert_gering: true  },
  ],
  team_avg_pct: 75.5,
  alert_count: 1,
};

export function KitchenPhase3039SpaetschichtAuslastungTicker({ locationId }: { locationId?: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-spaetschicht-auslastung?location_id=${locationId}`)
        .then(r => r.json())
  ...
  const teamAmpelCls = data.team_avg_pct >= ZIEL_PCT ? 'text-green-600' : data.team_avg_pct >= ALERT_PCT ? 'text-amber-600' : 'text-red-600';
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Spätschicht-Auslastung</span>
          ...
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-1.5 text-xs">
              ...
              <span className="text-red-700 dark:text-red-300 font-medium">
                {f.fahrer_name}: Geringe Auslastung! ({f.auslastung_pct.toFixed(1)} %)
              </span>
            </div>
          ))}

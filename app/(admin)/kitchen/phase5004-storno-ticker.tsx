...
export function KitchenPhase5004StornoTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-red-700 bg-red-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Storno-Quote-Ranking — Bester</div>
        <div className="text-sm font-bold text-red-100 truncate">
          #{top?.rang} {top?.fahrer_name} — {top?.storno_pct}%
        </div>
        <div className="text-xs text-gray-500">Team-Ø: {data.team_avg_pct}% · Ziel ≤5%</div>
      </div>
      {data.alert_count > 0 && (

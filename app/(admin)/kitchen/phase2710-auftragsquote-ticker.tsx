...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', quote: 1.2, trend: 'fallend',  alert: 'Auftragsquote zu niedrig!' },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   quote: 2.0, trend: 'stabil',   alert: null },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  quote: 3.1, trend: 'fallend',  alert: null },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   quote: 4.2, trend: 'steigend', alert: null },
  ],
  team_avg_quote: 2.6,
};

export function KitchenPhase2710AuftragsquoteTicker({ locationId }: { locationId?: string | null }) {
  ...
  const sorted    = [...enriched].sort((a, b) => a.quote - b.quote); // kritischste oben
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-3`}>
      ...
      >
        <div className="flex items-center gap-2">
          <Zap size={15} className={hasAlert ? 'text-red-500' : 'text-green-600'} />
          ...
          <span className="text-[10px] text-gray-400 ml-1">Ziel ≥{ZIEL}/h</span>
        </div>
          ...
          {sorted.map((f: FahrerEntry & { ampel: string }) => (
            <div key={f.fahrer_id} className="flex items-center justify-between py-1 border-b border-gray-100 last:border-0">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${textCls(f.ampel)}`}>{f.quote.toFixed(1)}/h</span>
              </div>
            </div>
          ))}
        </div>

...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; km_pro_stopp: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_top: boolean; }
interface ApiData { fahrer: FahrerRow[]; team_avg: number; bester_name: string; alert_count: number; }

const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 1, km_pro_stopp: 1.2, ampel: 'gruen', alert_top: false },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  rang: 2, km_pro_stopp: 1.8, ampel: 'gruen', alert_top: false },
    { fahrer_id: 'f3', fahrer_name: 'Max M.',   rang: 3, km_pro_stopp: 2.5, ampel: 'gelb',  alert_top: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, km_pro_stopp: 3.8, ampel: 'rot',   alert_top: true  },
  ],
  team_avg: 2.33,
  bester_name: 'Julia F.',
  alert_count: 1,
...
export function KitchenPhase4140KmProStoppTicker({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-gray-900">km/Stopp</span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-blue-600 font-medium">
            #{bester?.rang} {data.bester_name ?? bester?.fahrer_name} {bester?.km_pro_stopp?.toFixed(1)}km
          </span>
        </div>
      </div>
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-1.5">
              ...
              <span className={`text-[10px] font-bold ${tColor}`}>{f.km_pro_stopp?.toFixed(1)} km</span>
            </div>
          );
        })}
      </div>

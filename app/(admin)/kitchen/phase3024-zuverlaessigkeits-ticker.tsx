...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',  abschluss_pct: 98, lieferungen_heute: 20, trend: 'steigend', ampel: 'gruen', alert_niedrig: false },
    { fahrer_id: 'f2', fahrer_name: 'Lisa F.', abschluss_pct: 95, lieferungen_heute: 18, trend: 'steigend', ampel: 'gruen', alert_niedrig: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.', abschluss_pct: 88, lieferungen_heute: 15, trend: 'fallend',  ampel: 'gelb',  alert_niedrig: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',  abschluss_pct: 80, lieferungen_heute: 12, trend: 'fallend',  ampel: 'rot',   alert_niedrig: true  },
  ],
  team_durchschnitt: 90.3,
  alert_count: 1,
};

export function KitchenPhase3024ZuverlaessigkeitsTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Zuverlässigkeit</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-2">
          ...
          <div className="text-xs text-gray-400 dark:text-gray-500 pt-1">
            Ziel ≥{ZIEL_PCT}% · steigend=grün · 30-Min-Polling
          </div>
        </div>
      )}
    </div>

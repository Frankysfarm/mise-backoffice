...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f3', fahrer_name: 'Tim W.',  verspaetung_min: 14.0, trend: 'steigend' },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.', verspaetung_min:  5.0, trend: 'steigend' },
    { fahrer_id: 'f4', fahrer_name: 'Anna B.', verspaetung_min:  3.0, trend: 'fallend'  },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',  verspaetung_min:  1.5, trend: 'fallend'  },
  ],
  team_durchschnitt: 5.9,
  alert_count: 1,
};

export function KitchenPhase2650SchichtPuenktlichkeitTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-3`}>
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-semibold text-xs text-gray-800">Schichtpünktlichkeit</span>
          {hasAlert && (
          ...
          {hasAlert && (
            <div className="flex items-start gap-1.5 bg-red-100 border border-red-200 rounded-lg px-2 py-1.5">
              ...
              <p className="text-xs text-red-700 font-medium">
                Schicht-Verspätung: {alerts.map((f: { fahrer_name: string }) => f.fahrer_name).join(', ')}
              </p>
            </div>
          )}
            ...
            {sorted.map(f => (
              <div key={f.fahrer_id} className="flex items-center gap-1.5 py-0.5">
                ...
                <span className={`text-xs font-semibold ${textColor(f.ampel)}`}>
                  {f.verspaetung_min.toFixed(1)} Min
                </span>
              </div>
            ))}
          </div>

...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; avg_min: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_bottom: boolean; }
interface ApiData { fahrer: FahrerRow[]; team_avg_min: number; bester_name: string; alert_count: number; }

const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   rang: 1, avg_min: 12, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', rang: 2, avg_min: 15, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, avg_min: 22, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, avg_min: 35, ampel: 'rot',   alert_bottom: true  },
  ],
  team_avg_min: 21,
  bester_name: 'Max M.',
  alert_count: 1,
...
export function KitchenPhase4075LieferzeitTicker({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-gray-900">Lieferzeit</span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-blue-600 font-medium">
            #{bester?.rang} {data.bester_name ?? bester?.fahrer_name} {bester?.avg_min}min
          </span>
        </div>
      </div>
      ...
      <div className="flex justify-between text-[10px] text-gray-400 px-0.5 pt-0.5 border-t border-gray-100">
        <span>Ø {data.team_avg_min} min</span>

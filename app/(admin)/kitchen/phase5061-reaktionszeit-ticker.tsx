...
export function KitchenPhase5061ReaktionszeitTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-violet-700 bg-violet-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      <Timer className="w-4 h-4 text-violet-300 shrink-0" />
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Reaktionszeit-Ranking — Schnellste</div>
        <div className="text-sm font-bold text-violet-100 truncate">
          #{top?.rang} {top?.fahrer_name} — {top?.avg_reaktionszeit_min} min
        </div>
        <div className="text-xs text-gray-500">Team-Ø: {data.team_avg_min} min · Ziel ≤7 min</div>
      </div>
      {data.alert_count > 0 && (

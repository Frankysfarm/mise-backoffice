...
export function KitchenPhase3204ErstlieferzeitTicker({ locationId }: { locationId?: string | null }) {
          ...
          <span className="font-semibold text-xs text-gray-800 dark:text-gray-100 truncate">
            ...
            {bester && (
              <span className="ml-1 font-normal text-green-600 dark:text-green-400">
                — Bester: {bester.fahrer_name} ({bester.erstlieferzeit_min} Min)
              </span>
            )}
          </span>
        ...
        <div className="px-4 py-3 space-y-1.5">
          ...
          {list.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 text-xs py-0.5">
              ...
              <span className="font-semibold text-gray-800 dark:text-gray-100 shrink-0">{f.erstlieferzeit_min} Min</span>
          ...
          <div className="pt-1 flex items-center gap-1.5 text-xs text-gray-400">
            ...
            <span>Ø {data?.team_avg_min ?? 0} Min Team · Rang 1 = kürzeste Erstlieferzeit</span>
          </div>
        </div>
      )}
    </div>

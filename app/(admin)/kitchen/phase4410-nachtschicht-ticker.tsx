...
export function KitchenPhase4410NachtschichtTicker({ locationId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/delivery/admin/fahrer-nacht-ranking?location_id=${locationId}`);
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-bold text-violet-700">
            Nacht-Owl: {data.meister_name} — {bester?.nacht_anteil_pct.toFixed(1)}%
          </span>
        </div>
        <div className="flex items-center gap-1.5">
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold text-gray-900">{f.nacht_anteil_pct.toFixed(1)}%</span>
            </div>
          );
        })}
      </div>
      ...
      <div className="flex items-center justify-between text-[10px] text-gray-400 pt-0.5 border-t border-gray-100">
        ...
        <span>#1 = höchster Nacht-Anteil</span>
      </div>
    </div>
  );
}

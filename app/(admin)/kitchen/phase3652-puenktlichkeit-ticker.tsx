...
export function KitchenPhase3652PuenktlichkeitTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center gap-2">
        ...
        {puenktlichster && (
          <span className="ml-auto text-sm font-bold text-blue-700 truncate max-w-[130px]">
            {puenktlichster.fahrer_name} · {puenktlichster.puenktlichkeit_pct}%
          </span>
        )}
      </div>
      ...
      <div className="space-y-1">
        {sorted.map((f, i) => (
          <div key={f.fahrer_id} className="flex items-center gap-2 text-xs">
            ...
            <span className={`font-bold w-12 text-right ${f.ampel === 'gruen' ? 'text-blue-600' : f.ampel === 'gelb' ? 'text-orange-500' : 'text-red-600'}`}>
      ...
      <div className="text-xs text-gray-400 text-center">
        Team-Ø {data.team_avg_pct.toFixed(1)}% · Ziel ≥90%
      </div>
    </div>
  );
}

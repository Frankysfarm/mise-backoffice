...
export function KitchenPhase3697BestellungenProStoppTicker({ locationId }: { locationId: string | null }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center gap-2">
        ...
        {bester && (
          <span className="ml-auto text-sm font-bold text-emerald-700 truncate max-w-[140px]">
            {bester.fahrer_name} · {bester.bestellungen_pro_stopp.toFixed(1)}
          </span>
        )}
      </div>
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-1 px-2 py-1 bg-red-50 border border-red-200 rounded text-xs text-red-700">
          ...
          <span>{data.alert_count} Fahrer mit wenigen Bestellungen/Stopp!</span>
        </div>
      )}
        ...
        {sorted.map((f, i) => (
          <div key={f.fahrer_id} className="flex items-center gap-2 text-xs">
            ...
            <span className={`font-bold w-10 text-right ${f.ampel === 'gruen' ? 'text-emerald-600' : f.ampel === 'gelb' ? 'text-yellow-600' : 'text-red-600'}`}>
              {f.bestellungen_pro_stopp.toFixed(1)}
            </span>
            {f.rank_delta > 0 ? (
      ...
      <div className="text-xs text-gray-400 text-center">
        Team-Ø {data.team_avg.toFixed(1)} · Ziel ≥1.5 Bestellungen/Stopp
      </div>
    </div>
  );
}

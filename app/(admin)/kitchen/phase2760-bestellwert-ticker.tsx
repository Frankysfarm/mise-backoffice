...
function TrendIcon({ trend }: { trend: string }) {
  // steigend = höherer Wert = besser (grün), fallend = schlechter (rot)
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   avg_bestellwert_eur: 32.5, trend: 'steigend', alert: null                    },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  avg_bestellwert_eur: 21.8, trend: 'fallend',  alert: null                    },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   avg_bestellwert_eur: 18.4, trend: 'steigend', alert: null                    },
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', avg_bestellwert_eur: 11.2, trend: 'fallend',  alert: 'Niedriger Bestellwert!' },
  ],
  team_avg_bestellwert_eur: 20.9,
};

export function KitchenPhase2760BestellwertTicker({ locationId }: { locationId?: string | null }) {
  ...
  const enriched  = data.fahrer.map((f: FahrerEntry) => ({ ...f, ampel: calcAmpel(f.avg_bestellwert_eur) }));
  // absteigend: höchster Bestellwert (bester) oben
  const sorted    = [...enriched].sort((a, b) => b.avg_bestellwert_eur - a.avg_bestellwert_eur);
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-3`}>
      ...
      >
        <div className="flex items-center gap-2">
          <ShoppingBag size={15} className={hasAlert ? 'text-red-500' : 'text-indigo-600'} />
          <span className="font-semibold text-sm text-gray-800">Ø Bestellwert/Tour</span>
          {hasAlert && (
          ...
          <span className={`ml-1 text-xs font-bold ${textCls(teamAmpel)}`}>
            Ø {data.team_avg_bestellwert_eur}€
          </span>
          ...
          {sorted.map((f: FahrerEntry & { ampel: string }) => (
            <div key={f.fahrer_id} className="flex items-center justify-between py-1 border-b border-gray-100 last:border-0">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${textCls(f.ampel)}`}>{f.avg_bestellwert_eur}€</span>
              </div>
            </div>
          ))}
        </div>

...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', frequenz: 0.8, trend: 'fallend',  alert: 'Frequenz zu niedrig!' },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   frequenz: 1.2, trend: 'stabil',   alert: null },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  frequenz: 1.5, trend: 'stabil',   alert: null },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   frequenz: 2.1, trend: 'steigend', alert: null },
  ],
  team_avg_frequenz: 1.4,
};

export function KitchenPhase2730TourenFrequenzTicker({ locationId }: { locationId?: string | null }) {
  ...
  // aufsteigend = niedrigste Frequenz (kritischste) oben
  const sorted    = [...enriched].sort((a, b) => a.frequenz - b.frequenz);
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-3`}>
      ...
      >
        <div className="flex items-center gap-2">
          <Zap size={15} className={hasAlert ? 'text-red-500' : 'text-blue-600'} />
          ...
          {sorted.map((f: FahrerEntry & { ampel: string }) => (
            <div key={f.fahrer_id} className="flex items-center justify-between py-1 border-b border-gray-100 last:border-0">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${textCls(f.ampel)}`}>{f.frequenz.toFixed(1)}/h</span>
              </div>
            </div>
          ))}
        </div>

...
function DeltaIcon({ delta }: { delta: number }) {
  // For reaction time: negative = faster today = improved
...
export function KitchenPhase3144TourstartReaktionszeitTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">
            Tourstart-Reaktion · #1 {bester?.fahrer_name ?? data?.bester_name ?? '—'} ({bester ? `${bester.avg_min} Min` : '—'})
          </span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-2">
            ...
            {list.map(f => (
              <div key={f.fahrer_id} className="flex items-center justify-between text-xs py-1 border-b border-gray-100 dark:border-gray-800 last:border-0">
                ...
                <div className="flex items-center gap-1">
                  <span className="font-semibold text-gray-700 dark:text-gray-200">{f.avg_min} Min</span>
          ...
          <div className="text-xs text-gray-400 pt-1">Team-Ø {teamAvg} Min · Ziel Top 25% (schnellste Reaktion)</div>
        </div>
      )}
    </div>
  );

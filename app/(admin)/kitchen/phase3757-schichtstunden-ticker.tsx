...
export function KitchenPhase3757SchichtstundenTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      {/* Header mit Fleißigster #1 */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Clock className="w-4 h-4 text-blue-500" />
          <span className="text-sm font-semibold text-gray-900">Schichtstunden</span>
        ...
        {best && (
          <div className="flex items-center gap-1 text-xs">
            ...
            <span className="font-black text-emerald-600">{best.avg_stunden}h</span>
          </div>
        )}
      </div>
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-red-50 border border-red-200 rounded-lg text-[11px] text-red-700">
          ...
          <span>Wenige Schichtstunden!</span>
        </div>
      )}

      {/* Kompakt-Liste (absteigend: meiste Stunden = bester = Rang 1) */}
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5 text-xs">
                ...
                <span className={`font-bold ${tColor}`}>{f.avg_stunden}h</span>
                {f.rank_delta !== 0 && (
                  f.rank_delta < 0

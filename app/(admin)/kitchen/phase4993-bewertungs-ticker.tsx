...
export function KitchenPhase4993BewertungsTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-amber-700 bg-amber-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Bewertungs-Ranking — Champion</div>
        <div className="text-sm font-bold text-amber-100 truncate">
          #{top?.rang} {top?.fahrer_name} — {top?.avg_rating} ★
        </div>
        <div className="text-xs text-gray-500">Team-Ø: {data.team_avg_rating} ★ · Ziel ≥4.5 ★</div>
      </div>
      {data.alert_count > 0 && (

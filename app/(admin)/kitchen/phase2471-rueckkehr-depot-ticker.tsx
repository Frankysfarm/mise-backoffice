...
export function KitchenPhase2471RueckkehrDepotTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className={`border rounded-xl mb-2 overflow-hidden ${headerColor}`}>
      ...
      >
        <span className="flex items-center gap-1.5">
          <Clock size={12} />
          Depot-ETA
          <span className="font-bold">{data.team_avg_eta_min.toFixed(1)} min</span>
          {hasAlerts && (
          ...
          {hasAlerts && (
            <div className="flex items-start gap-1.5 bg-red-50 border border-red-200 rounded px-2 py-1 text-xs text-red-700">
              ...
              <span>
                ...
                {data.fahrer.filter(f => f.alert_verspaetung).map(f => f.fahrer_name).join(', ')} — über 30 min
              </span>
            </div>
          )}
          <div className="space-y-1">
            {data.fahrer.map(f => (
              <div key={f.fahrer_id} className="flex items-center gap-1.5 text-xs">
                ...
                <span className="font-semibold text-gray-600">{f.rueckkehr_eta_min} min</span>
              </div>
            ))}
          </div>
        </div>

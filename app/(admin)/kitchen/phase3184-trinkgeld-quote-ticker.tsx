...
export function KitchenPhase3184TrinkgeldQuoteTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div style={{ background: '#1a1a2e', border: '1px solid #2d2d4e', borderRadius: 10, padding: 12, marginBottom: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
        ...
        {best && (
          <span style={{ color: '#eab308', fontWeight: 700, fontSize: 13 }}>
            #{1} {best.driver_name} — {best.trinkgeld_quote.toFixed(1)}%
          </span>
        )}
        {hasAlert && (
          <span style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 4 }}>
            <AlertTriangle size={12} color="#fca5a5" />
            <span style={{ color: '#fca5a5', fontSize: 11 }}>Niedrige Trinkgeld-Quote!</span>
          </span>
        )}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {data.fahrer.map(f => (
          <div key={f.driver_id} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ color: '#ddd', fontSize: 12, width: 44, textAlign: 'right' }}>{f.trinkgeld_quote.toFixed(1)}%</span>
      ...
      <div style={{ marginTop: 8, color: '#888', fontSize: 11, textAlign: 'right' }}>Team-Ø: {data.team_avg.toFixed(1)}%</div>
    </div>
  );
}

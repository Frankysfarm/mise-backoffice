...
export function KitchenPhase5072SchichtPuenktlichkeitsTicker({ locationId }: { locationId: string | null }) {
  const [data, setData] = useState<ApiResponse | null>(null);

  async function load() {
    const url = locationId
      ? `/api/delivery/admin/fahrer-schicht-puenktlichkeit-ranking?location_id=${locationId}`
      : '/api/delivery/admin/fahrer-schicht-puenktlichkeit-ranking?location_id=mock';
  ...
  return (
    <div className="rounded-xl border border-green-700 bg-green-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Schicht-Pünktlichkeit — Höchste</div>
        <div className="text-sm font-bold text-green-100 truncate">
          #{top?.rang} {top?.fahrer_name} — {top?.puenktlichkeit_pct}%
        </div>
        <div className="text-xs text-gray-500">Team-Ø: {data.team_avg_pct}% · Ziel ≥80%</div>
      </div>
      {data.alert_count > 0 && (

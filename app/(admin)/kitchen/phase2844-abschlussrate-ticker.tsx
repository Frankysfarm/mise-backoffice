...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd1', fahrer_name: 'Anna K.',  rate_pct: 100, abgeschlossen: 10, gesamt: 10, ampel: 'gruen', trend: 'stabil',   alert: false },
    { fahrer_id: 'd2', fahrer_name: 'Max M.',   rate_pct:  97, abgeschlossen: 11, gesamt: 11, ampel: 'gruen', trend: 'steigend', alert: false },
    { fahrer_id: 'd3', fahrer_name: 'Sara K.',  rate_pct:  88, abgeschlossen:  7, gesamt:  8, ampel: 'gelb',  trend: 'fallend',  alert: false },
    { fahrer_id: 'd4', fahrer_name: 'Tim W.',   rate_pct:  75, abgeschlossen:  6, gesamt:  8, ampel: 'rot',   trend: 'fallend',  alert: true  },
  ],
  team_avg_pct: 90.0,
  alert_count: 1,
};

export function KitchenPhase2844AbschlussrateTicker({ locationId }: { locationId?: string | null }) {
        ...
        <div className="mt-2 space-y-2">
          {alerts.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-1 text-[10px] text-red-700 bg-red-100 rounded px-2 py-1">
              ...
              <span>— Niedrige Abschlussrate! ({f.rate_pct.toFixed(1)}%)</span>
            </div>
          ))}
          ...
          {sorted.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 py-1 border-b border-gray-100 last:border-0">
              ...
              <span className={`text-xs font-semibold ${textCls(f.ampel)}`}>{f.rate_pct.toFixed(1)}%</span>
            </div>
          ))}

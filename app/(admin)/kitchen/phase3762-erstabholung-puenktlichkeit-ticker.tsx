...
export function KitchenPhase3762ErstabholungPuenktlichkeitTicker({ locationId }: { locationId: string | null }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Erstabholung-Pünktlichkeit</span>
          {loading && <span className="w-2.5 h-2.5 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin" />}
        </div>
        {best && (
          <div className="flex items-center gap-1 text-xs">
            <span className="text-gray-500">🥇</span>
            <span className="font-bold text-gray-800">{best.fahrer_name}</span>
            <span className="font-black text-emerald-600">{best.rate_pct}%</span>
          </div>
        )}
      </div>
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5 text-xs">
                ...
                <span className={`font-bold ${tColor}`}>{f.rate_pct}%</span>
                {f.rank_delta !== 0 && (
                  f.rank_delta < 0

...
export function KitchenPhase4230StornoquoteTicker({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-gray-900">Storno-Quote</span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-green-600 font-medium">
            #{bester?.rang} {data.bester_name ?? bester?.fahrer_name} {bester?.stornoquote_pct}%
          </span>
        </div>
      </div>
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-1.5">
              ...
              <span className={`text-[10px] font-bold ${tColor}`}>{f.stornoquote_pct}%</span>
            </div>
          );
        })}
      </div>

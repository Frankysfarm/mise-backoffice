...
export function KitchenPhase2402TourenAnzahlTicker({ locationId }: { locationId?: string | null }) {
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={`font-semibold text-sm ${hasAlert ? 'text-orange-800' : 'text-blue-800'}`}>
            Touren-Anzahl {data ? `— Ø ${data.team_avg_touren.toFixed(1)} heute` : ''}
          </span>
          {hasAlert && (
                ...
                <div className="flex items-start gap-2 bg-orange-100 border border-orange-200 rounded-lg p-2">
                  ...
                  <p className="text-xs text-orange-800">
                    {data.fahrer.filter(f => f.alert_low).length > 0 &&
                      `Unterauslastung (<4): ${data.fahrer.filter(f => f.alert_low).map(f => f.fahrer_name).join(', ')}. `}
                    {data.fahrer.filter(f => f.alert_high).length > 0 &&
                      `Überbelastung (>12): ${data.fahrer.filter(f => f.alert_high).map(f => f.fahrer_name).join(', ')}.`}
                  </p>
                </div>
              )}
              <div className="space-y-1">
                {data.fahrer.map(f => (
                  <div key={f.fahrer_id} className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-semibold text-gray-800">{f.touren_heute} Touren</span>
                  </div>
                ))}
              </div>
            </>

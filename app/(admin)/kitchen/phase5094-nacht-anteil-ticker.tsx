...
export function KitchenPhase5094NachtAnteilTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-violet-700 bg-violet-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      <Moon className="w-4 h-4 text-violet-300 shrink-0" />
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Nacht-Anteil-Ranking — Meiste Nachtschichten</div>
        <div className="text-sm font-bold text-violet-100 truncate">
          #{top?.rang} {top?.fahrer_name} — {top?.nacht_anteil_pct}%
        </div>
        <div className="text-xs text-gray-500">Team-Ø: {data.team_avg_pct}% Nachtschichten</div>
      </div>
      {data.alert_count > 0 && (

...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', dichte: 0.11, trend: 'fallend',  alert: 'Lieferdichte zu gering!' },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   dichte: 0.21, trend: 'stabil',   alert: null },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  dichte: 0.31, trend: 'fallend',  alert: null },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   dichte: 0.42, trend: 'steigend', alert: null },
  ],
  team_avg_dichte: 0.26,
};

export function KitchenPhase2715LieferdichteTicker({ locationId }: { locationId?: string | null }) {
  ...
  const sorted    = [...enriched].sort((a, b) => a.dichte - b.dichte); // kritischste oben
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-3`}>
      ...
      >
        <div className="flex items-center gap-2">
          <MapPin size={15} className={hasAlert ? 'text-red-500' : 'text-green-600'} />
          ...
          <span className={`ml-1 text-xs font-bold ${textCls(teamAmpel)}`}>
            Ø {data.team_avg_dichte.toFixed(2)}/km
          </span>
          <span className="text-[10px] text-gray-400 ml-1">Ziel ≥{ZIEL}/km</span>
        </div>
        ...
        <div className="px-4 pb-3 space-y-1.5">
          {alerts.map((f: FahrerEntry & { ampel: string }) => (
            <div key={f.fahrer_id} className="flex items-center gap-1.5 rounded bg-red-100 border border-red-200 px-2 py-1 text-xs text-red-700 font-medium">
              ...
              <span className="font-bold">{f.fahrer_name}</span>: {f.alert} ({f.dichte.toFixed(2)}/km)
            </div>
          ))}
          ...
          {sorted.map((f: FahrerEntry & { ampel: string }) => (
            <div key={f.fahrer_id} className="flex items-center justify-between py-1 border-b border-gray-100 last:border-0">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold ${textCls(f.ampel)}`}>{f.dichte.toFixed(2)}/km</span>
              </div>
            </div>
          ))}
        </div>

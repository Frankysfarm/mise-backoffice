...
export function KitchenPhase5014UmsatzProStundeTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-cyan-700 bg-cyan-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      <Euro className="w-4 h-4 text-cyan-300 shrink-0" />
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Umsatz/h-Ranking — Bester</div>
        <div className="text-sm font-bold text-cyan-100 truncate">
          #{top?.rang} {top?.fahrer_name} — {top?.umsatz_pro_stunde} €/h
        </div>
        <div className="text-xs text-gray-500">Team-Ø: {data.team_avg_umsatz} €/h · Ziel ≥40 €/h</div>
      </div>
      {data.alert_count > 0 && (

...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f2', fahrer_name: 'Sarah K.',  erloes_heute: 78.30,  erloes_gestern: 165.40, trend: 'schlechter', ampel: 'rot',   alert: true  },
    { fahrer_id: 'f5', fahrer_name: 'Jana F.',   erloes_heute: 62.40,  erloes_gestern: 183.70, trend: 'schlechter', ampel: 'rot',   alert: true  },
    { fahrer_id: 'f3', fahrer_name: 'Lena S.',   erloes_heute: 142.60, erloes_gestern: 138.90, trend: 'stabil',     ampel: 'gelb',  alert: false },
    { fahrer_id: 'f4', fahrer_name: 'Tom B.',    erloes_heute: 215.80, erloes_gestern: 229.10, trend: 'stabil',     ampel: 'gruen', alert: false },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',    erloes_heute: 248.50, erloes_gestern: 211.20, trend: 'besser',     ampel: 'gruen', alert: false },
  ],
  team_avg_heute: 149.52, team_avg_gestern: 185.66, ziel: 200, alert_count: 2,
};

export function KitchenPhase2608SchichtErloesTicker({ locationId }: { locationId?: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-schicht-erloes?location_id=${locationId}`)
  ...
  const sorted     = [...data.fahrer].sort((a, b) => a.erloes_heute - b.erloes_heute);
  ...
  const teamColor  = data.team_avg_heute >= data.ziel ? 'text-green-600' : data.team_avg_heute >= 100 ? 'text-amber-600' : 'text-red-600';
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-3`}>
      ...
      >
        <div className="flex items-center gap-1.5">
          <Euro size={14} className={hasAlert ? 'text-red-500' : 'text-green-600'} />
          <span className="font-semibold text-xs text-gray-800">Schicht-Erlös</span>
          {hasAlert && (
        ...
        <div className="flex items-center gap-1.5">
          <span className={`text-xs font-bold ${teamColor}`}>Ø {data.team_avg_heute.toFixed(0)} €</span>
          ...
          {hasAlert && (
            <div className="flex items-start gap-1.5 bg-red-100 border border-red-200 rounded-lg px-2 py-1.5">
              ...
              <p className="text-xs text-red-700 font-medium">
                Fahrer Umsatz niedrig: {alertNames.join(', ')}
              </p>
            </div>
          )}

...
export function KitchenPhase4611SonntagAbendTicker({ locationId }: { locationId: string | null }) {
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Sonntagabend-Ticker nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-purple-200 dark:border-purple-800 bg-white dark:bg-gray-900 p-3 space-y-2">
      <div className="flex items-center gap-1.5">
        <Star className="w-4 h-4 text-purple-500" />
        <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">Sonntagabend-Ticker</span>
      </div>

      {top && (
        <div className="flex items-center gap-2">
          <span className="text-lg font-extrabold text-purple-600 dark:text-purple-400">#1</span>
          <span className="text-sm font-semibold text-gray-800 dark:text-gray-200 truncate">{top.fahrer_name}</span>
          <span className="ml-auto text-sm font-bold text-purple-500">{top.sonntag_pct}%</span>
        </div>
      )}

...
export function KitchenPhase2445WochenendTicker({ locationId }: { locationId?: string | null }) {
      ...
      {open && (
        <div className="bg-white px-4 pb-3 pt-2 space-y-2">
          ...
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-500 text-xs">Team-Ø Wochenend-Schicht</span>
            <span className={`font-bold text-base ${data.team_avg_wochenend_h > 12 ? 'text-red-600' : data.team_avg_wochenend_h > 8 ? 'text-amber-600' : 'text-teal-600'}`}>
              {fmtH(data.team_avg_wochenend_h)}
            </span>
          </div>
          ...
          {hasAlerts && (
            <div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-xs text-red-700">
              ...
              <span>
                ...
                {data.fahrer.filter(f => f.alert_ueberlastung).map(f => f.fahrer_name).join(', ')} — &gt;12h am Wochenende!
              </span>
            </div>
          )}
          ...
          <div className="space-y-1">
            {data.fahrer.map(f => (
              <div key={f.fahrer_id} className="flex items-center gap-2 text-xs py-0.5">
                ...
                <span className={`font-semibold ${f.alert_ueberlastung ? 'text-red-600' : f.wochenend_h > 8 ? 'text-amber-600' : 'text-teal-600'}`}>
                  {fmtH(f.wochenend_h)}
                </span>
              </div>
            ))}

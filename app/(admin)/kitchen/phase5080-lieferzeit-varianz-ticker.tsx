...
export function KitchenPhase5080LieferzeitVarianzTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-indigo-700 bg-indigo-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      <BarChart2 className="w-4 h-4 text-indigo-300 shrink-0" />
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Lieferzeit-Varianz-Ranking — Konsistenteste</div>
        <div className="text-sm font-bold text-indigo-100 truncate">
          #{top?.rang} {top?.fahrer_name} — ±{top?.stddev_min} min
        </div>
        <div className="text-xs text-gray-500">Team-Ø: ±{data.team_avg_stddev} min · Ziel ≤8 min</div>
      </div>
      {data.alert_count > 0 && (

...
import {
  ...
  Thermometer, BarChart2, RefreshCw, Activity, Target, Layers,
} from 'lucide-react';

// Phase 5056 — Smart Timing Countdown V34
// Neu: Batch-Queue-Prognose-Balken; Multi-Station Sync-Score; Cook-Duration-Heatmap;
// Peak-Alarm-Band; Kochstart-Empfehlung je Station; 15-Sek-Polling; Mock-Fallback
...
const MOCK: ApiData = {
  ...
  aktive_batches: 3,
  kritische_anzahl: 2,
  batch_prognose_min: 6,
  batch_queue_laenge: 5,
  sync_score_gesamt: 84,
  peak_alarm: null,
  alert: null,
  heatmap: [
    { stunde: '11h', orders: 4,  avg_cook_dauer: 11 },
    { stunde: '12h', orders: 12, avg_cook_dauer: 14 },
    { stunde: '13h', orders: 18, avg_cook_dauer: 16 },
    { stunde: '14h', orders: 10, avg_cook_dauer: 13 },
    { stunde: '15h', orders: 8,  avg_cook_dauer: 12 },
    { stunde: '16h', orders: 14, avg_cook_dauer: 15 },
  ],
  stationen: [
    { name: 'Ofen',  auslastung: 92, temp_status: 'kritisch', aktive_orders: 3, sync_score: 78, kochstart_empfehlung: 'Jetzt starten — Fahrer in 8 min' },
    { name: 'Grill', auslastung: 68, temp_status: 'heiss',    aktive_orders: 2, sync_score: 90, kochstart_empfehlung: null },
    { name: 'Kalt',  auslastung: 35, temp_status: 'kalt',     aktive_orders: 1, sync_score: 95, kochstart_empfehlung: null },
    { name: 'Herd',  auslastung: 58, temp_status: 'warm',     aktive_orders: 2, sync_score: 82, kochstart_empfehlung: 'Batch B2 vorbereiten' },
  ],
  bestellungen: [
    { id: '1', nr: '#0061', artikel: 'Pizza Salami',     minuten_verbleibend: 2,  ziel_min: 12, station: 'Ofen',  batch_id: 'B1', status: 'kritisch', prioritaet: 1, cook_dauer_ist: 10, cook_dauer_ziel: 12 },
    { id: '2', nr: '#0062', artikel: 'Burger Double',    minuten_verbleibend: 4,  ziel_min: 10, station: 'Grill', batch_id: null, status: 'kritisch', prioritaet: 1, cook_dauer_ist: 6,  cook_dauer_ziel: 10 },
    { id: '3', nr: '#0063', artikel: 'Pasta Bolognese',  minuten_verbleibend: 9,  ziel_min: 14, station: 'Herd',  batch_id: 'B2', status: 'warnung',  prioritaet: 2, cook_dauer_ist: 5,  cook_dauer_ziel: 14 },
    { id: '4', nr: '#0064', artikel: 'Caprese Salat',    minuten_verbleibend: 7,  ziel_min: 8,  station: 'Kalt',  batch_id: null, status: 'warnung',  prioritaet: 2, cook_dauer_ist: 1,  cook_dauer_ziel: 8 },
    { id: '5', nr: '#0065', artikel: 'Schnitzel Wiener', minuten_verbleibend: 13, ziel_min: 16, station: 'Grill', batch_id: 'B1', status: 'ok',       prioritaet: 3, cook_dauer_ist: 3,  cook_dauer_ziel: 16 },
    { id: '6', nr: '#0066', artikel: 'Döner Teller',     minuten_verbleibend: 0,  ziel_min: 14, station: 'Grill', batch_id: 'B3', status: 'fertig',   prioritaet: 1, cook_dauer_ist: 14, cook_dauer_ziel: 14 },
  ],
};
...
export function KitchenPhase5056SmartTimingCountdownV34({ locationId }: { locationId?: string | null }) {
    ...
    <div className="rounded-2xl border border-orange-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-orange-700 text-white">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-sm">Smart Timing V34</span>
        </div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-5 gap-2">
          {[
            ...
            { label: 'Velocity',  val: `${d.schicht_velocity}/h`,color: 'text-violet-600' },
            ...
            <div key={kpi.label} className="rounded-xl border border-border bg-muted/20 p-2 text-center">
              <div className={`text-base font-black ${kpi.color}`}>{kpi.val}</div>
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-1 text-xs font-semibold text-muted-foreground mb-1">
            <Thermometer className="h-3.5 w-3.5" />Stationen · Auslastung & Sync-Score
          </div>
          {d.stationen.map((st) => (

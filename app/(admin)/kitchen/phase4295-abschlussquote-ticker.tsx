...
export function KitchenPhase4295AbschlussquoteTicker({ locationId }: Props) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-bold text-green-700">
            Bester: {data.bester_name} — {bester?.quote_pct}%
          </span>
        </div>
        <div className="flex items-center gap-1.5">
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold text-gray-900">{f.quote_pct}%</span>
            </div>
          );
        })}
      </div>

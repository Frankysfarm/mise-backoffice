...
export function KitchenPhase3867KmProStoppTicker({ locationId }: { locationId: string | null }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">km/Stopp</span>
        ...
        {best && (
          <div className="flex items-center gap-1 text-xs">
            ...
            <span className="font-black text-emerald-700">{best.km_pro_stopp} km</span>
          </div>
        )}
      </div>
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-red-50 border border-red-200 rounded-lg text-[11px] text-red-700">
          ...
          <span>Hohe km/Stopp!</span>
        </div>
      )}

      {/* Kompakt-Liste (aufsteigend: wenigste km = Rang 1 = bester) */}
      ...
      <div className="flex items-center justify-between text-[10px] text-gray-400 border-t border-gray-100 pt-1.5">
        <span>Team-Ø {data.team_avg} km</span>
        <span>Ziel ≤1.5 km</span>
      </div>
    </div>
  );
}

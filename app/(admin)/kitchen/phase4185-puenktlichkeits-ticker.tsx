...
export function KitchenPhase4185PuenktlichkeitsTicker({ locationId }: Props) {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-violet-600 font-medium">
            #{bester?.rang} {data.bester_name ?? bester?.fahrer_name} {bester?.rate_pct?.toFixed(0)}%
          </span>
        </div>
      </div>
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-1.5">
              ...
              <span className={`text-[10px] font-bold ${tColor}`}>{f.rate_pct?.toFixed(0)} %</span>
            </div>
          );
        })}
      </div>

...
interface ApiResponse {
  fahrer: { fahrer_name: string; storno_reaktions_score: number; rang: number }[];
  team_avg_score: number;
  beste_name: string;
  alert_count: number;
}

export function KitchenPhase4721StornoreaktionsTicker({ locationId }: { locationId: string | null }) {
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Storno-Reaktions-Ticker nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-rose-200 dark:border-rose-900 bg-white dark:bg-gray-900 p-3 space-y-2">
      <div className="flex items-center gap-1.5">
        <ShieldCheck className="w-4 h-4 text-rose-800 dark:text-rose-400" />
        <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">Storno-Reaktions-Ticker</span>
      </div>
      ...
      <div className="flex items-center gap-2">
        <div className="bg-rose-50 dark:bg-rose-950 rounded-lg px-3 py-1.5 flex items-baseline gap-1">
          ...
          <span className="text-lg font-extrabold text-rose-800 dark:text-rose-300">{leader?.storno_reaktions_score}</span>
        </div>
      ...
      <div className="flex items-center justify-between text-[10px] text-gray-500 dark:text-gray-400 border-t border-gray-100 dark:border-gray-800 pt-1.5">
        <span>Team-Ø: <span className="font-bold text-gray-700 dark:text-gray-300">{data.team_avg_score.toFixed(1)}</span></span>

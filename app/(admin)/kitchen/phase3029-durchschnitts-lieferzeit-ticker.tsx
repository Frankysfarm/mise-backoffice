...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   avg_min: 22.5, trend: 'fallend',  ampel: 'gruen', alert_zu_lang: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', avg_min: 24.1, trend: 'stabil',   ampel: 'gruen', alert_zu_lang: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  avg_min: 27.8, trend: 'steigend', ampel: 'gelb',  alert_zu_lang: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   avg_min: 38.2, trend: 'steigend', ampel: 'rot',   alert_zu_lang: true  },
  ],
  team_avg_min: 28.2,
  alert_count: 1,
};

export function KitchenPhase3029DurchschnittsLieferzeitTicker({ locationId }: { locationId?: string | null }) {
  ...
  const teamAmpelCls = data.team_avg_min <= ZIEL_MIN ? 'text-green-600' : data.team_avg_min <= ALERT_MIN ? 'text-amber-600' : 'text-red-600';
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Ø Lieferzeit</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-2">
          ...
          <div className="text-xs text-gray-400 dark:text-gray-500 pt-1">
            Ziel ≤{ZIEL_MIN} Min · fallend=grün · 30-Min-Polling
          </div>
        </div>
      )}
    </div>

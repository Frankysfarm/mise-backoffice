...
import { AlertTriangle, ChevronDown, ChevronUp, Minus, TrendingDown, TrendingUp, XCircle } from 'lucide-react';
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 1, storno_rate: 2.1,  rank_delta: -1, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  rang: 2, storno_rate: 5.0,  rank_delta:  0, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f3', fahrer_name: 'Max M.',   rang: 3, storno_rate: 12.5, rank_delta:  1, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, storno_rate: 33.3, rank_delta:  0, ampel: 'rot',   alert_bottom: true  },
  ],
  team_avg_rate: 13.2,
  bester_name: 'Julia F.',
  alert_count: 1,
...
export function KitchenPhase3199StornoTicker({ locationId }: { locationId?: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-storno-ranking?location_id=${locationId}`)
        .then(r => r.json())
          ...
          <span className="font-semibold text-xs text-gray-800 dark:text-gray-100 truncate">
            ...
            {bester && (
              <span className="ml-1 font-normal text-green-600 dark:text-green-400">
                — Bester: {bester.fahrer_name} ({bester.storno_rate}%)
              </span>
            )}
          </span>
        ...
        <div className="px-4 py-3 space-y-1.5">
          {alerts.length > 0 && (
            <div className="flex items-center gap-1.5 rounded bg-red-50 border border-red-200 text-red-700 dark:text-red-300 dark:bg-red-950 dark:border-red-800 text-xs font-semibold px-2 py-1.5 mb-2">
              ...
              {alerts.map(f => f.fahrer_name).join(', ')} — Hohe Storno-Rate!
            </div>
          )}
          ...
          {list.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 text-xs py-0.5">
              ...
              <span className="font-semibold text-gray-800 dark:text-gray-100 shrink-0">{f.storno_rate}%</span>
          ...
          <div className="pt-1 flex items-center gap-1.5 text-xs text-gray-400">
            ...
            <span>Ø {data?.team_avg_rate ?? 0}% Team · Rang 1 = niedrigste Rate</span>
          </div>
        </div>
      )}
    </div>

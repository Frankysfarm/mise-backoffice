...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', auslastung_pct: 91.5, trend: 'steigend', ampel: 'gruen', alert_gering: false },
    { fahrer_id: 'f2', fahrer_name: 'Max M.',   auslastung_pct: 84.7, trend: 'stabil',   ampel: 'gruen', alert_gering: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  auslastung_pct: 72.3, trend: 'fallend',  ampel: 'gelb',  alert_gering: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   auslastung_pct: 55.0, trend: 'fallend',  ampel: 'rot',   alert_gering: true  },
  ],
  team_avg_pct: 75.9,
  alert_count: 1,
};

export function KitchenPhase3034FruehschichtAuslastungTicker({ locationId }: { locationId?: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-fruehschicht-auslastung?location_id=${locationId}`)
        .then(r => r.json())
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Frühschicht-Auslastung</span>

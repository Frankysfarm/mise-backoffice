...
export function KitchenPhase4845WochenendTicker({ locationId }: { locationId: string | null }) {
    ...
    <div className="rounded-xl border border-purple-800 bg-purple-950/40 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-sm font-bold text-purple-300 truncate">
          #{top?.rang} {top?.fahrer_name} — {top?.wochenend_anteil_pct.toFixed(1)}% Sa/So-Touren
        </div>
        <div className="text-xs text-gray-500">Team-Ø: {data.team_avg_pct.toFixed(1)}%</div>
      </div>
      {data.alert_count > 0 && (
        <div className="flex items-center gap-1 text-xs text-purple-300 shrink-0">

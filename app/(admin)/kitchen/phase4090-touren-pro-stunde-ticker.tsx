...
export function KitchenPhase4090TourenProStundeTicker({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-gray-900">Touren/h</span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-green-700 font-medium">
            #{bester?.rang} {data.bester_name ?? bester?.fahrer_name} {bester?.touren_pro_stunde?.toFixed(1)}/h
          </span>
        </div>
      </div>
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-1.5">
              ...
              <span className={`text-[10px] font-bold ${tColor}`}>{f.touren_pro_stunde?.toFixed(1)}/h</span>
            </div>
          );
        })}
      </div>
      <div className="flex justify-between text-[10px] text-gray-400 px-0.5 pt-0.5 border-t border-gray-100">
        <span>Ø {data.team_avg?.toFixed(1)} tours/h</span>
        <span>#1 = meiste Touren/h</span>
      </div>
    </div>
  );
}

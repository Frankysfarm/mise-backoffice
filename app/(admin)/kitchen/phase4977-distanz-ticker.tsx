...
export function KitchenPhase4977DistanzTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-emerald-700 bg-emerald-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Distanz-Ranking — Champion</div>
        <div className="text-sm font-bold text-emerald-100 truncate">
          #{top?.rang} {top?.fahrer_name} — {top?.distanz_gesamt}km
        </div>
        <div className="text-xs text-gray-500">Team-Ø: {data.team_avg_distanz}km · Ziel ≥500km/Monat</div>
      </div>
      {data.alert_count > 0 && (

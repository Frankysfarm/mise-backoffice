...
export function KitchenPhase3672BestellungenProTagTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center gap-2">
        ...
        <span className="font-semibold text-gray-900 text-sm">Bestellungen/Tag</span>
        {data.alert_count > 0 && (
      ...
      {bester && (
        <div className="bg-purple-50 rounded-lg px-3 py-2 flex items-center justify-between">
          ...
          <span className="text-lg font-black text-purple-600">{bester.bestellungen_pro_tag.toFixed(1)}/Tag</span>
        </div>
      )}

      {data.alert_count > 0 && (
        <div className="text-xs text-red-600 flex items-center gap-1">
          <AlertTriangle className="w-3 h-3" /> Wenige Bestellungen/Tag!
        </div>
      )}
        ...
        {sorted.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-2 text-xs">
              ...
              <span className={`font-semibold ${AMPEL_COLOR[f.ampel]}`}>{f.bestellungen_pro_tag.toFixed(1)}/Tag</span>
      ...
      <div className="flex justify-between text-xs text-gray-400 border-t pt-2">
        <span>Team-Ø: {data.team_avg.toFixed(1)}/Tag</span>
        <span>Ziel ≥10/Tag</span>
      </div>
    </div>
  );
}

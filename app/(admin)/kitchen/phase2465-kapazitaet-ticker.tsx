...
export function KitchenPhase2465KapazitaetTicker({ locationId }: { locationId?: string | null }) {
          ...
          {hasAlerts && (
            <div className="flex items-start gap-1.5 bg-red-50 border border-red-200 rounded px-2 py-1 text-xs text-red-700">
              ...
              <span>
                ...
                {data.fahrer.filter(f => f.alert_unterauslastung).map(f => f.fahrer_name).join(', ')} — unter 60%
              </span>
            </div>
          )}
          <div className="space-y-1">
            {data.fahrer.map(f => (
              <div key={f.fahrer_id} className="flex items-center gap-1.5 text-xs">
                ...
                <span className="font-semibold text-gray-600">{f.kapazitaet_score.toFixed(0)}%</span>
              </div>
            ))}
          </div>
        </div>

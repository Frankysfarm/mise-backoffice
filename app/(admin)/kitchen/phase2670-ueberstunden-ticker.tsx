...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   ueberstunden_min: 67, trend: 'steigend' },
    { fahrer_id: 'f5', fahrer_name: 'Jonas W.', ueberstunden_min: 51, trend: 'steigend' },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  ueberstunden_min: 32, trend: 'steigend' },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   ueberstunden_min: 12, trend: 'stabil'   },
    { fahrer_id: 'f4', fahrer_name: 'Lisa F.',  ueberstunden_min:  8, trend: 'fallend'  },
  ],
  team_avg_ueberstunden: 34,
};

export function KitchenPhase2670UeberstundenTicker({ locationId }: { locationId?: string | null }) {
  ...
  const sorted    = [...enriched].sort((a, b) => b.ueberstunden_min - a.ueberstunden_min);
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-4`}>
      ...
      >
        <div className="flex items-center gap-2">
          <Clock size={15} className={hasAlert ? 'text-red-500' : 'text-gray-500'} />
          <span className="font-semibold text-sm text-gray-800">Überstunden</span>
          <span className={`text-xs font-bold ${textCls(teamAmpel)}`}>
            Ø {data.team_avg_ueberstunden} Min
          </span>
          {hasAlert && (
        ...
        <div className="px-4 pb-3 space-y-2">
          ...
          {hasAlert && (
            <div className="flex items-start gap-2 bg-red-100 border border-red-200 rounded-lg px-3 py-1.5">
              ...
              <p className="text-xs text-red-700 font-medium">
                Überstunden zu hoch! {alerts.map(a => a.fahrer_name).join(', ')}
              </p>
            </div>
          )}
          ...
          {sorted.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              ...
              <span className={`text-xs font-bold ${textCls(f.ampel)}`}>{f.ueberstunden_min} Min</span>
            </div>
          ))}

          {/* Ziel */}
          <p className="text-xs text-gray-400 pt-1">Ziel ≤15 Min/Fahrer</p>
        </div>
      )}
    </div>
  );

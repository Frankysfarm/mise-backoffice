...
export function KitchenPhase2481StoppzeitTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className={`border rounded-xl mb-2 overflow-hidden ${headerColor}`}>
      ...
      >
        <span className="flex items-center gap-1.5">
          <MapPin size={12} />
          Stoppzeit
          <span className="font-bold">{data.team_avg_stoppzeit_min.toFixed(1)} min</span>
          {hasAlerts && (
        ...
        <div className="bg-white px-3 pb-2 pt-1 space-y-2">
          {hasAlerts && (
            <div className="flex items-start gap-1.5 bg-red-50 border border-red-200 rounded px-2 py-1 text-xs text-red-700">
              ...
              <span>
                <strong>Zu lang an der Adresse:</strong>{' '}
          ...
          <div className="space-y-1">
            {data.fahrer.map(f => (
              <div key={f.fahrer_id} className="flex items-center gap-1.5 text-xs">
                ...
                <span className="font-semibold text-gray-600">{f.avg_stoppzeit_min.toFixed(1)} min</span>
              </div>
            ))}
          </div>
        </div>

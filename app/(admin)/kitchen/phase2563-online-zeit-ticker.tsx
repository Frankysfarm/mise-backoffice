...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f5', fahrer_name: 'Jana F.',  effizienz_pct: 31, ampel: 'rot',   alert: true  },
    { fahrer_id: 'f3', fahrer_name: 'Lena S.',  effizienz_pct: 38, ampel: 'rot',   alert: true  },
    { fahrer_id: 'f2', fahrer_name: 'Sarah K.', effizienz_pct: 55, ampel: 'gelb',  alert: false },
    { fahrer_id: 'f4', fahrer_name: 'Tom B.',   effizienz_pct: 64, ampel: 'gruen', alert: false },
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   effizienz_pct: 72, ampel: 'gruen', alert: false },
  ],
  team_avg_pct: 52,
  alert_count: 2,
};
...
export function KitchenPhase2563OnlineZeitTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-700/60 bg-red-900/20' : 'border-matcha-700/50 bg-matcha-900/50'} overflow-hidden mb-3`}>
      ...
      >
        <div className="flex items-center gap-2">
          <Clock size={14} className={hasAlert ? 'text-red-400' : 'text-matcha-400'} />
          <span className="text-[11px] font-black uppercase tracking-widest text-matcha-300">
            Online-Zeit-Effizienz
          </span>
          {hasAlert && (
          ...
          {hasAlert && (
            <div className="flex items-center gap-2 rounded-lg bg-red-900/40 border border-red-700/60 px-2.5 py-1.5">
              ...
              <p className="text-[10px] text-red-300 font-semibold">
                Effizienz &lt;40%: {alertFahrer.join(', ')} — Leerlauf prüfen!
              </p>
            </div>
          )}
          ...
          <div className="space-y-1">
            {sorted.map(f => (
              <div key={f.fahrer_id} className="flex items-center gap-2 text-[10px]">
                ...
                <span className={`font-mono font-bold w-8 text-right ${f.alert ? 'text-red-400' : 'text-matcha-300'}`}>
                  {f.effizienz_pct}%
                </span>
              </div>
            ))}

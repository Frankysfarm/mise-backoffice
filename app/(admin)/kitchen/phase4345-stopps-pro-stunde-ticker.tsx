...
interface FahrerRow { fahrer_id: string; fahrer_name: string; rang: number; stopps_pro_stunde: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_bottom: boolean; }
interface ApiData { fahrer: FahrerRow[]; team_avg: number; bester_name: string; alert_count: number; }

const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'm1', fahrer_name: 'Julia F.', rang: 1, stopps_pro_stunde: 3.2, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'm2', fahrer_name: 'Sara K.',  rang: 2, stopps_pro_stunde: 2.8, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'm3', fahrer_name: 'Max M.',   rang: 3, stopps_pro_stunde: 2.1, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'm4', fahrer_name: 'Tim B.',   rang: 4, stopps_pro_stunde: 1.4, ampel: 'rot',   alert_bottom: true  },
  ],
  team_avg: 2.38,
  bester_name: 'Julia F.',
  alert_count: 1,
...
export function KitchenPhase4345StoppsProStundeTicker({ locationId }: Props) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-gray-900">Stopps/h</span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-teal-700 font-medium">
            #{bester?.rang} {data.bester_name ?? bester?.fahrer_name} {bester?.stopps_pro_stunde?.toFixed(1)}/h
          </span>
        </div>
      </div>
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-1.5">
              ...
              <span className={`text-[10px] font-bold ${tColor}`}>{f.stopps_pro_stunde?.toFixed(1)}/h</span>
            </div>
          );
        })}
      </div>
      <div className="flex justify-between text-[10px] text-gray-400 px-0.5 pt-0.5 border-t border-gray-100">
        <span>Ø {data.team_avg?.toFixed(1)} stopps/h</span>
        <span>#1 = meiste Stopps/h</span>
      </div>
    </div>
  );
}

...
export function KitchenPhase3109PuenktlichkeitsTicker({ locationId }: { locationId?: string | null }) {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-puenktlichkeits-ranking?location_id=${locationId}`)
        .then(r => r.json())
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">
            Pünktlichkeit · #1 {bester?.fahrer_name ?? data?.bester_name ?? '—'} ({bester ? `${bester.rate_pct}%` : '—'})
          </span>
        ...
        <div className="px-4 py-3 space-y-2">
          ...
          {alerts.length > 0 && (
            <div className="flex items-center gap-2 rounded-lg bg-red-50 border border-red-200 text-red-700 text-xs font-semibold px-3 py-2">
              ...
              {alerts.map(f => f.fahrer_name).join(', ')} — Niedrigste Pünktlichkeitsrate!
            </div>
          )}
            ...
            {list.map(f => (
              <div key={f.fahrer_id} className="flex items-center justify-between text-xs py-1 border-b border-gray-100 dark:border-gray-800 last:border-0">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-semibold text-gray-700 dark:text-gray-200">{f.rate_pct}%</span>
          ...
          <div className="text-xs text-gray-400 pt-1">Team-Ø {teamAvg}% · Ziel Top 25% (≥90%)</div>
        </div>
      )}
    </div>
  );

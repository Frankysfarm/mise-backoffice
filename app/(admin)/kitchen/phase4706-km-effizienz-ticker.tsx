...
interface ApiResponse {
  fahrer: { fahrer_name: string; avg_eur_pro_km: number; rang: number }[];
  team_avg_eur_pro_km: number;
  beste_name: string;
  alert_count: number;
}

export function KitchenPhase4706KmEffizienzTicker({ locationId }: { locationId: string | null }) {
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">KM-Effizienz-Ticker nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-violet-200 dark:border-violet-900 bg-white dark:bg-gray-900 p-3 space-y-2">
      <div className="flex items-center gap-1.5">
        <Route className="w-4 h-4 text-violet-800 dark:text-violet-400" />
        <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">KM-Effizienz-Ticker</span>
      </div>

      <div className="flex items-center gap-2">
        <div className="bg-violet-50 dark:bg-violet-950 rounded-lg px-3 py-1.5 flex items-baseline gap-1">
          <span className="text-[9px] text-violet-700 dark:text-violet-400">#1</span>
          <span className="text-lg font-extrabold text-violet-800 dark:text-violet-300">{leader?.avg_eur_pro_km.toFixed(2)} €/km</span>
        </div>
      ...
      <div className="flex items-center justify-between text-[10px] text-gray-500 dark:text-gray-400 border-t border-gray-100 dark:border-gray-800 pt-1.5">
        <span>Team-Ø: <span className="font-bold text-gray-700 dark:text-gray-300">{data.team_avg_eur_pro_km.toFixed(2)} €/km</span></span>

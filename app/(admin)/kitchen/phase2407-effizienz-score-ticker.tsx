...
export function KitchenPhase2407EffizienzScoreTicker({ locationId }: { locationId?: string | null }) {
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={`font-semibold text-sm ${hasAlert ? 'text-orange-800' : 'text-indigo-800'}`}>
            Effizienz-Score {data ? `— Team-Ø ${data.team_avg_score}/100` : ''}
          </span>
          {hasAlert && (
              ...
              {hasAlert && (
                <div className="flex items-start gap-2 bg-orange-100 border border-orange-200 rounded-lg p-2">
                  ...
                  <p className="text-xs text-orange-800">
                    Score &lt;50: {data.fahrer.filter(f => f.alert).map(f => f.fahrer_name).join(', ')} — Coaching empfohlen.
                  </p>
                </div>
              )}
              <div className="space-y-1">
                {data.fahrer.map(f => (
                  <div key={f.fahrer_id} className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-semibold text-gray-800">{f.score}/100</span>
                  </div>
                ))}
              </div>
            </>

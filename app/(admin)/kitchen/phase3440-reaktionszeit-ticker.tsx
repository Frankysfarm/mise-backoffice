...
export function KitchenPhase3440ReaktionszeitTicker({ locationId }: { locationId?: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-reaktionszeit-zuweisung?location_id=${locationId}`, { cache: 'no-store' });
    ...
    <div className="border rounded-lg bg-white dark:bg-gray-900 shadow-sm mb-2">
      ...
      >
        <span className="flex items-center gap-2 font-semibold text-xs">
          ...
          Reaktionszeit — Schnellster: {data.bester_name}
          {data.alert_count > 0 && (
        ...
        <div className="px-2.5 pb-2.5 space-y-1">
          {data.alert_count > 0 && (
            <div className="flex items-center gap-1 text-[10px] text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 border border-red-200 rounded px-2 py-1">
              ...
              Hohe Reaktionszeit! {data.alert_count} Fahrer über Zielwert
            </div>
          )}

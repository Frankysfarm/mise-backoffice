...
import { Clock, TrendingDown, TrendingUp, Minus, AlertTriangle, ChevronDown, ChevronUp } from 'lucide-react';
...
export function KitchenPhase3324AvgLieferzeitTicker({ locationId }: { locationId?: string | null }) {
        ...
        <div className="flex items-center gap-2">
          ...
          {schnellster && (
            <span className="text-xs text-gray-500 dark:text-gray-400">
              #1 {schnellster.fahrer_name} {schnellster.avg_min} min
            </span>
          )}
          {alerts.length > 0 && (
          ...
          {list.map(f => (
            <div key={f.fahrer_id} className="flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <span className={`text-sm font-bold ${ampelText(f.ampel)}`}>{f.avg_min} min</span>
                {f.rank_delta < 0

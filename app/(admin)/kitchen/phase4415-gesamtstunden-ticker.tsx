...
export function KitchenPhase4415GesamtstundenTicker({ locationId }: Props) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-bold text-blue-700">
            Fleißigster: {data.fleissigster_name} — {bester?.gesamt_stunden.toFixed(1)}h
          </span>
        </div>
        <div className="flex items-center gap-1.5">
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold text-gray-900">{f.gesamt_stunden.toFixed(1)}h</span>
            </div>
          );
        })}
      </div>

...
export function KitchenPhase3607StornoQuoteTicker({ locationId }: { locationId: string | null }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-900">Storno-Quote Ticker</span>
        </div>
        <div className="text-xs text-gray-500">
          <span className="font-bold text-emerald-700">#1 {bester?.fahrer_name}</span>
          <span className="ml-1">{bester?.quote_pct.toFixed(1) ?? '–'}%</span>
        </div>
      </div>
      ...
      <div className="text-xs text-gray-400 text-center">Team-Ø: {data.team_durchschnitt.toFixed(1)}% · Ziel &lt;5%</div>
    </div>
  );
}

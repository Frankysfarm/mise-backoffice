...
export function KitchenPhase3947TrinkgeldBetragTicker({ locationId }: { locationId: string | null }) {
  ...
  const best = sorted[0];
  const maxVal = Math.max(...sorted.map(f => f.avg_tip_eur), 1);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      ...
      <div className="flex items-center justify-between">
        ...
        {best && (
          <div className="flex items-center gap-1 text-xs">
            ...
            <span className="font-black text-gray-700">{best.avg_tip_eur?.toFixed(2)}€</span>
          </div>
        )}
      </div>
      ...
      {/* Kompakt-Liste (absteigend: hoechstes Trinkgeld = Rang 1 = bester) */}

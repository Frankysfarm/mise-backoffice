...
export function KitchenPhase3982TrinkgeldTicker({ locationId }: { locationId: string | null }) {
  ...
  const best = sorted[0];
  const maxVal = Math.max(...sorted.map(f => f.avg_tip_eur), 0.01);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      ...
      <div className="flex items-center justify-between">
        ...
        {best && (
          <div className="flex items-center gap-1 text-xs">
            ...
            <span className="font-black text-gray-700">{fmt(best.avg_tip_eur)}</span>
          </div>
        )}
      </div>
      ...
      {/* Kompakt-Liste (absteigend: höchstes Tip = Rang 1 = bester) */}
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5 text-xs">
                ...
                <span className={`font-bold ${tColor}`}>{fmt(f.avg_tip_eur)}</span>
                {DeltaIcon}
              </div>
              <div className="h-1 bg-gray-100 rounded-full overflow-hidden ml-5">
                <div className={`h-full rounded-full ${barColor}`} style={{ width: `${(f.avg_tip_eur / maxVal) * 100}%` }} />
              </div>
            </div>
          );
        })}
      ...
      <div className="flex items-center justify-between text-[10px] text-gray-400 border-t border-gray-100 pt-1.5">
        <span>Team-Ø {fmt(data.team_avg_eur)}</span>
        <span>Ø pro Lieferung</span>
      </div>
    </div>
  );
}

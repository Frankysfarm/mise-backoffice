...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   rang: 1, stopps_pro_stunde: 4.2, rank_delta:  0, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f2', fahrer_name: 'Julia F.', rang: 2, stopps_pro_stunde: 3.5, rank_delta: -1, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'f3', fahrer_name: 'Sara K.',  rang: 3, stopps_pro_stunde: 2.8, rank_delta:  1, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, stopps_pro_stunde: 1.5, rank_delta: -1, ampel: 'rot',   alert_bottom: true  },
  ],
...
export function KitchenPhase3099StoppEffizienzTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">
            Stopp-Effizienz · #1 {bester?.fahrer_name ?? data?.bester_name ?? '—'} ({bester?.stopps_pro_stunde?.toFixed(1) ?? '—'} Stp/h)
          </span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-2">
            ...
            {list.map(f => (
              <div key={f.fahrer_id} className="flex items-center justify-between text-xs py-1 border-b border-gray-100 dark:border-gray-800 last:border-0">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-semibold text-gray-700 dark:text-gray-200">{f.stopps_pro_stunde.toFixed(1)}</span>
          ...
          <div className="text-xs text-gray-400 pt-1">Team-Ø {teamAvg.toFixed(1)} Stp/h · Ziel Top 25%</div>
        </div>
      )}
    </div>
  );

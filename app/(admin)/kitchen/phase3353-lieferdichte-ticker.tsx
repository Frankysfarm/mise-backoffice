...
export function KitchenPhase3353LieferdichteTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          {bester && (
            <span className="text-xs text-violet-700 dark:text-violet-400 font-medium truncate">
              #{1} {bester.fahrer_name} {bester.stopps_pro_km.toFixed(2)} S/km
            </span>
          )}
          {alerts.length > 0 && (
        ...
        <div className="p-3 space-y-2">
          ...
          {list.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 text-xs">
              ...
              <span className={`font-bold tabular-nums ${textCls(f.ampel)}`}>{f.stopps_pro_km.toFixed(2)} S/km</span>
              {f.rank_delta > 0
          ...
          <div className="pt-1 border-t border-gray-100 dark:border-gray-700 flex justify-between text-xs text-gray-400">
            <span>Team-Ø: {d.team_avg.toFixed(2)} S/km</span>
            <span>Ziel: &gt;0.70 S/km</span>
          </div>
        </div>
      )}
    </div>

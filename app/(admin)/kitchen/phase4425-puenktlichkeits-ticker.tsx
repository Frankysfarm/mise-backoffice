...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  rang: 1, rate_pct: 94, ampel: 'gruen', alert_niedrig: false },
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 2, rate_pct: 89, ampel: 'gruen', alert_niedrig: false },
    { fahrer_id: 'f3', fahrer_name: 'Max M.',   rang: 3, rate_pct: 78, ampel: 'gelb',  alert_niedrig: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, rate_pct: 62, ampel: 'rot',   alert_niedrig: true  },
  ],
  team_avg_pct: 80.75,
  bester_name: 'Sara K.',
  alert_count: 1,
...
export function KitchenPhase4425PuenktlichkeitsTicker({ locationId }: Props) {
    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-bold text-gray-800">Pünktlichkeits-Ranking</span>
        </div>
        <div className="flex items-center gap-1.5">
      ...
      {bester && (
        <div className="bg-emerald-50 rounded-xl px-3 py-1.5 flex items-center justify-between">
          ...
          <span className="text-sm font-extrabold text-emerald-600 ml-2">{bester.rate_pct}%</span>
        </div>
      )}
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-semibold text-gray-800">{f.rate_pct}%</span>
            </div>
          );
        })}
      </div>

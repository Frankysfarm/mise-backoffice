...
export function KitchenPhase3254TrinkgeldTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-3 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-xs text-gray-800 dark:text-gray-100">Trinkgeld-Ticker</span>
          {bester && (
            <span className="text-xs text-yellow-700 dark:text-yellow-400 font-semibold ml-1">
              🏆 {bester.fahrer_name} {fmtEur(bester.avg_tip_eur)}
            </span>
          )}
          {d.alert_count > 0 && (
        ...
        <div className="px-3 pb-3 pt-2 space-y-1.5">
          ...
          {list.map(f => (
            <div key={f.fahrer_id} className="flex items-center justify-between text-xs py-1 border-b border-gray-100 dark:border-gray-800 last:border-0">
              ...
              <div className="flex items-center gap-1.5">
                <span className="font-bold text-gray-800 dark:text-gray-100">{fmtEur(f.avg_tip_eur)}</span>
          ...
          <div className="flex justify-between text-xs text-gray-400 pt-1">
            <span>Team-Ø: <span className="font-semibold text-blue-600 dark:text-blue-400">{fmtEur(d.team_avg_eur)}</span></span>
            <span>Ziel: möglichst hohes Trinkgeld</span>
          </div>
        </div>
      )}
    </div>

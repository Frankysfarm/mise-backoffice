...
import { Route, TrendingDown, TrendingUp, Minus, AlertTriangle, ChevronDown, ChevronUp } from 'lucide-react';
...
export function KitchenPhase3368KmProTourRankingTicker({ locationId }: { locationId?: string | null }) {
        ...
        <div className="flex items-center gap-2">
          ...
          {bester && (
            <span className="text-xs text-gray-500 dark:text-gray-400">
              #1 {bester.fahrer_name} {bester.km_avg.toFixed(1)} km
            </span>
          )}
          {alerts.length > 0 && (
        ...
        <div className="p-3 space-y-2">
          {alerts.length > 0 && (
            <div className="flex items-center gap-2 rounded-lg bg-red-50 border border-red-200 text-red-700 dark:text-red-300 dark:bg-red-950 dark:border-red-800 text-xs font-semibold px-3 py-2">
              ...
              {alerts.map(f => f.fahrer_name).join(', ')} — Hohe km pro Tour!
            </div>
          )}
          ...
          {list.map(f => (
            <div key={f.fahrer_id} className="flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <span className={`text-sm font-bold ${ampelText(f.ampel)}`}>{f.km_avg.toFixed(1)} km</span>
                {f.rank_delta < 0
          ...
          <div className="pt-1 border-t border-gray-100 dark:border-gray-700 flex justify-between text-xs text-gray-400">
            <span>Team-Ø: {d.team_avg.toFixed(1)} km · letzte 30 Tage</span>
          </div>
        </div>
      )}
    </div>

...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   touren_heute: 4, trend: 'steigend', ampel: 'gruen', alert: null },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  touren_heute: 3, trend: 'stabil',   ampel: 'gruen', alert: null },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   touren_heute: 2, trend: 'fallend',  ampel: 'gelb',  alert: null },
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', touren_heute: 1, trend: 'fallend',  ampel: 'rot',   alert: 'Zu wenige Touren!' },
  ],
  team_avg_touren: 2.5,
  alert_count: 1,
};

export function KitchenPhase2979TourenProTagTicker({ locationId }: { locationId?: string | null }) {
  ...
  const teamAmpel = data.team_avg_touren >= ZIEL_TOUREN ? 'gruen' : data.team_avg_touren >= 2 ? 'gelb' : 'rot';
    ...
    <div className={`rounded-xl border shadow-sm mb-4 overflow-hidden ${hasAlert ? 'border-red-300' : 'border-gray-200'} bg-white dark:bg-gray-900`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">
            Touren/Tag-Ticker
            <span className={`ml-2 font-black ${teamText}`}>{data.team_avg_touren.toFixed(1)}</span>
      ...
      {open && (
        <div className="px-4 py-3 space-y-2">
          ...
          <div className="text-xs text-gray-400 pt-1">Ziel ≥{ZIEL_TOUREN} Touren | 30-Min-Polling</div>
        </div>
      )}
    </div>
  );

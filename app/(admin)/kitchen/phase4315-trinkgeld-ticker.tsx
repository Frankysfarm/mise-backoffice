...
export function KitchenPhase4315TrinkgeldTicker({ locationId }: Props) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-bold text-amber-700">
            Bester: {data.bester_name} — {bester?.avg_tip_eur.toFixed(2)} €
          </span>
        </div>
        <div className="flex items-center gap-1.5">
        ...
        {data.fahrer.map((f) => {
          ...
          return (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold text-gray-900">{f.avg_tip_eur.toFixed(2)} €</span>
            </div>
          );
        })}
      </div>

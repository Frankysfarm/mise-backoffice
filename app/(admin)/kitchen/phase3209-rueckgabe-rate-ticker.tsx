...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Julia F.', rang: 1, rueckgabe_rate: 1.5,  rank_delta: -1, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  rang: 2, rueckgabe_rate: 3.2,  rank_delta:  0, ampel: 'gruen', alert_bottom: false },
    { fahrer_id: 'f3', fahrer_name: 'Max M.',   rang: 3, rueckgabe_rate: 9.1,  rank_delta:  1, ampel: 'gelb',  alert_bottom: false },
    { fahrer_id: 'f4', fahrer_name: 'Tim B.',   rang: 4, rueckgabe_rate: 22.2, rank_delta:  0, ampel: 'rot',   alert_bottom: true  },
  ],
  team_avg_rate: 9.0,
  bester_name: 'Julia F.',
  alert_count: 1,
...
export function KitchenPhase3209RueckgabeRateTicker({ locationId }: { locationId?: string | null }) {
          ...
          <span className="font-semibold text-xs text-gray-800 dark:text-gray-100 truncate">
            ...
            {bester && (
              <span className="ml-1 font-normal text-green-600 dark:text-green-400">
                — Bester: {bester.fahrer_name} ({bester.rueckgabe_rate}%)
              </span>
            )}
          </span>
        ...
        <div className="px-4 py-3 space-y-1.5">
          ...
          {list.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2 text-xs py-0.5">
              ...
              <span className="font-semibold text-gray-800 dark:text-gray-100 shrink-0">{f.rueckgabe_rate}%</span>
          ...
          <div className="pt-1 flex items-center gap-1.5 text-xs text-gray-400">
            ...
            <span>Ø {data?.team_avg_rate ?? 0}% Team · Rang 1 = niedrigste Quote</span>
          </div>
        </div>
      )}
    </div>

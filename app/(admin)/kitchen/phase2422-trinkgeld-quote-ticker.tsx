...
export function KitchenPhase2422TrinkgeldQuoteTicker({ locationId }: { locationId?: string | null }) {
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={`font-semibold text-sm ${hasAlert ? 'text-orange-800' : 'text-teal-800'}`}>
            Trinkgeld-Quote
            {data ? ` — Team-Ø ${data.team_avg_quote.toFixed(1)} %` : ''}
          </span>
          {hasAlert && (
                ...
                {data.fahrer.map(f => (
                  <div key={f.fahrer_id} className="flex items-center justify-between text-xs">
                    ...
                    <div className="flex items-center gap-2 text-gray-700">
                      <span>{f.trinkgeld_gesamt.toFixed(2)} €</span>
                      <span className="font-semibold">{f.trinkgeld_quote.toFixed(1)} %</span>
                    </div>
                  </div>
                ))}
              </div>

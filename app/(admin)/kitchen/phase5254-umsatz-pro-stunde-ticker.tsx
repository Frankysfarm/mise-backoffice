...
export function KitchenPhase5254UmsatzProStundeTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-violet-700 bg-violet-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Umsatz/Stunde — Meiste</div>
        <div className="text-sm font-bold text-violet-100 truncate">
          #{top.rang} {top.fahrer_name} — {top.umsatz_pro_stunde.toFixed(0)} €/h
        </div>
        <div className="text-[10px] text-gray-500 mt-0.5">
          Team-Ø: {data.team_avg_umsatz.toFixed(0)} €/h · {data.fahrer.length} Fahrer erfasst
          {data.alert_count > 0 && (

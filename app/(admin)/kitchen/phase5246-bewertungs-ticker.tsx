...
export function KitchenPhase5246BewertungsTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-yellow-700 bg-yellow-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Kundenbewertung — Beste</div>
        <div className="text-sm font-bold text-yellow-100 truncate">
          #{top.rang} {top.fahrer_name} — {top.avg_rating.toFixed(1)} ★
        </div>
        <div className="text-[10px] text-gray-500 mt-0.5">
          Team-Ø: {data.team_avg_rating.toFixed(1)} ★ · {data.gesamt} Fahrer erfasst
          {data.alert_count > 0 && (

...
export function KitchenPhase3992KmProTourTicker({ locationId }: { locationId: string | null }) {
  ...
  // ascending: lowest km_avg = Rang 1 = best
  const sorted = [...data.fahrer].sort((a, b) => a.rang - b.rang);
  const best = sorted[0];
  const maxVal = Math.max(...sorted.map(f => f.km_avg), 1);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      ...
      <div className="flex items-center justify-between">
        ...
        {best && (
          <div className="flex items-center gap-1 text-xs">
            ...
            <span className="font-black text-gray-700">{best.km_avg} km</span>
          </div>
        )}
      </div>
      ...
      {/* Kompakt-Liste (aufsteigend: niedrigste km = Rang 1 = bester) */}
          ...
          return (
            <div key={f.fahrer_id} className="space-y-0.5">
              <div className="flex items-center gap-1.5 text-xs">
                ...
                <span className={`font-bold ${tColor}`}>{f.km_avg} km</span>
                {DeltaIcon}
              </div>
              <div className="h-1 bg-gray-100 rounded-full overflow-hidden ml-5">
                <div className={`h-full rounded-full ${barColor}`} style={{ width: `${(f.km_avg / maxVal) * 100}%` }} />
              </div>
            </div>
          );
        })}

...
export function KitchenPhase2295DurchsatzTicker({
  locationId,
}: {
  ...
  const hinweis = useMemo(() => {
    if (!data) return '';
    if (data.team_avg_bph < 1.5)
      return 'Durchsatz kritisch — Dispatcher sofort informieren!';
    if (data.team_avg_bph < 2)
      return 'Durchsatz unter Ziel — Dispatcher auf Engpässe hinweisen.';
    return 'Durchsatz gut — kein Handlungsbedarf.';
  }, [data]);
    ...
    <div className={`rounded-xl border p-4 mb-3 ${borderFarbe}`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={`font-semibold text-sm ${titelFarbe}`}>Durchsatz-Ticker</span>
          {data && (
        ...
        <div className="mt-3 space-y-2">
          {alertFahrer.length > 0 && (
            <div className="flex items-center gap-2 rounded-lg bg-red-100 dark:bg-red-900/30 border border-red-300 dark:border-red-700 px-3 py-2 text-xs text-red-700 dark:text-red-300">
              ...
              <span>
                <strong>{alertFahrer.length} Fahrer</strong> unter 2 B/h:{' '}

...
export function KitchenPhase3827AuftragsablehnungsQuoteTicker({ locationId }: { locationId: string | null }) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
      ...
      {data.alert_count > 0 && (
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-orange-50 border border-orange-200 rounded-lg text-[11px] text-orange-800">
          ...
          <span>Hohe Ablehnungsquote!</span>
        </div>
      )}
      ...
      <div className="space-y-1.5">
        {data.fahrer.map(f => {
          ...
          const maxPct = Math.max(...data.fahrer.map(d => d.ablehnungsquote_pct), 1);

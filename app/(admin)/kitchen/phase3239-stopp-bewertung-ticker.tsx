...
export function KitchenPhase3239StoppBewertungTicker({ locationId }: { locationId: string | null }) {
  ...
  const load = async () => {
    try {
      const url = locationId
        ? `/api/delivery/admin/fahrer-stopp-bewertung?location_id=${locationId}`
        : `/api/delivery/admin/fahrer-stopp-bewertung`;
    ...
    <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 shadow-sm space-y-2">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-gray-700 dark:text-gray-200">Stopp-Bewertungs-Ranking</span>
          {best && (
            <span className="text-xs text-gray-500 dark:text-gray-400">
              #1 <span className="font-medium text-gray-700 dark:text-gray-200">{best.fahrer_name}</span> {best.avg_score.toFixed(1)} ★
            </span>
          )}
        </div>
        {data.alert_count > 0 && (
          <span className="text-xs px-1.5 py-0.5 rounded bg-red-100 text-red-700 font-medium">
            {data.alert_count}× Niedrige Stopp-Bewertung!
          </span>
        )}
      </div>

      {/* Kompakte Liste — absteigend */}
      ...
      <div className="text-xs text-gray-400 border-t border-gray-100 dark:border-gray-700 pt-1">
        Team-Ø: <span className="font-medium text-blue-600 dark:text-blue-400">{data.team_avg_score.toFixed(1)} ★</span>
      </div>
    </div>
  );
}

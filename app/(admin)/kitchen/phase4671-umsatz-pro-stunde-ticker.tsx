...
interface ApiResponse {
  fahrer: { fahrer_name: string; umsatz_pro_stunde: number; rang: number }[];
  team_avg_umsatz: number;
}

export function KitchenPhase4671UmsatzProStundeTicker({ locationId }: { locationId: string | null }) {
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Umsatz/h-Ticker nicht verfügbar</span>
      </div>
    );
  }
  ...
  const zielUph = 20;
  const zielDiff = data.team_avg_umsatz - zielUph;
  const zielOk = zielDiff >= 0;
  ...
  return (
    <div className="rounded-2xl border border-amber-200 dark:border-amber-900 bg-white dark:bg-gray-900 p-3 space-y-2">
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">Umsatz/h Ticker</span>
      </div>
      ...
      <div className="flex items-center gap-2">
        <div className="bg-amber-50 dark:bg-amber-950 rounded-lg px-3 py-1.5 flex items-baseline gap-1">
          ...
          <span className="text-lg font-extrabold text-amber-800 dark:text-amber-300">{leader?.umsatz_pro_stunde ?? 0}€/h</span>
        </div>
      ...
      <div className="flex items-center justify-between text-[10px] text-gray-500 dark:text-gray-400 border-t border-gray-100 dark:border-gray-800 pt-1.5">
        <span>Team-Ø: <span className="font-bold text-gray-700 dark:text-gray-300">{data.team_avg_umsatz}€/h</span></span>
        <span>
          Ziel ≥ {zielUph}€:{' '}
          <span className={`font-bold ${zielOk ? 'text-green-700 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
            {zielOk ? '+' : ''}{zielDiff}€
          </span>
        </span>
      </div>

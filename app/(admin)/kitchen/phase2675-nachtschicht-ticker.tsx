...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'f1', fahrer_name: 'Max M.',   nacht_h: 5.5, trend: 'steigend' },
    { fahrer_id: 'f4', fahrer_name: 'Julia F.', nacht_h: 3.5, trend: 'fallend'  },
    { fahrer_id: 'f2', fahrer_name: 'Sara K.',  nacht_h: 2.0, trend: 'steigend' },
    { fahrer_id: 'f3', fahrer_name: 'Tim B.',   nacht_h: 0.0, trend: 'fallend'  },
  ],
  team_avg_nacht_h: 2.75,
};

export function KitchenPhase2675NachtschichtTicker({ locationId }: { locationId?: string | null }) {
  ...
  const sorted    = [...enriched].sort((a, b) => b.nacht_h - a.nacht_h);
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm mb-4`}>
      ...
      >
        <div className="flex items-center gap-2">
          <Moon size={15} className={hasAlert ? 'text-red-500' : 'text-gray-500'} />
          <span className="font-semibold text-sm text-gray-800">Nachtschichten</span>
          <span className={`text-xs font-bold ${textCls(teamAmpel)}`}>
            Ø {data.team_avg_nacht_h} h
          </span>
          {hasAlert && (
        ...
        <div className="px-4 pb-3 space-y-2">
          ...
          {hasAlert && (
            <div className="flex items-start gap-2 bg-red-100 border border-red-200 rounded-lg px-3 py-1.5">
              ...
              <p className="text-xs text-red-700 font-medium">
                Nachtschicht zu lang! {alerts.map(a => a.fahrer_name).join(', ')}
              </p>
            </div>
          )}
          ...
          {sorted.map(f => (
            <div key={f.fahrer_id} className="flex items-center gap-2">
              ...
              <span className={`text-xs font-bold ${textCls(f.ampel)}`}>{f.nacht_h} h</span>
            </div>
          ))}

          {/* Ziel */}
          <p className="text-xs text-gray-400 pt-1">Ziel 0 h Nachtschicht/Fahrer</p>
        </div>
      )}
    </div>
  );

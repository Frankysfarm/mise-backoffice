...
export function KitchenPhase5089WochenendAnteilTicker({ locationId }: { locationId: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-teal-700 bg-teal-900/60 px-4 py-3 mb-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs text-gray-400">Wochenend-Anteil-Ranking — Flexibelste</div>
        <div className="text-sm font-bold text-teal-100 truncate">
          #{top?.rang} {top?.fahrer_name} — {top?.wochenend_anteil_pct}%
        </div>
        <div className="text-xs text-gray-500">Team-Ø: {data.team_avg_pct}% Wochenendschichten</div>
      </div>
      {data.alert_count > 0 && (

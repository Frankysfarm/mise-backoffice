...
import { ChevronDown, ChevronUp, AlertTriangle, Radio } from 'lucide-react';
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'd5', fahrer_name: 'Anna B.',  avg_reaktionszeit_sek: 95.0, ampel: 'rot',   alert: true  },
    { fahrer_id: 'd2', fahrer_name: 'Sarah K.', avg_reaktionszeit_sek: 78.3, ampel: 'rot',   alert: true  },
    { fahrer_id: 'd3', fahrer_name: 'Lena S.',  avg_reaktionszeit_sek: 44.7, ampel: 'gelb',  alert: false },
    { fahrer_id: 'd1', fahrer_name: 'Max M.',   avg_reaktionszeit_sek: 22.4, ampel: 'gruen', alert: false },
    { fahrer_id: 'd4', fahrer_name: 'Tom B.',   avg_reaktionszeit_sek: 18.2, ampel: 'gruen', alert: false },
  ],
  team_avg_reaktionszeit_sek: 51.7,
  alert_count: 2,
};

export function KitchenPhase2538ErreichbarkeitTicker({ locationId }: { locationId?: string | null }) {
    ...
    <div className={`rounded-xl border ${hasAlert ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'} shadow-sm`}>
      ...
      >
        <div className="flex items-center gap-2">
          <Radio size={15} className={hasAlert ? 'text-red-600' : 'text-gray-500'} />
          <span className="text-sm font-bold text-gray-800">Erreichbarkeit</span>
        ...
        <div className="border-t px-4 pb-3 pt-2 space-y-2">
          {alertFahrer.length > 0 && (
            <div className="flex items-center gap-2 rounded-lg bg-red-100 border border-red-200 px-3 py-1.5 text-xs font-semibold text-red-800">
              ...
              &gt;60 s: {alertFahrer.join(', ')} — Fahrer Erreichbarkeit prüfen!
            </div>
          )}

...
import { Clock, CheckCircle2, AlertCircle, MapPin, ChefHat, Bike, Package, Loader2, Navigation, Star } from 'lucide-react';

// Phase 5036 — Dynamische ETA Live Hub V2
// Bestell-Status-Timeline; Live-ETA-Countdown mit Sekunden; Fahrer-Annäherungs-Pulsbalken;
// Waze + Google Maps Navigation-Links; Bewertungs-Stars nach Zustellung; Konfidenz-Balken;
...
function SecondCountdown({ minutes }: { minutes: number }) {
  const [totalSecs, setTotal] = useState(minutes * 60);
  useEffect(() => {
    setTotal(minutes * 60);
  }, [minutes]);
  useEffect(() => {
...
function RatingStars({ rating }: { rating: number }) {
  return (
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((n) => (
        <Star key={n} className={`h-6 w-6 ${n <= rating ? 'text-amber-400 fill-amber-400' : 'text-muted-foreground'}`} />
      ))}
    </div>
  );
}

export function Phase5036DynamischeEtaLiveHubV2({ orderId, locationId }: { orderId?: string | null; locationId?: string | null }) {
  ...
  const proximity = d.fahrer_km != null ? Math.max(0, Math.min(100, 100 - (d.fahrer_km / 3) * 100)) : 0;
  ...
  return (
    <div className="rounded-2xl border border-indigo-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-indigo-700 text-white">
        ...
        {d.phase !== 'geliefert' && (
          <div className={`text-2xl font-black font-mono tabular-nums ${ETA_COLOR(d.eta_min)}`}>
            <SecondCountdown minutes={d.eta_min} />
          </div>
        )}
      </div>
      ...
      <div className="p-4 space-y-4">
        ...
        {d.phase !== 'geliefert' ? (
          <div className="rounded-xl border border-indigo-100 bg-indigo-50 px-4 py-3 text-center">
            <div className={`text-4xl font-black tabular-nums font-mono ${ETA_COLOR(d.eta_min)}`}>
              <SecondCountdown minutes={d.eta_min} />
            </div>
          ...
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-4 text-center space-y-3">
            <CheckCircle2 className="h-10 w-10 text-emerald-500 mx-auto" />
            <div className="font-bold text-emerald-700 text-lg">Geliefert! Guten Appetit 🍽️</div>
            {userRating === 0 ? (
              <div>
                <div className="text-xs text-muted-foreground mb-2">Wie war deine Lieferung?</div>
                <div className="flex justify-center gap-2">
                  {[1, 2, 3, 4, 5].map((n) => (
                    <button key={n} onClick={() => setUserRating(n)}>
                      <Star className="h-7 w-7 text-amber-300 hover:text-amber-500 transition" />
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col items-center gap-1">
                ...
                <div className="text-xs text-emerald-700 font-semibold">Danke für deine Bewertung!</div>
              </div>
            )}
          </div>
        )}
        ...
        {/* Fahrer-Info + Navigation */}
        {d.fahrer_name && d.phase === 'unterwegs' && (
          <div className="rounded-xl border border-blue-100 bg-blue-50 space-y-2 px-3 py-2.5">
            <div className="flex items-center gap-3">
              <div className="rounded-full bg-blue-600 h-9 w-9 flex items-center justify-center shrink-0">
              ...
              <div className={`text-lg font-black font-mono tabular-nums ${ETA_COLOR(d.eta_min)}`}>
                {d.eta_min} min
              </div>
            </div>
            ...
            {d.fahrer_km != null && (
              <div>
                <div className="flex items-center justify-between text-[10px] text-muted-foreground mb-1">
                  ...
                  <span className="font-bold">{d.fahrer_km} km</span>
                </div>
                <div className="h-2 rounded-full bg-blue-200 overflow-hidden">
                  <div
                    className="h-2 rounded-full bg-blue-500 transition-all duration-1000 animate-pulse"
            ...
            {/* Navigation Links */}
            {d.fahrer_lat != null && d.fahrer_lng != null && (
              <div className="flex gap-2">
                <a
                  href={`https://www.google.com/maps/dir/?api=1&destination=${d.fahrer_lat},${d.fahrer_lng}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1.5 rounded-lg bg-white border border-border px-3 py-1.5 text-xs font-semibold hover:bg-muted/30 transition"
                >
                  <Navigation className="h-3.5 w-3.5 text-blue-600" />Google Maps
                </a>
                <a
                  href={`https://waze.com/ul?ll=${d.fahrer_lat},${d.fahrer_lng}&navigate=yes`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1.5 rounded-lg bg-white border border-border px-3 py-1.5 text-xs font-semibold hover:bg-muted/30 transition"
                >
                  <Navigation className="h-3.5 w-3.5 text-sky-500" />Waze
                </a>
              </div>
            )}
          </div>

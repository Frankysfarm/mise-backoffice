...
import { ChevronDown, ChevronUp, ShoppingCart, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
const COACHING: Record<string, string> = {
  gruen: 'Top Bestellwert! Du fährst hochwertige Touren.',
  gelb: 'Gut — versuche Bestellungen mit höherem Warenkorbwert anzunehmen.',
  rot: 'Fokus auf größere Bestellungen: Mindestbestellwert-Touren bevorzugen.',
};

export function FahrerPhase3393MeinBestellwertProTour({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-bestellwert-pro-tour?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
          ...
          <div className="flex justify-around items-center py-2">
            <div className="text-center">
              <div className={`text-5xl font-black ${AMPEL_COLOR[me.ampel]}`}>
                {me.bestellwert_pro_tour}€
              </div>
              <div className="text-[10px] text-gray-500 mt-0.5">Ø pro Tour</div>
            </div>
            <div className="text-center">

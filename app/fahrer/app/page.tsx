...
export default async function FahrerAppPage() {
  ...
  const [{ data: status }, { data: openBatches }, { data: legacyActiveBatch }, { data: miseActiveBatch }] = await Promise.all([
    ...
    svc.from('delivery_batches')
      .select('*, stops:delivery_batch_stops(*, order:customer_orders(id,bestellnummer,kunde_name,kunde_adresse,kunde_plz,kunde_lat,kunde_lng,gesamtbetrag,bezahlt,zahlungsart,kunde_telefon))')
    ...
    miseDriver
      ? svc.from('mise_delivery_batches')
          .select('id, state, stops:mise_delivery_batch_stops(id, batch_id, order_id, sequence, completed_at, type, order:customer_orders(id,bestellnummer,kunde_name,kunde_adresse,kunde_plz,kunde_lat,kunde_lng,gesamtbetrag,bezahlt,zahlungsart,kunde_telefon))')

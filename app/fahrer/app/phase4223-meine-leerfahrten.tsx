...
interface ApiData { leerfahrten_pct: number; rang: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; team_avg_leerfahrten_pct: number; fahrer_count: number; }

const MOCK: ApiData = { leerfahrten_pct: 12, rang: 2, rank_delta: 0, ampel: 'gruen', team_avg_leerfahrten_pct: 19, fahrer_count: 4 };

interface Props { driverId: string; locationId: string | null; isOnline: boolean; }

export function FahrerPhase4223MeineLeerfahrten({ driverId, locationId, isOnline }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/delivery/admin/fahrer-leerfahrten-ranking?location_id=${locationId}&driver_id=${driverId}`);
      if (res.ok) {
        const json = await res.json();
        if (!json.error && json.fahrer?.length) {
          const me = json.fahrer[0];
          setData({ leerfahrten_pct: me.leerfahrten_pct, rang: me.rang, rank_delta: me.rank_delta, ampel: me.ampel, team_avg_leerfahrten_pct: json.team_avg_leerfahrten_pct, fahrer_count: json.gesamt });
        }
      }
  ...
  const coaching = data.ampel === 'gruen'
    ? 'Super – du vermeidest unnötige Leerfahrten und fährst effizient!'
    : data.ampel === 'gelb'
    ? 'Tipp: Optimiere deine Routen, um Leerfahrten zu reduzieren.'
    : 'Achtung: Deine Leerfahrten-Quote liegt deutlich über dem Teamdurchschnitt.';

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        <Route className="w-4 h-4 text-orange-500" />
        <span className="text-sm font-semibold text-gray-900">Meine Leerfahrten-Quote</span>
        {loading && <span className="w-3 h-3 border-2 border-gray-300 border-t-transparent rounded-full animate-spin ml-auto" />}
      </div>
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-bold ${valColor}`}>{data.leerfahrten_pct}</span>

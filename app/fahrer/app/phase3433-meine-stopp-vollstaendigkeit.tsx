...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Exzellent! Du schließt fast alle Stopps erfolgreich ab.',
  gelb: 'Gut — überprüfe, warum manche Stopps nicht abgeschlossen werden.',
  rot: 'Tipp: Kontaktiere Kunden frühzeitig bei Problemen und nutze alle Zustelloptionen.',
};

export function FahrerPhase3433MeineStoppVollstaendigkeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const [data, setData] = useState<FahrerStoppVollstaendigkeit | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-stopp-vollstaendigkeit?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
      if (!r.ok) return;
          ...
          <div className="flex items-end justify-center gap-4">
            <div className="text-center">
              <div className={`text-5xl font-bold font-mono ${AMPEL_COLOR[data.ampel]}`}>
                {data.vollstaendigkeits_pct}%
              </div>
              <div className="text-xs text-gray-500 mt-0.5">Vollständig</div>
            </div>
            <div className="text-center">

...
function coachingTipp(val: number): { text: string; color: string } {
  if (val <= 3) return {
    text: 'Blitzschnelle Reaktion! Du nimmst Aufträge sofort an — das schätzen Dispatcher und Kunden.',
    color: 'text-green-300',
  };
  if (val <= 6) return {
    text: 'Gute Reaktionszeit. Schnellere Annahme unter 3 Min verbessert dein Ranking und die Kundenzufriedenheit.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Zu langsame Reaktion. Halte das Handy bereit und nimm Aufträge unverzüglich an.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5256MeineReaktionszeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Reaktionszeit</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.avg_reaktionszeit_min.toFixed(1)}<span className="text-xl font-semibold"> min</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.avg_reaktionszeit_min / Math.max(teamAvg * 2, 1)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.avg_reaktionszeit_min.toFixed(1)} min</span>

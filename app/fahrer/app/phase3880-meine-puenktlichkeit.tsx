...
import { CheckCircle, TrendingUp, TrendingDown, Minus, Wifi } from 'lucide-react';
...
export function FahrerPhase3880MeinePuenktlichkeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const [teamAvg, setTeamAvg] = useState(79.5);
  ...
  const istGut = fahrer.puenktlichkeit_pct >= ziel;

  const coaching = fahrer.puenktlichkeit_pct < 65
    ? 'Deine Pünktlichkeitsquote ist sehr niedrig. Plane Routen besser und starte Touren frühzeitig.'
    : fahrer.puenktlichkeit_pct < ziel
      ? 'Die Pünktlichkeit kann verbessert werden. Berücksichtige Verkehr und plane mehr Puffer ein.'
      : 'Ausgezeichnete Pünktlichkeit! Du lieferst zuverlässig zur richtigen Zeit.';
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-end justify-between">
        <div>
          <div className={`text-5xl font-black leading-none ${valColor}`}>{fahrer.puenktlichkeit_pct}%</div>
          <div className="text-xs text-gray-400 mt-0.5">On-Time</div>
      ...
      <div>
        <div className="flex justify-between text-[10px] text-gray-400 mb-1">
          ...
          <span>{istGut ? '✓ Ziel erreicht' : `${ziel - fahrer.puenktlichkeit_pct}% unter Ziel`}</span>
        </div>
        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full ${fahrer.ampel === 'gruen' ? 'bg-emerald-500' : fahrer.ampel === 'gelb' ? 'bg-yellow-400' : 'bg-red-400'}`}
            style={{ width: `${Math.min(100, fahrer.puenktlichkeit_pct)}%` }}
          />
        </div>
      </div>

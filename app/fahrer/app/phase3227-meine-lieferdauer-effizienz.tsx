...
function coachingTipp(ampel: string): string {
  if (ampel === 'gruen') return 'Hervorragende Effizienz! Du lieferst schneller als geplant — Top-Leistung!';
  if (ampel === 'gelb')  return 'Solide Leistung. Optimiere deine Route für noch schnellere Lieferungen.';
  return 'Lieferdauer verbessern: Optimale Route wählen, Staus meiden und Übergaben beschleunigen.';
}

export function FahrerPhase3227MeineLieferdauerEffizienz({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/delivery/admin/fahrer-lieferdauer-effizienz?${params}`);
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-5 shadow-sm space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Lieferdauer-Effizienz</span>
      </div>
      ...
      <div className="flex items-center justify-center gap-6">
        ...
        <div className="text-center">
          <div className={`text-5xl font-black ${ampelColor}`}>{me.effizienz_pct}%</div>
          <div className="text-xs text-gray-400 mt-0.5">Effizienz</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-center text-xs">
        ...
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          ...
          <div className="font-semibold text-blue-600 dark:text-blue-400 mt-0.5">{data?.team_avg_pct}%</div>
        </div>
      </div>

...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 1.5) return {
    text: 'Klasse Trinkgeld! Kunden schätzen deine Freundlichkeit und deinen Service sehr.',
    color: 'text-green-300',
  };
  if (val >= 0.5) return {
    text: 'Gutes Trinkgeld. Freundlichkeit, Pünktlichkeit und ein Lächeln machen den Unterschied.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Niedriges Trinkgeld. Verbessere deinen Service — das zahlt sich direkt aus.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5199MeinTrinkgeldProLieferung({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Mein Ø-Trinkgeld/Lieferung — offline</span>
      </div>
    );
  }
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.avg_trinkgeld / Math.max(teamAvg * 1.5, 0.1)) * 100))}%` }}
          />
        </div>

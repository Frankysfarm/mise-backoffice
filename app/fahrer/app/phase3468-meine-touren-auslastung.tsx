...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Hervorragend! Deine Touren-Auslastung ist top — du nutzt deine Schichtzeit sehr effizient.',
  gelb: 'Gut — versuche mehr Touren pro Schicht zu absolvieren. Kurze Pausen strategisch planen.',
  rot: 'Tipp: Sprich mit dem Dispatcher — mehr Tourenzuweisung kann deine Auslastung deutlich steigern.',
};

export function FahrerPhase3468MeineTourenAuslastung({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const [data, setData] = useState<FahrerTourenAuslastungRanking | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-touren-auslastung-ranking?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
      if (!r.ok) return;
  ...
  // descending ranking: rank 1 = highest = best; bar fills proportionally to score
  const barWidth = data.auslastung_pct;
        ...
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-[10px] text-gray-400">
              <span>Niedrigste Auslastung</span>
              <span>Höchste (Bester)</span>
            </div>

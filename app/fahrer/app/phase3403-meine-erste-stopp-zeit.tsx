...
import { ChevronDown, ChevronUp, TimerReset, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
const COACHING: Record<string, string> = {
  gruen: 'Toller Anlauf! Du erreichst deinen ersten Stopp sehr schnell.',
  gelb: 'Gut — optimiere deine Startroute, um noch schneller zum ersten Stopp zu kommen.',
  rot: 'Fokus: Kürze die Zeit vom Tourstart bis zum ersten Stopp — Route vorher prüfen.',
};

export function FahrerPhase3403MeineErsteStoppZeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-erste-stopp-zeit?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
        ...
        <div className="px-3 pb-3 space-y-3">
          <div className="flex justify-around items-center py-2">
            <div className="text-center">
              ...
              <div className="text-[10px] text-gray-500 mt-0.5">min Ø</div>
            </div>
            <div className="text-center">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-gray-50 dark:bg-gray-800 rounded p-2 text-center">
              ...
              <div className={`text-sm font-semibold flex items-center justify-center gap-1 ${me.rank_delta < 0 ? 'text-emerald-600' : me.rank_delta > 0 ? 'text-red-600' : 'text-gray-500'}`}>
                ...
                {me.rank_delta < 0 ? me.rank_delta : me.rank_delta > 0 ? `+${me.rank_delta}` : '0'}
              </div>
            </div>
            <div className="bg-gray-50 dark:bg-gray-800 rounded p-2 text-center">
              <div className="text-[10px] text-gray-500 uppercase">Team-Ø</div>
              <div className="text-sm font-semibold">{data.team_avg} min</div>
            </div>
          </div>

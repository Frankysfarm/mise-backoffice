...
export function FahrerPhase3970MeineSchichtAuslastung({ driverId, locationId, isOnline }: Props) {
  ...
  if (!isOnline) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-2 text-gray-400 text-sm">
        ...
        <span>Offline – Auslastung nicht verfügbar</span>
      </div>
    );
  }
  ...
  const coaching =
    me.ampel === 'gruen'
      ? 'Ausgezeichnete Schicht-Auslastung – weiter so!'
      : me.ampel === 'gelb'
        ? 'Tipp: Wartezeiten reduzieren für bessere Auslastung.'
        : 'Achtung: Geringe Auslastung – aktiver auf Touren eingehen!';
  ...
  return (
    <div className={`bg-white rounded-xl border ${borderColor} p-4 space-y-3`}>
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-semibold text-gray-900">Meine Schicht-Auslastung</span>
      ...
      <div className={`rounded-xl p-4 ${bgColor} flex items-center justify-between`}>
        <div>
          <div className={`text-5xl font-black ${tColor}`}>{me.auslastung_pct}%</div>
          <div className="text-xs text-gray-500 mt-1">Auslastung</div>
        </div>
      ...
      <div className="flex items-center justify-between text-xs text-gray-500 px-1">
        <span>Ziel ≥80% Auslastung</span>
        <span className="font-medium text-gray-600">Team-Ø {data.team_avg}%</span>
      </div>

...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'me', name: 'Du', score: 74, touren_pro_stunde: 3.8, km_pro_stopp: 2.1, trend_delta: 2, ampel: 'gelb', rang: 2 },
  ],
...
function coaching(a: string): string {
  if (a === 'gruen') return 'Hervorragend! Du arbeitest sehr effizient. Dein Tempo und deine Routen sind optimal — weiter so!';
  if (a === 'gelb')  return 'Gut! Optimiere deine Routen und minimiere Wartezeiten beim Kunden, um noch effizienter zu werden.';
  return                   'Fokus! Plane deine Stopps kürzer und vermeide unnötige Umwege. Jede Minute zählt für deinen Score.';
}

export function FahrerPhase3277MeineSchichtEffizienz({
  driverId,
  locationId,
  isOnline,
}: {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Schicht-Effizienz</span>
          {me.ampel === 'rot' && (
      ...
      {open && (
        <div className="p-4 space-y-4">
          <div className="text-center">
            <div className={`text-5xl font-black ${col.text}`}>{me.score}</div>
          ...
          <div>
            <div className="text-xs text-gray-500 mb-1 flex justify-between">
              <span>Niedrigste Effizienz</span><span>Höchste Effizienz</span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-center text-xs">
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              <div className="text-gray-500 font-medium mb-0.5">Stopps/h</div>
              <div className={`font-bold text-lg ${col.text}`}>{me.touren_pro_stunde.toFixed(1)}</div>
            </div>
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              <div className="text-gray-500 font-medium mb-0.5">km/Stopp</div>
              <div className={`font-bold text-lg ${col.text}`}>{me.km_pro_stopp.toFixed(1)}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-center text-xs">
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="font-bold flex items-center justify-center gap-1">
                {me.trend_delta > 0
                  ? <><TrendingUp size={14} className="text-green-500" /><span className="text-green-600">+{me.trend_delta}</span></>
                  : me.trend_delta < 0
                  ? <><TrendingDown size={14} className="text-red-400" /><span className="text-red-500">{me.trend_delta}</span></>
            ...
            <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-2">
              ...
              <div className="font-bold text-blue-600 dark:text-blue-400">{d.team_durchschnitt} Pkt.</div>
            </div>
          </div>

...
export function FahrerPhase4559MeinNachtAnteil({ driverId, locationId, isOnline }: Props) {
  ...
  if (!isOnline) return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 flex items-center gap-3">
      ...
      <span className="text-sm text-gray-500 dark:text-gray-400">Offline — Nachtanteil nicht verfügbar</span>
    </div>
  );
  ...
  const coaching =
    me.nacht_pct >= 40 ? { text: 'Ausgezeichnet! Du bist ein echter Nachtfahrer.', cls: 'text-emerald-600 dark:text-emerald-400' } :
    me.nacht_pct >= 20 ? { text: 'Gut! Mehr Nachtschichten erhöhen deine Teamwertung.',  cls: 'text-yellow-600 dark:text-yellow-400' } :
                         { text: 'Kaum Nachtlieferungen — Nachtschichten steigern Effizienz & Prämien.', cls: 'text-red-500 dark:text-red-400' };

  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
      <div className="flex items-center gap-2 mb-3">
        <Moon className="w-5 h-5 text-indigo-500" />
        <h3 className="font-semibold text-gray-900 dark:text-white text-sm">Mein Nacht-Lieferungs-Anteil</h3>
      </div>
      ...
      <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">
        Anteil Nachtlieferungen (22–06 Uhr)
      </div>

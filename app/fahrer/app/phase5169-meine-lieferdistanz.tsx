...
function coachingTipp(val: number): { text: string; color: string } {
  if (val <= 4) return {
    text: 'Sehr effiziente Routen! Du lieferst nah und sparst Zeit und Kosten.',
    color: 'text-green-300',
  };
  if (val <= 7) return {
    text: 'Gute Routeneffizienz. Kurze Touren helfen dir, mehr Lieferungen pro Stunde zu schaffen.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Lange Lieferdistanzen. Sprich mit dem Disponenten über eine optimierte Zuteilung.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5169MeineLieferdistanz({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-km-pro-lieferung-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Ø-Lieferdistanz — offline</span>
      </div>
    );
  }
  ...
  const tipp    = coachingTipp(mein.avg_km_pro_lieferung);
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Ø-Lieferdistanz</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.avg_km_pro_lieferung.toFixed(1)} <span className="text-xl font-semibold">km</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.avg_km_pro_lieferung / Math.max(teamAvg * 2, 0.1)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.avg_km_pro_lieferung.toFixed(1)} km</span>
          <span>{teamAvg.toFixed(1)} km</span>
        </div>
      </div>

...
export function FahrerPhase3975MeineStornoquote({ driverId, locationId, isOnline }: Props) {
  ...
  if (!isOnline) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-2 text-gray-400 text-sm">
        ...
        <span>Offline – Stornoquote nicht verfügbar</span>
      </div>
    );
  }
  ...
  const coaching =
    me.ampel === 'gruen'
      ? 'Niedrige Stornoquote – exzellente Liefertreue!'
      : me.ampel === 'gelb'
        ? 'Tipp: Kommunikation mit Kunden verbessert Stornoquote.'
        : 'Achtung: Hohe Stornoquote – Ursachen analysieren!';
  ...
  return (
    <div className={`bg-white rounded-xl border ${borderColor} p-4 space-y-3`}>
      ...
      <div className={`rounded-xl p-4 ${bgColor} flex items-center justify-between`}>
        <div>
          <div className={`text-5xl font-black ${tColor}`}>{me.stornoquote_pct}%</div>
          <div className="text-xs text-gray-500 mt-1">Stornoquote</div>
        </div>
      ...
      <div className="flex items-center justify-between text-xs text-gray-500 px-1">
        <span>Ziel ≤{data.ziel_pct}% Stornoquote</span>
        <span className="font-medium text-gray-600">Team-Ø {data.team_avg_pct}%</span>
      </div>

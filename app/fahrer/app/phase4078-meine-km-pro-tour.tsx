...
interface ApiData { km_avg: number; rang: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; team_avg: number; fahrer_count: number; }

const MOCK: ApiData = { km_avg: 5.1, rang: 2, rank_delta: -1, ampel: 'gelb', team_avg: 6.35, fahrer_count: 4 };

interface Props { driverId: string; locationId: string | null; isOnline: boolean; }

export function FahrerPhase4078MeineKmProTour({ driverId, locationId, isOnline }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/delivery/admin/fahrer-km-pro-tour-ranking?location_id=${locationId}&driver_id=${driverId}`);
      if (res.ok) {
        const json = await res.json();
        if (!json.error && json.fahrer) {
          const me = json.fahrer.find((f: { fahrer_id: string }) => f.fahrer_id === driverId) ?? json.fahrer[0];
          if (me) setData({ km_avg: me.km_avg, rang: me.rang, rank_delta: me.rank_delta, ampel: me.ampel, team_avg: json.team_avg, fahrer_count: json.gesamt });
        }
      }
  ...
  if (!isOnline) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">km/Tour-Ranking nicht verfügbar (offline)</span>
      </div>
    );
  }

  const valueColor = data.ampel === 'gruen' ? 'text-emerald-600' : data.ampel === 'gelb' ? 'text-yellow-500' : 'text-red-500';
  const coaching = data.ampel === 'gruen' ? 'Hervorragend – du fährst die effizientesten Routen!' : data.ampel === 'gelb' ? 'Tipp: Kürzere Routen senken deinen km/Tour-Wert.' : 'Achtung: Deine km/Tour-Rate liegt deutlich über dem Team.';
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-bold tabular-nums ${valueColor}`}>{data.km_avg?.toFixed(1)}<span className="text-2xl font-semibold ml-1">km</span></span>
      ...
      <div className="flex justify-between text-[11px] text-gray-400 px-1">
        ...
        <span>Rang 1 = kürzeste km/Tour</span>
      </div>
    </div>
  );
}

...
const COACHING: Record<string, string> = {
  gruen: 'Stark! Du lieferst überdurchschnittlich viele Pakete pro Stunde.',
  gelb: 'Gut — optimiere deine Route, um deine Pakete/h-Rate zu steigern.',
  rot: 'Fokus: Effizientere Routen und schnellere Stops helfen dir, mehr Pakete/h zu erreichen.',
};

export function FahrerPhase3418MeinePaketeProStunde({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-pakete-pro-stunde?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
        ...
        <div className="px-3 pb-3 space-y-3">
          <div className="flex justify-around items-center py-2">
            <div className="text-center">
              <div className={`text-5xl font-black ${AMPEL_COLOR[me.ampel]}`}>
                {me.pakete_pro_stunde}
              </div>
              <div className="text-[10px] text-gray-500 mt-0.5">/h Ø</div>
            </div>
            <div className="text-center">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded p-2 text-center">
              ...
              <div className="text-sm font-semibold">{data.team_avg}/h</div>
            </div>
          </div>

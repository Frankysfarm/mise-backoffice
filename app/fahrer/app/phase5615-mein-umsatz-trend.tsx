...
// Phase 5615 — Mein Umsatz-Trend (Fahrer)
// TrendingUp emerald-400; umsatz_delta 4xl+Rang ABSTEIGEND; isOnline-Guard+WifiOff-Fallback;
...
function coaching(delta: number): { text: string; color: string } {
  if (delta > 0) return {
    text: `Dein Umsatz-Ø stieg um +${delta.toFixed(2)}€ pro Tour — du lieferst höherwertige Bestellungen! Weiter so.`,
    color: 'text-emerald-400',
  };
  if (delta === 0) return {
    text: 'Dein Umsatz ist stabil. Pünktliche Lieferungen und Upselling können den Durchschnitt steigern.',
    color: 'text-yellow-300',
  };
  return {
    text: `Umsatz gesunken um ${Math.abs(delta).toFixed(2)}€ — prüfe, ob du Touren mit kleineren Bestellungen fährst, und sprich mit deinem Dispatcher.`,
    color: 'text-red-400',
  };
}

export function FahrerPhase5615MeinUmsatzTrend({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const load = async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-umsatz-trend-ranking?driver_id=${driverId}${locationId ? `&location_id=${locationId}` : ''}`,
      );
      if (r.ok) {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-lg bg-gray-900 border border-gray-700/50 p-3 flex items-center gap-2">
        ...
        <span className="text-xs text-gray-500">Offline – Umsatz-Trend nicht verfügbar</span>
      </div>
    );
  }

  const { text: coachText, color: coachColor } = coaching(data.umsatz_delta);
  ...
  return (
    <div className={`rounded-lg bg-gray-900 border ${BORDER[data.ampel]} p-3 space-y-2`}>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-xs font-semibold text-white">Mein Umsatz-Trend</span>
        {data.alert_rueckfall && <AlertTriangle className="h-3 w-3 text-red-400 ml-auto" />}
      </div>

      {/* Hauptwert */}
      <div className="flex items-end gap-2">
        <span className={`text-4xl font-black leading-none ${data.umsatz_delta >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
          {data.umsatz_delta > 0 ? '+' : ''}{data.umsatz_delta.toFixed(2)}€
        </span>

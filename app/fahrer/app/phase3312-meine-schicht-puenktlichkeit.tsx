...
function coachingTipp(a: string): string {
  if (a === 'rot')  return 'Niedrige Pünktlichkeit! Versuche, deine Schichten pünktlich zu starten — 5 Minuten Toleranz sind eingerechnet.';
  if (a === 'gelb') return 'Gute Pünktlichkeit! Noch etwas mehr Disziplin bringt dich in die grüne Zone (≥90%).';
  return 'Ausgezeichnet! Du bist einer der pünktlichsten Fahrer im Team.';
}

export function FahrerPhase3312MeineSchichtPuenktlichkeit({
  driverId,
  locationId,
  isOnline,
}: {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Schicht-Pünktlichkeit</span>
          {me.alert_bottom && (
        ...
        <div className="p-4 space-y-4">
          <div className={`rounded-xl p-4 text-center ${cls.bg}`}>
            <div className={`text-5xl font-black ${cls.text}`}>
              {me.puenktlich_pct}<span className="text-2xl font-semibold">%</span>
            </div>
            <div className={`text-3xl font-bold ${cls.text} mt-1`}>Rang #{me.rang}</div>
            <div className="text-xs text-gray-400 mt-1">{me.puenktlich} von {me.gesamt} Schichten pünktlich</div>
          </div>
          ...
          <div className="text-xs text-gray-400 text-center">Schicht-Pünktlichkeit letzte 30 Tage · 5 Min Toleranz</div>
        </div>
      )}
    </div>
  );

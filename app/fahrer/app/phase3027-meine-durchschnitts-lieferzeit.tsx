...
const TIPPS: Record<string, string> = {
  gruen: '≤25 Min: Ausgezeichnet! Du lieferst sehr schnell. Weiter so!',
  gelb:  '25–35 Min: Noch Luft nach oben. Versuche deine Route besser zu optimieren.',
  rot:   '>35 Min: Zu lange — Route optimieren! Sprich mit dem Dispatcher über schnellere Wege.',
};

const MOCK_ENTRY: FahrerEntry = {
  fahrer_id: 'mock', avg_min: 23.5, avg_min_gestern: 26.0, trend: 'fallend', trend_delta: -2.5, ampel: 'gruen',
};
const MOCK_DATA: ApiData = { fahrer: [MOCK_ENTRY], team_avg_min: 28.2 };
...
export function FahrerPhase3027MeineDurchschnittsLieferzeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const load = () => {
      const url = `/api/delivery/admin/fahrer-durchschnitts-lieferzeit?location_id=${locationId}${driverId ? `&driver_id=${driverId}` : ''}`;
      fetch(url)
        .then(r => r.json())
  ...
  const pct     = Math.min(100, (entry.avg_min / MAX_BAR) * 100);
  const zielPct = (ZIEL_MIN / MAX_BAR) * 100;
  const teamAmpel = data.team_avg_min <= ZIEL_MIN ? 'gruen' : data.team_avg_min <= 35 ? 'gelb' : 'rot';
      ...
      {open && (
        <div className="px-4 py-4 space-y-4">
          <div className="text-center">
            <div className={`text-4xl font-black ${text}`}>{entry.avg_min.toFixed(1)} <span className="text-xl font-bold">Min</span></div>
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">Ø Lieferzeit heute</div>
            <div className="flex items-center justify-center gap-1 mt-1">
              <TrendIcon trend={entry.trend} />
              <span className="text-xs text-gray-500 dark:text-gray-400">
                {entry.trend_delta > 0 ? '+' : ''}{entry.trend_delta.toFixed(1)} Min vs. gestern
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2 text-center">
              ...
              <div className={`text-sm font-bold ${teamText}`}>{data.team_avg_min.toFixed(1)} Min</div>
            </div>
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2 text-center">
              ...
              <div className="flex items-center justify-center gap-1">
                ...
                <span className="text-sm font-bold text-gray-700 dark:text-gray-300">
                  {entry.trend_delta > 0 ? '+' : ''}{entry.trend_delta.toFixed(1)} Min
                </span>
              </div>
            </div>
          </div>

...
const MOCK: ApiData = { stopps_pro_stunde: 2.8, rang: 2, rank_delta: 1, ampel: 'gruen', team_avg: 2.4, fahrer_count: 4 };

interface Props { driverId: string; locationId: string | null; isOnline: boolean; }

export function FahrerPhase4058MeineStoppsProStunde({ driverId, locationId, isOnline }: Props) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!json.error && json.fahrer?.length) {
          ...
          setData({ stopps_pro_stunde: me.stopps_pro_stunde, rang: me.rang, rank_delta: me.rank_delta, ampel: me.ampel, team_avg: json.team_avg, fahrer_count: json.gesamt });
        }
      }
  ...
  if (!isOnline) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Stopps-Ranking nicht verfügbar (offline)</span>
      </div>
    );
  }

  const valueColor = data.ampel === 'gruen' ? 'text-emerald-600' : data.ampel === 'gelb' ? 'text-yellow-500' : 'text-red-500';
  const coaching = data.ampel === 'gruen' ? 'Top – deine Stopps-Rate ist hervorragend!' : data.ampel === 'gelb' ? 'Tipp: Optimiere deine Routen für mehr Stopps pro Stunde.' : 'Achtung: Deine Stopps-Rate ist unterdurchschnittlich.';
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-bold tabular-nums ${valueColor}`}>{data.stopps_pro_stunde?.toFixed(1)} /h</span>

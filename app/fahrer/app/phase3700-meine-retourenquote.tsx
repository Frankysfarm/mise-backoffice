...
const COACHING: Record<string, string> = {
  gruen: 'Exzellent! Deine Retourenquote ist sehr niedrig — du lieferst zuverlässig und korrekt.',
  gelb: 'Solide. Mit sorgfältigerer Übergabe und Adressprüfung kannst du deine Quote weiter senken.',
  rot: 'Hohe Retourenquote. Bitte Adressen vor Abfahrt prüfen und bei Übergabe besonders sorgfältig vorgehen.',
};

export function FahrerPhase3700MeineRetourenquote({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900">Meine Retourenquote</h3>
      </div>

      <div className="text-center space-y-1">
        <div className={`text-5xl font-black ${me.ampel === 'gruen' ? 'text-emerald-600' : me.ampel === 'gelb' ? 'text-yellow-600' : 'text-red-600'}`}>
          {me.retourenquote.toFixed(1)}%
        </div>
        <div className="text-sm text-gray-500">Retourenquote</div>

...
const MOCK: ApiData = {
  fahrer: [{ fahrer_id: 'me', fahrer_name: 'Du', rang: 2, retouren_pct: 5.0, returned: 1, total: 20, rank_delta: 1, ampel: 'gruen', alert_bottom: false }],
  team_avg_pct: 7.6,
  gesamt: 4,
};
...
function coaching(a: string): string {
  if (a === 'gruen') return 'Sehr gut! Du gehörst zur Spitzengruppe mit den wenigsten Retouren. Weiter so!';
  if (a === 'gelb')  return 'Solide! Kommuniziere bei Nicht-Erreichbarkeit früh mit dem Disponenten, um Retouren zu vermeiden.';
  return                   'Fokus auf Ablieferung! Nutze Nachbar-Option oder Paketsicherung, um Retouren zu reduzieren.';
}

export function FahrerPhase3267MeineRetourenquote({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  // Inverted bar: Rang 1 (lowest retouren) = full bar, last rank = small bar
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Retourenquote</span>
          {me.alert_bottom && (
      ...
      {open && (
        <div className="p-4 space-y-4">
          <div className="text-center">
            <div className={`text-5xl font-black ${col.text}`}>{fmtPct(me.retouren_pct)}</div>
            <div className="text-sm text-gray-500 mt-1">Retourenquote ({me.returned}/{me.total} Stopps)</div>
            <div className={`text-3xl font-bold mt-1 ${col.text}`}>Rang #{me.rang}</div>
          </div>

          <div>
            <div className="text-xs text-gray-500 mb-1 flex justify-between">
              <span>Höchste Retouren</span><span>Wenigste Retouren</span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-center text-xs">
            ...
            <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-2">
              ...
              <div className="font-bold text-blue-600 dark:text-blue-400">{fmtPct(d.team_avg_pct)}</div>
            </div>
          </div>

...
function coachingTipp(val: number): { text: string; color: string } {
  if (val <= 5) return {
    text: 'Top-Abholer! Deine kurze Wartezeit am Restaurant spart Zeit und steigert deinen Durchsatz.',
    color: 'text-green-300',
  };
  if (val <= 10) return {
    text: 'Gute Wartezeit. Prüfe, ob du früher am Restaurant eintreffen kannst, um noch schneller zu sein.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Lange Wartezeit am Restaurant. Sprich mit dem Disponenten oder kümmere dich um pünktliches Erscheinen.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5129MeineWartezeitRestaurant({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Wartezeit am Restaurant</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.avg_wartezeit_min} <span className="text-xl font-semibold">min</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.avg_wartezeit_min / Math.max(teamAvg * 2, 1)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.avg_wartezeit_min} min</span>
          <span>{teamAvg} min</span>
        </div>
      </div>

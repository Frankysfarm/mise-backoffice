...
const MOCK: ApiData = { avg_min: 15, rang: 2, rank_delta: 0, ampel: 'gruen', team_avg_min: 21, fahrer_count: 4 };

interface Props { driverId: string; locationId: string | null; isOnline: boolean; }

export function FahrerPhase4073MeineLieferzeit({ driverId, locationId, isOnline }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/delivery/admin/fahrer-lieferzeit-ranking?location_id=${locationId}&driver_id=${driverId}`);
      if (res.ok) {
        const json = await res.json();
        if (!json.error && json.fahrer) {
          const me = json.fahrer.find((f: { fahrer_id: string }) => f.fahrer_id === driverId) ?? json.fahrer[0];
          if (me) setData({ avg_min: me.avg_min, rang: me.rang, rank_delta: me.rank_delta, ampel: me.ampel, team_avg_min: json.team_avg_min, fahrer_count: json.gesamt });
        }
      }
  ...
  if (!isOnline) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Lieferzeit-Ranking nicht verfügbar (offline)</span>
      </div>
    );
  }

  const valueColor = data.ampel === 'gruen' ? 'text-emerald-600' : data.ampel === 'gelb' ? 'text-yellow-500' : 'text-red-500';
  const coaching = data.ampel === 'gruen' ? 'Super – deine Lieferzeit ist ausgezeichnet!' : data.ampel === 'gelb' ? 'Tipp: Optimiere deine Route für kürzere Lieferzeiten.' : 'Achtung: Deine Lieferzeit liegt deutlich über dem Teamdurchschnitt.';
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Meine Lieferzeit</span>
        </div>
        {loading && <span className="w-3 h-3 border-2 border-gray-300 border-t-transparent rounded-full animate-spin" />}
      </div>
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-bold tabular-nums ${valueColor}`}>{data.avg_min}<span className="text-2xl font-semibold ml-1">min</span></span>

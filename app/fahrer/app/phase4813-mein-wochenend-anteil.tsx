...
function coachingTipp(pct: number): { text: string; color: string } {
  if (pct >= 60) return { text: 'Sehr hoher Wochenend-Anteil! Plane mehr Wochentag-Schichten für eine bessere Balance.', color: 'text-red-400' };
  if (pct >= 30) return { text: 'Moderater Wochenend-Anteil — du arbeitest regelmäßig am Wochenende.', color: 'text-yellow-400' };
  return { text: 'Gute Balance — du arbeitest überwiegend unter der Woche!', color: 'text-green-400' };
}
...
export function FahrerPhase4813MeinWochenenAnteil({ driverId, locationId, isOnline }: { driverId: string; locationId: string | null; isOnline: boolean }) {
  ...
  return (
    <div className="rounded-xl border border-violet-800 bg-violet-950/40 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-violet-300">Mein Wochenend-Anteil</span>
      </div>

      <div className="text-center mb-4">
        <div className={`text-4xl font-bold ${ampelColor(me.ampel)}`}>{me.wochenend_anteil_pct.toFixed(1)}%</div>
        <div className="text-xs text-gray-400 mt-1">Sa/So-Schichten (letzte 30 Tage)</div>
      ...
      <div className="space-y-2 mb-3">
        <div>
          ...
          <div className="h-1.5 bg-gray-800 rounded-full overflow-hidden">
            <div className="h-full bg-violet-500 rounded-full" style={{ width: `${(me.wochenend_anteil_pct / maxPct) * 100}%` }} />
          </div>
        </div>
        <div>
          <div className="flex justify-between text-xs mb-1">

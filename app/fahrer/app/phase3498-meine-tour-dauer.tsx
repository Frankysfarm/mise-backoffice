...
const COACHING: Record<string, string> = {
  gruen: 'Ausgezeichnet! Deine Tour-Dauer ist sehr kurz — beste Effizienz!',
  gelb:  'Solide! Überprüfe ob Routen optimal geplant sind.',
  rot:   'Deine Tour-Dauer ist hoch. Bitte Routen-Planung mit Dispatch besprechen.',
};
...
export function FahrerPhase3498MeineTourDauer({
  driverId,
  isOnline,
}: {
  driverId: string;
  ...
  const load = useCallback(async () => {
    if (!isOnline) return;
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-tour-dauer?location_id=mock&driver_id=${driverId}`, { cache: 'no-store' });
  ...
  return (
    <div className="border rounded-xl bg-white dark:bg-gray-900 shadow-sm mb-3 p-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="font-semibold text-sm">Meine Tour-Dauer</span>
      </div>

      <div className="text-center mb-3">
        <div className={`text-5xl font-black tabular-nums ${AMPEL_COLORS[me.ampel]}`}>
          {me.avg_dauer_min}min
        </div>
        <div className="text-xs text-gray-400 mt-0.5">Ø Tour-Dauer (letzte 30 Tage)</div>
      ...
      <div className="flex justify-between text-xs text-gray-500 mb-3">
        ...
        <span>Team-Ø: <span className="font-semibold">{teamAvg}min</span></span>
        <span>Ziel: <span className="font-semibold text-emerald-600">≤50min</span></span>
      </div>

...
const MOCK: ApiData = {
  fahrer: [{ fahrer_id: 'me', fahrer_name: 'Ich', rang: 2, auslastung_pct: 65.8, rank_delta: -1, ampel: 'gelb', alert_bottom: false }],
  team_avg_pct: 59.5,
  gesamt: 4,
};
...
function tip(ampel: string): string {
  if (ampel === 'rot')  return 'Deine Auslastung ist im untersten Viertel. Übernimm weitere Touren, um dein Ranking zu verbessern.';
  if (ampel === 'gelb') return 'Du liegst im Mittelfeld. Noch etwas mehr Einsatz bringt dich in die Top 25%.';
  return 'Hervorragend! Du zählst zu den aktivsten Fahrern.';
}

export function FahrerPhase3077MeinAuslastungsRanking({
  driverId,
  locationId,
  isOnline,
}: {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Mein Auslastungs-Ranking</span>
        </div>
      ...
      {open && (
        <div className="p-4 space-y-3">
          ...
          <div className="flex items-center justify-center gap-6">
            ...
            <div className="text-center">
              <div className={`text-4xl font-bold leading-none ${cls.text}`}>{entry.auslastung_pct.toFixed(1)}<span className="text-xl">%</span></div>
          ...
          <div className="grid grid-cols-2 gap-2 text-center text-xs">
            ...
            <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-2">
              ...
              <div className={`font-bold ${teamAvg >= 60 ? 'text-green-600' : teamAvg >= 40 ? 'text-amber-600' : 'text-red-600'}`}>
                {teamAvg.toFixed(1)} %
              </div>
            </div>
          </div>

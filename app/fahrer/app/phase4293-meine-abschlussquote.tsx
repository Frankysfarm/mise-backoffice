...
export function FahrerPhase4293MeineAbschlussquote({ driverId, locationId, isOnline }: Props) {
  ...
  const coaching =
    me.ampel === 'gruen'
      ? 'Ausgezeichnet! Du schließt fast alle Aufträge erfolgreich ab.'
      : me.ampel === 'gelb'
      ? 'Tipp: Bei Problemen frühzeitig Kundenkontakt aufnehmen, um Abbrüche zu vermeiden.'
      : 'Achtung: Deine Abschlussquote liegt deutlich unter dem Teamdurchschnitt.';
  ...
  return (
    <div className={`rounded-2xl border p-4 space-y-3 ${ampelBg} border-gray-200`}>
      ...
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-extrabold ${ampelColor}`}>{me.quote_pct}</span>
        <div className="pb-1 space-y-0.5">
          <span className="text-sm text-gray-500">% Abschluss</span>
      ...
      <div className="flex items-center justify-between text-xs text-gray-500">
        <span>Team-Ø: <span className="font-semibold text-gray-700">{data.team_avg_pct}%</span></span>

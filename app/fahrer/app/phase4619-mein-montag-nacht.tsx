...
function coaching(pct: number): string {
  if (pct >= 30) return 'Top! Du bist oft montagnachts dabei.';
  if (pct >= 15) return 'Gut! Montagnacht-Anteil im Bereich.';
  return 'Wenig Montagnacht-Touren — Mo 22–02 Uhr einplanen?';
}

export function FahrerPhase4619MeinMontagNacht({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-sm">Offline — Montagnacht nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-indigo-200 dark:border-indigo-900 bg-white dark:bg-gray-900 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900 dark:text-gray-100">Mein Montagnacht</h3>
      </div>

...
import { ChevronDown, ChevronUp, Package, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Top! Du bündelst viele Pakete pro Tour — das macht dich besonders effizient.',
  gelb: 'Gut — versuche mehr Pakete pro Tour zu bündeln, um deinen Score zu steigern.',
  rot: 'Tipp: Sprich mit dem Dispatcher — mehr Pakete pro Tour verbessern die Gesamteffizienz.',
};

export function FahrerPhase3508MeinePaketeProTour({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const [data, setData] = useState<FahrerPaketeProTour | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-pakete-pro-tour?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
      if (!r.ok) return;
        ...
        <div className="px-4 pb-4 space-y-3">
          <div className="flex items-end justify-center gap-4">
            <div className="text-center">
              <div className={`text-5xl font-bold font-mono ${AMPEL_COLOR[data.ampel]}`}>
                {data.pakete_pro_tour}
              </div>
              <div className="text-xs text-gray-500 mt-0.5">Pakete/Tour</div>
            </div>
            <div className="text-center">
          ...
          <div className="grid grid-cols-2 gap-3 text-center">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2">
              ...
              <div className="text-xs font-semibold mt-0.5">{teamAvg}/Tour</div>
            </div>
          </div>

...
import { Star, AlertTriangle, ChevronDown, ChevronUp, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
function coachingTipp(a: string): string {
  if (a === 'rot')  return 'Niedrige Kundenbewertung! Achte auf freundliches Auftreten und pünktliche Lieferung — das steigert deine Sterne sofort.';
  if (a === 'gelb') return 'Gute Bewertung! Ein Lächeln und kurze Wartezeit an der Tür helfen, die 4.5-Sterne-Marke zu knacken.';
  return 'Top-Bewertung! Du begeisterst Kunden — mach weiter so und bleib #1 im Team!';
}

export function FahrerPhase3336MeineKundenbewertung({
  driverId,
  locationId,
  isOnline,
}: {
        ...
        <div className="p-4 space-y-4">
          <div className={`rounded-xl p-4 text-center ${cls.bg}`}>
            <div className={`text-5xl font-black ${cls.text}`}>
              {me.bewertung_avg}<span className="text-3xl"> ★</span>
            </div>
            <div className={`text-3xl font-bold ${cls.text} mt-1`}>Rang #{me.rang}</div>
            <div className="text-xs text-gray-400 mt-1">Ziel ≥ 4.5 ★</div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-center text-xs">
            ...
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="font-bold text-sm text-gray-700 dark:text-gray-200">{d.team_avg} ★</div>
            </div>
          </div>
          ...
          <div className="text-xs text-gray-400 text-center">Kundenbewertung · letzte 30 Tage</div>
        </div>
      )}
    </div>
  );

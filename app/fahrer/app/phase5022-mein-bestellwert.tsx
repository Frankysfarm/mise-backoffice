...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 40) return {
    text: 'Top Bestellwert! Du lieferst die wertvollsten Bestellungen — hervorragend.',
    color: 'text-green-300',
  };
  if (val >= 25) return {
    text: 'Solider Bestellwert. Mit besserer Zonenauswahl kannst du noch höhere Werte erreichen.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Niedriger Ø-Bestellwert. Sprich mit der Disposition über eine bessere Zonen-Zuteilung.',
    color: 'text-gray-400',
  };
}
...
export function FahrerPhase5022MeinBestellwert({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-bestellwert-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Mein Bestellwert — offline</span>
      </div>
    );
  }
  ...
  const tipp = coachingTipp(mein.avg_bestellwert);
  const maxBar = 60;
  const ichPct = Math.min(100, (mein.avg_bestellwert / maxBar) * 100);
  const avgPct = Math.min(100, (data.team_avg_bestellwert / maxBar) * 100);
  ...
  return (
    <div className="rounded-2xl border border-emerald-700 bg-emerald-950/40 overflow-hidden mb-3">
      ...
      <div className="px-4 py-3 border-b border-emerald-700/40 flex items-center gap-2 bg-emerald-900/20">
        ...
        <span className="text-sm font-semibold text-emerald-200">Mein Ø-Bestellwert je Tour (letzte 30 Tage)</span>
      </div>
      ...
      <div className="px-4 py-4 flex items-center justify-between">
        <div>
          <div className={`text-4xl font-black ${ampelColor(mein.ampel)}`}>
            {mein.avg_bestellwert.toFixed(2)} €
          </div>
          <div className="text-xs text-gray-500 mt-0.5">Ø Bestellwert je Tour</div>
        </div>
      ...
      <div className="px-4 pb-3 space-y-1.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-emerald-300 w-16 text-right">{mein.avg_bestellwert.toFixed(2)} €</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-gray-400 w-16 text-right">{data.team_avg_bestellwert.toFixed(2)} €</span>
        </div>
      </div>

...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 35) return {
    text: 'Hervorragend! Du lieferst hochwertige Bestellungen und maximierst den Umsatz je Fahrt.',
    color: 'text-green-300',
  };
  if (val >= 25) return {
    text: 'Guter Bestellwert. Optimiere deine Lieferrouten für noch mehr Umsatz je Stopp.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Niedriger Ø-Bestellwert. Mehr Lieferungen größerer Bestellungen steigern dein Ranking.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5187MeinUmsatzProLieferung({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-umsatz-pro-lieferung-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Mein Ø-Umsatz/Lieferung — offline</span>
      </div>
    );
  }
  ...
  const tipp    = coachingTipp(mein.avg_euro);
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Mein Ø-Umsatz/Lieferung</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.avg_euro.toFixed(2)} <span className="text-xl font-semibold">€</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.avg_euro / Math.max(teamAvg * 1.5, 1)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.avg_euro.toFixed(2)}€</span>

...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 35) return {
    text: 'Hervorragender Umsatz/Stunde! Du arbeitest sehr effizient und bringst hohen Umsatz je Schichtstunde.',
    color: 'text-green-300',
  };
  if (val >= 20) return {
    text: 'Solider Umsatz/Stunde. Mehr Lieferungen pro Stunde oder höhere Bestellwerte können den Schnitt steigern.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Niedriger Umsatz/Stunde. Kürzere Pausen und effizientere Routen können helfen.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5252MeinUmsatzProStunde({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Mein Umsatz/Stunde — offline</span>
      </div>
    );
  }
  ...
  const teamAvg = data.team_avg_umsatz;
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Mein Umsatz/Stunde</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.umsatz_pro_stunde.toFixed(0)}<span className="text-xl font-semibold"> €/h</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.umsatz_pro_stunde / Math.max(teamAvg * 2, 1)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.umsatz_pro_stunde.toFixed(0)} €/h</span>
          <span>{teamAvg.toFixed(0)} €/h</span>
        </div>
      </div>

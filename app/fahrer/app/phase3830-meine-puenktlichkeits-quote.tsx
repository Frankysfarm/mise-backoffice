...
const COACHING: Record<string, string> = {
  gruen: 'Sehr pünktlich! Weiter so.',
  gelb: 'Du hast einige Verspätungen. Versuche schneller zu navigieren.',
  rot: 'Viele Verspätungen! Bitte Route und Pausen prüfen.',
};

export function FahrerPhase3830MeinePuenktlichkeitsQuote({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/delivery/admin/fahrer-puenktlichkeits-quote?location_id=${locationId}&driver_id=${driverId}`,
      );
  ...
  const zielBarPct = Math.min((f.puenktlichkeits_pct / 100) * 100, 100);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="font-semibold text-gray-900 text-sm">Meine Pünktlichkeit</span>
        {loading && <span className="w-3 h-3 border-2 border-blue-400 border-t-transparent rounded-full animate-spin" />}
      </div>

      {/* Hauptwert */}
      <div className="text-center">
        <div className={`text-5xl font-black leading-none ${valueColor}`}>{f.puenktlichkeits_pct}%</div>
        <div className="mt-1 text-sm text-gray-500">Pünktlichkeitsquote (30 Tage)</div>
      ...
      {f.alert_verspaetet && (
        <div className="flex items-center gap-2 px-3 py-2 bg-orange-50 border border-orange-200 rounded-lg text-xs text-orange-800">
          ...
          <span>Viele Verspätungen! Bitte Route und Pausen prüfen.</span>
        </div>
      )}
      ...
      <div>
        <div className="flex justify-between text-[10px] text-gray-400 mb-0.5">
          ...
          <span>{f.puenktlichkeits_pct}% / 100%</span>
        </div>
        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">

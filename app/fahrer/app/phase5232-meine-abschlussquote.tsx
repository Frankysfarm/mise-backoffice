...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 98) return {
    text: 'Hervorragende Abschlussquote! Nahezu alle Touren erfolgreich abgeschlossen — Top-Zuverlässigkeit.',
    color: 'text-green-300',
  };
  if (val >= 95) return {
    text: 'Gute Abschlussquote. Kleine Verbesserungen bei kritischen Stopps können 98%+ erreichen.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Niedrige Abschlussquote. Analysiere abgebrochene Touren — häufige Gründe: Zeitdruck, Routing, Übergabeprobleme.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5232MeineAbschlussquote({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Abschlussquote</span>
      </div>
      ...
      <div className="px-4 py-4 text-center">
        ...
        <div className="text-xs text-gray-500 mt-1">
          {mein.completed_tours} / {mein.assigned_tours} Touren abgeschlossen
        </div>
      </div>

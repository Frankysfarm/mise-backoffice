...
export function FahrerPhase4038MeineAuslastung({ driverId, locationId, isOnline }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/delivery/admin/fahrer-auslastungs-ranking?location_id=${locationId}&driver_id=${driverId}`);
      if (res.ok) {
        const json = await res.json();
        if (!json.error && json.fahrer?.length) {
          const me = json.fahrer[0];
          setData({ ...me, team_avg_pct: json.team_avg_pct, fahrer_count: json.gesamt });
        }
      }
    } catch {
      // Mock-Fallback
  ...
  if (!isOnline) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Auslastung nicht verfügbar (offline)</span>
      </div>
    );
  }
  ...
  const coaching =
    data.ampel === 'gruen'
      ? 'Top – deine Auslastung ist sehr gut!'
      : data.ampel === 'gelb'
      ? 'Tipp: Nimm mehr Aufträge an, um deine Auslastung zu steigern.'
      : 'Achtung: Deine Auslastung ist unterdurchschnittlich.';

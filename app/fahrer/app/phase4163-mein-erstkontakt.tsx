...
import { Phone, TrendingUp, TrendingDown, Minus, WifiOff } from 'lucide-react';
...
export function FahrerPhase4163MeinErstkontakt({ driverId, locationId, isOnline }: Props) {
  ...
  const valColor = data.ampel === 'gruen' ? 'text-rose-500' : data.ampel === 'gelb' ? 'text-yellow-500' : 'text-red-500';
  ...
  const coaching = data.ampel === 'gruen'
    ? 'Stark – du nimmst sehr schnell Kontakt zu Kunden auf!'
    : data.ampel === 'gelb'
    ? 'Tipp: Bereite dich schon während der Anfahrt auf den Kundenkontakt vor.'
    : 'Achtung: Deine Erstkontaktzeit liegt über dem Teamdurchschnitt.';
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-bold ${valColor}`}>{data.avg_sek?.toFixed(0)}</span>
        <div className="pb-1 space-y-0.5">
          <span className="text-xl font-medium text-gray-500">sek</span>
      ...
      <div className="flex justify-between text-xs text-gray-400">
        <span>Team-Ø {data.team_avg_sek?.toFixed(0)} sek</span>

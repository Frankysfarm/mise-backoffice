...
export function FahrerPhase4012MeineFeierabendPuenktlichkeit({ driverId, locationId, isOnline }: Props) {
  ...
  if (!isOnline) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-2 text-gray-400 text-sm">
        ...
        <span>Offline – Feierabend-Pünktlichkeit nicht verfügbar</span>
      </div>
    );
  }
  ...
  const coaching =
    me?.ampel === 'gruen'
      ? 'Top – du beendest deine Schicht fast immer pünktlich!'
      : me?.ampel === 'gelb'
      ? 'Tipp: Plane deine letzte Tour so, dass du rechtzeitig fertig bist.'
      : 'Achtung: Deine Schichten enden oft zu spät – sprich mit dem Dispatcher.';
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-end justify-center gap-3 py-2">
        <div className="text-center">
          <div className={`text-5xl font-black ${rateColor}`}>{me?.feierabend_pct}%</div>
          <div className="flex items-center justify-center gap-1 mt-1">
            {DeltaIcon}
            <span className="text-xs text-gray-400">Feierabend-Quote (30 Tage)</span>
          </div>
        </div>

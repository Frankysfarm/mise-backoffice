...
function barColor(min: number) {
  ...
  if (min <= 30) return 'bg-amber-400';
  return 'bg-red-500';
}

function coachingTipp(ampel: string, avg: number): string {
  if (ampel === 'gruen') return `Top! Ø ${avg.toFixed(1)} min pro Stopp — du lieferst sehr effizient.`;
  if (ampel === 'gelb') return `Ø ${avg.toFixed(1)} min pro Stopp — etwas langsam. Versuche flüssiger zu liefern.`;
  return `Ø ${avg.toFixed(1)} min pro Stopp — über 30 min. Bitte prüfe deine Route und melde dich beim Dispatcher.`;
}

export function FahrerPhase2474MeineLieferzeitEffizienz({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-lieferzeit-effizienz?location_id=${locationId}&driver_id=${driverId}`)
        .then(r => r.json())
        .then(setData)
        .catch(console.error);
    load();
  ...
  const MAX = 45;
  const pct = Math.min(100, Math.round((f.avg_lieferzeit_min / MAX) * 100));
      ...
      {open && (
        <div className="bg-white px-4 pb-3 pt-2 space-y-3">
          ...
          <div className="text-center py-2">
            <div className={`text-4xl font-bold ${st.val}`}>{f.avg_lieferzeit_min.toFixed(1)} min</div>
            <div className="text-xs text-gray-400 mt-1">Ø Lieferzeit pro Stopp</div>
          </div>
          ...
          <div className="space-y-1">
            <div className="relative h-3 rounded-full bg-gray-200">
              <div
                className={`absolute left-0 top-0 h-full rounded-full ${barColor(f.avg_lieferzeit_min)}`}
                style={{ width: `${pct}%` }}
              />
              <div className="absolute top-0 h-full border-l-2 border-dashed border-green-500" style={{ left: `${(20 / MAX) * 100}%` }} />
              <div className="absolute top-0 h-full border-l-2 border-dashed border-amber-400" style={{ left: `${(30 / MAX) * 100}%` }} />
            </div>
            <div className="flex justify-between text-[10px] text-gray-400 px-0.5">
              <span>0 min</span>
              <span className="text-green-600">20 Ziel</span>
          ...
          <div className="grid grid-cols-2 gap-2 text-center">
            {[
              { label: 'Vorwoche', val: `${f.avg_lieferzeit_min_vw.toFixed(1)} min`, color: 'text-gray-600' },
              ...
              { label: 'Ziel', val: '≤20 min', color: 'text-green-600' },
              { label: 'Team-Ø', val: `${(data.team_avg_lieferzeit_min ?? 0).toFixed(1)} min`, color: 'text-blue-600' },
            ].map(k => (
          ...
          <div className={`rounded-lg px-3 py-2 text-xs ${st.bg} ${st.text} border`}>
            {coachingTipp(f.ampel, f.avg_lieferzeit_min)}
          </div>
        </div>
      )}
    </div>

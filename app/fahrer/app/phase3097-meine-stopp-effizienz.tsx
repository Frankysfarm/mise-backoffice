...
const MOCK: ApiData = {
  fahrer: [{ fahrer_id: 'me', fahrer_name: 'Ich', rang: 2, stopps_pro_stunde: 3.5, total_stopps: 7, aktive_stunden: 2.0, rank_delta: -1, ampel: 'gelb', alert_bottom: false }],
...
function tip(ampel: string): string {
  if (ampel === 'rot')  return 'Deine Stopp-Effizienz liegt im unteren Viertel. Kürzere Wartezeiten an den Stopps helfen, dein Ranking zu verbessern.';
  if (ampel === 'gelb') return 'Du liegst im Mittelfeld. Noch ein paar schnelle Stopps bringen dich in die Top 25%.';
  return 'Stark! Du gehörst zu den effizientesten Fahrern heute — top Stopp-Tempo!';
}

export function FahrerPhase3097MeineStoppEffizienz({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const url = driverId
      ? `/api/delivery/admin/fahrer-stopp-effizienz-ranking?location_id=${locationId}&driver_id=${driverId}`
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Stopp-Effizienz</span>
        </div>
      ...
      {open && (
        <div className="p-4 space-y-3">
          ...
          <div className="flex items-center justify-center gap-6">
            ...
            <div className="text-center">
              <div className={`text-4xl font-bold leading-none ${cls.text}`}>{entry.stopps_pro_stunde.toFixed(1)}</div>
              <div className="text-xs text-gray-500 mt-1">Stopps/h</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-center text-xs">
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5">Stopps heute</div>
              <div className="font-bold text-base text-gray-700 dark:text-gray-200">{entry.total_stopps}</div>
            </div>
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              <div className="text-gray-500 dark:text-gray-400 font-medium mb-0.5">Aktive Zeit</div>
              <div className="font-bold text-base text-gray-700 dark:text-gray-200">{entry.aktive_stunden.toFixed(1)} h</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-center text-xs">
            ...
            <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-2">
              ...
              <div className="font-bold text-blue-600 dark:text-blue-400">{teamAvg.toFixed(1)} Stp/h</div>
            </div>
          </div>

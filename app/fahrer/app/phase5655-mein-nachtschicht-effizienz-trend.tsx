...
// Phase 5655 — Mein Nachtschicht-Effizienz-Trend (Fahrer) — Batch 108
...
function coaching(delta: number): { text: string; color: string } {
  if (delta > 0) return {
    text: `Deine Nachtschicht-Effizienz ist um ${delta.toFixed(2)} Touren/Std gestiegen — top! Kluge Routenwahl in der Nacht macht den Unterschied.`,
    color: 'text-indigo-400',
  };
  if (delta === 0) return {
    text: 'Deine Nachtschicht-Effizienz ist stabil. Nutze ruhigere Straßen zwischen 20–24h für kürzere Lieferzeiten.',
    color: 'text-yellow-300',
  };
  return {
    text: `Effizienz um ${Math.abs(delta).toFixed(2)} T/Std gesunken — prüfe deine Nachtroutenplanung und reduziere Wartezeiten beim Restaurant im Nachtbetrieb.`,
    color: 'text-red-400',
  };
}

export function FahrerPhase5655MeinNachtschichtEffizienzTrend({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-nachtschicht-effizienz-trend-ranking?location_id=${locationId}`);
      if (r.ok) {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-lg bg-gray-900 border border-gray-700 p-3 flex items-center gap-2">
        ...
        <span className="text-xs text-gray-500">Kein Netz — Nachtschicht-Trend offline</span>
      </div>
    );
  }
    ...
    <div className={`rounded-lg bg-gray-900 border ${BORDER[data.ampel]} p-3 space-y-2.5`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-white">Mein Nachtschicht-Trend</span>
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-mono text-indigo-400 w-12 text-right">{data.aktuell_touren_pro_std.toFixed(2)} T/h</span>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="text-[9px] text-gray-600 text-right">20–24h · 30-Tage-Trend · Touren/Std</div>
    </div>
  );
}

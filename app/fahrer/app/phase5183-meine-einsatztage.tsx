...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 15) return {
    text: 'Hervorragende Präsenz! Deine Verlässlichkeit begeistert das Team und die Kunden.',
    color: 'text-green-300',
  };
  if (val >= 8) return {
    text: 'Gute Einsatzquote. Mehr Einsatztage steigern dein Ranking und dein Einkommen.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Wenige Einsatztage. Regelmäßige Verfügbarkeit verbessert dein Ranking deutlich.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5183MeineEinsatztage({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-einsatztage-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Einsatztage — offline</span>
      </div>
    );
  }
  ...
  const tipp    = coachingTipp(mein.einsatztage);
  const teamAvg = data.team_avg_tage;
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Einsatztage (30 Tage)</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.einsatztage} <span className="text-xl font-semibold">Tage</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.einsatztage / Math.max(teamAvg * 1.5, 1)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.einsatztage} T</span>
          <span>{teamAvg.toFixed(1)} T</span>
        </div>
      </div>

...
const TIPPS: Record<string, string> = {
  gruen: 'Super! Deine Stornoquote ist sehr niedrig. Weiter so — dein Engagement zahlt sich aus!',
  gelb:  'Stornoquote im mittleren Bereich. Prüfe ob Lieferadressen klar sind und du schnell erreichbar bist.',
  rot:   'Hohe Stornoquote! Kontaktiere den Dispatcher für Unterstützung und prüfe die Bestelldetails sorgfältig.',
};

const MOCK_ENTRY: FahrerEntry = {
  fahrer_id: 'mock', quote_pct: 0.0, trend: 'fallend', trend_delta: -3.0, ampel: 'gruen',
};
const MOCK_DATA: ApiData = { fahrer: [MOCK_ENTRY], team_durchschnitt: 7.0 };
...
export function FahrerPhase3007MeineStornoquote({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const load = () => {
      const url = `/api/delivery/admin/fahrer-stornoquote?location_id=${locationId}${driverId ? `&driver_id=${driverId}` : ''}`;
      fetch(url)
        .then(r => r.json())
  ...
  const pct = Math.min(100, (entry.quote_pct / MAX_BAR) * 100);
  const zielPct = (ZIEL_PCT / MAX_BAR) * 100;
  const teamAmpel = data.team_durchschnitt <= ZIEL_PCT ? 'gruen' : data.team_durchschnitt <= 15 ? 'gelb' : 'rot';
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Stornoquote</span>
        </div>
      ...
      {open && (
        <div className="px-4 py-4 space-y-4">
          <div className="text-center">
            <div className={`text-4xl font-black ${text}`}>{entry.quote_pct.toFixed(1)}%</div>
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">Stornoquote heute</div>
            <div className="flex items-center justify-center gap-1 mt-1">
              <TrendIcon trend={entry.trend} />
              <span className="text-xs text-gray-500 dark:text-gray-400">
                {entry.trend_delta > 0 ? '+' : ''}{entry.trend_delta.toFixed(1)}% vs. gestern
              </span>
            </div>
          </div>

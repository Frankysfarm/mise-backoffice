...
function coachingTipp(val: number): { text: string; color: string } {
  if (val <= 25) return {
    text: 'Super schnelle Lieferzeiten! Kunden sind begeistert von deiner Geschwindigkeit.',
    color: 'text-green-300',
  };
  if (val <= 40) return {
    text: 'Gute Lieferzeit. Optimiere deine Route für noch schnellere Zustellungen.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Lange Lieferzeit. Analysiere deine Stopps — Verzögerungen kosten Trinkgeld und Kundenzufriedenheit.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5174MeineAvgLieferzeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-avg-lieferzeit-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Ø-Lieferzeit — offline</span>
      </div>
    );
  }
  ...
  const tipp    = coachingTipp(mein.avg_lieferzeit_min);
  const teamAvg = data.team_avg_min;
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Ø-Lieferzeit</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.avg_lieferzeit_min.toFixed(1)} <span className="text-xl font-semibold">min</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.avg_lieferzeit_min / Math.max(teamAvg * 2, 1)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.avg_lieferzeit_min.toFixed(1)} min</span>
          <span>{teamAvg.toFixed(1)} min</span>
        </div>
      </div>

...
const COACHING: Record<string, string> = {
  gruen: 'Sehr schnell! Deine erste Lieferung startet früh — optimal!',
  gelb:  'Solide! Prüfe ob Abfahrt und Routenstart beschleunigt werden können.',
  rot:   'Deine erste Lieferung startet spät. Bitte Abfahrts-Routine mit Dispatch besprechen.',
};
...
export function FahrerPhase3503MeineErstelieferungZeit({
  driverId,
  isOnline,
}: {
  driverId: string;
  ...
  const load = useCallback(async () => {
    if (!isOnline) return;
    try {
      const r = await fetch(`/api/delivery/admin/fahrer-erste-lieferung-zeit?location_id=mock&driver_id=${driverId}`, { cache: 'no-store' });
  ...
  return (
    <div className="border rounded-xl bg-white dark:bg-gray-900 shadow-sm mb-3 p-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="font-semibold text-sm">Meine Erste-Lieferung-Zeit</span>
      </div>

      <div className="text-center mb-3">
        <div className={`text-5xl font-black tabular-nums ${AMPEL_COLORS[me.ampel]}`}>
          {me.avg_erste_lieferung_min}min
        </div>
        <div className="text-xs text-gray-400 mt-0.5">Ø Zeit bis erste Lieferung (letzte 30 Tage)</div>
      ...
      <div className="mb-3">
        <div className="flex justify-between text-[9px] text-gray-400 mb-1">
          <span>Langsamster Start</span>
          <span>Schnellster Start</span>
        </div>
      ...
      <div className="flex justify-between text-xs text-gray-500 mb-3">
        ...
        <span>Ziel: <span className="font-semibold text-emerald-600">≤10min</span></span>
      </div>

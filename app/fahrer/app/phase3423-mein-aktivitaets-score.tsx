...
import { ChevronDown, ChevronUp, Activity, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Stark! Du gehörst zu den aktivsten Fahrern im Team.',
  gelb: 'Gut unterwegs — mehr Touren und Stops steigern deinen Score.',
  rot: 'Tipp: Mehr Schichten und Touren annehmen verbessert deinen Aktivitäts-Score.',
};

export function FahrerPhase3423MeinAktivitaetsScore({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const [data, setData] = useState<FahrerAktivitaet | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-aktivitaets-score?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
      if (!r.ok) return;
        ...
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3 text-center">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2">
              ...
              <div className="text-xs font-semibold mt-0.5">{teamAvg} Pkt</div>
            </div>
          </div>

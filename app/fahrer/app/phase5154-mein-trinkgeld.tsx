...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 2) return {
    text: 'Exzellentes Trinkgeld! Dein Kundenservice überzeugt. Weiter so!',
    color: 'text-green-300',
  };
  if (val >= 1) return {
    text: 'Gutes Trinkgeld. Mit freundlichem Auftreten und Pünktlichkeit kannst du noch mehr erreichen.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Niedriges Ø-Trinkgeld. Fokussiere auf Freundlichkeit und schnelle Lieferzeiten.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5154MeinTrinkgeld({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Mein Ø-Trinkgeld — offline</span>
      </div>
    );
  }
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Mein Ø-Trinkgeld</span>
      </div>

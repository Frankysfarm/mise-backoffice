...
function coachingTipp(ampel: string): string {
  if (ampel === 'rot')  return 'Pünktlichkeit zu niedrig! Frühzeitig starten und Verzögerungen sofort melden.';
  if (ampel === 'gelb') return 'Fast auf Kurs! Kürzere Stopps und optimale Reihenfolge verbessern die Pünktlichkeit.';
  return 'Sehr pünktlich — weiter so! Kunden schätzen zuverlässige Lieferzeiten.';
}
...
export function FahrerPhase2927MeinePuenktlichkeit({ driverId, locationId, isOnline }: Props) {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-puenktlichkeit?location_id=${locationId}`)
        ...
        .then((d: ApiData) => {
          ...
          if (f) setData({ quote_pct: f.quote_pct, trend: f.trend, trend_delta: f.trend_delta, ampel: f.ampel, team_avg: d.team_durchschnitt });
          else setData(MOCK);
        })
        .catch(() => setData(MOCK));
    load();
      ...
      {open && (
        <div className="px-4 py-4 space-y-4">
          <div className="text-center">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">Pünktlichkeits-Rate heute</div>
          </div>

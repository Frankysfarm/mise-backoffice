...
function coachingTipp(val: number): { text: string; color: string } {
  if (val <= 5) return {
    text: 'Minimale Wartezeit! Du bist immer pünktlich zur Abholung — perfekte Abstimmung.',
    color: 'text-green-300',
  };
  if (val <= 10) return {
    text: 'Akzeptable Wartezeit. Bessere Timing-Abstimmung mit der Küche kann helfen.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Lange Wartezeit. Komme etwas später und stimme dich besser mit der Küche ab.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5210MeineWartezeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Ø-Wartezeit — offline</span>
      </div>
    );
  }
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Ø-Wartezeit (Restaurant)</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.avg_wartezeit_min.toFixed(0)} <span className="text-xl font-semibold">min</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.avg_wartezeit_min.toFixed(0)} min</span>

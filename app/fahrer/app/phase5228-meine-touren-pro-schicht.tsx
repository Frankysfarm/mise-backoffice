...
function coachingTipp(val: number, ziel: number): { text: string; color: string } {
  if (val >= 4) return {
    text: 'Starke Schicht-Auslastung! Du nutzt deine Schichten effizient mit vielen Touren.',
    color: 'text-green-300',
  };
  if (val >= 2.5) return {
    text: `Solide Leistung. Ziel ist ${ziel.toFixed(1)} Touren/Schicht — noch Potential nach oben.`,
    color: 'text-yellow-400',
  };
  return {
    text: 'Wenige Touren pro Schicht. Aktivere Annahme und kürzere Pausen können die Rate steigern.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5228MeineTourenProSchicht({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-touren-pro-schicht-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Ø-Touren/Schicht — offline</span>
      </div>
    );
  }
  ...
  const tipp    = coachingTipp(mein.touren_pro_schicht, data.ziel ?? 6);
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Ø-Touren/Schicht</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.touren_pro_schicht.toFixed(1)} <span className="text-xl font-semibold">/Sch.</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.touren_pro_schicht / Math.max(teamAvg * 2, 1)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.touren_pro_schicht.toFixed(1)}/Schicht</span>
          <span>{teamAvg.toFixed(1)}/Schicht</span>
        </div>
      </div>

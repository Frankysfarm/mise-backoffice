...
const COACHING: Record<string, string> = {
  gruen: 'Top Lieferzeit! Du lieferst besonders schnell.',
  gelb: 'Gut — kleine Optimierungen an deiner Route können die Zeit weiter senken.',
  rot: 'Fokus auf effiziente Navigation: Stopp-Reihenfolge optimieren.',
};

export function FahrerPhase3398MeineLieferzeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-durchschnitts-lieferzeit-ranking?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );

...
import { Sunset, WifiOff } from 'lucide-react';
...
export function FahrerPhase4574MeinAbendAnteil({ driverId, locationId, isOnline }: Props) {
  const [data, setData] = useState<AbendData | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-abend-anteil-ranking?${params}`, { cache: 'no-store' });
  ...
  if (!isOnline) return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 flex items-center gap-3">
      ...
      <span className="text-sm text-gray-500 dark:text-gray-400">Offline — Abend-Anteil nicht verfügbar</span>
    </div>
  );
  ...
  const coaching =
    me.abend_pct >= 55 ? { text: 'Top! Du bist ein echter Abendfahrer — großartig!',                    cls: 'text-emerald-600 dark:text-emerald-400' } :
    me.abend_pct >= 30 ? { text: 'Gut! Mehr Abendschichten (18–22 Uhr) steigern deinen Umsatz.',        cls: 'text-yellow-600 dark:text-yellow-400' } :
                          { text: 'Wenig Abendschichten — Stoßzeiten am Abend bringen die meisten Bestellungen.', cls: 'text-red-500 dark:text-red-400' };

  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
      <div className="flex items-center gap-2 mb-3">
        <Sunset className="w-5 h-5 text-rose-500" />
        <h3 className="font-semibold text-gray-900 dark:text-white text-sm">Mein Abend-Anteil</h3>
      </div>
      ...
      <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">
        Anteil Lieferungen zwischen 18:00–22:00 Uhr
      </div>

...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 95) return {
    text: 'Exzellente Akzeptanzrate! Du nimmst fast jeden Auftrag an — das maximiert deinen Verdienst und die Teameffizienz.',
    color: 'text-green-300',
  };
  if (val >= 80) return {
    text: 'Gute Akzeptanzrate. Über 95% steigert deinen Score und dein Ranking deutlich.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Akzeptanzrate zu niedrig. Nimm mehr Aufträge an, um Ranking und Verdienst zu verbessern.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5260MeineAkzeptanzrate({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Akzeptanzrate — offline</span>
      </div>
    );
  }
  ...
  const tipp = coachingTipp(mein.akzeptanz_rate);
  const teamAvg = data.team_avg_akzeptanz;
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Akzeptanzrate</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.akzeptanz_rate.toFixed(0)}<span className="text-xl font-semibold">%</span>
        </div>

...
// Phase 5659 — Mein Frühschicht-Effizienz-Trend (Fahrer) — Batch 109
...
function coaching(delta: number): { text: string; color: string } {
  if (delta > 0) return {
    text: `Deine Frühschicht-Effizienz ist um ${delta.toFixed(2)} Touren/Std gestiegen — stark! Frühes Starten und ruhige Straßen in der Frühschicht zahlen sich aus.`,
    color: 'text-amber-400',
  };
  if (delta === 0) return {
    text: 'Deine Frühschicht-Effizienz ist stabil. Nutze die ruhigen Morgenstraßen zwischen 6–11h für schnellere Routen.',
    color: 'text-yellow-300',
  };
  return {
    text: `Effizienz um ${Math.abs(delta).toFixed(2)} T/Std gesunken — prüfe deine Morgenroutenplanung und reduziere Wartezeiten beim Restaurant in der Frühschicht.`,
    color: 'text-red-400',
  };
}

export function FahrerPhase5659MeinFruehschichtEffizienzTrend({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-lg bg-gray-900 border border-gray-700 p-3 flex items-center gap-2">
        ...
        <span className="text-xs text-gray-500">Kein Netz — Frühschicht-Trend offline</span>
      </div>
    );
  }
    ...
    <div className={`rounded-lg bg-gray-900 border ${BORDER[data.ampel]} p-3 space-y-2.5`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-white">Mein Frühschicht-Trend</span>
      ...
      <div className="flex items-center gap-3">
        <div className="text-center">
          <div className={`text-3xl font-bold font-mono ${data.effizienz_delta > 0 ? 'text-amber-400' : data.effizienz_delta < 0 ? 'text-red-400' : 'text-gray-400'}`}>
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-mono text-amber-400 w-12 text-right">{data.aktuell_touren_pro_std.toFixed(2)} T/h</span>
        </div>
        <div className="flex items-center gap-2">

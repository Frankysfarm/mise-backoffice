...
function coachingTipp(a: string): string {
  if (a === 'rot')  return 'Niedriger Umsatz pro Tour! Achte darauf, vollständige Touren zu fahren und größere Bestellungen zu priorisieren.';
  if (a === 'gelb') return 'Guter Umsatz! Mit mehr Touren in Stoßzeiten kannst du deinen Ø-Wert weiter steigern.';
  return 'Top-Umsatz pro Tour! Du bist im Team führend — behalte diesen Rhythmus bei!';
}

export function FahrerPhase3341MeinUmsatzProTour({
  driverId,
  locationId,
  isOnline,
}: {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Mein Umsatz pro Tour</span>
          {me.alert_low && (
        ...
        <div className="p-4 space-y-4">
          <div className={`rounded-xl p-4 text-center ${cls.bg}`}>
            <div className={`text-5xl font-black ${cls.text}`}>
              {me.umsatz_avg.toFixed(2)}<span className="text-3xl"> €</span>
            </div>
            <div className={`text-3xl font-bold ${cls.text} mt-1`}>Rang #{me.rang}</div>
            <div className="text-xs text-gray-400 mt-1">Ø Umsatz pro Tour</div>
          </div>
          ...
          <div className="text-xs text-gray-400 text-center">Umsatz pro Tour · letzte 30 Tage</div>
        </div>
      )}
    </div>
  );

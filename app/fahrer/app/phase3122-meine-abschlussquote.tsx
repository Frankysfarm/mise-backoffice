...
import { CheckCircle2, ChevronDown, ChevronUp, Minus, TrendingDown, TrendingUp } from 'lucide-react';
...
const MOCK: ApiData = {
  fahrer: [{ fahrer_id: 'me', fahrer_name: 'Ich', rang: 2, quote_pct: 89, rank_delta: 1, ampel: 'gelb', alert_bottom: false }],
  team_avg_pct: 78,
  gesamt: 4,
};
...
function tip(ampel: string): string {
  if (ampel === 'rot')  return 'Deine Abschlussquote liegt im unteren Viertel. Stelle sicher, dass alle zugewiesenen Stopps abgearbeitet werden — kommuniziere Probleme frühzeitig.';
  if (ampel === 'gelb') return 'Du liegst im Mittelfeld. Konsequente Stopp-Abarbeitung bringt dich in die Top 25%.';
  return 'Stark! Du hast heute eine der höchsten Abschlussquoten — weiter so!';
}

export function FahrerPhase3122MeineAbschlussquote({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const url = driverId
      ...
      : `/api/delivery/admin/fahrer-abschlussquoten-ranking?location_id=${locationId}`;
    const load = () =>
      fetch(url)
        .then(r => r.json())
      ...
      {open && (
        <div className="p-4 space-y-3">
          <div className="flex items-center justify-center gap-6">
            ...
            <div className="text-center">
              <div className={`text-4xl font-bold leading-none ${cls.text}`}>{entry.quote_pct}%</div>
              <div className="text-xs text-gray-500 mt-1">Abschlussquote</div>
            </div>
          </div>

...
import { CheckCircle, AlertTriangle, ChevronDown, ChevronUp, TrendingDown, TrendingUp, Minus } from 'lucide-react';
...
const MOCK: ApiData = {
  fahrer: [{ fahrer_id: 'me', fahrer_name: 'Du', rang: 2, quote_pct: 95.0, delivered: 19, total: 20, rank_delta: 1, ampel: 'gruen', alert_bottom: false }],
  team_avg_pct: 88.9,
  gesamt: 4,
};
...
function coaching(a: string): string {
  if (a === 'gruen') return 'Ausgezeichnet! Deine Ablieferungsquote liegt im Top-Viertel. Weiter so!';
  if (a === 'gelb')  return 'Gut! Kommuniziere bei Problemen früh mit dem Disponenten für bessere Ablieferungsquote.';
  return                   'Fokus auf Ablieferung! Bei Nicht-Erreichbarkeit Nachbar-Option oder Rückruf nutzen.';
}

export function FahrerPhase3262MeineAblieferungsquote({
  driverId,
  locationId,
  isOnline,
}: {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Ablieferungsquote</span>
          {me.alert_bottom && (
      ...
      {open && (
        <div className="p-4 space-y-4">
          <div className="text-center">
            <div className={`text-5xl font-black ${col.text}`}>{fmtPct(me.quote_pct)}</div>
            <div className="text-sm text-gray-500 mt-1">Ablieferungsquote ({me.delivered}/{me.total} Stopps)</div>

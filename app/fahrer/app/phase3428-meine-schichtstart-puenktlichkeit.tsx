...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Klasse! Du startest deine Schichten zuverlässig pünktlich.',
  gelb: 'Gut — schau, ob du deinen Schichtstart noch konsequenter einhalten kannst.',
  rot: 'Tipp: Plane den Weg zum Depot großzügiger und stelle dir Erinnerungen für den Schichtbeginn ein.',
};

export function FahrerPhase3428MeineSchichtstartPuenktlichkeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const [data, setData] = useState<FahrerSchichtstart | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-schichtstart-ranking?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
      if (!r.ok) return;
        ...
        <div className="px-4 pb-4 space-y-3">
          <div className="flex items-end justify-center gap-4">
            <div className="text-center">
              ...
              <div className="text-xs text-gray-500 mt-0.5">Pünktlich</div>
            </div>
            <div className="text-center">

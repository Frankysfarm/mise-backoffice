...
import { WifiOff, Sunrise } from 'lucide-react';
...
function coachingTipp(tph: number): { text: string; color: string } {
  if (tph >= 3.0) return {
    text: 'Starke Frühschicht! Der Morgen läuft auf Hochtouren — nutze das ruhige Verkehrsaufkommen für schnelle Stopps.',
    color: 'text-sky-400',
  };
  if (tph >= 1.5) return {
    text: 'Gute Frühleistung. Frühmorgens sind die Wege oft frei — plane kurze Routen und spare Zeit beim Parken.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Potenzial für die Frühschicht. Wenig Verkehr = kürzere Lieferzeiten. Starte früh und optimiere deine ersten Stopps.',
    color: 'text-amber-400',
  };
}

function ampelColor(a: string) {
  if (a === 'gruen') return 'text-sky-400';
  if (a === 'gelb') return 'text-yellow-400';
  return 'text-sky-300';
}

export function FahrerPhase4933MeineFruehprod({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Frühschicht-Produktivität — offline</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-sky-700 bg-sky-950/40 overflow-hidden mb-3">
      {/* Header */}
      <div className="px-4 py-3 flex items-center gap-2 border-b border-sky-700/50 bg-sky-900/20">
        <Sunrise className="w-4 h-4 text-sky-300 shrink-0" />
        <span className="text-sm font-semibold text-sky-200">Meine Frühschicht-Produktivität</span>
      </div>
      ...
      <div className="px-4 pb-3 space-y-2">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 h-2 bg-sky-900/50 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full ${mein.ampel === 'rot' ? 'bg-sky-500' : mein.ampel === 'gelb' ? 'bg-yellow-500' : 'bg-sky-700'}`}
              style={{ width: `${barPct}%` }}
            />
          </div>
      ...
      <div className="px-4 pb-3">
        <div className={`text-xs ${tipp.color} bg-sky-900/30 rounded-lg px-3 py-2 leading-relaxed`}>
          {tipp.text}
        </div>
      </div>

      <div className="px-4 py-2 border-t border-sky-800/30">
        <span className="text-[10px] text-gray-600">Frühschicht 06:00–11:00 UTC · 30-Min-Polling · letzte 30 Tage</span>
      </div>
    </div>
  );
}

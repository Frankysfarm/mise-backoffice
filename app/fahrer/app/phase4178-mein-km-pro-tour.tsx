...
export function FahrerPhase4178MeinKmProTour({ driverId, locationId, isOnline }: Props) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!json.error && json.fahrer?.length) {
          ...
          setData({ km_avg: me.km_avg, rang: me.rang, rank_delta: me.rank_delta, ampel: me.ampel, team_avg: json.team_avg, fahrer_count: json.gesamt });
        }
      }
  ...
  const coaching = data.ampel === 'gruen'
    ? 'Super – deine km pro Tour sind sehr effizient!'
    : data.ampel === 'gelb'
    ? 'Tipp: Überprüfe deine Routen, um Umwege zu vermeiden.'
    : 'Achtung: Deine km pro Tour liegen über dem Teamdurchschnitt.';

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        <Route className="w-4 h-4 text-teal-500" />
        <span className="text-sm font-semibold text-gray-900">Meine Ø km / Tour</span>
        {loading && <span className="w-3 h-3 border-2 border-gray-300 border-t-transparent rounded-full animate-spin ml-auto" />}
      </div>
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-bold ${valColor}`}>{data.km_avg?.toFixed(1)}</span>
        <div className="pb-1 space-y-0.5">
          <span className="text-xl font-medium text-gray-500">km</span>

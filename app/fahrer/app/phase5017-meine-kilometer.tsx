...
function coachingTipp(km: number): { text: string; color: string } {
  if (km <= 8) return {
    text: 'Sehr effiziente Routen! Du nutzt kurze Wege optimal — weiter so.',
    color: 'text-green-300',
  };
  if (km <= 15) return {
    text: 'Gute Streckeneffizienz. Prüfe ob Routenoptimierungen weitere km einsparen können.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Hohe Durchschnittsdistanz: Sprich mit der Disposition zur besseren Zonen-Zuteilung.',
    color: 'text-gray-400',
  };
}
...
export function FahrerPhase5017MeineKilometer({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-kilometer-ranking?${params}`);
  ...
  const tipp = coachingTipp(mein.avg_km);
  const maxBar = 20;
  const ichPct = Math.min(100, (mein.avg_km / maxBar) * 100);
  const avgPct = Math.min(100, (data.team_avg_km / maxBar) * 100);
  ...
  return (
    <div className="rounded-2xl border border-indigo-700 bg-indigo-950/40 overflow-hidden mb-3">
      ...
      <div className="px-4 py-3 border-b border-indigo-700/40 flex items-center gap-2 bg-indigo-900/20">
        ...
        <span className="text-sm font-semibold text-indigo-200">Meine Ø-km je Tour (letzte 30 Tage)</span>
      </div>
      ...
      <div className="px-4 py-4 flex items-center justify-between">
        <div>
          ...
          <div className="text-xs text-gray-500 mt-0.5">Ø Distanz je Tour</div>
        </div>
      ...
      <div className="px-4 pb-3 space-y-1.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-indigo-300 w-16 text-right">{mein.avg_km} km</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-gray-400 w-16 text-right">{data.team_avg_km} km</span>
        </div>
      </div>

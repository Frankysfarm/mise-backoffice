...
function ampelCls(a: string) {
  ...
  return                   { text: 'text-blue-700 dark:text-blue-300',   bar: 'bg-blue-500',  bg: 'bg-blue-50 dark:bg-blue-950'   };
}

function coachingTipp(a: string): string {
  if (a === 'rot')  return 'Hohe km pro Tour! Optimiere deine Routen und nutze kürzere Wege zu Kunden.';
  if (a === 'gelb') return 'Solide Strecke! Mit cleverer Routenplanung kannst du noch effizienter werden.';
  return 'Top-Effizienz! Du fährst die kürzesten Touren im Team — weiter so!';
}

export function FahrerPhase3366MeineKmProTourRanking({
  driverId,
  locationId,
  isOnline,
}: {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine km pro Tour</span>
          {me.alert_top && (
        ...
        <div className="p-4 space-y-4">
          <div className={`rounded-xl p-4 text-center ${cls.bg}`}>
            <div className={`text-5xl font-black ${cls.text}`}>
              {me.km_avg.toFixed(1)}<span className="text-3xl"> km</span>
            </div>
            <div className={`text-3xl font-bold ${cls.text} mt-1`}>Rang #{me.rang}</div>
            <div className="text-xs text-gray-400 mt-1">Ø km pro Tour</div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-center text-xs">
            ...
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="font-bold text-sm text-gray-700 dark:text-gray-200">{d.team_avg.toFixed(1)} km</div>
            </div>
          </div>
          ...
          <div className="text-xs text-gray-400 text-center">km pro Tour · letzte 30 Tage</div>
        </div>
      )}
    </div>
  );

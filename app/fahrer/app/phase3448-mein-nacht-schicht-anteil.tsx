...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Top! Du übernimmst viele Nacht-Schichten — das ist sehr wertvoll für das Team.',
  gelb: 'Gut — versuche gelegentlich Nacht-Schichten zu übernehmen, um deinen Anteil zu steigern.',
  rot: 'Tipp: Sprich mit dem Dispatcher — mehr Nacht-Schichten helfen dir und dem Team in Spitzenzeiten.',
};

export function FahrerPhase3448MeinNachtSchichtAnteil({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const [data, setData] = useState<FahrerNachtSchicht | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-nacht-schicht-anteil?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
      if (!r.ok) return;
        ...
        <div className="px-4 pb-4 space-y-3">
          <div className="flex items-end justify-center gap-4">
            <div className="text-center">
              ...
              <div className="text-xs text-gray-500 mt-0.5">Nacht-Anteil</div>
            </div>
            <div className="text-center">

...
function coachingTipp(avg: number): string {
  if (avg >= 4.0) return 'Hervorragend! Du erzielst ein Top-Trinkgeld — weiter so!';
  if (avg >= 2.5) return 'Gut! Mit freundlichem Service kannst du dein Trinkgeld noch steigern.';
  return 'Tipp: Pünktlichkeit und höflicher Kontakt erhöhen das Trinkgeld deutlich.';
}

export function FahrerPhase4699MeinTrinkgeld({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) {
          setData(me);
          setTeamAvg(json.team_avg_trinkgeld);
        }
      } catch {
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-emerald-200 dark:border-emerald-900 bg-emerald-50 dark:bg-emerald-950 p-3">
        <span className="text-xs text-emerald-700 dark:text-emerald-300">Trinkgeld-Daten nicht verfügbar</span>
      </div>
    );
  }
  ...
  const maxVal = teamAvg !== null ? Math.max(data.avg_trinkgeld, teamAvg) * 1.1 : data.avg_trinkgeld * 1.1 || 1;
  const myBarWidth  = Math.min((data.avg_trinkgeld / maxVal) * 100, 100);
  ...
  return (
    <div className="rounded-2xl border border-emerald-200 dark:border-emerald-900 bg-emerald-50 dark:bg-emerald-950 p-3 space-y-2">
      ...
      <div className="flex items-end gap-3">
        <span className="text-4xl font-black leading-none text-emerald-800 dark:text-emerald-300">
          {data.avg_trinkgeld.toFixed(2)} €
        </span>
        <span className="text-[10px] text-gray-500 dark:text-gray-400 mb-1">Ø pro Tour (30 Tage)</span>
      </div>
        ...
        <div className="space-y-1">
          <div className="space-y-0.5">
            <div className="flex justify-between text-[9px] text-gray-500">
              <span>Ich</span>
              <span>{data.avg_trinkgeld.toFixed(2)} €</span>
            </div>
            <div className="bg-gray-100 dark:bg-gray-800 rounded-full h-1.5">
              <div className="h-1.5 rounded-full bg-emerald-500 dark:bg-emerald-400" style={{ width: `${myBarWidth}%` }} />
            </div>
          </div>
          <div className="space-y-0.5">
            <div className="flex justify-between text-[9px] text-gray-500">
      ...
      <div className="bg-white/60 dark:bg-emerald-900/30 rounded-xl p-2 text-[10px] text-emerald-900 dark:text-emerald-200">
        {coachingTipp(data.avg_trinkgeld)}
      </div>
    </div>
  );
}

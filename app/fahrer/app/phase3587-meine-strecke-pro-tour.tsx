...
interface ApiResponse {
  fahrer: { fahrer_id: string; fahrer_name: string; rang: number; strecke_pro_tour: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; alert_top: boolean }[];
  team_avg: number;
  gesamt: number;
}

const COACHING: Record<string, string> = {
  gruen: 'Sehr effizient! Kurze Strecken — optimale Routenplanung.',
  gelb: 'Gute Strecke. Mit optimierten Routen kannst du noch kürzer werden.',
  rot: 'Hohe Strecke/Tour. Prüfe Routenplanung und Zoneneinteilung.',
};

export function FahrerPhase3587MeineStreckeProTour({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900">Meine Strecke pro Tour</h3>
      </div>

      <div className="text-center space-y-1">
        <div className={`text-5xl font-black ${me.ampel === 'gruen' ? 'text-emerald-600' : me.ampel === 'gelb' ? 'text-yellow-600' : 'text-red-600'}`}>
          {me.strecke_pro_tour} <span className="text-2xl">km</span>
        </div>

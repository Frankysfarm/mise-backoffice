...
export function FahrerPhase4283MeinTourstart({ driverId, locationId, isOnline }: Props) {
  ...
  const ampelColor = me.ampel === 'gruen' ? 'text-rose-600' : me.ampel === 'gelb' ? 'text-yellow-600' : 'text-red-600';
  const ampelBg = me.ampel === 'gruen' ? 'bg-rose-50' : me.ampel === 'gelb' ? 'bg-yellow-50' : 'bg-red-50';
  ...
  const coaching =
    me.ampel === 'gruen'
      ? 'Super! Du startest deine Touren besonders zügig.'
      : me.ampel === 'gelb'
      ? 'Tipp: Bereite alles vor der Zuweisung vor, um schneller starten zu können.'
      : 'Achtung: Deine Tourstartzeit liegt deutlich über dem Teamdurchschnitt.';
    ...
    <div className={`rounded-2xl border p-4 space-y-3 ${ampelBg} border-gray-200`}>
      ...
      <div className="flex items-end gap-3">
        ...
        <div className="pb-1 space-y-0.5">
          <span className="text-sm text-gray-500">min bis Abfahrt</span>

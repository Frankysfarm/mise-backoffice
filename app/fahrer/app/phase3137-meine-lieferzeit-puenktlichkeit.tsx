...
import { CheckSquare, ChevronDown, ChevronUp, Minus, TrendingDown, TrendingUp } from 'lucide-react';
...
const MOCK: ApiData = {
  fahrer: [{ fahrer_id: 'me', fahrer_name: 'Ich', rang: 2, pct: 87, rank_delta: 5, ampel: 'gruen', alert_bottom: false }],
  team_avg_pct: 77,
  gesamt: 4,
};
...
function tip(ampel: string): string {
  if (ampel === 'rot')  return 'Deine Pünktlichkeitsrate liegt im unteren Viertel. Starte früher und orientiere dich an den ETA-Fenstern (±5 Min), um besser zu werden.';
  if (ampel === 'gelb') return 'Du liegst im Mittelfeld. Mit besserer Routenplanung und früherem Start kannst du in die Top 25% kommen.';
  return 'Stark! Du gehörst zu den pünktlichsten Fahrern heute — top Timings!';
}

export function FahrerPhase3137MeineLieferzeitPuenktlichkeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const url = driverId
      ? `/api/delivery/admin/fahrer-lieferzeit-puenktlichkeit-ranking?location_id=${locationId}&driver_id=${driverId}`
      : `/api/delivery/admin/fahrer-lieferzeit-puenktlichkeit-ranking?location_id=${locationId}`;
    const load = () =>
      fetch(url)
        .then(r => r.json())
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Lieferzeit-Pünktlichkeit</span>
        </div>
      ...
      {open && (
        <div className="p-4 space-y-3">
          <div className="flex items-center justify-center gap-6">
            ...
            <div className="text-center">
              <div className={`text-4xl font-bold leading-none ${cls.text}`}>{entry.pct}%</div>
              <div className="text-xs text-gray-500 mt-1">Pünktlichkeitsrate</div>
            </div>
          </div>

          <div>
            <div className="flex justify-between text-xs text-gray-400 mb-1">
              <span>Niedrigste Rate</span>
              <span>Höchste Rate (Rang 1)</span>
            </div>

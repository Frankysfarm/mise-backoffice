...
function coaching(pct: number): string {
  if (pct >= 38) return 'Top! Du bist oft samstagnachts dabei.';
  if (pct >= 20) return 'Gut! Samstagnacht-Anteil im Bereich.';
  return 'Wenig Samstagnacht-Touren — Sa 22–02 Uhr einplanen?';
}

export function FahrerPhase4644MeinSamstagNacht({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-sm">Offline — Samstagnacht nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-fuchsia-200 dark:border-fuchsia-900 bg-white dark:bg-gray-900 p-4 space-y-3">
      <div className="flex items-center gap-2">
        <Moon className="w-5 h-5 text-fuchsia-800 dark:text-fuchsia-300" />
        <h3 className="font-semibold text-gray-900 dark:text-gray-100">Mein Samstagnacht</h3>
      </div>

      <div className="flex items-end gap-4">
        <span className="text-5xl font-extrabold text-fuchsia-800 dark:text-fuchsia-300">{data.pct}%</span>
        <div className="pb-1">

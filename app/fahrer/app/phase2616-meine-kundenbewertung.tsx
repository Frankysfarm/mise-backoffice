...
function coachingTipp(ampel: string): string {
  if (ampel === 'rot')  return 'Deine Kundenbewertung ist unter 3.5 Sterne. Freundlichkeit, Pünktlichkeit und sorgfältige Übergabe verbessern dein Rating!';
  if (ampel === 'gelb') return 'Gutes Rating! Noch ein bisschen mehr Freundlichkeit und Sorgfalt, und du erreichst das Ziel von 4.5 Sternen.';
  return 'Ausgezeichnet! Du übertriffst das Bewertungs-Ziel von 4.5 Sternen. Deine Kunden sind begeistert!';
}
...
function SterneBalken({ avg, barClass }: { avg: number; barClass: string }) {
  const max     = 5;
  const goalPct = (4.5 / max) * 100;
...
const MOCK_ME: FahrerEntry = {
  ...
  bewertung_avg: 4.1, bewertungen_heute: 10,
  trend: 'stabil', trend_delta: 0.1, ampel: 'gelb', rang: 3,
};

export function FahrerPhase2616MeineKundenbewertung({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    if (!isOnline) return;
    if (!locationId) { setMe(MOCK_ME); setTeamAvg(3.92); return; }
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-kundenbewertung?location_id=${locationId}`)
        ...
        .catch(() => { setMe(MOCK_ME); setTeamAvg(3.92); });
    load();
  ...
  return (
    <div className={`rounded-xl border ${st.bg} shadow-sm mb-3`}>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <span className={`text-sm font-bold ${st.text}`}>{me.bewertung_avg.toFixed(1)} ★</span>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="text-center py-2">
            <div className={`text-4xl font-black ${st.text}`}>{me.bewertung_avg.toFixed(1)}</div>
            <div className="flex justify-center gap-1 mt-1">
              {[1, 2, 3, 4, 5].map(s => (
                <Star
                  key={s}
                  size={18}
                  className={s <= Math.round(me.bewertung_avg) ? 'text-amber-400 fill-amber-400' : 'text-gray-300 fill-gray-300'}
                />
              ))}
            </div>
            <div className="text-xs text-gray-500 mt-1">{me.bewertungen_heute} Bewertung{me.bewertungen_heute !== 1 ? 'en' : ''} heute</div>
            <div className="text-xs text-gray-400">Ziel: ≥4.5 ★</div>
          </div>

          {/* Balken */}
          <SterneBalken avg={me.bewertung_avg} barClass={st.bar} />
          <div className="flex justify-between text-xs text-gray-400">
            <span>0 ★</span>
            <span className="text-green-600">Ziel 4.5 ★</span>
            <span>5 ★</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className="text-base font-bold text-gray-700">{teamAvg.toFixed(1)} ★</div>

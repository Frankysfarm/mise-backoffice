...
export function FahrerPhase4348MeineNachtschicht({ driverId, locationId, isOnline }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/delivery/admin/fahrer-nachtschicht-ranking?location_id=${locationId}&driver_id=${driverId}`
      );
      if (res.ok) {
  ...
  const coaching =
    me.ampel === 'rot'
      ? 'Vorsicht: Dein Nachtschichtanteil ist sehr hoch. Achte auf ausreichend Ruhezeiten!'
      : me.ampel === 'gelb'
      ? 'Tipp: Du absolvierst regelmäßig Nachtschichten — denk an Erholung zwischen den Schichten.'
      : 'Super! Dein Nachtschichtanteil ist niedrig — gute Balance zwischen Tag- und Nachtschichten.';
    ...
    <div className={`rounded-2xl border p-4 space-y-3 ${ampelBg} border-gray-200`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">Meine Nachtschichten</span>
        </div>
      ...
      <div className="flex items-end gap-3">
        ...
        <div className="pb-1 space-y-0.5">
          <span className="text-sm text-gray-500">% Nachtanteil</span>

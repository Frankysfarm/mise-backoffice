...
function coaching(pct: number): string {
  if (pct >= 65) return 'Top! Du bist oft sonntagabends dabei.';
  if (pct >= 30) return 'Gut! Sonntagabend-Anteil im Bereich.';
  return 'Wenig Sonntagabend-Touren — So 17–22 Uhr einplanen?';
}

export function FahrerPhase4609MeinSonntagAbend({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-sm">Offline — Sonntagabend nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-purple-200 dark:border-purple-800 bg-white dark:bg-gray-900 p-4 space-y-3">
      <div className="flex items-center gap-2">
        <Star className="w-5 h-5 text-purple-500" />
        <h3 className="font-semibold text-gray-900 dark:text-gray-100">Mein Sonntagabend</h3>
      </div>

      <div className="flex items-end gap-4">
        <span className="text-5xl font-extrabold text-purple-500">{data.pct}%</span>
        <div className="pb-1">

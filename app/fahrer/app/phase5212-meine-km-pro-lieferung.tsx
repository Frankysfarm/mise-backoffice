...
function coachingTipp(val: number): { text: string; color: string } {
  if (val <= 4) return {
    text: 'Sehr kurze Wege! Du arbeitest effizient und sparst Zeit und Kosten — top Routenwahl.',
    color: 'text-green-300',
  };
  if (val <= 6) return {
    text: 'Akzeptable Streckenlänge. Optimierte Routenwahl kann km und Zeit sparen.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Lange Wege je Lieferung. Prüfe Routenoptimierung und nutze kürzere Alternativstrecken.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5212MeineKmProLieferung({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Ø-km/Lieferung — offline</span>
      </div>
    );
  }
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Ø-km/Lieferung</span>
      </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.avg_km_pro_lieferung / Math.max(teamAvg * 2, 1)) * 100))}%` }}
          />
        </div>

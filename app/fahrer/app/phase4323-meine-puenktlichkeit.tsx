...
export function FahrerPhase4323MeinePuenktlichkeit({ driverId, locationId, isOnline }: Props) {
  ...
  const coaching =
    me.ampel === 'gruen'
      ? 'Klasse! Du lieferst zuverlässig pünktlich — Kunden schätzen das sehr.'
      : me.ampel === 'gelb'
      ? 'Tipp: Plane etwas mehr Pufferzeit ein, um Verspätungen zu vermeiden.'
      : 'Achtung: Deine Pünktlichkeitsquote liegt deutlich unter dem Teamdurchschnitt.';
  ...
  return (
    <div className={`rounded-2xl border p-4 space-y-3 ${ampelBg} border-gray-200`}>
      ...
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-extrabold ${ampelColor}`}>{me.rate_pct.toFixed(1)}</span>
        <div className="pb-1 space-y-0.5">
          <span className="text-sm text-gray-500">% pünktlich</span>

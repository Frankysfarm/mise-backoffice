...
function coachingTipp(a: string): string {
  if (a === 'rot')  return 'Hohe Lieferzeit! Prüfe deine Route und vermeide unnötige Stopps — jede Minute zählt für den Kunden.';
  if (a === 'gelb') return 'Gute Leistung! Mit etwas mehr Fokus auf die Route kannst du dich in die Top-25% verbessern.';
  return 'Ausgezeichnet! Du bist einer der schnellsten Fahrer im Team — weiter so!';
}

export function FahrerPhase3322MeineAvgLieferzeit({
  driverId,
  locationId,
  isOnline,
}: {
        ...
        <div className="p-4 space-y-4">
          <div className={`rounded-xl p-4 text-center ${cls.bg}`}>
            <div className={`text-5xl font-black ${cls.text}`}>
              {me.avg_min}<span className="text-2xl font-semibold"> min</span>
            </div>
            <div className={`text-3xl font-bold ${cls.text} mt-1`}>Rang #{me.rang}</div>
            <div className="text-xs text-gray-400 mt-1">Ziel ≤ 20 min</div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-center text-xs">
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className={`font-bold text-sm flex items-center justify-center gap-1 ${me.rank_delta < 0 ? 'text-green-600' : me.rank_delta > 0 ? 'text-red-500' : 'text-gray-500'}`}>
                {me.rank_delta < 0
                  ? <><TrendingDown size={12} /> {me.rank_delta} min</>
                  : me.rank_delta > 0
                  ? <><TrendingUp size={12} /> +{me.rank_delta} min</>
                  : <><Minus size={12} /> 0</>}
              </div>
            </div>
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          ...
          <div className="text-xs text-gray-400 text-center">Ø Lieferzeit departed → delivered · heute</div>
        </div>
      )}
    </div>
  );

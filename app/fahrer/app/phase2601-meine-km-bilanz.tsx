...
function coachingTipp(ampel: string, ziel: number): string {
  if (ampel === 'rot')  return `Deine km-Bilanz liegt unter 50 km. Übernimm mehr Touren, um das Tagesziel von ${ziel} km zu erreichen!`;
  if (ampel === 'gelb') return `Du bist auf Kurs! Noch ein paar Touren und du erreichst das Ziel von ${ziel} km.`;
  return `Stark! Du hast das Tagesziel von ${ziel} km erreicht oder übertroffen. Weiter so!`;
}
...
const MOCK: SingleData = {
  fahrer_single: {
    ...
    km_heute: 67, km_gestern: 70, trend: 'stabil', trend_delta: -3, ampel: 'gelb', alert: false,
  },
  team_avg_heute: 65.2,
  team_avg_gestern: 75.2,
  ziel: 80,
};
    ...
    <div className={`rounded-xl border ${st.bg} shadow-sm mb-3`}>
      ...
      >
        <div className="flex items-center gap-2">
          <Route size={16} className={st.text} />
          <span className="font-semibold text-sm text-gray-800">Meine km-Bilanz</span>
        </div>
        <div className="flex items-center gap-2">
          <span className={`text-sm font-bold ${st.text}`}>{e.km_heute} km</span>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="text-center py-2">
            <div className={`text-4xl font-black ${st.text}`}>{e.km_heute} km</div>
            <div className="text-xs text-gray-500 mt-1">Kilometer heute (Ziel: ≥{data.ziel} km)</div>
          </div>

          {/* Balken */}
          <KmBalken km={e.km_heute} ziel={data.ziel} barClass={st.bar} />
          <div className="flex justify-between text-xs text-gray-400">
            <span>0 km</span>
            <span className="text-green-600">Ziel {data.ziel} km</span>
            <span>150 km</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className="text-base font-bold text-gray-700">
                {e.km_gestern !== null ? `${e.km_gestern} km` : '—'}
              </div>
            ...
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className="flex items-center justify-center gap-1">
                ...
                <span className="text-base font-bold text-gray-700">
                  {e.trend_delta > 0 ? '+' : ''}{e.trend_delta} km
                </span>
              </div>
              <div className="text-xs text-gray-400">Trend</div>
            </div>
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className="text-base font-bold text-green-600">≥{data.ziel} km</div>
              <div className="text-xs text-gray-400">Ziel</div>
            </div>
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className="text-base font-bold text-gray-700">{data.team_avg_heute.toFixed(1)} km</div>

...
export function FahrerPhase4008MeineLeerfahrten({ driverId, locationId, isOnline }: Props) {
  ...
  const coaching =
    me?.ampel === 'gruen'
      ? 'Top – deine Leerfahrtenquote ist sehr niedrig!'
      : me?.ampel === 'gelb'
      ? 'Tipp: Optimiere deine Routen um Leerfahrten zu reduzieren.'
      : 'Achtung: Deine Leerfahrtenquote ist zu hoch.';
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-end justify-center gap-3 py-2">
        <div className="text-center">
          <div className={`text-5xl font-black ${rateColor}`}>{me?.leerfahrten_pct}%</div>
          <div className="flex items-center justify-center gap-1 mt-1">
            {DeltaIcon}
            <span className="text-xs text-gray-400">Leerfahrtenquote (heute)</span>
          </div>
        </div>

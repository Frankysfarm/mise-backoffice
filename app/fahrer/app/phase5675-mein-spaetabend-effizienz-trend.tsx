...
// Phase 5675 — Mein Spätabend-Effizienz-Trend (Fahrer) — Batch 113
...
const MOCK: MyData = {
  rang: 2,
  effizienz_delta: 0.2,
  aktuell_touren_pro_std: 2.5,
  vorher_touren_pro_std: 2.3,
  team_avg_delta: 0.05,
  rank_delta: 0,
  ampel: 'gruen',
...
function DeltaIcon({ d }: { d: number }) {
  if (d > 0) return <TrendingUp   className="h-3.5 w-3.5 text-indigo-400" />;
...
function coaching(delta: number): { text: string; color: string } {
  if (delta > 0) return {
    text: `Deine Spätabend-Effizienz ist um ${delta.toFixed(2)} Touren/Std gestiegen — stark! Die späten Schichten 20–24h meisterst du besser als je zuvor.`,
    color: 'text-indigo-400',
  };
  if (delta === 0) return {
    text: 'Deine Spätabend-Effizienz ist stabil. Nutze ruhigere Nacht-Stunden für optimierte Routen und kürzere Wartezeiten.',
    color: 'text-yellow-300',
  };
  return {
    text: `Effizienz um ${Math.abs(delta).toFixed(2)} T/Std gesunken — prüfe deine Routenplanung für 20–24h und reduziere Leerfahrten in der Spätschicht.`,
    color: 'text-red-400',
  };
}

export function FahrerPhase5675MeinSpaetabendEffizienzTrend({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-lg bg-gray-900 border border-gray-700 p-3 flex items-center gap-2">
        ...
        <span className="text-xs text-gray-500">Kein Netz — Spätabend-Effizienz offline</span>
      </div>
    );
  }
    ...
    <div className={`rounded-lg bg-gray-900 border ${BORDER[data.ampel]} p-3 space-y-2.5`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-white">Mein Spätabend-Effizienz-Trend</span>
      ...
      <div className="flex items-center gap-3">
        <div className="text-center">
          <div className={`text-3xl font-bold font-mono ${data.effizienz_delta > 0 ? 'text-indigo-400' : data.effizienz_delta < 0 ? 'text-red-400' : 'text-gray-400'}`}>
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-mono text-indigo-400 w-14 text-right">{data.aktuell_touren_pro_std.toFixed(2)} T/h</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-mono text-gray-400 w-14 text-right">{data.vorher_touren_pro_std.toFixed(2)} T/h</span>
        </div>
      </div>

...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 80) return {
    text: 'Top Schicht-Bilanz! Du holst das Maximum aus jeder Schicht — weiter so!',
    color: 'text-green-300',
  };
  if (val >= 60) return {
    text: 'Solide Schicht-Bilanz. Mehr Touren pro Schicht oder kürzere Pausen können den Verdienst steigern.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Niedriger Ø-Netto-Verdienst. Spreche mit dem Dispatcher über optimale Schichteinteilung.',
    color: 'text-gray-400',
  };
}
...
export function FahrerPhase5032MeineSchichtBilanz({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-schicht-bilanz-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Schicht-Bilanz — offline</span>
      </div>
    );
  }
  ...
  const tipp = coachingTipp(mein.avg_netto);
  const maxBar = 120;
  const ichPct = Math.min(100, (mein.avg_netto / maxBar) * 100);
  ...
  return (
    <div className="rounded-2xl border border-amber-700 bg-amber-950/40 overflow-hidden mb-3">
      ...
      <div className="px-4 py-3 border-b border-amber-700/40 flex items-center gap-2 bg-amber-900/20">
        ...
        <span className="text-sm font-semibold text-amber-200">Mein Ø-Netto-Verdienst je Schicht (letzte 30 Tage)</span>
      </div>
      ...
      <div className="px-4 py-4 flex items-center justify-between">
        <div>
          <div className={`text-4xl font-black ${ampelColor(mein.ampel)}`}>
            {mein.avg_netto.toFixed(2)} €
          </div>
          <div className="text-xs text-gray-500 mt-0.5">Ø Netto je Schicht</div>
        </div>
      ...
      <div className="px-4 pb-3 space-y-1.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-amber-300 w-16 text-right">{mein.avg_netto.toFixed(2)} €</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-gray-400 w-16 text-right">{data.team_avg.toFixed(2)} €</span>
        </div>
      </div>

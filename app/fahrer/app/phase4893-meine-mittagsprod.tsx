...
function coachingTipp(tph: number): { text: string; color: string } {
  if (tph >= 3.0) return {
    text: 'Sehr hohe Mittagsschicht-Produktivität! Top-Leistung 11–14 Uhr — halte das Tempo und achte auf Pausen.',
    color: 'text-red-400',
  };
  if (tph >= 1.5) return {
    text: 'Gute Mittagsschicht-Effizienz. Mit optimierten Routen kannst du noch mehr Touren pro Stunde schaffen.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Noch Potenzial bei den Mittagstouren. Routenoptimierung oder früherer Touren-Start kann die Produktivität steigern.',
...
export function FahrerPhase4893MeineMittagsprod({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-lime-700 bg-lime-900/60 p-4 mb-3 flex items-center gap-2 text-gray-400">
        <WifiOff className="w-4 h-4" />
        <span className="text-xs">Offline — Mittagsschicht-Produktivität nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-lime-700 bg-lime-900/60 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-lime-100">Meine Mittagsschicht-Produktivität</span>
      </div>
      ...
      <div className="text-center mb-4">
        ...
        <div className="text-xs text-gray-400 mt-1">Mittagsschicht 11–14 Uhr UTC (letzte 30 Tage)</div>
      ...
      <div className="space-y-2 mb-3">
        <div>
          ...
          <div className="h-1.5 bg-gray-800 rounded-full overflow-hidden">
            <div className="h-full bg-lime-400 rounded-full" style={{ width: `${(me.touren_pro_std / maxTph) * 100}%` }} />
          </div>
        </div>
        <div>
          <div className="flex justify-between text-xs mb-1">

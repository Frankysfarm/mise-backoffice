...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Stark! Deine Schicht-Auslastung ist top — du nutzt deine Zeit optimal für Touren.',
  gelb: 'Gut — versuche Leerzeiten zwischen Touren zu minimieren, um deine Auslastung zu steigern.',
  rot: 'Tipp: Kürzere Wartezeiten zwischen Touren erhöhen deine Auslastung und damit deine Einnahmen.',
};

export function FahrerPhase3578MeineSchichtAuslastung({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const [data, setData] = useState<FahrerAuslastung | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-schicht-auslastungs-ranking?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
      if (!r.ok) return;
        ...
        <div className="px-4 pb-4 space-y-3">
          <div className="flex items-end justify-center gap-4">
            <div className="text-center">
              ...
              <div className="text-xs text-gray-500 mt-0.5">Auslastung</div>
            </div>
            <div className="text-center">
          ...
          <div className="grid grid-cols-2 gap-3 text-center">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2">
              ...
              <div className="text-xs font-semibold mt-0.5">{teamAvg}%</div>
            </div>
          </div>

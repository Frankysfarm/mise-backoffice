...
function coachingTipp(ampel: string, min: number): string {
  if (ampel === 'rot')  return `Deine Wartezeit beträgt ${min.toFixed(1)} Min — bitte Bestellungen schneller abholen! Ziel: ≤5 Min.`;
  if (ampel === 'gelb') return `Wartezeit ${min.toFixed(1)} Min. Etwas schneller und du erreichst das Ziel von ≤5 Min!`;
  return `Klasse! Deine Wartezeit von ${min.toFixed(1)} Min ist im grünen Bereich. Weiter so!`;
}
...
const MOCK_ME: FahrerEntry = {
  ...
  avg_min: 6.8, touren_heute: 9, trend: 'fallend', trend_delta: -1.3, rang: 2,
};

export function FahrerPhase2631MeineWartezeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    if (!isOnline) return;
    if (!locationId) { setMe(MOCK_ME); setTeamAvg(7.4); return; }
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-wartezeit-bestellung?location_id=${locationId}`)
        .then(r => r.json())
        .then((res: ApiData) => {
        ...
        .catch(() => { setMe(MOCK_ME); setTeamAvg(7.4); });
    load();
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="text-center py-2">
            ...
            <div className="text-sm text-gray-500 font-medium mt-0.5">Minuten Ø Wartezeit am Depot</div>
            <div className="text-xs text-gray-400 mt-0.5">Ziel: ≤5 Min</div>
          </div>
          ...
          <div className="flex justify-between text-xs text-gray-400">
            <span>0 Min</span>
            <span className="text-green-600">Ziel 5 Min</span>
            <span>20 Min</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className="text-base font-bold text-green-600">≤5 Min</div>

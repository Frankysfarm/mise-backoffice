...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 80) return {
    text: 'Hoher Aktivitätsscore! Du bist sehr aktiv und zuverlässig — das spiegelt sich im Ranking wider.',
    color: 'text-green-300',
  };
  if (val >= 60) return {
    text: 'Guter Aktivitätsscore. Über 80 Punkte maximiert dein Ranking und deinen Bonus.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Aktivitätsscore zu niedrig. Mehr aktive Schichten und Touren verbessern deinen Score.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5276MeinAktivitaetsScore({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-aktivitaets-score?location_id=${locationId}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Mein Aktivitätsscore — offline</span>
      </div>
    );
  }
  ...
  const tipp = coachingTipp(mein.aktivitaets_score);
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Mein Aktivitätsscore</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.aktivitaets_score}<span className="text-xl font-semibold"> / 100</span>
        </div>

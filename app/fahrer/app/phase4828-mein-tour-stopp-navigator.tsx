...
import { Navigation2, MapPin, Clock, CheckCircle2, WifiOff, ExternalLink, Package } from 'lucide-react';
  ...
  stopps: [
    {
      ...
      notiz: null, bestellnummer: '#1040', betrag: 18.50,
    },
    {
      ...
      notiz: 'Bitte klingeln — 3. OG', bestellnummer: '#1042', betrag: 24.80,
    },
    {
      stopp_nr: 3, adresse: 'Bahnhofstraße 8', plz: '52064', stadt: 'Aachen', kunde_name: 'Weber, K.',
      telefon: null, eta_min: 15, km_zur_naechsten: null, status: 'ausstehend',
      notiz: null, bestellnummer: '#1044', betrag: 14.20,
    },
  ],
};
...
export function FahrerPhase4828MeinTourStoppNavigator({
  driverId,
  locationId,
  isOnline,
  ...
  return (
    <div className="rounded-xl border border-slate-700 bg-slate-900/40 p-4 mb-3">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-slate-300">Tour-Stopp Navigator</span>
      ...
      {aktivStopp && (
        <div className={`rounded-lg border-2 p-3 mb-3 ${aktivStopp.status === 'aktiv' ? 'border-blue-500 bg-blue-950/30' : 'border-slate-600 bg-slate-900/20'}`}>
          ...
          <div className="text-xs text-gray-300 mb-2">{aktivStopp.kunde_name} · {aktivStopp.bestellnummer}</div>
          {aktivStopp.notiz && (

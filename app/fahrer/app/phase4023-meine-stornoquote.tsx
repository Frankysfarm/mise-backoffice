...
export function FahrerPhase4023MeineStornoquote({ driverId, locationId, isOnline }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/delivery/fahrer/meine-stornoquote?${params}`);
      if (res.ok) {
  ...
  if (!isOnline) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Stornoquote nicht verfügbar (offline)</span>
      </div>
    );
  }
  ...
  const coaching =
    data.ampel === 'gruen'
      ? 'Top – deine Stornoquote ist sehr niedrig!'
      : data.ampel === 'gelb'
      ? 'Tipp: Kommuniziere frühzeitig bei Problemen.'
      : 'Achtung: Deine Stornoquote ist zu hoch.';

...
interface ApiData { avg_bewertung: number; rang: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; team_avg_bewertung: number; ziel_bewertung: number; fahrer_count: number; }

const MOCK: ApiData = { avg_bewertung: 4.7, rang: 2, rank_delta: 0, ampel: 'gruen', team_avg_bewertung: 4.43, ziel_bewertung: 4.5, fahrer_count: 4 };

interface Props { driverId: string; locationId: string | null; isOnline: boolean; }

export function FahrerPhase4143MeineKundenbewertung({ driverId, locationId, isOnline }: Props) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!json.error && json.fahrer?.length) {
          ...
          setData({ avg_bewertung: me.avg_bewertung, rang: me.rang, rank_delta: me.rank_delta, ampel: me.ampel, team_avg_bewertung: json.team_avg_bewertung, ziel_bewertung: json.ziel_bewertung, fahrer_count: json.gesamt });
        }
      }
  ...
  const coaching = data.ampel === 'gruen'
    ? 'Stark – deine Kundenbewertung ist ausgezeichnet!'
    : data.ampel === 'gelb'
    ? 'Tipp: Ein freundliches Lächeln und pünktliche Lieferung erhöhen deine Bewertung.'
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-bold ${valColor}`}>{data.avg_bewertung?.toFixed(1)}</span>
        <div className="pb-1 space-y-0.5">
          <span className="text-xl font-medium text-amber-400">★ Sterne</span>
      ...
      <div className="flex justify-between text-xs text-gray-400">
        <span>Team-Ø ★ {data.team_avg_bewertung?.toFixed(1)} · Ziel ≥{data.ziel_bewertung?.toFixed(1)}</span>

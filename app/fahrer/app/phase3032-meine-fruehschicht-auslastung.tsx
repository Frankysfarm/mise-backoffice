...
import { ChevronDown, ChevronUp, TrendingUp, TrendingDown, Minus, Sun } from 'lucide-react';
...
const TIPPS: Record<string, string> = {
  gruen: '≥80 %: Ausgezeichnet! Du bist in der Frühschicht voll im Einsatz. Weiter so!',
  gelb:  '60–79 %: Noch Luft nach oben. Versuche, weniger Leerlauf in der Frühschicht zu haben.',
  rot:   '<60 %: Zu wenig aktiv heute! Bitte prüfe deinen Einsatz in der Frühschicht.',
};

const MOCK_ENTRY: FahrerEntry = {
  fahrer_id: 'mock', auslastung_pct: 84.7, auslastung_pct_vorwoche: 79.5, trend: 'steigend', trend_delta: 5.2, ampel: 'gruen',
};
const MOCK_DATA: ApiData = { fahrer: [MOCK_ENTRY], team_avg_pct: 75.9 };
...
export function FahrerPhase3032MeineFruehschichtAuslastung({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const load = () => {
      const url = `/api/delivery/admin/fahrer-fruehschicht-auslastung?location_id=${locationId}${driverId ? `&driver_id=${driverId}` : ''}`;
      fetch(url)
        .then(r => r.json())
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Frühschicht-Auslastung</span>
        </div>
      ...
      {open && (
        <div className="px-4 py-4 space-y-4">
          <div className="text-center">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">Auslastung Frühschicht heute</div>

...
function coachingTipp(val: number): { text: string; color: string } {
  if (val <= 3) return {
    text: 'Exzellente Storno-Rate! Deine Zuverlässigkeit ist ein echter Wettbewerbsvorteil.',
    color: 'text-green-300',
  };
  if (val <= 10) return {
    text: 'Gute Storno-Rate. Überprüfe stornierte Aufträge und spreche mit dem Disponenten über Optimierungen.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Hohe Storno-Rate. Dringend: Ursachen klären und mit dem Disponenten einen Verbesserungsplan erarbeiten.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5134MeineStornoRate({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-storno-rate-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Storno-Rate — offline</span>
      </div>
    );
  }
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Storno-Rate</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.rate_pct} <span className="text-xl font-semibold">%</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.rate_pct / Math.max(teamAvg * 2, 1)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.rate_pct} %</span>
          <span>{teamAvg} %</span>
        </div>
      </div>

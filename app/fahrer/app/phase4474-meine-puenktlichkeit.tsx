...
export function FahrerPhase4474MeinePuenktlichkeit({ driverId, locationId, isOnline }: Props) {
  const [data, setData] = useState<FahrerPuenktlichkeit | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-puenktlichkeit-ranking?${params.toString()}`, { cache: 'no-store' });
    if (res.ok) {
      const json = await res.json();
      const list = Array.isArray(json.ranking) ? json.ranking as FahrerPuenktlichkeit[] : [];

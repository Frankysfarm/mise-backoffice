...
function coachingTipp(ampel: string, ziel: number): string {
  if (ampel === 'rot')  return `Dein Ø Trinkgeld liegt unter 2 €. Freundlichkeit und Pünktlichkeit sind die besten Trinkgeld-Booster – Ziel: ${ziel} €!`;
  if (ampel === 'gelb') return `Guter Ansatz! Dein Trinkgeld steigt – bleib freundlich und pünktlich um das Ziel von ${ziel} € zu erreichen.`;
  return `Super! Du übertriffst das Trinkgeld-Ziel von ${ziel} €. Deine Kunden sind begeistert!`;
}
...
function TrinkgeldBalken({ avg, ziel, barClass }: { avg: number; ziel: number; barClass: string }) {
  const max     = 15;
...
const MOCK: SingleData = {
  fahrer_single: {
    ...
    avg_trinkgeld_heute: 3.50, avg_trinkgeld_gestern: 3.80, lieferungen_heute: 10,
    trend: 'stabil', trend_delta: -0.30, ampel: 'gelb', alert: false,
  },
  team_avg_heute: 3.68,
  team_avg_gestern: 4.76,
  ziel: 5,
};

export function FahrerPhase2611MeinTrinkgeld({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-trinkgeld-analyse?${params}`)
        .then(r => r.json())
    ...
    <div className={`rounded-xl border ${st.bg} shadow-sm mb-3`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800">Mein Trinkgeld</span>
        </div>
        <div className="flex items-center gap-2">
          <span className={`text-sm font-bold ${st.text}`}>{e.avg_trinkgeld_heute.toFixed(2)} €</span>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="text-center py-2">
            <div className={`text-4xl font-black ${st.text}`}>{e.avg_trinkgeld_heute.toFixed(2)} €</div>
            <div className="text-xs text-gray-500 mt-1">Ø Trinkgeld heute · {e.lieferungen_heute} Lieferungen</div>
            <div className="text-xs text-gray-400">Ziel: ≥{data.ziel} €</div>
          </div>

          {/* Balken */}
          <TrinkgeldBalken avg={e.avg_trinkgeld_heute} ziel={data.ziel} barClass={st.bar} />
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className="text-base font-bold text-gray-700">
                {e.avg_trinkgeld_gestern !== null ? `${e.avg_trinkgeld_gestern.toFixed(2)} €` : '—'}
              </div>
            ...
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className="text-base font-bold text-gray-700">{data.team_avg_heute.toFixed(2)} €</div>

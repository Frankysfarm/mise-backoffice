...
import { ChevronDown, ChevronUp, TrendingUp, TrendingDown, Minus, CalendarRange } from 'lucide-react';
...
const TIPPS: Record<string, string> = {
  gruen: '≥70 %: Ausgezeichnete Monatsauslastung! Du nutzt deinen Monat optimal.',
  gelb:  '50–69 %: Noch Luft nach oben. Versuche, in den kommenden Wochen aktiver zu sein.',
  rot:   '<50 %: Geringe Monatsauslastung! Bitte prüfe deine Schichteinteilung.',
};

const MOCK_ENTRY: FahrerEntry = {
  fahrer_id: 'mock', auslastung_pct: 74.5, auslastung_pct_vormonat: 72.0,
  wochen_pct: [78, 72, 76, 72], trend: 'steigend', trend_delta: 2.5, ampel: 'gruen',
};
const MOCK_DATA: ApiData = { fahrer: [MOCK_ENTRY], team_avg_pct: 65.9 };
...
export function FahrerPhase3057MeineMonatsauslastung({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const load = () => {
      const url = `/api/delivery/admin/fahrer-monatsauslastung?location_id=${locationId}${driverId ? `&driver_id=${driverId}` : ''}`;
      fetch(url)
        .then(r => r.json())
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Monatsauslastung</span>
        </div>
      ...
      {open && (
        <div className="px-4 py-4 space-y-4">
          <div className="text-center">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">Monatsauslastung (laufender Monat)</div>
            <div className="flex items-center justify-center gap-1 mt-1">
              <TrendIcon trend={entry.trend} />
              <span className="text-xs text-gray-500 dark:text-gray-400">
                {entry.trend_delta > 0 ? '+' : ''}{entry.trend_delta.toFixed(1)} % vs. Vormonat
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {KW.map((kw, i) => {
              const v = entry.wochen_pct[i] ?? 0;
              const color = v >= ZIEL_PCT ? 'text-green-600' : v >= 50 ? 'text-amber-600' : 'text-red-600';
              return (
                <div key={kw} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2 text-center">
                  <div className="text-[10px] text-gray-500 dark:text-gray-400">{kw}</div>
                  <div className={`text-sm font-bold ${color}`}>{v.toFixed(0)} %</div>
                </div>
              );
            })}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2 text-center">
              <div className="text-xs text-gray-500 dark:text-gray-400">Vormonat</div>
              <div className="text-sm font-bold text-gray-700 dark:text-gray-300">{entry.auslastung_pct_vormonat.toFixed(1)} %</div>
            </div>
          </div>

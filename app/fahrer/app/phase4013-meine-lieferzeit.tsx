...
export function FahrerPhase4013MeineLieferzeit({ driverId, locationId, isOnline }: Props) {
  ...
  const timeColor = me?.ampel === 'gruen' ? 'text-emerald-600' : me?.ampel === 'gelb' ? 'text-yellow-500' : 'text-red-500';
  const coaching =
    me?.ampel === 'gruen'
      ? 'Super – du lieferst besonders schnell!'
      : me?.ampel === 'gelb'
      ? 'Tipp: Optimiere deine Route für kürzere Lieferzeiten.'
      : 'Achtung: Deine Lieferzeit ist zu hoch.';
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-end justify-center gap-3 py-2">
        <div className="text-center">
          <div className={`text-5xl font-black ${timeColor}`}>{me?.avg_min}<span className="text-2xl ml-1">min</span></div>
          <div className="flex items-center justify-center gap-1 mt-1">
            {DeltaIcon}
            <span className="text-xs text-gray-400">Ø Lieferzeit (heute)</span>
          </div>
        </div>

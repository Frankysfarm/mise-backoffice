...
const TIPPS: Record<string, string> = {
  gruen: '≥75 %: Ausgezeichnete Tages-Auslastung! Du nutzt deine Schichten optimal.',
  gelb:  '50–74 %: Noch Luft nach oben. Versuche, in allen drei Schichten aktiver zu sein.',
  rot:   '<50 %: Geringe Gesamtauslastung! Bitte prüfe deine Schichtzeiten.',
};

const MOCK_ENTRY: FahrerEntry = {
  fahrer_id: 'mock', auslastung_pct: 82.1, frueh_pct: 88.0, spaet_pct: 79.0, nacht_pct: 79.3,
  auslastung_pct_vorwoche: 78.2, trend: 'steigend', trend_delta: 3.9, ampel: 'gruen',
};
const MOCK_DATA: ApiData = { fahrer: [MOCK_ENTRY], team_avg_pct: 74.8 };
...
export function FahrerPhase3047MeineGesamtschichtAuslastung({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const load = () => {
      const url = `/api/delivery/admin/fahrer-gesamtschicht-auslastung?location_id=${locationId}${driverId ? `&driver_id=${driverId}` : ''}`;
      fetch(url)
        .then(r => r.json())
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Tages-Gesamtauslastung</span>
        </div>
      ...
      {open && (
        <div className="px-4 py-4 space-y-4">
          <div className="text-center">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">Gesamtauslastung (Früh + Spät + Nacht)</div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Früh 06–14', value: entry.frueh_pct },
              { label: 'Spät 14–22', value: entry.spaet_pct },
              { label: 'Nacht 22–06', value: entry.nacht_pct },
            ].map(s => (
              <div key={s.label} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2 text-center">
                ...
                <div className="text-sm font-bold text-gray-700 dark:text-gray-300">{s.value.toFixed(0)} %</div>
              </div>
            ))}
          </div>

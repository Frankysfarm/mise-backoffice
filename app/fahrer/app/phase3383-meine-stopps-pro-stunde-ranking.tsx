...
const COACHING: Record<string, string> = {
  gruen: 'Hervorragende Effizienz! Weiter so.',
  gelb: 'Gut — versuche Stopps besser zu bündeln.',
  rot: 'Route optimieren: kürzere Wege zwischen Stopps wählen.',
};

export function FahrerPhase3383MeineStoppsProStundeRanking({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-stopps-pro-stunde-ranking?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
        ...
        <div className="px-3 pb-3 space-y-3">
          <div className="flex justify-around items-center py-2">
            <div className="text-center">
              <div className={`text-5xl font-black ${AMPEL_COLOR[me.ampel]}`}>
                {me.stopps_pro_stunde.toFixed(1)}
              </div>
              <div className="text-[10px] text-gray-500 mt-0.5">Stopps/h</div>
            </div>
            <div className="text-center">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded p-2 text-center">
              ...
              <div className="text-sm font-semibold">{data.team_avg.toFixed(1)}/h</div>
            </div>
          </div>

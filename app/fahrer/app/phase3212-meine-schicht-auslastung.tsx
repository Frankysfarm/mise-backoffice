...
const MOCK: ApiData = {
  fahrer: [{ fahrer_id: 'me', fahrer_name: 'Ich', rang: 2, auslastung_pct: 71.0, rank_delta: 1, ampel: 'gruen', alert_bottom: false }],
  team_avg_pct: 55.8,
  gesamt: 4,
};
...
function tip(ampel: string): string {
  if (ampel === 'rot')  return 'Deine Schicht-Auslastung ist niedrig. Prüfe ob aktive Touren vorhanden sind und nutze Wartezeiten effizient.';
  if (ampel === 'gelb') return 'Deine Auslastung liegt im Mittelfeld. Kürzere Pausen zwischen Touren können die Quote verbessern.';
  return 'Sehr gut! Deine Schicht-Auslastung gehört zu den höchsten im Team — bleib dran!';
}

export function FahrerPhase3212MeineSchichtAuslastung({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const url = driverId
      ? `/api/delivery/admin/fahrer-schicht-auslastung?location_id=${locationId}&driver_id=${driverId}`
  ...
  // Rank 1 = highest auslastung = best → bar width inversely proportional to rank
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Schicht-Auslastung</span>
        </div>
          ...
          {isOnline && me && (
            <>
              <div className="flex items-center justify-center gap-6">
                ...
                <div className="text-center">
                  <div className={`text-4xl font-black ${cls.text}`}>{me.auslastung_pct}%</div>
                  <div className="text-xs text-gray-500 mt-0.5">Schicht-Auslastung</div>
                </div>
              </div>

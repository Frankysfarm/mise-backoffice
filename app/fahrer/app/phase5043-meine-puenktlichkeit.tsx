...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 90) return {
    text: 'Exzellente Pünktlichkeit! Du lieferst fast immer pünktlich — weiter so!',
    color: 'text-green-300',
  };
  if (val >= 75) return {
    text: 'Gute Pünktlichkeit. Optimiere Routen und vermeide Wartezeiten an der Küche.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Niedrige Pünktlichkeit. Bitte spreche mit dem Dispatcher über Verbesserungsmöglichkeiten.',
    color: 'text-gray-400',
  };
}
...
export function FahrerPhase5043MeinePuenktlichkeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  return (
    <div className="rounded-2xl border border-teal-700 bg-teal-950/40 overflow-hidden mb-3">
      ...
      <div className="px-4 py-3 border-b border-teal-700/40 flex items-center gap-2 bg-teal-900/20">
        ...
        <span className="text-sm font-semibold text-teal-200">Meine Pünktlichkeit (letzte 30 Tage)</span>
      </div>
      ...
      <div className="px-4 pb-3 space-y-1.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-teal-300 w-12 text-right">{mein.puenktlichkeit_pct} %</span>
        </div>
        <div className="flex items-center gap-2">

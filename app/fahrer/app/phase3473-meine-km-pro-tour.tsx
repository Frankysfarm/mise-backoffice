...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Sehr effizient! Du fährst kurze, optimale Routen. Das spart Zeit und Sprit — super gemacht!',
  gelb: 'Gut — versuche die Navigation zu optimieren und unnötige Umwege zu vermeiden.',
  rot: 'Tipp: Stimme dich mit dem Dispatcher ab — möglicherweise lassen sich Touren besser bündeln, um die Strecke zu verkürzen.',
};

export function FahrerPhase3473MeineKmProTour({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const [data, setData] = useState<FahrerKmProTour | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-km-pro-tour?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
      if (!r.ok) return;
  ...
  // ascending ranking (lowest km = best = rank 1), so invert bar position
        ...
        <div className="px-4 pb-4 space-y-3">
          <div className="flex items-end justify-center gap-4">
            <div className="text-center">
              ...
              <div className="text-xs text-gray-500 mt-0.5">km/Tour</div>
            </div>
            <div className="text-center">
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-[10px] text-gray-400">
              <span>Höchste km (ungünstig)</span>
              <span>Niedrigste (Bester)</span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-center">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2">
              ...
              <div className="text-xs font-semibold mt-0.5">{teamAvg} km</div>
            </div>
          </div>

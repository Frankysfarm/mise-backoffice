...
function coachingTipp(ampel: string): string {
  if (ampel === 'gruen') return 'Wochenend-Schicht im grünen Bereich (≤8h) — gute Work-Life-Balance!';
  if (ampel === 'gelb') return '8–12h Wochenend-Schicht. Achte auf ausreichend Erholung nach dem Wochenende.';
  return 'Mehr als 12h Wochenend-Schicht — Überlastungsrisiko! Bitte Teamleitung informieren und Pausen einplanen.';
}
...
export function FahrerPhase2443MeineWochenendSchicht({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-wochenend-schicht?location_id=${locationId}&driver_id=${driverId}`)
        .then(r => r.json())
        .then(setData)
        .catch(console.error);
    load();
  ...
  const maxH = 16;
  const barPct = Math.min(100, (f.wochenend_h / maxH) * 100);
  const ziel8Pct = (8 / maxH) * 100;
  const ziel12Pct = (12 / maxH) * 100;
      ...
      {open && (
        <div className="bg-white px-4 pb-3 pt-2 space-y-3">
          ...
          <div className="text-center">
            <span className={`text-3xl font-bold ${style.text}`}>{fmtH(f.wochenend_h)}</span>
            <p className="text-xs text-gray-400 mt-0.5">Wochenend-Schicht (Sa + So)</p>
          </div>
          ...
          <div className="relative h-3 rounded-full bg-gray-200">
            ...
            <div
              ...
              style={{ left: `${ziel12Pct}%` }}
              title="+12h Alert"
            />
          </div>
          <div className="flex justify-between text-xs text-gray-400">
            <span>0h</span>
            <span className="text-amber-500">8h</span>
          ...
          <div className="grid grid-cols-4 gap-1.5 text-center">
            {[
              { label: 'Vorwoche', val: fmtH(f.wochenend_h_vw) },
              { label: 'Trend', val: <TrendIcon trend={f.trend} /> },
              { label: 'Ziel', val: '≤8h', color: 'text-green-600' },
              { label: 'Team-Ø', val: fmtH(data.team_avg_wochenend_h) },
            ].map((k, i) => (

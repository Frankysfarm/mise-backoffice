...
export function FahrerPhase4308MeineLeerfahrten({ driverId, locationId, isOnline }: Props) {
  ...
  const coaching =
    me.ampel === 'gruen'
      ? 'Sehr gut! Du optimierst deine Routen und fährst kaum Leerkilometer.'
      : me.ampel === 'gelb'
      ? 'Tipp: Aufträge bündeln und Rückfahrten für Folgeaufträge nutzen.'
      : 'Achtung: Deine Leerfahrtenquote liegt deutlich über dem Teamdurchschnitt.';
  ...
  return (
    <div className={`rounded-2xl border p-4 space-y-3 ${ampelBg} border-gray-200`}>
      ...
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-extrabold ${ampelColor}`}>{me.leerfahrten_pct}</span>
        <div className="pb-1 space-y-0.5">
          <span className="text-sm text-gray-500">% Leerfahrten</span>
      ...
      <div className="flex items-center justify-between text-xs text-gray-500">
        <span>Team-Ø: <span className="font-semibold text-gray-700">{data.team_avg_leerfahrten_pct.toFixed(1)}%</span></span>

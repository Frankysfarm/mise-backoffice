...
function coachingTipp(pct: number): { text: string; color: string } {
  if (pct >= 50) return { text: 'Hoher Mittagsschicht-Anteil. Achte auf ausreichend Erholung am Mittag!', color: 'text-red-400' };
  if (pct >= 25) return { text: 'Gute Balance. Dein Mittagsschicht-Anteil ist im Normalbereich.', color: 'text-yellow-400' };
  return { text: 'Niedriger Mittagsschicht-Anteil – gute Verteilung über den Tag!', color: 'text-green-400' };
}
...
export function FahrerPhase4798MeinMittagsschichtAnteil({ driverId, locationId, isOnline }: { driverId: string; locationId: string | null; isOnline: boolean }) {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-lime-800 bg-lime-950/40 p-4 mb-3 flex items-center gap-2 text-gray-400">
        <WifiOff className="w-4 h-4" />
        <span className="text-xs">Offline — Mittagsschicht-Anteil nicht verfügbar</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-lime-800 bg-lime-950/40 p-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        <Sun className="w-4 h-4 text-lime-400" />
        <span className="text-sm font-semibold text-lime-300">Mein Mittagsschicht-Anteil</span>
      </div>

      <div className="text-center mb-4">
        <div className={`text-4xl font-bold ${ampelColor(me.ampel)}`}>{me.mittagsschicht_anteil_pct.toFixed(1)}%</div>
        <div className="text-xs text-gray-400 mt-1">Touren 11:30–17:30 Uhr (30 Tage)</div>
      ...
      <div className="space-y-2 mb-3">
        <div>
          ...
          <div className="h-1.5 bg-gray-800 rounded-full overflow-hidden">
            <div className="h-full bg-lime-500 rounded-full" style={{ width: `${(me.mittagsschicht_anteil_pct / maxPct) * 100}%` }} />
          </div>
        </div>
        <div>
          <div className="flex justify-between text-xs mb-1">

...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 50) return {
    text: 'Hoher Abendanteil! Du arbeitest viel in der umsatzstarken Stoßzeit 18–22 Uhr — das maximiert deinen Verdienst.',
    color: 'text-green-300',
  };
  if (val >= 30) return {
    text: 'Guter Abendanteil. Mehr Abendschichten zwischen 18 und 22 Uhr steigern Ranking und Einkommen.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Geringer Abendanteil. Abendschichten (18–22 Uhr) sind besonders umsatzstark — nimm mehr wahr.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5280MeinAbendAnteil({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Mein Abendanteil — offline</span>
      </div>
    );
  }
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Mein Abendanteil (18–22 Uhr)</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.abend_pct.toFixed(0)}<span className="text-xl font-semibold">%</span>
        </div>

...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 35) return {
    text: 'Starker Express-Anteil! Du bewältigst viele Eilaufträge — das Team schätzt dich!',
    color: 'text-cyan-300',
  };
  if (val >= 15) return {
    text: 'Solider Express-Anteil. Zeige Bereitschaft für Eilaufträge um deinen Score zu steigern.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Geringer Express-Anteil. Melde dich beim Dispatcher für priorisierte Eilaufträge.',
    color: 'text-gray-400',
  };
}
...
export function FahrerPhase5053MeinExpressAnteil({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-express-anteil-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Mein Express-Anteil — offline</span>
      </div>
    );
  }
  ...
  const tipp = coachingTipp(mein.express_anteil_pct);
  const ichPct = Math.min(100, mein.express_anteil_pct * 2);
  const avgPct = Math.min(100, data.team_avg_pct * 2);
  ...
  return (
    <div className="rounded-2xl border border-cyan-700 bg-cyan-950/40 overflow-hidden mb-3">
      ...
      <div className="px-4 py-3 border-b border-cyan-700/40 flex items-center gap-2 bg-cyan-900/20">
        ...
        <span className="text-sm font-semibold text-cyan-200">Mein Express-Anteil (letzte 30 Tage)</span>
      </div>
      ...
      <div className="px-4 py-4 flex items-center justify-between">
        <div>
          <div className={`text-4xl font-black ${ampelColor(mein.ampel)}`}>
            {mein.express_anteil_pct} %
          </div>
          <div className="text-xs text-gray-500 mt-0.5">Express-Aufträge</div>
        </div>
        <div className="text-right">
          <div className={`text-2xl font-bold ${mein.rang === 1 ? 'text-cyan-300' : 'text-gray-300'}`}>
            #{mein.rang}
          </div>
      ...
      <div className="px-4 pb-3 space-y-1.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-cyan-300 w-12 text-right">{mein.express_anteil_pct} %</span>
        </div>
        <div className="flex items-center gap-2">

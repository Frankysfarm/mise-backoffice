...
export function FahrerPhase4053MeineTrinkgeldQuote({ driverId, locationId, isOnline }: Props) {
  ...
  if (!isOnline) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Trinkgeld-Ranking nicht verfügbar (offline)</span>
      </div>
    );
  }

  const valueColor = data.ampel === 'gruen' ? 'text-emerald-600' : data.ampel === 'gelb' ? 'text-yellow-500' : 'text-red-500';
  const coaching = data.ampel === 'gruen' ? 'Top – deine Trinkgeld-Quote ist hervorragend!' : data.ampel === 'gelb' ? 'Tipp: Freundlicher Service und pünktliche Lieferung erhöhen dein Trinkgeld.' : 'Achtung: Deine Trinkgeld-Quote ist unterdurchschnittlich.';
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-bold tabular-nums ${valueColor}`}>{data.trinkgeld_quote?.toFixed(1)} %</span>
      ...
      <div className="flex justify-between text-[11px] text-gray-400 px-1">
        ...
        <span>Rang 1 = höchste Trinkgeld-Quote</span>
      </div>
    </div>
  );
}

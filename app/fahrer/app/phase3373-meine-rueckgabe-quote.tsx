...
import { Undo2, AlertTriangle, ChevronDown, ChevronUp, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
function coachingTipp(a: string): string {
  if (a === 'rot')  return 'Hohe Rückgabequote! Überprüfe Bestelldetails vor der Übergabe und kommuniziere bei Problemen frühzeitig.';
  if (a === 'gelb') return 'Solide Quote! Mit gezielter Aufmerksamkeit bei der Übergabe kannst du Rückgaben weiter reduzieren.';
  return 'Top-Quote! Du hast die wenigsten Rückgaben im Team — exzellente Arbeit!';
}

export function FahrerPhase3373MeineRueckgabeQuote({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const res = await fetch(`/api/delivery/admin/fahrer-rueckgabe-quote-ranking?location_id=${locationId}`);
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          <Undo2 size={16} className="text-red-600" />
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Rückgabe-Quote</span>
          {me.alert_top && (
        ...
        <div className="p-4 space-y-4">
          <div className={`rounded-xl p-4 text-center ${cls.bg}`}>
            <div className={`text-5xl font-black ${cls.text}`}>
              {me.quote_pct.toFixed(1)}<span className="text-3xl"> %</span>
            </div>
            <div className={`text-3xl font-bold ${cls.text} mt-1`}>Rang #{me.rang}</div>
            <div className="text-xs text-gray-400 mt-1">Rückgabe-Quote</div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-center text-xs">
            ...
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="font-bold text-sm text-gray-700 dark:text-gray-200">{d.team_avg.toFixed(1)} %</div>
            </div>
          </div>
          ...
          <div className="text-xs text-gray-400 text-center">Rückgabe-Quote · letzte 30 Tage</div>
        </div>
      )}
    </div>
  );

...
function coachingTipp(ampel: string): string {
  if (ampel === 'gruen') return 'Exzellenter Erstkontakt! Du erreichst Kunden blitzschnell — Top-Performance!';
  if (ampel === 'gelb')  return 'Guter Erstkontakt. Optimiere die ersten Meter nach Tour-Start für noch schnellere Verbindung.';
  return 'Erstkontakt-Zeit verbessern: Direkt nach Abfahrt die Route fokussieren und Kunden zügig ansteuern.';
}

export function FahrerPhase3232MeineErstkontaktRanking({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/delivery/admin/fahrer-erstkontakt-ranking?${params}`);
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-5 shadow-sm space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Erstkontakt-Zeit</span>
      </div>
      ...
      <div className="flex items-center justify-center gap-6">
        ...
        <div className="text-center">
          <div className={`text-5xl font-black ${ampelColor}`}>{fmtSek(me.avg_sek)}</div>
          <div className="text-xs text-gray-400 mt-0.5">Ø Erstkontakt</div>
        </div>
      </div>

      {/* Rang-Balken (Rang 1 = bester = kürzeste Zeit = voll) */}
      <div>
        <div className="text-xs text-gray-500 mb-1">Rang-Position (Rang 1 = schnellster)</div>
        ...
        <div className="flex justify-between text-xs text-gray-400 mt-0.5"><span>#1 Schnellster</span><span>#{data?.gesamt} Letzter</span></div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-center text-xs">
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          ...
          <div className="font-semibold flex items-center justify-center gap-1 mt-0.5">
            {/* negativ=verbessert */}
            {me.rank_delta < 0 ? <><TrendingDown size={12} className="text-green-500" /><span className="text-green-600">{me.rank_delta}</span></> :
             me.rank_delta > 0 ? <><TrendingUp size={12} className="text-red-500" /><span className="text-red-500">+{me.rank_delta}</span></> :
             <><Minus size={12} className="text-gray-400" /><span className="text-gray-400">±0</span></>}
          </div>
        </div>
        <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
          <div className="text-gray-400">Team-Ø</div>
          <div className="font-semibold text-blue-600 dark:text-blue-400 mt-0.5">{fmtSek(data?.team_avg_sek ?? 0)}</div>
        </div>
      </div>

...
function coaching(pct: number): string {
  if (pct >= ZIEL_PCT) return `${pct.toFixed(1)}% — Perfekt! Du hast alle Touren erfolgreich abgeschlossen.`;
  if (pct >= WARN_PCT) return `${pct.toFixed(1)}% — Im gelben Bereich. Achte auf vollständige Tour-Abschlüsse (Ziel: ${ZIEL_PCT}%).`;
  return `${pct.toFixed(1)}% — Niedrige Abschlussrate! Bitte melde abgebrochene Touren zeitnah und vermeide unnötige Stornos.`;
}
...
const MOCK_ME: FahrerEntry = {
  fahrer_id: 'mock-me', fahrer_name: 'Ich', rate_pct: 88, abgeschlossen: 7, gesamt: 8,
  ampel: 'gelb', trend: 'fallend', trend_delta: -3, rang: 2,
};
const MOCK: ApiData = { fahrer: [MOCK_ME], team_avg_pct: 92.0, alert_count: 0 };

export function FahrerPhase2842MeineAbschlussrate({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-tour-abschlussrate?${params}`)
        .then(r => r.json())
      ...
      {open && (
        <div className="mt-3 space-y-3">
          <div className="text-center">
            ...
            <div className="text-xs text-gray-500 mt-1">{me.abgeschlossen} / {me.gesamt} Touren abgeschlossen</div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              ...
              <div className="text-xs font-bold text-gray-800">{data.team_avg_pct.toFixed(1)}%</div>
            </div>
            <div className="bg-white rounded-lg border border-gray-100 p-2 text-center">
              <div className="text-[10px] text-gray-500">Rang (Zuverlässigkeit)</div>

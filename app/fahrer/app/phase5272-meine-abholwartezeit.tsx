...
function coachingTipp(val: number): { text: string; color: string } {
  if (val <= 4) return {
    text: 'Kurze Abholwartezeit! Du bist pünktlich beim Restaurant und minimierst Wartezeiten — das hält die Touren flüssig.',
    color: 'text-green-300',
  };
  if (val <= 8) return {
    text: 'Akzeptable Wartezeit. Unter 4 Min steigert dein Ranking und die Kundenzufriedenheit.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Zu lange Wartezeit beim Abholen. Pünktlichere Abholung verbessert Touren-Durchsatz und Kundenbewertung.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5272MeineAbholwartezeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-abholwartezeit?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Abholwartezeit — offline</span>
      </div>
    );
  }
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Abholwartezeit</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.avg_min.toFixed(1)}<span className="text-xl font-semibold"> min</span>
        </div>
        ...
        <div className="text-xs text-gray-500 mt-1">
          {mein.touren_heute} Touren heute · {mein.touren_ueber8min} über 8 Min
        </div>
      </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.avg_min / Math.max(teamAvg * 2, 1)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.avg_min.toFixed(1)} min</span>

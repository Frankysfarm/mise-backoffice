...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 4) return {
    text: 'Sehr hohe Produktivität! Du lieferst schnell und effizient — weiter so.',
    color: 'text-green-300',
  };
  if (val >= 2.5) return {
    text: 'Solide Produktivität. Optimierte Routen und kürzere Pausen können die Rate steigern.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Niedrige Produktivität. Überprüfe Routen, reduziere Wartezeiten und optimiere Stopps.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5220MeineLieferungenProStunde({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-lieferungen-pro-stunde-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Ø-Lieferungen/h — offline</span>
      </div>
    );
  }
  ...
  const teamAvg = data.team_avg_pro_h;
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Ø-Lieferungen/h (Produktivität)</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.deliveries_pro_h.toFixed(1)} <span className="text-xl font-semibold">/h</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.deliveries_pro_h / Math.max(teamAvg * 2, 1)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.deliveries_pro_h.toFixed(1)}/h</span>

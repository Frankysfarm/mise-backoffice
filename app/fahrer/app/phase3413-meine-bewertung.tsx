...
const COACHING: Record<string, string> = {
  gruen: 'Ausgezeichnet! Deine Kundenbewertungen sind top — weiter so.',
  gelb: 'Gut — kleine Verbesserungen im Service können deine Bewertung auf 4.5+ heben.',
  rot: 'Fokus: Frage Kunden aktiv nach Feedback und arbeite an Pünktlichkeit & Freundlichkeit.',
};

export function FahrerPhase3413MeineBewertung({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-bewertungs-durchschnitt?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
        ...
        <div className="px-3 pb-3 space-y-3">
          ...
          <div className="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-2.5">
            <div
              ...
              style={{ width: `${Math.max(5, (me.bewertung_avg / 5) * 100)}%` }}
            />
          </div>

...
const COACHING: Record<string, string> = {
  gruen: 'Top-Umsatz pro Tour! Du bringst konstant hohe Bestellwerte — weiter so!',
  gelb: 'Solider Umsatz. Versuche höherwertige Bestellungen zu priorisieren und Bonusartikel anzubieten.',
  rot: 'Niedriger Umsatz/Tour! Spreche mit Dispatch über Bestellpriorisierung für umsatzstärkere Routen.',
};

export function FahrerPhase3655MeinUmsatzProTour({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const valColor = me.ampel === 'gruen' ? 'text-emerald-500' : me.ampel === 'gelb' ? 'text-orange-500' : 'text-red-500';
  const barColor = me.ampel === 'gruen' ? 'bg-emerald-400' : me.ampel === 'gelb' ? 'bg-orange-400' : 'bg-red-500';

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        <TrendingUp className="w-5 h-5 text-emerald-500" />
        <h3 className="font-semibold text-gray-900">Mein Umsatz/Tour</h3>
      </div>

      <div className="text-center space-y-1">
        <div className={`text-5xl font-black ${valColor}`}>
          {me.umsatz_avg.toFixed(0)}€
        </div>
        <div className="text-sm text-gray-500">Ø Umsatz pro Tour</div>
        ...
        <div className="flex items-center justify-center gap-1 text-sm text-gray-500">
          ...
          <span>Team-Ø: {data.team_avg.toFixed(0)}€</span>
        </div>
      </div>

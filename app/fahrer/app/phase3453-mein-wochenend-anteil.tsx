...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Top! Du übernimmst viele Wochenend-Schichten — das ist sehr wertvoll für das Team.',
  gelb: 'Gut — versuche gelegentlich Wochenend-Schichten zu übernehmen, um deinen Anteil zu steigern.',
  rot: 'Tipp: Sprich mit dem Dispatcher — mehr Wochenend-Schichten helfen dir und dem Team in Spitzenzeiten.',
};

export function FahrerPhase3453MeinWochenenAnteil({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const [data, setData] = useState<FahrerWochenend | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-wochenend-anteil?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
      if (!r.ok) return;
          ...
          <div className="flex items-end justify-center gap-4">
            <div className="text-center">
              <div className={`text-5xl font-bold font-mono ${AMPEL_COLOR[data.ampel]}`}>
                {data.wochenend_anteil}%
              </div>
              <div className="text-xs text-gray-500 mt-0.5">Wochenend-Anteil</div>
            </div>
            <div className="text-center">

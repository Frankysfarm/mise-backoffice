...
import { ChevronDown, ChevronUp, LogOut, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Sehr gut! Du beendest deine Touren pünktlich — das schützt vor Überstunden und zeigt gute Planung.',
  gelb: 'Gut — versuche deine Touren häufiger innerhalb des Zeitfensters abzuschließen, um Überstunden zu vermeiden.',
  rot: 'Tipp: Sprich mit dem Dispatcher über realistischere Tour-Planung, um Überstunden-Risiken zu reduzieren.',
};

export function FahrerPhase3458MeineFeierabendPuenktlichkeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const [data, setData] = useState<FahrerFeierabend | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-feierabend-puenktlichkeit?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
      if (!r.ok) return;
    ...
    <div className="border rounded-xl bg-white dark:bg-gray-900 shadow-sm mb-3">
      ...
      >
        <span className="flex items-center gap-2 font-semibold text-sm">
          ...
          Meine Feierabend-Pünktlichkeit
        </span>
        ...
        <div className="px-4 pb-4 space-y-3">
          <div className="flex items-end justify-center gap-4">
            <div className="text-center">
              ...
              <div className="text-xs text-gray-500 mt-0.5">Pünktlichkeit</div>
            </div>
            <div className="text-center">

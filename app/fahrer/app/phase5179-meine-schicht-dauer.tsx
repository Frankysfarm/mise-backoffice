...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 6) return {
    text: 'Tolle Einsatzbereitschaft! Deine langen Schichten machen den Unterschied.',
    color: 'text-green-300',
  };
  if (val >= 3) return {
    text: 'Gute Schichtdauer. Mehr Stunden steigern deinen Einfluss auf das Team.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Kurze Schichten. Längere Einsätze verbessern dein Ranking und dein Einkommen.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5179MeineSchichtDauer({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-schichtstunden-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Schichtdauer — offline</span>
      </div>
    );
  }
  ...
  const tipp    = coachingTipp(mein.avg_stunden);
  const teamAvg = data.team_avg_stunden;
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Ø-Schichtdauer</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.avg_stunden.toFixed(1)} <span className="text-xl font-semibold">h</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.avg_stunden / Math.max(teamAvg * 1.5, 1)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.avg_stunden.toFixed(1)}h</span>

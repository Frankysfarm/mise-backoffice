...
import { Trophy, TrendingUp, TrendingDown, Minus, WifiOff, AlertTriangle } from 'lucide-react';

// Phase 5623 — Mein Gesamtleistungs-Score-Trend (Fahrer) — Batch 100 Milestone
// Trophy violet; score_delta 4xl+Rang ABSTEIGEND; isOnline-Guard+WifiOff-Fallback;
...
function coaching(delta: number): { text: string; color: string } {
  if (delta > 0) return {
    text: `Dein Gesamt-Score ist um ${delta.toFixed(1)} Punkte gestiegen — ausgezeichnet! Deine Kombination aus Bewertungen, Pünktlichkeit und Reaktionszeit verbessert sich spürbar.`,
    color: 'text-violet-400',
  };
  if (delta === 0) return {
    text: 'Dein Gesamt-Score ist stabil. Fokussiere dich auf schnellere Annahmezeiten und freundlichen Kundenkontakt, um deinen Rang zu verbessern.',
    color: 'text-yellow-300',
  };
  return {
    text: `Score um ${Math.abs(delta).toFixed(1)} Punkte gesunken — achte besonders auf Pünktlichkeit und Kundenbewertungen, um deinen Trend umzukehren.`,
    color: 'text-red-400',
  };
}

export function FahrerPhase5623MeinGesamtleistungsTrend({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const load = async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-gesamtleistungs-score-trend-ranking?driver_id=${driverId}${locationId ? `&location_id=${locationId}` : ''}`,
      );
      if (r.ok) {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-lg bg-gray-900 border border-gray-700/50 p-3 flex items-center gap-2">
        ...
        <span className="text-xs text-gray-500">Offline – Gesamtleistungs-Trend nicht verfügbar</span>
      </div>
    );
  }

  const { text: coachText, color: coachColor } = coaching(data.score_delta);
  ...
  return (
    <div className={`rounded-lg bg-gray-900 border ${BORDER[data.ampel]} p-3 space-y-2`}>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-xs font-semibold text-white">Mein Gesamtleistungs-Trend</span>
        {data.alert_rueckfall && <AlertTriangle className="h-3 w-3 text-red-400 ml-auto" />}
      </div>

      {/* Hauptwert */}
      <div className="flex items-end gap-2">
        <span className={`text-4xl font-black leading-none ${data.score_delta >= 0 ? 'text-violet-400' : 'text-red-400'}`}>
      ...
      <div className="space-y-1">
        <div>
          <div className="flex justify-between text-[10px] text-gray-500 mb-0.5">
            <span>Aktuell</span>
            <span>{data.aktuell_score.toFixed(1)} Pkt.</span>
          </div>
          <div className="h-1.5 rounded-full bg-gray-800">
            <div className="h-1.5 rounded-full bg-violet-400 transition-all duration-500" style={{ width: `${curPct}%` }} />
          </div>
        </div>
        <div>
          <div className="flex justify-between text-[10px] text-gray-500 mb-0.5">
            <span>Vormonat</span>
            <span>{data.vorher_score.toFixed(1)} Pkt.</span>
          </div>
          <div className="h-1.5 rounded-full bg-gray-800">

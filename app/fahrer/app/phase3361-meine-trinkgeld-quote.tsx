...
import { BadgeDollarSign, AlertTriangle, ChevronDown, ChevronUp, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
function ampelCls(a: string) {
  ...
  return                   { text: 'text-emerald-700 dark:text-emerald-300',   bar: 'bg-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-950'  };
}

function coachingTipp(a: string): string {
  if (a === 'rot')  return 'Deine Trinkgeld-Quote ist niedrig. Pünktliche Lieferung, freundliche Übergabe und ein Lächeln machen den Unterschied!';
  if (a === 'gelb') return 'Gute Quote! Ein herzliches "Guten Appetit!" bei der Übergabe kann deine Trinkgelder noch weiter steigern.';
  return 'Top-Trinkgeld-Quote! Deine Kunden schätzen deinen Service — weiter so!';
}

export function FahrerPhase3361MeineTrinkgeldQuote({
  driverId,
  locationId,
  isOnline,
}: {
        ...
        <div className="p-4 space-y-4">
          <div className={`rounded-xl p-4 text-center ${cls.bg}`}>
            <div className={`text-5xl font-black ${cls.text}`}>
              {me.trinkgeld_quote.toFixed(2)}<span className="text-2xl"> €/Stopp</span>
            </div>
            <div className={`text-3xl font-bold ${cls.text} mt-1`}>Rang #{me.rang}</div>
            <div className="text-xs text-gray-400 mt-1">Durchschnittliches Trinkgeld pro Stopp heute</div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-center text-xs">
            ...
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="font-bold text-sm text-gray-700 dark:text-gray-200">{d.team_avg.toFixed(2)} €/Stopp</div>
            </div>
          </div>
          ...
          <div className="text-xs text-gray-400 text-center">Trinkgeld-Quote · heute</div>
        </div>
      )}
    </div>
  );

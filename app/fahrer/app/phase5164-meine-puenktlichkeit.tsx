...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 95) return {
    text: 'Exzellente Pünktlichkeit! Kunden verlassen sich auf dich. Weiter so!',
    color: 'text-green-300',
  };
  if (val >= 80) return {
    text: 'Gute Pünktlichkeit. Wenige Minuten früher losfahren kann deinen Score weiter verbessern.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Niedrige Pünktlichkeit. Prüfe deine Routen und sprich mit dem Disponenten über Optimierungen.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5164MeinePuenktlichkeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Pünktlichkeit</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.puenktlichkeit_pct} <span className="text-xl font-semibold">%</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, mein.puenktlichkeit_pct)}%` }}
          />
        </div>

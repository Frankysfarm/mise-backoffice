...
import { WifiOff, AlertOctagon } from 'lucide-react';
...
function coachingTipp(val: number): { text: string; color: string } {
  if (val <= 1) return {
    text: 'Ausgezeichnete Genauigkeit! Fast keine Fehler — Kunden können sich auf dich verlassen.',
    color: 'text-green-300',
  };
  if (val <= 3) return {
    text: 'Akzeptable Fehlerquote. Prüfe jede Bestellung kurz vor der Übergabe sorgfältig.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Hohe Fehlerquote. Kontrolliere Artikel und Adresse bei jeder Lieferung genau.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5203MeineFehlerquote({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-reklamation-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Fehlerquote — offline</span>
      </div>
    );
  }
  ...
  const tipp    = coachingTipp(mein.reklamation_pct);
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Fehlerquote</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.reklamation_pct.toFixed(1)} <span className="text-xl font-semibold">%</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.reklamation_pct / Math.max(teamAvg * 2, 1)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.reklamation_pct.toFixed(1)}%</span>

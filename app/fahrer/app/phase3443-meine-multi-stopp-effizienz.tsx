...
import { ChevronDown, ChevronUp, BarChart2, TrendingUp, TrendingDown, Minus } from 'lucide-react';

interface FahrerMultiStopp {
  fahrer_id: string;
  fahrer_name: string;
  rang: number;
...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Top! Du packst besonders viele Stopps pro Tour — das macht dich sehr effizient.',
  gelb: 'Gut — versuche Touren mit mehr Stopps zu bündeln, um deine Effizienz zu steigern.',
  rot: 'Tipp: Sprich mit dem Dispatcher — dichtere Touren helfen dir, mehr Stopps pro Tour zu erreichen.',
};

export function FahrerPhase3443MeineMultiStoppEffizienz({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const [data, setData] = useState<FahrerMultiStopp | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-multi-stopp-effizienz?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
      if (!r.ok) return;
        ...
        <div className="px-4 pb-4 space-y-3">
          <div className="flex items-end justify-center gap-4">
            <div className="text-center">
              <div className={`text-5xl font-bold font-mono ${AMPEL_COLOR[data.ampel]}`}>
                {data.stopps_pro_tour.toFixed(1)}
              </div>
              <div className="text-xs text-gray-500 mt-0.5">Stopps/Tour</div>
            </div>
            <div className="text-center">
          ...
          <div className="grid grid-cols-2 gap-3 text-center">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2">
              ...
              <div className="text-xs font-semibold mt-0.5">{teamAvg} Stopps/Tour</div>
            </div>
          </div>

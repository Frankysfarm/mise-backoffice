...
function coachingTipp(val: number): { text: string; color: string } {
  if (val <= 2) return {
    text: 'Hervorragende Storno-Quote! Nahezu keine Aufträge werden storniert — sehr zuverlässig.',
    color: 'text-green-300',
  };
  if (val <= 5) return {
    text: 'Gute Storno-Quote. Achte auf häufige Stornogründe und kommuniziere frühzeitig mit Kunden.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Zu hohe Storno-Quote. Bitte Stornogründe analysieren — Kontakt zum Dispatcher empfohlen.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5240MeineStornoQuote({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Storno-Quote</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.stornoquote_pct.toFixed(0)}<span className="text-xl font-semibold">%</span>
        </div>

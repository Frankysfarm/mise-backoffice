...
function coachingTipp(anzahl: number, teamAvg: number): string {
  if (anzahl >= teamAvg * 1.15) return 'Stark! Du gehörst zu den aktivsten Fahrern im Team.';
  if (anzahl >= teamAvg * 0.9) return 'Solide – du liegst nah am Team-Durchschnitt.';
  return 'Mehr Schichten helfen, deine Tourenanzahl zu steigern.';
}

export function FahrerPhase4694MeineTourenAnzahl({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) {
          setData(me);
          setTeamAvg(json.team_avg_touren);
        }
      } catch {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 flex items-center gap-2 text-gray-400">
        ...
        <span className="text-xs">Offline – Tourenanzahl nicht verfügbar</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-cyan-200 dark:border-cyan-900 bg-cyan-50 dark:bg-cyan-950 p-3">
        <span className="text-xs text-cyan-700 dark:text-cyan-300">Touren-Daten nicht verfügbar</span>
      </div>
    );
  }
  ...
  const maxVal = teamAvg !== null ? Math.max(data.touren_anzahl, teamAvg) * 1.1 : data.touren_anzahl;
  const myBarWidth  = Math.min((data.touren_anzahl / maxVal) * 100, 100);
  ...
  return (
    <div className="rounded-2xl border border-cyan-200 dark:border-cyan-900 bg-cyan-50 dark:bg-cyan-950 p-3 space-y-2">
      ...
      <div className="flex items-end gap-3">
        <span className="text-4xl font-black leading-none text-cyan-800 dark:text-cyan-300">
          {data.touren_anzahl}
        </span>
        <span className="text-[10px] text-gray-500 dark:text-gray-400 mb-1">Touren (30 Tage)</span>
      </div>
        ...
        <div className="space-y-1">
          <div className="space-y-0.5">
            <div className="flex justify-between text-[9px] text-gray-500">
              <span>Ich</span>
              <span>{data.touren_anzahl}</span>
            </div>
            <div className="bg-gray-100 dark:bg-gray-800 rounded-full h-1.5">
              <div className="h-1.5 rounded-full bg-cyan-500 dark:bg-cyan-400" style={{ width: `${myBarWidth}%` }} />
            </div>
          </div>
          <div className="space-y-0.5">
            <div className="flex justify-between text-[9px] text-gray-500">
              <span>Team-Ø</span>
              <span>{Math.round(teamAvg)}</span>
            </div>
            <div className="bg-gray-100 dark:bg-gray-800 rounded-full h-1.5">
      ...
      <div className="bg-white/60 dark:bg-cyan-900/30 rounded-xl p-2 text-[10px] text-cyan-900 dark:text-cyan-200">
        {coachingTipp(data.touren_anzahl, teamAvg ?? data.touren_anzahl)}
      </div>
    </div>
  );
}

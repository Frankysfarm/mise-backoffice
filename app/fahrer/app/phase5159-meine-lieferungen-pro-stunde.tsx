...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 2.5) return {
    text: 'Sehr hohe Produktivität! Du lieferst effizient und pünktlich. Weiter so!',
    color: 'text-green-300',
  };
  if (val >= 1.5) return {
    text: 'Gute Produktivität. Optimiere deine Routen für noch mehr Lieferungen pro Stunde.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Niedrige Produktivität. Sprich mit dem Disponenten über Routen-Optimierung.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5159MeineLieferungenProStunde({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Lieferungen / Stunde</span>
      </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.deliveries_pro_h / Math.max(teamAvg * 2, 0.1)) * 100))}%` }}
          />
        </div>

...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: 'me', fahrer_name: 'Du', quote_pct: 81.6, rang: 2, trend_delta: 1.0, ampel: 'gelb' },
  ],
...
function coaching(a: string): string {
  if (a === 'gruen') return 'Ausgezeichnet! Du lieferst fast immer pünktlich — eine sehr gute Leistung. Weiter so!';
  if (a === 'gelb')  return 'Deine Pünktlichkeit ist solide. Plane Puffer für schwierige Adressen ein und informiere bei Verzögerungen den Kunden.';
  return                   'Deine Pünktlichkeitsrate ist zu niedrig. Optimiere deine Route, fahre früher los und melde Hindernisse rechtzeitig.';
}

export function FahrerPhase3292MeinePuenktlichkeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const cutoff = Math.ceil(totalFahrer * 0.25);
  const isAlert = me.rang > totalFahrer - cutoff;
      ...
      {open && (
        <div className="p-4 space-y-4">
          <div className="text-center">
            ...
            <div className="text-sm text-gray-500">Pünktlichkeitsrate heute</div>
            <div className={`text-3xl font-bold mt-1 ${col.text}`}>Rang #{me.rang}</div>
          </div>

          <div>
            <div className="text-xs text-gray-500 mb-1 flex justify-between">
              <span>Niedrigste Rate</span><span>Höchste Rate</span>
            </div>

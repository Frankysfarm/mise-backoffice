...
interface ApiData { avg_kmh: number; rang: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; team_avg_kmh: number; ziel_kmh: number; fahrer_count: number; }

const MOCK: ApiData = { avg_kmh: 25, rang: 2, rank_delta: 0, ampel: 'gruen', team_avg_kmh: 22, ziel_kmh: 25, fahrer_count: 4 };

interface Props { driverId: string; locationId: string | null; isOnline: boolean; }

export function FahrerPhase4158MeineGeschwindigkeit({ driverId, locationId, isOnline }: Props) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!json.error && json.fahrer?.length) {
          ...
          setData({ avg_kmh: me.avg_kmh, rang: me.rang, rank_delta: me.rank_delta, ampel: me.ampel, team_avg_kmh: json.team_avg_kmh, ziel_kmh: json.ziel_kmh, fahrer_count: json.gesamt });
        }
      }
  ...
  const valColor = data.ampel === 'gruen' ? 'text-sky-500' : data.ampel === 'gelb' ? 'text-yellow-500' : 'text-red-500';
  ...
  const coaching = data.ampel === 'gruen'
    ? 'Stark – du fährst sehr zügig durch deine Touren!'
    : data.ampel === 'gelb'
    ? 'Tipp: Achte auf flüssiges Fahren ohne unnötige Stopps.'
    : 'Achtung: Deine Durchschnittsgeschwindigkeit liegt unter dem Teamdurchschnitt.';

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        <Zap className="w-4 h-4 text-sky-500" />
        <span className="text-sm font-semibold text-gray-900">Meine Ø Geschwindigkeit</span>
        {loading && <span className="w-3 h-3 border-2 border-gray-300 border-t-transparent rounded-full animate-spin ml-auto" />}
      </div>
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-bold ${valColor}`}>{data.avg_kmh?.toFixed(0)}</span>
        <div className="pb-1 space-y-0.5">
          <span className="text-xl font-medium text-gray-500">km/h</span>

...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 4.5) return {
    text: 'Exzellente Kundenbewertungen! Deine Freundlichkeit und Sorgfalt machen den Unterschied.',
    color: 'text-green-300',
  };
  if (val >= 3.5) return {
    text: 'Gute Bewertungen. Mit einem Lächeln und pünktlicher Lieferung erreichst du die Spitze.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Niedrige Bewertungen. Freundlichkeit, sorgfältige Übergabe und Pünktlichkeit helfen.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5195MeineKundenzufriedenheit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Kundenbewertung — offline</span>
      </div>
    );
  }
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.round((mein.avg_rating / 5) * 100)}%` }}
          />
        </div>

...
export function FahrerPhase4268MeineKundenzufriedenheit({ driverId, locationId, isOnline }: Props) {
  ...
  const ampelColor = me.ampel === 'gruen' ? 'text-yellow-600' : me.ampel === 'gelb' ? 'text-orange-600' : 'text-red-600';
  const ampelBg = me.ampel === 'gruen' ? 'bg-yellow-50' : me.ampel === 'gelb' ? 'bg-orange-50' : 'bg-red-50';
  ...
  const coaching =
    me.ampel === 'gruen'
      ? 'Ausgezeichnet! Deine Kunden sind sehr zufrieden mit dir.'
      : me.ampel === 'gelb'
      ? 'Tipp: Freundlicher Umgang und pünktliche Lieferung steigern die Bewertung.'
      : 'Achtung: Deine Kundenbewertung liegt unter dem Zielwert – bitte verbessern.';
  ...
  return (
    <div className={`rounded-2xl border p-4 space-y-3 ${ampelBg} border-gray-200`}>
      ...
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-extrabold ${ampelColor}`}>{me.avg_sterne} ★</span>
        <div className="pb-1 space-y-0.5">
          <span className="text-sm text-gray-500">Ø Sterne</span>
      ...
      <div className="flex items-center justify-between text-xs text-gray-500">
        <span>Team-Ø: <span className="font-semibold text-gray-700">{data.team_avg} ★</span></span>

...
interface ApiData { trinkgeld_quote: number; rang: number; rank_delta: number; ampel: 'gruen' | 'gelb' | 'rot'; team_avg: number; fahrer_count: number; }

const MOCK: ApiData = { trinkgeld_quote: 9, rang: 2, rank_delta: 0, ampel: 'gruen', team_avg: 7.5, fahrer_count: 4 };

interface Props { driverId: string; locationId: string | null; isOnline: boolean; }

export function FahrerPhase4193MeineTrinkgeldQuote({ driverId, locationId, isOnline }: Props) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!json.error && json.fahrer?.length) {
          ...
          setData({ trinkgeld_quote: me.trinkgeld_quote, rang: me.rang, rank_delta: me.rank_delta, ampel: me.ampel, team_avg: json.team_avg, fahrer_count: json.gesamt });
        }
      }
  ...
  const valColor = data.ampel === 'gruen' ? 'text-amber-500' : data.ampel === 'gelb' ? 'text-yellow-500' : 'text-red-500';
  ...
  const coaching = data.ampel === 'gruen'
    ? 'Hervorragend – Kunden schätzen deinen Service mit Trinkgeld!'
    : data.ampel === 'gelb'
    ? 'Tipp: Freundlichkeit und Pünktlichkeit steigern die Trinkgeldquote.'
    : 'Achtung: Deine Trinkgeldquote liegt unter dem Teamdurchschnitt.';

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        <Star className="w-4 h-4 text-amber-500" />
        <span className="text-sm font-semibold text-gray-900">Meine Trinkgeld-Quote</span>
        {loading && <span className="w-3 h-3 border-2 border-gray-300 border-t-transparent rounded-full animate-spin ml-auto" />}
      </div>
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-bold ${valColor}`}>{data.trinkgeld_quote?.toFixed(1)}</span>
      ...
      <div className="flex justify-between text-xs text-gray-400">
        <span>Team-Ø {data.team_avg?.toFixed(1)}% · Trinkgeld-Quote</span>

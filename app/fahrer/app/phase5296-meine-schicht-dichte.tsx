...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 5) return {
    text: 'Hohe Schicht-Dichte! Du bist regelmäßig im Einsatz — das sichert Bonuschancen und Stammkunden-Status.',
    color: 'text-green-300',
  };
  if (val >= 3) return {
    text: 'Gute Schicht-Häufigkeit. Mehr als 5 Schichten pro Woche steigern dein Ranking und Einkommen.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Geringe Schicht-Dichte. Mehr regelmäßige Schichten verbessern dein Ranking und sichern Boni.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5296MeineSchichtDichte({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-schicht-dichte-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Schicht-Dichte — offline</span>
      </div>
    );
  }
  ...
  const tipp = coachingTipp(mein.schichten_pro_woche);
  const teamAvg = data.team_avg;
  const maxBar = Math.max(mein.schichten_pro_woche, teamAvg, 1) * 1.2;
    ...
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.schichten_pro_woche.toFixed(1)}<span className="text-xl font-semibold text-gray-400">/Wo</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="relative h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min((mein.schichten_pro_woche / maxBar) * 100, 100)}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.schichten_pro_woche.toFixed(1)}/Wo</span>
          <span>{teamAvg.toFixed(1)}/Wo</span>
        </div>
      </div>

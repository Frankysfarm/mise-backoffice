...
function coaching(pct: number, alert: string | null): string {
  if (alert) return 'Deine Leerfahrten-Quote ist zu hoch. Stimme dich besser mit dem Dispatch ab, damit du seltener ohne Auftrag fährst.';
  if (pct < ZIEL) return `Gut gemacht — nur ${pct.toFixed(1)}% Leerfahrten! Du fährst sehr effizient.`;
  return `${pct.toFixed(1)}% Leerfahrten. Ziel ist <${ZIEL}% — melde dich früher im Dispatch, wenn du frei bist.`;
}
...
const MOCK: ApiData = {
  fahrer: [
    { fahrer_id: MOCK_ID, fahrer_name: 'Ich',     quote_pct: 12.0, leerfahrten_heute: 2, touren_heute: 17, trend: 'fallend',  alert: null },
    { fahrer_id: 'f2',    fahrer_name: 'Max M.',   quote_pct:  5.0, leerfahrten_heute: 1, touren_heute: 20, trend: 'fallend',  alert: null },
    { fahrer_id: 'f3',    fahrer_name: 'Julia F.', quote_pct: 31.0, leerfahrten_heute: 4, touren_heute: 13, trend: 'steigend', alert: 'Zu viele Leerfahrten!' },
  ],
  team_avg_pct: 16.0,
};

export function FahrerPhase2723MeineLeerfahrten({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const ampel   = calcAmpel(me.quote_pct);
  const cls     = ampelCls(ampel);
  const fill    = Math.min(100, (me.quote_pct / MAX) * 100);
  const zielPct = (ZIEL / MAX) * 100;
  const rang    = [...data.fahrer].sort((a, b) => a.quote_pct - b.quote_pct).findIndex(
    (f: FahrerEntry) => f.fahrer_id === me.fahrer_id
  ) + 1;
    ...
    <div className={`rounded-xl border ${cls.bg} shadow-sm mb-4`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800">Meine Leerfahrten</span>
        </div>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3">
          <div className="text-center">
            <div className={`text-4xl font-black ${cls.big}`}>{me.quote_pct.toFixed(1)}%</div>
            <div className="text-xs text-gray-500 mt-0.5">Leerfahrten-Quote</div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              {
                label: 'Trend',
                value: me.trend === 'fallend' ? '↓ Besser' : me.trend === 'steigend' ? '↑ Schlechter' : '→ Stabil',
                c: me.trend === 'fallend' ? 'text-green-600' : me.trend === 'steigend' ? 'text-red-500' : 'text-gray-500',
              },
              { label: 'Ziel',   value: `<${ZIEL}%`,                             c: 'text-gray-600' },
              { label: 'Ampel',  value: ampel === 'gruen' ? 'Grün ✓' : ampel === 'gelb' ? 'Gelb ⚠' : 'Rot ✗', c: cls.text },
              { label: 'Team-Ø', value: `${data.team_avg_pct.toFixed(1)}%`,      c: 'text-gray-600' },
            ].map(({ label, value, c }) => (
          ...
          <div className={`rounded-lg px-3 py-2 text-xs ${cls.text} bg-white/50 border ${cls.bg.split(' ')[1]}`}>
            <div className="flex items-start gap-1.5">
              ...
              <span>{coaching(me.quote_pct, me.alert)}</span>
            </div>
          </div>

          <div className="text-center text-[10px] text-gray-400">
            Rang {rang}/{data.fahrer.length} · {me.leerfahrten_heute} Leer · {me.touren_heute} Touren gesamt
          </div>
        </div>
      )}
    </div>

...
function coachingTipp(tph: number): { text: string; color: string } {
  if (tph >= 3.0) return {
    text: 'Starke Abendschicht-Produktivität! Der Abend ist dein stärkstes Zeitfenster — nutze das Momentum und bleib fokussiert.',
    color: 'text-orange-400',
  };
  if (tph >= 1.5) return {
    text: 'Gute Abendleistung. Kürzere Wartezeiten am Restaurant und optimierte Abendroutenplanung holen noch mehr Touren raus.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Noch Potenzial für die Abendschicht. Der Abend ist oft die beste Lieferzeit — setze auf schnelle Stopps und direkte Wege.',
    color: 'text-amber-400',
  };
}

function ampelColor(a: string) {
  if (a === 'gruen') return 'text-amber-400';
  if (a === 'gelb') return 'text-yellow-400';
  return 'text-orange-400';
}

export function FahrerPhase4923MeineAbendprod({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Abendschicht-Produktivität — offline</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-orange-700 bg-orange-950/40 overflow-hidden mb-3">
      ...
      <div className="px-4 py-3 flex items-center gap-2 border-b border-orange-700/50 bg-orange-900/20">
        ...
        <span className="text-sm font-semibold text-orange-200">Meine Abendschicht-Produktivität</span>
      </div>
      ...
      <div className="px-4 pb-3 space-y-2">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 h-2 bg-orange-900/50 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full ${mein.ampel === 'gruen' ? 'bg-amber-500' : mein.ampel === 'gelb' ? 'bg-yellow-500' : 'bg-orange-500'}`}
              style={{ width: `${barPct}%` }}
            />
          </div>

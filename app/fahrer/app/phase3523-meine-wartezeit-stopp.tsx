...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Exzellent! Du bist sehr zügig an den Stopps — kurze Wartezeiten bedeuten mehr Lieferungen.',
  gelb: 'Gut — versuche Pakete vorzubereiten bevor du ankommst, um deine Wartezeit zu senken.',
  rot: 'Tipp: Bereite Pakete während der Fahrt vor — das spart wertvolle Minuten am Stopp.',
};

export function FahrerPhase3523MeineWartezeitStopp({
  driverId,
  locationId,
  isOnline,
}: {
        ...
        <div className="px-4 pb-4 space-y-3">
          <div className="flex items-end justify-center gap-4">
            <div className="text-center">
              ...
              <div className="text-xs text-gray-500 mt-0.5">min/Stopp</div>
            </div>
            <div className="text-center">
              ...
              <div className="flex items-center justify-center gap-1 text-xs font-semibold mt-0.5">
                {data.rank_delta > 0 ? (
                  <>
                    ...
                    <span className="text-emerald-600">{data.rank_delta} ↑</span>
                  </>
                ) : data.rank_delta < 0 ? (
                  <>
                    <TrendingDown className="w-3 h-3 text-red-500" />
                    <span className="text-red-600">{Math.abs(data.rank_delta)} ↓</span>
                  </>
                ) : (
                  <>

...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Sehr gut! Du reagierst besonders schnell auf neue Tourenzuweisungen.',
  gelb: 'Gut — halte dein Handy griffbereit und starte nach Zuweisung zügig.',
  rot: 'Tipp: Richte Push-Benachrichtigungen ein und starte so schnell wie möglich nach Zuweisung.',
};

export function FahrerPhase3438MeineReaktionszeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const [data, setData] = useState<FahrerReaktionszeit | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-reaktionszeit-zuweisung?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
      if (!r.ok) return;
  ...
  // lower rank = better; bar width: rank 1 = 100%, rank N = ~0%
          ...
          <div className="flex items-end justify-center gap-4">
            <div className="text-center">
              <div className={`text-5xl font-bold font-mono ${AMPEL_COLOR[data.ampel]}`}>
                {data.reaktionszeit_min}
              </div>
              <div className="text-xs text-gray-500 mt-0.5">Minuten</div>
            </div>
            <div className="text-center">

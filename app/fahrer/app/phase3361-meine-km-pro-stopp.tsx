...
function coachingTipp(a: string): string {
  if (a === 'rot')  return 'Hohe km/Stopp! Du fährst viele Kilometer pro Lieferung. Versuche Touren mit näher beieinanderliegenden Stopps zu wählen, um kürzere Wege zu erreichen.';
  if (a === 'gelb') return 'Guter Wert! Mit fokussierteren Routen in dichteren Gebieten kannst du die km/Stopp noch weiter reduzieren.';
  return 'Hervorragende Streckeneffizienz! Du lieferst mit minimalen Kilometern pro Stopp — perfekte Routenoptimierung!';
}

export function FahrerPhase3361MeineKmProStopp({
  driverId,
  locationId,
  isOnline,
}: {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine km/Stopp</span>
          {me.alert_top && (
        ...
        <div className="p-4 space-y-4">
          <div className={`rounded-xl p-4 text-center ${cls.bg}`}>
            <div className={`text-5xl font-black ${cls.text}`}>
              {me.km_pro_stopp.toFixed(2)}<span className="text-2xl"> km/S</span>
            </div>
            <div className={`text-3xl font-bold ${cls.text} mt-1`}>Rang #{me.rang}</div>
            <div className="text-xs text-gray-400 mt-1">Kilometer je Stopp heute</div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-center text-xs">
            ...
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="font-bold text-sm text-gray-700 dark:text-gray-200">{d.team_avg.toFixed(2)} km/S</div>
            </div>
          </div>
          ...
          <div className="text-xs text-gray-400 text-center">km/Stopp · heute · Ziel &lt;2.0 km</div>
        </div>
      )}
    </div>
  );

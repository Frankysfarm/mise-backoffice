...
const MOCK: ApiData = { leerfahrten_pct: 12, rang: 2, rank_delta: 1, ampel: 'gruen', team_avg_leerfahrten_pct: 19.25, fahrer_count: 4 };

interface Props { driverId: string; locationId: string | null; isOnline: boolean; }

export function FahrerPhase4148MeineLeerfahrten({ driverId, locationId, isOnline }: Props) {
  ...
  const coaching = data.ampel === 'gruen'
    ...
    : data.ampel === 'gelb'
    ? 'Tipp: Stimme Touren besser ab, um Leerfahrten zu reduzieren.'
    : 'Achtung: Deine Leerfahrtenquote liegt über dem Teamdurchschnitt.';
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-bold ${valColor}`}>{data.leerfahrten_pct?.toFixed(0)}</span>
        <div className="pb-1 space-y-0.5">
          <span className="text-xl font-medium text-gray-500">% leer</span>

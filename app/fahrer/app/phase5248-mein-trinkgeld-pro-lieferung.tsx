...
import { WifiOff, Banknote } from 'lucide-react';
...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 2) return {
    text: 'Top-Trinkgeld! Kunden schätzen deinen Service und belohnen ihn großzügig.',
    color: 'text-green-300',
  };
  if (val >= 1) return {
    text: 'Solides Trinkgeld. Freundlichkeit und sorgfältige Übergabe können den Schnitt steigern.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Wenig Trinkgeld. Mehr Kundenfreundlichkeit und pünktliche Lieferung helfen nachweislich.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5248MeinTrinkgeldProLieferung({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Mein Trinkgeld/Lieferung — offline</span>
      </div>
    );
  }
  ...
  const tipp = coachingTipp(mein.avg_trinkgeld);
  const teamAvg = data.team_avg_trinkgeld;
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Mein Ø-Trinkgeld/Lieferung</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.avg_trinkgeld.toFixed(2)}<span className="text-xl font-semibold"> €</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.avg_trinkgeld / Math.max(teamAvg * 2, 0.01)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.avg_trinkgeld.toFixed(2)} €</span>
          <span>{teamAvg.toFixed(2)} €</span>
        </div>
      </div>

...
export function FahrerPhase4168MeineStoppdauer({ driverId, locationId, isOnline }: Props) {
  ...
  const coaching = data.ampel === 'gruen'
    ? 'Stark – du erledigst deine Stopps sehr effizient!'
    : data.ampel === 'gelb'
    ? 'Tipp: Bereite Pakete schon während der Anfahrt vor, um Stopps zu verkürzen.'
    : 'Achtung: Deine Stopp-Dauer liegt über dem Teamdurchschnitt.';

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center gap-2">
        <Timer className="w-4 h-4 text-orange-500" />
        <span className="text-sm font-semibold text-gray-900">Meine Stopp-Dauer</span>
        {loading && <span className="w-3 h-3 border-2 border-gray-300 border-t-transparent rounded-full animate-spin ml-auto" />}
      </div>
      <div className="flex items-end gap-3">
        <span className={`text-5xl font-bold ${valColor}`}>{data.avg_sec?.toFixed(0)}</span>

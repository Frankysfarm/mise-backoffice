...
import { ChevronDown, ChevronUp, ListChecks, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Top! Du erledigst viele Stopps pro Schicht — das zeigt hohe Produktivität.',
  gelb: 'Gut — versuche deine Routen besser zu planen, um mehr Stopps pro Schicht zu erreichen.',
  rot: 'Tipp: Sprich mit dem Dispatcher — effizientere Routenplanung steigert deine Stopps/Schicht.',
};

export function FahrerPhase3513MeineStoppsProSchicht({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const [data, setData] = useState<FahrerStoppsProSchicht | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-stopps-pro-schicht?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
      if (!r.ok) return;
        ...
        <div className="px-4 pb-4 space-y-3">
          <div className="flex items-end justify-center gap-4">
            <div className="text-center">
              <div className={`text-5xl font-bold font-mono ${AMPEL_COLOR[data.ampel]}`}>
                {data.stopps_pro_schicht}
              </div>
              <div className="text-xs text-gray-500 mt-0.5">Stopps/Schicht</div>
            </div>
            <div className="text-center">
          ...
          <div className="grid grid-cols-2 gap-3 text-center">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2">
              ...
              <div className="text-xs font-semibold mt-0.5">{teamAvg}/Schicht</div>
            </div>
          </div>

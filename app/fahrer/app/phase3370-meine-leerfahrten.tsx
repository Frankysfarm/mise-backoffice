...
function ampelCls(a: string) {
  ...
  return                   { text: 'text-orange-700 dark:text-orange-300',   bar: 'bg-orange-500', bg: 'bg-orange-50 dark:bg-orange-950'   };
}

function coachingTipp(a: string): string {
  if (a === 'rot')  return 'Viele Leerfahrten! Sprich mit dem Dispatcher — enge Bündelung von Touren reduziert unnötige Leerkilometer.';
  if (a === 'gelb') return 'Einige Leerfahrten heute. Versuche, Stopps besser zu bündeln und kurze Rückwege zu vermeiden.';
  return 'Super! Kaum Leerfahrten — deine Touren sind gut optimiert. Weiter so!';
}

export function FahrerPhase3370MeineLeerfahrten({
  driverId,
  locationId,
  isOnline,
}: {
        ...
        <div className="p-4 space-y-4">
          <div className={`rounded-xl p-4 text-center ${cls.bg}`}>
            <div className={`text-5xl font-black ${cls.text}`}>
              {me.leerfahrten}<span className="text-2xl"> LF</span>
            </div>
            <div className={`text-3xl font-bold ${cls.text} mt-1`}>Rang #{me.rang}</div>
            <div className="text-xs text-gray-400 mt-1">Leerfahrten heute (weniger = besser)</div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-center text-xs">
            ...
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2">
              ...
              <div className="font-bold text-sm text-gray-700 dark:text-gray-200">{d.team_avg_leerfahrten} LF</div>
            </div>
          </div>
          ...
          <div className="text-xs text-gray-400 text-center">Leerfahrten · heute</div>
        </div>
      )}
    </div>
  );

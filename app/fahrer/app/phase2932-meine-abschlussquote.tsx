...
function coachingTipp(ampel: string): string {
  if (ampel === 'rot')  return 'Abschlussquote zu niedrig! Nicht-zustellbare Stopps sofort im System melden.';
  if (ampel === 'gelb') return 'Noch Potenzial! Kunden vor der Ankunft anrufen bei unklaren Adressen.';
  return 'Hervorragende Abschlussquote — weiter so! Kunden erhalten ihre Bestellungen zuverlässig.';
}
...
export function FahrerPhase2932MeineAbschlussquote({ driverId, locationId, isOnline }: Props) {
    ...
    const load = () =>
      fetch(`/api/delivery/admin/fahrer-abschlussquote?location_id=${locationId}`)
        ...
        .then((d: ApiData) => {
          ...
          if (f) setData({ quote_pct: f.quote_pct, trend: f.trend, trend_delta: f.trend_delta, ampel: f.ampel, team_avg: d.team_avg_quote });
          else setData(MOCK);
        })
        .catch(() => setData(MOCK));
    load();
  ...
  const ampel = calcAmpel(data.quote_pct);
  const { text, bar } = ampelColors(ampel);
  const pct     = Math.min(100, data.quote_pct);
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 bg-white dark:bg-gray-900 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Abschlussquote</span>
        </div>
      ...
      {open && (
        <div className="px-4 py-4 space-y-4">
          <div className="text-center">
            <div className={`text-4xl font-extrabold ${text}`}>{data.quote_pct.toFixed(0)}%</div>
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">Abschlussquote heute</div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2 text-center">
              ...
              <div className="text-sm font-bold text-gray-700 dark:text-gray-300">{data.team_avg.toFixed(0)}%</div>
            </div>
          </div>

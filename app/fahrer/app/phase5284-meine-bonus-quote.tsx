...
import { WifiOff, Gift } from 'lucide-react';
...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 50) return {
    text: 'Starke Bonus-Quote! Du erzielst auf mehr als der Hälfte deiner Touren einen Bonus — top Leistung.',
    color: 'text-green-300',
  };
  if (val >= 25) return {
    text: 'Solide Bonus-Quote. Mit schnellen Lieferzeiten und hoher Kundenzufriedenheit erreichst du öfter Bonus-Schwellen.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Geringe Bonus-Quote. Fokussiere auf Pünktlichkeit und kurze Abholzeiten, um mehr Touren mit Bonus abzuschließen.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5284MeineBonusQuote({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-bonus-quote-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Bonus-Quote — offline</span>
      </div>
    );
  }
  ...
  const tipp = coachingTipp(mein.bonus_quote_pct);
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Bonus-Quote</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.bonus_quote_pct.toFixed(0)}<span className="text-xl font-semibold">%</span>
        </div>

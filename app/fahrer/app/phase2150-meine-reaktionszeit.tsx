...
function tipp(avg: number, trend: string): string {
  if (avg <= 2) return 'Hervorragend! Du reagierst sofort — das freut die Küche.';
  if (avg <= 3 && trend === 'besser') return 'Reaktionszeit verbessert! Weiter so — Ziel erreicht.';
  if (avg <= 3) return 'Sehr gut — Ziel unter 3 Min. erreicht.';
  if (avg <= 5) return 'Gut — aber noch Luft. Batch-Notifications gleich bestätigen.';
  if (trend === 'schlechter') return 'Reaktionszeit steigt — bitte Batches schneller akzeptieren.';
  return 'Über 5 Min. — informiere den Dispatcher wenn du überlastet bist.';
}
...
export function FahrerPhase2150MeineReaktionszeit({ driverId, locationId, isOnline }: Props) {
  ...
  const color  = mein.avg_min <= 3 ? 'text-green-600' : mein.avg_min <= 5 ? 'text-amber-600' : 'text-red-600';
  ...
  return (
    <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
      ...
      >
        ...
        <span className={cn('text-xs font-black tabular-nums', color)}>{mein.avg_min.toFixed(1)} Min</span>
        ...
        <div className="p-3 space-y-3">
          <div className="flex items-center gap-3">
            <div>
              ...
              <p className={cn('text-3xl font-black tabular-nums', color)}>{mein.avg_min.toFixed(1)}</p>
              <p className="text-[9px] text-muted-foreground">Minuten</p>
            </div>

...
function coachingTipp(val: number): { text: string; color: string } {
  if (val >= 3) return {
    text: 'Sehr hohe Tour-Frequenz! Du nutzt deine Schichten maximal aus — top Einsatz.',
    color: 'text-green-300',
  };
  if (val >= 2) return {
    text: 'Solide Tour-Frequenz. Schnellere Tourübergaben und kürzere Pausen steigern die Rate.',
    color: 'text-yellow-400',
  };
  return {
    text: 'Niedrige Tour-Frequenz. Stimme dich besser mit Dispatch ab und nutze Leerzeiten aktiv.',
    color: 'text-red-400',
  };
}
...
export function FahrerPhase5224MeineTourenProTag({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/delivery/admin/fahrer-touren-pro-tag-ranking?${params}`);
  ...
  if (!isOnline) {
    return (
      <div className="rounded-xl border border-gray-700 bg-gray-900/40 px-4 py-3 flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-500">Meine Ø-Touren/Tag — offline</span>
      </div>
    );
  }
  ...
  const tipp    = coachingTipp(mein.avg_touren_pro_tag);
  ...
  return (
    <div className={`rounded-2xl border mb-3 overflow-hidden ${
      ...
      <div className={`px-4 py-3 flex items-center gap-2 border-b ${
        ...
        <span className="text-sm font-semibold text-gray-200">Meine Ø-Touren/Tag (Frequenz)</span>
      </div>

      <div className="px-4 py-4 text-center">
        <div className={`text-4xl font-black tabular-nums ${ampelColor(mein.ampel)}`}>
          {mein.avg_touren_pro_tag.toFixed(1)} <span className="text-xl font-semibold">/Tag</span>
        </div>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.round((mein.avg_touren_pro_tag / Math.max(teamAvg * 2, 1)) * 100))}%` }}
          />
        </div>
        <div className="flex items-center justify-between text-[10px] mt-0.5 text-gray-500">
          <span>{mein.avg_touren_pro_tag.toFixed(1)}/Tag</span>
          <span>{teamAvg.toFixed(1)}/Tag</span>
        </div>
      </div>

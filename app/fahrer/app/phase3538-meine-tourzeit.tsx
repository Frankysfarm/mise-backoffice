...
const COACHING_TIP: Record<string, string> = {
  gruen: 'Ausgezeichnet! Du erledigst deine Touren sehr zügig — das ist top für die Kundenzufriedenheit.',
  gelb: 'Gut — versuche unnötige Wartezeiten zu vermeiden, um deine Tourzeit weiter zu senken.',
  rot: 'Tipp: Plane deine Route vorab und minimiere Stopps ohne Lieferung, um die Tourzeit zu reduzieren.',
};

export function FahrerPhase3538MeineTourzeit({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/delivery/admin/fahrer-tourzeit?location_id=${locationId}&driver_id=${driverId}`,
        { cache: 'no-store' }
      );
      if (!r.ok) return;
        ...
        <div className="px-4 pb-4 space-y-3">
          <div className="flex items-end justify-center gap-4">
            <div className="text-center">
              ...
              <div className="text-xs text-gray-500 mt-0.5">min/Tour</div>
            </div>
            <div className="text-center">

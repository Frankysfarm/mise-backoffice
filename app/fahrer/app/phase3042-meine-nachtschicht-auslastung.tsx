...
function ampelCls(a: string) {
  ...
  return                   { text: 'text-indigo-600', bar: 'bg-indigo-500', tip: 'bg-indigo-50 text-indigo-700 border-indigo-200' };
}

const TIPPS: Record<string, string> = {
  gruen: '≥80 %: Starke Nachtschicht! Du nutzt deine Zeit optimal — weiter so!',
  gelb:  '60–79 %: Noch Luft nach oben. Versuche, Wartezeiten in der Nachtschicht zu reduzieren.',
  rot:   '<60 %: Zu geringe Nacht-Auslastung! Bitte prüfe deinen Einsatz zwischen 22 und 6 Uhr.',
};

const MOCK_ENTRY: FahrerEntry = {
  fahrer_id: 'mock', auslastung_pct: 76.0, auslastung_pct_vorwoche: 71.5, trend: 'steigend', trend_delta: 4.5, ampel: 'gelb',
};
const MOCK_DATA: ApiData = { fahrer: [MOCK_ENTRY], team_avg_pct: 68.0 };
...
export function FahrerPhase3042MeineNachtschichtAuslastung({
  driverId,
  locationId,
  isOnline,
}: {
  ...
  useEffect(() => {
    ...
    const load = () => {
      const url = `/api/delivery/admin/fahrer-nachtschicht-auslastung?location_id=${locationId}${driverId ? `&driver_id=${driverId}` : ''}`;
      fetch(url)
        .then(r => r.json())
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden bg-white dark:bg-gray-900">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-gray-100">Meine Nachtschicht-Auslastung</span>
        </div>
      ...
      {open && (
        <div className="px-4 py-4 space-y-4">
          <div className="text-center">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">Auslastung Nachtschicht (22–06 Uhr)</div>

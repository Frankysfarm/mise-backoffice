...
export function AvailabilityLoop({ shifts, initial }: { shifts: OpenShift[]; initial: { shift_id: string; state: string; applied: boolean }[] }) {
  ...
  async function choose(shiftId: string, state: 'kann' | 'moechte' | 'kann_nicht', applied = false) {
    setBusy(shiftId); const response = await fetch('/api/scheduling/planner', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'availability', shiftId, state, applied }) }); const result = await response.json().catch(() => null); setBusy('');
    if (!response.ok) return toastError('Rückmeldung nicht gespeichert', result?.error ?? 'Bitte versuche es erneut.');
    setStates(current => new Map(current).set(shiftId, { state, applied }));
    toastSuccess('Rückmeldung gespeichert');
  }

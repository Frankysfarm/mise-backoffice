...
type Handover = {
  ...
  read_at: string | null; confirmed_at: string | null;
  open_task_ids: string[]; open_tasks: { id: string; title: string }[]; incidents: string | null; inventory_notes: string | null; damage_notes: string | null; cleaning_notes: string | null; important_notes: string | null;
...
export function MyOperations({
  ...
  const taskTitleById = new Map([...tasks.map((task) => [task.id, task.title] as const), ...handovers.flatMap((handover) => handover.open_tasks.map((task) => [task.id, task.title] as const))]);

  function updateTask(task: Task, status: string, reviewNote?: string) {
    mutate({ action: 'update_task', taskId: task.id, status, ...(reviewNote ? { reviewNote } : {}) }, (result) => setTasks((current) => current.map((item) => item.id === task.id ? { ...item, ...result.task } : item)));
  }
  function rejectTask(task: Task) {
    const note = window.prompt('Was ist zu beanstanden? Der Grund geht direkt an die zuständige Person.');
    if (note === null) return;
    if (note.trim().length < 3) { setError('Bitte einen kurzen Grund angeben.'); return; }
    updateTask(task, 'nicht_bestanden', note.trim());
  }
  function acknowledgeHandover(handover: Handover, action: 'read_handover' | 'confirm_handover') {
    setError(''); startTransition(async () => {
      const response = await fetch('/api/operations/tasks', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action, id: handover.id, locationId }) });
      const result = await response.json().catch(() => null);
      if (!response.ok) { setError(result?.error ?? 'Übergabe konnte nicht gespeichert werden.'); return; }
      setHandovers((current) => current.map((item) => item.id === handover.id ? { ...item, ...result.handover } : item));
    });
  }
...
function HandoverFact({ label, children }: { label: string; children: React.ReactNode }) { return <div className="rounded-xl bg-slate-50 p-3"><strong className="text-xs">{label}</strong><div className="mt-1 whitespace-pre-line text-slate-700">{children}</div></div>; }
function showLegacyNote(handover: Handover) { return Boolean(handover.note && handover.note !== handover.important_notes && handover.note !== handover.incidents && handover.note !== 'Übergabe ohne zusätzliche Hinweise'); }

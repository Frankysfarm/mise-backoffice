...
export default async function ShopSettings() {
  ...
  const Card = ({ title, children }: any) => (<div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 16, padding: 22, boxShadow: '0 1px 2px rgba(15,23,42,.05)' }}><h3 style={{ fontFamily: "'Space Grotesk'", fontSize: 16, fontWeight: 700, marginBottom: 14 }}>{title}</h3>{children}</div>);
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, alignItems: 'start' }}>
      <Card title="Deine Shop-Adresse">
        <div style={{ fontSize: 13, color: '#64748B', marginBottom: 6 }}>MISE-Shop-URL</div>
        <div style={{ fontFamily: "'Space Grotesk'", fontSize: 15, fontWeight: 600, color: '#4338CA', wordBreak: 'break-all', background: '#EEF2FF', borderRadius: 10, padding: '10px 12px' }}>{shopUrl || '—'}</div>

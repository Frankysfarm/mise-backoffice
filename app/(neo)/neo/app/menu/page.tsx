  ...
  return (
    <div style={{ maxWidth: 1180 }}>
      <div style={{ display: 'flex', alignItems: 'stretch', gap: 16, marginBottom: 22 }}>
        <Soon href="/menu/import" style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 16, background: '#fff', border: '2px dashed #CBD5E1', borderRadius: 16, padding: '18px 22px', cursor: 'pointer', textAlign: 'left' }}>
          ...
          <div style={{ flex: 1 }}><div style={{ fontSize: 15, fontWeight: 700, color: '#0F172A' }}>Menü hochladen</div><div style={{ fontSize: 13, color: '#64748B' }}>Kategorien, Artikel, Preise & Steuersätze werden automatisch erkannt</div></div>
        ...
        <Soon href="/menu" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 22px', border: 'none', borderRadius: 16, background: 'linear-gradient(135deg,#4F46E5,#4338CA)', color: '#fff', fontSize: 14.5, fontWeight: 700, cursor: 'pointer', boxShadow: '0 8px 20px rgba(79,70,229,.28)' }}><span dangerouslySetInnerHTML={{ __html: '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><path d="M12 5v14M5 12h14"/></svg>' }} />Artikel manuell anlegen</Soon>
      </div>

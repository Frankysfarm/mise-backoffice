...
export default function Shell({ children, newCount = 0, tenantName = 'Mein Shop', shopUrl = '#' }: { children: React.ReactNode; newCount?: number; tenantName?: string; shopUrl?: string }) {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", background: '#F1F5F9', color: '#0F172A' }}>
      <aside style={{ width: 256, flexShrink: 0, background: '#fff', borderRight: '1px solid #E2E8F0', display: 'flex', flexDirection: 'column', height: '100vh' }}>
        ...
        <div style={{ padding: '14px 14px 6px' }}>
          <Link href="/neo" style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#EEF2FF', border: '1px solid #C7D2FE', borderRadius: 11, padding: '9px 11px', textDecoration: 'none' }}>
          ...
          {NAV.map((grp) => (
            <div key={grp.label} style={{ marginTop: 14 }}>
              ...
              {grp.items.map(([key, label]) => {
                const on = key === active;
                return (
                  <Link key={key} href={href(key)} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '9px 11px', borderRadius: 10, fontSize: 14, fontWeight: 600, marginBottom: 2, textDecoration: 'none', transition: 'background .12s', background: on ? '#EEF2FF' : 'transparent', color: on ? '#4338CA' : '#64748B' }}>
                    <Svg html={ICONS[key]} />
        ...
        <div style={{ flexShrink: 0, padding: '12px 14px', borderTop: '1px solid #F1F5F9' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 8px', borderRadius: 10 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 700, color: '#334155', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{tenantName}</div><div style={{ fontSize: 11.5, color: '#94A3B8' }}>Lieferservice</div></div>
          </div>
        </div>
      </aside>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
        <header style={{ height: 64, flexShrink: 0, background: '#fff', borderBottom: '1px solid #E2E8F0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 26px' }}>
          <div>
            ...
            <div style={{ fontSize: 12.5, color: '#94A3B8', marginTop: 1 }}>{sub}</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, height: 38, background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 10, padding: '0 12px', width: 230 }}>
            ...
            <a href={shopUrl} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 7, height: 38, background: '#0F172A', borderRadius: 10, padding: '0 14px', color: '#fff', fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>
